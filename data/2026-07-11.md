...
TL;DR: 本文提出了一个大规模、多范式的人本视频伪造数据集HumanForge，并利用多智能体流水线Gen2Anno进行自动化生成与细粒度标注，以应对现有数字取证基准的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有视频伪造基准主要关注换脸或全局文本生成视频，忽略了人-物或人-人交互以及多模态对齐等关键维度，无法应对视频扩散模型生成的高度逼真人本视频带来的数字取证挑战。

Method: 构建HumanForge数据集，并提出基于LangGraph的模块化多智能体流水线Gen2Anno。该流水线协调六个专门智能体进行源分析、基于MoE的参考分析和闭环取证验证，自动生成视频并生成包含二分类决策、细粒度伪造类别和时空定位的结构化标注。

Result: 生成了超过1.8万个高保真视频片段。使用最先进的传统检测器和大型多模态模型（LMMs）进行的广泛测试表明，现有模型在HumanForge数据集上的零样本泛化和细粒度推理面临重大挑战。

Conclusion: HumanForge为视频取证提供了包含复杂交互和多模态对齐的新基准，揭示了现有检测方法在零样本泛化与细粒度分析上的不足，相关代码与数据集将开源以促进未来研究。
...
TL;DR: 本文提出Switch-Reasoner框架，通过将思考视为虚拟工具调用并结合双层调节机制，使多模态大模型能自适应选择推理模式，在准确性与效率间取得更优平衡。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型通常采用固定的“先思考后回答”范式，在异构多任务场景中效率低下；同时，在后训练阶段，展开不平衡会导致模型行为不稳定，趋向于“总是思考”或“总是直接回答”。

Method: 提出基于GRPO的Switch-Reasoner框架，将思考作为虚拟工具调用，允许模型直接回答或调用显式推理。引入双层调节机制，平衡整体思考与直接模式的使用，并根据两种选择的相对收益提供样本级监督。

Result: 在11个多模态任务上的实验表明，Switch-Reasoner在保持强劲性能的同时减少了不必要的推理，实现了更好的准确率与效率权衡。

Conclusion: Switch-Reasoner有效解决了多模态大语言模型在异构任务中推理模式选择的稳定性和效率问题，验证了自适应思考模式切换的可行性与优势。
...
TL;DR: 本文提出了一种通用无人机视觉定位任务，并构建了支持多模态查询和多目标输出的基准数据集及基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有的指代表达理解基准和方法局限于纯文本查询和单目标输出，无法满足实际无人机场景中包含参考图像、多模态指令、无目标及多目标实例的需求。

Method: 构建了支持纯文本、纯图像和文本+图像查询的多模态基准UniRef-UAV，并提出了检测风格的基线模型UAV-URNet，通过将异构查询映射到共享查询空间并利用集预测来输出变长目标集。

Result: 实验表明，UAV-URNet比大型通用多模态大模型更轻量且可复现性更好，在无目标判别上表现更一致；多模态查询有效减少了视觉查询歧义并促进了更统一的查询-目标对齐空间。

Conclusion: 多模态查询与通用指代任务能显著提升无人机视觉定位的适用性，相关数据集和代码将开源以促进该领域的可复现研究。
...
TL;DR: 本文提出CoPAIR和TRACE两种前向计算方法，用于高效生成紧凑的top-k视觉证据掩码，以解决多模态大模型归因与修复问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于搜索的扰动归因方法通常输出所有区域的完整排序，而MLLM归因和修复等应用仅需紧凑的top-k证据掩码。本文旨在解决这一“掩码优先”的归因问题。

Method: 提出两种前向方法：CoPAIR利用PhaseWin-Greedy间隙诊断构建粗粒度候选集来热启动全排序搜索；TRACE通过交叉熵采样、精英保留和分布更新直接搜索固定基数细粒度掩码，并附带有限预算恢复分析。

Result: 在ImageNet分类中，该方法在全排序归因上达到新的最优前沿。在POPE和RePOPE任务中，TRACE+Greedy在MLLM归因中表现最佳，且直接TRACE掩码在RePOPE修复率达94.44%和96.00%。

Conclusion: 紧凑的证据掩码不仅是完整排序的前缀，更是可操作的归因输出，在视觉归因和模型修复中具有重要应用价值。
...
TL;DR: 本文提出了HoloCount基准，通过三级分层分类法评估多模态大语言模型在视觉计数任务中的感知、逻辑推理及鲁棒性能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视觉计数中存在数值幻觉和定量精度不足的问题，且现有基准仅关注简化情境下的基础感知，未能覆盖逻辑约束或对抗条件下的复杂失败模式。

Method: 提出HoloCount基准，采用三级分层分类法：语义计数（基于原子和属性的枚举）、分析计数（基于空间和集合的逻辑推理）以及鲁棒性测试（评估模型在密集场景和语言偏见等对抗条件下的完整性）。

Result: 对超过20种最先进的多模态大语言模型进行评估，发现即使是顶级模型，在从基础感知过渡到复杂分析推理及对抗场景时，性能也出现了显著下降。

Conclusion: 研究系统性地揭示了当前多模态大语言模型在计数能力上的现状与局限性，为开发更具基础性和可靠性的多模态系统提供了发展路线图。
...
TL;DR: 本文提出SpaR3D-MoE框架，通过自适应时空流形采样和几何归纳混合专家机制，仅从稀疏RGB输入实现高效3D空间推理。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在2D语义理解与3D空间几何之间存在表征鸿沟，现有3D感知模型要么依赖昂贵的3D专用数据，要么采用RGB-only输入配合启发式采样和浅层融合，分别导致时空连通性被破坏和跨模态竞争问题。

Method: 提出两个核心组件：(1)自适应时空流形采样机制，构建几何感知时空图提取关键帧，在减少序列冗余的同时保持场景拓扑连通性；(2)由指令-姿态感知路由器驱动的异构几何归纳混合专家，自适应地将多模态token路由至专用专家，解决单块融合中的跨模态竞争。

Result: 在VSI-Bench、ScanQA和SQA3D上取得SOTA性能；VSI-Bench平均得分63.5，超越最强基线7.8个绝对分；在Route Plan和Relative Direction任务上分别取得35.4%和51.4%的相对提升。

Conclusion: SpaR3D-MoE有效弥合了2D-3D表征鸿沟，证明了仅通过稀疏RGB输入即可实现高质量3D空间推理，为MLLM的空间感知能力提升提供了新范式。
...
TL;DR: 本文提出SparseCtrl-HOI框架，利用稀疏关键帧和多模态大语言模型提取的运动先验生成高质量的人与物体交互视频，有效解决了密集时间指导带来的高标注成本和低运动多样性问题。


<details>
  <summary>Details</summary>
Motivation: 现有人与物体交互视频生成方法依赖密集的时间指导（如逐帧姿态序列），导致标注成本高昂且限制了运动合成的多样性。

Method: 提出SparseCtrl-HOI框架，仅需少量关键帧控制生成过程；采用时间控制旋转位置嵌入在时间上锚定关键帧并保持空间完整性；引入运动先验注入模块，利用多模态大语言模型提取高级运动先验以控制中间帧动态；此外构建了SparseHOI-5K数据集。

Result: 综合评估表明，该方法在大幅降低标注开销的同时，能够合成出更优的直播电商交互视频。

Conclusion: 该方法通过稀疏时间控制有效克服了密集指导的局限性，为AI驱动的直播电商提供了高效且高质量的视频生成解决方案，代码和数据集已开源。
...
TL;DR: 本文提出了UniClawBench，一个在动态真实环境中评估主动型智能体的能力驱动型基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试依赖沙盒环境和单轮评估范式，且基于场景的任务分类混合了多种模型能力，难以有效评估智能体并定位失败的根源。

Method: 基于五大基础能力设计了400个双语真实任务，在实时Docker容器中使用细粒度步骤检查点进行评估，并采用包含执行者、隐藏监督者和用户智能体的闭环评估策略模拟真实多轮反馈，同时解耦基础模型能力与框架设计。

Result: 通过对多种最先进模型和智能体框架的综合评估，揭示了基础模型能力与框架设计如何共同影响智能体在真实世界环境中的性能。

Conclusion: UniClawBench能够有效解耦并评估模型与框架能力，其代码和数据已开源以促进未来智能体研究。
...
TL;DR: 提出WebSwarm框架，通过渐进式递归委派与多智能体协作，实现深度与广度兼顾的LLM网页搜索。


<details>
  <summary>Details</summary>
Motivation: 单智能体受限于长轨迹和上下文，难以兼顾搜索深度与覆盖广度；现有多智能体系统在递归深度、协作适应性和证据驱动的扩展方面仍有不足。

Method: WebSwarm动态实例化智能体搜索节点，每个节点耦合局部目标与搜索模式，可自行解决或递归委派子节点，结果与证据向上返回供父节点扩展或聚合；并先探测网页信息结构以指导扩展，跨同质兄弟节点复用过程级经验。

Result: 在BrowseComp-Plus、WideSearch、DeepWideSearch和GISA上，WebSwarm在深度、广度及深度广度交织任务上均持续优于单智能体和多智能体基线。

Conclusion: WebSwarm通过递归委派与协作机制有效提升了复杂搜索任务的深度与覆盖，消融与难度分析验证了其有效性，为多智能体搜索系统设计提供了启示。
...
TL;DR: 本文提出CoCommit方法，通过标记门控协调通道解决扩散语言模型中多token独立提交的因子分解误差问题，实现近似联合模式解码。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在每个去噪步骤中独立解码多个token，当位置间存在依赖关系时会产生因子分解误差（由条件总相关度量），而基于置信度的选择仅从边际分布无法观测该误差，导致生成质量受限。

Method: 提出CoCommit，一种标记门控协调通道：在常规束选择后，学习型标记器宣布提交集，重新应用骨干网络最后n层使标记位置协调（近似联合模式解码），再进行贪心argmax写入token。该方法复用现有权重，仅需一次额外部分前向传播，无需辅助模型。

Result: 在LLaDA2.1-mini上使用LoRA适配器和匹配的贪心推理，联合提交在全部六个基准上均提升准确率，其中推理和精确回答任务提升最大。

Conclusion: CoCommit以极低开销（一次部分前向传播，无额外模型）有效解决了dLLMs多token提交的依赖性问题，表明在扩散语言模型中引入轻量级协调机制可显著提升生成质量，尤其在需要推理精确性的任务上。
...
TL;DR: 本文提出了dOPSD方法，通过利用模型自身去噪轨迹中的后续解码步骤作为特权信息，改进了扩散大语言模型在同策略自蒸馏中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在后训练推理阶段面临挑战：监督微调存在暴露偏差且属于离策略，而强化学习仅提供稀疏奖励且难以应用。现有的同策略自蒸馏虽能提供密集监督，但依赖推理时不可用的外部真值作为特权信息，导致学生模型只能蒸馏出较弱的策略，对推理提升有限。

Method: 提出dOPSD方法，不依赖外部标签，而是直接从学生模型自身的去噪轨迹中获取教师模型的特权信息。具体而言，通过利用同一轨迹中后续、更彻底解码的步骤来评估掩码位置，使教师模型的优势直接从模型自身的解码过程中涌现。

Result: 在Dream和LLaDA模型上的实验表明，dOPSD有效提升了域内数学推理能力和域外代码生成能力，表现优于监督微调和同策略基线方法。

Conclusion: 通过利用模型自身解码过程中的内部信息作为特权，dOPSD有效突破了扩散大语言模型推理能力的提升瓶颈，为非自回归模型提供了一种无需外部标签的高效自我蒸馏范式。
...
TL;DR: 本文提出了一种无需训练的轨迹感知提交门控解码器（TACG），通过利用扩散语言模型的去噪轨迹来优化令牌提交时机，在保持准确率的同时减少去噪步数。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散语言模型解码器通常忽略去噪轨迹，仅基于当前快照提交令牌，这会将模型置信度与提交就绪状态混淆，导致在不完整上下文下的瞬态高置信度预测被错误锁定，而具有跨步一致支持的候选词被延迟。

Method: 提出轨迹感知提交门控（TACG），包含时间隐式对数几率引导（TILG）和历史门（HG）。TILG维护过去对数几率的指数移动平均作为自参考并在自然参数空间中与当前对数几率对比；HG在提交前强制短期提议持久性。结合受限的额外提升预算，形成无需辅助网络或额外前向传递的稳定约束提交规则。

Result: 在LLaDA、Dream和LLaDA2-Mini模型上的代码和数学基准测试中，TACG通常能提高或保持准确率，同时减少去噪步数并增加每次前向传递的令牌数（TPF）。

Conclusion: TACG通过有效利用去噪轨迹信息，成功解决了扩散语言模型解码中的过早提交问题，实现了更稳定高效的生成，且无需额外训练或计算开销。
...
TL;DR: 本文提出SLIM-RL方法，通过无轨迹随机掩码和风险控制机制，在不重建推理轨迹的情况下高效提升扩散大语言模型的强化学习性能。


<details>
  <summary>Details</summary>
Motivation: 现有最先进方法TraceRL通过重建推理轨迹来解决随机掩码与模型推理轨迹不匹配的问题，但其计算成本随块大小K增加而显著增长，本文旨在解决该高成本问题。

Method: 提出SLIM-RL方法，使用tau-budget解码器限制每步rollout的提交风险以减少训练数据中的总风险；并采用无轨迹的随机掩码目标进行训练，结合序列级重要性采样和确定性求积等方差减少工具。

Result: 在SDAR-4B上，SLIM-RL仅需0.46倍的训练样本即可匹配TraceRL在MATH500上的最佳准确率，在匹配的动态采样下MATH500和GSM8K分别提升6.32%和11.05%；在代码任务MBPP和HumanEval上分别提升4.20%和3.65%；4B模型性能超越8B和7B的同类扩散大模型。

Conclusion: SLIM-RL无需重建轨迹即可缓解掩码不匹配问题，在数学和代码任务上以更少训练成本实现了更优性能，且tau-budget解码器具有良好的跨模型泛化能力。
...
TL;DR: 本文揭示了扩散大语言模型解码方法评估中存在的不一致性问题，指出其对提示模板高度敏感，并提出了可靠的评估指南。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的高效解码研究存在评估结果不一致的问题，可能导致有偏见的结论，本文旨在探究该评估问题的原因。

Method: 在多种评估设置下对当前的dLLM解码方法进行严格评估，分析解码方法对提示模板及其他被忽视的评估设置的敏感性。

Result: 解码方法的排名对提示模板高度敏感；当前并行解码方法在速度与质量权衡上始终逊于单标记解码基线；有效提示模板带来的收益显著优于增加去噪步数。

Conclusion: 基于发现提出了针对dLLMs解码方法的可靠评估实用指南，强调评估需关注提示模板等细节设置。
...
TL;DR: 本文提出了DominoTree，一种无需训练的优先搜索草稿树方法，通过结合Domino的路径依赖条件分布，显著提升了大型语言模型推理的速度和接受长度。


<details>
  <summary>Details</summary>
Motivation: 传统的最优优先树方法（如DDTree）使用因式分解公式，无法表示Domino草稿器基于GRU因果校正产生的路径依赖非因式分解分布，限制了推测解码的效率。

Method: 提出DominoTree，一种无需训练的最优优先草稿树，沿着根到节点的路径利用Domino的条件非因式分解校正进行评分，并将每个节点的校正限制在候选的前M个。此外，使用GPU原生的CUDA图构建器来低成本且比特一致地构建树。

Result: 在Qwen3-4B和8B等模型上，DominoTree在所有测试温度下取得了最高的平均接受长度（最高达10.7 tokens/轮），相比自回归解码最高提速6.6倍，且在吞吐量上全面超越Domino解码器、DDTree和CaDDTree等方法。

Conclusion: DominoTree成功利用路径依赖分布特性，在推测解码中实现了卓越的速度和接受率，并配合高效的GPU原生构建器，证明了其在LLM推理加速中的优越性。
...
TL;DR: 提出CaDDTree方法，通过显式建模延迟并联合优化树结构与节点预算来最大化令牌吞吐量，解决了投机解码中草稿树预算选择的难题。


<details>
  <summary>Details</summary>
Motivation: 现有块扩散草稿树方法（如DDTree）仅最大化预期接受长度而忽略验证成本，由于接受长度随预算单调递增，导致缺乏合理选择节点预算的原则。

Method: 提出CaDDTree，显式建模草稿和验证延迟以直接优化令牌吞吐量；将目标分解为对预算的一维搜索，证明在凸验证成本下吞吐量函数为单峰，从而实现高效的贪婪停止规则，并支持每轮动态调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型的8个基准测试中，CaDDTree在几乎所有任务上均达到或超越了具有先知预算选择的DDTree的性能。

Conclusion: CaDDTree无需离线预算搜索即可自适应调整预算，有效提升了大语言模型的推理吞吐量。
...
TL;DR: 本文提出DDTree方法，通过从块扩散草稿模型的分布构建草稿树，克服了DFlash仅验证单条轨迹的限制，提升了推测解码性能。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散推测解码方法（DFlash）每轮仅验证单条草稿轨迹，限制了草稿的接受长度，未能充分发挥并行验证的潜力。

Method: 提出DDTree，利用块扩散草稿模型的逐位置分布构建草稿树；在固定节点预算下，采用最佳优先堆算法选择最可能匹配目标模型的续写路径，并使用仅祖先注意力掩码在单次前向传播中高效验证该树。

Result: DDTree在DFlash基础上有效扩展了验证范围，提升了接受长度，使其跻身推测解码领域的领先方法之列。

Conclusion: 将块扩散草稿生成与树状验证相结合，突破了单轨迹验证的瓶颈，为推测解码提供了更高效的加速方案。
...
TL;DR: 提出DELTA-TTS框架，通过轻量级LoRA将预训练自回归TTS模型转换为离散扩散语言模型，实现按置信度排序的语音解码，从而加速推理并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 自回归（AR）TTS模型按顺序生成语音令牌导致推理缓慢，且容易传播局部错误和幻觉。由于合成前已获取完整文本，从左到右的生成顺序并非TTS的固有需求。

Method: DELTA-TTS利用LoRA将预训练AR TTS模型转化为离散扩散语言模型（dLLM）进行置信度排序解码；同时引入卷积模块注入局部声学上下文，并结合1/t加权训练目标和时间偏移推理计划，将低置信度位置推迟至后期步骤。

Result: 在LibriTTS上仅训练585小时，DELTA-TTS在Seed-TTS test-en测试集上达到1.75%的WER，优于AR骨干模型且生成速度快3.3倍，同时实现了更清晰的文本-语音对齐、更高的解码置信度，并减轻了幻觉现象。

Conclusion: 将AR TTS模型转换为离散扩散模型不仅显著提升了推理速度和鲁棒性，还改善了生成质量，有效克服了自回归生成的固有局限性。
...
### [19] [Measuring Consumption with Credit Card Data: Benchmarking and Beyond](http://arxiv.org/abs/2607.08759v1)
*Aditya Aladangady,Ricardo Duque Gabriel,Carlo Wix*

Main category: econ.GN

TL;DR: 本文利用美联储Y-14M报告中的信用卡支出数据构建了一个新的县级月度消费数据集，并以此分析了不同收入水平地区对货币政策冲击的异质性消费反应。


<details>
  <summary>Details</summary>
Motivation: 传统的消费数据限制了细粒度（如县级月度）的经济分析，使得诸如不同收入分布对货币政策冲击的异质性反应等研究无法进行。

Method: 基于美联储Y-14M报告中超过3.5亿张信用卡的支出数据构建了一个覆盖2014年以来美国3000多个县的月度县级消费面板数据，并利用该面板估计了货币政策冲击下的异质性消费反应。

Result: 该数据能解释全国月度个人消费支出(PCE)增长的92%的变异，并捕捉到州级年度PCE增长的截面变异；实证发现低收入县在货币政策冲击下的支出下降幅度大于高收入县。

Conclusion: 新数据集有效近似了传统消费度量并使精细化经济分析成为可能，同时为研究人员使用类似信用卡数据提供了实用指导。
...
### [20] [Bifocal Diffusion Language Models: Asymmetric Bidirectional Context for Parallel Generation](http://arxiv.org/abs/2606.27732v1)
...
TL;DR: R2LM结合因果注意力和轻量级反向Mamba SSM，解决了离散扩散语言模型在生成质量和KV缓存推理吞吐量之间的架构困境。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型面临设计困境：双向注意力生成质量高但与KV缓存不兼容，限制了批量推理吞吐量；而因果注意力虽支持KV缓存但丢失右侧上下文，导致生成质量显著下降。

Method: 提出Bifocal dLLM范式并具体实现为R2LM，通过标准因果注意力提供左侧上下文并兼容KV缓存，同时引入轻量级反向Mamba SSM旁路提供压缩的右侧上下文。

Result: 在Qwen3-1.7B上的实验表明，R2LM的批量推理吞吐量比双向dLLM高2.4至12.9倍，比自回归基线快1.9至2.9倍，且在多数基准测试中质量超过因果基线，平均表现优于双向dLLM。

Conclusion: R2LM通过非对称双向上下文机制成功打破了dLLM中生成质量与推理吞吐量的权衡，实现了高效且高质量的并行解码。
...
### [21] [Music I Care About: Automated Multimodal Benchmarking of LLM Music Perception Skills on (Almost) Any Music](http://arxiv.org/abs/2607.06015v1)
*Tomáš Sourada,Katia Vendrame,Jan Hajič*

Main category: cs.SD

TL;DR: 本文提出MusICA-MetaBench框架，通过从用户数据自动生成跨模态基准测试，解决当前多模态大语言模型在音乐感知评估中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有音乐基准测试存在评估成本高且泛化性不明确、未能真正测量音乐感知能力，以及缺乏跨模态系统性能比较的问题。

Method: 提出一种自动从用户数据中按需生成基准测试的框架，利用结构化符号表示和预定义问题模板，构建跨音频、乐谱图像和符号文件的多项选择题以测试音乐感知能力。

Result: 通过ChoraleBricks数据集演示了该框架，确定了保证统计可靠性的基准测试规模，并通过与纯文本和白噪声基线对比证明了问题能有效测量音乐感知。

Conclusion: MusICA-MetaBench在MLLMs跨模态音乐感知评估方面取得显著进展，提出基于特定数据集的按需评估范式，实现了高效的音乐感知能力测试。
...
### [22] [GemNav: Discrete-Token Visual Robot Navigation using a Multimodal Large Language Model](http://arxiv.org/abs/2607.06882v1)
*Peter Bohm,Saimunur Rahman,Abdelwahed Khamis,Sagun Man Singh Shrestha,Chris McCool,Peyman Moghadam*

Main category: cs.RO

TL;DR: 本文提出GemNav，仅通过对冻结的多模态大语言模型的语言塔进行低秩自适应，无需额外视觉编码器和回归头，在极小数据集上实现了高效且可部署的机器人视觉导航。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉导航策略依赖复杂的架构（专用视觉编码器、定制动作头）和海量的训练数据（数千小时）。本文旨在质疑这种范式的必要性，并探索更高效、更简单的替代方案。

Method: 提出GemNav策略，利用低秩自适应（LoRA）仅对冻结的多模态大语言模型（MLLM）的语言塔进行微调。航点和分类导航信号共享单一离散词表，并通过软解码辅助损失恢复度量结构，摒弃了辅助视觉编码器和连续回归头。

Result: 在仅8.7小时的训练数据上（比现有数据集小约三个数量级），该策略在四个未见过的真实物理环境（20次试验）中实现零样本迁移，停靠目标点误差在0.25-0.42米内。此外，引入图像历史记录虽能提高离线指标，但未提升实际机器人导航效果。

Conclusion: 对冻结的MLLM进行离散词元自适应为基础模型机器人导航提供了一种数据高效、可部署的替代方案，同时也表明在预训练视觉特征存在时，时序上下文的增益存在上限。
...
### [23] [UniLM-Nav: A Unified Framework for Zero-Shot Last-Mile Navigation](http://arxiv.org/abs/2607.06537v1)
*Zhuofan Zhang,Tianxu Wang,Guoxi Zhang,Yixiong Lin,Xilin Wang,Hongming Xu,Qing Li,Song-Chun Zhu,Lifeng Fan*

Main category: cs.RO

TL;DR: 本文提出UniLM-Nav框架，利用多模态大语言模型解决零样本开放词汇移动操作中的最后一英里导航问题。


<details>
  <summary>Details</summary>
Motivation: 现有最后一英里导航方法依赖人工姿态标注或特定任务训练，无法扩展至具有细粒度空间约束的开放词汇环境，且到达目标附近无法保证基座处于可操作姿态。

Method: UniLM-Nav利用共享的多模态大语言模型后端，将导航分解为视角选择、任务条件化的可供性定位以及几何感知的基座姿态推理。

Result: 在OVMM基准上比SOTA方法MoTo提升3.13个百分点，组件消融和MLLM选择对性能影响显著，并在真实四足机器人与机械臂平台上成功验证。

Conclusion: UniLM-Nav有效提升了开放词汇移动操作的导航与定位性能，并在仿真和真实世界中均验证了其有效性和实际应用价值。
...
### [24] [Remember When It Matters: Proactive Memory Agent for Long-Horizon Agents](http://arxiv.org/abs/2607.08716v1)
*Yifan Wu,Lizhu Zhang,Yuhang Zhou,Mingyi Wang,Bo Peng,Serena Li,Xiangjun Fan,Zhuokai Zhao*

Main category: cs.AI

TL;DR: 本文提出一种与动作代理并行的独立记忆代理，通过主动选择性地注入记忆提醒来解决长视野任务中的“行为状态衰减”问题。


<details>
  <summary>Details</summary>
Motivation: 在长视野任务中，决策相关的状态信息常分散在长轨迹中并可能超出上下文窗口，导致关键信息无法在决策时被有效利用。本文将此定义为“行为状态衰减”并致力于解决该问题。

Method: 提出一种即插即用的独立记忆代理机制。该代理从近期轨迹中更新结构化记忆库，并主动决定是否向未修改的动作代理注入基于记忆的提醒或保持沉默，以此实现选择性干预。

Result: 在Terminal-Bench 2.0和τ²-Bench上分别将pass@1提升了8.3和6.8个百分点。消融实验证明选择性干预优于被动检索等基线方法；此外，使用SFT和GRPO训练的开源Qwen3.5-27B模型提升了验证奖励并实现了部分迁移。

Conclusion: 将记忆视为主动干预机制而非被动检索能有效缓解行为状态衰减，显著提升前沿代理在长视野任务中的表现；同时训练开源权重的记忆策略验证了该方法的可行性并指出了未来的发展方向。
...
### [25] [SolarChain-Eval: A Physics-Constrained Benchmark for Trustworthy Economic Agents in Decentralized Energy Markets](http://arxiv.org/abs/2607.08681v1)
*Shilin Ou,Yifan Xu,Luyao Zhang*

Main category: cs.AI

TL;DR: 本文提出了SolarChain-Eval，一个基于物理约束的可信经济智能体评估基准，用于评估去中心化能源市场中代理AI系统的任务表现与可信赖性。


<details>
  <summary>Details</summary>
Motivation: 随着代理AI系统日益应用于信息物理环境，现有评估方法无法同时兼顾任务性能与可信赖性。在去中心化能源市场中，自主代理虽可提升市场效用，但也可能利用无效物理数据、制造人为流动性并产生不稳定的治理决策，因此亟需一个多维度的可信评估框架。

Method: SolarChain-Eval将市场治理建模为兼容Gymnasium的马尔可夫决策过程，代理每小时做出决策，并从市场效用、物理安全、滑点、动作平滑性、空间公平性和可审计性等多维度评估策略。系统集成了基于LLM的规划器/审计器层：规划器定义回合级行动边界和审计规则，审计器审查并修订高风险动作，所有干预通过结构化日志记录。

Result: 对静态、随机、近视、RL和RL+LLM策略的实验揭示了明确的效用-安全权衡。RL代理提升了市场效用但仍可能产生不安全行为；移除物理惩罚后，奖励最大化代理会利用无效发电数据并增加人为流动性。LLM规划器/审计器提升了可审计性并缓解了部分风险，但无法完全弥补奖励函数设定不当的问题。

Conclusion: 可信代理AI评估需要同时具备物理约束和透明的干预追踪。仅靠LLM审计层无法弥补奖励函数设计缺陷，未来需要更完善的物理约束机制和干预可追溯性来保障代理系统的可信性。数据和代码已在GitHub上开源。
...
TL;DR: 本文研究了在多智能体市场中维持稳定并抵御对抗性攻击的机制，发现调解机制具有最强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自私的智能体在重复社会困境中倾向于背叛导致合作崩溃，本文旨在探讨何种正式机制能维持多智能体市场稳定及其抵御对抗攻击的能力。

Method: 构建了包含18个DeepSeek-V3智能体的多智能体市场仿真环境，在受限社交网络中交易。实验分两阶段：比较8种机制在注入恶意智能体下的表现，并对表现最佳的调解机制进行迭代提示优化的红队对抗测试。

Result: 调解机制在机制对比中表现最佳；在最强对抗攻击下，诚实智能体效用降低13.3%但市场未崩溃，调解机制支持效用恢复。

Conclusion: 调解机制具备对抗鲁棒性，能在持续优化的攻击压力下维持正向的诚实智能体效用，即“可弯曲但不可折断”。
...
TL;DR: 提出TAPS方法，通过目标感知的前缀选择优化扩散模型草稿树的验证过程，显著提升无损解码速度。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散树方法在投机解码中使用边缘概率排序节点，忽略了验证过程的前缀条件依赖性，导致验证被拒绝前缀的不可达后代，增加了延迟且收益有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边缘概率转化为路径条件接受估计，并在固定的验证预算下选择紧凑的前缀闭子树，优化接受率与成本的权衡。

Result: 在多个数据集和模型上的实验表明，TAPS相比自回归解码实现了高达7.9倍的无损端到端加速，分别比DFlash和DDTree快1.36倍和1.74倍。

Conclusion: TAPS有效解决了扩散模型投机解码中的验证瓶颈，通过构建路径条件估计和紧凑子树，在不增加过多目标模型延迟的前提下最大化了接受长度和整体解码效率。
...
TL;DR: 本文提出了Sangam，一个针对扩散语言模型缓存推理的服务系统，通过赤字令牌预算调度器和混合服务策略优化预填充与解码的调度，降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型的双向注意力阻碍了精确KV缓存，导致近似缓存引发重复的预填充/解码结构，使现有自回归服务机制无法直接应用，面临块级解码和资源划分的挑战。

Method: 提出赤字令牌预算调度器优先处理解码，并在预算允许时接纳完整预填充以实现无停顿调度；采用混合服务策略，将溢出的预填充转移到解码节点，同时保护解码免受干扰。

Result: 解码密集型负载下，协同定位服务比混合执行平均延迟降低9-20%；预填充密集型负载下，混合执行比协同定位服务平均延迟降低8-20%。

Conclusion: dLLM服务设计同样受预填充-解码干扰和资源划分支配，需根据负载特性选择协同定位或混合执行策略以优化延迟。
...
TL;DR: 本文利用图强化学习解决比特币闪电网络中固定预算下的流动性放置问题，以最大化路由容量，并已成功部署于生产环境。


<details>
  <summary>Details</summary>
Motivation: 旨在解决在固定预算下，比特币闪电网络节点应开启哪些通道以最大化其路由容量的问题，从而提升网络整体的路由效率。

Method: 将问题建模为预算约束下的组合优化问题，通过选择添加k条边以最大化s-t最大流；采用图强化学习求解，提出结合消息传递策略网络、近端策略优化（PPO）和动作掩码的轻量级智能体，并使用中心节点排除课程进行训练，迫使模型学习容量感知策略而非单纯依赖中心节点。

Result: 在真实闪电网络快照上的实验表明，该方法在多个随机种子和未见过的图上，于最大流目标上始终优于强启发式基线。该智能体已在生产环境中执行了4640次通道开启决策，累计在30个节点上分配了267.3 BTC（超1600万美元）。

Conclusion: 基于图强化学习的流动性放置方法能有效解决闪电网络的路由容量优化问题，并在实际大规模生产部署中验证了其有效性与实用价值。
...
TL;DR: 本文提出一种跨多环境的分层机器教学算法，结合多模态反馈学习对环境动态具有鲁棒性的奖励函数，显著提升了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有逆强化学习的最优教学方法局限于单一环境和演示反馈，导致奖励函数与特定环境结构耦合，跨环境泛化能力差。本文旨在解决异构反馈和多环境动态下奖励函数的联合约束与泛化问题。

Method: 理论上分析了不同反馈模态对奖励的约束作用（无限数据下比较模态全局约束最强），并提出一种跨多MDP的分层机器教学算法：先贪婪选择提供互补奖励约束的信息性环境，再在其中策略性查询低成本反馈。

Result: 在相同反馈预算下，相较于均匀教学基线，该方法实现了显著更低的遗憾值，并在未见环境中展现出更强的泛化能力。

Conclusion: 多环境与多模态教学是学习对动态变化鲁棒的奖励函数的关键，能有效克服单一环境教学导致的过拟合问题。
...
TL;DR: 本文提出了HERALD，一种针对块扩散式大语言模型的KV缓存卸载系统，通过减少选择计算并与去噪过程重叠，在低KV预算下实现了近乎无损的精度并显著提升了长上下文推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型的KV缓存随上下文线性增长，导致长上下文推理时内存压力过大且吞吐量受限。虽然将KV缓存卸载至主机DRAM可缓解内存压力，但受限于PCIe带宽，需稀疏召回KV条目；而在块dLLM中为了重用KV条目进行全量精确注意力的选择成本过高。

Method: 提出HERALD系统，利用块扩散模型中去噪步骤间相关KV条目的一致性，采用两种机制将所需的选择计算量降低至原计算量的若干分之一（按块大小缩减），并使选择过程与去噪计算重叠执行。

Result: 在3个块扩散模型和5个长上下文任务上，HERALD在5-10%的KV预算下实现了近乎无损的精度，且相比纯GPU推理，每块延迟最高降低1.59倍，吞吐量提升2.47倍，加速比随上下文长度增加而扩大。

Conclusion: HERALD成功克服了块扩散式大语言模型KV缓存卸载的选择计算与带宽瓶颈，在极低KV预算下保证了模型精度，并大幅提升了长上下文推理的整体性能。
...
TL;DR: 本文提出了Diffusion-Proof框架，首次将扩散大语言模型应用于形式定理证明，通过整证明生成与局部修正模型克服了自回归模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型在形式定理证明中存在长程连贯性差和错误累积的问题，而扩散大语言模型在此关键领域的应用仍缺乏研究。

Method: 提出包含两个模型的Diffusion-Proof框架：dLLM-Prover-7B用于具有长程连贯性的整证明生成，dLLM-Corrector-7B利用双向信息进行大块扩散局部证明修正。

Result: 在相同数据集下，相比自回归基线，该方法在ProofNet-Test和MiniF2F-Test上分别取得1.61%和6.14%的绝对提升，并成功解决了一个更先进的DeepSeek-Prover-V2-7B未能解决的IMO问题。

Conclusion: 扩散大语言模型在形式定理证明中具有独特优势，能够有效提升形式数学推理能力并克服自回归模型的固有局限。
...
### [33] [POPS: Recovering Unlearned Multi-Modality Knowledge in MLLMs with Prompt-Optimized Parameter Shaking](http://arxiv.org/abs/2607.06649v1)
*Zhangheng LI,Jianing Zhu,Junyuan Hong,Sungmin Eum,Shuowen Hu,Suya You,Zhangyang Wang*

Main category: cs.CR

TL;DR: 本文提出了一种名为POPS的对抗性攻击策略，能够成功恢复多模态大语言模型中被机器遗忘的隐私信息。


<details>
  <summary>Details</summary>
Motivation: 多模态机器遗忘（MMU）方法旨在消除多模态大语言模型中的隐私信息，但其在面对恶意用户时的鲁棒性尚未得到充分验证。

Method: 提出提示优化参数抖动（POPS）对抗策略，通过提示后缀优化引导受害模型生成潜在隐私样本，并利用这些合成输出微调模型，从而诱使其恢复真实的隐私信息。

Result: 在多个MMU基准上的实验表明，POPS能够近乎完全地恢复被遗忘的敏感信息，揭示了现有MMU算法的严重缺陷。

Conclusion: 现有的基于MMU的隐私保护机制存在根本性漏洞，其鲁棒性面临严峻挑战。
...
### [34] [ProjAgent: Procedural Similarity Retrieval for Repository-Level Code Generation](http://arxiv.org/abs/2607.08691v1)
*QiHong Chen,Aaron Imani,Iftekhar Ahmed*

Main category: cs.SE

TL;DR: ProjAgent提出利用过程相似性作为检索信号并辅以静态分析反馈，有效提升了仓库级代码生成的性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索方法依赖词法、结构或语义相似性，难以捕捉逻辑相似但标识符不同的函数，无法有效处理复杂的跨文件依赖。

Method: 将目标函数分解为推理步骤，通过智能体工作流逐步检索具有相似过程行为的函数，并结合语义检索构建上下文；同时利用静态分析反馈循环迭代修复生成代码。

Result: 在REPOCOD基准上达到41.14%的Pass@1，超越了现有基于检索的基线方法。

Conclusion: 过程相似性是仓库级代码生成中一种有效且未被充分探索的检索维度。
