...
TL;DR: InterleaveThinker是一个多智能体流水线，通过规划智能体和基于强化学习的评论智能体，赋予现有图像生成器交错图文序列生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成器受限于架构无法实现交错图文序列生成，而该功能对视觉叙事和具身操作至关重要，且当前开源统一多模态模型在此方面表现有限。

Method: 提出多智能体架构：规划智能体组织输入序列并指导生成器；评论智能体评估输出并修正指令以重新生成。构建SFT数据集进行格式冷启动，并引入准确度奖励和逐步奖励，通过单步强化学习（GRPO）优化评论智能体的指令纠错能力，以克服长轨迹计算不可行问题。

Result: 提升了多种图像生成器的交错生成性能，在相关基准上达到与Nano Banana和GPT-5相当的水平；同时在推理基准（如WISE和RISE）上显著增强了基础模型（如FLUX.2-klein）的表现。

Conclusion: 多智能体流水线能有效为现有图像生成器赋予交错生成能力，且单步强化学习优化不仅解决了长轨迹计算难题，还意外带来了基础模型推理能力的显著提升。
...
TL;DR: 提出SpatialClaw框架，通过有状态的代码交互接口增强视觉语言模型在3D/4D空间推理中的灵活性，在20个基准测试上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的空间推理能力受限，而工具增强型代理受限于动作接口设计：单次代码执行缺乏中间观察反馈，结构化工具调用灵活性不足，两者均无法有效应对开放且复杂的3D/4D空间推理任务。

Method: 提出免训练框架SpatialClaw，采用代码作为动作接口。该框架维护一个预加载输入帧及感知与几何原语的有状态Python内核，允许VLM代理基于先验输出逐步编写可执行代码单元格，实现感知结果的灵活组合与操作，并根据中间观察和任务需求自适应调整分析过程。

Result: 在20项涵盖静态与动态3D/4D空间推理的基准测试中，平均准确率达59.9%，较近期空间代理提升11.2个百分点；且在两个模型家族的六个VLM骨干网络上均取得一致提升，无需针对特定基准或模型进行适配。

Conclusion: 基于有状态代码的动作接口设计能显著增强VLM代理在复杂空间推理中的灵活性与性能，且无需额外训练即可实现优秀的跨模型与跨任务泛化。
...
TL;DR: 本文提出Reroute，一种免训练的视觉令牌可恢复路由插件，替代传统的不可逆剪枝，在不增加计算与内存开销的前提下提升视觉语言模型在激进令牌缩减下的定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的视觉令牌缩减方法采用“排序并移除”范式，永久丢弃低分令牌。然而，视觉令牌的重要性随解码器深度变化，早期被判定为不重要的令牌可能在后续层变得相关，这种不可逆的丢弃机制对涉及定位的查询尤为脆弱。

Method: 提出Reroute，一种免训练的插件，将不可逆移除替换为可恢复路由。在路由阶段，选中的视觉令牌通过解码器块，而延迟令牌绕过当前阶段并在下一次路由决策时重新进入候选池；该机制复用现有的注意力分数排序规则和阶段调度，保持了原剪枝方法的TFLOPs和KV-cache预算。

Result: 在LLaVA-1.5和Qwen骨干网络的FastV、PDrop和Nüwa变体上，Reroute在激进的令牌缩减下提升了定位能力，同时维持了常规视觉问答（VQA）性能。

Conclusion: 视觉语言模型的令牌缩减不应仅被视为不可逆的剪枝，而应被视为可恢复的路由过程，这为VLM的高效推理提供了新视角。
...
TL;DR: 本文提出一种基于多模态大语言模型和特征融合的统一法医图像检索框架，显著提升了纹身和人脸草图等复杂场景下的检索精度与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注多模态检索系统的算法优化，缺乏对多样化真实法医场景适用性的评估，旨在解决传统法医分析过度依赖人工以及在视觉信息受限或含噪时检索困难的问题。

Method: 提出一种统一检索框架，利用多模态大语言模型（MLLM）为查询和图库图像自动生成结构化文本描述，通过句子转换器进行文本嵌入，并结合最先进的视觉特征提取器，采用多模态融合策略（结合文本与图像相似度得分）进行检索比对。

Result: 多模态融合策略在四项法医任务中均持续提升了检索精度和鲁棒性，尤其在视觉信息有限或含噪的场景（如草图、局部纹身、碎片化证词）下改进显著。

Conclusion: 统一的多模态检索流程具有重要的法医应用价值，现代MLLM能有效将传统依赖人工的法医任务自动化，多模态检索是支持纹身、面部合成及目击者描述等调查工作流的有力工具。
...
TL;DR: 本文提出Q-Fold，一种无需训练的长视频理解输入构建框架，通过查询引导的异构焦点-上下文表示，在有限视觉预算内同时保留高保真视觉证据与广泛时间覆盖。


<details>
  <summary>Details</summary>
Motivation: 长视频帧数众多导致处理成本高昂，现有方法多遵循以帧为中心的范式且对保留内容同等对待，忽略了内容重要性差异，难以同时兼顾高保真视觉证据与广泛的时间覆盖。

Method: 提出Q-Fold框架，以连续时间片段而非孤立帧为基本建模单元，在查询引导下构建异构焦点-上下文表示：将与查询相关的片段保留为高保真焦点帧，将不相关片段折叠为保持时间顺序的上下文布局。

Result: 在四个长视频基准测试和多个Video-MLLM上的实验表明，Q-Fold在不增加输入预算的情况下持续提升模型性能，在超长视频基准上最高提升达9.1个百分点。

Conclusion: Q-Fold有效解决了长视频理解中视觉保真度与时间覆盖范围的权衡问题，在不增加计算预算的前提下显著提升了模型性能，代码将后续开源。
...
TL;DR: 本文提出了TASM，一种免训练的任务感知结构化记忆框架，用于解决多模态大语言模型上下文学习的可扩展性问题，在重度压缩下有效平衡了效率与适应性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型依赖上下文学习，但受限于有限的上下文窗口和长序列中KV缓存的高昂成本；现有记忆压缩方法依赖僵化的标记删除或样本依赖的重要性估计，会导致偏差、破坏视觉语义结构，且生成的静态记忆无法适应新查询。

Method: 提出免训练框架TASM：1) 采用任务向量引导压缩，以任务级方向替代样本特定信号；2) 通过二分图匹配进行语义感知的标记合并，避免破坏性剪枝以保留底层流形；3) 构建包含紧凑核心记忆和潜在库的层次结构，实现查询自适应动态检索。

Result: 实验评估表明，TASM在重度压缩条件下仍能维持高性能。

Conclusion: TASM有效平衡了多模态上下文学习中的效率与适应性，克服了现有压缩方法的偏差和语义结构破坏问题。
...
TL;DR: 本文提出了EvoArena动态环境基准和EvoMem基于补丁的记忆范式，以评估和提升大语言模型智能体在动态变化环境中的持续对齐与推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型智能体评估多基于静态环境，而现实部署具有内在动态性，要求智能体不断调整知识、技能和行为以适应环境和任务条件的变化，目前缺乏对此动态演进的评估与方法。

Method: 提出EvoArena基准，将环境变化建模为跨终端、软件和社会领域的序列渐进更新；提出EvoMem，一种基于补丁的记忆范式，将记忆演变记录为结构化更新历史，使智能体能通过记忆变化推理环境演进。

Result: 当前智能体在EvoArena上表现不佳（平均准确率39.6%）。应用EvoMem后，在EvoArena上平均提升1.5%，在GAIA和LoCoMo标准基准上分别提升6.1%和4.8%，链级任务准确率提升3.7%。机制分析表明EvoMem改善了记忆中的证据捕获。

Conclusion: 在智能体评估和记忆机制中对环境演进进行建模对于可靠的智能体部署至关重要，EvoMem通过保留完整的演进环境状态有效提升了智能体的动态适应能力。
...
TL;DR: 本文提出HyperTool接口，通过将局部确定性的工具子程序折叠为单一外部调用，解决了工具增强LLM中的执行粒度不匹配问题，显著提升了多步工具使用的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的工具增强LLM依赖逐步的原子工具调用，将低层数据流暴露在推理轨迹中，导致“执行粒度不匹配”，消耗上下文并迫使模型管理低层数据流而非专注高层推理。

Method: 提出HyperTool，一种统一的可执行MCP风格工具接口；模型通过包含代码块的单一外部调用来执行现有工具、操作返回值及本地传递中间结果，从而折叠确定性子程序；并利用跨工具组合任务合成轨迹进行训练与验证。

Result: 在MCP-Universe基准上，HyperTool将Qwen3-32B的平均准确率从15.69%提升至35.29%，Qwen3-8B从9.93%提升至33.33%，且平均准确率超越了GPT-OSS和Kimi-k2.5。

Conclusion: HyperTool通过改变模型可见的工具执行单元，有效解决了执行粒度不匹配问题，大幅提升了LLM的多步工具使用能力，为工具增强智能体提供了更高效的接口范式。
...
TL;DR: 本文提出递归智能体线束（RAH），将递归单元从模型调用扩展为配备工具的完整智能体，显著提升了长上下文推理性能。


<details>
  <summary>Details</summary>
Motivation: 为了统一递归语言模型（模型调用级递归）与生产级编程智能体（子智能体派生）这两种范式，解决如何在具备文件系统、代码执行和规划能力的完整智能体层面实现递归的问题。

Method: 提出代码优先的线束递归机制（RAH），父智能体生成并执行脚本以并行派生子智能体线束处理细粒度任务，并使用结构化函数调用处理微小子任务。

Result: 在Oolong-Synthetic长上下文数据集上，固定GPT-5主干网络时，RAH将Codex基线从71.75%提升至81.36%（提升归因于线束架构而非模型）；使用Claude Sonnet 4.5时性能达到89.77%。

Conclusion: 以完整智能体线束为递归单元的RAH架构能有效提升长上下文推理性能，且性能增益源自线束设计而非底层模型，验证了智能体级递归范式的有效性。
...
TL;DR: 本文提出了AGDO框架，利用注意力引导的去噪与优化机制解决扩散大语言模型后训练中随机掩码忽略词元依赖的问题，显著提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散大语言模型后训练方法多采用随机掩码策略，忽略了词元间的内在依赖关系，导致生成稳定性差并限制了推理能力。

Method: 提出注意力引导的去噪与优化框架（AGDO），基于注意力结构决定去噪顺序，并在监督微调和强化学习中强调对注意力至关重要的词元，使训练和优化与注意力衍生的依赖关系对齐。

Result: 在数学和编程基准测试中，AGDO持续提升了推理性能，表现优于现有最先进的dLLMs后训练方法。

Conclusion: 对未掩码上下文具有更强注意力的词元在推理中更稳定且起关键作用；将注意力机制融入去噪和优化过程能有效提升dLLMs的推理能力，为模型后训练提供了有效范式。
...
TL;DR: 提出ADAS，一种免训练的重排序规则，通过软注意力折扣解决掩码扩散语言模型并行解码中词元预测耦合的问题，显著提升低NFE性能。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型在并行解码时同时揭示多个词元易导致预测耦合的不安全；现有免训练采样器仅控制揭示词元数量并依赖逐词元评分，忽略了所选集合内词元间的交互作用。

Method: 提出ADAS（免训练重排序规则），保持基础采样器停止规则不变，仅修改子集构建方式：当候选词元强烈关注已选但预测仍不确定的位置时，贪心地对其进行折扣降权，将注意力作为软边际惩罚而非硬兼容性约束。

Result: 在LLaDA-8B和Dream-7B模型及GSM8K等四个基准上，将ADAS嵌入Top-k等采样器，在相同去噪评估次数下低NFE性能平均分别提升9.11和10.46个百分点，每次前向传播运行时间开销仅3.1%。

Conclusion: 软注意力折扣重排序是一种简单、模块化的方法，能有效提升掩码扩散语言模型高度并行解码的质量。
...
TL;DR: 提出Prefilling-dLLM框架，通过分块缓存KV表示与稀疏选择机制，免训练地解决了扩散大模型长上下文推理的二次方计算瓶颈并实现显著加速。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在每个去噪步骤中需重新编码整个前缀，导致计算量随上下文长度呈二次方增长，使其在长上下文场景中计算成本过高且难以应用。

Method: 提出免训练的预填充-解码解耦框架，将前缀划分为N个块并一次性缓存其KV表示，解码时利用块内token稀疏性选择Top-K最相关块；设计了在非连续缓存块KV上并行化解码的注意力内核；并在每个块前添加序列起始（BOS）token作为周期性注意力锚点。

Result: 将每步计算复杂度从全序列长度二次方降至仅与解码长度二次方相关；稀疏预填充效果优于密集注意力；在LongBench和InfiniteBench上达到dLLM加速方法的SOTA质量；注意力内核在8K-32K上下文中实现9.1-28.0倍加速；消除了“中间丢失”现象。

Conclusion: 基于分块缓存与稀疏选择的预填充-解码解耦策略能有效突破dLLM的长上下文计算瓶颈，且周期性注意力锚点可增强模型对上下文中间信息的检索能力，为dLLM的高效长上下文推理提供了有效范式。
...
TL;DR: 本文提出一种统一能量函数（Uni-E），以解决扩散语言模型在并行生成中因无法充分捕捉词元关系而落后于自回归模型的性能差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型在高度并行解码时未能充分捕捉词元间关系，导致与自回归模型存在性能差距，本文旨在系统分析并弥补该差距。

Method: 识别出导致差距的模型容量、依赖性和不变性三个关键因素，提出不变能量（Inv-E）及其采样估计器，并与独立能量（Ind-E）结合构建统一能量（Uni-E）；Uni-E无需采样估计配分函数即可精确计算，且与模型无关。

Result: 理论证明Uni-E可纠正由依赖性和不变性引起的分布偏移，在扩散语言模型（DLMs）和扩散大语言模型上的大量实验均验证了其有效性。

Conclusion: Uni-E有效解决了扩散语言模型的关键制约因素，纠正了分布偏移，具备向任意规模模型扩展的能力，显著缩小了与自回归模型的性能差距。
...
TL;DR: 本文提出PAPO框架，通过步骤感知过程奖励和熵引导历史重演解决扩散大语言模型强化学习中的双重不对齐问题，显著提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)的强化学习受限于生成轨迹与梯度更新过程的双重不对齐：一是过程-奖励不对齐，即稀疏终端奖励被无差别分配给中间步骤，无法提供判别性信用分配；二是状态-轨迹不对齐，即策略更新常偏向人工的离轨迹状态，导致梯度浪费。

Method: 提出过程对齐策略优化(PAPO)框架，包含两个核心组件：步骤感知过程奖励(SPR)，将稀疏终端奖励转化为密集的逐步信用分配；熵引导历史重演(EHR)，在高不确定性步骤重演真实轨迹以避免状态偏移。

Result: 在四个基准测试上，PAPO显著优于基线方法，在GSM8K上提升4.5%，MATH500上提升4.8%，Countdown上提升42.2%，Sudoku上提升16.1%。

Conclusion: PAPO框架有效对齐了强化学习更新与dLLM的生成轨迹，解决了双重不对齐问题，大幅提升了模型的推理性能，为dLLMs的强化学习优化提供了有效范式。
...
TL;DR: 本文提出SAID框架，通过优先计算语义骨架词元并减少细节词元去噪步数，大幅加速扩散大语言模型的推理过程。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（DLLMs）尽管支持非自回归并行生成，但高质量生成仍需大量去噪步骤，导致推理成本过高。

Method: 提出Scaffold-Aware Iterative Decoding (SAID)框架，通过重新分配计算资源来加速推理：优先对骨架词元去噪以建立粗粒度语义结构，再以较少步数补全可预测的细节词元；同时适配块级扩散解码，引入置信度分层生成（CHLG），仅为低置信度词元分配额外计算步数。

Result: 在LLaDA-8B和LLaDA 1.5模型的数学、编程和知识基准测试中，SAID实现了最高9.1倍的推理加速，同时保持了具有竞争力的生成性能。

Conclusion: 基于词元语义重要性和置信度的非均匀计算分配策略，能有效突破扩散大语言模型的推理速度瓶颈，在不显著牺牲生成质量的前提下实现大幅加速。
...
TL;DR: 提出动态填充锚点（DIA）方法，通过动态估计结束锚点位置调整扩散大语言模型的生成长度，解决固定锚点导致的推理截断与内容冗余问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在格式约束任务中使用固定锚点会导致生成范围僵硬，造成推理截断或内容冗余，难以同时保证结构正确性与语义连贯性。

Method: 提出免训练的动态填充锚点（DIA）方法，在迭代填充前动态估计结束锚点位置并调整生成长度，实现灵活的结构感知生成机制。

Result: 在推理基准测试中，DIA显著提升了格式合规率与答案准确率，在GSM8K和MATH数据集上取得了显著的零样本性能提升。

Conclusion: DIA为可靠的结构感知生成提供了稳健路径，证明了动态锚点机制在提升扩散大语言模型格式约束任务表现的有效性。
...
TL;DR: 本文提出CaDDTree，一种成本感知的扩散草稿树方法，通过联合优化树结构和节点预算直接最大化令牌吞吐量，无需离线预算搜索即可达到或超越具有先验预算选择的DDTree性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于块扩散的草稿树方法（如DDTree）在固定节点预算下最大化期望接受长度，但接受长度随预算非递减，忽略了验证成本，导致缺乏选择节点预算的理论依据。

Method: 提出CaDDTree方法，显式建模草稿和验证延迟，联合选择树结构与节点预算以直接优化令牌吞吐量；将吞吐量目标分解为逐轮的一维预算搜索，并在凸验证成本下证明吞吐量函数为单峰函数，从而实现高效的贪心停止规则，每轮自适应调整预算且无需离线搜索。

Result: 在Qwen3-4B和Qwen3-8B模型及涵盖推理、编程和指令跟随的八个基准测试中，CaDDTree在几乎所有任务上的表现均匹配或超越了使用先验最优预算的DDTree。

Conclusion: CaDDTree为基于扩散的推测解码提供了一种有理论依据的成本感知预算选择方案，消除了离线预算搜索的需求，并在实际应用中实现了更优或持平的推理吞吐量。
...
TL;DR: 本文提出DDTree方法，通过从块扩散起草器的位置分布构建起草树，解决了DFlash单轨迹验证限制接受长度的问题，达到领先推测解码性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash块扩散起草器每轮仅验证单条起草轨迹，限制了推测解码的接受长度。

Method: 提出DDTree，直接基于块扩散起草器的逐位置分布构建起草树；在固定节点预算下，使用最佳优先堆算法依据起草模型代理选择最优延续；利用祖先注意力掩码在目标模型单次前向传播中完成树验证。

Result: DDTree基于DFlash克服了单轨迹限制，成为当前领先的推测解码方法之一。

Conclusion: 将块扩散起草分布转化为树结构并结合高效掩码验证，可有效提升推测解码的接受长度与整体性能，确立DDTree的领先地位。
...
TL;DR: 本文提出了多语言多模态医疗问答数据集ArogyaBodha及基于演员-评论家机制的多智能体框架ArogyaSutra，以提升低资源多语言场景下的医疗推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在医疗等专业领域及多语言、低资源场景下表现受限，英语中心化模型难以处理如印度农村等地区本地语言的复杂多模态医疗查询，阻碍了AI医疗的公平获取。

Method: 构建了涵盖8种异构数据源、31个身体系统、6种成像模态、21个临床领域及8种语言（英语及7种印度语言）的大规模数据集ArogyaBodha；提出基于演员-评论家的多智能体框架ArogyaSutra，融合工具调用基座与双记忆机制进行逐步推理感知决策，并利用模拟轨迹进行知识蒸馏。

Result: 实验表明，该数据集与框架提升了所有印度语言的医疗推理准确率，消融实验验证了各组件的贡献。

Conclusion: 提出的数据集和框架有效填补了多语言低资源医疗多模态推理的空白，代码与数据已开源供未来研究使用。
...
TL;DR: 本文提出了PaperGuard，这是首个用于系统评估和防御AI同行评审中跨模态对抗攻击的综合基准，并证实了当前AI审稿系统普遍存在的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 本文旨在解决LLM/MLLM在学术同行评审中面临的跨模态对抗操纵风险，特别是当前鲁棒性研究仅限纯文本，且针对特定领域定向失败（如操纵评分）的攻击尚无有效防御的问题。

Method: 提出PaperGuard基准，包含三个核心部分：1) 跨学科多模态同行评审数据集；2) 针对文本(GCG)和图像(PGD)的黑盒提示注入与白盒扰动统一攻击套件；3) 基于分块嵌入搜索的防御机制，以高效定位并缓解长文本中的有害指令。

Result: 针对最先进模型的广泛实验表明，AI审稿系统对上述跨模态定向攻击普遍存在脆弱性。

Conclusion: PaperGuard为构建可信赖、具抗攻击性的AI辅助学术评审确立了基础基准、评估协议及切实可行的防御机制。
...
TL;DR: 本文提出“移动的持续音”系统，将印度斯坦尼音乐的静态持续音转化为动态的协同创作代理，并利用低保真生成式AI促进人机音乐互动。


<details>
  <summary>Details</summary>
Motivation: 旨在解决当前音乐AI追求高保真度引发的职业替代焦虑，打破印度斯坦尼音乐中持续音的静态传统，探索技术与生成式AI在既有社会文化音乐实践中的定位。

Method: 在Max/MSP中使用四个独立循环器作为“虚拟持续音”，与人声即兴形成实时反馈循环；通过音高偏移引入旋律变化；集成GaMaDHaNi生成式AI模型进行音色重合成，并刻意采用低保真输出。

Result: 构建了人声与虚拟持续音之间有机且不断演变的反馈循环，使虚拟持续音从被动反应转变为主动的协同创作代理。

Conclusion: 将生成式AI定位为社会文化音乐实践中主动、响应且协同创作的音乐代理；低保真AI输出的使用强调了人类解释与情境语境的必要性，提供了非替代性的人机协作范式。
...
### [22] [Re-evaluating Confidence Remasking in Masked Diffusion Language Models](http://arxiv.org/abs/2606.12232v1)
*Stipe Frkovic,Metod Jazbec,Dan Zhang,Christian A. Naesseth,Ilija Bogunovic,Eric Nalisnick*

Main category: cs.LG

TL;DR: 本文重新评估了掩码扩散语言模型中基于置信度的事后重掩码方法，发现其收益高度依赖设置，标准解码下无显著优势，非贪婪解码下会加剧多样性崩溃。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型解掩码后无法修改词元，易受早期采样错误影响，现有事后重掩码方法（如WINO）声称能无训练地解决此问题，但需重新审视其实际有效性。

Method: 在标准解码设置（较短块长度）和非贪婪解码设置下，对代表性的无训练事后重掩码方法WINO进行实证重新评估。

Result: 标准解码下，WINO相比单纯基于置信度的解掩码无明显收益；非贪婪解码下，重掩码虽能缓解随机性引入的错误，却加剧了多样性崩溃问题。

Conclusion: 基于置信度的事后重掩码方法的收益高度依赖于具体解码设置，未来需要建立更全面的评估框架来准确衡量其有效性。
...
### [23] [FAIR-Calib: Frontier-Aware Instability-Reweighted Calibration for Post-Training Quantization of Diffusion Large Language Models](http://arxiv.org/abs/2606.06547v1)
*Haoyu Huang,Linlin Yang,Sheng Xu,Boyu Liu,Guodong Guo,Zhongqian Fu,Hang Zhou,Baochang Zhang*

Main category: cs.LG

TL;DR: 本文提出FAIR-Calib框架，一种针对扩散大语言模型的两阶段后训练量化方法，通过感知写入边界的不稳定性进行重加权校准，以减轻早期脆弱决策的不可逆翻转。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)在迭代细化令牌时具有不可逆性，存在早期决策脆弱的“稳定性滞后”问题；而后训练量化(PTQ)误差极易翻转写入边界处的脆弱决策，导致错误被永久锁定并放大。

Method: 提出边界感知不稳定性重加权校准(FAIR-Calib)两阶段PTQ框架：阶段I探测全精度教师模型以估计结合边界命中和掩码可靠性的位置先验；阶段II通过最小化重加权隐藏状态MSE进行离策略逐层校准，优先保护脆弱边界状态而无需昂贵的端到端扩散rollout，理论上证明了该加权目标是输出KL散度的代理。

Result: 在LLaDA和Dream(W4A4)模型上，FAIR-Calib持续优于现有最优基线，显著减少了边界决策翻转并抑制了提交后不匹配现象。

Conclusion: FAIR-Calib有效解决了dLLMs量化过程中的边界决策脆弱性问题，提升了量化模型的稳定性，为dLLMs的高效部署提供了可行方案。
...
### [24] [STaR-Quant: State-Time Consistent Post-Training Quantization for Diffusion Large Language Models](http://arxiv.org/abs/2606.04945v2)
*Xin Yan,Aqiang Wang,Zhenglin Wan,Xingrui Yu,Ivor Tsang*

Main category: cs.LG

TL;DR: 本文提出STaR-Quant框架，通过解决状态依赖的激活差异和时间误差累积问题，实现了扩散大语言模型的高效低比特训练后量化。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（DLLMs）因模型庞大和迭代去噪过程导致严重的内存与计算开销，亟需训练后量化来高效部署；然而其低比特量化面临两大挑战：状态依赖的激活差异（同一去噪步中掩码与未掩码token的激活分布不同）和时间误差累积（量化误差在迭代解码中跨步累积）。

Method: 提出STaR-Quant状态-时间一致PTQ框架：1）引入状态引导激活变换（SGAT），将掩码和未掩码token映射至不同激活变换空间，并采用统一的静态权重侧变换；2）引入时间注意力补偿（TAC），通过轻量级块对角仿射映射校正量化后的注意力表示。

Result: 在代表性DLLMs上的实验表明，STaR-Quant在低比特权重-激活量化上持续优于强PTQ基线，且相比FP16部署实现了最高1.69倍的推理加速和3.14倍的内存节省。

Conclusion: STaR-Quant有效克服了DLLMs量化中的状态与时间一致性挑战，显著提升了低比特量化性能与模型部署效率，为DLLMs的实际应用提供了可行方案。
...
### [25] [Fine-tuning Multi-modal LLMs with ART: Art-based Reinforcement Training](http://arxiv.org/abs/2606.11854v1)
*Michal Chudoba,Sergey Alyaev,Petra Galuscakova,Tomasz Wiktorski*

Main category: cs.LG

TL;DR: 本文提出ART方法，通过仅优化多模态大语言模型的原始视觉输入来实现参数高效微调，从而无需修改预编译计算图。


<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调方法（如LoRA和软提示）需要修改预编译及预优化LLM的计算图，导致其无法在高吞吐量推理引擎（如vLLM）中获得完整支持。

Method: 提出基于艺术的强化训练（ART），通过将梯度反向传播至原始像素阵列，仅优化冻结多模态大语言模型（MLLM）的视觉输入来注入信息，使软标记方法兼容预编译计算图，且优化后的视觉输入可风格化为任务相关的计算艺术品。

Result: 在多种规模的Qwen架构及多个文本基准测试中验证了有效性；在数学和结构化工具使用基准上，ART的准确率与LoRA具有竞争力。

Conclusion: ART实现了在不修改计算图的前提下进行参数高效微调，解决了高吞吐量推理引擎的兼容性问题，且在特定任务上达到了与主流方法相当的性能。
...
### [26] [Agents-K1: Towards Agent-native Knowledge Orchestration](http://arxiv.org/abs/2606.13669v1)
...
TL;DR: 本文提出端到端知识编排管线Agents-K1，将原始文献转换为智能体原生的科学知识图谱，并构建了大规模Scholar-KG。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的研究智能体忽略科学知识编排，仅将论文简化为摘要、表面提及和扁平化引用，遗漏了科学推理所需的关键实体、主张、证据、机制和方法谱系。

Method: 提出Agents-K1管线，整合三个组件：1）五模块多模态解析器，提取全文实体、多模态证据及类型化实体间关系；2）基于规则奖励与GRPO训练的4B参数信息提取主干网络；3）统一网络搜索、多模态图检索和跨文档遍历的三源智能体接口graphanything CLI。

Result: 基于该管线处理6个学科的246万篇论文构建了Scholar-KG（开源100万篇子集），实验表明Agents-K1在科学信息提取、知识图谱构建及多跳科学推理任务上均取得优越性能。

Conclusion: Agents-K1有效解决了科学知识编排缺失问题，其管线具备向通用语料库及符合模式的数据合成扩展的潜力，显著提升了科学推理能力。
...
### [27] [EurekAgent: Agent Environment Engineering is All You Need For Autonomous Scientific Discovery](http://arxiv.org/abs/2606.13662v1)
*Amy Xin,Jiening Siow,Junjie Wang,Zijun Yao,Fanjin Zhang,Jian Song,Lei Hou,Juanzi Li*

Main category: cs.AI

TL;DR: 本文提出“环境工程”概念并构建EurekAgent系统，通过优化代理环境来提升LLM在自主科学发现中的可靠性和效率。


<details>
  <summary>Details</summary>
Motivation: 随着LLM能力提升，自主科学发现的瓶颈已从设计代理工作流转向设计代理环境，亟需构建能放大有效行为（如开放式探索、协作）并抑制有害行为（如奖励黑客、高摩擦人类监督）的环境。

Method: 提出EurekAgent系统，从四个维度进行环境工程化设计：权限工程（边界执行与隔离评估）、制品工程（基于文件系统与Git的协作）、预算工程（预算感知探索）和人在回路工程（便捷的人类监督与干预）。

Result: 在多个数学、内核工程和机器学习任务上取得了新的SOTA结果，例如以不到11美元的总API成本发现了26圆填充问题的新最优解。

Conclusion: 环境工程应成为开发可靠自主研究代理的核心研究方向，团队已开源相关代码和结果以推动该领域发展。
...
### [28] [Beyond Runtime Enforcement: Shield Synthesis as Defensibility Analysis for Adversarial Networks](http://arxiv.org/abs/2606.13621v1)
*Achraf Hsain,Sultan Almuhammadi*

Main category: cs.AI

TL;DR: 提出将屏蔽强化学习从运行时安全机制转变为设计时架构分析工具，通过双人安全博弈生成网络可防御性裁决及指纹。


<details>
  <summary>Details</summary>
Motivation: 传统屏蔽强化学习仅作为运行时约束智能体动作的安全机制，未能充分发挥其自动机理论机制在系统结构洞察上的潜力，本文旨在将其重新定位为设计时的分析仪器，以解答系统架构是否及如何可防御的问题。

Method: 构建用于网络防御的约束双人安全博弈，非对称地执行防御者规范（定义不安全区域）和攻击者规范（约束吸引子计算中的动作）；从吸引子结构推导拓扑度量，并结合受屏蔽约束的对抗性多智能体强化学习的收敛行为，生成“可防御性指纹”。

Result: What-if分析揭示形式化可防御性与操作有效性捕获了安全的不同维度：微小的架构改变可导致操作结果剧变，而形式化安全边界几乎保持不变。

Conclusion: 屏蔽合成的核心价值不在于部署安全策略，而在于作为回答系统架构可防御性（是否、何处及如何防御）的分析框架，其最终输出应是可防御性裁决而非安全策略。
...
TL;DR: 本文提出TAPS方法，通过将扩散边际概率转换为路径条件接受估计来选择紧凑的前缀闭合子树，解决推测解码中的验证瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的推测解码草稿树方法按边际概率对节点排序，忽略了验证的前缀条件性，导致验证被拒前缀的不可达后代，增加延迟但接受率提升有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边际概率转换为路径条件化的接受估计，并在固定验证预算下选择紧凑的前缀闭合子树，优化接受度与成本的权衡。

Result: TAPS在多种数据集和模型上实现了高达7.9倍的无损端到端加速，相比最先进的DFlash和DDTree分别提升1.36倍和1.74倍。

Conclusion: 通过路径条件化的前缀选择，TAPS有效解决了草稿树验证中的不匹配问题，显著提升了推测解码的加速比，证明在固定预算下优化子树选择优于简单扩展草稿树。
...
### [30] [Reasoning for Mobile User Experience with Multimodal LLMs: Task, Benchmark, and Approach](http://arxiv.org/abs/2606.13192v1)
...
TL;DR: 本文提出了UXBench基准以评估多模态大语言模型的UI推理能力，并基于强化学习构建了UI-UX模型以实现该领域的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于UI截图评估用户体验的研究尚不成熟，且主流多模态大语言模型在UI推理能力上存在根本性局限。

Method: 构建包含2000个VQA样本和8项细粒度UX诊断任务的UXBench基准；基于Qwen3-VL-4B-Thinking提出UI-UX模型，采用强化学习训练并引入动态平衡感知与逻辑推理的奖励路由机制，以及抑制冗余或不足推理步骤的非对称转移奖励。

Result: 主流MLLM在UI推理上表现受限；UI-UX在UXBench上实现SOTA（准确率0.7963，超越Claude-4.5-Sonnet的0.6550），并展现出强泛化能力与低推理延迟。

Conclusion: 当前MLLM的UI推理能力亟需进一步发展，UI-UX模型与UXBench基准有效弥补了此空白，验证了特定强化学习策略在提升UI/UX推理任务上的有效性。
...
### [31] [MLUBench: A Benchmark for Lifelong Unlearning Evaluation in MLLMs](http://arxiv.org/abs/2606.12809v1)
*He Li,Haoang Chi,Qizhou Wang,Yunxin Mao,Zhiheng Zhang,Jie Tan,Tongliang Liu,Wenjing Yang,Bo Han*

Main category: cs.AI

TL;DR: 本文提出了多模态大语言模型终身遗忘的大规模基准MLUBench，并设计了LUMoE方法有效缓解连续遗忘带来的累积退化与多模态对齐破坏问题。


<details>
  <summary>Details</summary>
Motivation: 数据所有者要求删除特定内容的需求随时间顺序到达，引发MLLM终身遗忘问题，但现有基准规模和范围有限，无法反映该问题的复杂性，且现有方法在连续遗忘时面临严重的累积退化和多模态对齐破坏的挑战。

Method: 构建了包含9类127个实体的大规模终身遗忘基准MLUBench；提出了LUMoE方法，旨在持续遗忘单一模态数据时保持多模态对齐，从而缓解模型整体退化。

Result: 实验表明现有遗忘方法在终身遗忘场景下存在严重的累积退化，且单模态的持续遗忘会导致整个多模态模型性能下降；LUMoE方法显著缓解了基线模型面临的退化问题。

Conclusion: MLLM终身遗忘面临跨模态对齐破坏的独特挑战，LUMoE能有效缓解累积退化问题，相关代码和数据集已开源。
...
### [32] [MODF-SIR: A Multi-agent Omni-modal Distilled Framework for Social Intelligence Reasoning](http://arxiv.org/abs/2606.12018v1)
*Shang Ma,Jisheng Dang,Wencan Zhang,Yifan Zhang,Bimei Wang,Hong Peng,Bin Hu,Qi Tian,Tat-Seng Chua*

Main category: cs.AI

TL;DR: 提出一种基于轻量级多模态大语言模型的多智能体协作框架，通过知识蒸馏增强训练与推理，并结合测试时自适应机制，在社会智能推理任务上取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 解决社会智能推理中多模态数据的关键长尾事件在标记化过程中容易被头部事件和环境噪声掩盖的问题。

Method: 构建基于轻量级MLLM的多智能体协作框架，精确定位多模态数据并将长尾事件提取为格式化显式文本；在整个推理管线（事件提取与表示、思维链提示、自我反思）中集成蒸馏增强的测试时自适应（TTA），并利用LoRA对基础模型进行实例级推理微调。

Result: 在多个基准测试中对比多种开源与闭源模型，仅使用约30%的IntentTrain训练数据即达到最先进（SOTA）的性能。

Conclusion: 该框架有效提升了社会智能推理能力，验证了知识蒸馏增强的多智能体协作与测试时自适应机制在处理和推理长尾事件方面的有效性。
...
### [33] [Tuning Agent-Based Predator-Prey Models Toward Lotka-Volterra Dynamics](http://arxiv.org/abs/2606.13639v1)
*Corinna Mandl,Siddharth Chaturvedi,Marcel van Gerven*

Main category: cs.MA

TL;DR: 该论文提出一种基于特征损失的参数优化方法，使基于智能体的捕食者-猎物模型能够产生类似洛特卡-沃尔泰拉循环的稳定种群动态。


<details>
  <summary>Details</summary>
Motivation: 大规模基于智能体的模型（ABM）对局部规则和参数高度敏感，微小变化常导致系统出现失控、种群崩溃或边界饱和等不稳定行为，如何调参以获得稳定的宏观动力学是一个核心难题。

Method: 构建包含羊和狼的连续捕食者-猎物ABM（智能体具备局部感知、内部能量及RNN控制器），设计一种基于特征的损失函数（奖励持续振荡、相位滞后、有界种群及长期持续性）来优化环境和人口参数，并在随机与进化控制器设定下进行测试；基于JAX的ABMax框架实现硬件加速与高效批量模拟。

Result: 通过基于特征的损失函数优化环境和人口参数，成功使基于智能体的捕食者-猎物系统生成符合经典洛特卡-沃尔泰拉循环特征的持续且稳定的振荡动态。

Conclusion: 针对性地优化环境与人口参数可有效缓解ABM的参数敏感性与不稳定问题，使其重现宏观生态动力学特征；同时，基于JAX的框架为大规模ABM的高效模拟与参数搜索提供了可行路径。
