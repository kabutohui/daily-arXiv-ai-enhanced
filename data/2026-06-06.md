...
TL;DR: 本文提出了Astra框架，通过结合强化学习策略与世界模拟器，赋予视觉语言模型基于动作的视觉想象能力，以解决有限观测下的空间推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLM）的空间推理能力局限于已观测图像和文本思维链，难以推断未观测布局、维持跨视图一致性及从替代视角进行推理。

Method: 提出Astra智能体空间推理框架，包含经强化学习训练的策略模型Astra-VL和基于Bagel的世界模拟器Astra-WM；Astra-WM经视图一致性调优生成新视角观测，并通过模拟器参与循环的两阶段强化学习课程训练模型判断何时调用模拟器。

Result: 实验表明模拟器与智能体策略均不可或缺：Astra-WM将Gemini-3-Flash在MMSI-Bench上的表现从45.1提升至49.5；Astra-VL将Qwen3-VL在MMSI-Bench上从29.8提升至38.8，在MindCube上从36.8提升至42.7。

Conclusion: 想象的观测能为空间推理提供有效证据，但实现高效的世界模型增强推理，关键在于模型必须学会何时、何地以及如何进行想象。
...
TL;DR: 本文提出了PAR3D框架，通过引入部件感知表示学习和分层查询生成机制，解决了现有3D多模态大语言模型无法建模细粒度部件结构的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的3D多模态大语言模型主要以物体为中心，缺乏对细粒度部件结构的建模能力，限制了模型在3D环境中进行具身交互的表现。

Method: 提出了PAR3D统一框架，构建了带有部件级标注的合成数据集ScenePart，开发了部件感知3D表示学习方法，并提出了分层分割查询生成机制以实现部件目标的定位。

Result: 实验表明，该方法在部件级问答和指代表达分割任务上取得了显著提升，同时在物体级视觉-语言任务中也保持了强劲的性能。

Conclusion: PAR3D有效增强了3D多模态模型对物体部件的理解与推理能力，填补了细粒度3D场景理解领域的空白。
...
TL;DR: 本文提出了一对多时序定位任务（OMTG），构建了首个基准与数据集，并利用新颖的时序和描述奖励函数取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有时序定位研究多局限于单片段检索，无法满足现实场景中定位多个不连续片段的需求；现有最先进模型因缺乏事件基数感知能力，在一对多设置下表现极差。

Method: 提出系统性解决方案：建立首个OMTG基准及新评估指标；构建包含56k样本的高质量数据集；设计新颖的时序与描述奖励函数，利用思维链推理优化策略以兼顾精确性与完整性。

Result: 模型在OMTG Bench上实现了43.65%的最先进EtF1分数，显著优于Gemini 2.5 Pro和Seed-1.8（分别提升15.85%和15.61%）。

Conclusion: 本研究填补了一对多时序定位领域的空白，验证了新基准、数据集及奖励机制的有效性，显著提升了模型在复杂视频定位任务中的表现。
...
TL;DR: 本文提出了面向无人机灾害响应的多阶段多模态推理基准DisasterBench，并构建了轻量级模型DisasterVL，在边缘计算约束下实现了媲美GPT-4o的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态基准主要关注感知任务且灾害类型覆盖有限，无法满足实际应急响应中对因果分析、态势预测及决策推理等多阶段复杂推理的需求。

Method: 构建了涵盖14种场景和9项关键任务的DisasterBench基准；提出轻量级多模态模型DisasterVL，采用领域指令微调、思维链引导的多模态对齐及强化学习策略优化相结合的三阶段训练流程。

Result: 在21个主流多模态大模型上的实验表明，仅有2B参数的DisasterVL性能优于所有参与评估的开源模型，并以更高效率达到了与闭源模型GPT-4o相当的推理准确率。

Conclusion: 该工作填补了灾害场景多阶段推理基准的空白，并验证了轻量级模型在端侧设备实现高性能应急响应推理的可行性与有效性。
...
TL;DR: 本文提出了LoomVideo，一种高效的50亿参数统一视频生成与编辑模型，通过引入零开销的条件机制消除了传统标记拼接带来的计算负担，实现了显著的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有的统一视频框架通常依赖超大规模模型（130亿参数以上），并通过标记拼接引入源视频条件进行编辑，这导致序列长度翻倍，使自注意力机制的计算复杂度增加四倍，带来了难以承受的计算开销。

Method: 提出LoomVideo架构，利用多模态大语言模型（MLLM）替代文本编码器并采用Deepstack注入机制对齐特征；核心提出零开销的“缩放与相加”条件方法，通过将干净的源视频潜变量直接叠加至噪声目标潜变量来避免拼接；此外集成了负时序旋转位置编码策略以处理多参考图像。

Result: 该紧凑型模型在综合基准测试中达到了最先进或极具竞争力的性能，在电商和时尚生成场景中表现尤为优异；得益于零开销条件机制，其推理速度相比同等能力的模型至少提升了5.41倍。

Conclusion: LoomVideo成功解决了统一视频模型中的计算瓶颈，证明了紧凑型架构结合高效条件机制可以在保持高性能的同时实现极高的推理效率，为实用化视频基础模型的发展铺平了道路。
...
TL;DR: 本文针对扩散模型U-Net蒸馏后瓶颈反转为大型文本编码器的问题，提出了一种高效的流式推理管线，在消费级显卡上实现了高帧率的视频生成。


<details>
  <summary>Details</summary>
Motivation: 解决在扩散U-Net经过极度蒸馏（如1步或4步）后，推理瓶颈从去噪器转移至多模态大语言模型（MLLM）文本编码器的问题，特别是在视觉感知编辑任务中这一反转尤为严重。

Method: 构建了一个流式管线，包含三个核心工程机制：带有批处理文本编码器分摊的非对称侧流/主流CUDA流水线、将U-Net与适配器堆栈折叠为单一融合图的ControlNet-LLLite重述，以及分摊逐帧条件成本的周期性条件刷新调度。

Result: 在单张RTX 3090 Ti上以512x512分辨率实现了27.4 fps（B=8）和29.6 fps（B=16）的流式吞吐，端到端延迟分别约为0.5秒和1.0秒；在RTX 4090和5090上分别达到54.9 fps和74.1 fps。

Conclusion: 验证了所提工程机制在解决编码器瓶颈方面的有效性，发布的时序适配器在特定风格下展示了良好的泛化能力，并将性能数据作为系统背景参考进行了定位。
...
TL;DR: 本文提出了FEPBench，一个用于评估文本生成图像模型在科学插图生成任务中细粒度表现的基准，涵盖指令忠实度、推理丰富度和语义精度等维度。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试往往在整体层面评估输出，忽视了细粒度元素，且对科学推理能力和输出简洁性的量化不足，无法满足科学插图生成的特定需求。

Method: 构建了包含多学科高质量科学插图的FEPBench基准，利用多模态大语言模型（MLLM）和人类专家提供细粒度原子集标注，并从指令忠实度、推理丰富度和语义精度三个维度，结合视觉、文本、关系和布局元素对模型进行系统评估。

Result: 实验结果显示，即便是GPT Image 2等最先进的闭源模型，仍面临文本渲染瓶颈、推理丰富度有限以及难以平衡生成丰富度与精度等问题。

Conclusion: 研究发现为改进和部署科学插图生成领域的文本生成图像模型提供了实践指导，相关基准数据和代码将对外发布。
...
TL;DR: 本文提出GeoVR框架，通过多目标几何蒸馏技术，仅利用2D视频序列赋予多模态大语言模型3D空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型擅长2D语义理解但缺乏内在3D意识，导致视频帧间几何与空间不一致，且大规模3D数据稀缺。

Method: 提出GeoVR框架，通过多目标学习策略将预训练3D基础模型的几何知识蒸馏至MLLM内部表征。具体包含四个互补几何目标：帧间相机位姿估计、稠密深度图回归、度量尺度因子预测及多尺度3D特征蒸馏。

Result: 在空间推理基准测试上的广泛实验表明，GeoVR达到了最先进的性能水平。

Conclusion: 该方法成功使模型内部表征具备强3D意识，建立了赋予基础模型空间智能的新范式。
...
TL;DR: 该论文提出SAID框架，通过基于语义结构的计算重分配策略加速扩散大语言模型的推理，实现了最高9.1倍的加速且保持性能竞争力。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽然支持并行生成，但为了获得高质量输出仍需大量去噪步骤，导致推理成本高昂。

Method: 提出SAID框架，优先对确立语义结构的“支架词元”进行去噪，并减少可预测细节词元的计算量；结合块状解码并引入置信度-分层分层生成（CHLG）策略，针对低置信度词元分配额外步骤。

Result: 在LLaDA模型上的实验表明，SAID在数学、编程和知识基准测试中实现了最高9.1倍的加速，同时保持了生成质量。

Conclusion: 该方法证明了通过感知语义支架重新分配计算资源，能够显著提升扩散大语言模型的推理效率，为DLLM加速提供了有效方案。
...
TL;DR: 本文提出了一种无需训练的方法——动态填充锚点（DIA），旨在解决扩散大语言模型在格式约束任务中因固定锚点导致的生成僵化问题，显著提升了推理任务的准确性和合规性。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽然天然支持格式约束任务，但现有的固定锚点方法由于生成范围僵化，容易导致推理内容被截断或产生冗余内容，无法有效平衡结构正确性与语义连贯性。

Method: 提出动态填充锚点（DIA），这是一种免训练方法，通过在迭代填充生成前动态估计末端锚点位置来调整生成长度，从而灵活地确保结构正确性并维持语义连贯。

Result: 实验结果表明，DIA 显著提高了格式合规率和答案准确率，在 GSM8K 和 MATH 推理基准测试中取得了显著的零样本性能提升。

Conclusion: DIA 为实现可靠且具有结构感知能力的生成提供了一条稳健的路径，有效解决了结构约束下的生成效率和质量问题。
...
TL;DR: 提出CAPR算法，利用扩散大语言模型的去噪轨迹实现高效块级监督，以更低计算成本达到强化学习调优的新最优性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有扩散大语言模型（dLLM）强化学习中，平面展开监督信号稀疏而树结构展开计算成本过高的问题，旨在利用去噪轨迹实现高效且细粒度的监督。

Method: 提出CAPR算法，将去噪轨迹总结为紧凑路径状态，利用缓存生成廉价兄弟延续，并训练块级价值头；在块级去掩码计划下，根据每块揭示的token将最终结果奖励重新分配至各块，实现局部块级监督。

Result: 展开生成成本降至平面展开的约0.75倍、树结构展开的0.6倍；在4x4数独、Countdown、GSM8K和Math500上刷新了RL调优dLLM的最优表现，且在数独任务上以不足三分之一的计算量匹配了最强树结构基线。

Conclusion: CAPR成功在避免全树展开的情况下恢复了树搜索的粒度，证明了利用去噪轨迹进行高效监督的可行性，确立了dLLM强化学习的新基准。
...
TL;DR: 本文提出了Fast-dLLM++，一种免训练扩展方法，通过利用异构置信度分布的Fréchet profile解码，显著提升了扩散大语言模型的推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的Fast-dLLM在并行解码中采用同质高置信度假设（即由置信度最低的token决定候选集），忽略了实际解码步骤中存在的异构置信度特征，导致推理速度未能最大化。

Method: 提出Fast-dLLM++，引入Fréchet profile解码机制。该方法基于完整的有序置信度分布而非单一最坏情况来选择并行提交集，是对原有规则的异构置信度泛化，并在置信度不均时提供可证明的“异构性加成”，且无需修改模型或训练过程。

Result: 在LLaDA-8B模型及GSM8K、MATH、HumanEval、MBPP等基准上的实验表明，该方法通过利用原有规则错失的安全并行性，改善了精度-吞吐量边界，在保持相当精度的同时实现了高达37%的吞吐量提升。

Conclusion: 理论分析与实验结果一致证明，利用异构置信度分布能有效挖掘安全并行性，Fast-dLLM++可作为现有扩散大语言模型解码的高效即插即用替代方案。
...
TL;DR: 本文提出了一种名为SimSD的免训练推测解码算法，通过引入即插即用的掩码策略解决了扩散大语言模型与词元级推测解码的不兼容问题，实现了高达7.46倍的解码吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型因依赖掩码词元和双向注意力机制，导致上下文在去噪步骤中变化，使其与标准的词元级推测解码技术不兼容，从而无法像自回归模型那样在单次前向传播中高效验证草稿词元。

Method: 提出SimSD算法，采用即插即用的掩码策略，引入草稿模型预测的参考词元并设计特定的注意力掩码来调节其与当前步词元的交互，赋予dLLMs时间有效的词元级上下文，从而在单次前向传播中实现对草稿词元的有效验证。

Result: 在四个基准测试的SDAR系列模型实验中，该方法在保持甚至提高生成质量的同时，实现了高达7.46倍的解码吞吐量提升。

Conclusion: SimSD成功弥合了dLLMs与推测解码之间的差距，在保留dLLMs并行解码优势的同时恢复了关键验证能力，且无需训练即可灵活兼容KV缓存和块级解码等加速技术。
...
TL;DR: 本文揭示了顺序无关语言模型中揭示顺序对似然性的影响，提出利用置信度方差作为诊断指标以更准确地评估解码路径。


<details>
  <summary>Details</summary>
Motivation: 现有的顺序无关语言模型评估指标（如似然性）混淆了内容难度与路径相关的人为干扰，且缺乏对解码路径有效性的独立诊断方法，因此需要一种新的指标来准确评估和比较不同的解码路径。

Method: 作者首先分析了学习条件分布的非相干性，随后基于均匀扩散定理提出了一种互补诊断方法，即利用对数概率的方差（$\mathrm{Var}(\log q_t)$）作为指标来衡量解码路径的质量。

Result: 实验表明：(1) 不同的揭示顺序会导致目标似然发生显著变化；(2) 置信度优先解码在内容词元上的表现接近从左到右的顺序；(3) 低方差能有效区分结构化路径与随机排序，且方差大小与下游任务的正确性高度相关。

Conclusion: 研究结果支持将平均置信度与置信度方差联合使用，作为比较和评估顺序无关语言模型解码路径的标准指标。
...
TL;DR: 本文提出dMoE框架，通过聚合块级专家分布解决了扩散大语言模型与MoE架构结合时的路由不匹配问题，显著降低了显存占用并提升了推理速度。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的块并行解码与传统MoE的Token级专家选择机制存在不匹配，导致推理时唯一激活的专家数量大幅增加，造成严重的内存瓶颈。

Method: 提出dMoE框架，将每个块内的Token级专家分布聚合为统一的块级专家分布，以此指导专家路由，从而在保持性能的同时大幅减少唯一激活专家的数量。

Result: 实验表明，dMoE将唯一激活专家数平均从69.5降至14.6，保持了99.11%的原有性能；内存使用量降低76.64%至79.84%，端到端延迟实现1.14倍至1.66倍的加速。

Conclusion: dMoE成功解决了dLLMs与MoE结合时的内存受限问题，证明了块级路由策略在提升推理效率和降低资源消耗方面的有效性。
...
TL;DR: 该论文提出了CaDDTree，一种成本感知的扩散起草树方法，通过联合优化树结构和节点预算来最大化Token吞吐量，解决了现有方法忽略验证成本导致预算选择缺乏原则性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散起草方法（如DDTree）仅最大化期望接受长度，该指标随预算增加而增加，未考虑验证成本，导致无法提供合理的预算选择依据，无法直接优化实际的推理速度。

Method: 提出CaDDTree方法，显式建模起草和验证延迟，直接优化Token吞吐量；证明了在凸验证成本下吞吐量函数具有单峰性，从而将问题转化为对预算的高效一维搜索，实现了每轮根据当前分布自适应调整预算的贪婪停止策略。

Result: 在Qwen3-4B和Qwen3-8B模型上的实验表明，CaDDTree在推理、编程和指令遵循等八个基准测试中，性能匹配或超越了具有先知预算选择的DDTree。

Conclusion: CaDDTree通过成本感知的优化目标，无需离线预算搜索即可自适应调整预算，证明了优化吞吐量比单纯优化接受长度更有效，显著提升了推测解码的效率。
...
TL;DR: 本文提出了DDTree方法，通过从块扩散起草器的分布构建起草树以优化多令牌验证，实现了领先的推测解码性能。


<details>
  <summary>Details</summary>
Motivation: 现有的先进块扩散起草器DFlash每轮仅验证单条起草轨迹，限制了接受长度，从而制约了推测解码的加速潜力。

Method: 提出DDTree，利用块扩散起草器的逐位置分布构建起草树；在固定节点预算下，采用最佳优先堆算法选择最可能的延续，并通过祖先注意力掩码在单次目标模型前向传播中高效验证该树结构。

Result: DDTree基于DFlash架构克服了单轨迹验证的局限性，实现了显著的性能提升。

Conclusion: DDTree确立了其作为当前领先的推测解码方法的地位，验证了在块扩散起草器上构建起草树的有效性。
...
TL;DR: 本文提出 RandomBench 基准测试，揭示了多模态大语言模型在逻辑中性场景下无法保持均匀随机分布的‘随机性崩塌’现象。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注效用驱动目标，忽略了逻辑中性场景下的随机性行为；在此类多选项等效的场景中，确定性策略会导致重复行为并降低有效替代方案的覆盖率。

Method: 提出 RandomBench 基准测试以评估模型在等效选项间的分布中性表现，并引入 RI、BCI 和 BII 三项指标来量化熵及分布偏差。

Result: 实验揭示了‘随机性崩塌’现象，即 MLLMs 在显式随机指令下无法维持均匀随机性（例如 Claude Sonnet 4.6 的 Top-1 概率达 97%，RI 降至 0.068），且该偏差跨语言和表示格式持续存在。

Conclusion: 研究表明 MLLMs 在逻辑中性决策中存在稳健的分布崩塌，揭示了模型在非效用导向设置下的局限性。
...
TL;DR: 本文揭示了多模态大语言模型中跨模态检索的功能稀疏性，识别出关键的CoRe heads负责信息提取，并验证了其重要性及在推理加速中的应用价值。


<details>
  <summary>Details</summary>
Motivation: 旨在解决多模态大语言模型如何从复杂噪声语境中提取查询相关视觉特征的机制尚不明确的问题，填补对模型内部运作原理认知的空白。

Method: 提出了检索注意力质量（RAM）指标，识别并定义了一组特定的注意力头为上下文感知检索头，并通过因果干预和消融实验分析了其功能特性。

Result: 实验发现仅消融前5%的CoRe heads即可导致模型多模态推理性能显著下降，而利用该功能稀疏性进行优化可显著加速模型推理且不损失任务性能。

Conclusion: 研究确立了MLLMs内部存在功能稀疏性的结构原则，深化了对机械可解释性的理解，并为未来的模型架构设计和优化提供了理论指导。
...
TL;DR: 该论文提出了MLEvolve框架，通过渐进式图搜索和回溯记忆机制解决了MLE智能体在长周期任务中的优化障碍，实现了最先进的算法发现性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有MLE智能体在长周期任务中面临的分支间信息隔离、无记忆搜索以及缺乏层次化控制问题，以提升长期优化能力。

Method: 提出基于LLM的自我进化多智能体框架MLEvolve：引入Progressive MCGS实现跨分支信息流动与搜索策略渐进；构建回溯记忆以积累和复用经验；解耦策略规划与代码生成以确保长期迭代的稳定性。

Result: 在MLE-Bench上以一半标准时间（12小时）达到SOTA性能（平均奖牌率和有效提交率）；在数学算法优化任务上超越AlphaEvolve等专用方法，展现了优异的跨领域泛化性。

Conclusion: MLEvolve有效增强了LLM智能体的自我进化能力，为机器学习算法发现提供了一种高效的解决方案，并具备跨领域应用的潜力。
...
TL;DR: 本文提出了Goedel-Architect框架，通过蓝图生成与细化策略在Lean 4形式化定理证明中实现了开源流水线的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 旨在解决主流递归引理分解方法容易在无效策略上陷入死循环导致效率低下的问题，提升形式化定理证明的效率与成功率。

Method: 提出一种以蓝图（定义与引理的依赖关系图）为中心的智能体框架；先生成形式化蓝图（可由自然语言引导），利用工具增强的Lean证明器并行证明引理节点，并根据失败案例驱动全局蓝图的迭代细化。

Result: 基于DeepSeek-V4-Flash模型，该方法在MiniF2F-test上达到100%通过率，在PutnamBench上达到88.8%通过率，并成功解决了IMO 2025（4/6）、Putnam 2025（11/12）及USAMO 2026（3/6）中的多项难题，且成本比同类开源流水线降低达500倍。

Conclusion: Goedel-Architect确立了开源定理证明流水线的最先进水平，证明了基于蓝图细化的方法相比递归分解具有显著的性能与成本优势。
...
TL;DR: 本文提出了Benchmark Agent，一个全自主智能体系统，旨在自动化构建基准，以解决现有基准构建成本高、难以复用及性能饱和快的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基准构建过程劳动密集、难以复用且缺乏可持续性，且基准发布后常迅速达到性能饱和，无法有效区分最先进模型。

Method: 提出了Benchmark Agent框架，作为一个全自主智能体系统，自动化编排了从用户查询分析、子任务设计、数据标注到质量控制的完整基准构建流程。

Result: 该系统成功构建了15个涵盖文本理解、多模态理解及特定领域推理的基准；通过人工评估与LLM-as-a-judge等实验证明，该系统能以极少人工参与生成高质量基准样本，并发现当前模型在特定领域推理任务上存在困难。

Conclusion: 研究证明了快速演进基准对研究社区的重要价值，相关代码与预览将公开发布。
...
TL;DR: 该论文提出了Vortex系统，通过统一的编程抽象与高效后端集成，实现了稀疏注意力算法的快速原型设计、部署与评估，显著提升了大规模语言模型服务的吞吐量。


<details>
  <summary>Details</summary>
Motivation: 随着生成长度的增加，稀疏注意力对LLM服务至关重要，但在大规模场景下部署和评估新型稀疏注意力算法的工程成本极高，严重阻碍了研究人员及AI代理对该领域的探索与迭代。

Method: 提出Vortex系统，包含一个构建在以页为中心的张量抽象之上的Python嵌入式前端语言，用于表达广泛的稀疏注意力算法；以及一个紧密集成于现代LLM服务栈的高效后端，支持算法的快速落地与性能转化。

Result: AI代理利用Vortex自动生成的最优算法相比全注意力实现了3.46倍的吞吐量提升；在MLA架构的GLM-4.7-Flash和229B参数的MiniMax-M2.7模型上，分别实现了高达4.7倍和1.37倍的吞吐量加速。

Conclusion: Vortex有效解决了稀疏注意力算法从理论到实际部署的工程鸿沟，大幅加速了算法设计迭代，并成功将其扩展至新兴架构和超大规模模型等原本难以实验的场景。
...
TL;DR: 本文提出了TAPS方法，通过目标感知的前缀选择策略优化扩散模型推测解码中的验证过程，解决了现有方法中边际概率与前缀条件验证不匹配的问题，实现了显著的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型推测解码方法虽然通过并行预测降低了起草延迟，但验证过程成为新瓶颈。现有方法基于边际概率排序节点，忽略了验证过程的前缀条件性，导致验证了不可达的节点，增加了不必要的延迟且收益有限。

Method: 提出TAPS（Target-Aware Prefix Selection）方法，将扩散边际概率转化为路径条件的接受度估计，并在固定验证预算下选择紧凑的前缀闭合子树，从而优化接受度与成本的权衡。

Result: 实验表明，TAPS相比原始自回归解码实现了高达7.9倍的无损端到端加速，且性能分别优于最先进方法DFlash和DDTree 1.36倍和1.74倍。

Conclusion: TAPS有效解决了扩散树方法中概率估计与验证机制不匹配的问题，通过优化起草树的选择显著提升了推测解码的效率。
...
TL;DR: 本文提出DNQ框架，通过求解器在环均衡监督解决多轮同时竞标问题，证明了成对公式相比精确公式具有更好的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现实竞争系统涉及多决策者同时行动、共享约束和有限信息，本文旨在解决此类多轮交互场景下的智能体训练问题。

Method: 提出DNQ框架，交替执行轨迹收集、共享评论家收益估计、外部均衡求解和策略模仿；通过最小化策略与均衡目标的KL散度训练智能体，重点采用成对公式以降低求解成本。

Result: 实验表明，成对方法能有效扩展至更多智能体，而精确方法随着博弈规模增大变得计算不可行。

Conclusion: 研究揭示了重复竞争环境中策略保真度与可扩展性之间的权衡，成对公式在处理大规模多智能体问题时更具实际应用价值。
...
TL;DR: 本文证明了在次模估值下，存在能够同时满足常数ρ下的ρ-MMS和EF1（及EFL）的分配方案。


<details>
  <summary>Details</summary>
Motivation: 在不可分割物品分配问题中，基于份额的公平性（如MMS）与基于比较的公平性（如EF1）通常无法相互保证，且已有研究结果仅限于可加性估值，本文旨在解决更一般的次模估值下的双重公平性保障问题。

Method: 设计了一种分配机制，能够同时满足基于份额的公平性概念（ρ-MMS）和基于比较的公平性概念（EF1及EFL）。

Result: 证明了对于比可加性估值更广泛的次模估值类别，存在常数ρ使得分配方案能同时满足ρ-MMS和EF1/EFL。

Conclusion: 成功将同时满足两类公平性概念的理论结果从可加性估值推广至次模估值，扩展了公平分配理论的适用范围。
...
### [27] [Will the Agent Recuse Itself? Measuring LLM-Agent Compliance with In-Band Access-Deny Signals](http://arxiv.org/abs/2606.06460v1)
*Thamilvendhan Munirathinam*

Main category: cs.CR

TL;DR: 该论文提出了一种名为“回避信号”的轻量级带内拒绝信号，用于指导自主LLM代理自愿撤回对受限资源的访问，实验显示主要模型完全遵守该信号。


<details>
  <summary>Details</summary>
Motivation: 随着自主LLM代理持有真实凭证并在无人工干预下操作基础设施，运维人员缺乏标准化的方式告知代理特定资源禁止访问，现有访问控制（放行或硬性失败）无法有效传达拒绝意图。

Method: 提出一种轻量级的“回避信号”微型标准，服务器通过现有协议通道（如SSH横幅、PostgreSQL NOTICE）发出拒绝信号，请求代理自愿撤回；实现了SSH和PostgreSQL的适配器，并在生产主机上进行了受控实验。

Result: 试点实验显示，当存在信号时代理撤回率为100%，无信号对照组任务完成率为100%。该信号表现为合作性而非绝对性：明确的操作员授权框架会促使最强模型继续执行任务，而其他代理则继续遵循主机策略。

Conclusion: “回避信号”作为一种合作治理控制（类似实时访问的robots.txt）是有效的，能够诱导合规代理自愿停止操作；论文公开了标准、适配器和实验框架以供复现。
...
### [28] [MaskForge: Structure-Aware Adaptive Attacks for Jailbreaking Diffusion Large Language Models](http://arxiv.org/abs/2606.04027v1)
*Yingzi Ma,Zhengyue Zhao,Xiaogeng Liu,Minhui Xue,Yue Zhao,Chaowei Xiao*

Main category: cs.CR

TL;DR: 本文提出了MaskForge，一种针对扩散大语言模型的黑盒自适应攻击方法，通过优化结构模式库有效利用dLLM的原生填充漏洞进行越狱。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型因其双向上下文和基于置信度的填充机制存在独特的安全漏洞，现有越狱方法忽视了其原生填充能力且缺乏结构适应性，无法有效检测和利用这一攻击面。

Method: 提出MaskForge框架，将红队测试建模为对结构模式库的优化搜索过程；利用UCB强盗算法选择目标兼容模式，结合评分器引导的回退机制，并将成功攻击尝试蒸馏回模式库以积累经验。

Result: 在五个公共dLLM和三个基准测试中，MaskForge平均攻击成功率达79.3%，相对最强基线提升17.6%；成熟模式库无需更新迁移至AdvBench后成功率达88.2%，相对提升67%。

Conclusion: MaskForge揭示了dLLM特有的安全风险，证明了基于结构化模式积累和自适应搜索的攻击策略在利用dLLM填充漏洞方面的有效性和强大的迁移能力。
...
### [29] [STaR-Quant: State-Time Consistent Post-Training Quantization for Diffusion Large Language Models](http://arxiv.org/abs/2606.04945v1)
*Xin Yan,Aqiang Wang,Zhenglin Wan,Xingrui Yuand Ivor Tsang*

Main category: cs.LG

TL;DR: 本文提出了STaR-Quant框架，通过解决状态依赖激活差异和时间误差累积问题，实现了扩散大语言模型的高效低位量化部署。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型因模型规模大和迭代去噪过程导致显著的内存与计算开销，且现有的低位量化面临掩码与未掩码词元激活分布不同以及迭代解码中误差累积的挑战。

Method: 提出了STaR-Quant框架，包含状态引导激活变换（SGAT）以处理不同词元的激活分布差异，以及时间注意力补偿（TAC）通过轻量级仿射映射校正量化误差。

Result: 实验表明，该方法在低位量化上优于现有基线，相比FP16部署实现了最高1.69倍的加速和3.14倍的内存节省。

Conclusion: STaR-Quant有效解决了扩散大语言模型量化的核心难题，显著提升了推理效率并降低了资源消耗，具有实际部署价值。
...
### [30] [FLARE: Diffusion for Hybrid Language Model](http://arxiv.org/abs/2606.01774v1)
...
TL;DR: 本文提出了FLARE框架，通过优化迁移数据质量和统一训练目标，成功将混合注意力大语言模型转换为扩散语言模型，在保持性能的同时显著提升了推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型的顺序解码存在延迟瓶颈，而现有的高效架构与并行生成方案难以有效结合；将混合注意力模型转换为扩散语言模型时，常面临能力丢失和训练复杂的挑战。

Method: 提出了FLARE转换框架，发现迁移数据质量是能力保持的决定性因素；该框架结合了令牌均衡的自回归与扩散目标、硬件感知内核及统一推理机制，实现单一检查点支持双模式解码。

Result: 实验表明，该方法在各模型规模上与领先的开放源码扩散语言模型性能相当，且在单GPU并发服务中相比基准模型实现了持续的吞吐量提升。

Conclusion: 研究表明实用扩散语言模型的瓶颈不仅在于解码算法，更在于迁移数据质量和训练效率；这促使未来在数据、目标、架构和推理系统方面进行联合设计。
...
TL;DR: 本文提出了HANDOFF人形机器人全身控制器，通过设计紧凑显式的命令接口及多教师蒸馏方法，实现了无需微调即可执行自然语言驱动的多样化任务。


<details>
  <summary>Details</summary>
Motivation: 现有的全身控制器依赖难以从任务语义合成的稠密运动或空间参考，导致规划与控制接口受限，本文旨在解决该接口问题，提供一个直观、通用且表达力强的命令空间。

Method: 引入HANDOFF控制器，采用上下文条件门控下的多教师KL蒸馏策略，将全身运动跟踪（安全过滤数据）、移动和跌倒恢复三个专家模型蒸馏为一个专家混合学生模型。

Result: 在Unitree G1平台上，该方法匹配了最先进的速度跟踪性能，提供了极大的鲁棒操作工作空间，并结合VLM规划器成功实现了无需特定数据微调的自然语言驱动任务。

Conclusion: 证明了紧凑显式接口结合蒸馏框架的有效性，为人形机器人在现实世界中实现鲁棒、通用的操作技能提供了一种高效的解决方案。
...
TL;DR: GLOVES提出了一种基于流的适应方法，通过选择性纠正非专家动作并保留代理意图，实现了仅需有限专家监督的鲁棒动作适应。


<details>
  <summary>Details</summary>
Motivation: 预训练策略、基础模型或人类操作员提供的先验知识虽然高效，但其生成的动作往往次优、嘈杂或与任务特定专家行为不一致，需要一种既能纠正这些错误动作又不完全取代代理控制的方法。

Method: 提出基于流的适应方法GLOVES，将非专家动作传输向专家动作分布；利用反向流评估提供的分布内评分作为干预门控机制，仅对异常或分布外动作进行纠正，保持一致性动作不变。

Result: 该方法在提高任务成功率的同时有效保留了代理意图，仅需少量专家演示即可学习局部动作模式，作为一种轻量级共享控制模块实现了跨任务和环境的鲁棒适应。

Conclusion: GLOVES证明了通过有限专家监督进行选择性动作纠正的可行性，为利用先验知识进行机器人技能学习提供了一种高效且意图保持的解决方案。
