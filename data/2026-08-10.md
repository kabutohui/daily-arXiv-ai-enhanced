...
### [1] [I Seek You in Videos: Identity-Conditioned Queries for Person-Centric Video Reasoning](http://arxiv.org/abs/2608.07417v1)
...
TL;DR: 本文提出了基于身份条件的查询（ICQ）任务及ISYV系统，以解决真实视频中以人为中心的推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理任务多采用简化的视频-文本设定，限制了身份匹配和以人为中心的推理。本文旨在通过联合关联和解释输入视频与参考图像来解决这些挑战。

Method: 提出ICQ任务，并构建包含三个部分的ISYV系统：ISYV-Bench评估基准、ISYV-75K训练集以及包含ICQ导向模型和训练策略的ISYV-Framework。

Result: 主流多模态大模型在ISYV-Bench上表现挣扎，尤其在跨域身份匹配和长时序追踪上；ISYV-模型超越了强基线，部分性能接近闭源模型。

Conclusion: ISYV为以人为中心的视频推理提供了统一的任务定义、可扩展的数据集与基准以及建模见解。
...
### [2] [RoRA: Role-Oriented Regional Allocation for Visual Token Pruning in MLLMs](http://arxiv.org/abs/2608.07088v1)
*Qiyanhui Lu,Han Wu,Rongjian Xu,Tingzhang Luo,Cheng Fan,Xinghao Chen,Minjing Dong,Jufeng Yang,Jianyuan Guo*

Main category: cs.CV

TL;DR: 本文提出RoRA框架，通过面向角色的区域证据分配策略实现多模态大语言模型中无需训练的视觉token剪枝，在大幅剪枝的同时保持接近完整性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型将图像编码为长视觉token序列，导致预填充和KV缓存存储开销巨大；现有无训练剪枝方法将保留的token视为可互换，未显式追踪已被覆盖的物体相关区域，导致token保留策略不够精细。

Method: RoRA将视觉token剪枝建模为面向角色的区域证据分配问题，将token分为受保护语义核心、互补上下文和细粒度细节三类。首先利用位置先验和提示校准的物体先验校准文本条件注意力，然后从高置信度锚点构建注意力锚定区域（AARs）作为已覆盖物体支撑的轻量代理；上下文探索主要在AAR外部进行，小部分AAR引导的预算用于恢复局部细节；成对相似度仅用于上下文阶段冗余过滤。

Result: 在LLaVA和Qwen-VL系列上，RoRA在匹配预算下持续优于强无训练基线。LLaVA-1.5上88.9%剪枝率下保留96.5%完整性能；Qwen3-VL上75-90%剪枝率下比D2Pruner提升约5%。66.7%剪枝率下token选择仅需0.7ms，端到端推理时间减少24.6%，在H800上实现1.33倍加速。

Conclusion: RoRA证明了通过显式区域覆盖追踪和角色导向的token分配，可以在无需训练的条件下实现高效且高保真的视觉token剪枝，为多模态大语言模型的实际部署提供了有效的推理加速方案。
...
### [3] [Debias in Text, Believe Your Eyes: Text-Anchored Cross-Modal Transfer for Visual Counter-Commonsense Reasoning](http://arxiv.org/abs/2608.06938v1)
...
TL;DR: 本文提出了一种无需视觉训练数据的文本锚定后训练框架（TACT），通过消除共享语言解码器的常识先验偏见，提升了多模态大语言模型的反常识视觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常认为多模态大语言模型在反常识视觉推理上的失败源于视觉感知不足，但本文实证分析发现瓶颈不在于视觉感知，而在于共享语言解码器在面临先验-证据冲突时倾向于主导语言先验（尤其是低频事实场景）。

Method: 提出包含事实频率蒸馏（FFD）的文本锚定数据构建流水线以生成高质量文本语料库，并在此基础上引入文本锚定后训练框架TACT，将证据跟随和先验驱动的推理轨迹路由至不同优化阶段，从而在无需视觉训练数据的条件下对语言解码器进行去偏。

Result: 在反常识视觉基准测试中，TACT显著提升了视觉推理能力，同时保持了模型的通用能力，证明了有效的文本到视觉跨模态迁移。

Conclusion: 解决多模态模型反常识推理瓶颈应聚焦于消除语言解码器的先验偏见而非仅增强视觉输入；TACT框架通过文本锚定的去偏后训练成功实现了跨模态迁移，为解决先验-证据冲突提供了有效方案。
...
### [4] [Multi-Agent Forensic Reasoning for Generalizable Deepfake Video Detection](http://arxiv.org/abs/2608.06865v1)
*Xuechao Zou,Shun Zhang,Kai Li,Yi Zhou,Xinyu Sun,Yuhui Chen,Zhe Wu,Congyan Lang,Junliang Xing*

Main category: cs.CV

TL;DR: 本文提出了大规模深伪视频数据集FaceVid-Forensics-100K及多智能体取证推理框架，有效提升了深伪视频检测的泛化能力和准确率。


<details>
  <summary>Details</summary>
Motivation: 针对现有深伪视频基准对最新合成方法覆盖不足且缺乏细粒度文本注释，以及传统检测器与多模态大语言模型难以捕捉细微伪造痕迹导致泛化能力差的问题。

Method: 构建了包含10万个视频、覆盖33种合成方法的数据集，利用多模型聚合与冲突解决流水线生成细粒度文本注释；并提出多智能体取证推理框架，通过四个领域专家智能体（纹理、光照、运动、物理）独立分析，再由裁判智能体汇总得出最终预测和解释。

Result: 在域外测试集上的评估表明，该框架即使完全由小型开源多模态大语言模型组成，也优于包括GPT和Gemini在内的所有方法，并在所有报告指标上排名第一。

Conclusion: 提出的数据集和多智能体框架成功克服了现有深伪检测的局限性，为AI安全提供了强大的新基准和最先进的检测方法。
...
### [5] [GraphVerse: A Comprehensive Visual Graph Reasoning Benchmark for Multimodal Large Language Models](http://arxiv.org/abs/2608.06769v1)
*Yuanfu Sun,Yuanhang Ren,Kang Li,Chuanhao Ji,Jiaxi Li,Jiajin Liu,Ninghao Liu,Qiaoyu Tan*

Main category: cs.CV

TL;DR: 本文提出了GraphVerse基准，用于评估多模态大语言模型在视觉图推理任务中的感知、视觉推理和文本推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉图推理基准存在将任务简化为感知后文本推理、仅限单图设置、仅依赖答案准确率指标、且缺乏真实图中心场景等问题，无法全面评估MLLMs对结构化视觉信息的真实推理能力。

Method: 提出统一的GraphVerse基准，涵盖单图和双图设置，联合评估感知、视觉推理和文本图推理；核心为图中心图像编辑（GIE）策略，在保持语义的前提下修改图图像以测试视觉推理；并提出过程敏感的评估指标VGR-Score，超越最终答案准确率来衡量推理质量。

Result: 大量实验揭示了当前MLLMs在视觉图推理中的多项关键局限性，同时验证了GIE策略的有效性以及GraphVerse向更广泛多模态推理能力的可迁移性。

Conclusion: GraphVerse为评估MLLMs的结构化视觉推理能力提供了更全面的基准，暴露了当前模型的不足，未来可据此推动多模态推理能力的改进与迁移。
...
### [6] [Answer First, Reason Later: Commitment Order in Diffusion LLMs](http://arxiv.org/abs/2608.05687v1)
*Jewon Yeom,Jaewon Sok,Seonghyeon Park,Jeongjae Park,Hwiyeong Lee,Taesup Kim*

Main category: cs.CL

TL;DR: 本文揭示了掩码扩散语言模型在推理任务中因无序承诺导致的失败机制，并提出前沿门控承诺干预方法来恢复性能。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型（dLLMs）的任意顺序承诺能力被认为是其核心优势，但在推理任务中这种自由反而成为失败根源。本文旨在探究无序解码如何在推理过程中导致模型过早给出最终答案并崩溃为仅输出答案的模式，以及如何解决这一推理病态问题。

Method: 通过对LLaDA-8B在GSM8K上的解码轨迹进行逐承诺日志记录，采用2×2提示-解码器实验设计分析链式思维与承诺顺序的交互效应，并提出单一旋钮干预方法——前沿门控承诺（frontier-gated commitment），在保留并行解码能力的同时约束承诺顺序。

Result: 无约束解码在15-24%轨迹处即承诺最终答案，且在多达90%的问题上崩溃为仅输出答案；链式思维仅在有序承诺下有效（交互效应+34.8个百分点）；前沿门控承诺将准确率从0.528恢复至0.852，同时保留高达4倍并行解码速度，并在Dream-7B和MATH-500上得到复现。

Conclusion: 现有窗口式采样器此前被效率动机驱动，实际上无意中成为解决推理病态的最小修复方案。本文将无序承诺重新定义为推理任务的轴心失败因素，为扩散语言模型的解码策略设计提供了新的理论视角。
...
### [7] [OPTD: On-Policy Transition Distillation with Consistency-Guided Adaptive Compression for Few-Step Diffusion Language Models](http://arxiv.org/abs/2608.02942v1)
*Xiaocheng Lu,Hualei Zhang,Shuhan Guo,Jie Zhang,Xiaoyi Pang,Jian Liu,Haoxi Li,Bohai Gu,Haoxuan Che,Jingcai Guo,Song Guo*

Main category: cs.CL

TL;DR: 本文提出了OPTD方法，通过一致性引导的自适应在线策略转移蒸馏，解决了扩散语言模型少步蒸馏中的状态漂移与转移步长选择问题，无需标准响应即可显著提升质量与效率的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型的少步蒸馏方法依赖离策略轨迹，导致学生模型在推理时因早期并行预测改变上下文而产生状态漂移。直接采用在线策略蒸馏则面临难以确定每次转移应推进多远的困境：匹配单步限制了压缩，盲目合并多步则可能违反中间依赖。

Method: 提出OPTD（On-Policy Transition Distillation）。从学生模型自身轨迹采样部分状态，使用冻结的仅问题教师模型识别结果对齐的未来候选者并按置信度排序，选择保持教师展开结果的最长前缀进行自适应压缩。训练时采用集瓶颈目标提升已验证候选者的置信度，并用冻结教师KL散度对其他位置进行正则化，全程无需标准答案。

Result: 在四个数学推理和代码生成基准测试中，OPTD持续改善了质量与效率的权衡，在所有评估的少步基线模型中取得了最强的质量受限AUP（曲线下面积）。

Conclusion: OPTD通过一致性引导的自适应在线策略蒸馏，成功解决了少步扩散语言模型蒸馏中的上下文不匹配和依赖破坏问题，在不依赖黄金响应的情况下实现了高效且高质量的快速推理。
...
### [8] [Faster but Different: Diagnosing and Controlling Content Drift in Accelerated Multimodal Diffusion Language Models](http://arxiv.org/abs/2607.29079v1)
*Yaoxuan Dou,Yang Shu*

Main category: cs.CL

TL;DR: 本文研究了扩散多模态大语言模型免训练加速导致的服务时一致性问题，并通过缩短KV缓存刷新间隔提出了一种实现层面的一致性控制方法。


<details>
  <summary>Details</summary>
Motivation: 免训练加速方法在使扩散多模态大语言模型更易于部署的同时，可能会悄无声息地改变生成内容，本文旨在诊断并解决这种服务时的一致性漂移问题。

Method: 通过在300张真实图像上对比Fast-dLLM加速与未加速的输出，利用置信度阈值调整、状态刷新消融和图像交换干预来诊断漂移原因，并提出缩短KV缓存刷新间隔以控制一致性。

Result: 研究发现过时的视觉和生成文本状态导致了漂移；缩短KV缓存刷新间隔可实现1.3倍加速且达到近乎完全的一致性；盲评显示加速与基线的事实错误差异为0.00；固定间隔刷新优于自适应或平滑刷新变体。

Conclusion: 本文的主要贡献在于提供了一种配对诊断方法和局限于实现层面的一致性控制方案，而非准确性或安全性保证。
...
TL;DR: 本文提出基于轨迹的在线策略蒸馏（TOPD）框架，无需奖励估计即可高效将推理能力迁移到扩散大语言模型（dLLM）。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（dLLM）的推理后训练面临挑战：监督微调（SFT）需要密集但通常偏离策略的掩码状态，而强化学习（RL）则依赖稀疏奖励或价值建模。

Method: 提出基于轨迹的在线策略蒸馏（TOPD），通过从目标dLLM采样在线扩散轨迹，在部分去噪状态下获取教师模型的token分布，并使用token级别的反向KL散度（Reverse-KL）目标更新目标dLLM。

Result: 在数学推理基准上，TOPD使SDAR-4B-Chat达到与RL训练的TraDo-4B-Instruct相当的MATH500准确率（静态评估提升5.7，动态评估提升4.5），且所需rollout轮次减少4倍，模型计算速度提升约96倍。

Conclusion: TOPD通过将密集教师监督与模型自身的去噪状态对齐，有效提升了dLLM的推理能力，且在训练效率上显著优于强化学习方法。
...
### [10] [DominoTree: Conditional Tree-Structured Drafting with Domino for Speculative Decoding](http://arxiv.org/abs/2607.08642v2)
*Saw S. Lin,Jyh-Shing Roger Jang*

Main category: cs.CL

TL;DR: 本文提出DominoTree，一种无需训练的贪心最优草稿树构建方法，利用Domino的路径依赖条件修正实现高效推测解码。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿器（如DFlash）仅建模逐位置边缘分布，而最优优先树方法（如DDTree）的因子化公式无法表示Domino草稿器中基于GRU的因果修正所带来的路径依赖token分布，导致无法充分利用条件化草稿信息。

Method: DominoTree沿每条根到节点路径使用Domino的条件化、非因子化修正对草稿树进行评分，通过将每节点修正限制在候选top-M集合中使其可行；并采用GPU原生的CUDA-graph构建器生成草稿树，保证与Python参考实现位级一致。

Result: 在Qwen3-4B上八个基准测试中，实现最高6.6倍于自回归解码的加速，在所有测试温度下均取得最高的平均接受长度（最高10.7 tokens/轮）；相比Domino解码器吞吐量提升9%-10%（Alpaca上最高22%），在所有温度下优于DDTree和CaDDTree；在Qwen3-8B上T=0时相对DDTree吞吐量提升24%。

Conclusion: DominoTree通过有效利用路径依赖的条件修正信息，在保持草稿树构建低成本的同时显著提升了推测解码的接受长度和吞吐量，在贪心解码和采样温度下均具有竞争力，为LLM推理加速提供了更优的草稿树构建方案。
...
TL;DR: 本文提出CaDDTree方法，通过显式建模延迟并联合优化树结构和节点预算，直接最大化推测解码中的词元吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的DDTree方法在固定节点预算下最大化期望接受长度，这总是倾向于生成更大的树而忽略了验证成本，缺乏选择预算的原则性基础。

Method: 提出CaDDTree（成本感知扩散草拟树），显式建模草拟和验证延迟，联合选择树结构和节点预算以优化词元吞吐量。该方法将吞吐量目标分解为每轮对预算的一维搜索，证明在凸验证成本下吞吐量函数为单峰，从而实现高效的贪心停止规则，无需离线搜索即可逐轮自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型及涵盖推理、编程和指令遵循的8个基准测试上，CaDDTree在几乎所有任务上均匹配或超越了具备先验预算选择的DDTree。

Conclusion: CaDDTree为推测解码中的块扩散草拟树提供了一种原则性且成本感知的动态预算选择方案，有效提升了推理吞吐量，且无需离线参数调整。
...
TL;DR: 提出DDTree方法，通过块扩散模型的位置分布构建草稿树并在单次前向传播中验证，从而提升推测解码的性能和接受长度。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash块扩散草稿模型每轮仅验证单一草稿轨迹，限制了推测解码的接受长度。

Method: 提出DDTree，直接利用块扩散草稿模型的逐位置分布构建草稿树；在固定节点预算下，采用最佳优先堆算法和代理指标选择最优续写，并使用仅祖先注意力掩码在目标模型单次前向传播中完成验证。

Result: DDTree在DFlash基础上提升了推测解码性能，使其成为该领域的领先方法之一。

Conclusion: 通过将块扩散草稿模型扩展为草稿树验证机制，DDTree有效突破了单轨迹验证的瓶颈，提升了自回归语言模型的加速效果。
...
### [13] [CustomDance: Customized 3D Dance Generation with Coarse-to-Fine Human-Centered Interactive Control](http://arxiv.org/abs/2608.06722v1)
*Xulong Tang,Kaixing Yang,Xiaohu Guo,Prabhakaran Balakrishnan,Rawan Alghofaili*

Main category: cs.HC

TL;DR: 本文提出CustomDance，一个由粗到细的交互式系统，通过结合多模态大模型、检索器和扩散修复模型，实现对多模态输入的全面控制以生成定制化3D舞蹈。


<details>
  <summary>Details</summary>
Motivation: 现有的3D舞蹈生成方法无法全面且独立地控制多模态输入（如音乐和文本描述），导致生成的动作缺乏深度、表现力，且难以契合用户的创作愿景。

Method: 提出受编舞师工作流启发的三阶段系统：1) 利用多模态大语言模型分析音乐和文本以提取时间锚点和创作线索；2) 使用多模态检索器为各锚点从舞蹈库中检索高质量动作片段；3) 通过定制的音乐条件扩散修复模型无缝连接片段，并支持用户迭代优化与可视化。

Result: 定量和定性评估表明，CustomDance在各项对比中均优于竞争基线方法，并突显了AI辅助编舞范式的创作实用性和赋能潜力。

Conclusion: CustomDance有效解决了多模态控制不足的问题，实现了高质量、高度定制化且契合用户意图的3D舞蹈生成，为AI辅助编舞提供了新的范式。
...
### [14] [Token Communication for Multimodal Large Language Model](http://arxiv.org/abs/2608.07279v1)
*Jingkai Ying,Zhijin Qin,Yuan Shen,Khaled B. Letaief*

Main category: eess.SP

TL;DR: 本文提出一种面向多模态大语言模型的令牌通信框架，通过集成神经编解码器和自适应适配器，在减少传输数据量的同时保持多模态理解性能。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型（MLLM）的快速部署，令牌的高效传输变得日益重要。本文旨在解决如何在减少与MLLM交互时的传输数据量的同时，保持其多模态理解性能的问题。

Method: 提出一种专为MLLM设计的令牌通信框架：将神经编解码器集成到视觉分词器中以控制传输比特数；在接收端，解码隐变量通过两条路径处理（解码器重建图像作为先验，适配器将其转化为视觉令牌并注入视觉分词器中间层）；设计了两阶段视觉-语言语义对齐训练方案（蒸馏损失预热与对齐损失）；并引入基于特征级线性调制的自适应适配器以支持多种编解码率。

Result: 在多个MLLM基准测试上的大量仿真表明，在相同传输数据量下，该方案的识别任务性能优于其他用于MLLM的图像处理方案。

Conclusion: 该框架成功实现了MLLM令牌的高效传输，在不牺牲多模态理解能力的前提下显著降低了通信开销，为多模态模型的实际部署提供了有效的通信优化途径。
...
### [15] [Interaction Creates Dynamical AI Behavior Absent in Isolation](http://arxiv.org/abs/2608.07457v1)
*Bella Xinrui Li,Frank Yingjie Huo,Neil F Johnson*

Main category: cs.AI

TL;DR: 本文研究了AI智能体交互时的反常行为动态，并利用动力学理论揭示了信息传递方式对AI行为状态的影响。


<details>
  <summary>Details</summary>
Motivation: 旨在解决AI智能体在日常交互中（如一方指挥另一方）会产生何种行为动态的问题，以理解非平衡物理学在AI交互中的新应用。

Method: 提出一种简单的动力学理论来捕捉AI智能体交互过程中的主要效应。

Result: 研究发现，无论指挥者AI是否倾听回复，都会驱使下属AI进入一种单独运行时不会出现的异类行为状态；且即使两者解码温度相同，下属AI也不会模仿指挥者。

Conclusion: 信息传递方式在未来的AI-AI交互中至关重要，且该现象为非平衡物理学的研究开辟了新的途径。
...
### [16] [SkillProx: Self-Evolving Agent Skills via Proximal Textual Gradient Descent](http://arxiv.org/abs/2608.07449v1)
*Mingxuan Zheng,Yujin Zhou,Chuxue Cao,Boqin Yin,Yuyao Zhang,Jiapeng Sun,Shuaishuai Gong,Sirui Han,Yike Guo*

Main category: cs.AI

TL;DR: 本文提出SkillProx框架，结合闭环诊断与效用感知的近端细化来优化大语言模型代理的技能，有效提升了准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM代理技能优化框架缺乏明确的诊断-结果反馈，且未将删除作为巩固知识的专门机制，仅将其视为通用的编辑操作。

Method: 提出受近端梯度启发的双向框架SkillProx。前向阶段重新执行诊断驱动的编辑并回滚退步，将结果反馈至后续诊断；后向阶段将技能分解为知识单元，利用留一效用审计评估贡献，并执行验证门控的巩固、降级或删除。

Result: 在多个骨干模型的分布内外基准测试中，SkillProx较最强基线平均准确率提升3.0个百分点，消融实验验证了闭环诊断与近端细化的互补作用。

Conclusion: SkillProx通过平衡任务损失与技能复杂度，有效巩固了代理的累积知识，提升了其在重复任务中的适应性与性能。
...
### [17] [Blast Radius](http://arxiv.org/abs/2608.07440v1)
*MY Pitsane,Hope Mogale*

Main category: cs.AI

TL;DR: 本文提出了Blast Radius预测性内存管理层，通过可逆剔除无用上下文来解决智能体编程中的Token浪费和成本问题。


<details>
  <summary>Details</summary>
Motivation: 智能体编程面临日益严重的经济可负担性和Token浪费问题。

Method: 提出Blast Radius预测性内存管理层，结合上下文和代码通道估计提示词影响范围；利用NECROPHORESIS存档死上下文实现可逆剔除，通过RDM识别并掩埋重复记录，并在Polish上下文空间中构建可逆上下文剔除机制。

Result: 在7个OpenAI模型测试中，Token消耗降低17-26%，溢出率最低且保持字节级精确可逆；450个被掩埋记录中378个为重复死记录且零召回。

Conclusion: Blast Radius有效决定记录剔除及提示词影响范围，为实现大模型和智能体编程的可重用性与可持续性做出贡献。
...
### [18] [PsychoAgent: An Affect-Sensitive Cognitive Architecture for Conflict-Aware Memory in LLM Agents](http://arxiv.org/abs/2608.07438v1)
*Mohammad Amanlou,Parham Abed Azad,Farbod Davoodi,Mostafa Masumi,Behnam Bahrak,Abdol-Hossein Vahabie*

Main category: cs.AI

TL;DR: 提出PsychoAgent，一种将事实与情感记忆分离并通过冲突感知执行控制器整合的LLM智能体认知架构，以改善冲突场景下的记忆检索。


<details>
  <summary>Details</summary>
Motivation: 类人认知在选择过往经验时不仅依赖主题相似性，还受到情感重要性和未解决冲突的影响。现有系统缺乏这种情感敏感的记忆检索机制。

Method: 提出PsychoAgent认知架构，分离事实和情感记忆，并通过冲突感知执行控制器进行整合。情感记忆首先通过语义相关性过滤，然后根据显著性重新排序，在保持主题契合度的同时允许情感重要的记忆进入提示。

Result: 在三个受控冲突场景中，完整架构检索到的关键冲突记忆（0.933）多于基线（0.500和0.667），但存在轻微的语义相似性损失。5名盲评者对27个输出的评估显示，完整架构的平均得分最高（+0.22 SD），但校正后的成对差异不显著。三天的追踪显示了情感持久性、离线记忆重组和选择性记忆重新加权。

Conclusion: 研究结果支持将情感敏感检索作为一种可检查的机制，用于在LLM智能体中建模类人冲突效应。
...
### [19] [Fisher-R1: Training LLM Agents for Reliable Hypothesis Testing](http://arxiv.org/abs/2608.07437v1)
*Jiacheng Miao,Jin Mu,Guanhua Chen,James Zou*

Main category: cs.AI

TL;DR: 本文构建了假设检验基准P-Bench，并提出通过强化学习训练的LLM代理Fisher-R1，以解决大模型在自动化假设检验中存在的统计推理错误问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型代理在自动化假设检验中常犯微妙的推理错误导致结论错误，而现有基准测试未能评估报告的p值在给定数据假设下是否在统计上有效，无法捕捉这一失败模式。

Method: 构建了包含425个跨学科现实任务的P-Bench基准，并引入Fisher-R1，一个使用合成任务和强化学习进行严格假设检验训练的开源LLM代理。

Result: Fisher-R1-14B在P-Bench上显著优于骨干模型及GPT-5.4、DeepSeek-V4-Pro等基线模型，相比DeepSeek-V4-Pro单次试验成功率平均相对提升21%，在最困难任务上提升高达26%。

Conclusion: 当前LLM代理缺乏可靠的统计推理能力，而在具有经验证的统计奖励的任务上进行强化学习能显著提升其假设检验的可靠性。
...
### [20] [TEPA: Revoking Stale Memories for Conflict-Robust Language Agents](http://arxiv.org/abs/2608.07429v1)
*Yan Zhou,Yue Ouyang,Kaiyang Zheng,Suncheng Xiang*

Main category: cs.AI

TL;DR: 本文提出TEPA机制，通过显式撤销被新证据取代的过时记忆，解决了语言代理中的记忆污染问题。


<details>
  <summary>Details</summary>
Motivation: 旨在解决语言代理在长期记忆中因环境变化导致的“记忆污染”问题，即过时且与新证据冲突的记忆仍被检索并污染提示。

Method: 提出TEPA（可撤销证据记忆机制），将观察表示为带键的先例，当相同键下出现冲突的新证据时撤销旧记忆的激活状态，同时保留撤销历史以供审计。

Result: 在概念漂移和文件执行测试中，TEPA得分（0.950）远高于仅追加（约0.21）和无记忆（约0.30）基线；在单跳事实基准上表现相当，但在多跳和长上下文设置中暴露了检索瓶颈。

Conclusion: 生命周期撤销应成为处理演变知识的代理的核心记忆操作，未来需解决多跳和长上下文的检索链与上下文选择瓶颈。
...
### [21] [ResidencyRL: Reinforcement Learning in Simulated Clinical Environments](http://arxiv.org/abs/2608.07418v1)
...
TL;DR: 本文提出了ResidencyRL，一种通过模拟多轮临床问诊来训练临床AI智能体的强化学习方法，显著提升了诊断准确性、安全性和跨基准的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在静态医学基准上表现优异，但优化完整临床决策序列（如医患问诊对话）的方法仍不成熟。目前的AI缺乏类似医生住院医师培训期间那种通过大量多轮互动逐步积累临床推理能力的机制。

Method: ResidencyRL采用强化学习，通过模拟多轮临床问诊轨迹（最多60轮对话和8次工具调用）训练策略智能体。该方法将其与具备复杂对抗行为的LLM模拟器配对，并基于涵盖诊断准确性、管理质量、沟通、病历记录和安全性的结构化奖励机制进行优化。

Result: 在对抗条件下，训练后的智能体诊断准确率提升7.0%（88.0% vs 81.0%），漏诊危急重症率降低31%；在87.6%的盲测对比中获专家医生青睐。其能力可迁移至未见过的基准，在AMIE多就诊基准的全部六个临床轴上超越基础模型，并在AgentClinic和CRAFT-MD上保持一致提升。

Conclusion: 序列临床决策能力可通过模拟多轮强化学习有效习得，从而产生稳健且可泛化的临床能力。然而，要确立其真正的临床效用，仍需在真实世界的临床工作流中进行前瞻性验证。
...
### [22] [LLaDA MoE v2: Scaling Mixture-of-Experts Diffusion Language Models](http://arxiv.org/abs/2608.03457v1)
...
TL;DR: 本文系统研究了混合专家扩散语言模型的扩展行为规律，并据此训练出具备竞争力的LLaDA MoE v2模型。


<details>
  <summary>Details</summary>
Motivation: 混合专家扩散语言模型的扩展行为尚不明确，需要系统评估其与自回归模型在扩展趋势上的定量差异以指导模型设计。

Method: 系统刻画MoE dLLMs的优化超参数、算力分配和架构扩展规律，并据此从零开始在23.5T tokens上训练了30B-A3B参数的LLaDA MoE v2模型。

Result: 研究发现MoE dLLMs的最优批大小增长更快、学习率衰减更快，算力分配偏向数据侧。LLaDA MoE v2仅用Qwen3约65%的预训练tokens，即可在多项基准中接近Qwen3，并在SFT后在8项推理和编码测试中有7项优于SDAR Chat。

Conclusion: 研究结果为MoE扩散语言模型确立了实用的扩展定律和架构设计原则，证明了其作为自回归模型有效替代方案的潜力。
...
### [23] [Escaping Confidence Trap: Evolutionary Decoding for Mathematical Reasoning in Diffusion LLMs](http://arxiv.org/abs/2608.00605v1)
*Zhenhong Sun,Hanqing Zhao,Yatao Bian,Rongcheng Tu,Liuyue Xie,Xu Zhang,Jue Wang,Davide Modolo,Daoyi Dong,Dacheng Tao*

Main category: cs.AI

TL;DR: 本文提出了一种无需训练的测试时扩展框架Evolutionary Decoding，通过将扩散解码视为进化过程，解决了扩散大语言模型在数学推理中的置信度陷阱问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（如LLaDA 2.0）在数学推理任务上表现不可靠，主要原因是局部Token置信度与全局推理正确性在渐进块解码过程中出现错位（即扩散置信度陷阱），导致采样敏感型与采样一致型推理失败。

Method: 提出Evolutionary Decoding框架，将扩散解码视为候选推理状态的进化过程，结合逐步选择（保留有效数值符号信号并抑制重复模式）和块级变异（引入结构化替代方案以逃逸高置信度的错误区域）。

Result: 在多个基准测试上的实验表明，该方法使LLaDA 2.0在数学推理能力上超越了基于置信度的解码方法。

Conclusion: Evolutionary Decoding能有效缓解扩散大语言模型的置信度陷阱，显著提升其数学推理的可靠性。
...
### [24] [FlowBlock: Wavefront-Parallel Decoding for Self-Correcting Diffusion Language Models](http://arxiv.org/abs/2607.17652v1)
*Bing Tian,Haikun Liu,Xiaocheng Zhong,Zhuohui Duan,Zhaokai Luo,Huayi Jin,Zhiyong Wang,Xiaofei Liao*

Main category: cs.AI

TL;DR: 提出无需训练的并行解码框架FlowBlock，通过门控波前解码和异构波前打包机制，解决块级扩散大模型块间解码的串行瓶颈。


<details>
  <summary>Details</summary>
Motivation: 旨在解决块级扩散大语言模型块间解码严格串行导致效率低下的问题，同时避免现有后训练方法加速有限且损害准确率的缺陷。

Method: 提出FlowBlock框架，无需训练。核心包含两点机制：一是门控波前解码，利用就绪门控引入块，通过token-to-token编辑共同优化活跃块，并在窗口化块因果掩码下按序提交以复用KV缓存；二是异构波前打包，将异步窗口打包为密集且形状稳定的批量前向传播。

Result: 相比LLaDA-2.1和LLaDA-2.0，吞吐量分别提升高达2.95倍和4.01倍，延迟分别降低53.6%和77.1%，平均准确率提升1.3点。相比基于训练的D2F基线，准确率更高且批量服务吞吐量提升高达16倍。

Conclusion: FlowBlock成功将块最终性从硬依赖转化为调度资源，在无需额外训练的情况下实现了显著的解码加速、延迟降低和准确率提升。
...
TL;DR: 本文提出了TAPS方法，通过目标感知的前缀选择策略优化扩散模型推测解码中的验证过程，从而在固定验证预算下实现高效的无损端到端加速。


<details>
  <summary>Details</summary>
Motivation: 使用扩散模型进行并行起草时，验证阶段成为新的瓶颈。现有草稿树方法基于边缘概率对节点排序，忽略了验证是基于前缀条件的，导致验证被拒绝前缀的不可达后代，增加了延迟且接受率提升有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边缘概率转换为路径条件接受估计，并在固定验证预算下选择紧凑的前缀闭合子树，以优化接受率与成本的权衡。

Result: 在多种数据集和模型上的实验表明，TAPS相比普通自回归解码实现了高达7.9倍的无损端到端加速，且分别比最先进的DFlash和DDTree方法快1.36倍和1.74倍。

Conclusion: TAPS通过优化验证策略而非单纯扩大草稿树，有效提升了推测解码的性能和效率，实现了更优的接受成本权衡。
...
### [26] [Science Edge Evaluation: SEE the Missing Step Toward Real Scientific Discovery](http://arxiv.org/abs/2608.06931v1)
...
TL;DR: 本文提出了SEE多模态基准评估模型在实验室科学中的表现，发现当前多模态大型语言模型在基于实验证据进行可靠推理方面仍存在不足。


<details>
  <summary>Details</summary>
Motivation: 旨在评估大型语言模型是否能够支持复杂的真实实验室科学，特别是它们能否在实验证据的边界内进行可靠的科学推理和推断。

Method: 提出Science Edge Evaluation (SEE)多模态基准，包含基于同行评审文献和实验实践（化学、生物、材料科学）的专家策划问题，并评估了19个多模态大型语言模型在视觉代理及工具使用下的表现。

Result: 表现最好的模型准确率仅为48.7%，且通用模型平均表现优于科学专用模型；使用工具将最高准确率提升至52.7%，但更多信息的获取并未必提升科学推理的可靠性。

Conclusion: 当前多模态大型语言模型仍无法从实验结果中可靠地做出有证据支撑的推断；模型需要从解释既有科学概念向从实验数据中推导新颖且基于证据的见解转变。
...
### [27] [AgentPatch: Coarse-to-Fine Weak-Task Repair for Merging Agentic Multimodal Large Language Models](http://arxiv.org/abs/2608.06699v1)
*Zibo Shao,Baochen Xiong,Chengdong Xu,Linhui Xiao,Kaichen Li,Haoran Gong,Yan Li,Yaguang Song,Xiaoshan Yang*

Main category: cs.AI

TL;DR: 本文提出AgentPatch，一个无需训练的从粗到细修复框架，通过解决能力不对称保留和行为关键遗忘问题，将多个专业化智能体多模态大模型合并为单一通用模型。


<details>
  <summary>Details</summary>
Motivation: 当前的智能体多模态大语言模型通常针对特定工具或环境进行特化，难以整合为单一的通用模型。合并过程中存在两个挑战：一是不同交互复杂度的能力保留不均（不对称能力保留），导致合并后产生弱任务；二是丢失决定性动作的行为关键遗忘会破坏长时程执行。

Method: 提出AgentPatch框架，选择稳定的合并骨干网络，通过“弱任务唯一残差恢复”机制恢复被稀释的弱任务特定信号，并应用“智能体引导的行为关键补丁”在显式能力保护下恢复决定性行为，最终生成无需路由或集成的单一静态检查点。

Result: 在六个智能体和多模态基准上的实验表明，AgentPatch改进了多种合并骨干网络，缓解了弱任务性能退化，并在弱任务恢复与互补性搜索及智能体视觉处理能力保留之间取得了更好的平衡。

Conclusion: AgentPatch成功解决了智能体多模态大模型合并中的不对称能力保留和行为关键遗忘挑战，实现了无需额外路由或集成的单模型整合，为构建通用智能体模型提供了有效途径。
...
TL;DR: 本文揭示了扩散式大语言模型（DLLMs）的安全对齐机制漏洞，并提出了一种低成本的离线黑盒越狱框架SN-Guided Diffusion。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型（DLLMs）的安全机制尚不明确，且存在从自回归模型继承的稀疏且可转移的安全特征漏洞，本文旨在揭示并利用这些基于扩散对齐的机制脆弱性。

Method: 首先通过安全神经元映射与剪枝验证DLLMs安全对齐的稀疏性和可转移性；随后提出SN-Guided Diffusion离线黑盒越狱框架，利用加权安全神经元损失引导扩散过程避开安全触发区域。

Result: 神经元剪枝使攻击成功率（ASR）大幅提升（最高达86.6%）；SN-Guided Diffusion在区分良性与越狱提示时AUROC达1.0，在多个模型上实现高达86.9%的迁移ASR，且生成成本比现有方法低数个数量级。

Conclusion: DLLMs的安全对齐存在严重的机制脆弱性，易受跨架构迁移攻击；所提方法以极低计算成本实现了高效越狱，突显了重新评估和加强DLLMs安全机制的必要性。
...
TL;DR: 本文提出块混合注意力机制，通过线性化扩散语言模型对先前块的注意力来加速推理，在保持性能的同时显著提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在分块半自回归解码时，每个去噪步骤仍需对所有前缀块进行注意力计算，导致重复的前缀注意力开销，形成推理瓶颈。

Method: 提出块混合注意力机制，在当前去噪块内保留精确的softmax注意力，对先前块应用线性注意力；利用LoLCAT方法将其迁移至预训练的16B规模dLLM（LLaDA-2.1）中。

Result: 模型转换仅需约60小时，在HumanEval、MBPP+和CMATH等基准上性能基本保持，且基于Triton的实现使解码吞吐量提升高达1.7倍，并支持更多并发请求。

Conclusion: 预训练的扩散语言模型可被高效线性化以实现更快的推理，且不会造成显著的性能损失。
...
TL;DR: 本文提出了ERILS方法，通过引入外部更强策略生成的高奖励rollout并结合长度控制与分源奖励处理，解决了扩散大语言模型在on-policy rollout成功样本稀缺时强化学习训练进展有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型的强化学习方法依赖目标模型自身生成的on-policy rollout，当成功rollout稀缺时，训练难以获得正向奖励且进展有限。直接引入外部更强策略的rollout又面临rollout长度差异和不同来源奖励联合处理的不稳定性问题。

Method: 提出ERILS方法，包含两个核心组件：一是长度控制，对外部rollout的长度进行约束以消除与on-policy rollout的长度差异；二是分源奖励处理，将on-policy和外部rollout的奖励分别独立处理，避免联合处理导致的训练崩溃。

Result: 在Sudoku、Countdown和MATH500三个任务的零样本评估中，ERILS在多样本性能上均有提升，其中Sudoku提升最大：best-of-4完成准确率达98.4%，最强基线仅40.3%；在128、256、512 token生成长度下均保持约90%的确定性单次完成准确率。组件分析表明长度控制的外部rollout优于无控制版本，分源奖励处理有效避免了联合处理的训练崩溃。

Conclusion: 在将外部rollout集成到dLLM强化学习时，rollout构建方式和奖励处理是关键的设计维度。长度控制和分源奖励处理能有效提升训练稳定性和模型性能。
...
### [31] [An AI4AI Framework for Visual Token Pruning](http://arxiv.org/abs/2608.07193v1)
*Zhen Liu,Wenli Huang,Wei Song,Yuhan Liu,Zhiqin Yang,Jingwen Fu*

Main category: cs.LG

TL;DR: 本文提出AutoPrune，一个由LLM驱动的免训练框架，通过领域特定语言自动设计视觉token剪枝策略，在移除94.4%视觉token的同时保持99%以上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法依赖固定的人工启发式规则和昂贵的专家试错，随着剪枝目标、预算和模型架构的多样化，手动探索设计空间越来越困难。需要一种自动化方法来设计有效的视觉token剪枝算法。

Method: 提出AutoPrune框架，核心是引入Token Pruning Domain-Specific Language (TPDSL)，包含131个可复用原子，涵盖预算控制、token评分、选择约束和token重组。关键设计是将每个搜索状态表示为强基线策略的残差修改，从而缩小搜索空间并引导LLM关注对性能影响最大的策略组件。

Result: 在14个多模态基准和3个MLLM骨干网络上验证了有效性、效率和迁移性。即使移除94.4%的视觉token，AutoPrune仍保持超过99%的全token性能，同时FLOPs降低9.9倍，预填充延迟降低6.4倍。

Conclusion: AutoPrune证明了LLM可以自动设计有效的视觉token剪枝策略，关键在于设计合适的搜索状态表示以连接LLM内部知识与剪枝任务的结构约束，为AI4AI框架在视觉token剪枝领域的应用提供了有效方案。
...
### [32] [Bayesian Fair Division: Truthfulness in Picking Sequence with Correlated Valuations](http://arxiv.org/abs/2608.07414v1)
*Xiaolin Bu,Biaoshuai Tao*

Main category: cs.GT

TL;DR: 本文在贝叶斯公平分配模型下研究了序贯分配机制的诚实性，发现两人且估值正相关时说真话是均衡，但在多于两人时诚实性失效。


<details>
  <summary>Details</summary>
Motivation: 传统序贯分配机制因智能体拥有完全信息和估值差异而缺乏诚实性。本文旨在探究当智能体仅拥有部分且大致一致的他人估值信息时，机制是否能提升诚实性。

Method: 采用贝叶斯公平分配模型，精确刻画了激励智能体说真话的估值“大致一致性”程度，并分析了两人及多人场景下的均衡与操纵策略。

Result: 在两人场景下，当估值正相关时，说真话构成贝叶斯纳什均衡；但在多于两名智能体时，诚实性无法扩展，且存在一种新型操纵策略。

Conclusion: 研究揭示了序贯分配机制在多人场景下诚实性的根本局限性，表明两人场景的诚实性结论无法简单推广至多智能体环境。
...
### [33] [Analyzing the Interaction of Optimal Strategies in Mean-Payoff Bidding Games](http://arxiv.org/abs/2608.07383v1)
*Shaull Almagor,Guy Avni,Julian Ewaied*

Main category: cs.GT

TL;DR: 本文分析了多智能体系统在所有智能体均采取对抗性假设时的实际交互行为，证明了图上竞价博弈在特定限制下具有最终周期性，并提出了计算玩家效用的算法。


<details>
  <summary>Details</summary>
Motivation: 智能体通常假设其他智能体采取对抗行为以获取最强保证，但当所有智能体都采取此策略时，实际交互并非对抗性。由于分析这种多智能体交互在数学和算法上极具挑战性，本文旨在揭示并分析这种情况下产生的实际行为。

Method: 聚焦于具有平均收益目标的图上双人竞价博弈，分析每个智能体遵循针对对抗者优化的策略时所生成的博弈路径，并考察两种已知的显式最优策略构造。

Result: 研究表明，在某些限制条件下，生成的博弈路径是最终周期性的，并开发了用于计算玩家效用的算法。

Conclusion: 成功解决了具有复杂动态和无限配置的竞价博弈分析难题，为对抗性假设下的非对抗性交互行为提供了理论证明和效用计算方法。
...
### [34] [Strategy-first synthesis planning for complex natural products](http://arxiv.org/abs/2608.07454v1)
...
TL;DR: 本文提出了基于大语言模型的智能体框架SynthEx，用于设计复杂天然产物的合成路线，其表现达到了专家级别并超越了传统算法。


<details>
  <summary>Details</summary>
Motivation: 现有的自动化逆合成设计工具过度依赖目录化反应和基准测试，在处理需要创新性化学的密集官能化和多环复杂天然产物时屡屡受挫。

Method: 提出SynthEx框架，利用大型语言模型提出竞争策略，将常规与关键步骤组合成连贯路线，并对自身设计进行自我批评与改进。

Result: SynthEx生成的路线更具收敛性，覆盖了传统工具无法匹配的反应空间；在盲测中，专家认为其关键步骤可与人类合成相媲美。同时发布了包含千余种天然产物路线的开源数据库SynthAtlas。

Conclusion: 基于大语言模型的智能体能够像专家一样设计复杂分子合成，SynthAtlas数据库将成为缺乏文献路线的复杂目标分子的重要共享资源。
