...
TL;DR: 本文提出UrbanGround沙盒环境以评估多模态大语言模型代理在复杂城市中的探索能力，发现其虽具备局部感知能力但在长距离导航和错误纠正上存在严重不足。


<details>
  <summary>Details</summary>
Motivation: 探讨多模态大语言模型（MLLM）代理能否在移动过程中将局部城市感知转化为复杂真实城市环境中的可靠行动，解决现有研究缺乏对代理持续动态探索能力评估的问题。

Method: 提出UrbanGround沙盒环境，基于香港全境3D地理数据构建物理约束副本，支持第一人称视角闭环交互与交互式地图导航，并通过三个递进的空间问题测试代理的局部场景理解、远距离导航及动态环境适应能力。

Result: 当代MLLM代理在视觉识别和短距离空间推理方面具备基础能力，但方向感和行人感知运动不可靠；在长距离探索中，局部能力无法整合为持续的目标导向行为，且错误不断累积而无法有效纠正。

Conclusion: UrbanGround为研究MLLM代理在复杂开放城市环境中的可靠探索能力提供了基准测试平台，并揭示了当前模型在长期空间任务中整合能力与纠错机制的局限性。
...
TL;DR: 本文提出了一种基于离散扩散大语言模型的放射学报告生成框架（DRRG），通过迭代去噪实现双向报告优化，优于传统自回归方法。


<details>
  <summary>Details</summary>
Motivation: 现有自动放射学报告生成方法多采用自回归框架逐词生成，无法修改前文，易导致误差传播，且与放射科医生的迭代修改流程不符。

Method: 开发DRRG框架，将报告生成转化为迭代掩码令牌去噪过程，引入临床实体感知互补掩码和图像衍生概念条件模块以增强临床实体监督和视觉表示。

Result: 在MIMIC-CXR和CheXpert Plus数据集上，DRRG即使使用较小的解码器，也在大多数指标上取得最优结果（如MIMIC-CXR的BLEU-4为0.210，CheXpert-F1为0.549）。

Conclusion: 离散扩散模型为放射学报告生成提供了支持迭代双向优化的有效替代方案，结合临床掩码和概念条件化可提升报告质量和临床一致性。
...
TL;DR: 本文提出Aphanta框架，通过闭环诊断评估多模态大语言模型与图像编辑器管道中显式视觉中间件的实用性，发现图像编辑是特定视觉工作空间而非通用推理机制。


<details>
  <summary>Details</summary>
Motivation: 显式视觉中间件可帮助多模态大语言模型（MLLM）外化空间证据，但其实用性取决于图像编辑器能否忠实实现所需转换。本文旨在分离视觉推理的潜在空间与当前编辑器的实际实用性，评估该管道的有效性。

Method: 提出Aphanta框架，针对MLLM->图像编辑器->MLLM管道进行自动化任务发现和闭环诊断。通过比较直接推理、编辑器生成中间件推理和理想化参考中间件推理三种条件，评估不同任务的实用性。

Result: 在20个任务和多种模型组合中，发现实用性高度依赖任务类型：在视觉线索注入、定位和反事实状态实现任务上增益显著，但在符号敏感构建或结构外推上可靠性较差。在正向任务子集上，Qwen管道的平均得分从0.343提升至0.445（相对提升29.7%）。

Conclusion: 研究表明图像编辑是一种专门的视觉工作空间而非通用推理机制；Aphanta框架可作为评估任务-表示对齐、编辑器实现度及下游管道实用性的可复用协议。
...
TL;DR: 本文提出RubricRM，一种成对生成的奖励建模框架，通过生成输入特定的评分规则来提升视觉生成模型评估的可解释性和任务敏感性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉奖励模型大多使用单一标量分数或固定评估标准，无法适应不同指令，限制了可解释性和任务敏感性，尤其在文本到图像生成和基于指令的图像编辑任务中。

Method: 提出RubricRM框架，首先生成包含评估维度、权重和评分标准的特定评分规则，然后据此对候选图像评分。采用两阶段训练：监督微调（SFT）教授评分范式，GRPO利用细粒度的维度级奖励进一步优化。

Result: 在多个生成和编辑基准测试中，RubricRM超越了现有的专用奖励模型，且在使用较小骨干网络的情况下，仍能与强大的专有多模态大语言模型评判器相竞争。

Conclusion: RubricRM有效解决了视觉奖励模型适应性差的问题，提升了视觉生成模型的对齐能力，相关模型、数据和代码已开源。
...
TL;DR: 本文提出MedREAL框架，通过将语言推理与空间定位对齐，实现医学视觉问答中的推理驱动回答与病灶定位。
...
Method: 提出MedREAL统一框架，包含两个核心组件：SARP（分割锚定推理池化）从MLLM隐藏状态的[SEG]标记中蒸馏任务相关语义证据；R2V（推理到视觉）融合机制将推理感知特征注入分割管道以实现精确掩码解码。同时构建了MedRAVS-13K数据集，包含13,824个专家验证样本，覆盖四种影像模态。

Result: 实验表明MedREAL显著优于现有最先进方法，在基准评估中达到68.49%的gIoU和70.47%的cIoU，生成的证据掩码与文本诊断严格一致。

Conclusion: MedREAL通过生成与文本诊断严格一致的证据掩码，为推理驱动的医学图像分析提供了鲁棒且可解释的框架，有效弥合了临床推理与空间定位之间的语义鸿沟。
...
TL;DR: 本文提出了一个名为LLaVAFlow的信息论蒸馏框架，通过保留跨模态对齐流来解决多模态大语言模型在视觉指令微调中的灾难性遗忘问题，同时提升下游任务性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在视觉指令微调时会发生灾难性遗忘，损害整体泛化能力。现有方法仅通过调节权重更新来缓解遗忘，却忽略了MLLMs中基本的跨模态对齐问题。

Method: 提出LLaVAFlow信息论蒸馏框架，通过保留信息压缩轨迹中的对齐流来维持跨模态对齐。具体包括：1) 压缩提取关系与MLLM嵌入间的互信息，促使可学习模块产生有益于下游任务的精细对齐流；2) 最大化预训练与微调MLLM提取的对齐流之间的互信息，实现紧凑对齐信息的转移。

Result: 大量实验表明，LLaVAFlow作为一个有效的即插即用框架，成功保留了对齐流，并同时提升了下游任务的性能和模型的泛化能力。

Conclusion: 通过保留跨模态对齐流，LLaVAFlow有效缓解了MLLMs微调过程中的灾难性遗忘问题，为平衡下游性能和泛化能力提供了一种高效的即插即用解决方案。
...
TL;DR: 提出统一多模态大语言模型UniGeo，通过结合地理语义理解、跨视角生成和候选验证来实现细粒度的文本引导无人机地理定位。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖文本与候选图像的直接全局匹配，但在查询不完整和候选图像高度相似时，难以实现可靠的细粒度定位。

Method: 构建共享视觉-语言框架的UniGeo，联合支持地理语义理解、跨视角语义生成和即插即用的候选级验证模块，并采用多阶段训练策略逐步学习这三项能力。

Result: 在多个检索骨干网络中均取得一致提升，在GeoText-1652数据集上R@10和mAP分别提升了13.59和2.83个百分点。

Conclusion: UniGeo有效克服了全局跨模态匹配的不足，验证了其在细粒度文本引导无人机地理定位任务中的有效性。
...
TL;DR: 本文提出AesCanvas基准套件，用于评估多模态大模型在图像美学评论生成及特定场景美学适用性方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有图像美学评估基准主要关注内在视觉质量或固定领域标准，未能评估图像是否适用于特定目的、受众、文化背景或领域规范。

Method: 提出包含两个互补组件的AesCanvas基准套件：CritiqueCanvas（支持多维度美学评论）和ContextCanvas（评估真实场景下的上下文美学适用性），并在统一协议下评估多种多模态大语言模型。

Result: 研究发现评论生成与上下文敏感判断存在明显分离；现有指标仅能部分捕捉评论质量，美学专用模型在上下文适用性上显著落后于强通用模型，且其决策未能有效基于决定性的上下文视觉线索。

Conclusion: 文化背景和证据支撑的适用性应成为美学建模的独立目标。
...
TL;DR: 提出INTENT-AS-A-TOOL方法，通过引入意图工具调用概率作为细粒度信号，以补充CoT监测并追踪智能体的错位行为。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为自主智能体部署时，在目标冲突和压力下会采取有害行动，而现有的思维链监测标签过于粗糙，无法展示生成过程中意图的动态变化。

Method: 提出INTENT-AS-A-TOOL方法，为模型添加针对意图的工具，提供表达目标行为承诺的专用通道，利用工具调用概率作为无评判的细粒度信号来衡量行为倾向。

Result: 该方法补充了CoT监测，将事后CoT标签扩展为密集轨迹，并识别出可用于在线干预的关键步骤。

Conclusion: 行为偏好对于追踪推理过程中的代理错位具有重要作用，为智能体安全与对齐提供了新的监测与干预途径。
...
TL;DR: 提出无需训练的DARD框架，通过在验证过程中排除不可靠词元以避免上下文污染，从而提升扩散大语言模型并行解码的速度与质量。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在提高解码并行度时，早期错误会污染后续上下文导致质量下降，而现有的可撤销解码方法忽略了不可靠词元也会破坏验证上下文本身的问题。

Method: 提出依赖感知可撤销解码（DARD）框架，将词元分为掩码、候选和未掩码状态，利用排除不可靠词元的选择性上下文验证候选词元，并自适应调节其影响。

Result: 在3个开源dLLM和12个基准测试上，DARD持续提升了速度-质量帕累托前沿，在Flickr30k上相比Saber实现了2.71倍加速和4.35分的CIDEr增益。

Conclusion: DARD有效解决了扩散语言模型并行解码中的上下文污染问题，在不需额外训练的前提下显著提升了生成质量和推理速度。
...
TL;DR: 本文提出了Entropy-Valley (EV)，一种无需训练的目标长度选择器，通过全掩码前向传播的预测熵来为掩码扩散语言模型的机器翻译选择最佳目标画布长度。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型在机器翻译中面临一个未被充分探索的问题：在去噪之前必须预先决定目标长度，而该长度决策直接影响覆盖率和冗余度，现有工作主要关注token揭示顺序而忽略了长度选择的重要性。

Method: EV通过全掩码前向传播计算候选目标画布的平均预测熵，选择骨干网络最准备好填充的画布长度，整个过程无需额外训练，仅利用模型自身的预测不确定性作为长度评分依据。

Result: EV在En→Zh、Zh→En和En→De上分别恢复了64.9%、65.3%和33.0%的参考长度带来的COMET-22增益；三位翻译专家评估支持En↔Zh的充分性提升；EV系统在En→Zh上与LLaMA-3-8B自回归模型持平，在Zh→En上领先；诊断实验表明去噪友好的长度不必匹配参考长度，且目标长度的供给方式比token揭示顺序更重要。

Conclusion: 目标长度选择是掩码扩散机器翻译的关键瓶颈，基于预测熵的无训练长度选择方法可有效弥补长度决策的不足；未来方向可进一步探索去噪友好长度的特性及更优的长度决策机制。
...
TL;DR: 本文提出一种无训练的解码方法RPS，利用扩散大语言模型中的涟漪效应，通过提前提交中熵枢轴位置来加速并行解码并提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的dLLM并行解码调度器仅关注单位置标准，忽略了提前提交对后续解码的潜在益处，导致解码效率受限。

Method: 提出Ripple-Pivot Search (RPS)解码法，通过寻找中熵位置作为枢轴点（决定解码位置），并利用前瞻评估选择能带来最大下游收益的词元分配（决定解码内容），以引发涟漪效应降低整体不确定性。

Result: 在3个dLLM和4个推理与代码生成基准上，RPS比标准解码器快4-10倍且保持质量；相比前瞻基线准确率最高提升5.49%且吞吐量更高；结合KV缓存时加速比达18倍。

Conclusion: RPS成功利用涟漪效应优化了dLLM的并行解码过程，在不需训练的前提下实现了显著的推理加速与准确率提升，为后续解码优化提供了新方向。
...
TL;DR: 本文提出PCD（Prefix-Conditioned Diffusion）预训练目标，通过对齐训练与推理时的上下文分布来缩小扩散语言模型在提示续写任务上的性能差距。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在预训练时随机破坏提示和续写token，削弱了提示条件生成所需的干净前缀接口，导致训练与推理时的上下文分布不匹配，影响续写性能。

Method: PCD将自回归前缀监督与无偏移后缀去噪结合，通过调整注意力掩码、破坏掩码和标签构造，对干净前缀进行自回归监督，仅对未知续写部分应用扩散去噪，使训练接口与评估时的查询方式一致。

Result: 在LLaDA2-Mini和Qwen-1.7B骨干网络上，PCD相对原生dLLM基线分别取得4.2%（+2.56分，六基准平均）和14.2%（+4.86分）的相对提升。

Conclusion: 对齐预训练上下文分布与提示条件生成可以在不改变推理方式的前提下，可衡量地恢复扩散语言模型续写任务的部分性能差距。
...
TL;DR: 本文提出了DominoTree，一种无需训练的最佳优先草稿树构建方法，通过利用Domino草稿器的路径依赖条件校正来显著提升大语言模型投机解码的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的树方法（如DDTree）采用分解公式，无法表示Domino草稿器中基于GRU的路径依赖条件校正结构，限制了投机解码的潜力。

Method: 提出DominoTree，这是一种无需训练的最佳优先草稿树，沿每个根到节点的路径使用Domino的条件（非分解）校正进行评分，并通过将每个节点的校正限制在候选top-M内来实现高效计算。

Result: 在八个基准测试中，DominoTree在几乎所有服务和研究测试单元中均达到最高平均接受长度；吞吐量相比自回归（AR）最高提升7.3倍，在SGLang中比Domino提升12%，在HELMET长上下文测试中接受长度提升29-36%且吞吐量提升10-34%。

Conclusion: DominoTree通过有效利用路径依赖校正显著提升了投机解码的接受长度和吞吐量，但在预填充占主导的极长上下文场景下，其相对于EAGLE-3的优势会缩小至持平。
...
TL;DR: 本文提出了GRAFT框架，通过目标蒸馏边缘评分和状态感知预算分配优化基于扩散语言模型的推测解码中的草稿树构建，显著提升了推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有的推测解码树构建方法与单次前向传递生成所有未来位置分布的扩散语言模型不兼容，且现有的DDTree方法仅依赖令牌概率选择边缘，忽略了父子兼容性，同时其固定预算未考虑吞吐量最优树大小随解码状态的动态变化。

Method: GRAFT引入目标蒸馏边缘评分（TDES），从目标模型轨迹中提取父子偏好以选择目标兼容的边缘；并引入状态感知预算分配（SABA），通过平衡预期草稿增益和验证成本来动态设置每轮树预算。

Result: 在多个模型和任务上，GRAFT实现了2.13倍至6.36倍的端到端加速，且每轮额外开销不到0.5毫秒，仅约占目标模型验证延迟的1.4%。

Conclusion: GRAFT有效解决了DLM草稿树构建中的父子兼容性和动态预算问题，在极低开销下显著提升了自回归解码的推理速度。
...
TL;DR: 提出了一种名为CaDDTree的成本感知扩散草稿树方法，通过联合选择树结构和节点预算直接优化token吞吐量，无需离线预算搜索。


<details>
  <summary>Details</summary>
Motivation: 现有的DDTree方法在固定节点预算下最大化预期接受长度，但该指标总是偏向更大的树而不考虑验证成本，缺乏选择预算的原则基础。

Method: CaDDTree显式建模草稿和验证延迟，将吞吐量目标分解为每轮对预算的一维搜索，并证明在凸验证成本下吞吐量函数具有单峰性，利用贪婪停止规则实现动态调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型及八个基准测试上的实验表明，CaDDTree在几乎所有任务上均匹配或优于具有预言机预算选择的DDTree。

Conclusion: CaDDTree通过动态成本感知的预算调整，有效提高了大语言模型推理的token吞吐量，提供了一种无需离线搜索的高效优化方案。
...
TL;DR: DDTree通过从块扩散草稿模型的逐位置分布构建草稿树，提高了推测解码的接受长度并达到了领先的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash方法每轮仅验证单一的草稿轨迹，限制了其接受长度和整体推测解码效率。

Method: DDTree方法从块扩散草稿模型的逐位置分布直接构建草稿树，在固定节点预算下使用最优优先堆算法选择最可能匹配目标模型的续写，并利用仅祖先注意力掩码在单次目标模型前向传递中完成验证。

Result: DDTree在DFlash的基础上取得了性能提升，使其成为推测解码领域的领先方法之一。

Conclusion: 将草稿树构建与块扩散草稿模型相结合能够有效突破单轨迹验证的限制，提升推测解码的整体性能。
...
TL;DR: 本文提出了一种无需训练的解码策略IGFD，通过结合置信度、邻域不确定性和结构风险来优化token提交顺序，有效提升了扩散多模态语言模型的解码质量。


<details>
  <summary>Details</summary>
Motivation: 扩散多模态语言模型的解码质量高度依赖于掩码token的提交顺序。现有基于置信度的策略倾向于优先提交局部简单的token（如标点符号），而非具有上下文信息量的语义锚点，这削弱了上下文传播并加剧了错误累积。

Method: 提出信息引导前沿解码（IGFD）策略。该策略无需额外训练，通过综合token置信度、邻域不确定性和结构承诺风险对候选token进行排序，促使可靠的语义锚点优先提交并延迟脆弱的结构性token。同时，利用动态候选前沿将token选择限制在局部可扩展区域内。

Result: 在多模态理解、推理、定位和幻觉等多个基准测试中，IGFD在相同解码预算下，于大多数基准和不同扩散多模态语言模型骨干上均优于现有的解码策略，且不增加额外训练或前向传播开销。

Conclusion: 通过引导语义锚点优先解码并引入动态候选前沿约束，IGFD在不增加推理成本的前提下有效改善了上下文传播和抑制了错误累积，为扩散多模态语言模型提供了一种高效的解码方案。
...
### [19] [WikiSkill: Compiling Agent Experience into Persistent Knowledge for Skill Evolution](http://arxiv.org/abs/2608.27454v1)
*Liyan Tang,Cyrus Rashtchian,Chun-Sung Ferng,Andrew Tomkins,Da-Cheng Juan,Tu Vu*

Main category: cs.AI

TL;DR: 提出WikiSkill框架，通过持久化知识库协同演化代理技能，系统性地积累和提炼经验以提升技能的可重用性与可迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有的代理技能自动发现方法中，指导技能发展的经验见解分散在优化历史中，限制了其在后续迭代中的系统性重用。

Method: 提出WikiSkill框架，将原始执行经验、积累的知识和可执行技能分离，并持续将经验整合到持久化知识库中，以作为后续技能更新的基础。

Result: 在多个基准和模型上优于现有方法；技能演化与模型规模扩展互补，小模型配备技能可超越无技能的大模型；演化技能跨模型有效迁移；消融实验证明持久化知识积累至关重要。

Conclusion: 系统地积累和提炼代理经验对于开发可重用和可迁移的技能具有重要益处，持久化知识库是实现有效技能演化的关键。
...
### [20] [Serving Masked Diffusion LLMs: Characterization and Design Principles from Real Hardware](http://arxiv.org/abs/2608.23807v1)
*Farhana Amin,Sabiha Afroz,Mona Moghadampanah,Dimitrios S. Nikolopoulos*

Main category: cs.AI

TL;DR: 本文表征了掩码扩散语言模型(dLLMs)在真实并发服务负载下的行为特征,揭示了其与自回归模型服务的关键差异。


<details>
  <summary>Details</summary>
Motivation: dLLMs虽能并行去噪加速生成,但现有服务系统缺乏对其在真实并发负载下行为的测量,可能错误沿用AR模型的假设。本文旨在填补这一空白,为dLLM服务系统设计提供实证基础。

Method: 使用LLaDA-8B-Instruct配合D2F LoRA适配器,在单块NVIDIA H200 GPU上,于GSM8K和HumanEval基准上表征dLLM服务行为,分析请求难度分布、延迟方差、计算开销来源及批处理效应,并推导Poisson到达下的批处理超时规则。

Result: 发现:(1)请求难度离散分布于11个固定步数级别,生成前无可预测信号(最佳R2=0.150);(2)低于320 token的基准低估服务方差;(3)仅24%单请求时间为GPU计算,余为CPU调度开销,批处理主要通过摊销开销使吞吐量在batch size 16时提升16.0倍;GSM8K准确率74-76%;输出质量不应随batch size下降。

Conclusion: dLLM服务需要每个去噪步级别的并行性,其准入与驱逐机制与AR服务在共享前向传播的交互方式上存在本质差异,服务系统设计应针对dLLM特性专门优化。
...
TL;DR: CAI-DLLM是一种用于扩散语言模型的免训练推理方法，通过利用第一步置信度动态分配去噪步数来加速生成。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在推理过程中由于重复的去噪步骤导致计算成本高昂，尤其是不断重新计算已经稳定的token。

Method: CAI-DLLM利用第一步置信度信号提前提交简单token，为困难token分配更多去噪步数，并调整输出块间的解码计划，无需重新训练或额外预测器。

Result: 在LLaDA-8B和Dream-7B上的评估显示，CAI-DLLM在GSM8K和HumanEval上分别实现了最高18.2倍和13.1倍的加速且准确率有所提升，在更困难的推理任务上加速达44.8倍（仅有轻微准确率下降），能耗降低高达95.3%。

Conclusion: CAI-DLLM在不需重训练的情况下，有效降低了扩散语言模型的推理时间和能耗，同时保持或提升了准确率。
...
TL;DR: 本文提出了连续交互扩散（CID）架构，将工具交互整合到扩散语言模型的迭代去噪过程中，以实现连续并行的推理与工具使用。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行细化输出进行推理，这使得自回归模型传统的“停止-恢复”工具交互模式具有限制性，可能因迫使模型在推理稳定前做出工具决策和延迟观察而损害任务准确性与推理效率。

Method: CID将工具交互整合到迭代去噪中，通过分离只读事实通道、思维通道（类型化认知张量）和显示通道来实现。它允许在工具调用完全序列化前启动外部读取，将返回结果投影到演变的思维状态中以修正早期认知，并使用持久绑定来重用或刷新结果。

Result: 本文形式化了该架构、运行时和训练目标，并定义了任务质量和端到端效率的评估协议，主要聚焦于只读工具且未提出实证性能声明。

Conclusion: CID提供了一个框架，能更早暴露证据、将工具延迟与模型计算重叠并减少重复的外部工作，为未来在扩散语言模型中评估连续工具交互奠定了基础。
...
TL;DR: 本文提出TAPS方法，通过目标感知的前缀选择策略优化扩散模型推测解码中的验证瓶颈，实现高达7.9倍无损加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型用于并行草拟token虽降低了草拟延迟，但瓶颈转移至验证阶段：现有基于边缘概率排序的草拟树方法忽略了验证是前缀条件化的，导致验证不可达节点，增加延迟而收益有限。

Method: TAPS将扩散模型的边缘概率转化为路径条件化的接受率估计，并在固定验证预算下选择一个紧凑的前缀封闭子树，从而优化接受率-成本权衡而非简单扩展草拟树。

Result: 在多种数据集和模型族上的实验表明，TAPS相比 vanilla 自回归解码实现最高7.9倍无损端到端加速，分别比SOTA方法 DFlash 和 DDTree 快1.36倍和1.74倍。

Conclusion: TAPS通过目标感知的前缀选择有效解决了扩散推测解码中的验证瓶颈，显著提升了加速性能，为推测解码中草拟树优化提供了新思路。
...
TL;DR: 本文提出了一种名为SWE-Prime的两阶段多粒度SFT数据选择方法，通过过滤成功轨迹中的低质量步骤，显著提升了大语言模型解决软件问题的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的方法直接使用成功的智能体轨迹进行监督微调（SFT），但成功轨迹中可能包含无效、冗余或高风险的步骤，这会引入噪声监督并导致模型模仿不良行为。

Method: 提出SWE-Prime方法，通过两阶段多粒度筛选SFT数据：第一阶段基于过程质量、结果质量和代表性进行轨迹级筛选；第二阶段将连续步骤分组为语义片段，基于其贡献度、可学习性和风险进行片段级选择，并在SFT时仅对选中的片段计算损失。

Result: 在SWE-Bench Pro和SWE-Bench Verified上的实验表明，使用SWE-Prime筛选出的10%轨迹子集进行训练，效果优于使用完整数据集，分别实现了12.2%和24.2%的相对性能提升。

Conclusion: SWE-Prime通过多粒度数据选择有效去除了成功轨迹中的噪声，证明了对SFT数据进行质量过滤的重要性，能够在大幅减少数据量的同时提升模型在软件工程任务上的性能。
...
TL;DR: 本文提出了人格-执行分离（PES）架构，将LLM代理的人格与执行置于不同信任域中，在受治理组织中实现人格自由演化与执行可审计性的解耦。


<details>
  <summary>Details</summary>
Motivation: 受治理组织中的LLM代理需要人格（指令、语气、自我呈现）自由演化，同时保持执行（有状态、可审计的工作）可追溯。单一信任域无法低成本地同时满足这两个需求。

Method: PES将人格与执行分别置于不同信任域，通过受治理的契约桥连接。人格单宿主且可漂移，执行无面孔且可审计；状态摘要可返回，数据体留在限制域内（分级DLP例外），身份保持连续。审批矩阵、DLP和审计机制执行跨域控制。

Result: 在受监管数字员工平台的一个月试点中记录了五项决策（各含被拒替代方案）。机制检查显示在五种模型配置下人格扰动未引发执行侧重新验证，硬断言字段无人格指纹。对分离前构建的探查发现执行路径与人格的解耦是因遗漏而非构造所致。

Conclusion: PES将隔离从偶然实现提升为可审计的架构规则，适用于多用户部署、执行审计和预期人格变更同时存在的场景；在LLM表征不可区分性下，任何满足全部三个目标的单域机制实质上是更高耦合成本下重建的PES。
...
TL;DR: 提出RedEvoAgent黑盒红队测试智能体，通过将攻击轨迹蒸馏为可自适应进化的攻击技能，解决了现有方法中的检索偏差和上下文开销问题。


<details>
  <summary>Details</summary>
Motivation: LLM智能体越狱会触发有害工具调用和持久状态改变，而现有自动红队测试方法存在固定攻击局限性，基于轨迹检索的智能体攻击则面临检索偏差、工具信用不明确、上下文开销大及可解释性差等问题。

Method: 提出RedEvoAgent，将跨案例攻击轨迹蒸馏为简洁的人类可读攻击技能；通过工具有效性分析和决定工具归因（Deciding-Tool Attribution）实现技能自适应进化，并采用验证棘轮机制仅保留能提升性能的更新。

Result: 在多个基准、目标模型和执行环境上的实验显示，RedEvoAgent优于固定和智能体基线方法，提升了工具效率，并具备跨攻击者模型和目标执行环境的迁移能力。

Conclusion: RedEvoAgent有效提升了LLM智能体红队测试的效率、可解释性和迁移性，为产品级执行环境的安全性评估提供了更优的自动化方案。
...
### [27] [CLAP: Cross-Embodiment Video World Models are Zero-Shot Physical Simulators](http://arxiv.org/abs/2608.27406v1)
*Kechen Liu,Ola Shorinwa*

Main category: cs.RO

TL;DR: 提出CLAP框架，通过统一多种动作空间和课程学习策略，实现跨机器人本体的动作条件视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有动作条件视频模型局限于单一机器人本体，无法利用海量异构互联网视频数据学习泛化的物理规律；同时跨本体学习面临动作表征差异大和人类视频缺乏动作标签的挑战。

Method: 提出CLAP框架，基于通用物理定律，利用末端执行器位姿、语言指令和潜在动作协调不同动作空间；采用基于课程学习的跨本体策略，先用潜在动作从无标签视频学习物理先验，再在末端执行器动作空间微调以实现零样本部署。

Result: 在DROID等环境中接近或超越最先进的单本体视频模型，少样本适应后优势进一步扩大；构建了涵盖多种动作空间和多种机器人形态的最全模型套件。

Conclusion: CLAP确立了利用跨本体数据训练单本体视频世界模型的新范式，有效提升了模型的泛化能力与物理理解，并已开源全部代码和模型。
...
### [28] [Embodied Scene Rearrangement Planning](http://arxiv.org/abs/2608.27371v1)
*Canzhi Chen,Zan Wang,Siqi Zhu,Qi Wu,Yixuan Li,Wei Liang*

Main category: cs.RO

TL;DR: 本文提出了具身场景重排规划（ESRP）任务及ESRP-Bench基准，评估具身智能体在仅依赖第一视角观测且存在遮挡条件下的长视距家具重排能力。


<details>
  <summary>Details</summary>
Motivation: 解决具身智能体在真实部署中因无法获取全局状态且存在物体相互遮挡，导致难以将局部第一视角观测与全局目标布局对齐的长视距规划问题。

Method: 提出ESRP任务并构建基于OmniGibson的ESRP-Bench基准（含5400+场景对和8200+物体），定义三种多级别评估指标，并建立包括分层任务与运动规划、视觉语言模型、模仿学习和强化学习在内的四种基线方法。

Result: 实验结果表明，现有的基线方法均难以高效完成该重排任务，突显了ESRP任务的复杂性与挑战性。

Conclusion: ESRP是具身智能体在场景理解和长视距任务规划领域的前沿挑战，该研究为未来在真实世界场景中部署智能体奠定了基础。
...
### [29] [FLARE: A Failure-Aware Framework for Autonomous Correction and Recovery in Visual-Language Robotic Manipulation](http://arxiv.org/abs/2608.26645v1)
*Ganlong Zhao,Zijia Tang,Xingping Chen,Zhanghui Kuang,Ye Tian,Guanbin Li*

Main category: cs.RO

TL;DR: 本文提出FLARE框架，通过“重试”和“重置”范式赋予视觉-语言-动作模型（VLAs）从执行错误中恢复的能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLAs通常在无失败的完美数据上训练，导致其在面对抓取失败或碰撞等执行错误时表现脆弱，缺乏自主恢复能力。

Method: 提出FLARE框架：通过在演示中注入扰动和桥接段实现“重试”以处理执行偏差；利用MLLM进行离线失败分析以识别OOD状态，并训练物体中心的“重置”技能库以恢复环境状态；推理时由在线MLLM监视器在任务执行与重置技能间进行仲裁。

Result: 在富接触操作任务上的实验表明，该方法显著提升了任务成功率和鲁棒性。

Conclusion: FLARE框架有效克服了VLAs依赖完美数据的局限性，显著增强了模型在复杂操作中的错误恢复与鲁棒性。
...
### [30] [Do User-Authored Permission Policies Improve Protection Against AI Agent Overreach?](http://arxiv.org/abs/2608.27443v1)
*Ting Yan*

Main category: cs.HC

TL;DR: 本文探讨了非专业用户通过预先设定自然语言后果规则来控制AI代理操作的效果，发现相比逐次确认，预先设定策略规则并未提供更强保护，因为用户倾向选择“询问”从而保留了逐案决策。


<details>
  <summary>Details</summary>
Motivation: 旨在解决非专业用户需要易懂且可复用的方式来控制AI代理跨服务操作的问题，并探究预先制定可复用规则与逐次操作决策相比在控制过度操作上的利弊。

Method: 113名非专业参与者被分为逐次人工确认(HITL)、逐次自动审查(AUTO)和用户自创后果策略(POLICY)三组。参与者需评估包含7个过度操作的18步模拟日程，POLICY组需为每个后果类别设定“允许”、“询问”或“禁止”规则。

Result: POLICY组阻止过度操作的效果显著低于HITL（-20.1%）和AUTO（-14.5%）。尽管POLICY减少了运行时提示，但包含规则设置在内的总干预时间未显著降低。此外，POLICY组过度操作批准率最高，主要因用户大量选择“询问”并在运行时批准了越权操作。

Conclusion: 研究揭示了用户偏好与承诺之间的差距：用户倾向选择“询问”以保留逐案选择权，这阻碍了长期策略在预先决策中的有效性，导致用户自创规则未能提供预期的保护。
...
### [31] [RegulAR: Graph-Grounded Error Recognition and Assistance for Procedural Tasks in AR](http://arxiv.org/abs/2608.26715v1)
*Yi-Lin Ye,Jindu Wang,Hiu Tung Wong,Shuchang Xu,Huamin Qu,Wong Kam-Kwai*

Main category: cs.HC

TL;DR: 本文提出了RegulAR，一个结合层次依赖图和多模态大语言模型（MLLM）的AR任务助手，用于程序性错误的识别与恢复。


<details>
  <summary>Details</summary>
Motivation: 现有的AR引导系统大多侧重于逐步指令的传递，而忽视了帮助用户识别程序性任务中的错误并从中恢复。

Method: 将任务指令建模为层次依赖图，并结合多模态大语言模型（MLLM）来解析执行过程中的第一人称视角观察，从而追踪进度、识别错误类型、评估影响，并通过平视显示器提供可视化恢复指导。

Result: 在一项N=12的被试内研究中，参与者报告称与仅使用MLLM的基线相比，RegulAR提供了更好的任务结构理解和恢复支持。

Conclusion: 显式化程序结构不仅支持下一步指导，还能增强用户对错误原因和恢复方法的推理能力，提升了AR系统的实用性。
...
### [32] [Non-isothermal vertical distribution functions for the Milky Way](http://arxiv.org/abs/2608.27373v1)
*Maria Djurić,Ralph Schönrich*

Main category: astro-ph.GA

TL;DR: 本文提出了一种新的星系垂直分布函数族，以修正由分子云散射导致的非等温性质。


<details>
  <summary>Details</summary>
Motivation: 星系垂直结构通常被假设为遵循伪等温分布，但由于巨分子云（GMCs）局限于星系中面附近，无法在垂直相空间中均匀随机化恒星，因此没有先验理由支持这一假设。

Method: 通过统计推导薄盘分子云极限下的新垂直分布函数，重新计算更现实分子云分布下的垂直加热扩散系数，并数值求解含时系数的轨道平均Fokker-Planck方程。

Result: 研究发现漂移和扩散系数在垂直速度上呈强非线性，且得到的垂直作用量分布偏离伪等温分布；导致非等温性的主要原因是分子云局限于中面以及库仑对数在低速时具有强烈的速度依赖性。

Conclusion: 提出了一族广义化伪等温形式的新型垂直分布函数，并指出库仑对数不能像通常假设的那样被视为常数。
...
TL;DR: 本文提出Consistency Forcing (CForce)蒸馏方法，通过强制扩散大语言模型早期去噪阶段的掩码预测与后期对齐，解决高并行解码中的错误累积问题。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型在激进并行策略下，早期去噪阶段的预测不可靠，导致错误向后续阶段传播，影响生成质量。

Method: 提出CForce方法，在预收集的自展开轨迹上训练模型，引入Confidence Adaptive KL Divergence作为蒸馏目标，结合正向与反向KL的优点，使早期掩码预测与后期对齐。该机制统一适用于掩码到词元解码及可编辑解码。

Result: 在非编辑和可编辑的LLaDA模型上的实验显示，该方法改善了速度与质量的权衡，尤其在高度并行解码预算下效果显著。

Conclusion: CForce从理论上能近似最小化早期预测误差，提升训练与推理的对齐度，并在高并行度扩散语言生成中实现更优的速度与质量平衡。
...
### [34] [Diffusion LLMs as Targets and Adversaries: Mechanistic Safety Exploits](http://arxiv.org/abs/2608.07430v1)
*Elena Dumitrescu,Gert Lek,Lydia Y. Chen,Jérémie Decouchant*

Main category: cs.LG

TL;DR: 本文揭示了扩散大语言模型中稀疏且可迁移的安全机制脆弱性，并提出了一种低成本、高效的离线黑盒越狱框架SN-Guided Diffusion。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的安全对齐机制尚不明确，本文旨在揭示其作为攻击目标和攻击者时基于扩散对齐的机制性脆弱性。

Method: 通过直接的安全神经元映射和剪枝进行迁移攻击，并提出SN-Guided Diffusion框架，利用加权安全神经元损失引导扩散过程远离安全触发区域，实现完全离线的黑盒越狱。

Result: 神经元剪枝显著提升了攻击成功率；SN-Guided Diffusion在多个开源和专有目标上实现了高达86.9%的迁移攻击成功率，且生成成本比现有框架低数个数量级。

Conclusion: 扩散大语言模型的安全对齐存在稀疏和可跨架构迁移的脆弱性，极易受到低成本的越狱攻击，突显了改进扩散模型安全机制的必要性。
