...
TL;DR: 本文提出了DRRG，一种基于离散扩散大语言模型的放射学报告生成框架，通过迭代掩码去噪实现报告的双向细化与完善。


<details>
  <summary>Details</summary>
Motivation: 现有自回归方法逐词生成且无法修改早期内容，易导致错误传播且与放射学报告的迭代完善过程不符；而自然支持迭代细化的离散扩散模型在放射学报告生成中尚未被充分研究。

Method: 提出DRRG框架，将报告生成表述为迭代掩码token去噪；引入临床实体感知互补掩码以提高token监督覆盖率并强调重要临床实体，同时采用概念条件模块将图像衍生的临床概念注入视觉表示中。

Result: 在MIMIC-CXR和CheXpert Plus数据集上，DRRG在大多数指标（如BLEU-4、CheXpert-F1等）上优于对比方法，且在解码器模型规模显著更小的情况下取得了更好的表现。

Conclusion: 离散扩散通过实现迭代的、双向的报告完善，为自回归放射学报告生成提供了有效的替代方案；结合临床聚焦的掩码和图像衍生的概念条件能显著提升报告质量和临床一致性。
...
TL;DR: 本文提出First Things First Reinforcement Learning (FTF-rl)方法，通过优化多优先级用户需求的推理能力，提升多模态大语言模型在复杂真实任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在处理具有复杂、结构化要求的真实世界任务时表现极差，常常误解任务需求、违反必须满足的条件并产生无效解决方案。

Method: 提出First Things First Reinforcement Learning (FTF-rl)方法，显式优化对多优先级用户需求（必须满足、最好满足、无法满足需拒答）的推理；并构建了3649个真实服务场景问题集进行评估。

Result: 实验表明，FTF-rl相比强基线显著提高了任务成功率，并在LogicVista、MathVision和InfoQA等逻辑与数学推理任务上展现出泛化有效性。

Conclusion: 增强需求感知推理能力是提升多模态大语言模型智能体泛化能力的一条简单而有效的途径。
...
TL;DR: 本文提出LatentStream框架，将流媒体视频理解中的记忆机制从“存储与检索”转变为“检索与内化”，通过构建渐进式潜在工作记忆实现连续因果推理。


<details>
  <summary>Details</summary>
Motivation: 现有的流媒体视频理解方法通常将历史观测压缩为外部记忆库并进行检索，这阻碍了将历史证据内化为紧凑且不断演化的潜在记忆，从而无法持续指导流媒体推理。

Method: LatentStream包含三个协同组件：1）查询无关的层级流媒体记忆，通过Jenks引导的自适应整合在固定内存预算下组织视觉历史；2）层级潜在记忆演化，赋予潜在记忆令牌渐进扩展的感受野以检索并内化历史证据至固定长度的潜在记忆中；3）渐进置信度引导的潜在记忆优化，利用基于分组预测熵的层级递进奖励，联合优化潜在记忆令牌和检索到的证据。

Result: 广泛的实验表明，LatentStream在现有的在线和离线视频基准测试上取得了新的最优结果。

Conclusion: LatentStream成功弥合了流媒体记忆范式的鸿沟，通过将历史证据内化为演化的潜在记忆，显著提升了流媒体视频理解与推理能力。
...
TL;DR: 本文提出CORE方法，通过将多模态大语言模型重排器的组合判断能力蒸馏到嵌入模型中，显著提升了组合检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有的MLLM嵌入模型在组合检索中难以区分包含相同概念但属性-对象绑定不同的场景。鉴于相同骨干网络在作为交叉注意力重排器时能解决此问题，本文旨在将其组合判断能力蒸馏到嵌入模型中。

Method: 提出CORE框架，合成跨越五个组合匹配级别的候选列表，并引入Rank-KL目标函数训练嵌入模型以复现重排器的细粒度排序。同时对比了对比学习、成对CoSENT和列表级Rank-KL三种方法。

Result: Rank-KL在利用多级监督方面表现最佳。CORE-RERANKER-8B在三个基准上平均得分82.7%，超越Jina-Reranker 10.7分；CORE-EMBED-8B在所有评估的嵌入模型中取得最佳平均得分（0.666）。改进可迁移至MCMR基准，且未牺牲COCO和Flickr30K的检索性能。

Conclusion: 将重排器的组合判断能力蒸馏入嵌入模型是提升组合检索的有效途径，Rank-KL优于传统对比学习，且模型具备良好的泛化能力与基线保持能力。
...
TL;DR: 本文提出了一个无需训练的框架GraFT，通过紧凑的3D场景图（3DSG）为多模态大语言模型（MLLMs）提供缺失的3D空间结构，从而显著提升其3D空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在3D空间推理上表现不可靠，难以进行精确几何测量、视角转换和细粒度外观定位。现有的改进方法通常依赖于昂贵的大规模数据微调或绑定特定骨干网络的专用编码器。

Method: 提出GraFT框架，利用3DSG提供三种空间推理能力：基于符号工具的确定性几何、基于鸟瞰图（BEV）渲染的客观视角布局，以及基于任务相关第一人称视角的视觉属性定位。

Result: 在ScanQA基准上，GraFT全面超越同骨干基线，CIDEr提升27%；在VSI-Bench上，使冻结的MLLMs性能提升高达65%，超越了所有专有、通用开源以及多种微调的空间模型。

Conclusion: GraFT作为一种无需训练的框架，成功为MLLMs补充了3D空间结构，不仅在多项基准测试中取得显著领先，还避免了高昂的监督微调成本和骨干网络耦合问题。
...
TL;DR: 本文提出了一种基于InfoNCE风格的内在奖励机制，使多模态大语言模型无需预热监督微调即可学会使用放大工具处理高分辨率图像。


<details>
  <summary>Details</summary>
Motivation: 现代视觉智能体在处理高分辨率图像时需要使用放大工具，而以往方法通常需要大量预热的监督微调（SFT）阶段来教授此技能，这既低效又依赖额外标签。本文旨在解决此问题并消除对SFT的依赖。

Method: 提出一种无需额外标签或预热SFT的内在奖励方法。该方法采用InfoNCE风格的奖励，通过包含逐渐增加难度的负样本工具调用作为对比训练信号来学习工具使用。

Result: 在V*、HRBench和MME-RealWorld数据集上的实验表明，该方法在更具效率的同时表现出竞争力；作为SFT的直接替代方案时，甚至超越了所有基线。此外，利用新提出的Muffin&Chihuahua (M&C) 合成数据集及其感兴趣区域标签，发现召回率是衡量放大区域与最终任务性能关联的最强指标。

Conclusion: 研究证明，预热SFT对于学习放大工具使用并非必需。所提出的对比内在奖励方法更为高效且有效，且召回率能直接反映模型的放大能力，为视觉智能体的工具学习提供了新的方向。
...
TL;DR: 本文提出ShallowStream框架，利用多模态大语言模型的浅层网络进行帧编码和检索索引构建，显著降低了流式视频理解中的计算和延迟开销。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在处理连续流式视频流时计算成本过高，且现有降本方法忽略了模型深度维度，导致全深度预填充极其昂贵且KV缓存增长过快。

Method: ShallowStream利用MLLM的浅层同时进行帧编码和检索索引构建，在流处理期间使用浅层KV缓存维护轻量级索引；在查询时利用浅层注意力得分评估上下文帧，并结合多样性感知选择策略检索精确证据。

Result: 该方法在性能上与现有的最强流式方法相当，同时将每帧预填充延迟和10秒端到端延迟分别降低了高达52.1倍和11.9倍。

Conclusion: 通过利用模型浅层进行流式视频的编码与检索，ShallowStream在不牺牲性能的前提下大幅缓解了计算瓶颈，为实时视频理解应用提供了高效的解决方案。
...
TL;DR: 本文分析了扩散大语言模型在去噪过程中的安全行为，并提出了一种免训练的解码方法RAEC来增强安全性。


<details>
  <summary>Details</summary>
Motivation: 旨在解决扩散大语言模型通过迭代去噪生成文本时，生成时机和位置对安全对齐的影响问题。

Method: 提出了一种名为Refusal-Aware Early Commitment (RAEC)的免训练解码方法，在早期去噪步骤提交持久的拒绝信号。

Result: 在LLaDA和Dream模型上的实验表明，RAEC降低了攻击成功率，同时基本保持了模型的实用性。

Conclusion: 扩散大语言模型的拒绝信号集中在早期去噪步骤和响应前部，利用该特性通过RAEC方法可有效提升安全性。
...
TL;DR: 本文提出了一种轨迹级推测解码框架，通过置信度分层树探索和跨块前瞻机制，显著提升了基于扩散的语言模型的并行生成吞吐量和速度。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的语言模型在低置信度下会退化为单令牌生成，限制了吞吐量，且其多令牌更新的去噪轨迹特性使得传统的自回归推测解码方法不再适用。

Method: 提出一种轨迹级推测框架，利用置信度分层树探索构建草稿去噪轨迹，并通过带双向注意力掩码的块并行评估进行验证；同时引入块间推测以利用双向结构进行跨块前瞻。

Result: 该框架将去噪迭代次数减少30-40%，每步令牌数从2.6增至4.3，在准确率变化不到1%的情况下，相比原始dLLM实现7-14倍加速，相比Fast-dLLM实现1.3倍加速。

Conclusion: 轨迹级推测解码能有效克服dLLM的吞吐量瓶颈，在保持准确性的同时大幅提高推理速度，为扩散语言模型的高效解码提供了新方向。
...
TL;DR: 本文提出了一种名为DARD的免训练框架，通过分离标记状态并使用选择性上下文进行验证，解决了扩散大语言模型中并行解码质量下降的问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在增加解码并行度时，早期错误会污染后续上下文，从而降低生成质量。现有的可撤销解码方法忽略了不可靠标记对验证上下文本身的破坏。

Method: 提出依赖感知可撤销解码（DARD），将标记分为掩码、候选和未掩码状态。该框架使用排除了不可靠标记的选择性上下文来验证候选标记，并自适应地调节这些标记对后续解码的影响。

Result: 在3个开源dLLM的12个文本和多模态基准测试中，DARD持续提升了速度-质量帕累托前沿。与Saber相比，在Flickr30K上实现了2.71倍的加速和4.35分的CIDEr分数增益。

Conclusion: DARD有效解决了dLLM并行解码中的上下文污染问题，在保证生成质量的同时显著提升了推理速度，为扩散模型的解码优化提供了新方向。
...
TL;DR: 本文提出了一种名为Entropy-Valley (EV) 的免训练长度选择器，通过平均预测熵为掩码扩散语言模型的机器翻译选择最佳目标长度，显著提升了翻译质量。


<details>
  <summary>Details</summary>
Motivation: 在掩码扩散语言模型（dLLMs）的机器翻译任务中，去噪前需决定目标长度，但现有研究多关注于token的揭示顺序，而忽视了这一直接影响覆盖率和冗余度的长度决策问题。

Method: 引入Entropy-Valley (EV) 这一免训练的长度选择器。它通过全掩码前向传播计算候选目标画布的平均预测熵，并选择模型最准备好填充的画布长度。

Result: 相较于基于语料库长度统计的基线，EV在英译中、中译英和英译德任务上分别恢复了64.9%、65.3%和33.0%的参考长度带来的COMET-22增益。专家评估支持了英中双向的充分性提升。与同源微调数据的LLaMA-3-8B自回归模型相比，EV系统在英译中上打平，在中译英上领先。

Conclusion: 适合去噪的长度不必与参考长度匹配。在当前的掩码扩散机器翻译设置中，决定如何提供目标长度比决定先揭示哪些token更为重要。
...
TL;DR: 本文提出一种无需训练的解码方法Ripple-Pivot Search (RPS)，通过利用中等熵枢轴位置引发的“涟漪效应”来减少不确定性，从而加速扩散大语言模型的并行解码。


<details>
  <summary>Details</summary>
Motivation: 现有的并行解码调度器仅在满足逐位置标准时才提交位置，忽略了提前提交位置可能对后续解码产生的益处。本文旨在解决如何通过早期策略性提交来降低剩余位置的不确定性并加速解码的问题。

Method: 提出Ripple-Pivot Search (RPS)方法。该方法通过寻找具有中等熵值的位置作为候选枢轴（决定在何处解码），并通过前瞻评估确定能产生最大下游收益的词元分配（决定解码什么），以利用“涟漪效应”。

Result: 在3个dLLMs和4个推理与代码生成基准上，RPS在保持生成质量的同时比标准解码器实现4-10倍时间加速，比先前前瞻基线准确率提升最高5.49%，且在多数设置下吞吐量更高；结合KV缓存时进一步实现高达18倍加速。

Conclusion: 策略性提前提交中等熵位置的词元能够引发“涟漪效应”，显著降低不确定性并加速并行解码，这表明在不牺牲生成质量的前提下大幅提升扩散大语言模型推理速度具有巨大潜力。
...
TL;DR: 本文提出了无需训练的最佳优先草稿树DominoTree，通过沿路径应用非分解条件修正，显著提升了LLM投机解码的接受长度和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿器及树方法（如DDTree）仅建模逐位置边缘分布，无法表示Domino中基于GRU因果修正的路径依赖分布。

Method: 提出DominoTree，一种无需训练的最佳优先草稿树；其通过沿每条根到节点的路径对Domino的条件（非分解）修正进行评分，并将每个节点的修正限制在候选top-M中以实现实用化。

Result: 在多个基准测试及SGLang中，DominoTree在所有服务单元（不同模型大小、单请求/并发负载、最长32K上下文）下均达到最高平均接受长度。在Qwen3-8B上吞吐量相比自回归最高提升7.3倍，在HELMET长上下文任务中接受长度比Domino提升29-36%，吞吐量提升10-34%。

Conclusion: DominoTree通过有效利用路径依赖的修正机制改进了投机解码，在多种模型大小和上下文长度下均显著提升了接受长度和吞吐量。
...
TL;DR: 本文提出了GRAFT框架，通过优化基于扩散语言模型（DLM）的推测解码中的草稿树构建，实现了显著的端到端加速。


<details>
  <summary>Details</summary>
Motivation: 现有的DLM树状推测解码方法（如DDTree）在边选择上仅依赖令牌概率而忽略父子兼容性，且采用固定预算而未考虑解码状态对最优树大小的影响。

Method: 提出GRAFT框架，包含目标蒸馏边缘评分（TDES）以从目标模型轨迹中学习父子偏好并选择目标兼容的边，以及状态感知预算分配（SABA）通过平衡预期草稿增益与验证成本来动态设定每轮树预算。

Result: 在多个模型和任务上，GRAFT相比自回归解码实现了2.13倍至6.36倍的端到端加速，且每轮增加的额外开销不到0.5毫秒（约占目标模型验证延迟的1.4%）。

Conclusion: GRAFT有效解决了DLM草稿树构建中的父子兼容性和动态预算问题，在极低开销下显著提升了解码速度。
...
TL;DR: 该论文提出了CaDDTree，一种成本感知的扩散草稿树方法，通过联合优化树结构和节点预算来最大化推测解码中的token吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的如DDTree等块扩散草稿方法在固定节点预算下最大化预期接受长度，但接受长度随预算非递减，总是偏向更大的树而忽略验证成本，缺乏选择预算的原则性依据。

Method: CaDDTree显式建模草稿和验证延迟，将吞吐量目标分解为每轮对预算的一维搜索，并在凸验证成本下证明吞吐量函数是单峰的，从而实现高效的贪婪停止规则，无需离线预算搜索即可自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型上跨越八个基准测试（涵盖推理、编码和指令遵循任务）的实验表明，CaDDTree在几乎所有任务上均匹配或优于带有Oracle预算选择的DDTree。

Conclusion: CaDDTree为推测解码中的树结构和预算选择提供了一种原则性、成本感知的解决方案，能够根据每轮分布和验证成本动态调整预算，有效提升实际推理吞吐量。
...
TL;DR: 本文提出DDTree方法，通过从块扩散起草器的逐位置分布构建草稿树，提升了推测解码的接受长度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散起草器（如Vanilla DFlash）每轮仅验证单个草稿轨迹，限制了其接受长度。

Method: DDTree利用块扩散起草器的逐位置分布直接构建草稿树，在固定节点预算下使用最佳优先堆算法选择最可能匹配目标模型的延续，并通过仅祖先注意力掩码在单次目标模型前向传播中完成高效验证。

Result: 基于DFlash的DDTree克服了单轨迹验证的局限，使其跻身推测解码领域的领先方法之列。

Conclusion: DDTree通过将单轨迹验证扩展为树状验证，在不增加额外目标模型前向传播的前提下，显著提升了推测解码的整体性能和接受长度。
...
### [17] [Design Docs Are All You Need: An AI-native Machine-Learning Performance Tool](http://arxiv.org/abs/2609.05364v1)
...
TL;DR: SMART是一个几乎没有代码的符号性能建模库，通过AI代理从自然语言设计文档重新生成代码，以应对ML系统快速演变带来的技术债务。


<details>
  <summary>Details</summary>
Motivation: 机器学习性能建模的快速演变使得现有抽象很快失效，迫使框架不断重构；同时AI编程代理已足够强大，重新生成整个库比修补技术债务成本更低。

Method: 提出SMART库，主分支不包含代码，而是由自然语言设计文档组成的DAG。AI子代理根据文档重新生成实现。可靠性依赖于：包含逐步示例的设计文档风格，以及极简的递归算子IR（带有符号成本表达式，支持快速分析汇总和慢速模调度）。

Result: 重新生成的实现能够以舍入精度重现人工审核的参考模型，包括在TPU pod切片上提供服务的DeepSeek-V3。

Conclusion: 设计文档而非代码，可以作为机器学习系统协同设计工具的持久化产物。
...
### [18] [PRISM-Bench: An Audio-Centric Diagnostic Benchmark for Text-to-Audio-Video Generation](http://arxiv.org/abs/2609.04867v1)
*Yuchen Sun,Qian Yang,Jun Wang,Detai Xin,Guoqiao Yu,Guanglu Wan,Qi Jia*

Main category: cs.MM

TL;DR: 本文提出了PRISM-Bench，这是首个以音频为中心的文本到音视频（T2AV）生成诊断基准，通过解耦音频类型和声源可见性来全面评估音频生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的T2AV评估方法低估了音频模态，将其视为视频质量的附属或脱离视听语境进行孤立评估，导致难以准确诊断音频生成系统真实的优缺点。

Method: 构建包含900个人工验证样本的数据集，沿音频类型（语音、音乐、声音）与声源可见性（屏幕内/外）两个正交轴进行解耦评估；采用35项细粒度标准覆盖四个感知维度，并结合基于盲测与真实参考并排比较的增强型多模态大模型评判（MLLM-as-a-Judge）协议。

Result: 该评判协议与人类评估者表现出强一致性（平均超70%）；评估揭示了前沿模型与开源模型间的显著性能差距，并发现当前模型在感知保真度上过拟合，在复杂接地与控制任务（如音乐和同步屏幕内音频生成）上表现不佳。

Conclusion: PRISM-Bench填补了T2AV生成中音频模态诊断评估的空白，揭示了现有系统在视听接地与精细控制方面的局限性，为未来模型的优化指明了方向。
...
### [19] [Trust-Aware Adaptive Disclosure for Inference Privacy Preservation in Multi-Agent Networks](http://arxiv.org/abs/2609.05340v1)
*Puspanjali Ghoshal,Tobias J. Oechtering*

Main category: cs.MA

TL;DR: 本文提出了一个信任感知隐私控制框架，用于在目标推断攻击下保护多智能体系统的隐私。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统越来越多地部署在信息关键系统中，而每个智能体的潜在目标需要隐藏起来，防止被观察到的对手推断出来。

Method: 提出了一种信任感知隐私控制框架，该框架基于智能体之间动态信任关系来调整消息披露，使用信任依赖的随机策略来控制信息发布，从而在共识性能和隐私保护之间实现权衡。

Result: 实验表明，与代表性基线相比，所提方法降低了对手目标推断准确性，同时保持了具有竞争力的共识效用。

Conclusion: 信任感知机制在多智能体系统的隐私保护方面是有效的，能够在共识性能和隐私保护之间实现良好的权衡。
...
### [20] [The History Is the Detector: Executing CVE Patch History, End-to-End](http://arxiv.org/abs/2609.05335v1)
*Qiushi Wu,Kevin Eykholt,Youngja Park,Xiaokui Shu,Dhilung Kirat,Douglas Lee Schales,Ian Molloy*

Main category: cs.CR

TL;DR: 本文提出了BUGSTONE-E2E框架，将漏洞历史转化为可执行的检测规则，通过多阶段漏斗式流水线自动检测和修复漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有公开漏洞数据库和修复提交主要为人工检查设计，未能实现自动化复用，导致大量漏洞检测知识被闲置，相同的不安全代码条件可能依然存在于其他代码库中。

Method: BUGSTONE-E2E首先从已验证的修复提交中挖掘可重用规则（包含扫描锚点、修复语义和CVE出处），并按CWE和语言进行组织。随后采用漏斗式检测流水线：早期阶段使用Tree-sitter枚举匹配锚点的调用点，并通过轻量级启发式算法剔除良性位点；后期阶段利用LLM代理在规则引导下检查剩余候选，并对其进行重新分流、构建运行时验证，最后生成经双向差异测试验证的补丁。

Result: 基于2022至2026年的19,325个高危CVE，系统识别了2,710个修复提交，在56个CWE家族中构建了1,033条检测规则（封装为172个技能）。在14个程序上的应用中，该系统为644个发现产生了运行时证据。

Conclusion: 研究证明CVE历史可以被转化为可执行的工作流，从而将过去的漏洞转化为可复现的检测与修复方案。
...
### [21] [Mitigating Disease Spread by Design in Refugee and IDP Camps](http://arxiv.org/abs/2609.05342v1)
*Giulia Zarpellon,Joseph Aylett-Bullock,Frank Krauss,Miguel Luengo-Oroz*

Main category: physics.soc-ph

TL;DR: 本文提出了一种利用JUNE基于智能体的流行病模型来探索难民营布局对疾病传播影响的方法。


<details>
  <summary>Details</summary>
Motivation: 疾病传播在难民和境内流离失所者（IDP）定居点中日益严峻。由于人们的移动和互动受营地布局影响，布局可能显著影响疾病传播，因此本文旨在解决如何通过营地布局规划来缓解疾病传播的问题。

Method: 采用JUNE基于智能体的流行病模型，创建一种评估不同营地布局对疾病传播潜在缓解效果的方法论。

Result: 展示了概念验证实验，并讨论了该方法所能提供的关于营地运营和疾病控制的实用见解。

Conclusion: 该方法论为未来研究营地布局与疾病传播的关系提供了分析框架和理论基础。
...
### [22] [Optimal Rates for Agentic Networked Information Aggregation](http://arxiv.org/abs/2609.05318v1)
*MohammadHossein Bateni,Zahra Hadizadeh,MohammadTaghi Hajiaghayi,Mahdi JafariRaviz,Shayan Taherijam*

Main category: cs.LG

TL;DR: 本文闭合了网络化学习中线性回归与逻辑分类模型的信息聚合误差界，确定了最优误差率为常数（当D<M^2时）及Θ(M^2/D)（当D≥M^2时）。


<details>
  <summary>Details</summary>
Motivation: 先前的研究在代理间通过传递预测进行信息聚合的线性回归模型中，证明了超额均方误差为O(M/√D)，但下界仅为Ω(M/D)。本文旨在闭合这一上下界差距。

Method: 提出了对循环实例的更尖锐分析，并构造了在深度D≥M^2下误差为Ω(M^2/D)的M-covered路径；同时证明了固定分布下误差沿路径几何收缩。最后将该方法扩展至逻辑分类（BCE损失）的logit传递模型中。

Result: 证明了当D<M^2时误差率为常数，当D≥M^2时为Θ(M^2/D)；并在逻辑分类模型中实现了相同的O(M^2/D)最优上界。

Conclusion: 确立了网络化学习模型中信息聚合的最优误差率，闭合了先前研究留下的上下界差距，并将理论结果推广至分类任务。
...
### [23] [CForce: Boosting Parallel Decoding for dLLMs via Consistency Forcing](http://arxiv.org/abs/2608.13925v1)
*Yuji Ren,Chenkai Xu,Zhuocheng Gong,Jianguo Li,Zhijie Deng*

Main category: cs.LG

TL;DR: 本文提出一致性强制方法，通过对齐早期与后期去噪阶段的掩码预测，有效解决了扩散大语言模型在高并行解码下的预测误差累积问题，提升了速度与质量的平衡。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在激进的并行策略下，早期去噪阶段的掩码预测不可靠，导致错误传播至后续阶段，影响生成质量。

Method: 提出CForce蒸馏方法，在预收集的自展开轨迹上训练模型，强制早期阶段预测与后期阶段对齐；引入置信度自适应KL散度结合正向和反向KL的优势，该一致性目标同样适用于掩码到词元解码和具备编辑能力的解码。

Result: 在非编辑和具备编辑能力的LLaDA模型上的实验表明，CForce改善了速度与质量的权衡，特别是在高并行解码预算下表现优异。

Conclusion: CForce通过理论证明与实验验证，有效减少了早期去噪阶段的预测误差，提升了dLLMs的训练-推理对齐，且代码已开源。
...
### [24] [StrixAE: An Intelligent Agent for Audio Enhancement under Complex Distortion Coupling in Real-World Scenarios](http://arxiv.org/abs/2609.03414v1)
*Chenglin Wu,Junjie Wu,Jinhang Chen,Mingyang Chen,Zixu Lin,Jiabian Chen,Xinghao Ding,Xiaotong Tu*

Main category: cs.SD

TL;DR: 本文提出基于多模态大语言模型的智能体StrixAE，通过两阶段训练（CoT监督微调与音频感知强化学习）协调多个模型，以解决真实场景中复杂的音频失真与个性化增强问题。


<details>
  <summary>Details</summary>
Motivation: 真实场景下的音频增强面临复杂失真耦合且需要个性化处理，而现有解决方案难以同时兼顾两者，因此亟需提升系统的鲁棒性并实现自主操作。

Method: 提出基于MLLM的StrixAE智能体，将MLLM作为控制器协调多个音频增强与个性化模型。系统采用两阶段训练：在AcoustBench上进行CoT监督微调以实现基础推理和工具调用；随后引入音频感知强化学习（APRL），通过联合优化格式有效性、结构连贯性和感知质量的奖励机制，确保生成可执行、无幻觉的增强计划。

Result: 在真实世界测试数据集上，该方法在多个感知指标上超越了大多数现有的开源和专有解决方案，取得了最先进的性能，并展现出强大的泛化鲁棒性。

Conclusion: 结合MLLM的多模态推理与APRL强化学习策略，StrixAE有效解决了真实场景中复杂的音频增强需求，实现了高鲁棒性、强泛化能力和可解释的自主音频恢复。
...
### [25] [RoboSPA: Can VLA Models Go Beyond Simple Scenes and Short-Horizon Tasks?](http://arxiv.org/abs/2609.05324v1)
...
TL;DR: 该论文提出了RoboSPA，一个大规模机器人操作数据集和基准，用于诊断视觉-语言-动作模型在精细空间推理和长时程程序规划方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的数据集和基准主要在预定义设置下评估任务完成情况，对模型在空间和程序复杂性增加时的推理能力提供的见解有限。

Method: 提出RoboSPA基准，包含10个任务类别和56个基础任务，跨越5个难度级别（共280个变体），并收集了527K条轨迹；同时引入了超越简单二元成功率的细粒度诊断指标。

Result: 对代表性VLA模型的实验显示，当前系统在处理复杂空间关系、精确的低层执行以及内存密集型规划时仍面临重大挑战。

Conclusion: RoboSPA为开发更强大、可靠和具泛化能力的具身智能体提供了一个极具挑战性的诊断基准。
...
### [26] [Multi-Step Tool-Calling over Korean Open Public APIs: A Benchmark and a Data-Synthesis Recipe](http://arxiv.org/abs/2609.05395v1)
*Dain Kim,Eungi Cho,Kyumin Kim,Shinyeong Noh,Kyuseong Lim*

Main category: cs.AI

TL;DR: 本文提出了KOPA-Bench基准和EDGE数据合成框架，通过实时执行验证的动态图合成多步工具调用轨迹，显著提升了开源小模型在多步API调用任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 数据主权法规要求公共机构部署开源的本地LLM代理以进行跨政府API的多步工具调用，但开源模型在此场景下表现不佳，且此前缺乏衡量该性能差距的专用基准。

Method: 提出了EDGE（基于执行的动态图工具调用数据合成）框架，该框架构建工具间输入输出的关系图，通过实际调用实时API仅保留成功的链接，并遍历这些已验证的链接来合成可执行的多步轨迹，最后通过GRPO算法对模型进行微调。

Result: 基于EDGE合成的数据集微调的9B模型，在KOPA-Bench和BFCL基准上均取得了大幅提升，其性能几乎与同系列未微调的27B模型相当。

Conclusion: EDGE框架有效弥补了开源模型在多步工具调用上的不足，使较小的开源模型能够达到接近大模型的性能，为受数据主权限制的机构提供了可行的本地化代理部署方案。
...
### [27] [Necessary or Sufficient? Evaluating LLM Explanations With Behavioural Evidence](http://arxiv.org/abs/2609.05385v1)
*Urja Pawar,Rajitha Ramanayake,Nabeel Kemal,Ashwin Kandath,Owen O'Neill,Guillaume Bourgeon,Houssem Chatbri*

Main category: cs.AI

TL;DR: 本文评估了大型语言模型决策组件提供的解释是否与其可观察的决策行为相符。


<details>
  <summary>Details</summary>
Motivation: 操作员通常依赖LLM解释中提及的因素来监控系统、诊断错误或决定是否升级输出，这假设了解释与实际决策行为一致，本文旨在检验该假设。

Method: 提出一种黑盒干预评估框架，通过测量改变某因素是否改变输出（必要性）以及保留某因素是否维持输出（充分性），来验证LLM引用的解释因素。

Result: 在八个模型的测试中，解释中引用的因素排名与必要性或充分性得分之间的平均相关性仅为弱至中等，且在多数情况下未被引用的因素影响力得分高于被引用的最低分因素。

Conclusion: LLM提供的解释因素虽包含有用信息，但不能可靠识别出影响最大的因素；所提框架为代理监督中使用的解释提供了黑盒可靠性检查。
...
### [28] [CUA-Universe: A Scalable and Dynamic Environment for Hybrid GUI+CLI Agents](http://arxiv.org/abs/2609.05374v1)
*Haoting Shi,Wenhao Wang,Weicheng Fang,Yaozhong Liang,Tian Jin,Pengxiang Zhao,Guangyi Liu,Siheng Chen,Yanfeng Wang*

Main category: cs.AI

TL;DR: 本文提出了CUA-Universe，一个将真实桌面软件转化为混合GUI+CLI环境的可扩展环境到数据流水线，用于训练更高效、更有能力的计算机使用代理。


<details>
  <summary>Details</summary>
Motivation: 现有的计算机使用代理主要依赖GUI，效率低下，且缺乏可扩展的混合GUI+CLI环境（通常需要大量人工工程）。此外，现有的代理难以互补使用这两种接口，纯CLI代理缺乏视觉感知，纯GUI代理在执行命令时效率低下。

Method: 提出CUA-Universe流水线，包含三个组件：App-Forge（将应用适配到可复现的VM中并发现/生成CLI接口）、Task-Weave（基于种子文件合成可控难度的混合任务）、Path-Steer（引导rollout沿高效混合路径进行并收集验证轨迹用于后训练）。

Result: 在该数据上训练的9B模型在多个基准测试中显著提升了成功率与效率，如在CUA-Verse（得分+39.3分，步数-37%，token-60%）、OSWorld（成功率+16.8分，步数-57%，token-44%）和OSWorld-MCP（得分+7.84分，步数-27%，token-30%）上均取得显著改进。

Conclusion: CUA-Universe为开发更强大、更高效的计算机使用代理提供了一条可扩展的路径，成功促使代理行为从低效的GUI交互和脆弱的CLI脚本转向有效的GUI+CLI协调操作。
...
### [29] [Does Your Agent's Memory Survive a Model Upgrade? A Controlled Study of Memory Portability](http://arxiv.org/abs/2609.05339v1)
*Ankit Goyal,Jaideep Ray*

Main category: cs.AI

TL;DR: 本文研究了模型升级对不同记忆存储格式的影响，发现固定模式知识图谱在模型迁移中可靠性最高，而压缩笔记则与模型高度耦合。


<details>
  <summary>Details</summary>
Motivation: 本文旨在解决模型升级后记忆退化的问题。尽管代理可以保留相同的记忆存储，但新模型可能对旧笔记产生不同解读，或混合嵌入版本可能破坏检索，因此需要研究不同记忆格式在模型替换时的表现。

Method: 该研究比较了四种记忆保存方法：长上下文原始历史阅读（LC-RAW）、分块检索增强生成（RAG）、模型压缩的自然语言笔记（NOTES）和固定模式知识图谱（KG-fixed）。实验使用了48个带有随机答案代码的合成历史，以及两个参数少于100亿的开放权重模型。

Result: 实验结果表明，固定模式结构（KG-fixed）在模型替换后准确率几乎不变，而压缩笔记（NOTES）表现出与模型高度耦合，准确率出现不对称变化。RAG系统中的部分嵌入迁移仅获得极小增益。诊断分析显示，NOTES的准确率缺陷主要源于初始构建时的信息丢失，而RAG的缺陷主要由检索失败导致。此外，仅对NOTES进行存储修复在所有测试案例中均未达标，而保留原始历史记录则在多数情况下实现了成功恢复。

Conclusion: 研究结论强调了进行特定方向的迁移测试、严格隔离嵌入空间以及保留源历史记录以进行记忆修复的必要性。
...
TL;DR: 本文首次对掩码扩散语言模型在真实并发负载下的服务行为进行了系统特征分析。


<details>
  <summary>Details</summary>
Motivation: 现有的dLLM服务系统缺乏对模型在真实并发服务负载下行为的实证测量，盲目沿用自回归（AR）模型的服务假设可能并不适用。

Method: 使用LLaDA-8B-Instruct和D2F LoRA适配器在单块NVIDIA H200 GPU上对dLLM服务进行特征分析，并推导了泊松到达下固定填充同步批处理的超时规则。

Result: 研究发现：1) 请求难度离散分布，无法提前预测（最佳R2=0.150）；2) 短生成预算（低于320 tokens）的基准测试会低估服务方差；3) 单请求中GPU计算仅占24%，其余为CPU调度开销，批处理通过摊销此开销使吞吐量提升16.0倍，且输出质量不随批大小下降。

Conclusion: 扩散语言模型的服务需要每个去噪步骤级别的并行性，这与AR服务在准入和驱逐与已共享前向传播的交互方式上存在根本差异。
...
TL;DR: 本文提出了一种无需训练的推理方法CAI-DLLM，利用第一步置信度指导去噪过程，从而加速扩散语言模型的生成并降低能耗。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在推理过程中需要反复进行去噪步骤，导致生成成本高昂，尤其是模型会重复计算已经稳定的token。

Method: 提出CAI-DLLM，一种利用第一步置信度信号指导去噪的免训练推理方法。该方法提前确定简单token，将更多去噪步骤分配给困难token，并调整不同输出块的解码计划。无需重新训练、额外预测器或权重更新。

Result: 在LLaDA-8B-Instruct和Dream-7B-Instruct模型的多个任务上进行评估。在LLaDA GSM8K上实现了高达18.2倍的推理加速且准确率提升（76.27%至77.41%）；在Dream HumanEval上实现13.1倍加速且pass@1提高（46.95%至48.17%）。在更困难的推理任务中，加速比达到44.8倍，最大准确率下降4.4个百分点，能耗降低高达95.3%。

Conclusion: CAI-DLLM无需重新训练即可有效加速扩散语言模型的推理过程，在大幅减少时间与能耗的同时，在多数任务上保持或提升了准确率。
...
### [32] [Continuous Interaction Diffusion: A Diffusion-Native Runtime for Asynchronous Tool-Augmented Reasoning](http://arxiv.org/abs/2608.10438v1)
*Yuhang Cao*

Main category: cs.AI

TL;DR: 本文提出了连续交互扩散（CID）架构，将工具交互集成到扩散语言模型的迭代去噪过程中，以解决传统停止-恢复交互模式的限制。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行细化输出进行推理，传统的停止-恢复式工具调用模式会迫使模型在推理稳定前做出工具决策、延迟观察并引入冗余计算，从而损害准确性和效率。

Method: 提出连续交互扩散（CID）架构，将工具交互集成到迭代去噪中。该架构分离了只读事实通道、思维通道和显示通道；在完整调用序列化前启动外部读取并在去噪时继续，将结果投射至思维状态以修正早期认知，并利用持久绑定复用静态结果。

Result: 本文形式化了该架构、运行时和训练目标，并定义了任务质量和端到端效率的评估协议。本文侧重于只读工具，未提出任何实证性能声明。

Conclusion: CID架构能够更早暴露证据、将工具延迟与模型计算重叠、减少重复外部工作，并在新证据到达后保留有用计算，未来可进行实证评估。
...
TL;DR: 本文提出了TAPS方法，通过目标感知的前缀选择优化扩散模型在投机解码中的草稿树验证过程，实现了显著的端到端无损加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型用于并行起草虽降低了起草延迟，却将瓶颈转移至验证阶段。现有方法基于边际概率对草稿树节点排序，忽略了验证的前缀条件性，导致验证被拒绝前缀的不可达后代，增加了延迟且接受度提升有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边际概率转化为路径条件的接受估计，并在固定验证预算下选择一个紧凑的前缀闭合子树，以改善接受与成本的权衡。

Result: 实验表明，TAPS在多种数据集和模型上相较于普通自回归解码实现了高达7.9倍的无损端到端加速，且分别比最先进的DFlash和DDTree高出1.36倍和1.74倍。

Conclusion: TAPS有效解决了扩散模型投机解码中的验证瓶颈，通过优化前缀选择和子树构建在固定预算下最大化接受度，为无损加速自回归生成提供了更优方案。
...
### [34] [InSituMeasure: Probing Situated Measurement Grounding in Industrial Scenes with Multimodal Large Language Models](http://arxiv.org/abs/2609.04014v1)
*Chao Shen,Xinyuan Li,Yunfan Zhou,Jianguo Yao,Haibing Guan,Zhihai Wang,Xijun Li*

Main category: cs.AI

TL;DR: 本文提出了InSituMeasure基准，用于评估多模态大语言模型在真实工业场景中的仪表读数与故障诊断能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在通用基准上表现优异，但在连续值测量任务上仍不可靠。现有基准将测量与真实场景隔离，缺乏实际背景、专业仪器、现实噪声和匹配的诊断注释，限制了根因分析。

Method: 引入InSituMeasure基准，包含2,922个真实工业监控场景，涵盖八大类专业工程仪器，并提供密集的仪表属性注释和噪声标签。同时定义了容差下的数值准确率、单位一致性、拒绝虚假或不可回答任务的能力，以及模型失败与注释错误因素之间的对齐指标。

Result: 对24个最先进的MLLMs进行测试，表现最好的模型联合值-单位准确率仅为25.7%，置信度诊断F1分数仅为51.8%，揭示了通用多模态能力与可靠的情境测量之间的巨大差距。

Conclusion: 模型在连续值测量上的失败主要源于文本诱导的捷径、过度自信的回答以及真实的工业噪声（如混合干扰、视角偏差、遮挡和环境干扰），未来需重点提升模型在真实复杂情境下的测量可靠性。
...
### [35] [NeoRed: A Knowledge-Logic-Alignment Multimodal Large Language Model for Neonatal Respiratory Disease Diagnosis](http://arxiv.org/abs/2609.03527v1)
*Yinan Liu,Hongtai Xia,Haoran Xu,Jiankang Hong,Jingkuan Song,Ye Luo*

Main category: cs.AI

TL;DR: 本文提出了首个针对新生儿呼吸系统疾病的多模态大语言模型NeoRed，通过知识-逻辑-对齐框架生成准确的诊断报告。


<details>
  <summary>Details</summary>
Motivation: 新生儿呼吸系统疾病是发病和死亡的主要原因，但现有多模态大语言模型因依赖成人训练数据且缺乏多维临床上下文整合，在新生儿诊断中存在局限性。

Method: 提出了NeoRed模型并收集了两个临床数据集，设计了知识-逻辑-对齐（KLA）框架，包含知识先验注入（KPI）、诊断逻辑约束（DLC）和视觉语义对齐（VSA）三个模块来增强异构临床上下文与胸片的联合诊断。

Result: 在NeoCXR数据集上，NeoRed的ROUGE-L达到53.29%，临床疗效F1分数达到65.19%，优于现有MLLMs，同时在成人基准上也保持了竞争力。

Conclusion: NeoRed填补了新生儿诊断报告生成的空白，通过KLA框架有效提升了跨模态联合诊断的准确性，并在成人数据集上保持了泛化能力。
