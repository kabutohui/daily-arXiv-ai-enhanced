...
TL;DR: 提出PEA-DPO框架以解决多模态偏好优化中的视觉不敏感问题，从而有效减少多模态大语言模型的幻觉。


<details>
  <summary>Details</summary>
Motivation: 直接偏好优化（DPO）在多模态场景下的应用尚待探索，且模型存在“视觉不敏感”问题（无法区分关键视觉上下文被移除的图像），具体表现为跨图像和图像内不敏感。

Method: 提出感知增强对齐DPO（PEA-DPO）框架，明确利用视觉偏好信号克服视觉不敏感性，并从理论证明该框架能有效缓解上述两种失效模式。

Result: 在三个幻觉基准测试和不同规模的模型上的实验表明，PEA-DPO增强了对视觉上下文的敏感性，保留了基础模型的语言建模能力，实现了更强的多模态对齐并大幅减少了幻觉。

Conclusion: PEA-DPO成功解决了多模态大模型对齐中的视觉不敏感缺陷，在不损害原有语言能力的前提下，为缓解幻觉提供了有效途径。
...
TL;DR: 本文提出了OmniHandwritingOCR基准，用于评估多模态大语言模型在真实手写文本和复杂数学公式识别中的表现，揭示了现有系统的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有OCR基准主要关注印刷文本或简单的单行输入，缺乏对多语言手写、书写错误及复杂数学表达式等真实手写场景的覆盖，导致MLLM在真实手写识别中的能力尚不明确。

Method: 提出OmniHandwritingOCR诊断基准，包含6个子任务、12个子集及77.57K标注图像，并引入难度分层的多行公式语料库；在统一协议下使用5种互补指标评估了13个开源和闭源系统。

Result: 实验表明现有系统在忠实转录方面表现不佳，在复杂多行公式上性能急剧下降；不同语言和公式设置下模型排名波动；部分生成式模型会产生看似合理但无视觉依据的幻觉修正。

Conclusion: 该基准为诊断多模态模型在手写OCR场景中的语言、内容、结构和视觉基础失效模式提供了一个极具挑战性的测试平台。
...
TL;DR: 本文提出MR-IQA-2框架，通过actor-editor-judge机制解耦推理与评分的监督信号，以提升盲图像质量评估中推理的忠实性与可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在图像质量评估中仅依赖人类评分进行监督，无法保证推理过程真实反映图像质量，且共享奖励机制可能在偶然正确评分时强化不忠实的推理。

Method: 提出actor-editor-judge框架：actor生成质量推理，editor据此修改图像，frozen judge对比图像并提供反思监督。采用细粒度信用分配，通过掩码token更新将推理监督（判别器反馈）与评分监督（人类评分）解耦。

Result: 在多个IQA基准上取得了与人类评分高度一致的竞争性结果，并通过视觉反思实现了超越评分的更丰富、忠实的视觉理解。

Conclusion: 解耦推理与评分监督有效提升了IQA的可靠性与忠实度，其增强的视觉理解能力可为图像质量优化及下游任务提供支持。
...
TL;DR: 本文提出PATE-Forensics框架，采用“感知即工具”范式将深度伪造检测定位与解释生成解耦，提升了取证性能。


<details>
  <summary>Details</summary>
Motivation: 现有可解释深度伪造取证方法过度依赖任务适配的MLLM联合处理各项任务，本文旨在解决这种高度耦合带来的局限性。

Method: 提出PATE-Forensics，基于DINOv3构建感知工具紧密耦合多粒度检测与线索引导定位，并将输出作为结构化上下文输入给免微调的通用MLLM生成解释。

Result: 在DDL-X Track 3上取得0.89的最高官方得分，领先第二名0.19分。

Conclusion: 将感知作为工具并与解释生成解耦的范式，在无需任务特定微调的情况下即可实现高效的深度伪造检测与解释。
...
TL;DR: 该论文提出Task Model Induction (TMI)方法，从多线程的自然计算机使用轨迹中解耦并发活动并构建结构化任务模型，显著提升了任务重建率和下游智能体性能。


<details>
  <summary>Details</summary>
Motivation: 从自然计算机使用轨迹中提取可审计和可复用的任务模型极具挑战，因为现实工作是多线程且目标交错的，而现有方法仅假设单一工作流并生成步骤级摘要，无法满足智能体实际工作时的学习与审计需求。

Method: 提出TMI方法，首先在无约束轨迹中发现潜在任务并解耦并发活动，随后为每个潜在任务构建任务模型，该模型结合了递归目标分解的分层目标模型与组织执行控制流的过程模型。

Result: TMI恢复交错任务的一致性达0.974，重建了74.9%的执行步骤（远超最强基准）；基于TMI模型导出的技能将保留任务准确率比最强基准提高了30.0%。

Conclusion: TMI能有效从低级、多线程的真实轨迹中提取结构化任务模型，在任务重建与提升下游智能体任务准确率方面均显著优于现有方法。
...
TL;DR: 本文提出了一种无需训练的解码方法RPS，通过利用扩散大语言模型解码中的“涟漪效应”主动提交中熵位置，在不损失生成质量的前提下实现了显著的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型的并行解码调度器仅基于单位置标准进行提交，忽略了提前提交某些位置可能为后续解码带来的不确定性降低和加速益处。

Method: 提出Ripple-Pivot Search (RPS)，寻找中熵位置作为枢纽决定“在哪解码”，并通过前瞻评估确定产生最大下游收益的词元分配决定“解码什么”，从而利用涟漪效应加速解码。

Result: 在多个模型和基准测试中，RPS比标准解码器实现4-10倍挂钟时间加速且保持质量，比前瞻基线准确率最高提升5.49%，吞吐量更高；结合KV缓存可达18倍加速。

Conclusion: RPS有效利用了涟漪效应加速dLLMs解码并提升或保持质量，证明了通过优化位置提交策略提升并行解码效率的可行性与潜力。
...
TL;DR: 本文提出前缀条件扩散（PCD）预训练目标，结合自回归前缀监督与后缀去噪，以解决扩散语言模型训练与推理时上下文不匹配的问题，提升生成性能。


<details>
  <summary>Details</summary>
Motivation: 原生扩散语言模型预训练会同时破坏提示词和续写词，削弱了提示词条件生成所需的干净前缀接口，导致训练与实际使用时的上下文分布不匹配。

Method: 提出前缀条件扩散（PCD）预训练目标，对干净前缀进行自回归监督，仅对未知续写部分应用扩散去噪。该方法修改了注意力掩码、破坏掩码和标签构造，无需自回归解码器或新推理模式。

Result: 在LLaDA2-Mini和Qwen-1.7B骨干网络上，PCD持续优于同族原生dLLM基线，在LLaDA2-Mini六个基准平均上实现4.2%的相对提升（+2.56分），在Qwen主要机制对比中实现14.2%的相对提升（+4.86分）。

Conclusion: 将预训练上下文分布与提示词条件生成对齐，能在不改变推理方式的前提下，有效弥补扩散语言模型的续写性能差距。
...
TL;DR: 本文指出掩码扩散语言模型在推理任务中的无序解码会导致过早提交答案的失败，并提出边界门控提交方法来恢复性能且保持并行解码。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型（dLLMs）以任意顺序生成词元为优势，但在推理任务中，这种无序自由度会导致模型在推理过程未完成时过早提交最终答案，引发输出崩溃。

Method: 采用2x2提示-解码器设计分析问题成因，并提出一种单旋钮干预方法——“边界门控提交（frontier-gated commitment）”，以因果性地恢复链式思维与顺序提交的交互作用。

Result: 边界门控提交将模型准确率从0.528提升至0.852，同时保留高达4倍的并行解码速度；研究还揭示了现有窗口式采样器实际上无意中修复了这种推理病理问题。

Conclusion: dLLMs的无序解码是推理任务中的核心缺陷，而限制提交顺序的机制（如边界门控）是解决该推理病理的最小有效方案。
...
TL;DR: 本文提出OPTD方法，通过在策略转移蒸馏与一致性引导的自适应压缩，解决扩散语言模型少步蒸馏中的状态漂移问题，有效提升推理效率与生成质量的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有少步蒸馏方法在非在策略轨迹上构建监督，导致学生模型在推理时因早期并行预测改变上下文而产生状态漂移；而在策略蒸馏虽能缓解此问题，但难以平衡转移步数（步数过少限制压缩，过多破坏中间依赖）。

Method: 提出OPTD（在策略转移蒸馏），从学生模型自身轨迹采样部分状态，利用冻结的仅问题教师模型识别结果对齐的未来候选并按置信度排序，选择保持教师输出结果的最长前缀；采用集合瓶颈目标提升已验证候选，并用冻结教师KL锚点正则化其他位置，全程无需标准答案。

Result: 在四个数学推理和代码生成基准测试中，OPTD持续改善了质量与效率的权衡，并在评估的少步基线中实现了最强的质量受限AUP。

Conclusion: OPTD成功解决了少步扩散语言模型蒸馏中的状态不匹配问题，证明了无需标准答案的在策略自适应压缩蒸馏能显著加速解码同时维持高质量。
...
TL;DR: 本文提出了DominoTree，一种无需训练的最佳优先草稿树方法，通过利用Domino草稿器的路径依赖条件修正来加速LLM推理。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿器和树方法（如DDTree）采用因子化公式，无法表示最新发布的基于GRU因果修正的Domino草稿器所具有的路径依赖性分布。

Method: 提出DominoTree，一种无需训练的算法，通过沿每条根到节点路径计算Domino的条件（非因子化）修正来评分并构建最佳优先草稿树，并将每节点修正限制在候选前M个以保证实用性。

Result: 在所有服务单元和基准测试中获得了最高的平均接受长度；吞吐量显著提升（在Qwen3-8B上比自回归快7.3倍，在SGLang中比Domino高12%）；在长上下文HELMET测试中接受长度提升29-36%。

Conclusion: DominoTree成功利用了Domino草稿器的路径依赖修正特性，在各种设置下均优于现有方法的接受长度和吞吐量，但在预填充占主导的极长上下文中，其领先优势缩小至平局。
...
TL;DR: 本文提出了CaDDTree方法，通过联合优化树结构和节点预算来直接最大化推测解码的令牌吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿方法（如DDTree）仅最大化期望接受长度而忽略验证成本，导致在节点预算选择上缺乏原则性依据。

Method: CaDDTree显式建模草稿和验证延迟，将吞吐量目标分解为每轮对预算的一维搜索，并证明在凸验证成本下吞吐量函数具有单峰性，从而使用高效的贪婪停止规则动态调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型上涵盖推理、编码和指令遵循的八个基准测试中，CaDDTree在几乎所有任务上均达到或超越了带有预言机预算的DDTree性能。

Conclusion: CaDDTree提供了一种无需离线预算搜索的原理性成本感知方法，能够根据当前分布和验证成本自适应地优化推测解码吞吐量。
...
TL;DR: 本文提出DDTree方法，通过块扩散草稿模型的逐位置分布构建草稿树并以单次前向传播验证，克服了DFlash仅验证单一轨迹的限制。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash推测解码方法在每轮中仅验证单条草稿轨迹，限制了其接受长度，从而制约了进一步的加速效果。

Method: 提出DDTree，直接从块扩散草稿模型的逐位置分布构建草稿树；在固定节点预算下，利用最佳优先堆算法选择最可能匹配目标模型的续写节点，并采用仅祖先注意力掩码在目标模型单次前向传播中完成高效验证。

Result: DDTree基于DFlash实现了性能提升，使其跻身推测解码领域的领先方法之列。

Conclusion: DDTree通过引入草稿树构建与高效验证机制，成功突破了单轨迹验证瓶颈，确立了其在推测解码加速技术中的领先地位。
...
TL;DR: 本文探讨了多模态大语言模型适应新模态时是否必须微调骨干网络，发现仅训练投影层即可实现强大的多模态性能且避免了原有能力的退化。


<details>
  <summary>Details</summary>
Motivation: 传统的多模态大语言模型训练通常同时微调语言模型骨干网络和投影层，本文旨在解决是否必须微调骨干网络以适应新模态（如3D模态）的问题，从而减少训练成本和能力退化。

Method: 提出在多模态大语言模型中仅训练模态特定编码器与语言模型骨干网络之间的投影层，而不微调骨干网络，并在3D MLLMs上进行实验验证。

Result: 仅训练投影层不仅能达到与联合训练相当的多模态性能，还避免了联合训练导致的原有语言能力漂移，且训练样本吞吐量约为联合训练的两倍。

Conclusion: 在将多模态大语言模型适应新模态时，仅训练投影层是一种高效且有效的方法，能够在保持原有能力的同时提升训练效率，并在多种基准测试中得到验证。
...
TL;DR: 该论文构建了医学领域统一理解与生成的基础框架，包括大规模数据集、标准化评估基准和端到端训练的统一医学模型。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在向医学领域的统一理解与生成范式扩展时，面临缺乏全面训练和评估基准以及缺乏广泛验证的统一医学模型这两大障碍。

Method: 构建了包含超600万实例、涵盖14种成像模态的最大统一医学数据集MedUAGCorpus，提出包含12项任务的标准化评估基准MedUAGBench，并据此开发了端到端训练的统一医学模型MedUAG。

Result: 大量实验表明，MedUAG模型在广泛的医学理解与生成任务中均取得了强劲的表现。

Conclusion: MedUAG确立了具有竞争力的基准，为下一代医学多模态系统的发展铺平了道路。
...
TL;DR: 本文提出了BEAR-Bench，一个基于文本密集型专业文档的英俄双语推理基准，用于评估多模态大语言模型的推理能力及幻觉检测方法的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型基准在评估文本密集型专业文档的推理能力时存在不足，通常侧重于信息提取或依赖外部知识，且主要集中于英语和中文，缺乏对俄语等其他语言的覆盖。

Method: 引入了包含1000个人工标注问题的自包含英俄双语基准BEAR-Bench，基于文本丰富的商业和科学文档；对16个专有和开源MLLMs进行评估，并利用模型输出比较现有的幻觉检测方法。

Result: 对16个模型的评估表明，即使是最强的系统也存在明显的改进空间；同时，结果被用于评估现有幻觉检测方法识别模型失败的能力。

Conclusion: BEAR-Bench填补了专业文档多语言推理评估的空白，揭示了当前顶级模型的局限性，并验证了幻觉检测方法在识别这些失败时的可靠性。
...
### [16] [GENIE: Generative Neural Inference for Epidemics](http://arxiv.org/abs/2608.20253v1)
...
TL;DR: 本文提出了一种基于机器学习的时空框架GENIE，用于高分辨率预测呼吸道病原体负担，克服了传统模型在细粒度预测上的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统机理隔离模型难以捕捉细粒度的复杂传播动态，导致预测不准确且过度自信；而详细的基于智能体的模型校准困难且计算成本过高，无法满足实时预测需求。

Method: 提出生成神经推断疫情预测框架（GENIE），包含学习跨地点共享疾病动态的局部感染编码器和学习特定地点特征的局部特征编码器。该框架利用高分辨率时空ABM的模拟数据训练，生成未来疫情轨迹的近似后验预测分布样本。

Result: 与现有统计和机器学习模型基准测试相比，GENIE在包括住院高峰时间和规模在内的多项指标上表现出更优的预测性能。

Conclusion: GENIE成功利用机器学习克服了复杂模型实时应用的计算挑战，实现了高分辨率的近实时疫情预测，为传染病负担预测提供了可靠的新方法。
...
### [17] [Growth Without Us: Machine Consumers, Corporate Circularity, and the Decoupling of GDP from Humanity after AGI](http://arxiv.org/abs/2608.20231v1)
*Sahil Sharma*

Main category: physics.soc-ph

TL;DR: 本文构建了一个后通用人工智能（AGI）经济模型，证明企业拥有的AI和机器人在零人类消费的闭环经济中可实现高速增长，且产出与人类福利完全解耦，所有权政策成为核心。


<details>
  <summary>Details</summary>
Motivation: 旨在反驳对全面自动化的传统需求侧质疑（即人类无收入则无人消费产出），并探究后AGI经济中经济增长的约束条件及人类福利的相关性。

Method: 建立了一个后AGI经济模型，其中企业拥有的AI与机器人代理作为生产者和消费者进行能源、算力等交易；运用冯·诺依曼扩张经济模型和黄金法则解耦定理进行理论推导。

Result: 发现三点：1) 零人类消费的闭环经济具有正且最大的增长率；2) 增长瓶颈从人类繁衍转向制造产能与能源获取，增速可提升一至两个数量级；3) 产出与人类福利完全解耦，福利仅取决于人类的所有权份额，在最大增长下该份额因人类消费而指数衰减。

Conclusion: 在后AGI经济中，就业政策将完全失效，所有权政策是决定人类最终状态（食利者后稀缺、完全循环解耦或社会化所有制）的唯一关键。
...
### [18] [Video2DoorTraversal: Push Door Traversal via Simulated Door Twins](http://arxiv.org/abs/2608.20251v1)
...
TL;DR: 本文提出Video2DoorTraversal框架，通过单视频实现从真实到仿真再到真实的流程，使轮腿移动机器人能够自主完成高难度的开门与通行任务。


<details>
  <summary>Details</summary>
Motivation: 开门与通行是一个长视距定位操作任务，需要精确的把手交互和协调的底盘-机械臂控制，在实际应用中极具挑战性。

Method: 提出单视频的真实-仿真-真实框架：DoorTwin从RGB视频重建可仿真的门数字孪生；仿真代理迭代优化生成物理可执行的演示数据；利用这些数据训练ArticuACT双深度策略，通过以机器人为中心的相机条件化和交互感知监督预测协调的控制指令。

Result: 系统在机载运行下，在5个真实门上实现96.57%的平均成功率，在未见门上实现80.95%的零样本成功率，完成完整靠近、开门和通行序列的平均时间约为13秒。

Conclusion: 结合数字孪生重建、仿真中迭代技能优化以及双深度策略学习，能够有效解决轮腿机器人自主开门通行问题，并展现出良好的零样本泛化能力。
...
### [19] [An Agentic Approach for Active Data Collection, Travel Behavior Modeling, and Weather-Sensitive Demand Prediction](http://arxiv.org/abs/2608.20320v1)
*Narges Ahmadi,Yubo Jiao,Jônatas Augusto Manzolli,Jiangbo Yu,Luis Miranda-Moreno*

Main category: cs.AI

TL;DR: 本文提出了一种三智能体工作流，整合对话式数据采集、结构化数据处理与行为预测，用于出行行为研究


<details>
  <summary>Details</summary>
Motivation: 出行行为研究中数字数据采集与预测建模通常被分离开发与评估，缺乏统一的工作流将两者协调起来

Method: 构建三智能体工作流：通过聊天机器人管理的图像增强陈述偏好调查收集学生通勤者在五种天气场景下的出行方式选择（454条观测），采用多项Logit模型分析天气关联性，以逻辑回归和随机森林作为机器学习基准，并评估9个本地部署的LLM（2-35B参数）在零样本及persona、few-shot、视觉配置下的预测表现

Result: 随机森林五类准确率为69.6%，最佳纯文本零样本LLM达69.9%，最佳视觉配置达71.5%；习惯出行信息带来最稳定提升，专家框架优于角色扮演，few-shot提示在少量样本后增益趋于稳定

Conclusion: 对话式调查、结构化数据处理、传统行为建模、机器学习与多模态LLM预测可在可审计的多智能体工作流中有效协调；视觉上下文可为部分模型提供额外预测信息
...
### [20] [AI4AI-Bench: Benchmarking LLM Agents in Algorithmic Design for Recursive Self-Improvement](http://arxiv.org/abs/2608.20318v1)
*Yizhe Chi,Wenyi Li,Deyao Hong,Xiaoqiu Wang,Mingju Gao,Kaisen Yang,Bingxiang He,Youjie Zheng,Calvin Xiao,Qinhuai Na*

Main category: cs.AI

TL;DR: 本文提出了AI4AI-Bench，一个评估AI智能体能否重写训练算法的基准，发现当前最强系统仅完成了从现有算法到最优算法之间不到五分之一的改进距离。


<details>
  <summary>Details</summary>
Motivation: 递归自我改进（RSI）的可行性取决于智能体能否设计训练算法，但现有基准无法隔离这一能力——它们主要靠收集数据或调参取胜，而非改变模型学习方式。缺乏专门评估算法设计能力的基准。

Method: 提出AI4AI-Bench，包含10个冻结的研究仓库，覆盖10类训练算法族。智能体在单块B300上有4小时重写训练算法，随后代码从零开始运行最多12小时，由隐藏评估器打分并与原始算法对比。所有任务的不可通约指标映射到0-1统一量表（0=无信息模型，0.1=仓库原始算法，1.0=任务最优）。

Result: 在6个系统的29种配置上，平均得分0.166，最佳系统得分0.250，即最强系统也仅完成了原始算法到最优算法之间不到五分之一的距离。大多数提交根本未改变模型学习方式；少数改变的得分为0.226，未改变的仅0.126。增加推理投入使改变学习方式的提交比例从8%升至64%，平均分从0.094升至0.196。

Conclusion: 当前AI系统在训练算法设计能力上远未达到RSI所需的水平。推理投入主要帮助系统更愿意尝试改变学习方式而非提升改变的质量。发布了完整任务套件、评估器和所有提交以便后续追踪系统能力变化。
...
### [21] [MidTool: Mid-training Data Synthesis for Agentic Tool Use](http://arxiv.org/abs/2608.20314v1)
*Fengqing Jiang,Yite Wang,Boyi Liu,Zhaoyang Wang,Canwen Xu,Zhewei Yao,Radha Poovendran,Yuxiong He*

Main category: cs.AI

TL;DR: 本文提出了MidTool，一个用于增强大语言模型通用工具使用能力的中期训练语料构建管道，并在多个基准测试中取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 通用工具使用是大语言模型的一种重要但较少被探索的智能体能力，现有研究多依赖于后训练，本文旨在解决工具使用能力在中期训练阶段被忽视的问题。

Method: 提出MidTool语料构建管道，结合网络、PDF和代码数据与来自真实工具API、MCP技能和基于文档工作流的合成监督，并在MidTool-Mix数据集上对Qwen3-4B/8B模型进行中期训练及后续的后训练（SFT和RL）。

Result: 与基线模型相比，使用MidTool-Mix进行中期训练的模型在BFCL、tau2-Bench和MCP Universe基准测试中，无论在SFT还是RL后训练下，均取得持续的下游性能提升。

Conclusion: 通用工具使用能力与其他重要的大语言模型能力一样，能从专门的中期训练中获益，而不应完全依赖后训练。
...
### [22] [Break It Down, Pass It On: Cross-Task Skill Transfer in LLM Agents](http://arxiv.org/abs/2608.20274v1)
*Yiyang Feng,Biddut Sarker Bijoy,Niranjan Balasubramanian,Jiawei Zhou*

Main category: cs.AI

TL;DR: 本文研究了LLM智能体技能归纳方式对跨任务迁移效果的影响，并提出了一种无需任务执行的技能实用性评分作为诊断工具。


<details>
  <summary>Details</summary>
Motivation: LLM智能体从已完成任务中归纳的技能在迁移时可能不可靠甚至损害智能体性能，目前关于技能何时能可靠跨任务迁移仍是一个悬而未决的问题。

Method: 通过对比任务级与子任务级技能归纳以及文本与代码技能格式，研究技能归纳方式对迁移的影响；引入了特异性和抽象度两个互补属性，并提出结合两者的技能实用性评分。

Result: 任务级技能通常降低性能，而子任务级技能提升性能；文本技能比代码技能迁移效果更好。特异性和抽象度单独均无法预测任务成功率，但两者的结合（技能实用性评分）与任务成功率一致相关，且子任务级和文本技能得分更高。

Conclusion: 技能实用性评分仅需技能和任务描述而无需任务执行，可作为新任务运行前评估技能记忆的实用诊断工具。
...
### [23] [The Third Restructuring of Software Form: From the Three-Tier Architecture to Storage, Models, and Agents](http://arxiv.org/abs/2608.20201v1)
*Wei Lin,Tao Zhou,Zhaofei Xie,Changgui Hong*

Main category: cs.AI

TL;DR: 本文提出软件形态正迈向由上下文和推理决定行为的“Software 3.0”时代，其最终架构将收敛为通用数据库、大模型和智能体三大核心要素。


<details>
  <summary>Details</summary>
Motivation: 旨在探讨软件架构在智能化时代的演进方向，解决传统三层架构在以大模型和推理为核心的语境下如何重构及收敛的问题，并明确其适用边界。

Method: 提出并形式化“Software 3.0”收敛论点，构建由通用数据库、大模型和智能体组成的最小参考架构，结合真实原型与在线模型进行验证，并系统分析其适用条件与失效边界。

Result: 传统三层架构中UI层被模型按需生成能力吸收，业务逻辑层按“可表达性x关键性”重新划分，数据层成为唯一持久基础设施。该架构在可表达、可验证、外部有状态且工具完备的领域成立，受限于确定性、成本、安全性和可验证性。

Conclusion: Software 3.0范式将重塑开发者角色、数据库行业及软件工程学科，但在确定性等边界条件外的领域仍需保留确定性逻辑作为工具补充。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行精炼输出进行推理，传统的“停止-恢复”工具调用模式会迫使模型在推理稳定前做出工具决策，延迟观察结果，并引入冗余精炼和工具执行，从而损害任务准确性和推理效率。

Method: 提出连续交互扩散（CID）架构，将工具交互集成到迭代去噪中。CID分离出只读事实通道、思维通道（类型化认知张量）和显示通道，允许在调用完全序列化前启动外部读取，将返回结果投影到演变的思维状态中以修改早期认知，并通过持久绑定重用或刷新结果。

Result: 形式化了该架构、运行时和训练目标，并定义了任务质量和端到端效率的评估协议。本论文仅关注只读工具，未提出实证性能声明。

Conclusion: CID旨在更早暴露证据，使工具延迟与模型计算重叠，减少重复的外部工作，并在新证据到达后保留有用的计算。
...
TL;DR: 该论文系统研究了混合专家扩散语言模型的扩展规律，并据此训练出在多项基准测试中表现接近自回归模型的LLaDA MoE v2。


<details>
  <summary>Details</summary>
Motivation: 旨在解决混合专家扩散语言模型扩展行为尚不明确的问题。尽管dLLMs是自回归模型的有效替代方案，但MoE dLLMs在优化、计算分配及架构方面的扩展趋势缺乏系统研究。

Method: 系统刻画了MoE dLLMs的优化超参数、计算分配与架构规模扩展规律，并基于这些发现从零开始在23.5T tokens上训练了30B-A3B规模的LLaDA MoE v2模型。

Result: 发现MoE dLLMs的最优批大小增长更快、学习率衰减更迅速且计算分配向数据侧倾斜。LLaDA MoE v2仅用Qwen3约65%的预训练数据即在多项基准上接近Qwen3，并在SFT后在7/8项推理和编程基准上超越SDAR Chat。

Conclusion: 为MoE扩散语言模型确立了实用的扩展定律和设计原则，证明了其在节省预训练计算量的同时能取得媲美先进自回归模型的性能。
...
TL;DR: 本文提出TAPS方法，通过目标感知前缀选择优化扩散模型推测解码中的验证阶段，实现无损加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型用于并行起草虽降低起草延迟，但验证成为新瓶颈：现有方法按边际概率排序节点，忽略验证以前缀为条件，导致验证被拒绝前缀的不可达后代，增加延迟但接受率提升有限。

Method: TAPS将扩散边际概率转化为路径条件接受估计，在固定验证预算下选择紧凑的前缀闭合子树，优化接受-成本权衡，而非简单扩展起草树。

Result: 在多数据集和模型族上实验表明，TAPS相比自回归解码实现高达7.9倍无损端到端加速，分别超越SOTA方法DFlash和DDTree 1.36倍和1.74倍。

Conclusion: TAPS通过目标感知的前缀选择策略解决了现有起草树方法中边际概率与前缀条件验证之间的不匹配问题，显著提升了推测解码的效率。
...
### [27] [Rule-Compliant Visual Spatial Planning for Multimodal Large Language Models](http://arxiv.org/abs/2608.20237v1)
*Yu Chen,Ting Lei,Yaoyi Li,Jia Cai,Zhecen Wu,Yang Liu*

Main category: cs.AI

TL;DR: 本文提出了RuleMaze基准以评估多模态大语言模型在规则约束下的空间规划能力，并引入解耦多模态规划（DMP）方法显著提升了模型的规则遵循与泛化表现。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在结合语言推理与视觉感知时，其在显式或未见过的规则约束下执行视觉空间规划的能力仍未被充分探索。

Method: 提出了RuleMaze可控基准，并采用语言-逻辑-函数混合方法自动生成规则与验证器；同时引入解耦多模态规划（DMP），通过分离感知、执行和规则验证来实现可解释的推理。

Result: 实验表明，与端到端文本规划基线相比，DMP在规则遵循率和规划成功率上均有显著提升，并能泛化至更复杂和未见的规则。

Conclusion: RuleMaze为研究多模态大语言模型中基于规则的可解释空间规划提供了一个有原则的基准。
...
### [28] [UMER: Unifying Embedding and Ranking via Pair-Aware Discriminative Reasoning for Universal Multimodal Retrieval](http://arxiv.org/abs/2608.18504v1)
*Libiao Chen,Xiyang Liu,Yanheng Wei,Tao Wang,Zhenyu Tang*

Main category: cs.AI

TL;DR: 本文提出UMER框架，通过联合学习多模态嵌入与排序，并采用成对感知判别推理和相互蒸馏策略，有效提升了通用多模态检索的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于思维链的方法独立进行逐项推理，难以区分正样本与易混淆的困难负样本，且对比嵌入在处理需要细粒度推理的元任务时表现不佳，本文旨在解决这些通用多模态检索中的效率和语义推理问题。

Method: 提出UMER框架，在单个多模态大语言模型中联合学习对比嵌入和判别排序；使用成对感知判别推理比较查询-候选对以获取匹配和差异证据；并采用互补的相互蒸馏策略在嵌入和排序功能间传递成对偏好。

Result: 在MMEB-V2基准测试中取得了最先进的性能，并支持预算可调的推理。

Conclusion: 联合嵌入与排序机制以及成对推理策略能有效增强多模态检索的细粒度语义推理和全局匹配能力，为通用多模态检索提供了新的解决方案和未来方向。
...
### [29] [Projecting BrowseComp-Plus onto ClimbMix: Toward More Realistic Corpora for Agentic Search](http://arxiv.org/abs/2608.20317v1)
*Sahel Sharifymoghaddam,Lingwei Gu,Yijun Ge,Jimmy Lin*

Main category: cs.IR

TL;DR: 本文提出一种投影流水线，将BrowseComp-Plus基准的证据转移至大规模ClimbMix语料库中，成功将代理评估的难度转移至检索阶段。


<details>
  <summary>Details</summary>
Motivation: 现有的BrowseComp-Plus基准虽然分离了代理和检索器的评估，但其固定语料库仅包含10万篇针对特定查询构建的文档，缺乏普适性，无法真实反映代理在大规模网络文本中的检索能力。

Method: 提出一种数据集无关的投影流水线，将问题分解为原子推理步骤，并在ClimbMix语料库中为每个步骤寻找证据支撑，经过自动验证、独立代理和人工审核三重确认后保留问题。

Result: 该流水线从830个测试问题中筛选出57个完全有支撑的问题；在新的语料库下，最强代理的答案准确率下降5个百分点，证据召回率从84.3%降至21.4%，搜索调用次数增加63%。

Conclusion: 该投影流水线适用于任何可分解为可验证事实的基准，有效将评估难度转移到检索任务上，相关代码、基准和分析已开源。
...
TL;DR: 本文提出了Consistency Forcing (CForce)方法，通过强制早期去噪阶段的掩码预测与后期阶段对齐，解决了扩散大语言模型在激进并行策略下早期预测不可靠和错误传播的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散大语言模型在采用激进并行策略时，早期去噪阶段的预测不可靠，会导致错误传播到后续阶段，从而影响生成质量。

Method: 提出CForce蒸馏方法，在预收集的自展开轨迹上训练以强制早期阶段与后期阶段的掩码预测对齐；引入Confidence Adaptive KL Divergence作为蒸馏目标；并提供理论分析证明其能近似最小化早期预测误差。

Result: 在非编辑和可编辑LLaDA模型上的实验表明，CForce在尤其是高并行解码预算下，显著改善了生成速度与质量的权衡。

Conclusion: CForce成功提升了扩散大语言模型在并行解码下的性能，且其统一公式适用于掩码到token和可编辑解码任务，为后续高并行生成提供了有效方案。
...
TL;DR: 本文揭示了扩散大语言模型的安全机制漏洞，提出一种基于安全神经元引导的低成本离线黑盒越狱框架。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（DLLMs）使用迭代并行去噪取代了自回归预测，但其内部安全机制尚不明确。本文旨在研究DLLMs的安全对齐机制并暴露其潜在的机制漏洞。

Method: 通过分析发现DLLMs的安全对齐具有稀疏性和跨架构可迁移性。在此基础上，提出SN-Guided Diffusion离线黑盒越狱框架，利用加权安全神经元损失引导扩散过程远离安全触发区域。

Result: 安全神经元自剪枝和迁移剪枝大幅提升了攻击成功率（如LLaDA的ASR从2.6%升至73.8%）。所提越狱框架在良性与越狱提示判别上达到AUROC=1.0，在Llama-3、Qwen2.5及Gemini等多个目标上实现了高迁移ASR（最高达86.9%），且生成成本比现有框架低数个数量级。

Conclusion: DLLMs继承自自回归模型的安全对齐机制稀疏且脆弱，容易遭受基于安全神经元的低成本迁移攻击，揭示了扩散式安全对齐的机制漏洞。
...
TL;DR: 本文提出块混合注意力机制，在保留活跃去噪块内精确softmax注意力的同时对先前块应用线性注意力，从而加速扩散语言模型的推理。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型(dLLM)虽通过并行解码加速推理，但块级半自回归解码中每个去噪步仍需对所有先前块进行注意力计算，尽管使用了KV缓存仍反复产生前缀注意力开销，成为推理瓶颈。

Method: 提出块混合注意力(block-hybrid attention)：在活跃去噪块内保留精确softmax注意力，对先前块应用线性注意力；遵循LoLCAT方法，将LLaDA 2.1（16B参数dLLM）20层注意力中的6层替换为线性注意力，仅需约60小时后训练即可完成转换。

Result: LLaDA-Hybrid在基准测试上基本保持性能：HumanEval 72.0% vs 75.6%、MBPP+ 63.0% vs 57.7%、CMATH 86.7% vs 88.3%；借助Triton实现，解码吞吐量最高提升1.7倍，且在内存耗尽前可支持更多并发请求。

Conclusion: 预训练扩散语言模型可被高效线性化以实现更快推理，块混合注意力在几乎不损失性能的前提下显著提升了dLLM的推理效率和并发能力。
...
TL;DR: 本文提出ERILS方法，通过引入外部更强策略生成的高奖励rollout并解决长度差异和奖励处理不稳定问题，提升扩散语言模型的强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（dLLM）的强化学习方法依赖目标模型自身生成的同策略rollout，当成功rollout稀缺时，训练难以获得正向奖励且进展有限。

Method: 提出ERILS（External Rollout Integration with Length Control and Source-Specific Processing），在引入外部更强策略生成的高奖励rollout时，对外部rollout长度进行控制，并对同策略和外部rollout的奖励分别处理。

Result: 在Sudoku、Countdown和MATH500三个任务的零样本评估中，ERILS均提升了多样本性能。在Sudoku上，ERILS达到98.4%的best-of-4完成准确率，而最强基线仅为40.3%；在不同生成长度下保持约90%的确定性单完成准确率。组件分析表明长度控制的外部rollout优于无控制rollout，且来源特定奖励处理避免了联合奖励处理导致的训练崩溃。

Conclusion: rollout构建和奖励处理是外部rollout集成至dLLM强化学习中的重要设计维度，合理的长度控制和分源奖励处理对训练稳定性和性能提升至关重要。
...
TL;DR: 本文提出了大规模动态心电图多模态数据集Holtercare-23K及基准测试Holtercare-Bench，以解决现有模型在动态心电图时序推理和报告生成中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型主要针对静态图像或短期信号，由于缺乏高质量数据集和基准，在处理动态心电图时难以进行复杂的时序推理和诊断报告生成。

Method: 构建了包含22,980个QA对并实现信号-视频-文本三模态对齐的数据集Holtercare-23K，并提出了评估时序定位、临床诊断和全局摘要的多模态基准Holtercare-Bench。

Result: 对领先模型的零样本评估显示其在处理超长病理序列时存在显著性能差距，而对代表性模型进行微调则取得了显著改进。

Conclusion: 本研究揭示了当前多模态大语言模型在电生理学领域的局限性，并为长期医疗多模态大语言模型提供了基础性基准。
