...
### [1] [Unleashing Multimodal Large Language Models for Training-free HOI Detection in the Wild](http://arxiv.org/abs/2607.13881v1)
*Ting Lei,Jialin Liu,Zhu Xu,Yuxin Peng,Yang Liu*

Main category: cs.CV

TL;DR: AgentHOI是一个无需训练的智能体框架，通过协调视觉基础模型进行多轮推理和多方面定位，实现开放世界的人-物交互检测。


<details>
  <summary>Details</summary>
Motivation: 传统HOID方法依赖特定数据集的监督，难以泛化到开放世界场景；现有基于提示的方法未充分利用多模态推理能力，无法为模糊交互提供有效的上下文推理。

Method: 提出无需训练的智能体框架AgentHOI，模块化协调视觉基础模型进行语义推理和空间定位，并引入上下文感知多轮推理与多方面交互定位两个关键机制来优化交互发现和定位精度。

Result: 实验表明，AgentHOI在无需HOID训练数据的情况下，其性能超越了现有最先进的监督和弱监督方法。

Conclusion: 通过将基础模型的多模态推理能力迁移至HOID，AgentHOI证明了无需训练的智能体方法在开放世界和复杂交互场景中的有效性。
...
### [2] [Towards Enhancing 3D Spatial Reasoning in Medical Multimodal Large Language Models](http://arxiv.org/abs/2607.13860v1)
*Zhuoyuan Fu,Zeshang Li,Yiqiong Zhang,Hangui Lin,Yan Shu,Yan Li,Binyang Li,Yaru Zhao*

Main category: cs.CV

TL;DR: 本文提出了一种逐切片数据合成范式构建结构化推理数据集，使2D预训练的多模态大语言模型能够实现具有可解释临床逻辑的3D体积医学图像理解。


<details>
  <summary>Details</summary>
Motivation: 旨在解决多模态大语言模型扩展至3D体积成像时面临的标注成本高昂、数据集不透明，以及现有数据格式（如VQA对或非结构化临床报告）无法捕捉明确临床推理过程的问题。

Method: 提出受放射科医生诊断流程启发的逐切片数据合成范式，将3D阅读过程分解为逐切片观察并合成为可解释的思维链，并在指令微调阶段对2D预训练模型引入序列空间跟踪、多切片空间感知和鉴别排除等临床原则。

Result: 在多个3D医学基准测试中，该方法显著优于2D基线模型，展现出强大的空间推理能力，且性能可媲美资源密集型的原生3D架构。

Conclusion: 这种以数据为中心的策略无需进行计算昂贵的3D特定预训练，即可解锁深度的体积理解和高度可解释的临床逻辑，并且已开源数据集和训练流程。
...
### [3] [EgoProceVQA: A Novel Egocentric Procedural Understanding Task with Self-Skill-Exploration Agent](http://arxiv.org/abs/2607.13792v1)
*Junlong Li,Junxi Li,Yuxiang Yang,Wenbin Zou,Lap-Pui Chau,Yi Wang*

Main category: cs.CV

TL;DR: 本文提出了第一人称程序化理解问答任务（EgoProceVQA）及基准数据集，并开发了一个自我技能探索智能体框架以提升多模态大语言模型的程序化推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的第一人称视频理解评估缺乏对程序化理解和关键步骤级复杂推理的关注，而这对于构建可部署在可穿戴设备上的程序化AI助手至关重要。

Method: 提出EgoProceVQA任务和EgoProceGen数据生成平台构建基准测试；进一步提出EgoProceAgent智能体框架，利用通用工具库和标准化子技能库进行无真值监督的自我探索，以发现有效的技能组合策略。

Result: 评估表明现有MLLMs和智能体在程序化理解上仍有很大提升空间，而EgoProceAgent在多个任务中取得了开源模型中的最优性能。

Conclusion: 本文提出的基准测试、生成平台和智能体框架为第一人称程序化理解任务建立了一个统一的基础。
...
### [4] [Groc-PO: Grounded Context Preference Optimization for Truthful Multimodal LLMs](http://arxiv.org/abs/2607.13712v1)
*Zhixiao Zheng,Zheren Fu,Zhiyuan Yao,Chunxiao Liu,Dongming Zhang,Zhendong Mao*

Main category: cs.CV

TL;DR: 本文提出基于接地上下文偏好优化（Groc-PO）的框架，通过在多个接地阶段引入显式偏好监督，缓解多模态大语言模型中跨阶段误差传播导致的不忠实问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在视觉幻觉、内容捏造和不忠实推理等问题，而基于人类偏好的对齐方法（如DPO）仅在最终答案层面进行优化，无法对早期接地阶段的错误提供阶段特定监督，难以抑制误差在推理链路中的传播。

Method: 提出Groc-PO框架，构建涵盖物体接地、上下文接地和接地推理三阶段的多阶段偏好数据集GCPD，在多个接地阶段引入显式偏好优化监督，强化上下文依赖推理并减少跨阶段误差传播。

Result: 大量实验表明，与标准DPO及其他强基线相比，Groc-PO在幻觉缓解、忠实推理和整体可靠性方面均取得性能提升。

Conclusion: 在多个接地阶段提供显式偏好监督对可信多模态推理具有重要价值，能够有效增强上下文依赖推理能力并提升模型可信度。
...
### [5] [GeoAnchor: Collaborative Reasoning via Latent Decomposition for 3D Spatial Understanding](http://arxiv.org/abs/2607.13454v1)
*Hao Li,Han Fang,Zixin Pan,Xin Wei,Hongbo Sun,Jinglin Xu,Zhiyu Lin,Ye Yuan,Zhongjiang He,Yu Yu,Hao Sun*

Main category: cs.CV

TL;DR: 本文提出了一种名为GeoAnchor的交错文本-潜在推理框架，通过将3D空间信息分解为三种互补的潜在表示，以增强多模态大语言模型对2D图像中3D空间关系的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在从2D图像理解3D空间关系时面临挑战，主要因为符号文本标记缺乏表示连续几何信息的保真度，且依赖单一潜在表示无法适应空间任务的多样性，导致复杂几何场景下的错位。

Method: GeoAnchor将3D空间信息分解为用于目标定位的位置潜在表示、用于关系方向的方向潜在表示以及用于场景结构的几何潜在表示，并在结构化空间中重组以构建局部证据并捕获全局上下文。此外，引入协同训练策略，引导模型从局部空间感知过渡到全面的3D理解。

Result: 在多样且复杂的3D推理任务上的大量实验表明，GeoAnchor优于现有最先进的方法。

Conclusion: GeoAnchor框架有效提升了多模态大语言模型对3D空间的理解，验证了其有效性和泛化能力。
...
### [6] [Open-KNEAD: Knowledge-grounded Nutrition Estimation via Agentic Decomposition](http://arxiv.org/abs/2607.12911v1)
*Bruce Coburn,Jingbo Yue,Jinge Ma,Siddeshwar Raghavan,Gautham Vinod,Fengqing Zhu*

Main category: cs.CV

TL;DR: 本文提出了Open-KNEAD，一个无训练且可本地部署的知识驱动型智能体框架，通过营养感知检索和食谱先验步骤，在保护隐私的同时提升了膳食营养估计的准确性和可追溯性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs的直接估计已超越传统的检索增强方法，但临床应用仍需要准确的份量估计、可追溯的逐项记录、最小用户负担和隐私保护。本文旨在解决如何在这些临床关键需求下有效利用检索技术的问题。

Method: 提出Open-KNEAD框架，将食物分解并通过营养感知检索匹配到FNDDS数据库代码以生成可审计记录；引入智能体内部的食谱先验步骤以恢复不可见的烹饪添加能量；全程在本地硬件上运行以保证隐私。

Result: 在两个开源MLLM系列和三种菜系上的大多数设置中，Open-KNEAD在份量估计上优于先前的接地方法和直接估计；在经验证过的ACETADA数据集上，本地开源智能体的份量估计超越两个前沿闭源模型约30%和53%。

Conclusion: 检索技术仍能在保障隐私和可解释性的前提下提供准确的份量和可追溯记录；作者开源了Open-KNEAD框架及其FNDDS知识库，为临床膳食评估提供了有效工具。
...
### [7] [EvoGraph-R1: Self-Evolving Multimodal Knowledge Hypergraphs for Agentic Retrieval](http://arxiv.org/abs/2607.12764v1)
...
TL;DR: 本文提出EvoGraph-R1框架，将知识图谱重构为动态环境，通过马尔可夫决策过程驱动智能体在检索-推理中进行交互式自进化，以解决静态GraphRAG的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有GraphRAG方法将知识图谱视为静态的离线数据结构并进行单趟查询，这与知识密集型推理的交互和迭代特性不符，导致跨模态推理受阻、结构冻结无法纠错更新、以及缺乏自适应优化的单趟检索三大瓶颈。

Method: 将检索形式化为马尔可夫决策过程（MDP），智能体通过执行图谱查询、网络搜索、图谱编辑或终止作答等动作来重塑超图结构，并生成反馈信号以整合新证据、纠正错误与优化结构，从而实现闭环的多跳推理进化。

Result: 在多模态视觉问答（VQA）和文本问答基准测试中，相较于现有RAG基线，该模型在准确率、覆盖率和可追溯性方面均取得了显著提升。

Conclusion: 自进化的知识图谱作为跨模态的基础范式，能够有效支持复杂的迭代式知识推理，突破传统静态检索的局限。
...
### [8] [DominoTree: Conditional Tree-Structured Drafting with Domino for Speculative Decoding](http://arxiv.org/abs/2607.08642v2)
*Saw S. Lin,Jyh-Shing Roger Jang*

Main category: cs.CL

TL;DR: 本文提出DominoTree，一种无需训练的best-first draft tree方法，利用Domino的条件非因子化校正路径评分，在多个基准上实现高达6.6倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有的block-diffusion drafters（如DFlash）仅建模per-position marginals，而DDTree等best-first tree方法无法表示Domino drafter引入的基于GRU的路径依赖条件校正结构，导致无法充分利用Domino的校正能力来构建更优的draft tree。

Method: 提出DominoTree，一种无需训练的best-first draft tree，沿每条root-to-node路径使用Domino的条件、非因子化校正进行评分，通过将per-node校正限制在candidate top-M集合实现实用化；并采用GPU原生的CUDA-graph builder构建树，与参考Python实现比特级一致，保证接受率不变的同时降低树构建开销。

Result: 在Qwen3-4B上跨8个基准达到最高6.6倍加速，所有测试温度下平均接受长度最高（最高10.7 tokens/round）；相比Domino decoder吞吐量提升9%-10%（Alpaca上最高22%），在所有温度下均优于DDTree和CaDDTree。在Qwen3-8B上T=0时相比DDTree吞吐量提升24%，高温下优势缩小至持平或小幅落后。

Conclusion: DominoTree通过利用Domino的非因子化条件校正构建draft tree，在多个模型和温度设置下实现了显著的推理加速和更高的接受长度，证明了路径依赖校正对draft tree构建的价值；GPU-native CUDA-graph builder有效降低了树构建开销。
...
### [9] [Cost-Aware Diffusion Draft Trees for Speculative Decoding](http://arxiv.org/abs/2606.01813v1)
*Shuai Zhang,Huachuan Qiu,Hongliang He,Yong Dai*

Main category: cs.CL

TL;DR: 提出CaDDTree方法，通过联合优化树结构和节点预算直接最大化token吞吐量，解决了投机解码中预算选择缺乏原则性基础的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿器（如DDTree）在构建候选树时，接受长度随预算非递减，总是偏好更大的树而忽略验证成本，缺乏选择预算的原则性基础，导致无法优化实际推理吞吐量。

Method: 提出CaDDTree（成本感知扩散草稿树），通过联合选择树结构和节点预算直接优化token吞吐量。该方法显式建模草稿和验证延迟，证明在凸验证成本下吞吐量函数是单峰的，实现高效的贪婪停止规则，并在线自适应调整预算，无需离线搜索。

Result: 在Qwen3-4B和Qwen3-8B模型上的8个基准测试（涵盖推理、编码和指令遵循任务）表明，CaDDTree在几乎所有任务上均匹配或超越了使用Oracle预算选择的DDTree。

Conclusion: CaDDTree提供了一种原则性强且高效的投机解码草稿树构建方法，通过显式考虑验证成本优化吞吐量，未来可能进一步应用于其他大模型推理加速场景。
...
### [10] [Accelerating Speculative Decoding with Block Diffusion Draft Trees](http://arxiv.org/abs/2604.12989v1)
*Liran Ringel,Yaniv Romano*

Main category: cs.CL

TL;DR: DDTree通过从块扩散模型构建草稿树，提升了DFlash推测解码的接受长度和整体性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash推测解码方法每轮仅验证单一草拟轨迹，限制了接受长度，从而未能充分利用并行验证的潜力。

Method: 提出DDTree方法，利用块扩散草拟模型的位置分布直接构建草稿树，并在固定节点预算下使用最佳优先堆算法选择最可能匹配目标模型的连续片段，最后通过仅祖先注意力掩码在单次前向传播中完成验证。

Result: 基于DFlash的DDTree克服了单一轨迹的限制，使其跻身推测解码的领先方法之列。

Conclusion: 将块扩散草拟模型的输出扩展为树结构可以有效提高推测解码的接受长度和效率。
...
### [11] [Rethinking Penetration Testing for AI-Enabled Systems: From Resource Compromise to Behavioral Objective Violation](http://arxiv.org/abs/2607.14006v1)
*Mohammad Allahbakhsh,Mohammad Hassan Bahari,Moslem Attar-Raouf*

Main category: cs.CR

TL;DR: 该论文将AI系统的渗透测试重新定义为以目标为导向的行为评估，以应对针对AI行为而非底层基础设施的对抗性攻击。


<details>
  <summary>Details</summary>
Motivation: 传统渗透测试无法应对AI系统中的新型攻击途径（如提示词注入、数据投毒等），攻击者可在不破坏基础设施的情况下改变AI系统行为，因此需要新的评估范式。

Method: 提出以目标为导向的行为评估工作流，包括识别操作目标、映射AI治理行为、分析对抗性影响面、定义失败标准、执行场景测试并报告关联证据。

Result: 通过AI安全运营中心助理的案例，演示了通过行为影响而非基础设施破坏进行渗透的过程，并整合定义与工作流形成了技术评估框架。

Conclusion: 所提出的定义和工作流为评估已部署AI系统中的对抗性风险提供了技术框架，有效扩展了传统渗透测试的适用范围。
...
### [12] [Agent Skill Security: Threat Models, Attacks, Defenses, and Evaluation](http://arxiv.org/abs/2607.13987v1)
*Sanket Badhe,Priyanka Tiwari*

Main category: cs.CR

TL;DR: 本文提出了SkillSec-Eval框架，用于系统评估大型语言模型智能体可重用技能在整个生命周期中的安全性。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM智能体安全研究主要关注提示注入和运行时执行，忽略了可重用技能在更广泛生命周期中的安全风险。

Method: 提出生命周期感知框架SkillSec-Eval，构建涵盖存储库准入、语义检索、规划器选择、执行和技能演进的威胁分类体系，并基于327个真实世界技能库进行实证评估。

Result: 评估发现，除运行时执行阶段外，安全漏洞在技能生命周期的多个其他阶段同样存在。

Conclusion: 研究强调了针对可重用智能体技能进行全生命周期安全分析的必要性。
...
### [13] [Building Shor's Algorithm in Lean: An Agentic Formalization of Quantum Attacks on RSA-2048 and P-256](http://arxiv.org/abs/2607.14082v1)
*Lei Zhang,Yusheng Zhao,Hongshun Yao,Xin Wang*

Main category: quant-ph

TL;DR: 论文利用智能体形式化方法在Lean中证明了Shor算法及其变体，为RSA-2048和P-256的量子密码分析提供了机器检验基础。


<details>
  <summary>Details</summary>
Motivation: 旨在解决Shor算法及其变体在Lean形式化过程中的高难度挑战，以准确分析RSA-2048和P-256等密码学场景下的量子攻击。

Method: 采用智能体形式化方法：软件代理分析源码、编写并修复Lean代码，辅以人工审查和机器证明检查，形式化了从求阶算法到可逆量子电路的逻辑资源估计。

Result: 在Lean中成功建立了分析RSA-2048和P-256量子攻击的数学基础，形式化了相关量子逻辑资源估计，并提供了经典运算的额外估计。

Conclusion: 该成果为更广泛的机器检验量子密码分析铺平了道路，是迈向AI辅助量子算法设计与验证的重要一步。
...
### [14] [AI-accelerated End-to-End Framework for Rapid Professional Upskilling](http://arxiv.org/abs/2607.14044v1)
*Tam Nguyen,Hung Nguyen,Robert Ogburn*

Main category: cs.AI

TL;DR: 本文提出了一个在技能提升项目五个阶段应用AI加速的端到端框架，并获得了行业验证。


<details>
  <summary>Details</summary>
Motivation: 企业技能差距关闭时间不断延长，而现有框架通常仅能加速单一阶段且缺乏行业验证，无法满足大规模劳动力重塑技能的需求。

Method: 提出一个端到端框架，在知识获取、内容开发、内容审查与验证、教学和评估开发五个阶段应用AI加速，并重点关注生产与学习效率。

Result: 该框架获得美国国家州会计委员会协会的CPE学分批准；3名学习者极短时间内通过NVIDIA Agentic AI认证考试；其知识库支持生成包含1267个风险项的数据集以进行复杂下游分析。

Conclusion: 该AI加速端到端框架有效提升了技能培训效率并通过了行业验证，能够支持复杂的下游分析任务，为解决企业技能差距提供了可行方案。
...
### [15] [Do Agent Optimizers Compound? A Continual-Learning Evaluation on Terminal-Bench 2.0](http://arxiv.org/abs/2607.14004v1)
*Wenxiao Wang,Priyatham Kattakinda,Soheil Feizi*

Main category: cs.AI

TL;DR: 本文研究了代理优化增益是否能在持续学习中累加，发现仅有引入回归控制的RELAI-VCL方法能实现持续提升并避免灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有代理优化方法多为一次性测试，未评估实际部署中递归优化新任务时增益是否能累加而不被侵蚀。

Method: 基于Terminal-Bench 2.0构建两阶段持续学习评估，在同等预算下对比GEPA、Meta Harness和RELAI-VCL三种代理优化方法。

Result: 尽管三种方法在静态测试中均有提升，但在持续学习场景中仅RELAI-VCL能正向迁移并持续改进，以76.4%的终身平均通过率显著优于其他方法。

Conclusion: 只有在优化循环中内置回归控制以避免无法泛化的捷径方案时，代理优化的增益才能实现累加。
...
TL;DR: 提出TAPS方法，通过目标感知的前缀选择将扩散模型的边缘概率转化为路径条件接受估计，从而优化推测解码中的验证瓶颈并实现显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的并行草稿推测解码方法将瓶颈转移到了验证阶段。现有方法使用边缘概率对草稿树节点排序，忽略了验证的前缀条件性，导致验证被拒绝前缀的不可达后代，增加了延迟且接受率有限。

Method: 提出TAPS（目标感知前缀选择），将扩散边缘概率转化为路径条件接受估计，并在固定的验证预算下选择紧凑的前缀闭子树，以改善接受成本权衡。

Result: 实验表明，TAPS相比原始自回归解码实现了高达7.9倍的无损端到端加速，且分别比最先进的DFlash和DDTree高出1.36倍和1.74倍。

Conclusion: TAPS通过解决草稿树验证中的前缀条件性不匹配问题，有效优化了验证效率，大幅提升了无损推测解码的端到端加速比。
...
### [17] [FormalAnalyticGeo: A Neural-Symbolic Based Framework for Multimodal Analytic Geometry Problem Generation](http://arxiv.org/abs/2607.12982v1)
*Ruoran Xu,Wending Gao,Qiufeng Wang*

Main category: cs.AI

TL;DR: 本文提出了FormalAnalyticGeo框架，利用形式语言和多LLM组件全自动生成高质量的多模态解析几何题目，并构建了AnalyticGeo7K数据集。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在解析几何领域的发展因缺乏标注样本而受限，且现有的图表生成方法无法处理约束驱动的布局，缺乏精确渲染圆锥曲线的几何精度。

Method: 提出一种基于条件描述语言（CDL）作为中间表示的框架，结合符号距离场（SDF）引擎进行精确图表渲染。框架包含四个专门的LLM组件（生成器、形式化转换器、测量器、质量验证器），通过质量验证器的结构化反馈形成闭环，实现无需人工标注的全自动生成。

Result: 大规模应用该框架生成了包含超7000个已验证多模态题目的AnalyticGeo7K数据集。实验表明，生成题目的中值真值相对误差为0.70%，82.3%的答案在精确符号解的5%误差范围内。

Conclusion: 该框架与数据集有效解决了解析几何题目生成中的精度与规模问题，且将公开发布，以推动多模态大模型在解析几何推理领域的研究。
...
### [18] [Do We Really Need Multimodal Emotion Language Models Larger Than 1B Parameters?](http://arxiv.org/abs/2607.12787v1)
*Kaiwen Zheng,Junchen Fu,Wenhao Deng,Hu Han,Joemon M. Jose,Xuri Ge*

Main category: cs.AI

TL;DR: 本文提出了轻量级多模态情感识别框架Light-MER，通过知识蒸馏在保持高性能的同时显著提升了推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在多模态情感识别中性能提升伴随着参数量剧增，导致计算成本高且推理效率低，阻碍了在资源受限平台上的实时部署。

Method: 提出Light-MER框架，利用知识蒸馏将大规模教师模型知识转移至轻量级学生模型，并采用结合Sliced Wasserstein Distance的最优传输损失与基于GRPO的多奖励优化策略来增强知识转移。

Result: 在九个基准数据集上的广泛实验表明，Light-MER达到了最先进的性能，同时显著提升了推理效率。

Conclusion: 小型多模态情感语言模型在高质量情感识别中具有巨大潜力，挑战了必须依赖大模型的固有假设。
...
### [19] [Lyapunov Exponent as Physics-Informed Dense Reward: RL Discovery of Stabilization Beyond the Kapitza Pendulum](http://arxiv.org/abs/2607.14001v1)
*Slava Andrejev*

Main category: cs.LG

TL;DR: 本文提出使用李雅普诺夫特征指数（LCE）作为密集奖励信号，以解决通过垂直运动稳定倒立摆的强化学习问题。


<details>
  <summary>Details</summary>
Motivation: 旨在解决通过垂直运动稳定倒立摆的强化学习问题，通过引入密集奖励信号来克服传统方法在指导智能体学习复杂稳定策略时的困难。

Method: 将李雅普诺夫特征指数（LCE）作为强化学习智能体的密集奖励信号进行策略优化。

Result: 智能体不仅成功发现了被称为Kapitza摆的振荡运动，还抑制了摆的枢轴转动，使其保持在严格直立的位置。

Conclusion: LCE作为密集奖励信号在倒立摆稳定控制中十分有效，能够引导智能体自主探索出复杂的振荡稳定机制并实现严格直立稳定。
...
### [20] [TRACE: Turn-level Reward Assignment via Credit Estimation for Long-Horizon Agents](http://arxiv.org/abs/2607.13988v1)
*Leitian Tao,Baolin Peng,Wenlin Yao,Tao Ge,Hao Cheng,Mike Hang Wang,Jianfeng Gao,Sharon Li*

Main category: cs.LG

TL;DR: 本文提出TRACE，一种用于智能体强化学习的密集信用分配方法，通过时序差分变化推导每步动作奖励，显著提升了长周期工具使用能力。


<details>
  <summary>Details</summary>
Motivation: 多轮智能体在长周期工具交互中面临结果奖励稀疏且方差大的问题，仅基于结果的训练会错误地将失败轨迹中的有用动作与最终错误一同赋予负优势。

Method: TRACE在工具调用边界将轨迹表示为状态转换，利用冻结参考模型的标准答案对数概率生成对数比率状态值，并将该值的时序差分变化作为每步动作的奖励，无需额外评论家或过程标签训练。

Result: 在BrowseComp-Plus基准上，TRACE通过纯强化学习将Qwen3-4B从7.2提升至35.6，Qwen3-30B-A3B从8.4提升至42.6；且学到的搜索行为可迁移至开网基准，训练收敛更快。

Conclusion: TRACE有效解决了长周期智能体强化学习中的信用分配难题，无需冷启动微调或中间训练即可大幅提升模型的复杂搜索能力，并具备良好的泛化性。
...
### [21] [PhysClaw-0: A Symbiotic Agentic System for Robot Autonomy via Language Corrections](http://arxiv.org/abs/2607.14047v1)
...
TL;DR: PhysClaw-0是一个人机共生智能体系统，通过在多轮次中保留和重用自然语言纠正，降低了机器人操作策略学习数据收集的人工监督成本。


<details>
  <summary>Details</summary>
Motivation: 现有的自主数据收集流水线在失败重复出现时需要重新发布针对单次回合的修正，导致监督成本随会话长度而非不同问题的数量增加。

Method: 该系统自主收集、验证和重置数据，仅在重试预算耗尽时才暂停以供远程操作员介入。LLM解析器将自然语言指令映射为存储在“纠正记忆”中的结构化调整，从而在相同条件下避免重复纠正。

Result: 在真实机器人桌面清理测试平台中，PhysClaw-0在匹配遥操作回合成功率的同时，将人工工作时间降至16%。语言纠正在所有四种评估设置中提高了验证器与人工的一致性，并将平均单次尝试成功率从12.5%提升至47.5%。使用其数据微调的策略以极低的人工成本达到了遥操作训练策略的成功率。

Conclusion: 通过保留和重用自然语言纠正，人机共生系统能够在不牺牲数据质量和策略性能的前提下，有效减少数据收集过程中的人工干预。
...
### [22] [The Dynamic Verifiable Multi-Agent Human Agentic Loyalty Loop (DVM-HALL) Model and the Net Human-Agent Score (NHAS) in Autonomous Commerce](http://arxiv.org/abs/2607.13998v1)
*Sai Srikanth Madugula,Peplluis Esteva de la Rosa,Daya Shankar*

Main category: cs.SI

TL;DR: 本文提出了DVM-HALL模型，以重新定义代理型人工智能时代的客户忠诚度，整合了可验证执行机制和人机对齐度量标准。


<details>
  <summary>Details</summary>
Motivation: 代理型AI的快速发展颠覆了传统的客户忠诚度范式，现有模型未能考虑算法有限理性和AI自主执行购买决策的能力，需要对消费者-品牌关系进行结构性重新评估。

Method: 提出动态可验证多智能体人类代理忠诚度循环（DVM-HALL）模型，使用softmax概率公式化品牌选择并包含递归更新机制。引入Net Human-Agent Score (NHAS)指标以衡量人机对齐，并集成了针对DeFi环境的可验证执行层（考虑gas成本、MEV暴露等执行风险）。

Result: 建立了将人类情感资产、机器体验效用、校准信任、委派权限及可验证执行风险作为核心预测因子的理论框架，并设计了可审计、风险加权的人机对齐度量指标。

Conclusion: 该框架为品牌应对即将到来的机器客户转型提供了基础理论，并提出了包含受控购物实验、多智能体市场模拟和DeFi测试床的三阶段实证验证计划。
...
### [23] [Early Adoption of Agentic Coding Tools by GitHub Projects](http://arxiv.org/abs/2607.14037v1)
*Maliha Noushin Raida,Daqing Hou*

Main category: cs.SE

TL;DR: 本文分析了GitHub上的25,264个智能体PR，发现智能体编码工具的采用集中在少数项目，且人机协作主要呈现单人监督模式。


<details>
  <summary>Details</summary>
Motivation: 以往研究多关注智能体贡献的PR级别结果，但对智能体编码工具在项目层面的采用和管理情况缺乏了解。

Method: 通过分析来自2,361个热门GitHub存储库的25,264个智能体PR，调查了智能体工具的采用率、项目级智能体PR生产力以及人机协作模式。

Result: 多数项目在三个月内仅产生1至2个智能体PR，密集采用集中于少数项目；小项目的参与度和活跃度更高；项目间智能体PR生产力差异显著，多数低于行业估计阈值；人机协作以单人监督模式为主。

Conclusion: 研究为开源项目如何组织人类监督智能体工具提供了早期实证证据，表明智能体贡献的成功整合不仅取决于智能体能力，还依赖于人类和组织流程，未来需持续追踪采用模式的演变。
...
### [24] [VisualRepair: Dynamic Tool Calling and Region Focusing for Visual Software Issue Repair](http://arxiv.org/abs/2607.14075v1)
*Jingyu Xiao,Zhongyi Zhang,Haoran Hou,Yuxuan Wan,Yuan Jiang,Yintong Huo,Michael R. Lyu*

Main category: cs.SE

TL;DR: 本文提出了基于多模态大语言模型的VisualRepair框架，通过类型感知工具调用和动态区域聚焦提升视觉软件问题修复的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有自动程序修复难以有效利用错误截图中的视觉信息，且异构图像类型和大量无关区域增加了模型感知负担并干扰修复。

Method: 提出VisualRepair框架，包含图像类型感知工具调用（ITTC）模块用于分类并动态调用定制工具链，以及动态测试时区域聚焦（DTRF）模块通过自适应缩放策略精炼错误区域以提升故障定位和补丁多样性。

Result: 在SWE-bench Multimodal基准测试中，VisualRepair在测试集和开发集上分别解决196和25个实例，超越最强基线10和11个实例，取得最优性能。

Conclusion: 类型感知的视觉理解与区域聚焦定位机制能有效提升自动化视觉软件问题修复的效果。
