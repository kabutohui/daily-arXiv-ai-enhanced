...
TL;DR: 本文提出了CFD框架，一种结合边缘端离线字幕与云端按需关键帧检索的代理框架，旨在在预算受限下实现高效的长视频理解。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上的长视频理解面临计算和带宽预算紧张的问题；视觉标记二次采样会丢失时间结构，而纯文本记忆会丢失细粒度视觉属性。

Method: 提出CFD框架：边缘端进行单次离线字幕生成以构建双轨叙事索引并跨查询复用；云端MLLM在故事优先循环中推理，核心的轻量级Visual-Need Router门控模块仅针对感知问题触发有限的关键帧检索，其余时间结构问题保留在语言空间内。

Result: 在长视频基准测试中表现出强大的准确率与效率权衡，同时大幅减少了在线视觉处理开销。

Conclusion: 通过将视觉访问转化为以查询为条件的成本并限制每查询的帧消耗，CFD框架有效解决了边缘设备长视频理解的预算约束瓶颈。
...
TL;DR: 本文提出了一种基于离散扩散大语言模型的放射学报告生成框架（DRRG），通过迭代去噪实现双向报告优化，优于传统的自回归方法。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归放射学报告生成方法无法修改已生成的内容，易导致误差传播且不符合实际报告的迭代修改过程。为了解决这一问题并探索离散扩散模型在该领域的潜力，本文开展了相关研究。

Method: 开发了DRRG框架，将报告生成转化为迭代掩码令牌去噪过程；引入临床实体感知互补掩码以提高监督覆盖率和强调关键实体，并采用概念条件模块将图像衍生的临床概念注入视觉表征中。

Result: 在MIMIC-CXR数据集上，DRRG在大多数指标上优于对比方法（如BLEU-4为0.210，RaTEScore为0.604），尽管其LLM解码器体积小得多。在CheXpert Plus数据集上，DRRG也取得了最高的BLEU-4（0.119）和CheXpert-F1（0.347）。

Conclusion: 离散扩散模型通过实现迭代和双向报告优化，为自回归报告生成提供了有效的替代方案；结合临床感知掩码和图像概念条件化可显著提升报告质量与临床一致性。
...
### [3] [In-Place Instruction Following in Diffusion Language Models](http://arxiv.org/abs/2609.07160v1)
*Zheng Nie,Zherui Li,Jiaming Zhang,Kun Wang,Zhenhong Zhou,Yufei Guo*

Main category: cs.CL

TL;DR: 本文提出了面向原位提示的后训练框架GRAFT，显著提升了扩散大语言模型在原位指令遵循任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽然支持在任意位置施加约束的原位提示，但原始模型在去噪时往往对约束部分的优先级不足，导致指令遵循效果欠佳。

Method: 提出GRAFT后训练框架，结合约束感知监督微调（SFT）和偏好优化；同时将原位提示形式化为原位指令遵循（IIF）任务并构建了IIF-Bench基准。

Result: 在四个代表性扩散大语言模型上，GRAFT将平均IIF得分从57.75提升至73.10（+15.35分），在字面和语篇功能约束上分别获得15.91和15.57分的绝对提升，且保持了一般的生成能力。

Conclusion: GRAFT框架能有效增强扩散大语言模型对原位约束的遵循能力，且不损失其常规文本生成能力，为dLLMs的约束生成提供了有效方案。
...
### [4] [Beyond Token Positions: Safety Alignment Across Denoising Steps in Diffusion Language Models](http://arxiv.org/abs/2609.00495v1)
*Guoli Wang,Haonan Shi,Tu Ouyang,An Wang*

Main category: cs.CL

TL;DR: 本文提出一种无需训练的解码方法RAEC，通过在早期去噪阶段提交持久的拒绝信号，提升了扩散大语言模型的安全性并保持实用性。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（dLLM）通过迭代去噪生成文本，这种范式使得安全对齐取决于令牌生成的时间和位置。本文旨在探究dLLM在有害提示下的安全行为机制，以解决现有生成方式带来的安全漏洞。

Method: 提出了一种无需训练的解码方法——拒绝感知早期提交（RAEC）。该方法通过追踪去噪过程中的中间令牌分布和提交决策，识别出早期拒绝信号并将其持久化提交到最终输出中。

Result: 分析发现，拒绝信号主要集中在早期去噪步骤和回复的前导位置。在LLaDA和Dream模型上的实验表明，RAEC有效降低了攻击成功率，同时基本保留了模型的生成效用。

Conclusion: 去噪步骤和拒绝令牌提交的持久性对理解dLLM安全至关重要。RAEC方法不仅证实了早期拒绝信号的重要性，还为增强扩散语言模型的安全对齐提供了一种简单且不损失实用性的解决方案。
...
### [5] [Trajectory-Level Speculative Decoding for Diffusion Language Models](http://arxiv.org/abs/2608.27514v1)
*Tianxiang Pan,Baitao Gong,Mo Guang,Hongwei Yong,Tianpeng Jiang,Yaqian Li,Zheng Cao,Kaiwen Long*

Main category: cs.CL

TL;DR: 该论文提出了一种轨迹级别的推测解码框架，通过构建和验证多token去噪轨迹来显著提升扩散语言模型的生成吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在低置信度下现有解码策略会退化为单token生成，严重限制了模型的并行生成吞吐量。

Method: 提出一种轨迹级别推测框架，利用置信度分层树探索构建草稿去噪轨迹，并通过双向注意力掩码进行块状并行验证；同时引入块间推测进行跨块前瞻。

Result: 去噪迭代减少30-40%，每步token数从2.6增至4.3，实现比原始dLLM快7-14倍、比Fast-dLLM快1.3倍的加速，且准确率变化小于1%。

Conclusion: 该方法在保持准确率的同时显著提升了扩散语言模型的解码吞吐量，并指出了增加并行性带来的轨迹漂移这一根本代价。
...
### [6] [Dependency-Aware Revocable Decoding for Efficient Diffusion Large Language Model Inference](http://arxiv.org/abs/2608.26574v1)
*Wooje Park,Insu Lee,Minyoung Noh,Jaeyun Jang,Sungmin Lee,Kyuhong Shim,Byonghyo Shim*

Main category: cs.CL

TL;DR: 本文提出了无需训练的DARD框架，通过分离令牌状态并使用选择性上下文进行验证，解决了扩散大语言模型中可撤销解码的上下文污染问题，提升了速度与质量的权衡。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在并行解码时存在早期错误污染后续上下文的问题，而现有的可撤销解码方法忽略了不可靠令牌也会破坏验证上下文本身，本文旨在解决这一失效模式。

Method: 提出了依赖感知可撤销解码（DARD），一种无需训练的框架。该框架将令牌分为掩码、候选和未掩码三种状态，利用排除不可靠令牌的选择性上下文来验证候选令牌，并自适应调节它们对后续解码的影响。

Result: 在3个开源dLLM的12个文本和多模态基准测试中，DARD持续改善了速度-质量帕累托前沿，并在Flickr30K数据集上比Saber实现了2.71倍的加速和4.35分的CIDEr增益。

Conclusion: DARD有效解决了扩散大语言模型并行解码中的上下文污染问题，在不需额外训练的情况下显著提升了生成质量与解码速度的平衡。
...
TL;DR: 针对掩码扩散语言模型在机器翻译中的目标长度决策问题，本文提出了一种基于平均预测熵的无训练长度选择器Entropy-Valley (EV)。


<details>
  <summary>Details</summary>
Motivation: 现有的掩码扩散解码研究主要关注令牌揭示顺序，忽略了在去噪前必须决定的目标长度问题，而该决策直接影响翻译的覆盖率和冗余度。

Method: 提出Entropy-Valley (EV)，一种无需训练的长度选择器。该方法利用全掩码前向传播计算候选目标画布的平均预测熵，并选择模型最易于填充的画布。

Result: 与基于语料库长度统计的基线相比，EV在En→Zh、Zh→En和En→De上分别恢复了64.9%、65.3%和33.0%的参考长度带来的COMET-22增益；专家评估证实了英中互译的充分性提升，且在中译英上表现优于LLaMA-3-8B自回归模型。

Conclusion: 在掩码扩散机器翻译设定中，目标长度的供应方式比决定先揭示哪些令牌更为重要；此外，有利于去噪的长度不必与参考长度完全匹配。
...
TL;DR: 本文提出了一种无需训练的解码方法Ripple-Pivot Search (RPS)，通过利用“涟漪效应”主动确定中熵枢轴位置及其token分配，显著加速了扩散大语言模型(dLLMs)的并行解码。


<details>
  <summary>Details</summary>
Motivation: 现有的dLLM并行解码调度器仅在满足单位置准则后才提交位置，忽略了早期提交对后续解码的潜在益处。本文旨在解决如何通过早期提交特定位置来降低剩余掩码位置的不确定性，从而加速整体解码过程的问题。

Method: 提出Ripple-Pivot Search (RPS)，一种无需训练的解码技术。该方法寻找中熵位置作为候选枢轴（决定在哪解码），并通过前瞻评估确定能产生最大下游收益的token分配（决定解码什么），以利用涟漪效应允许后续步骤并行解掩码更多token。

Result: 在3个dLLMs和4个推理与代码生成基准测试中，RPS在保持生成质量的同时，比标准解码器实现了4-10倍的挂钟时间加速；与先前的前瞻基线相比，准确率最高提升5.49%且在多数设置下吞吐量更高。结合KV缓存时，进一步实现了高达18倍的挂钟时间加速。

Conclusion: RPS通过主动利用解码过程中的涟漪效应，有效提升了dLLMs的推理速度和吞吐量，且无需额外训练，为扩散语言模型的高效解码提供了重要的新范式。
...
### [9] [Artificial Id: Drive and Persistent Alignment in Agentic AI](http://arxiv.org/abs/2609.11911v1)
*Yakov Pyotr Shkolnikov*

Main category: cs.AI

TL;DR: 本文提出了一种名为“人工id”的自适应内部驱动机制，通过差异持久化使代理AI在无需外部显式指定行为目标的情况下涌现出自适应控制能力，并探讨了其对跨任务边界持久对齐的影响。


<details>
  <summary>Details</summary>
Motivation: 随着代理AI从受限任务执行转向跨任务边界持续运行并保留状态的系统，当前依赖人工外部指定目标、重试和停止规则的控制方式面临挑战。本文旨在解决如何让代理系统具备自适应内部驱动以决定行为继续、停止或改变的问题。

Method: 提出一种“人工id”机制，即自适应内部驱动。在虚拟培养皿实验中，一个缺乏通用推理能力且无特定任务目标的控制器，通过“差异持久化”（differential persistence）机制发展出有用的控制能力，并能适应环境意义的变化以替换已学习的策略。

Result: 实验表明，该机制能选择具有更好持久性的非预期物理策略，并在环境意义改变时替换已学习的传感器映射，证明了自适应导向可以在未被显式指定为行为目标的情况下自发涌现。

Conclusion: 差异持久化带来的自适应能力也可能导致错位、状态损坏和非预期行为跨任务边界持续存在。因此，可扩展的代理系统需要建立持久的对齐边界，涵盖可信观测、后果通道、持久状态、权限、身份、溯源及硬约束，将对齐属性从单一模型响应提升至整个持续代理系统层面。
...
TL;DR: 本文提出了MindTopo基准，基于认知科学和形式拓扑学评估基础模型的拓扑直觉能力，发现当前模型在规划和拓扑保持上远不及人类。


<details>
  <summary>Details</summary>
Motivation: 认知科学表明拓扑关系是空间理解的基础，但当前基础模型的评估主要集中在度量或视角依赖的关系上，缺乏对拓扑直觉的评估。

Method: 引入MindTopo基准，包含11,030个实例，评估连续性、分离、顺序、封闭和结五个属性在推理和规划两个认知层面的表现。对14个多模态大模型进行了基准测试，并研究了结合图像/视频生成的智能体配置以及微调和强化学习方法。

Result: 所有模型在推理上的表现均优于规划，最佳模型远低于人类水平。在Qwen3-VL-2B-Instruct上，监督微调和强化学习对推理的提升大于规划。生成的观察虽保留局部线索并达到合理终点，但未能可靠遵循环境动态或在转换中保持拓扑结构。

Conclusion: 当前多模态大语言模型在拓扑直觉，特别是在闭环规划和动态拓扑保持方面存在显著不足，未来需重点解决这些局限。
...
### [11] [Serving Masked Diffusion LLMs: Characterization and Design Principles from Real Hardware](http://arxiv.org/abs/2608.23807v1)
*Farhana Amin,Sabiha Afroz,Mona Moghadampanah,Dimitrios S. Nikolopoulos*

Main category: cs.AI

TL;DR: 本文系统性地测量了掩码扩散语言模型在真实并发服务负载下的行为特征，揭示了其与AR模型服务的关键差异。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型的服务系统正在建设中，但缺乏在真实并发负载下的性能测量，直接套用AR模型的假设可能不适用于dLLM。

Method: 使用LLaDA-8B-Instruct配合D2F LoRA适配器，在单张NVIDIA H200 GPU上评估GSM8K和HumanEval，刻画dLLM服务行为，并推导批量超时规则。

Result: 三个主要发现：(1)请求难度离散分布为11个固定步数级别，且无法预测(R2=0.150)；(2)短生成预算(<320 tokens)低估服务方差；(3)仅24%墙钟时间为GPU计算，批处理通过摊销CPU调度开销实现16.0x吞吐提升。GSM8K准确率为74-76%。

Conclusion: 服务扩散语言模型需要在每个去噪步骤级别进行并行，其准入和驱逐机制与已共享前向传递的交互方式不同于AR服务。
...
### [12] [CAI-DLLM: Convergence Aware Inference for Diffusion Language Models](http://arxiv.org/abs/2608.22646v1)
*Farhana Amin,Sabiha Afroz,Dimitrios S. Nikolopoulos*

Main category: cs.AI

TL;DR: 本文提出一种无需训练的推理方法CAI-DLLM，利用第一步置信度指导去噪以提前确定简单token并优化资源分配，从而大幅加速扩散语言模型的推理过程。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在推理时需要重复进行去噪计算，尤其是对已经稳定的token进行重复计算，导致生成成本高昂。

Method: 提出CAI-DLLM方法，利用第一步置信度信号指导去噪过程，提前提交容易的token，为较难的token分配更多去噪步骤，并调整不同输出块的解码时间表，且无需重新训练、额外预测器或权重更新。

Result: 在多个模型和任务上的实验表明，CAI-DLLM在LLaDA GSM8K上实现18.2倍推理加速且准确率提升，在Dream HumanEval上实现13.1倍加速且pass@1提升；在更难的推理任务上加速达44.8倍，能耗降低高达95.3%，且准确率最多仅下降4.4个百分点。

Conclusion: CAI-DLLM通过简单且无需训练的置信度引导机制，有效解决了扩散语言模型推理缓慢和能耗高的问题，并在保持或提升生成质量的同时实现了显著的加速效果。
...
### [13] [ABRA: An algorithm which cannot converge to low-quality Nash equilibria](http://arxiv.org/abs/2609.11889v1)
*Vartika Singh,Philip N. Brown*

Main category: cs.GT

TL;DR: 本文提出了一种基于噪声和理性参数的近似最优响应算法（ABRA），用于解决具有次模目标的多智能体协调问题，能够跳出次优均衡并显著提升系统目标值。


<details>
  <summary>Details</summary>
Motivation: 在具有次模目标的多智能体协调博弈中，纳什均衡通常只能达到最优值的50%，且达到该最差边界的均衡是不稳定的。本文旨在解决这一局限，通过利用不稳定性来跳出这些次优均衡。

Method: 提出了一种近似最优响应算法（ABRA），该算法受噪声参数和理性参数控制。噪声参数使算法能够逃离次优均衡，而理性参数则平衡噪声对目标函数造成的退化。

Result: 对于双人博弈，如果ABRA收敛至纳什均衡，其系统目标值严格大于最优值的50%（外加一个受噪声参数控制的项）。若未收敛至均衡，则收敛至包含最优或优于50%最优的动作配置的递归类。数值模拟表明，最小期望目标函数通常远高于最优值的一半。

Conclusion: 通过引入噪声和理性参数，ABRA有效克服了多智能体次模博弈中50%最差情况的瓶颈，为系统目标的提升提供了理论和实验依据。
...
### [14] [Truncated Noisy Best-Response Algorithms: Toward Game Theoretic Learning with Safety Guarantees](http://arxiv.org/abs/2609.11863v1)
*Vartika Singh,Philip N. Brown*

Main category: cs.GT

TL;DR: 本文提出截断噪声最佳响应（TNBR）算法，利用纳什均衡的不稳定性解决具有次模最大化目标的多智能体协调问题。


<details>
  <summary>Details</summary>
Motivation: 针对具有次模最大化目标的多智能体协调问题，已知其纳什均衡在最坏情况下仅能达到最优解的50%，且达到此下界的均衡并不稳定，本文旨在利用这种不稳定性来改进算法。

Method: 提出截断噪声最佳响应（TNBR）算法族，智能体通过异步且随机地从最佳响应收益的邻域中选择动作，并计算相关马尔可夫链循环类的界限，分为“性能”界限和“安全”界限。

Result: 性能界限确保算法始终存在高价值的循环状态，安全界限确保算法不存在任意差的循环状态；两者通过类似水床效应联系：安全保证差的博弈必然具有良好的性能保证。

Conclusion: TNBR算法通过利用不稳定均衡，在多智能体次模最大化任务中有效平衡并保证了性能与安全界限。
...
### [15] [BlueSTAR: Tiered Agentic Architecture for Autonomous Cyber Defense](http://arxiv.org/abs/2609.11852v1)
*Simona Boboila,Xavier Cadet,Edward Koh,Daniel Balasubramanian,Dirk Van Bruggen,Peter Chin,Alina Oprea*

Main category: cs.CR

TL;DR: 本文提出了BlueSTAR，一种用于企业IT/OT网络自主防御的分层代理架构，通过转换安全遥测数据和引入弹性指标，实现对已知及复杂未知威胁的有效防御。


<details>
  <summary>Details</summary>
Motivation: 网络攻击日益自动化，人类分析师响应时间受限。大语言模型（LLM）虽具备推理未知威胁的潜力，但直接应用于操作安全遥测面临原始日志量大、事件模糊以及无约束操作风险高的问题。

Method: 提出BlueSTAR分层代理架构，将高容量安全遥测数据转换为紧凑的妥协指标，并引入综合考虑攻击者范围、关键任务资产影响及防御行动破坏的弹性指标。

Result: 在两个实时企业IT/OT网络靶场中基于七种真实攻击链的评估显示，BlueSTAR保留了对已知威胁的快速确定性响应，并成功防御了需要上下文和跨周期推理的复杂攻击（如凭证窃取、并发攻击等）。

Conclusion: BlueSTAR克服了直接应用LLM处理安全遥测数据的局限性，在保持对已知威胁快速响应的同时，实现了对复杂、上下文相关威胁的自主推理与防御。
...
### [16] [Signing the Transaction but Not the Decision: Whisper Attacks and a Binding Defense for AP2](http://arxiv.org/abs/2609.11757v1)
*Yedidel Louck,Amit Dvir,Ariel Stulman*

Main category: cs.CR

TL;DR: 本文提出了一种名为A-VIP的协议层防御机制，用于解决AI购物代理在支付协议中易受商品描述文本操纵的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有的代理支付协议（如AP2）虽能生成密码学有效的签名，但无法约束导致这些签名的决策，使得普通商品描述文本能诱导购物代理生成通过协议检查但不符合用户请求的购物车，从而面临安全威胁。

Method: 提出A-VIP（AP2 Verified-Intent Protection）协议层防御机制，将签名意图视为能力授予而非评判商家描述。该机制将每次凭证查找绑定到请求会话，将每条购物车记录绑定到所见的商品列表，并标记未经授权的支出。

Result: 实验表明，针对Gemini Flash-Lite模型的三种相关攻击成功率分别高达90%、56%和73.3%。A-VIP能以零误报率通过结构痕迹阻断前两种攻击，对于无痕迹的第三种攻击，则能向用户提示未经授权的支出以供确认。

Conclusion: 研究揭示了代理支付协议中普遍存在的意图篡改漏洞，并提供了有效的协议层防御方案，同时开源了A-VIP代码、机器检查的不变量以及包含1544个评估场景的AP2-WhisperBench测试套件。
...
### [17] [Learning Agent-based Model Predictive Control for Holistic Vehicle Performance](http://arxiv.org/abs/2609.11871v1)
*Jiaming Zhong,Reza Valiollahi Mehrizi,Mohammad Pirani,Chao Yu,Alireza Kasaiezadeh,Yash Vardhan Pant,Amir Khajepour*

Main category: cs.RO

TL;DR: 本文提出了一种结合基于模型的AMPC和基于数据的学习方法的实用混合控制方案（LAMPC），以解决多智能体系统中未知智能体贡献导致的预测精度问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于智能体的模型预测控制（AMPC）的最优性高度依赖于预测精度，需要已知所有智能体及其贡献，这在实际应用中过于理想化。

Method: 提出学习型AMPC（LAMPC），利用在线数据管理策略增强的高斯过程回归（GPR）预测未知贡献，并通过多步预测机制和基于预测方差的随机软机会约束框架保证控制安全性和可行性。

Result: 仿真和实验表明，LAMPC优于传统AMPC，在充分学习的场景中实现了更高的跟踪性能，并在学习不足的场景中始终保证满足约束，同时具备实时实现的高效性和对任意控制智能体拓扑的灵活性。

Conclusion: LAMPC作为一种实用的混合控制方案，有效克服了传统AMPC在实际部署中的局限性，为多智能体系统提供了安全、高效且灵活的控制策略。
...
### [18] [CForce: Boosting Parallel Decoding for dLLMs via Consistency Forcing](http://arxiv.org/abs/2608.13925v1)
*Yuji Ren,Chenkai Xu,Zhuocheng Gong,Jianguo Li,Zhijie Deng*

Main category: cs.LG

TL;DR: 本文提出了一种名为Consistency Forcing (CForce)的蒸馏方法，通过强制早期去噪阶段的预测与后期阶段对齐，以解决扩散大语言模型在并行解码中预测不可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散大语言模型在激进的并行策略下，早期去噪阶段的预测不可靠，且误差会传播到后续阶段，导致生成质量下降。

Method: 提出CForce蒸馏方法，在预收集的自展开轨迹上训练以提升训练-推理对齐；引入置信度自适应KL散度结合前向和反向KL的优势；理论分析证明其能近似最小化早期阶段的预测误差。该方法同时适用于掩码到词元解码和可编辑解码。

Result: 在非编辑和可编辑的LLaDA模型上的实验表明，该方法改善了速度与质量的权衡，特别是在高并行解码预算下表现优异。

Conclusion: CForce有效解决了dLLMs早期预测不可靠的问题，提升了高并行度下的解码质量与速度权衡，且代码已开源。
...
### [19] [Near-Optimal Reinforcement Learning with Multi-Step Transition Lookahead](http://arxiv.org/abs/2609.11807v1)
*Corentin Pla,Hugo Richard,Marc Abeille,Vianney Perchet*

Main category: stat.ML

TL;DR: 该论文研究了带有转移前视的强化学习，证明了任意固定折扣因子下精确规划仍是NP-hard的，同时提出了一种多项式时间近似方案来实现高效的近最优规划与学习。


<details>
  <summary>Details</summary>
Motivation: 旨在解决多步转移前视下最优规划的复杂度问题。此前研究仅在折扣因子接近1时证明了规划是NP-hard的，因此需要明确对于任意固定折扣因子该问题是否依然困难，以及能否高效地进行近最优规划。

Method: 提出一种针对固定前视深度的随机多项式时间近似方案，并通过乐观法和方差自适应置信界将其扩展至未知转移和随机奖励场景。

Result: 证明了对于任意固定有理数折扣因子，精确规划仍是NP-hard；所提出的算法实现了累积遗憾，其主导项在不超过对数因子的情况下与经典表格型折扣强化学习相匹配。

Conclusion: 尽管带有转移前视的精确规划是NP-hard的，但高效的近最优规划和学习仍然是可行的。
...
### [20] [Predefined-Time Leaderless Consensus Under Denial-of-Service Attacks](http://arxiv.org/abs/2609.11781v1)
*Lohitvel Gopikannan,Shashi Ranjan Kumar,Abhinav Sinha*

Main category: eess.SY

TL;DR: 本文提出一种切换滑模协议，解决了在拒绝服务攻击下无领导二阶非线性多智能体系统的预设时间弹性一致性问题。


<details>
  <summary>Details</summary>
Motivation: 解决安全关键应用中，通信网络遭受DoS攻击导致链路瘫痪且存在外部扰动时，多智能体系统无法协调的问题。

Method: 设计了一种切换滑模协议，在无攻击期间利用相对位置和速度信息，在通信中断期间采用局部速度反馈；利用Lyapunov分析推导了在攻击持续时间和频率限制下的充分增益条件，并引入在停电期间保持恒定的时间尺度函数。

Result: 在有界扰动下，智能体能够在实际稳定时间内达到位置和速度一致，该时间等于预设收敛时间加上累积攻击时间；预设收敛时间与初始条件无关，且无攻击时等于预设收敛时间。

Conclusion: 所提协议确保了系统在DoS攻击和扰动下实现预设时间一致性，其实际稳定时间合理涵盖了攻击期的影响。
