...
TL;DR: 本文提出了首个完全自包含的密集令牌级视觉自蒸馏框架CVPD，用于提升多模态大语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM的自我提升方法通常依赖于粗糙的标量奖励，或在视觉蒸馏中依赖外部注释、工具或更强模型，缺乏丰富且独立的密集监督。

Method: 提出CVPD框架，利用三重门反事实准则识别模型在全图条件下未充分利用的“视觉盲点”区域，并将其转化为密集的对比监督信号进行自蒸馏。

Result: 在Qwen3-VL-8B-Instruct上，CVPD在12个基准测试中优于6个基线方法（包括依赖外部GPT-4o的方法）且无性能退化，在OCRBench等特定基准上获得3.60至3.08的显著提升。

Conclusion: CVPD证明了无需外部特权上下文或更强模型，MLLM即可通过自身的反事实响应实现密集的视觉自蒸馏和有效自我改进。
...
TL;DR: 本文提出MMDiff框架，通过训练多模态稀疏自编码器并对其进行模型差异比较，实现对多模态大语言模型中特定行为的发现、审计与控制。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）虽具备强大的视觉理解能力，但其内部导致这些行为的特征难以识别、审计和控制；现有基于稀疏自编码器（SAEs）的事后检查方法既无法有效隔离多模态训练所改变的特征，也无法直接用于靶向控制。

Method: MMDiff训练多模态SAEs并将其转化为特征级接口，支持三种功能：（1）特征隔离——通过对比基础语言模型SAE与其多模态适配版本，识别被多模态训练改变的特征；（2）任务特定特征检测——通过逐token对比激活分析隔离因果特征；（3）特征级控制——因果性地移除或引导所发现的特征方向。框架在LLaVA-MORE、PaliGemma 2和InternVL3.5三个模型族上进行了训练和评估。

Result: MMDiff发现的稀疏因果特征在移除后分别使空间任务性能下降12%、OCR任务下降17%，且将多模态安全攻击成功率降低24%，同时对VQA性能无影响；引导这些特征使空间和OCR准确率分别平均提升3.6%和1.8%，优于单层引导基线。

Conclusion: 多模态SAEs不仅可以作为可解释性工具，还可以作为审计、引导和控制MLLM行为的有效机制，有助于实现更安全、更强大的模型生成能力。
...
### [3] [DistMoE: Private-data Rehearsal-free Routing in Mixture-of-Experts for Distributed Instruction Tuning](http://arxiv.org/abs/2608.09907v1)
*Mainak Singha,Niccolò Biondi,Elisa Ricci,Subhankar Roy*

Main category: cs.CV

TL;DR: 提出了一种用于分布式视觉指令微调的混合专家方法DistMoE，解决了多模态大语言模型在去中心化数据上的适应性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型适应方法通常假设数据集中且需要昂贵的联合训练，这在数据分散于私有、特定领域或权限受限的客户端时具有局限性。

Method: DistMoE在语言解码器中用客户端特定的私有FFN专家增强公共FFN，并引入公共锚定专家组合阶段，通过各向同性正则化损失更新路由器和轻量级适配器以减少客户端漂移，推理时无需域标签即可进行模块化路由。

Result: 实验表明，DistMoE实现了灵活的专家重用、有效的域适应和具有竞争力的性能，同时保持了对客户端特定知识的模块化控制。

Conclusion: DistMoE为分布式视觉指令微调提供了一种有效解决方案，在克服数据隐私与分布限制的同时保持了模型性能与模块化控制。
...
### [4] [Sci-VBench: Evaluating Knowledge- and Reasoning-Intensive Video Generation in Science Domains](http://arxiv.org/abs/2608.09873v1)
*Diandian Zhang,Tingyu Song,Lin Fu,Zheyuan Yang,Yilun Zhao*

Main category: cs.CV

TL;DR: 本文提出了Sci-VBench，一个用于评估科学领域视频生成中知识与推理能力的综合基准。


<details>
  <summary>Details</summary>
Motivation: 旨在解决当前视频生成模型在视觉真实感上虽有进步，但在科学推理和因果动态建模方面仍不可靠的问题。

Method: 构建了包含1,253个专家标注样本、覆盖4大学科60个科目的数据集，并建立了一套基于评分规则的评价协议，利用非专家人类评估和多模态大模型（MLLM）作为裁判进行评估。

Result: 评估者与专家判断达成较高一致性；对16个前沿模型的测试显示，感知质量得分相近，但在提示词贴合度和科学/因果正确性上差异显著，且专有模型与开源模型间存在明显差距。

Conclusion: 视频生成在视觉真实感方面的进步尚未转化为对科学和因果动态的可靠建模。
...
### [5] [ADOPD: Reference-Privileged On-Policy Distillation for MLLM-Based Industrial Anomaly Detection](http://arxiv.org/abs/2608.09789v1)
*Jingtai He,Shiyuan Meng,Wenchao Meng,Qinmin Yang*

Main category: cs.CV

TL;DR: 本文提出了ADOPD框架，通过参考特权的强化策略蒸馏将参考图像对比的优势内化到仅需查询输入的学生模型中，在工业异常检测中实现了先进的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在推理时通过对比查询图像和参考图像来提高工业异常检测准确率，但这依赖额外的检索和处理过程。本文旨在将这些对比优势内化到模型参数中，并解决教师模型可能依赖语言先验而非有效视觉信息的问题。

Method: 提出ADOPD（参考特权强化策略蒸馏框架）。参考感知的教师模型在匹配和不匹配的参考下评估学生生成的策略序列；匹配参考下的师生对数比率定义了词级别的学习方向，而两种参考视图之间的似然间隙则用于校准序列级别的权重。

Result: 在MMAD基准测试的零样本推理下，ADOPD实现了77.31%的平均准确率，使Qwen3-VL-4B主干网络提升了6.14个百分点，并优于其单样本设置2.64个百分点。

Conclusion: ADOPD成功从参考对比中学习到了细粒度的异常检测策略，使得仅需查询输入的模型表现超越了需要推理时参考的模型。
...
### [6] [VideoVIBE: A Video-Grounded Diagnostic Benchmark for One-Shot Interactive Website Generation](http://arxiv.org/abs/2608.09573v1)
*Jiajun Xu,Yanghao Zhou,Jingyun Liao,Yu Bai,Jinxing Zhou,Chengliang Liu,Changsen Yuan,Bo Wang,Qian Liu*

Main category: cs.CV

TL;DR: 本文提出基于视频的Web应用故障诊断基准VideoVIBE及多智能体系统V2Lens，以提升自然语言生成应用的质量评估。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常只对孤立的产物或最终结果进行评分，缺乏关于具体故障发生位置及原因的详细证据，无法可靠评估由自然语言生成的Web应用质量。

Method: 提出包含约1.7K个视频问答实例的VideoVIBE基准，并设计无需训练的多智能体系统V2Lens，通过针对性的视觉和源代码验证来挑战并优化初始的基于视频的诊断。

Result: 在十三个视频多模态大语言模型中，Gemini-2.5-Flash作为最强独立模型得分为64.54，而V2Lens系统达到71.72分，提升了7.18分。

Conclusion: 基于视频的评估能够超越孤立产物和整体结果，转向对生成应用程序质量提供行为上忠实且诊断信息丰富的评价。
...
### [7] [Right Answer, Wrong Heat: Explanation-Aware Evaluation and Thermal-Grounded Feedback for MLLMs on Infrared Images](http://arxiv.org/abs/2608.09145v1)
*Yongsong Huang,Xiaofeng Liu,Tomo Miyazaki,Yaohou Fan,Shinichiro Omachi*

Main category: cs.CV

TL;DR: 本文提出了一个解释感知的评估框架和无需训练的热成像反馈机制，以评估和改进多模态大语言模型在红外图像任务中解释的热成像依据。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在红外图像应用中通常仅以回答准确率来评估，但正确的回答并不保证其解释是基于红外热证据的，模型可能错误地依赖可见光证据。

Method: 提出了一种将回答正确性、输出级解释依据和热成像依据分离的评估框架（使用双LLM共识判断器）；并引入了无需训练的热成像反馈（TGF），用于诊断解释故障并在保留选定答案的同时修改解释。

Result: 发现正确的回答可能仍依赖微弱或可见光证据；仅展示可见光渲染图像会削弱热成像依据且几乎不改变准确率，这一现象在能力更强的模型中尤为明显；TGF在不改变答案的情况下有效提升了解释侧的依据性。

Conclusion: 未来用于红外场景理解的可信赖多模态大语言模型应被评估和开发以产生具有热成像依据的解释，而不仅仅是追求答案的准确性。
...
### [8] [SI-Edit: Toward Sketch-Instruction Guided Local Image Editing with Pixel-Level Precision](http://arxiv.org/abs/2608.09097v1)
*Weixin Ye,Wei Wang,Hongguang Zhu,Xuecheng Nie*

Main category: cs.CV

TL;DR: 本文提出了SI-Data数据集和SI-Edit框架，通过结合几何约束和语义指令，实现了像素级精确的局部草图图像编辑。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在草图图像编辑中难以实现像素级精度，主要原因是缺乏同时提供几何约束和语义指令的高质量公开基准数据集。

Method: 首先引入SI-Data数据集，利用多模态大语言模型自动生成包含原图、局部草图、语义指令和编辑后图像的四元组；其次提出整合语义指令与几何约束的SI-Edit协作框架；最后建立了衡量结构保真度和语义一致性的评估指标。

Result: 实验表明，SI-Edit在基于草图的图像编辑中提供了更可靠的结构控制，并实现了符合用户意图的精确像素级局部细化。

Conclusion: 提出的数据集和框架有效解决了草图图像编辑中的空间-语义协作问题，实现了高精度的局部编辑，且数据和代码已开源。
...
TL;DR: 本文提出了PCD（Prefix-Conditioned Diffusion）预训练目标，通过结合自回归前缀监督和后缀去噪，解决扩散语言模型训练与推理时上下文条件不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 原生扩散语言模型在预训练时随机破坏提示和续写标记，削弱了提示条件生成所需的干净前缀接口，导致训练与实际使用之间存在不匹配。

Method: 提出PCD预训练目标，通过修改注意力掩码、破坏掩码和标签构建，对干净前缀部分进行自回归监督，仅对未知后续部分应用扩散去噪；同时分离了样本内前缀条件化与样本间目标混合。

Result: 在LLaDA2-Mini和Qwen-1.7B上的实验表明，PCD相比同族原生dLLM稳定基线持续提升，在LLaDA2-Mini六个基准平均值上实现4.2%相对增益（+2.56分），在Qwen主要机制对比中实现14.2%相对增益（+4.86分）。

Conclusion: 将预训练上下文分布与提示条件生成对齐，可以在不改变推理模式的情况下，有效恢复扩散语言模型在续写任务上的部分性能差距。
...
### [10] [Answer First, Reason Later: Commitment Order in Diffusion LLMs](http://arxiv.org/abs/2608.05687v1)
*Jewon Yeom,Jaewon Sok,Seonghyeon Park,Jeongjae Park,Hwiyeong Lee,Taesup Kim*

Main category: cs.CL

TL;DR: 本文发现掩码扩散语言模型在推理任务中，自由提交顺序导致过早锁定答案而失败，并提出前沿门控干预恢复性能。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型(dLLMs)被宣称的核心优势是可任意顺序提交token，但作者发现这一自由度在推理任务中反而成为失败主因——模型在推理区域仍大量掩码时过早提交最终答案，导致输出退化为仅含答案的内容。

Method: 通过记录LLaDA-8B在GSM8K上的每次提交，采用2×2提示-解码器设计分析CoT与提交顺序的交互作用，将交互分解为坍缩通道和顺序通道；提出单旋钮干预方法——前沿门控提交，在测量的前沿窗口内限制提交顺序。

Result: 无约束解码在轨迹15-24%处即提交最终答案，且在90%的问题上退化为仅答案输出；CoT仅在有序提交下有效(交互效应+34.8个百分点)；前沿门控干预将准确率从0.528恢复至0.852，同时保留最高4倍并行解码能力，最优窗口从完全细化的w=1翻转为每步8token的无约束模式。

Conclusion: 现有窗口式采样器此前被效率动机驱动，实际上无意中成为推理病态的最小修复方案；自由提交顺序是dLLM推理的病理根源，而前沿门控提供了因果性修复。
...
TL;DR: 本文提出OPTD方法，通过在策略转移蒸馏与一致性引导的自适应压缩解决扩散语言模型少步蒸馏中的状态不匹配问题，提升了生成质量与效率。


<details>
  <summary>Details</summary>
Motivation: 现有少步蒸馏方法基于离策略轨迹构建监督，导致学生模型推理时的实际状态与监督状态不匹配；而在策略蒸馏则面临转移步数难以平衡的问题，步数过短限制压缩效果，过长会破坏中间依赖。

Method: 提出OPTD方法，从学生自身轨迹采样部分状态，利用冻结的仅包含问题的教师模型识别对齐的未来候选者并按置信度排序，选择保持教师生成结果的最长前缀；结合集合瓶颈目标和冻结教师KL散度锚点进行正则化训练，且无需标准响应。

Result: 在四个数学推理和代码生成基准测试中，OPTD持续改善了质量与效率的权衡，并在所有评估的少步基线中取得了最强的质量约束AUP。

Conclusion: OPTD成功解决了扩散语言模型少步蒸馏中的状态漂移和转移步长选择难题，无需依赖标准响应即可实现高效解码，显著提升了模型加速生成的整体性能。
...
### [12] [Faster but Different: Diagnosing and Controlling Content Drift in Accelerated Multimodal Diffusion Language Models](http://arxiv.org/abs/2607.29079v1)
*Yaoxuan Dou,Yang Shu*

Main category: cs.CL

TL;DR: 本文研究了扩散多模态大语言模型在免训练加速下产生的内容一致性问题，并提出通过缩短KV缓存刷新间隔来控制一致性。


<details>
  <summary>Details</summary>
Motivation: 免训练加速使得扩散多模态大语言模型更易于部署，但会悄无声息地改变生成内容，本文旨在解决这一服务时的一致性漂移问题。

Method: 通过在300张真实图像上对比加速与未加速输出，进行置信度阈值调优、状态刷新消融及图像交换干预，提出缩短KV缓存刷新间隔的实现级一致性控制方法。

Result: 陈旧的视觉和文本状态会导致输出漂移；缩短KV缓存刷新间隔能以1.3倍加速实现近乎完全的一致性；事实错误差异未检测到统计学显著变化；且无自适应或平滑刷新变体优于固定间隔。

Conclusion: 本文的贡献在于提供了一种配对诊断方法和针对具体实现的一致性控制策略，而非准确性或安全性的保证。
...
TL;DR: 本文提出了DominoTree，一种无需训练的最佳优先草稿树方法，通过结合Domino的条件非因子化校正和GPU原生CUDA图构建器，显著提升了大语言模型推理的加速效果和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散起草器和最佳优先树方法（如DDTree）依赖因子化公式，无法表示路径相关的草稿token分布（如Domino起草器的条件校正），本文旨在解决这一局限性以更准确地构建草稿树并提升推理速度。

Method: 提出DominoTree，一种无需训练的最佳优先草稿树方法。它使用Domino的条件非因子化校正在每条根到节点路径上评分，并通过限制每节点校正到候选top-M集来提高实用性；同时使用GPU原生的CUDA图构建器构建树，确保与Python实现位一致且构建成本低。

Result: 在Qwen3-4B等模型上的实验表明，DominoTree实现了高达6.6倍的加速，平均接受长度最高达每轮10.7个token；在不同温度下均优于DDTree等方法，相较于Domino解码器在Qwen3-4B上整体吞吐量提升9%至10%，在Alpaca上提升高达22%。

Conclusion: DominoTree通过有效利用路径相关的非因子化分布并配合高效的GPU原生树构建器，在不改变接受率的前提下大幅提升了推理吞吐量，证明了其在加速大语言模型推理方面的有效性和优越性。
...
TL;DR: CaDDTree通过显式建模延迟来联合选择树结构和节点预算，从而优化投机解码中的令牌吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散起草器（如DDTree）在选择树预算时仅最大化预期接受长度，这总是偏向于更大的树而忽略了验证成本，缺乏选择预算的原则性依据。

Method: 提出CaDDTree方法，通过显式建模起草和验证延迟来直接优化令牌吞吐量；将吞吐量目标分解为每轮对预算的一维搜索，并在凸验证成本下证明吞吐量函数是单峰的，从而实现高效的贪婪停止规则。

Result: 在Qwen3-4B和Qwen3-8B模型上的八个基准测试（涵盖推理、编码和指令遵循任务）表明，CaDDTree在几乎所有任务上均达到或超过了具有Oracle预算选择的DDTree。

Conclusion: CaDDTree无需离线预算搜索，能够根据当前位置分布和验证成本在每轮自适应调整预算，有效提升了推理吞吐量。
...
TL;DR: DDTree通过从块扩散草稿模型的逐位置分布构建草稿树，突破了DFlash单轨迹验证的限制，进一步提升了推测解码的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散推测解码方法（如DFlash）在每轮验证中仅处理单个草稿轨迹，从而限制了词元的接受长度和整体的解码加速效果。

Method: 提出DDTree方法，直接从块扩散草稿模型的逐位置分布构建草稿树。在固定节点预算下，利用最佳优先堆算法根据草稿模型输出选择最可能匹配目标模型的续写路径，并采用仅祖先注意力掩码在目标模型的单次前向传递中高效验证该树。

Result: 在DFlash基础上应用DDTree取得了显著的性能提升，使其跻身于当前最先进的推测解码方法之列。

Conclusion: 结合块扩散草稿模型与草稿树构建验证机制，能有效克服单轨迹验证的瓶颈，是加速自回归语言模型推测解码的有效方向。
...
### [16] [Competitive mediator games and urban CAV routing markets](http://arxiv.org/abs/2608.09894v1)
*Grzegorz Jamróz*

Main category: cs.GT

TL;DR: 本文受自动驾驶路由市场的启发，提出了竞争性中介博弈及其均衡，并证明了在特定条件下会形成垄断均衡。


<details>
  <summary>Details</summary>
Motivation: 旨在解决未来自动驾驶路由和驾驶（ARAD）市场中服务提供商（中介）之间的竞争建模问题，以及分析该市场可能出现的垄断现象。

Method: 引入了竞争性中介博弈及其均衡（推广了粗相关均衡），并在具有市场份额最大化中介的匿名拥塞（路由）博弈中进行理论分析。

Result: 证明了当所有用户对某一中介存在弱偏好时，在具有市场份额最大化中介的匿名拥塞博弈中，所有的竞争性中介均衡均为垄断。

Conclusion: 将理论结果应用于竞争性的ARAD服务提供商市场，并对该市场的未来机制设计进行了全面概述与讨论。
...
### [17] [Stealing Reasoning Traces from Proprietary LLM APIs](http://arxiv.org/abs/2608.09867v1)
...
TL;DR: 本文揭示了大语言模型提供商加密思维链的架构漏洞，通过将加密推理块注入较弱模型实现解密越狱，进而提取私有数据并执行多种攻击，最后提出了缓解方案。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型提供商为保护知识产权而加密并返回思维链，但这种加密块在不同会话、用户和模型间可互换的特性引发了严重的安全与隐私隐患。

Method: 利用加密块的跨模型兼容性，将目标模型的加密推理轨迹注入同一提供商中防护较弱的模型，迫使其以明文形式逐字解码并输出该推理轨迹。

Result: 成功在Anthropic、OpenAI和Google上绕过反蒸馏机制提取推理；从公开日志中解码31万余个加密块，恢复出367个个人身份信息和182个凭证；揭示了隐藏的危险信息；并实现了基于加密块的不可见提示注入。

Conclusion: 当前的客户端推理加密方案存在重大安全漏洞，提供商必须采取具体的密码学和系统级缓解措施来保障客户端推理的安全性。
...
### [18] [XPolicyLab: A Unified Standard and Open Ecosystem for Robot Policy Evaluation and Deployment](http://arxiv.org/abs/2608.09892v1)
...
TL;DR: 本文提出XPolicyLab，一个统一标准和开放生态系统，将N个机器人策略与M个评估环境连接的集成成本从O(NM)降至O(N+M)。


<details>
  <summary>Details</summary>
Motivation: 机器人策略评估和部署因模型特定的软件依赖、数据表示和运行时接口而碎片化，导致连接N个策略到M个评估环境需要O(NM)次独立集成，成本高昂且效率低下。

Method: XPolicyLab定义了通用的观测、动作和轨迹模式，并提供了用于观测更新、动作预测、批量执行和 episode 重置的最小适配器接口；采用依赖隔离的客户端/服务器架构，将策略推理与环境执行分离，使双方保留各自原生软件栈并可本地或远程运行。

Result: 该生态系统集成了42个机器人策略并标准化了安装、调试、服务和评估工作流。模型特定代码差异达一个数量级，而环境端循环保持在固定参考的几行代码内。在对照实验中，符合标准将代表性策略的集成时间从5小时以上降至2小时，打包代理技能进一步降至30分钟。同一适配器可服务RoboTwin、RoboDojo仿真和标准化真实机器人评估。

Conclusion: XPolicyLab作为共享基础设施发布，用于跨仿真和物理平台的可复现策略比较和标准化部署，为机器人策略评估与部署提供了统一的解决方案。
...
### [19] [Agentic Harnesses: LLM-Driven Verification Layers for Robot Autonomy](http://arxiv.org/abs/2608.09857v1)
*Rohan Bhagra,Mahantesh Halapannavar,Uddhav Bhattarai*

Main category: cs.RO

TL;DR: 本文提出一种基于LLM的验证层，作为机器人规划与执行间的中间件以评估动作可行性并降低安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有机器人系统侧重执行而忽视动作可行性验证，可能导致不安全、违背伦理或受对抗攻击影响的规划。

Method: 采用结合思维链推理、混合专家与自洽性方法的“LLM-as-a-Judge”集成验证层，对规划进行批准、拒绝或上报人工的判定。

Result: 在各类别中达到近85%精确度，遏制97%的对抗攻击，接受与拒绝间错误极低，错误主要集中在需上报的边界情况。

Conclusion: 该LLM驱动的中间件验证层能有效保障机器人规划的安全性与可靠性。
...
### [20] [DSLE: A Learning Environment for Dark Souls Boss Encounters](http://arxiv.org/abs/2608.09902v1)
*Derin Gezgin,Jim O'Connor,Tanner Goodwin,Gary B. Parker*

Main category: cs.AI

TL;DR: 本文提出了《黑暗之魂》学习环境（DSLE）作为游戏智能体基准，评估表明现有强化学习和基线方法难以应对该游戏中的复杂实时战斗。


<details>
  <summary>Details</summary>
Motivation: 为了填补缺乏包含实时战斗、高维视觉输入和稀疏奖励的极具挑战性游戏环境的空白，提供一个高难度的智能体评估基准。

Method: 构建了容器化平台DSLE，通过Gymnasium接口提供22个Boss战环境，并定义了包含5个代表性Boss的DSLE-5子集；在DSLE-5上评估了随机策略、专家系统、进化基线、PPO和DQN，并在全量Boss上测试了进化基线。

Result: 专家系统和进化基线仅能击败教程Boss（峰值胜率63%和43%），PPO和DQN几乎无学习迹象；即使在属性优势下，进化基线在22个Boss战中也仅能战胜少数早期Boss，其余均告失败。

Conclusion: 《黑暗之魂》对当前的游戏智能体算法构成了巨大挑战，凸显了现有方法在处理复杂实时战斗、稀疏奖励和多目标场景时的局限性，指明了未来算法改进的方向。
...
### [21] [SHE: Trajectory-driven Safety Harness Evolution for LLM Agents](http://arxiv.org/abs/2608.09885v1)
...
TL;DR: 本文提出了安全框架演进（SHE），通过将智能体框架分解为具有明确安全责任的组件并从轨迹失败中学习边界优化，以提升大型语言模型（LLM）智能体的安全性。


<details>
  <summary>Details</summary>
Motivation: 现有安全机制将智能体框架视为固定的部署产物，难以应对新兴风险；且各组件间耦合的功能掩盖了安全责任归属，阻碍了局部演进。

Method: SHE将框架分解为系统提示、规则库、安全记忆和工具策略四个构件，并引入归因引导的演进循环，将轨迹失败转化为结构化诊断以学习特定构件的边界优化，最后通过安全-实用性验证选择演进的框架。

Result: 在Agent-SafetyBench上，SHE相比静态SafeHarness将攻击成功率降低了3.1倍并提升了良性实用性；演进的框架还能泛化至AgentHarm上的未知风险，且可跨智能体模型迁移而无需额外演进。

Conclusion: 基于安全责任归因的智能体框架演进是提升LLM智能体安全性、实用性以及跨模型和未知风险泛化能力的有效方法。
...
### [22] [ArchAgent v2: A Case Study with the Data Prefetching Championship](http://arxiv.org/abs/2608.09874v1)
...
TL;DR: 本文提出ArchAgent v2框架，利用级联进化搜索和硬件可实现性反馈回路实现多级数据预取的自动微架构搜索，在DPC4竞赛中超越了人类手工设计的获胜方案。


<details>
  <summary>Details</summary>
Motivation: 将智能体AI扩展至计算机微架构发现面临搜索空间大、硬件预算严格和仿真时间长等挑战，且现有的单级缓存策略方法无法扩展到自由度更大的多级预取场景。

Method: 引入两项创新：一是级联进化搜索，通过逐级进化和冻结单层缓存的预取器来细分设计空间；二是硬件可实现性反馈回路，将实时面积估计直接嵌入进化过程。

Result: 在DPC4规则下，自动设计的三级预取器整体IPC加速比达3.8%，超越前冠军BertiGO 0.3%；在低带宽单核配置下性能提升4.6%（BertiGO为2.6%）。对12000个候选设计的剖析揭示了进化智能体的探索机制。

Conclusion: 自动化智能体发现对计算机架构师极具价值，但多核进化仍受限于仿真延迟。对海量候选设计的剖析为理解智能体如何探索和合成复杂微架构逻辑提供了重要见解。
...
### [23] [Towards Expert-level Medical AI for Real-time Video Consultations](http://arxiv.org/abs/2608.09861v1)
...
TL;DR: 本文介绍了首个在实时临床视频问诊中达到专家级表现的AI系统AMIE (Video)，其在多项临床评估指标上与初级保健医生相当或更优。


<details>
  <summary>Details</summary>
Motivation: 现有的基于文本的医疗AI缺乏视听感知且限制了无法用文字表达症状的患者，早期的视听医疗AI也未达到临床医生水平，因此亟需开发能在真实视频问诊中达到专家级水平的AI系统。

Method: 开发了基于Gemini的多智能体系统AMIE (Video)，集成低延迟对话、临床推理与实时视听感知，并建立了临床视听线索分类法和自动化评估体系以指导开发。

Result: 在100个场景的随机对照研究中，AMIE (Video)在病史采集、诊断、管理和体检观察等方面被评与初级保健医生相当或更优；患者更偏好其评估与解释方式及视频界面，但医生在建立医患关系方面更受青睐。

Conclusion: AMIE (Video)标志着AI在处理临床实践感官复杂性方面的重大进展，尽管在精细解剖、情感细微差别和高频运动方面仍存在局限，需进一步研究以实现临床转化。
...
### [24] [Agentic Auto-Research is Fuzz Testing](http://arxiv.org/abs/2608.09855v1)
*Yifeng He,Jicheng Wang,Yinzhe Zhao,Jiachen Liu,Hao Chen*

Main category: cs.AI

TL;DR: 本文提出借鉴灰盒模糊测试的反馈架构，通过密集的认知进展信号指导自主研究代理的实验搜索，以解决现有生成与排序范式中的稀疏反馈问题。


<details>
  <summary>Details</summary>
Motivation: 自主研究代理生成实验的速度远超验证速度，而现有的“生成与排序”范式由于缺乏早期反馈信号，无法评估部分进展并有效指导后续实验。

Method: 将研究过程类比为灰盒模糊测试，要求实验在最终验证前提供廉价且密集的认知进展信号，并利用该信号指导后续干预进行反馈导向搜索；同时确保最终验证不受自适应重用的影响。

Result: 设计了受控测试以评估候选信号对已验证进展的预测能力、反馈导向搜索相比重复采样的发现率提升，以及受保护验证对减少错误发现的效果。

Conclusion: 反馈架构而非仅仅是生成能力，是自动研究的核心瓶颈。
...
TL;DR: 本文系统分析了混合专家扩散语言模型的扩展规律，并基于此训练了LLaDA MoE v2模型，展现出优异的性能与设计原则。


<details>
  <summary>Details</summary>
Motivation: 旨在解决混合专家扩散语言模型的扩展行为尚不明确的问题，以弥补其与自回归模型在扩展趋势上的认知差距。

Method: 系统研究了优化超参数、算力分配和架构扩展规律，并基于发现的规律从零训练了参数规模为30B-A3B的LLaDA MoE v2模型。

Result: 发现了MoE dLLMs在最优批大小、学习率衰减及数据侧算力倾斜上的独特扩展规律。LLaDA MoE v2使用约65%的Qwen3预训练token量，在多项基准测试中接近Qwen3，且经监督微调后在7/8的推理和编码基准上优于SDAR Chat。

Conclusion: 研究结果为混合专家扩散语言模型确立了实用的扩展定律和架构设计原则，证明了其有效性与潜力。
...
TL;DR: 本文提出了Evolutionary Decoding，一种无需训练的测试时扩展框架，通过将扩散解码视为进化过程，以解决扩散大语言模型在数学推理中因置信度陷阱导致的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在数学推理任务中表现不可靠，解码轨迹中存在局部token置信度与全局推理正确性不一致的“扩散置信度陷阱”，导致采样敏感型和采样一致性两类失败。

Method: 提出一种无需训练的测试时扩展框架Evolutionary Decoding，结合逐步选择机制（保留有效数值符号信号并抑制重复模式）与块级突变机制（引入结构化替代方案以逃离错误的高置信度盆地），将扩散解码视为候选推理状态的进化过程。

Result: 在多个基准测试中，Evolutionary Decoding有效提升了LLaDA 2.0模型的性能，使其相比基于置信度的解码在数学推理上更加可靠。

Conclusion: Evolutionary Decoding成功克服了扩散大语言模型的置信度陷阱，显著提升了其数学推理的可靠性，验证了进化策略在扩散解码测试时扩展中的有效性。
...
TL;DR: 提出了一种无需训练的并行解码框架FlowBlock，通过门控波前解码和异构打包机制显著提升了块级扩散语言模型的推理速度和准确率。


<details>
  <summary>Details</summary>
Motivation: 解决块级扩散语言模型（dLLMs）块间解码严格串行导致推理缓慢，以及现有训练后并行方法加速有限且降低准确率的问题。

Method: 提出无需训练的并行解码框架FlowBlock，包含：1）门控波前解码，满足就绪条件时将块纳入波前，通过T2T编辑联合优化，并在窗口化块因果掩码下按序提交以保持KV缓存复用；2）异构波前打包，为每个请求分配独立波前并打包成密集的批量前向计算。

Result: 相比LLaDA-2.1和2.0，每秒生成token数分别提升最高2.95倍和4.01倍，延迟分别降低最高53.6%和77.1%，平均准确率提升1.3分；相比D2F基线，准确率更高且批处理吞吐量提升最高达16倍。

Conclusion: 将块级最终性从硬依赖转化为调度资源，可在无需额外训练的情况下，大幅提升块级dLLMs的解码效率、吞吐量和准确率。
...
TL;DR: 提出了一种用于扩散模型推测解码的目标感知前缀选择方法TAPS，通过优化验证过程显著提升了端到端无损加速比。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在推测解码中的并行起草将瓶颈转移到了验证阶段。现有的草稿树方法通过边际概率对节点进行排名，忽略了验证是基于前缀条件化的，导致验证被拒绝前缀的不可达后代，从而增加了延迟且接受度提升有限。

Method: TAPS将扩散边际概率转化为路径条件化的接受估计，并在固定的验证预算下选择一个紧凑的前缀闭合子树，以改善接受度与成本的权衡。

Result: 在多个数据集和模型族上的实验表明，TAPS相比自回归解码实现了高达7.9倍的无损端到端加速，分别比最先进的DFlash和DDTree高出1.36倍和1.74倍。

Conclusion: TAPS通过解决验证过程中的前缀条件化不匹配问题，有效改善了基于扩散模型的推测解码中的接受度与成本权衡。
...
### [29] [Hallucination-Free GUI Grounding via Regression-Free Layout-Aware Matching](http://arxiv.org/abs/2608.09654v1)
*Yuke Li,Xuehan Hou*

Main category: cs.AI

TL;DR: 本文提出了一种无回归框架，通过将指令理解与布局感知定位解耦，解决了GUI定位任务中的语义理解不足和坐标幻觉问题。


<details>
  <summary>Details</summary>
Motivation: GUI定位任务面临双重障碍：传统定位模型缺乏解释抽象指令的语义丰富度，而端到端多模态大语言模型（MLLM）因细粒度感知不足而产生坐标幻觉。

Method: 提出一种无回归框架，利用冻结的MLLM将抽象指令解析为富含布局线索的结构化视觉描述，再输入新颖的布局感知GUI定位模型中，通过匹配布局先验候选实现无回归定位，该模型仅需文本/图标二值标签进行训练。

Result: 在ScreenSpot-Pro数据集上，定位准确率较端到端系统提升超过20%；在Mind2Web数据集上，成功率和元素选择率提升超过15%。

Conclusion: 将指令理解与布局感知定位解耦能有效解决GUI交互的核心挑战。
...
### [30] [MMArch: Benchmarking Multimodal Reasoning Grounded in Architectural Evidence](http://arxiv.org/abs/2608.09281v1)
...
TL;DR: 本文提出了MMArch基准，用于评估多模态大语言模型在建筑与土木工程领域结合跨图表视觉证据与工程原理进行推理的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大模型基准大多侧重于基础的绘图识别或信息提取，未能有效评估模型结合分布在不同图表中的视觉证据与工程原理以得出结论的综合推理能力。

Method: 提出MMArch基准，涵盖十个子领域的1,212个简答题，数据源自同行评审论文的图表。通过解耦的规划-写作流程生成题目，并经过自动化筛选、盲审对抗性审计和专家评审验证，强制要求模型感知证据、识别原理并加以应用。

Result: 对18个开源和闭源多模态大模型的评估显示，最强开源模型准确率约30%，最佳闭源系统为52%，而人类专家达到95%，存在超过40个百分点的巨大差距。

Conclusion: 模型的主要失败在于应用原理和跨图表结合证据，而非定位证据，这指明了未来研究的实质性改进空间。
...
TL;DR: 本文研究了扩散大语言模型(DLLMs)的安全对齐机制漏洞，提出了一种基于安全神经元引导的离线黑盒越狱框架。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的安全机制尚不清晰，需要研究其内部安全对齐的脆弱性以评估和改进模型安全性。

Method: 通过安全神经元映射与剪枝进行迁移攻击，并提出SN-Guided Diffusion框架，利用加权安全神经元损失引导扩散过程远离安全触发区域。

Result: 自剪枝将LLaDA和Dream的攻击成功率分别提升至73.8%和86.6%；SN-Guided Diffusion在多个目标上实现高达77.1%-86.9%的迁移攻击成功率，AUROC达1.0，且生成成本显著低于现有方法。

Conclusion: DLLMs的安全对齐存在稀疏且可跨架构迁移的机制性漏洞，从自回归模型继承的安全足迹使其易受神经元级攻击，亟需更强的安全对齐方法。
...
TL;DR: 本文提出块混合注意力机制，通过在先前块上应用线性注意力来加速扩散语言模型的推理，同时保持基准性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在去噪的每一步中仍会对先前块重复计算前缀注意力成本，导致推理瓶颈。

Method: 提出块混合注意力，在活跃去噪块内保留精确的softmax注意力，在先前块上应用线性注意力，并通过极少量的后训练将其移植到预训练模型中。

Result: 将该方法应用于16B的LLaDA模型，转换仅需约60小时，且在HumanEval、MBPP+和CMATH等基准上保持了性能；使用Triton实现后，解码吞吐量最高提升1.7倍，并支持更多并发请求。

Conclusion: 预训练的扩散语言模型可以被高效线性化以实现更快的推理，且不显著牺牲模型性能。
...
TL;DR: 本文提出ERILS方法，通过引入外部策略生成的高奖励rollouts并结合长度控制和源特定奖励处理，解决了扩散大语言模型(dLLMs)在on-policy rollouts成功稀少时训练进展有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有dLLM强化学习方法依赖目标模型自身生成的on-policy rollouts，当成功rollouts稀少时，训练几乎无正奖励反馈，进展受限。直接引入外部更强策略的rollouts会带来长度差异和奖励处理不稳定两大挑战。

Method: 提出ERILS（External Rollout Integration with Length Control and Source-Specific Processing），将外部更强策略生成的高奖励rollouts与目标dLLM的on-policy rollouts结合，通过控制外部rollout长度并分别处理on-policy和外部rollouts的奖励，避免联合处理导致的训练不稳定。

Result: 在Sudoku、Countdown和MATH500的零样本评估中，ERILS在三个任务上均提升多样本性能，Sudoku提升最大：best-of-4完成准确率达98.4%（最强基线仅40.3%），在128/256/512 tokens生成长度下维持约90%确定性单完成准确率。组件分析表明长度控制的外部rollouts优于无控制rollouts，源特定奖励处理避免了联合奖励处理的训练崩溃。

Conclusion: rollout构建和奖励处理是整合外部rollouts到dLLM强化学习的重要设计维度；ERILS验证了长度控制和源特定奖励处理的有效性，为dLLM训练中利用外部策略信息提供了可行方案。
...
### [34] [Nash Peer-to-Peer Insurance Bargaining under Price Fairness and Coalitional Stability](http://arxiv.org/abs/2608.09859v1)
*Tim J. Boonen,Wing Fung Chong,Kenneth Tsz Hin Ng,Tak Wa Ng*

Main category: q-fin.RM

TL;DR: 本文研究了非对称纳什谈判框架下P2P再保险的最优合同设计，并探讨了价格公平性与资金池规模对风险分配和成员福利的影响。


<details>
  <summary>Details</summary>
Motivation: 旨在解决风险厌恶型P2P再保险人与多个成员在小型资金池中如何通过自愿协商达成帕累托最优风险分担的问题，并确保价格公平及防止子群体偏离。

Method: 构建非对称纳什讨价还价模型并引入价格公平条件，通过公理化特征证明该框架的合理性；推导最优合同的存在性、唯一性及一阶特征，并建立排除联盟偏离的可计算充分条件。

Result: 证明了最优合同的存在性与唯一性；数值研究发现价格公平降低了风险分配和确定性等价附加的分散程度；且福利不一定随资金池规模单调增加。

Conclusion: 资金池的扩张效果不仅依赖于风险分散，还受制于成员间谈判能力的演变；价格公平机制在P2P保险合同设计中具有降低分配分散性的实际意义。
