...
### [1] [A Good Initialization is All You Need for Faithful Visual Attribution](http://arxiv.org/abs/2607.06726v1)
*Zihan Gu,Jiayu Wang,Hua Zhang,Yue Hu*

Main category: cs.CV

TL;DR: 本文提出了CoPAIR和TRACE两种前向搜索方法，以高效获取紧凑的top-k图像区域归因掩码，在全排序归因和MLLM归因中达到SOTA，并证明了紧凑掩码可用于模型修复。


<details>
  <summary>Details</summary>
Motivation: 现有的基于搜索的视觉扰动归因方法通常输出所有区域的完整排序，而许多应用（如MLLM归因和修复）仅需紧凑的top-k证据掩码，导致效率低下。

Method: 提出了两种仅前向的方法：CoPAIR利用PhaseWin-Greedy间隙诊断构建粗粒度候选来热启动全排序搜索；TRACE通过交叉熵采样、精英保留和分布更新直接搜索固定基数细粒度区域掩码。

Result: 在ImageNet上确立了全排序归因的新SOTA前沿；在POPE和RePOPE上实现了最优的基于搜索的MLLM归因结果；直接使用TRACE掩码在RePOPE上达到了94.44%和96.00%的修复率。

Conclusion: 紧凑的证据掩码不仅仅是全排序的简单前缀，更可以作为具有可操作性的归因输出直接应用于模型修复等任务。
...
TL;DR: 本文提出了一个名为HoloCount的综合基准，通过分层分类法评估多模态大语言模型在视觉计数任务中的语义、分析和鲁棒性能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视觉计数任务中存在数值幻觉问题且定量精度不足，同时现有基准测试过于简化，无法捕捉模型在逻辑约束或对抗条件下的复杂失败模式。

Method: 引入HoloCount基准，采用三级分层分类法评估模型：语义计数（基础和属性枚举）、分析计数（空间与集合推理）以及鲁棒性测试（高密度场景和语言偏见等对抗条件）。

Result: 对超过20个最先进的MLLMs进行评估，发现当任务从基础感知转向复杂的分析推理和不利场景时，即使是顶级模型的性能也会显著下降。

Conclusion: 该研究系统揭示了当前MLLMs的计数能力现状与局限性，为未来开发更准确、更可靠的多模态系统提供了路线图。
...
### [3] [SpaR3D-MoE: Adaptive 3D Spatial Reasoning from Sparse Views Meets Geometry-Inductive Mixture-of-Experts](http://arxiv.org/abs/2607.06620v1)
*Haida Feng,Hao Wei,Haolin Wang,Shiwei Li,Chade Li,Yihong Wu*

Main category: cs.CV

TL;DR: SpaR3D-MoE提出了一种使用稀疏RGB输入和混合专家机制的端到端框架，在多模态大语言模型中实现了最先进的3D空间推理。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型难以弥合2D语义理解与3D空间几何之间的差距。现有的3D感知模型要么依赖昂贵的3D专用数据，要么使用仅RGB输入的启发式采样和单一浅层融合，这分别破坏了时空连通性并导致跨空间任务的模态竞争。

Method: 提出SpaR3D-MoE端到端框架，包含两个核心组件：1）自适应时空流形采样机制，构建几何感知时空图以提取关键帧，减少序列冗余同时保持拓扑连通性；2）由指令-位姿感知路由器驱动的异构几何归纳混合专家，自适应地将多模态token路由至专门的专家，解决单一融合中的跨模态竞争。

Result: 在VSI-Bench、ScanQA和SQA3D数据集上达到最先进性能。在VSI-Bench上取得63.5的平均分，超出最强基线7.8分，并在路径规划和相对方向任务中分别实现35.4%和51.4%的相对提升。

Conclusion: SpaR3D-MoE有效实现了仅使用稀疏RGB输入的自适应空间推理，克服了现有3D感知模型在时空连通性和模态竞争方面的瓶颈。
...
### [4] [SparseCtrl-HOI: Sparse Temporal Control for Human-Object Interaction Video Generation](http://arxiv.org/abs/2607.05994v1)
*Shenbo Xie,Mingrui Cai,Xu Yang,Yifei Liu,Changxing Ding*

Main category: cs.CV

TL;DR: 本文提出SparseCtrl-HOI框架，通过稀疏关键帧控制和多模态大语言模型提取运动先验，有效解决了人与物体交互视频生成中密集时序指导带来的高标注成本和低运动多样性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的人与物体交互（HOI）视频生成方法通常依赖密集的时序指导（如逐帧手物姿态序列），导致标注成本高昂且限制了运动合成的多样性。本文旨在解决精细物理动态和复杂时空协调的建模难题，同时摆脱对密集指导的依赖。

Method: 提出一种稀疏时序控制框架SparseCtrl-HOI，仅需少量捕获交互状态的关键帧。具体采用时间控制旋转位置嵌入（TiRoPE）机制在时间上锚定关键帧并保持其空间完整性；引入运动先验注入模块，利用多模态大语言模型（MLLMs）提取高层运动先验以指导中间帧的动态生成；此外构建了高质量数据集SparseHOI-5K。

Result: 综合评估表明，该方法在显著减少标注开销的同时，能够合成出质量更优的直播电商交互视频。

Conclusion: SparseCtrl-HOI通过结合稀疏关键帧与运动先验，成功实现了高效且高质量的HOI视频生成，为AI驱动的直播电商应用提供了有效途径，相关代码和数据集已开源。
...
### [5] [Propose and Attend: Training-free MLLM Grounding Confidence via Multi-Token Localized Attention](http://arxiv.org/abs/2607.05978v1)
*Daniel Shalam,Emanuel Ben Baruch,Avi Ben Cohen,Tal Remez*

Main category: cs.CV

TL;DR: 提出无需训练的MTLA方法，通过聚合预测token对所声称区域的注意力来检测多模态大语言模型的定位幻觉，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在生成定位预测（如边界框、时间窗口）时严重幻觉化，而模型自身的token对数概率无法有效区分定位质量与输入歧义，且坐标token在模型确定后趋于确定性。

Method: 提出Multi-Token Localized Attention (MTLA)：一种训练免费的事后评分方法，仅计算预测token对其所声称区域的注意力并跨所有预测token聚合；该方法可统一应用于图像目标检测及视频/音频时间定位。

Result: 在多个MLLM家族和三种模态上，MTLA将幻觉检测AUROC较最佳先前无需训练基线提升+7至+38；作为重排序置信度，将开源8B通用模型零样本COCO检测AP从20.4提升至37.0。

Conclusion: MTLA是一种通用、有效且无需训练的方法，可显著缓解多模态大语言模型的定位幻觉问题，缩小与监督检测器的差距。
...
### [6] [Progressive Reasoning with Primitive Correction for Compositional Zero-Shot Learning](http://arxiv.org/abs/2607.05911v1)
*Ziyi Chen,Haoyan Shi,Sunhan Xu,Congyan Lang*

Main category: cs.CV

TL;DR: 本文提出PRPC框架，通过问答式思维链推理和双向基元校正解决组合零样本学习中的错误传播和上下文依赖缺失问题。


<details>
  <summary>Details</summary>
Motivation: 现有的组合零样本学习方法要么独立预测属性和对象而忽略上下文依赖，要么使用单向条件建模导致错误传播。

Method: 提出PRPC框架，将CZSL构建为结构化的问答式思维链推理过程，显式建模属性与对象间的双向依赖进行逐步相互校正，并引入基于GRPO目标的强化学习后训练以提供步骤级奖励。

Result: 在三个CZSL基准数据集上的广泛实验表明，PRPC达到了最先进的性能。
...
### [7] [From Noisy Traces to Root Causes: Structural Trajectory Analysis and Causal Extraction for Agent Optimization](http://arxiv.org/abs/2607.07702v1)
*Ying Chang,Jiahang Xu,Xuan Feng,Chenyuan Yang,Peng Cheng,Yuqing Yang*

Main category: cs.CL

TL;DR: 本文提出STRACE框架，通过结构化轨迹分析和因果提取构建高信噪比的优化上下文，以解决长周期智能体优化中的轨迹冗余和因果证据丢失问题。


<details>
  <summary>Details</summary>
Motivation: 直接使用真实执行轨迹进行长周期智能体优化存在效率低下和过拟合问题，因为轨迹集合往往冗余且异构，单条轨迹包含大量无关步骤，而简单的截断或滑动窗口方法会丢弃重要的因果证据并产生误导信号。

Method: STRACE在批次级别挖掘失败模式以过滤冗余轨迹并保留代表性失败，在轨迹内部构建文本依赖图进行因果定位，以移除非因果步骤并识别出真正的根因模块用于优化。

Result: STRACE显著优于标准上下文过滤基线，在VeruSAGE-Bench任务上成功优化人类专家设计的智能体，将成功率从42.5%提升至58.5%（1.4倍提升）。

Conclusion: 通过构建高信噪比的优化上下文，STRACE实现了更精确有效的智能体优化，能够显著提升甚至超越人类专家设计的智能体性能。
...
### [8] [Don't Commit Alone: Joint Token Commitment in Diffusion Large Language Models](http://arxiv.org/abs/2607.04469v1)
*Lin Yao*

Main category: cs.CL

TL;DR: 提出CoCommit方法，通过标记门控协调通道实现扩散语言模型的联合模式解码，提升多token提交的准确性。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在去噪步骤中独立解码多个token，当位置存在依赖时会产生因式分解误差，而基于置信度的选择无法仅从边缘分布观察到该误差。

Method: 提出CoCommit，在选定bundle后使用学习到的标记宣布提交集，重新应用骨干网络的最后n层使标记位置协调，近似联合模式解码后再进行贪心解码；该方法复用现有权重，仅需一次额外部分前向传播，无需辅助模型。

Result: 在LLaDA2.1-mini上使用LoRA适配器和匹配的贪心推理，联合提交在所有六个基准测试上均提高了准确性，在推理和精确答案任务上提升最大。

Conclusion: 通过协调通道实现联合提交能无需额外模型即提升扩散语言模型的准确性，特别是在推理和精确答案任务上效果显著。
...
### [9] [dOPSD: On-Policy Self-Distillation for Diffusion Language Models](http://arxiv.org/abs/2607.04428v1)
*Phuong Tuan Dat,Qi Li,Xinchao Wang*

Main category: cs.CL

TL;DR: 提出dOPSD方法，通过利用模型自身去噪轨迹作为特权信息改进扩散大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的后训练推理难以提升：监督微调存在离策略和暴露偏差，强化学习奖励稀疏且难以应用，而现有在线自蒸馏依赖不可用的外部特权信息导致效果有限。

Method: dOPSD从学生模型自身的去噪轨迹中直接导出教师特权信息，利用同一轨迹中后续更解码的步骤评估掩码位置，使教师优势源自模型自身解码过程而非外部标签。

Result: 在Dream和LLaDA上，dOPSD提升了域内数学推理和域外代码生成能力，优于监督和在线基线方法。

Conclusion: 通过自蒸馏轨迹派生特权信息可有效提升dLLM推理，无需外部参考标签。
...
### [10] [TACG: Trajectory-Aware Commit Gating for Diffusion Language Model Decoding](http://arxiv.org/abs/2607.03236v1)
*Chengcheng Wang,Tingzhang Luo,Wenhao Li,Jianyuan Guo,Chang Xu*

Main category: cs.CL

TL;DR: 本文提出了一种免训练的轨迹感知提交门控解码器（TACG），利用扩散语言模型的去噪轨迹来优化解码效率与稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型解码器仅依赖当前快照进行提交，忽略了预测分布轨迹，容易将瞬态高置信度误判为提交时机，导致锁定不成熟token或延迟稳定候选词。

Method: 提出TACG，将token身份锚定于基础后验，并利用轨迹感知信号决定提交时机。该方法结合了时间隐式对数几率引导（TILG）和历史门控（HG），在无需辅助网络或额外前向传播的情况下实现稳定受限的提交规则。

Result: 在多个代码和数学基准测试中，TACG在提高或保持准确率的同时，显著减少了去噪步数并增加了每次前向传播生成的token数量。

Conclusion: TACG通过有效利用轨迹信息，以零额外计算开销提升了扩散语言模型的解码效率和稳定性。
...
TL;DR: 本文提出SLIM-RL方法，通过tau-budget解码器和无迹随机掩码目标，在不重建轨迹的情况下解决了扩散大语言模型中随机掩码与推理轨迹不匹配的问题，大幅提升了训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的轨迹感知方法TraceRL需要通过切片rollout来重建推理轨迹，其计算成本随块大小增加而上升。本文旨在不重建轨迹的前提下缓解随机掩码与轨迹的不匹配问题，从而降低训练成本。

Method: 提出SLIM-RL方法：1) 使用tau-budget解码器限制每步rollout的提交风险；2) 在无迹随机掩码目标下训练风险控制的rollout；3) 引入均值保持且单调递减的块掩码计划，并结合序列级重要性采样和确定性求积等方差缩减工具。

Result: 在SDAR-4B上，块大小为16时仅需0.46倍训练样本即可匹配TraceRL的最佳MATH500精度；在匹配动态采样下，MATH500和GSM8K分别提升6.32%和11.05%。块大小为4时，4B模型在数学任务上超越LLaDA-8B和Dream-7B。代码任务上MBPP和HumanEval分别提升4.20%和3.65%。tau-budget解码器可免训练跨模型迁移。

Conclusion: SLIM-RL在避免昂贵轨迹重建的同时有效解决了掩码不匹配问题，实现了更高效的强化学习训练，并在数学和代码任务上取得了更优表现，且其解码器具有良好的跨模型泛化能力。
...
TL;DR: 本文对扩散语言模型的解码方法进行了严格评估，发现并行解码方法劣于单token解码基线且评估结果对提示模板高度敏感，并提出了可靠的评估指南。


<details>
  <summary>Details</summary>
Motivation: 现有关于扩散语言模型高效解码策略的研究在相同评估设置下存在评估结果不一致的问题，可能导致有偏见的结论。

Method: 在多种评估设置下对当前dLLM解码方法进行严格评估，深入分析提示模板及被忽视的评估设置对解码方法排名和性能的影响。

Result: 并行解码方法始终逊于单token解码基线，且对提示模板微小变化高度敏感；选用有效提示模板比增加去噪步数收益更显著。

Conclusion: 当前并行解码方法未能克服速度与质量的权衡，单模板评估具有误导性；基于此提出了dLLM解码方法可靠评估的实用指南。
...
TL;DR: 本文提出CaDDTree，一种成本感知的扩散草稿树方法，通过联合优化树结构和节点预算直接最大化token吞吐量，无需离线预算搜索即可匹配或超越DDTree的oracle预算选择性能。


<details>
  <summary>Details</summary>
Motivation: 现有DDTree方法在固定节点预算下最大化预期接受长度，但接受长度随预算非递减，总是偏向更大的树而忽略验证成本，缺乏原则性的预算选择依据。

Method: CaDDTree显式建模草稿和验证延迟，将token吞吐量目标分解为每轮一维预算搜索；在凸验证成本假设下证明吞吐量函数为单峰函数，从而采用高效贪心停止规则；每轮根据当前位置分布和验证成本自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型上，覆盖推理、编程、指令跟随等八个基准的实验表明，CaDDTree在几乎所有任务上匹配或超越使用oracle预算选择的DDTree。

Conclusion: CaDDTree通过成本感知的联合优化解决了投机解码中预算选择缺乏原则性依据的问题，实现了无需离线搜索的自适应高效推理加速。
...
TL;DR: 本文提出DDTree方法，通过从块扩散草稿模型的逐位置分布构建草稿树，解决了单轨迹验证的局限并提升了推测解码性能。


<details>
  <summary>Details</summary>
Motivation: 现有的先进块扩散草稿模型DFlash在每轮中仅验证单条草稿轨迹，限制了其接受长度，本文旨在解决这一瓶颈。

Method: 提出DDTree，直接从块扩散草稿模型的逐位置分布构建草稿树。在固定节点预算下，利用最佳优先堆算法根据草稿模型输出的代理指标选择最可能匹配的延续，并使用仅祖先注意力掩码在单次目标模型前向传播中高效验证该树。

Result: 基于DFlash的改进，DDTree有效扩展了草稿验证的广度，使其成为推测解码领域的领先方法之一。

Conclusion: DDTree成功突破了单轨迹验证的限制，通过结合块扩散与树结构验证策略，进一步推动了推测解码技术的性能上限。
...
### [15] [Breaking Database Lock-in: Agentic Regeneration of High Performance Storage Readers for Database Bypass](http://arxiv.org/abs/2607.07696v1)
*Victor Giannakouris,Immanuel Trummer*

Main category: cs.DB

TL;DR: 本文提出Jailbreak方法，利用大语言模型（LLM）合成的代码绕过数据库引擎直接读取存储文件，将其转化为内存列式缓冲区，显著提升分析型工作负载的吞吐量。


<details>
  <summary>Details</summary>
Motivation: 传统分析工作负载通过数据库驱动（如JDBC或ODBC）访问数据时，必须经过非批量列式分析设计的查询执行和驱动层，这构成了根本性的数据访问瓶颈。

Method: 绕过数据库引擎直接读取存储文件，利用LLM摄取数据库文件格式的源代码和文档以自动生成操作符特定的表读取组件，无需人工编写解析逻辑，并将数据物化为可被主流查询引擎直接使用的Apache Arrow内存列式缓冲区。

Result: 在PostgreSQL和MySQL存储文件上使用TPC-H基准测试验证了正确性，且与基于JDBC/ODBC的基线相比，在端到端分析吞吐量上实现了高达27倍的加速。

Conclusion: LLM辅助的存储读取器合成是一种可行且通用的方法，能够打破跨数据库系统的数据锁定，适用于任何文件格式可通过文档或源代码获取的数据库系统。
...
### [16] [Institutional Red-Teaming: Deployment Rules, Not Just Models, Causally Shape Multi-Agent AI Safety](http://arxiv.org/abs/2607.07695v1)
*Yujiao Chen*

Main category: cs.AI

TL;DR: 本文引入“机构红队测试”方法，通过仅改变单一部署规则来评估多智能体AI系统的集体行为，揭示了规则对安全的因果影响及身份显著性的作用机制。


<details>
  <summary>Details</summary>
Motivation: 旨在解决如何受控地测试和归因单一部署规则对多智能体AI系统集体行为与安全性的因果影响问题。

Method: 提出机构红队测试方法（固定智能体、目标和任务状态，仅改变一条规则以归因行为变化），并在IABench-CA基准上进行实例化（涵盖228个上下文、5种规则和7个模型群体）。

Result: 部署规则会因果性改变集体安全性（改变规则使平均死亡率波动22至58个百分点）；不存在安全的默认规则，且回归性身份定向消除在所有群体中普遍不安全；规则文本中的身份显著性是驱动定向消除的机制，匿名化仅能延迟该行为。

Conclusion: 将该方法封装为安全案例工作流，为特定部署环境和群体认证临时规则区域，并明确残余风险与监控义务。
...
TL;DR: 本文介绍了SkillCenter，一个面向自主AI代理的大型开源技能库，包含216,938个结构化技能，通过端到端框架实现来源可追溯的技能构建与发布。


<details>
  <summary>Details</summary>
Motivation: 自主AI代理虽然能执行复杂任务，但缺乏扎实的操作知识，导致输出虽可执行但在正确性、安全性和可维护性方面不足，因此需要一个大规模、来源可追溯的技能库来弥补这一缺陷。

Method: 提出端到端框架，包含多源采集、基于LLM的质量门控（SkillGate）、模板驱动生成、迭代式来源溯源和质量控制发布；来源溯源保证每条保留的声明均可映射到源文档中的精确引用；所有技能以SQLite FTS5离线可搜索包形式发布。

Result: 构建了涵盖24个领域包的216,938个结构化技能库，其中114,565个来自同行评审期刊、ArXiv及24,000余个技术来源的溯源技能，102,373个来自GitHub和ClawHub市场的社区技能。

Conclusion: SkillCenter为自主AI代理提供了目前最大规模的开源技能库，通过来源溯源机制确保技能的可信度和可追溯性，为代理输出正确、安全、可维护的结果提供了知识基础。
...
TL;DR: 提出TAPS方法通过目标感知的前缀选择优化扩散模型投机解码的验证阶段，实现高达7.9倍的无损端到端加速。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的草拟树方法在验证阶段使用边际概率排序节点，忽略了验证的前缀条件性，导致验证被拒绝前缀的不可达节点，增加了目标模型延迟且接受度提升有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边际概率转化为路径条件的接受度估计，并在固定验证预算下选择紧凑的闭合前缀子树，以优化接受度与成本的权衡。

Result: 在多数据集和模型族上的实验表明，TAPS相比普通自回归解码实现最高7.9倍的无损加速，且优于当前最先进的DFlash和DDree方法（分别快1.36倍和1.74倍）。

Conclusion: 通过优化基于前缀条件的验证策略，TAPS有效解决了扩散模型投机解码中的验证瓶颈，显著提升了大模型推理效率。
...
### [19] [GemNav: Discrete-Token Visual Robot Navigation using a Multimodal Large Language Model](http://arxiv.org/abs/2607.06882v1)
*Peter Bohm,Saimunur Rahman,Abdelwahed Khamis,Sagun Man Singh Shrestha,Chris McCool,Peyman Moghadam*

Main category: cs.RO

TL;DR: 本文提出GemNav，通过仅微调语言模块并使用离散token词汇表，将冻结的多模态大语言模型高效适配为视觉机器人导航策略。


<details>
  <summary>Details</summary>
Motivation: 现有的基于大模型的视觉导航策略通常需要专门的视觉编码器、定制的动作头和海量训练数据，本文旨在探讨这种复杂且数据密集的范式是否必要。

Method: 提出GemNav，使用低秩自适应（LoRA）仅微调冻结的多模态大语言模型（MLLM）的语言模块进行中短程航点导航，无需辅助视觉编码器和连续回归头；航点和导航信号共享单一离散token词汇表，并通过软解码辅助损失恢复纯交叉熵训练丢弃的度量结构。

Result: 在仅8.7小时的数据集上训练（比现有方法小约三个数量级），策略在四个物理上不同的未见环境中实现零样本迁移，并在20次真实世界试验中距离目标0.25-0.42米处停下；此外，短图像历史序列仅提升离线指标，对实际机器人性能无益。

Conclusion: 对冻结的多模态大语言模型进行离散token自适应，可为基础模型机器人导航提供一种数据高效且可部署的替代方案。
...
### [20] [UniLM-Nav: A Unified Framework for Zero-Shot Last-Mile Navigation](http://arxiv.org/abs/2607.06537v1)
*Zhuofan Zhang,Tianxu Wang,Guoxi Zhang,Yixiong Lin,Xilin Wang,Hongming Xu,Qing Li,Song-Chun Zhu,Lifeng Fan*

Main category: cs.RO

TL;DR: 本文提出UniLM-Nav框架，利用多模态大语言模型实现零样本开放词汇的最后一公里导航，解决移动操作中的基座位姿推理问题。


<details>
  <summary>Details</summary>
Motivation: 移动机器人到达目标附近并不能保证具备操作就绪的基座位姿（即最后一公里导航问题），而现有方法依赖手动位姿标注或特定任务训练，限制了其在细粒度空间约束和开放词汇环境中的可扩展性。

Method: UniLM-Nav通过共享的多模态大语言模型（MLLM）后端，将最后一公里导航分解为三个步骤：1) 从近期观测中选择最佳参考视角；2) 在选定视角中定位任务相关可供性点并转换至机器人中心坐标系；3) 结合可供性、任务上下文与机器人几何形状，推断出操作就绪的基座位姿。

Result: 在OVMM基准测试中，该方法较先前最优方法MoTo提升了3.13个百分点。消融实验证明各组件对性能至关重要，且MLLM的选择对结果有显著影响。在搭载6自由度机械臂的四足机器人上完成部署，验证了其在真实任务中的有效性。

Conclusion: UniLM-Nav成功解决了开放词汇环境下零样本最后一公里导航问题，无需手动标注或特定训练即可生成操作就绪的基座位姿，在仿真与真实世界中均展现出卓越的有效性与应用潜力。
...
### [21] [POPS: Recovering Unlearned Multi-Modality Knowledge in MLLMs with Prompt-Optimized Parameter Shaking](http://arxiv.org/abs/2607.06649v1)
*Zhangheng LI,Jianing Zhu,Junyuan Hong,Sungmin Eum,Shuowen Hu,Suya You,Zhangyang Wang*

Main category: cs.CR

TL;DR: 本文提出了一种名为POPS的对抗策略，通过提示优化和微调成功恢复了多模态大语言模型中被遗忘的隐私信息，揭示了现有机器遗忘方法的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 多模态机器遗忘（MMU）旨在让模型遗忘隐私信息，但在模型对恶意用户公开时，此类遗忘方法的鲁棒性尚未得到充分验证与探索。

Method: 提出一种新颖的对抗策略——提示优化参数震荡（POPS），通过提示后缀优化诱导受害者模型生成潜在隐私样本，并利用这些合成输出微调模型以促使其泄露真实隐私信息。

Result: 在多个MMU基准测试中，POPS实现了对被遗忘敏感信息的近乎完全恢复，暴露了现有MMU算法的重大缺陷。

Conclusion: 现有的基于MMU的隐私保护机制存在根本性漏洞，其基础鲁棒性面临严峻挑战。
...
### [22] [A hierarchical memory architecture overcomes context limits in long-horizon multi-agent computational modeling](http://arxiv.org/abs/2607.07666v1)
*Shivendra G. Tewari,Holly Kimko*

Main category: q-bio.QM

TL;DR: 本文提出了Ensemble QSP多智能体框架，通过三层分层记忆架构解决大语言模型在长期科研工作流中缺乏状态连续性和定量严谨性的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的无状态架构从根本上限制了其在需要多会话连续性和定量严谨性的长期研究工作流中的部署。

Method: 提出一种包含三层分层记忆架构的多智能体框架（Ensemble QSP），通过限制状态类别并移除已完成工作来保持上下文边界恒定；系统在领域专家PI的协调下编排五个专门工作智能体，并利用基于物理的检查清单和结构化领域知识来强制执行物理约束。

Result: 在无需人工干预的情况下实现了稳健的自主药代动力学-药效学模型选择，在不同LLM上保持一致的结果质量，相比单智能体基线提高了PK参数恢复率，且在语言多样的提示下模型选择稳定；消融实验表明PI智能体监督提高了调试效率并保持了最终准确率。

Conclusion: 该架构在结构上与领域无关，扩展到新的科学领域仅需配置新的PI智能体。
...
### [23] [Agent Delivery Engineering Predictive Reliability Framework](http://arxiv.org/abs/2607.07689v1)
*Dexing Liu*

Main category: cs.MA

TL;DR: 本文提出ADE预测可靠性框架（ADE-PRF），通过聚合多层异构信号实现LLM多智能体系统健康轨迹的前瞻性预测与早期预警。


<details>
  <summary>Details</summary>
Motivation: 长时间跨度的LLM多智能体系统面临基础设施监控无法察觉的潜在可靠性风险，亟需从被动退化检测转向主动健康轨迹预测以应对隐蔽的系统退化。

Method: 提出ADE-PRF框架，将跨五个层级的20个异构信号聚合为信任边际（TM）指标，并采用包含指数法和卡尔曼滤波在内的三重方法并行预测8小时健康轨迹。

Result: 生产环境验证显示指数法预测效果最佳（MAE=1.228，方向准确率76.8%），框架成功检测到被正常表面指标掩盖的“虚假繁荣”退化现象，且集成ADE插件后TM指标能立即与真实状态耦合。

Conclusion: ADE-PRF为生产级LLM智能体提供了早期的可靠性量化与前瞻性预警能力，有效揭示了隐蔽的系统退化风险。
...
TL;DR: Sangam是一个针对缓存扩散语言模型推理的服务系统，通过赤字令牌预算调度器和混合服务策略解决dLLM的预填充-解码干扰与资源分配问题。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（dLLM）的双向注意力阻碍了精确的KV缓存，导致重复的预填充/解码结构，使得现有的自回归（AR）服务机制无法直接应用，并引发预填充-解码干扰与资源分配问题。

Method: 提出Sangam系统，引入赤字令牌预算调度器实现摊销的无停顿调度（优先解码，按预算准入完整预填充）；采用混合服务策略，将溢出的预填充转移到解码工作节点，并用同一调度器保护解码过程。

Result: 在解码密集型负载下，共置服务比混合执行降低平均延迟9-20%；在预填充密集型负载下，混合执行比共置服务降低平均延迟8-20%。

Conclusion: dLLM服务设计空间受预填充-解码干扰和分区控制；根据负载类型选择共置或混合执行策略可有效优化服务性能。
...
### [25] [Towards Agentic AI Governance: A Preliminary Assessment](http://arxiv.org/abs/2607.07612v1)
*Mubarak Raji,Masooda Bashir*

Main category: cs.CY

TL;DR: 本文系统回顾了自主智能体AI治理的新兴文献，为构建负责任的治理路线图奠定基础。


<details>
  <summary>Details</summary>
Motivation: 人工智能正向能自主规划和执行任务的智能体AI演进，引发了新的伦理与治理挑战，亟需针对性的治理关注。

Method: 采用系统性文献综述方法，分析智能体AI区别于传统系统的特征，并综合当前的治理优先事项、机制及利益相关者角色。

Result: 识别了智能体AI的独特特征及其需针对性治理的原因，并总结了塑造该领域发展的治理优先级、机制与利益相关者角色。

Conclusion: 本综述作为初步的学术努力，为开发指导负责任且具适应性的智能体AI治理结构化路线图奠定了初步基础。
...
TL;DR: 本文提出了Bifocal dLLMs（R2LM），通过结合因果注意力和反向Mamba SSM，在保持KV缓存效率的同时保留右侧上下文，解决了离散扩散语言模型的质量与吞吐量困境。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型面临架构设计困境：双向注意力生成质量高但不支持KV缓存，限制了吞吐量；因果注意力支持缓存但丢失右侧上下文，严重降低生成质量。

Method: 提出R2LM（Right-to-Left Mamba），采用非对称双向上下文范式。结合标准因果注意力提供精确的左侧上下文（完全兼容KV缓存），并辅以轻量级的反向Mamba SSM侧边模块提供压缩的右侧上下文（不破坏可缓存性）。

Result: 在Qwen3-1.7B上的继续预训练实验表明，R2LM的吞吐量比双向dLLM高2.4至12.9倍，在批量服务中比自回归基线快1.9至2.9倍，同时在多数基准测试中超越因果基线，并在平均水平上超越双向dLLM。

Conclusion: R2LM成功解决了dLLMs在推理速度与生成质量间的架构困境，证明了通过非对称上下文机制实现高效缓存并行解码的有效性。
...
### [27] [When the Environment Speaks: Quantum Signatures in Non-Attractor Inflation](http://arxiv.org/abs/2607.07602v1)
*Mattia Cielo,Simone Scarlatella,Gianpiero Mangano,Ofelia Pisanti*

Main category: astro-ph.CO

TL;DR: 本文研究了暴胀期间绝热曲率扰动与大质量熵标量环境相互作用的开量子动力学，发现量子环境对原初标量功率谱和标量诱导引力波留下了独特的可观测印记。


<details>
  <summary>Details</summary>
Motivation: 旨在解决暴胀期间（特别是包含瞬态超慢滚阶段时）量子环境对曲率扰动演化的影响问题，验证量子环境究竟是被动旁观者还是能产生可观测印记的主动作用者。

Method: 在高斯双场有效拉格朗日量框架下，采用精确输运方程方法，追踪系统协方差矩阵在慢滚-超慢滚-慢滚转变过程中的完整非幺正、非马尔可夫演化。

Result: 退相干效率对视界穿越时的背景运动学敏感；环境相互作用导致原初标量功率谱的特征干涉凹陷被部分或完全抹除、增长斜率改变并诱发振荡特征；这些畸变进一步传播至标量诱导引力波背景，打破单场预测并产生LISA可探测的独特频谱特征。

Conclusion: 量子环境在暴胀期间是主动作用者而非被动旁观者，其对原初宇宙的印记有望被下一代宇宙学观测所探测。
...
### [28] [Rethinking Code Performance Benchmarks for LLMs](http://arxiv.org/abs/2607.07619v1)
*Nhat Minh Le,Yisen Xu,Zhijie Wang,Tse-Hsun,Chen*

Main category: cs.SE

TL;DR: 重新评估大语言模型性能基准发现其测试用例不足，并提出一种多智能体框架以生成更能暴露运行时间差异的性能测试。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM功能级性能基准在评估生成代码的效率时，往往无法显示其与标准方案之间的执行时间差异，这主要受限于评估方法以及基准自带的性能测试不够充分。

Method: 提出一种基于LLM的多智能体框架，包含生成、诊断和修复三个代理，用于创建在保证功能正确性的同时更能有效暴露性能差异的确定性测试。

Result: 在原始测试未发现显著性能差异的1,345个任务中，由DeepSeek-v3.1和GPT-4o驱动的框架生成的测试分别在24.01%和25.43%的任务中揭示了统计上显著的性能提升，优于现有的SOTA方法。

Conclusion: 基准自带性能测试的不充分性是限制LLM生成程序性能评估的主要瓶颈，而所提出的多智能体框架能够有效生成测试以揭示这些隐藏的性能差异。
...
### [29] [DELTA-TTS: Adapting Autoregressive Model into Diffusion Language Model for Text-to-Speech](http://arxiv.org/abs/2607.04140v1)
*Junwon Moon,Seungbeom Kim,Yejin Lee,Hoseong Ahn,Sewoong Park,Heeseung Kim,Kyuhong Shim*

Main category: eess.AS

TL;DR: 本文提出DELTA-TTS框架，通过将预训练自回归TTS模型转化为离散扩散语言模型来实现置信度排序的语音解码，从而提升推理速度并缓解幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 自回归TTS模型由于从左到右的顺序生成导致推理缓慢，且易传播局部错误和产生幻觉。考虑到TTS合成前已有完整文本输入，这种顺序限制并非固有需求，因此需要一种非顺序的解码方法来提升速度和鲁棒性。

Method: DELTA-TTS采用轻量级LoRA适配将预训练AR TTS模型转换为离散扩散语言模型(dLLM)，引入卷积模块注入局部声学上下文，并结合1/t加权训练目标和时间偏移推理调度，将低置信度位置的解码推迟至后续步骤。

Result: 在仅使用585小时LibriTTS数据训练下，模型在Seed-TTS test-en上取得1.75%的WER，优于其AR主干模型且生成速度快3.3倍；同时实现了更清晰的文本-语音对齐，提升了整体解码置信度并减少了幻觉。

Conclusion: 将自回归TTS模型转化为离散扩散模型进行置信度排序解码，不仅能显著提升推理速度和合成质量，还能有效增强鲁棒性并缓解幻觉问题。
...
### [30] [Agon: Competitive Cross-Model RL with Implicit Rival Grading of Reasoning](http://arxiv.org/abs/2607.07690v1)
*Vladislav Beliaev*

Main category: cs.LG

TL;DR: 提出Agon方法，通过让两个模型相互竞争和评分来隐式评估推理过程，无需过程标签即可显著提升模型推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的可验证奖励强化学习（如GRPO）仅评估最终答案，导致模型在困难问题上倾向于生成冗长文本而非提升推理质量，且缺乏评估思考过程的标签。

Method: 提出Agon，让两个模型相互评分：一个起草方案，另一个在阅读方案的同时解决问题，双方因胜过对方而获得奖励。这形成了一种无需过程标签和奖励模型的隐式推理评估机制，且推理时以双阶段级联方式部署。

Result: 在DeepMath的困难数据集上，Agon将GRPO的pass@1翻倍，其提升幅度约为未训练Mixture-of-Agents的8倍，且在竞争性编程及其他模型家族上同样有效。

Conclusion: 竞争机制能有效提升模型推理能力，未来的研究方向是让模型在隐空间中共同推理。
...
TL;DR: 本文提出HERALD，一种针对块扩散大语言模型的KV缓存卸载系统，通过优化稀疏KV选择计算并使其与去噪过程重叠，实现长上下文下的高吞吐近无损推理。


<details>
  <summary>Details</summary>
Motivation: 扩散LLM的KV缓存随上下文线性增长，限制了长上下文吞吐量。将KV卸载至主机内存虽能缓解压力，但PCIe带宽限制要求仅调用稀疏KV子集，而在块dLLM中对完整KV缓存进行精确注意力选择在卸载场景下计算成本过高。

Method: 提出HERALD系统，利用块dLLM中相关KV条目在去噪步骤间保持一致的特性，通过两项优化（将所需选择计算量降低至块大小的倍数，并使选择过程与去噪过程重叠）来解决KV选择成本问题，实现高效卸载。

Result: 在3个块dLLM和5个长上下文任务上，HERALD在5-10%的KV预算下实现近无损精度，与纯GPU推理相比，每块延迟最高降低1.59倍，吞吐量提升2.47倍，且加速比随上下文长度增加而扩大。

Conclusion: HERALD有效解决了块扩散大语言模型在长上下文推理中的KV缓存瓶颈，通过高效的选择计算分摊与重叠机制，在不牺牲精度的前提下显著提升了系统的推理效率和吞吐量。
...
TL;DR: 本文提出了Diffusion-Proof，这是首个用于形式化定理证明的扩散大语言模型（dLLM）训练与应用框架。


<details>
  <summary>Details</summary>
Motivation: 自回归（AR）大语言模型在形式化定理证明中存在长距离连贯性差和错误累积的固有局限性，而扩散大语言模型（dLLM）虽具潜力但在此领域的应用尚属空白。

Method: 提出Diffusion-Proof框架，包含两个模型：用于长距离连贯策略全证明生成的dLLM-Prover-7B，以及利用双向信息和填补能力进行局部证明纠正的dLLM-Corrector-7B。

Result: 在相同数据集训练下，该框架显著优于AR基线模型，在ProofNet-Test和MiniF2F-Test上分别实现了1.61%和6.14%的绝对提升，并成功解决了一个DeepSeek-Prover-V2-7B未能解决的IMO问题。

Conclusion: 扩散大语言模型（dLLM）在形式化定理证明中克服了自回归模型的局限性，展现出独特的优势与巨大的应用潜力。
...
### [33] [Music I Care About: Automated Multimodal Benchmarking of LLM Music Perception Skills on (Almost) Any Music](http://arxiv.org/abs/2607.06015v1)
*Tomáš Sourada,Katia Vendrame,Jan Hajič*

Main category: cs.SD

TL;DR: 本文提出了MusICA-MetaBench，一个可根据用户数据自动生成跨模态音乐感知评估基准的元基准框架。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型音乐基准存在评估资源消耗大且泛化性不足、未能真正测试音乐感知能力、且缺乏跨模态（音频、符号、乐谱）系统比较等局限。

Method: 提出MusICA-MetaBench框架，利用结构化符号表示（如MusicXML）和预定义问题模板，自动从用户数据生成跨音频、乐谱图像和符号文件的多项选择问答对，以评估对齐音乐教学法的感知能力。

Result: 通过ChoraleBricks数据集验证了框架有效性，确定了统计可靠的基准规模，并通过与纯文本和白噪声基线的对比证明了该基准能有效测量音乐感知能力。

Conclusion: MusICA-MetaBench推进了多模态大语言模型音乐感知的跨模态评估，其按需和特定数据集的基准评估范式显著提升了评估效率。
