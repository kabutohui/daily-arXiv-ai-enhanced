...
TL;DR: 提出EviSelect框架，基于目标MLLM内部注意力证据进行动态视觉选择，实现长视频理解的高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部代理评分器和固定规则进行帧选择，与目标MLLM内在证据不对齐，无法适应视频非均匀时空信息密度，导致效率与性能受限。

Method: 通过稀疏预填充和高度压缩视觉输入近似目标MLLM注意力图作为结构化先验，设计轻量级选择器基于三组互补注意力成分定位查询相关时间戳并自适应调整局部采样率和空间分辨率，将选择器建模为随机策略并用GRPO在联合准确率-效率奖励下优化。

Result: 在三个长视频理解基准上超越现有方法，同时减少约50%的视觉token选择量，实现3.9倍端到端加速。

Conclusion: 基于MLLM内部注意力证据的动态视觉选择可有效平衡准确率与效率，使计算资源按视频信息密度自适应分配，为长视频理解提供了高效且对齐的解决方案。
...
TL;DR: 本文提出VideoRouter（VR），通过构建视频时间层次结构和验证引导路由器协调全局与局部互补视图，解决长视频理解中全局上下文与细粒度证据难以兼顾的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频理解中受限于token限制，难以捕获时间稀疏证据。现有均匀采样或帧选择方法无法同时保持全局剧情上下文和细粒度证据，在广泛时间覆盖与局部相关性之间存在权衡困境。

Method: VideoRouter将每个视频组织为问题无关的时间层次结构，将视频划分为由粗到细的时间连贯片段；上层节点捕获全局剧情上下文与事件进展，下层节点保留细粒度局部细节与关键证据，形成全局视图（覆盖导向推理）和局部视图（细节导向证据恢复）两种互补视图；进一步引入验证引导路由器判断哪种视图更受证据支持，并选择最终答案。

Result: 在VideoMME数据集上，基于LLaVA-Video-7B骨干网络，VideoRouter比最先进的帧选择方法分别提升2.9分；实验表明验证引导路由器能有效协调全局与局部推理。

Conclusion: 验证引导路由器有效协调全局与局部推理，将长视频理解重新定义为协调互补证据视图而非选择单一帧子集，显著提升了长视频理解性能；代码将公开释放。
...
TL;DR: 本文提出了SPRINT基准，通过真实体育视频评估多模态大语言模型（MLLM）主动预测物理危害的能力，揭示了模型在危害敏感性与因果理解之间的巨大差距。


<details>
  <summary>Details</summary>
Motivation: 现有的MLLM评估多集中于有害内容或一般风险，忽视了对现实世界安全至关重要的主动物理危害预测问题。

Method: 构建了包含2,888个真实体育视频的SPRINT基准，提供早期危害线索、事故时间和层级原因的细粒度标注，并在多种提示和时间窗口下评估最先进的MLLM。

Result: 最佳模型在发出危害信号方面准确率超95%，但在识别原因方面低于50%；诊断实验表明，明确的风险查询会在无危害视频中引发严重误报。

Conclusion: 当前MLLM仅具备表面的主动安全能力，缺乏稳定且基于原因的早期预警机制，强调了在动态物理环境中实现可靠主动安全的必要性。
...
TL;DR: 本文实证比较了程序化工具调用（PTC）与原生JSON工具调用在BFCL v4基准上跨14个语言模型的表现，发现PTC在多数模型中匹配或超越JSON基线，是一种可行且稳健的替代方案。


<details>
  <summary>Details</summary>
Motivation: 尽管程序化工具调用能够通过脚本自然地链式和并行化工具调用，取代僵化的JSON调用，但目前尚无研究在已建立的基准上、跨当前及历史模型代际、在真实任务条件下系统性地评估工具作为代码的使用效果。

Method: 在程序化工具调用范式下，工具以带类型的Python桩函数形式暴露，模型通过代码调用它们，执行与结果处理在单一智能体轮次内完成；研究在BFCL v4上对14个语言模型进行PTC与原生JSON工具调用的实证对比。

Result: PTC在14个模型中的11个上匹配或超过JSON基线，GPT-5.6系列提升10.6%；在并行扇出场景中14个模型中13个匹配或优于基线；在上下文腐烂条件下保持稳定，而基线平均下降2.3%。

Conclusion: 程序化工具调用是一种可行且稳健的JSON工具调用替代方案，其性能随模型发布代际的能力提升而同步增长，表明该方法具有良好的可扩展性和鲁棒性。
...
TL;DR: 本文提出了一个无参考框架，利用大语言模型裁判来评估面向任务的对话智能体基准的质量。


<details>
  <summary>Details</summary>
Motivation: 现有的面向任务的对话智能体基准很少评估其自身的质量，而糟糕的基准（如任务不一致、场景简单、策略覆盖有限）会导致模型评估结果不可靠。

Method: 引入一个无参考框架，利用大语言模型裁判评估基准的一致性、复杂性和策略覆盖，并提供可操作的弱点诊断。

Result: 该框架的指标在各领域和裁判模型中均能一致区分基准质量水平，与人工标注高度一致，并成功评估了不同大语言模型生成及受质量退化扰动影响的基准。

Conclusion: 该框架为评估合成和人工精选的对话智能体基准提供了一种实用的方法。
...
TL;DR: 本文提出了首个针对国家标准文档结构化审查的基准GB/T-Bench，并开发了多智能体框架GB/T-Reviewer以提升大语言模型在规则密集型文档审查中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在规则密集型文档审查方面的能力缺乏充分评估，且现有基准忽略了专业文档的内在质量审查，该过程严重依赖昂贵且难以扩展的人类专家。

Method: 提出了GB/T-Bench基准，包含层级化的审查分类法（25种错误类型）、可控的反例生成机制、诊断导向的评估协议，以及一个将审查知识转化为专业技能并协调全局检查、定向诊断、规则扫描和结果验证的多智能体框架GB/T-Reviewer。

Result: 对14个主流大模型的实验表明，最强模型仅取得0.3280的CMCS分数，远低于人类专家的0.6640；而GB/T-Reviewer将最优CMCS分数提升至0.5094。

Conclusion: 结构化的技能协调对于规则密集型文档审查具有重要价值，该研究为标准化及其他高风险文档领域的可信AI奠定了基础。
...
TL;DR: 本文发现掩码扩散语言模型在推理任务中的无序解码自由度会导致过早提交答案，并提出前沿门控承诺机制有效修复此问题。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型（dLLM）以任意顺序提交token的优势在推理任务中反而导致失败：无约束解码会在推理区域仍被掩码时过早提交最终答案，导致高达90%的问题退化为仅输出答案。

Method: 通过2x2提示-解码器设计揭示了链式思维仅在有序承诺下有效，并提出了一种单旋钮干预措施——前沿门控承诺，通过限制位置可达性来恢复有序提交。

Result: 在LLaDA-8B和GSM8K上的实验中，该方法将准确率从0.528提升至0.852，同时保留了最高4倍的并行解码速度，且在Dream-7B和MATH-500数据集上复现了结果。

Conclusion: 研究重新定义了现有的窗口式采样器，指出它们虽以效率为初衷，但实际上是解决dLLM推理病理学问题的最小修复方案。
...
TL;DR: 本文提出OPTD方法，通过在策略过渡蒸馏与一致性引导的自适应压缩相结合，解决扩散语言模型少步蒸馏中轨迹漂移与步数压缩的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有少步蒸馏方法在非策略轨迹上构建监督，但学生模型在推理时的早期并行承诺会改变后续预测上下文，导致实际访问的状态偏离监督状态，尤其在压缩最激进时最为严重；而在策略蒸馏虽可缓解此不匹配，但如何确定每个过渡应推进的距离仍悬而未决。

Method: OPTD从少步学生自身轨迹采样部分状态，使用冻结的仅问题输入的教师模型识别与结果对齐的未来候选，并按当前状态置信度排序；然后选择最长的前缀，使其联合承诺保持教师rollout的结果不变；采用集合瓶颈目标将每个验证的未来候选提升至解码器释放阈值，同时用冻结教师KL锚正则化所有其他活跃位置。

Result: 在四个数学推理和代码生成基准上，OPTD持续改善质量-效率权衡，并在所有评估的少步基线中取得了最强的质量约束AUP（曲线下面积）。

Conclusion: OPTD无需黄金响应进行目标构建或训练，通过自适应压缩在策略蒸馏有效解决了状态漂移与中间依赖违反之间的矛盾，为扩散语言模型的高效生成提供了新的质量-效率帕累托前沿。
...
TL;DR: 研究发现免训练加速方法会导致扩散多模态大语言模型的输出发生漂移，通过缩短KV缓存刷新间隔可以有效控制这种一致性漂移并保持加速效果。


<details>
  <summary>Details</summary>
Motivation: 免训练加速技术虽提高了基于扩散的多模态大语言模型的部署便利性，但会悄无声息地改变生成内容，引发服务时间一致性问题。

Method: 在300张真实图像上对比Fast-dLLM加速与未加速的输出，通过置信度阈值调优、状态刷新消融和图像交换干预诊断漂移原因，并评估缩短KV缓存刷新间隔及其他缓存策略的效果。

Result: 置信度阈值调优不影响基准一致性，陈旧的视觉和文本状态导致漂移；缩短KV缓存刷新间隔在1.3倍加速下实现近乎完全一致；盲评显示事实性错误无显著差异；自适应刷新变体未优于固定间隔。

Conclusion: 提出了一种配对诊断方法和基于实现范围的一致性控制手段，但不提供准确性或安全性保证。
...
TL;DR: 本文提出了基于轨迹的在策略蒸馏框架TOPD，用于将推理能力从教师模型迁移到扩散大语言模型，在保持密集监督的同时避免了强化学习的稀疏奖励问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的推理导向后训练面临挑战：监督微调需要密集但常偏离策略的掩码状态，强化学习依赖稀疏奖励或价值建模，效率低下且难以稳定训练。

Method: TOPD从目标dLLM采样在策略扩散轨迹，由教师模型在对应部分去噪状态上提供token分布，使用token级Reverse KL目标更新目标模型，聚焦于与最终响应一致的轨迹对齐token决策，保持密集教师监督同时与模型自身去噪状态对齐。

Result: SDAR-4B-Chat在MATH500上匹配RL训练对应模型TraDo-4B-Instruct的准确率，静态评估提升+5.7，动态评估提升+4.5；相比RL方法仅需1/4的rollout轮次，估计模型计算加速96.0倍。

Conclusion: TOPD为dLLM推理后训练提供了一种高效替代方案，无需奖励估计即可实现与RL相当的性能，显著降低计算成本，为扩散语言模型的推理能力提升开辟新路径。
...
TL;DR: 本文提出了DominoTree，一种无需训练的最佳优先草稿树方法，通过利用Domino的路径依赖条件校正，显著提升了大语言模型推理的接受长度和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的最佳优先树方法（如DDTree）采用因子化公式，无法表示像Domino这类引入了基于GRU因果校正的草稿器所产生的路径依赖分布，限制了推测解码的性能。

Method: 提出DominoTree，一种无需训练的最佳优先草稿树。该方法通过将每个节点的校正限制在候选top-M集合中，利用Domino的条件非因子化校正对根到节点的路径进行评分，并使用GPU原生的CUDA图构建器进行低成本且位一致的树构建。

Result: 在Qwen3-4B上实现了高达6.6倍的加速，并在所有测试温度下获得了所有评估方法中最高的平均接受长度（每轮高达10.7个token）；相比Domino解码器吞吐量整体提升9%至10%（在Alpaca上高达22%），并在所有温度下优于DDTree和CaDDTree。在Qwen3-8B上，各温度下保持了最高接受长度，且在T=0时吞吐量比DDTree提升24%。

Conclusion: DominoTree有效结合了路径依赖草稿器与树构建方法，显著提升了推理吞吐量和接受长度，但在较大模型的高温设定下其相对于其他树方法的优势会缩小甚至持平。
...
TL;DR: 本文提出CaDDTree方法，通过联合优化树结构和节点预算直接最大化token吞吐量，解决了DDTree中预算选择缺乏原则性依据的问题。


<details>
  <summary>Details</summary>
Motivation: 现有DDTree方法在固定节点预算下最大化期望接受长度，但接受长度随预算单调递增，总是偏好更大的树而忽略验证成本，无法为预算选择提供原则性依据。

Method: CaDDTree显式建模草稿和验证延迟，将吞吐量目标分解为每轮对预算的一维搜索，证明在凸验证成本下吞吐量函数为单峰函数，从而使用高效贪婪停止规则，无需离线预算搜索，每轮根据当前位置分布和验证成本自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型上、涵盖推理、编码和指令遵循的八个基准测试中，CaDDTree在几乎所有任务上匹配或超越了使用oracle预算选择的DDTree。

Conclusion: CaDDTree通过成本感知的联合优化，在不依赖离线预算调优的情况下实现了与oracle预算相当或更优的性能，为推测解码中的草稿树构建提供了更原则性的方法。
...
TL;DR: 本文提出DDTree方法，利用块扩散草稿模型的分布构建草稿树，以解决推测解码中单轨迹验证的限制并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash推测解码方法每轮仅验证单条草稿轨迹，限制了接受长度（acceptance length），未能充分发挥加速潜力。

Method: 提出DDTree（Diffusion Draft Tree），直接从块扩散草稿模型的逐位置分布构建草稿树；在固定节点预算下，使用最佳优先堆算法选择最可能匹配目标模型的续写，并利用仅祖先注意力掩码在单次目标模型前向传播中高效验证。

Result: 基于领先的DFlash模型，DDTree的性能提升使其跻身推测解码领域的领先方法之列。

Conclusion: 通过将单条草稿轨迹扩展为草稿树，DDTree有效突破了接受长度的限制，进一步推动了推测解码技术的发展。
...
TL;DR: 本文提出了多模态隐喻理解基准M3R-Bench与推理模型M3R-Reasoner，通过联合标注和课程强化学习解决现有模型跨模态证据与映射不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要通过孤立的子任务评估隐喻理解，缺乏基于证据的解释，难以衡量模型是否真正基于视觉和文本线索建立目标-源映射。

Method: 提出M3R-Bench基准（包含隐喻、映射、情感及阶段性解释的联合标注）以及M3R-Reasoner模型（结合基于课程的推理监督与任务感知强化学习，对齐模型推理与隐喻解释）。

Result: 现有模型存在忽略视觉证据的问题；仅8B参数的M3R-Reasoner在四项统一任务指标上超越大型商用多模态模型，其视觉证据和情感解释得分大幅领先GPT-5.5，平均评分超越Claude-Sonnet-4.6。

Conclusion: 提出的基准和方法有效弥补了多模态隐喻理解中的跨模态推理缺陷，证明小型模型通过特定训练策略可超越大型商用模型。
...
### [15] [AV-AIVAT: 74x Cheaper Agent Evaluation with Certified Anytime-Valid Stopping in Imperfect-Information Games](http://arxiv.org/abs/2608.06362v1)
*Boning Li,Yu Chen,Longbo Huang*

Main category: cs.GT

TL;DR: 该论文提出了结合AIVAT与置信序列的AV-AIVAT方法，以实现游戏智能体评估的高效、可审计的随时有效早停。


<details>
  <summary>Details</summary>
Motivation: 评估智能体需要消耗资源且游戏次数未知；固定预算评估要么浪费资源要么过早停止，而朴素停时与普通置信区间会破坏有效性保证；现有方差缩减工具AIVAT无法解决何时停止的问题。

Method: 提出结合动作信息价值评估工具（AIVAT）与连续监测置信序列的随时有效AIVAT（AV-AIVAT）；其在线价值模型仅从历史游戏中学习以避免自校正；采用渐近CS进行筛查，并使用经验伯恩斯坦CS进行精确认证。

Result: 在95%名义水平下，使用渐近CS时，经AIVAT校正的结果所需的手牌数中位数仅为原始结果的1/74；在HUNL中经验伯恩斯坦CS的停时比率中位数为1.37倍；同时在Leduc扑克中建立了结构化边界。

Conclusion: AV-AIVAT将方差缩减转化为高效且可审计的提前停止，区分了渐近筛查与精确认证，使评估在证据充分时即刻停止，并允许第三方在停时重新验证结果。
...
### [16] [Resourced Authority A Mechanism-Design Model for Participatory Governance of Deployed AI Agents](http://arxiv.org/abs/2608.06353v1)
*Praphul Chandra,Sujit Gujar,Ganesh Ghalme*

Main category: cs.GT

TL;DR: 提出一种形式化机制设计模型，通过计算预算分配实现已部署AI代理的持续参与式治理。


<details>
  <summary>Details</summary>
Motivation: 旨在解决已部署AI代理的安全治理问题，确立“算力是有效治理杠杆”的安全AI范式，使授权过程能够自我执行。

Method: 将治理周期建模为扩展型博弈，利益相关者使用独立于AI算力的治理货币投票。系统通过资金聚合器将贡献转换为广度加权有效支持，经双阈值迟滞门转换为二元授权，并在外生安全上限约束下释放以硬件签名算力许可证形式实现的计量预算。

Result: 刻画了该机制能够治理的代理类别，并指出被治理代理对治理选民的操纵是核心未决问题。

Conclusion: 该机制通过算力分配实现了AI代理的自我执行治理，未来的主要挑战是解决被治理代理对治理选民的操纵问题。
...
### [17] [Fair and Efficient Balanced Allocations for Additive Valuations](http://arxiv.org/abs/2608.06325v1)
*Benjamin Cookson,Nisarg Shah,Paritosh Verma*

Main category: cs.GT

TL;DR: 本文证明了在平衡性约束下，任意加性估值均存在满足EF1和fPO的不可分物品分配，并将此方法扩展至类别约束。


<details>
  <summary>Details</summary>
Motivation: 解决在平衡性约束下不可分物品分配中，如何同时实现公平性（EF1）和有效性（fPO）的问题，并突破以往研究仅在特殊估值条件下成立的局限。

Method: 将KKM引理应用于加权福利对偶框架，并提出一种新的价格交错引理，随后将此技术扩展至类别约束（划分拟阵约束）。

Result: 证明了对于任意加性估值存在满足EF1和fPO的平衡分配；在类别约束下，证明了存在满足弱类别敏感EF1放松条件的fPO分配。

Conclusion: 将现有结果推广至任意加性估值，并扩展到类别约束场景，同时展示了大语言模型在复杂数学定理证明中的辅助作用。
...
### [18] [CalibForge: Adversarial Solver Calibration for Scaling Learnable Terminal Tasks](http://arxiv.org/abs/2608.06352v1)
*Fanzhe Meng,Guoxin Chen,Jiale Zhao,Shuang Sun,Zhiyu Lin,Wayne Xin Zhao,Ruihua Song,Ji-Rong Wen,Kai Jia*

Main category: cs.LG

TL;DR: CalibForge是一个自主的终端任务合成系统，它利用对抗性求解器校准来生成具有适当挑战性的任务，以训练终端智能体。


<details>
  <summary>Details</summary>
Motivation: 本文旨在解决训练终端智能体时，现有可执行验证仅能确认任务可行性，但无法揭示任务相对于特定求解器设置是否具有适当学习挑战性的问题。

Method: 提出了CalibForge系统，通过对抗性求解器校准（包括针对异构求解器池分歧的多求解器校准，以及针对强通过/弱失败关系的对比求解器校准）来修改候选任务，从而在已证明可解的基础上构建求解器相对的可学习区域。

Result: 构建了5,431个校准终端任务。消融实验表明，两种校准策略均比单独编写验证或普通单求解器反馈提供更有效的监督。在完整数据集上训练的模型在Terminal-Bench 2.0上达到32.58%和47.57%，在Terminal-Bench 2.0、SWE-bench Pro和Doc2Repo上较基础模型最大提升分别达24.71、27.68和30.04个百分点。

Conclusion: 研究结果支持将求解器相对的可学习性作为构建有效且可迁移的智能体训练数据的实际目标。
...
TL;DR: 本文提出ERILS方法，通过整合外部更强策略的rollouts来改进扩散大语言模型的强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 当成功的on-policy rollouts稀缺时，dLLM的强化学习训练获得很少正奖励，进展有限。直接引入外部rollouts面临长度差异和奖励处理不稳定的挑战。

Method: 提出ERILS（External Rollout Integration with Length Control and Source-Specific Processing），控制外部rollout长度，并对on-policy和外部rollouts的奖励分别处理，避免联合处理导致的训练崩溃。

Result: 在Sudoku、Countdown和MATH500的零样本评估中均提升多样本性能，Sudoku提升最大。Sudoku上达到98.4%的best-of-4完成准确率，而最强基线仅40.3%。在不同生成长度（128/256/512 tokens）下保持约90%的确定性单次完成准确率。

Conclusion: rollout构建和奖励处理是整合外部rollouts到dLLM强化学习中的重要设计维度；长度控制的外部rollouts比无控制的更有效，源特定的奖励处理避免了训练崩溃。
...
TL;DR: 本文提出了连续时间、离散状态空间的强化学习框架，基于随机控制方法推导出连续时间版PPO和GRPO，并应用于基于分数的离散扩散模型微调及掩码扩散大语言模型的RL后训练。


<details>
  <summary>Details</summary>
Motivation: 现有基于GRPO的离散扩散模型微调方法仅依赖终端奖励，无法在去噪轨迹中间引入奖励信号；同时掩码扩散大语言模型中轨迹似然的计算成本过高，需要一种统一的连续时间RL框架来克服这些限制。

Method: 将状态动力学建模为受控连续时间马尔可夫链(CTMC)，推导策略梯度方法得到连续时间版PPO和GRPO；在掩码扩散模型上提出词汇表单纯形上解析可处理概率比的策略参数化；针对dLLM提出轨迹子采样技术以高效估计计算上不可行的轨迹似然，降低逐位置概率比的计算成本。

Result: 在低维熵正则化优化问题上验证了方法有效性，并在数学推理和编码任务的dLLM强化学习后训练中展示了方法的实用性；框架允许在整个去噪轨迹中引入中间奖励或优势信号，且无需奖励信号的可微性。

Conclusion: 该连续时间RL框架为离散扩散模型提供了统一的探索与策略优化视角，特别是在掩码扩散大语言模型上实现了高效的奖励驱动优化；轨迹子采样技术使该方法在实际大规模模型中具备可行性。
...
### [21] [MMAligner: Safeguarding Multimodal Large Language Models through Representation Calibration](http://arxiv.org/abs/2608.05909v1)
*Shenyi Zhang,Keyan Guo,Zihao Wang,Xuebin Li,Lingchen Zhao,Hongxin Hu,Chao Shen,Qian Wang*

Main category: cs.CR

TL;DR: 本文提出MMAligner方法，通过几何分析发现多模态大语言模型的安全降级源于表征错配，并将不安全的多模态表征校准至已有的拒绝区域内以恢复安全性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）虽能拒绝不安全文本提示，却对语义等价的多模态输入生成有害响应。现有防御方法要么依赖外部护栏增加推理开销且无法修复内在缺陷，要么安全微调可能牺牲实用性或需要大量多模态数据。本文旨在找出这种安全差异的根本原因并加以解决。

Method: 作者从几何角度分析MLLM的表征，发现从文本学习到的安全机制（共享安全子空间和拒绝边界）在跨模态间持续有效，但不安全多模态输入会经历表征偏移，使其落在拒绝边界之外。基于此发现，提出MMAligner，通过硬下界确保拒绝、软上界避免过度修改、以及对良性输入的保持目标，将不安全多模态表征校准至已有的拒绝区域内。

Result: 在多个开源MLLM上的实验表明，MMAligner将不安全多模态输入的平均拒绝率提升至99%，实用性下降不到2%，且仅需极少量训练数据，在安全性与实用性的权衡上显著优于现有基线方法。

Conclusion: 多模态安全降级源于表征错配而非安全能力的缺失。MMAligner通过利用模型已有的安全子空间，以最小数据和极低实用性代价有效修复了多模态输入的安全漏洞，为MLLM的内在安全对齐提供了新范式。
...
### [22] [PromptShield Home: Ambient Multimodal Prompt Injection Defense for Smart-Home Agents](http://arxiv.org/abs/2608.05495v1)
*He Zhang,Feilong Li,Dingning Long,Yilin Cui,Peijun Zhang,Yuewen Zhang,Qianyao Xu,Xinyi Fu*

Main category: cs.CR

TL;DR: 该论文提出PromptShield-Home基准，评估智能家居中多模态大语言模型代理区分真实指令与环境噪音的能力，发现传统检测器与MLLM代理表现互补，建议采用学习路由与传感器融合提升安全性。


<details>
  <summary>Details</summary>
Motivation: 旨在解决智能家居助手在使用多模态大语言模型时，难以区分真实用户指令与看似指令的环境噪音（如电视语音、屏幕文本、旁人对话）的安全隐患，以防止误执行。

Method: 提出PromptShield-Home基准，构建了包含多种智能家居场景的测试集，对比评估了传统检测器(L0)、单MLLM代理(L1)和多智能体调解(L2)三种抽象层的不安全执行率与安全完成率。

Result: 实验表明传统检测器倾向于执行所有指令，而MLLM代理则过度拒绝真实指令；两者的正确集合互不相交，最佳单一层准确率为76.5%，而理论路由上限可达94.1%。

Conclusion: 研究结论指出不应直接用MLLM替代传统检测器，智能家居代理的安全性应通过实现学习路由与传感器融合机制来保障。
...
### [23] [OneEmo: A Unified Multimodal Reasoning Model for Emotion Perception, Understanding, and Interaction](http://arxiv.org/abs/2608.06013v1)
*Jiahao Huang,Zheng Lian,Jingyi Zhang,Zhide Chen,Xiaojiang Peng,Shaonan Wang*

Main category: cs.HC

TL;DR: 本文提出统一情感大模型OneEmo，通过构建综合数据集与新型强化学习策略，在多模态情感感知、理解与交互任务上取得突破。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在情感智能研究中过度关注单一任务，忽略了任务间的协同效应，且未充分探索其潜在的推理能力。

Method: 构建EmoWorld-130K数据集并通过人在环工作流提取显式推理轨迹进行监督微调以实现多任务互惠；提出Emo-Chord强化学习策略，利用统一的多任务奖励分配来稳定优化并释放潜在推理潜力。

Result: OneEmo在大多数基准测试中超越了同等规模的基线模型，达到SOTA性能，且在参数量远小于商业模型的情况下展现出极具竞争力的表现。

Conclusion: 该研究证明了多任务学习在情感计算中的协同效益，为构建更可靠、可解释的情感计算系统铺平了道路。
...
### [24] [M$^3$Prune: Hierarchical Collaborative Pruning for Efficient Multi-Modal Multi-Agent Retrieval-Augmented Generation](http://arxiv.org/abs/2608.05967v1)
*Taolin Zhang,Weizi shao,Zijie Zhou,Chen Chen,Daiyang Yu,Tingyuan Hu,Chengyu Wang,Xiaofeng He*

Main category: cs.MM

TL;DR: 本文提出M3Prune框架，通过层级化通信图剪枝消除多模态多智能体RAG系统中的冗余通信边，在保持任务性能的同时显著提升token效率。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体多模态RAG系统虽然通过多智能体协作取得了优异性能，但存在大量token开销和计算成本过高的问题，难以大规模部署。

Method: 提出层级化通信图剪枝框架M3Prune：首先在文本和视觉模态内进行模态内图稀疏化以识别任务关键的通信链路；然后构建模态间通信图，通过模态对齐评分鼓励跨模态一致推理并稀疏化跨模态连接；最后渐进式剪枝冗余边以获得高效的层级拓扑结构。

Result: 在通用领域和特定领域的mRAG基准上的广泛实验表明，M3Prune在性能上持续优于单智能体和强多智能体mRAG系统，同时显著提升了token效率。

Conclusion: 通过层级化通信图剪枝可以有效消除多模态多智能体系统中模态内和模态间的冗余通信，在任务性能与token开销之间取得更优平衡，为大规模部署提供了可行方案。
...
### [25] [Tracing the Heart: An Evidence-Linked Pipeline for Heart-Failure Feature Engineering](http://arxiv.org/abs/2608.06366v1)
...
TL;DR: 本文开发了Nimblemind多智能体系统，通过自动化、可审计的流程解决电子健康记录中心力衰竭特征工程的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录特征工程耗时且现有方法缺乏可维护性和证据可追溯性，特别是在需要整合碎片化数据与临床推理的心力衰竭领域，亟需自动化解决方案。

Method: 提出Nimblemind多智能体系统，这是一种基于评分标准和证据链接的自动化心衰特征工程流程，利用受限LLM对500份虚拟病历生成并审计特征的结构完整性和溯源性。

Result: 生成了132个结构化特征和70个聚合特征，显著提升了HFrEF和HFpEF表型预测的AUROC（分别提升至0.963和0.910），且独立LLM评估得分达到最高分的81.5%。

Conclusion: 验证了为复杂心血管EHR数据进行自动化、可审计特征工程的可行性，但未来需在真实多中心队列中进行外部验证。
...
### [26] [TRAJDEBUG: Tracing Error Lifecycle to Identify Critical Failures in Long-Horizon Agent Trajectories](http://arxiv.org/abs/2608.06346v1)
...
TL;DR: 本文提出TrajDebug框架以解决大语言模型智能体长轨迹中的关键错误定位问题，并构建了TrajErrBench基准。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的智能体系统在长轨迹中易出现级联错误且难以调试，主要面临长轨迹中错误证据分散以及多个局部错误难以归因最终失败两大挑战。

Method: 提出错误生命周期追踪框架TrajDebug，通过多粒度历史压缩和基于证据的错误识别来发现错误，并追踪错误的解决状态和终端影响以实现关键归因；同时构建了TrajErrBench基准数据集。

Result: 在多个智能体基准上的实验显示TrajDebug的整体性能优于现有基线，且其诊断能提供可操作反馈以提升下游智能体的成功率。

Conclusion: TrajDebug有效解决了长轨迹关键错误检测与归因问题，其诊断结果对改进智能体系统具有实际应用价值，相关代码与数据将开源。
...
### [27] [Beyond Top-K: Replacing Black-Box Retrieval with Interpretable Agentic Operations](http://arxiv.org/abs/2608.06305v1)
*Sagar Tamang,Ayush Vyas,Tabarakul Hazarika*

Main category: cs.AI

TL;DR: 本文提出了READ，一种基于模型上下文协议的无嵌入智能体文档搜索系统，通过确定性操作有效解决了长篇财务文档中传统分块检索设计导致的上下文丢失问题。


<details>
  <summary>Details</summary>
Motivation: 传统的“分块-嵌入-检索top-k”设计对于财务报表等复杂文档在结构上是不合理的，分块边界经常将数字与其单位或表头分离，导致数量级错误；即使是表感知的分块器也无法完全解决此问题。

Method: 提出READ（可靠的无嵌入智能体文档搜索），智能体通过模型上下文协议暴露的三种确定性操作（规范化词法搜索、结构导航和有界跨度读取）来读取原始文档，形成可重放的审计轨迹。

Result: 在51个验证问题上，READ的准确率为58.8%，远高于密集检索的15.7%（调优后为35.3%）。该提升源于接口设计而非迭代过程。同时发现BM25在统计上与READ表现相当。

Conclusion: READ成功克服了密集检索在复杂文档中的上下文分离缺陷，其核心优势在于无嵌入的检索接口而非智能体的迭代特性，证明了词法检索在特定场景下的潜力。
...
### [28] [LLaDA MoE v2: Scaling Mixture-of-Experts Diffusion Language Models](http://arxiv.org/abs/2608.03457v1)
...
TL;DR: 本文系统研究了混合专家扩散语言模型的扩展规律，并据此训练出性能接近Qwen3的LLaDA MoE v2模型。


<details>
  <summary>Details</summary>
Motivation: 旨在解决混合专家扩散语言模型的扩展行为尚不明确的问题，为其提供实用的扩展定律和架构设计原则。

Method: 系统分析了MoE dLLMs在优化超参数、计算分配和架构规模上的扩展趋势，并基于发现从头训练了拥有300亿总参数和30亿激活参数的LLaDA MoE v2。

Result: 发现了最优批次增长更快、学习率衰减更快等不同于自回归模型的趋势；LLaDA MoE v2仅用约65%于Qwen3的预训练token即在多项任务上接近其表现，且经SFT后在8项基准中7项优于SDAR Chat。

Conclusion: 确立了MoE扩散语言模型的实用扩展定律和设计原则，证明了其在多种自然语言处理任务中的巨大潜力。
...
### [29] [Escaping Confidence Trap: Evolutionary Decoding for Mathematical Reasoning in Diffusion LLMs](http://arxiv.org/abs/2608.00605v1)
*Zhenhong Sun,Hanqing Zhao,Yatao Bian,Rongcheng Tu,Liuyue Xie,Xu Zhang,Jue Wang,Davide Modolo,Daoyi Dong,Dacheng Tao*

Main category: cs.AI

TL;DR: 本文提出了一种无需训练的测试时扩展框架Evolutionary Decoding，通过进化式解码策略解决扩散大语言模型在数学推理中的置信度陷阱问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽然在通用任务上表现优异，但在数学推理中由于局部token置信度与全局推理正确性错位（置信度陷阱），导致推理不可靠，存在采样敏感型失败和采样一致型失败两种典型问题。

Method: 提出Evolutionary Decoding框架，将扩散解码视为候选推理状态的进化过程，结合逐步选择机制（保留有用数值符号信号、抑制重复模式）和块级变异机制（引入结构化替代方案以逃离错误高置信区域）。

Result: 在多个基准测试中，Evolutionary Decoding相比基于置信度的解码方法显著提升了LLaDA 2.0的数学推理可靠性。

Conclusion: 扩散大语言模型的解码轨迹存在置信度陷阱问题，通过进化式解码策略可在无需额外训练的情况下有效缓解该问题，提升数学推理的可靠性。
...
TL;DR: 本文提出了FlowBlock，一个无需训练的并行解码框架，通过门控波前解码和异构波前打包机制，在保持准确率的同时显著加速块级扩散语言模型（dLLMs）的推理。


<details>
  <summary>Details</summary>
Motivation: 块级扩散语言模型在块间解码时严格串行，导致推理速度受限；已有的后训练方法加速有限且会降低准确率，本文旨在无需训练地解锁块间并行性。

Method: 提出FlowBlock框架，包含两个机制：（1）门控波前解码，通过就绪门控制块进入有界波前，利用T2T自校正编辑协同优化活跃块，并在窗口化块因果掩码下按序提交块，保持精确的冻结前缀KV缓存复用；（2）异构波前打包，为每个请求分配独立波前，将异步窗口打包为密集且形状稳定的批量前向计算。

Result: 在多个基准上，FlowBlock相比LLaDA-2.1和LLaDA-2.0分别实现高达2.95×和4.01×的TPS提升，延迟分别降低53.6%和77.1%，平均准确率提升1.3分；相比基于训练的基线D2F，准确率更高且批量服务吞吐量提升高达16×。

Conclusion: FlowBlock通过将块最终性从硬依赖转化为调度资源，实现了训练-free的块间并行解码，在不牺牲准确率的前提下大幅提升了dLLMs的推理效率和服务吞吐量。
...
TL;DR: 提出了一种名为LaCache的免训练加速框架，通过无损缓存和混合精度量化缓解扩散大语言模型的算子级冗余，实现显著的端到端加速。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的大型语言模型在去噪步骤中会重新计算整个序列，忽略了块内不变的前缀和掩码后缀，导致严重的算子级冗余。本文旨在解决此冗余问题以提升文本生成效率。

Method: 提出LaCache框架，利用无损状态记忆（LSM）缓存嵌入输出、预注意力状态和FlashAttention中的在线softmax统计量，以跳过未更改token的冗余计算；同时针对FFN层引入基于组的FP8量化策略以缓解内存带宽瓶颈。

Result: 实验表明，LaCache单独使用时可实现约1.3倍的端到端加速；与现有加速方法结合时，最高可达40.2倍的端到端加速，且任务准确率保持相当。

Conclusion: LaCache通过无损缓存和量化有效消除了扩散大语言模型的算子级冗余，在不损失精度的前提下实现了巨大的推理加速，具有显著的实用价值。
...
TL;DR: 提出TAPS方法，通过目标感知的前缀选择优化扩散模型推测解码中的验证瓶颈，实现显著无损加速。


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型在推测解码中将瓶颈转移至验证阶段的问题，现有方法基于边际概率排序忽略了前缀条件，导致验证不可达节点而增加延迟。

Method: 提出TAPS（目标感知前缀选择），将扩散边缘概率转化为路径条件接受估计，并在固定验证预算下选择紧凑的前缀闭合子树。

Result: TAPS实现了高达7.9倍的无损端到端加速，分别比最先进的DFlash和DDTree快1.36倍和1.74倍。

Conclusion: 通过选择紧凑的子树而非盲目扩展草稿树，TAPS有效改善了接受率与验证成本的权衡。
...
### [33] [PaDoc: Layout-Grounded Parallel Decoding for Document Parsing](http://arxiv.org/abs/2608.06146v1)
*Hao Yu,Jiabo Zhan,Kang Liu,Linnan Zhao,Dongxu Yue,Rui Chen,Jinglin Wang,Chong Sun,Chen Li,Jing Lyu,Chun Yuan*

Main category: cs.AI

TL;DR: 本文提出了PaDoc，一种基于布局的文档解析器，通过将预测布局视为共享页面表示上的分支结构，在保留整页上下文的同时实现区域内容的并行解码，从而显著提升了端到端文档解析的速度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端文档解析器将页面布局和区域内容序列化为单个自回归序列，导致独立区域的解码路径过长且效率低下；而基于裁剪的两阶段解析器虽能实现区域级并行，但存在重复视觉预填充和页面上下文碎片化的问题。本文旨在保留全页上下文的同时消除区域间的解码依赖，以提升解析效率。

Method: 提出PaDoc解析器，利用前缀条件分解将布局流和区域内容分支并发推进；在单一多模态大语言模型（MLLM）中，通过打包变长祖先注意力在标准下一词训练中保持可见性，并利用掩码并行解码创建分支，结合vLLM后端的缓存驻留共享前缀重用机制进行高效服务。

Result: 在OmniDocBench Full数据集上，PaDoc取得了91.1的布局F1分数，在端到端解析器中总体得分达94.24，且文本编辑（0.038）和公式CDM（95.59）均为最佳；在单张A800 GPU上处理384页子集时，有效页面吞吐量提升了67.4-118%，P95延迟降低了39.2-54.9%，是五个并发级别下最快的端到端解析器。

Conclusion: PaDoc成功在保留全页上下文与并行解码之间取得平衡，实现了高精度与高效率的端到端文档解析，证明了其在处理复杂文档布局时的有效性和优越性。
...
### [34] [Unified Agent: Managing Interactions across Devices](http://arxiv.org/abs/2608.05729v1)
*Xinshuang Liu,Runfa Blark Li,Shaoxiu Wei,Xin Lin,Truong Nguyen*

Main category: cs.AI

TL;DR: 提出了一种名为Unified Agent的有状态代理，通过维护紧凑且可操作的跨设备与跨时间状态来显著提升AI代理性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有AI代理在跨设备和跨时间场景下因缺乏有效状态管理，而无法妥善处理分散观察数据的问题。

Method: 设计一种将交互证据、陈述事实和持续请求组织成紧凑且可操作形式的状态，并基于此原则构建了Unified Agent及相应的跨设备与时间交互基准测试。

Result: Unified Agent在默认设置下显著优于四种现有设计的改编版本，且在多模态大语言模型（MLLM）的家族、能力和推理力度发生变化时依然保持领先。

Conclusion: 良好的状态设计能为跨设备、跨时间的代理请求提供鲁棒的优势，该优势不随MLLM设置的变化而改变。
