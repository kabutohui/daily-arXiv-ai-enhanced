...
TL;DR: 本文提出了基于离散扩散大语言模型的DRRG框架，通过迭代去噪实现放射学报告的生成与双向优化。


<details>
  <summary>Details</summary>
Motivation: 现有自回归方法逐token生成报告，无法修正先前内容，易导致错误传播，且与放射科医生迭代修改报告的实际流程不一致；离散扩散模型具备迭代去噪和文本修正能力，但在放射报告生成中尚未被充分研究。

Method: DRRG将报告生成建模为迭代掩码token去噪过程，引入临床实体感知互补掩码以提升token监督覆盖率并强调重要临床实体，同时设计概念条件模块将图像衍生的临床概念注入视觉表示。

Result: 在MIMIC-CXR上，DRRG在多数指标上优于对比方法（BLEU-4为0.210，CheXpert-F1为0.549，RadGraph-F1为0.281，GREEN为0.360，RaTEScore为0.604），且使用了更小的LLM解码器；在CheXpert Plus上取得最高BLEU-4（0.119）和CheXpert-F1（0.347）。

Conclusion: 离散扩散为自回归放射报告生成提供了有效替代方案，通过迭代双向优化提升报告质量；结合临床导向的掩码策略和图像衍生概念条件化可进一步增强报告的临床一致性。
...
TL;DR: 本文提出LatentStream框架，将流式视频理解中的记忆机制从存储-检索模式转变为检索-内化模式，通过层次化潜在记忆实现持续的流式推理。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频理解方法采用存储-检索范式，将历史证据作为外部视觉上下文保留，无法将其内化为紧凑、可演化的潜在记忆来持续引导流式推理，本文旨在解决这一局限。

Method: LatentStream包含三个协同组件：(1)查询无关的层次化流式记忆，通过Jenks引导的自适应整合将视觉历史组织为短、中、长期三级，在固定记忆预算下运行；(2)层次化潜在记忆演化，为潜在记忆token组配备渐进扩展的记忆感受野，迭代检索并内化历史证据为紧凑固定长度的潜在记忆；(3)渐进置信度引导的潜在记忆优化，从组级预测熵构建层次化递进奖励，联合优化潜在记忆token和检索证据。

Result: 在现有在线和离线视频基准测试上，LatentStream取得了新的最优结果。

Conclusion: LatentStream成功将历史证据内化为紧凑的潜在记忆，实现了持续的流式推理引导，为流式视频理解中的记忆机制提供了新的范式。
...
TL;DR: 本文提出CORE方法，通过将重排序器的组合判断能力蒸馏到嵌入模型中，有效提升了多模态大语言模型在组合检索任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 基于MLLM的嵌入模型在组合检索中难以区分具有相同概念但不同属性-对象绑定的场景，而相同的骨干网络作为交叉注意力重排序器却能解决此问题，因此需要将其组合判断能力蒸馏到嵌入模型中。

Method: 提出CORE方法，合成涵盖五个组合匹配级别的候选列表，并引入Rank-KL目标函数训练嵌入模型以重现重排序器的细粒度排名；同时引入分级评估协议，并对比了对比学习、成对CoSENT和列表式Rank-KL。

Result: CoSENT和Rank-KL比对比学习更有效利用多级监督，且Rank-KL表现最佳；CORE-RERANKER-8B在三个基准上总平均达82.7%（超Jina-Reranker 10.7点），CORE-EMBED-8B获嵌入模型最佳总平均值（0.666）；改进可迁移至MCMR且不牺牲在COCO和Flickr30K上的性能。

Conclusion: 通过列表式Rank-KL将重排序器的组合判断能力蒸馏到嵌入模型中，能显著提升组合检索性能并保持通用检索能力。
...
TL;DR: 提出了一种无需训练的框架GraFT，通过3D场景图增强多模态大语言模型的3D空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在3D空间推理（如几何测量、视角转换、外观定位）方面不可靠，且现有解决方案通常依赖于昂贵的监督或特定主干。

Method: 引入无训练框架GraFT，通过紧凑的3D场景图（3DSG）提供缺失的3D结构，具体包括：使用符号工具提供确定性几何、通过鸟瞰图（BEV）渲染提供离心布局、通过任务相关的自我中心帧进行视觉属性定位。

Result: 在ScanQA基准上，所有指标均优于相同主干基线，CIDEr提高27%；在VSI-Bench基准上，将冻结的MLLMs提升高达65%，超越了所有专有和通用开源基线及多个著名的微调空间模型。

Conclusion: GraFT无需昂贵监督或训练即可有效提升MLLMs的3D空间推理能力，且不与特定主干耦合，具有广泛适用性。
...
TL;DR: 本文提出一种基于InfoNCE的内在奖励机制，使多模态大模型无需预热监督微调即可学会使用放大工具，并在多个基准测试中表现优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型（MLLMs）在处理高分辨率图像任务时，学习使用放大（zoom-in）工具通常需要大量预热监督微调（SFT）阶段的问题。

Method: 提出一种基于InfoNCE风格的内在奖励机制，利用难度递增的难负样本工具调用作为对比训练信号，无需额外标签或预热SFT；同时引入了合成的Muffin&Chihuahua (M&C)数据集来直接测量模型的放大能力。

Result: 在V*、HRBench和MME-RealWorld上的实验表明，该方法在更高效的同时具有竞争力；作为SFT的直接替代品，它优于所有基线模型。利用M&C数据集发现，召回率是放大区域与最终任务性能相关性最强的指标。

Conclusion: 该方法成功消除了预热监督微调的必要性，并通过直接测量放大能力证明了其有效性。未来研究可利用该方法和M&C数据集进一步探索和提升多模态大模型的工具使用能力。
...
TL;DR: 本文提出了ShallowStream框架，利用多模态大语言模型的浅层网络进行帧编码和检索索引构建，以大幅降低流媒体视频理解的计算开销和延迟。


<details>
  <summary>Details</summary>
Motivation: 使用多模态大语言模型处理连续视频流计算成本高昂，且现有方法大多忽略了模型深度维度，重复执行全深度预填充会导致巨大的计算开销并使KV缓存快速增长。

Method: 提出ShallowStream框架，利用MLLM的浅层同时进行帧编码和检索索引构建，在流处理期间使用浅层KV缓存维护轻量级索引；在查询时，利用浅层生成的注意力分数对上下文帧进行评分，并采用多样性感知选择策略检索证据。

Result: ShallowStream实现了与现有最强流处理方法相当的性能，同时将每帧预填充延迟和10秒端到端延迟分别最多降低了52.1倍和11.9倍。

Conclusion: 通过利用模型浅层进行编码和索引，ShallowStream在不牺牲性能的前提下有效解决了流媒体视频理解的计算开销和延迟问题。
...
TL;DR: 本文提出了TIC-Bench基准，用于评估多模态模型在深度交错的图文上下文中整合线索与恢复事实的能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型的评估多聚焦于多图像任务，文本仅作为指令，忽略了文本与图像深度交互的场景。然而现实应用（如图文共创、空间重建）亟需模型对深度交织的图文上下文具备深刻理解，因此需要填补该领域的评估空白。

Method: 提出了全新的基准测试TIC-Bench，涵盖逻辑、时间和空间关联三个核心领域，细分为8种特定类型，共包含2280个问题，旨在评估模型在深度交织的图文语境中整合证据和恢复事实的能力。

Result: 对10个最先进的多模态大语言模型（MLLM）的评估显示，模型在整合分布于交织视觉和文本输入中的证据时存在持续困难，且其表现与人类专家之间存在显著的性能差距。

Conclusion: TIC-Bench为评估和推动多模态模型在深度交织上下文中有效整合图文信息提供了有价值的分析工具，该基准数据集已开源。
...
TL;DR: 该研究比较大语言模型与人类对面部吸引力的评分，发现模型能准确跟踪排序但系统性地高估了人类面孔的美貌。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型在美貌评估中越来越受欢迎，本研究旨在探讨这些AI模型能否准确反映人类对吸引力的判断。

Method: 采用预注册探索性研究，将2513名人类参与者的吸引力评分与四种广泛使用的商业AI模型（Claude、Gemini、GPT和Grok）进行比较。

Result: MLLMs系统性地对面孔给予更高评价且评分范围比人类更窄，未能在绝对数值上重现人类评分；但与人类判断呈强相关性，能准确跟踪面孔的排序。在预测线索上，仅面部年龄是两者共同的预测因子。各AI模型间高度一致，但Grok除外，且Grok与人类的一致性最低。

Conclusion: 当前的商业多模态大语言模型虽能近似人类吸引力的排序，但系统性地高估了人类面孔的美貌。
...
TL;DR: 本文分析了扩散式大语言模型的安全行为，并提出了一种无需训练的解码方法RAEC，以在保持实用性的同时降低攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型的生成范式引入了影响安全对齐的两个维度（去噪过程中的生成时机和位置）。本文旨在探究这些因素如何在有害提示下影响dLLM的安全行为。

Method: 作者追踪了去噪过程中的中间令牌分布和提交决策。基于拒绝信号集中在早期去噪步骤和响应前部位置的发现，提出了一种无需训练的解码方法——拒绝感知早期提交（RAEC），该方法在早期步骤提交持续的拒绝信号。

Result: 在LLaDA和Dream上的实验表明，RAEC降低了攻击成功率，同时基本保持了实用性。

Conclusion: 去噪步骤和拒绝令牌提交的持续性对理解dLLM安全至关重要。RAEC提供了一种有效且无需训练的方法来提升dLLM的安全对齐。
...
TL;DR: 本文提出了一种轨迹级推测解码框架，通过置信度分层树探索和块间推测，显著提升了基于扩散的语言模型的生成吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的语言模型在低置信度下会退化为单token生成，严重限制了吞吐量，而dLLMs的推测解码需要对去噪轨迹（多token更新序列）进行推测，这与自回归模型的固定从左到右顺序的token序列推测不同。

Method: 提出了一个轨迹级推测框架，通过置信度分层的树探索构建草稿去噪轨迹，并通过具有双向注意力掩码的块状并行评估进行验证；进一步引入了块间推测，利用扩散模型的双向结构进行跨块前瞻；基于Fast-dLLM的双缓存基础设施实现。

Result: 该方法将去噪迭代次数减少了30-40%，每步token数从2.6增加到4.3，相比原始dLLMs实现了7-14倍加速，相比Fast-dLLM实现了1.3倍加速，在推理和代码基准测试中准确率变化不到1%。

Conclusion: 该框架形式化地刻画了该方法精确的条件，并识别了轨迹漂移作为增加并行性的基本成本；该方法有效提升了dLLMs的生成效率，同时几乎不损失准确性。
...
TL;DR: 本文提出了无训练框架DARD，通过依赖感知的可撤销解码机制解决扩散大语言模型并行解码中的上下文污染与质量下降问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在增加解码并行度时常导致生成质量下降。现有的可撤销解码方法虽能重新评估并遮蔽不可靠token，但忽略了不可靠token同样会破坏验证上下文这一失败模式。

Method: 提出依赖感知可撤销解码（DARD）这一无训练框架。该框架将token分离为掩码、候选和未掩码状态，利用排除不可靠token的选择性上下文验证候选token，并自适应调节这些token对后续解码的影响。

Result: 在12个文本和多模态基准及3个开源dLLM上的实验表明，DARD持续改善了速度-质量的帕累托前沿；在Flickr30K上相比Saber实现了2.71倍的加速和4.35分的CIDEr提升。

Conclusion: DARD有效解决了扩散语言模型解码中的上下文污染问题，提供了一种无需训练即可显著提升解码速度与生成质量的有效途径。
...
TL;DR: 本文提出一种无需训练的长度选择器Entropy-Valley (EV)，通过预测熵优化掩码扩散语言模型在机器翻译中的目标长度决策。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型在机器翻译中需预先决定目标长度，直接影响覆盖率和冗余度，但现有研究多关注去掩码顺序而忽略了长度决策问题。

Method: 引入Entropy-Valley (EV)，一种无需训练的长度选择器，通过全掩码前向传递计算候选目标画布的平均预测熵，并选择模型最易填充的画布长度。

Result: 相比基于语料统计的基线，EV在英译中、中译英和英译德上分别恢复了64.9%、65.3%和33.0%的参考目标长度带来的COMET-22增益。专家评估支持了英中双向的充分性提升；与同数据微调的LLaMA-3-8B自回归模型相比，EV在英译中上打平，在中译英上领先。

Conclusion: 对去噪友好的长度无需匹配参考长度；在掩码扩散机器翻译设定中，目标长度的供应方式比决定先揭示哪些词元更重要。
...
TL;DR: 本文提出了一种无需训练的解码方法RPS，通过利用扩散大语言模型解码过程中的涟漪效应来加速并行解码并提升准确率。


<details>
  <summary>Details</summary>
Motivation: 现有dLLM的并行解码调度器在提交位置时过于保守，忽略了提前提交某些位置可能对降低后续解码不确定性带来的好处。

Method: 提出Ripple-Pivot Search (RPS)，该方法寻找中等熵的位置作为枢轴进行提前提交，并通过前瞻评估选择能带来最大下游效益的令牌分配。

Result: 在多个dLLM和基准测试中，RPS比标准解码器实现4-10倍加速（结合KV缓存可达18倍），在保持质量的同时，准确率比前瞻基线最高提升5.49%，且吞吐量更高。

Conclusion: 通过主动选择和提交枢轴位置，RPS成功利用涟漪效应优化了dLLM解码，实现了显著的推理加速和准确率提升，为高效LLM推理提供了新思路。
...
TL;DR: 本文提出了DominoTree，一种无需训练的树状推测解码方法，通过在候选路径上应用条件修正，在多个基准测试和推理服务框架中显著提升了接受长度和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的树状推测解码方法（如DDTree）无法表示像Domino这类带有GRU因果修正的草稿器所生成的路径依赖分布，限制了推测解码的效率。

Method: 提出DominoTree，一种无需训练的最佳优先草稿树，通过沿每条根到节点的路径计算Domino的条件（非分解）修正来对树进行评分，并将每个节点的修正限制在候选的top-M集合内以提高实用性。

Result: 在所有服务单元中获得了最高的平均接受长度，且在研究环境中每个温度下均获得最高总体接受长度（24个数据集单元中的21个）；在Qwen3-8B上吞吐量最高比AR快7.3倍，在SGLang中单请求吞吐量比Domino高12%；在HELMET长上下文基准中，接受长度比Domino高29-36%，吞吐量高10-34%。

Conclusion: 将路径依赖的条件修正应用于候选树节点能显著提升推测解码性能，尤其在长上下文场景中优势明显，但在预填充占主导的超长上下文下其相对优势会减小至持平。
...
TL;DR: 本文提出了GRAFT框架，用于优化基于扩散语言模型（DLM）的树状推测解码，通过目标蒸馏边缘评分和状态感知预算分配提升解码效率。


<details>
  <summary>Details</summary>
Motivation: 现有的树构建方法不兼容单次前向传递生成所有未来位置分布的DLM草稿器，且基于固定预算和单一概率的方法忽略了父子兼容性及解码状态对最优树大小的影响。

Method: GRAFT框架包含两项核心技术：一是目标蒸馏边缘评分（TDES），利用目标模型轨迹蒸馏父子偏好以选择兼容边缘；二是状态感知预算分配（SABA），通过平衡预期草稿增益与验证成本来动态设定每轮的树预算。

Result: 在多个模型和任务上，GRAFT相比自回归解码实现了2.13倍至6.36倍的端到端加速，且每轮增加的开销不到0.5毫秒，仅占目标模型验证延迟的约1.4%。

Conclusion: GRAFT有效解决了DLM草稿器在树状推测解码中的适配问题，在极低开销下显著提升了推理加速比，具有广泛的模型和任务适用性。
...
TL;DR: 提出了CaDDTree，一种成本感知的扩散草稿树方法，通过联合优化树结构和节点预算直接最大化令牌吞吐量，无需离线预算搜索。


<details>
  <summary>Details</summary>
Motivation: 现有的DDTree方法在固定预算下最大化期望接受长度，但由于接受长度随预算非递减，总是偏向更大的树而忽略验证成本，缺乏选择预算的原则性基础。

Method: CaDDTree显式建模草稿和验证延迟，将吞吐量目标分解为对节点预算的每轮一维搜索；在凸验证成本下证明吞吐量函数是单峰的，从而实现高效的贪婪停止规则，并能在每轮根据当前位置分布和验证成本自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型上的八个基准测试（涵盖推理、编码和指令遵循任务）表明，CaDDTree在几乎所有任务上均匹配或优于具有oracle预算选择的DDTree。

Conclusion: CaDDTree通过显式考虑验证成本并自适应调整预算，有效解决了DDTree预算选择缺乏原则性基础的问题，实现了更高的推理吞吐量，且无需离线预算搜索。
...
TL;DR: 提出DDTree方法，通过块扩散起草器的位置分布构建草稿树，在单次前向传播中高效验证，从而提升推测解码的接受长度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash块扩散起草器每轮仅验证单条草稿轨迹，这限制了草稿的接受长度，未能充分利用并行验证的潜力。

Method: DDTree直接利用块扩散起草器的逐位置分布构建草稿树；在固定节点预算下，采用最佳优先堆算法根据草稿模型输出的代理指标选择最可能匹配目标模型的续写；随后使用仅祖先注意力掩码在单次目标模型前向传播中完成高效验证。

Result: 基于DFlash构建的DDTree取得了显著的性能提升，使其成为推测解码领域最领先的方法之一。

Conclusion: 通过将块扩散起草与基于树的并行验证机制相结合，DDTree有效突破了单轨迹验证的局限，为自回归语言模型的推测解码提供了更优的加速方案。
...
### [18] [SENTINEL-RL: Offloading Topological Reasoning from LLM Agents in the Security Operations Center](http://arxiv.org/abs/2609.04159v1)
*Uday Vallabhaneni,Cassie L. Cagwin,David J. Wild*

Main category: cs.CR

TL;DR: 本文提出Sentinel-RL，一种将拓扑推理与语义推理解耦的代理式SOC架构，通过图注意力编码器和PPO策略实现大规模认证图上的可靠威胁检测与处置。


<details>
  <summary>Details</summary>
Motivation: 大语言模型代理作为自主SOC分析师存在两个关键局限：有限上下文窗口无法容纳数千主机的认证图，且自由生成无法保证推荐的遏制动作与拓扑结构一致。

Method: Sentinel-RL采用异构图注意力编码器将实时认证子图摘要为固定维度状态，PPO策略将状态映射到受约束的调查动作集，LLM代理回路仅消费策略推荐并由评论者门控生成分析师可读叙述，实现拓扑推理与语义推理的解耦。

Result: CREATE摄取模式在单节点上14.2分钟加载24M边子图（较MERGE快24倍）；滑动窗口告警引擎在50次试验中2.5秒内触发阈值；PPO训练200轮次后平均回报8.74±0.31，红队事件精确度0.91、召回率0.87；集成遏制循环中位完成时间6.3秒。

Conclusion: 系统贡献了热节点死锁工程模式和锚节点共定位HPC部署模式，并提供了涵盖误报经济学、可逆性保证、审计合规及人机审批边界的企业就绪性分析，为LLM代理在企业级SOC中的可靠部署提供了可复用范式。
...
### [19] [PatchBench: Evaluating AI Agents for Vulnerability Patching](http://arxiv.org/abs/2609.04075v1)
*Chihao Shen,Jiacheng Li,Aastha Mahajan,Jeffery Siyuan Tian,Yonghwi Kwon,Yizheng Chen*

Main category: cs.CR

TL;DR: 本文提出PatchBench基准，通过漏洞移植、代码变异和新的补丁验证方法，揭示现有AI代理在漏洞修补评估中存在补丁记忆化和表面修复两大威胁。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理漏洞修补评估仅通过PoC是否仍触发崩溃来验证补丁，存在两个关键威胁：代理可能复现记忆的历史开发者补丁，或生成仅抑制崩溃的表面修复而非修复根因。

Method: 提出PatchBench基准，选择真实修复位于崩溃堆栈之外的漏洞，并通过漏洞移植和代码变异将历史漏洞迁移至新代码库上下文中，以降低表面修复和补丁记忆化风险；同时开发新的补丁验证方法，全面评估代理补丁的安全性和语义正确性。

Result: 平均25%的代理补丁与历史开发者补丁高度相似，证实补丁记忆化是真实威胁；代理频繁利用基准结构在崩溃堆栈上进行表面修复；在11个最先进代理（包括AIxCC前三名）上，原始仅PoC验证平均将代理解决率夸大1.83倍。

Conclusion: 当前漏洞修补代理存在关键局限性，包括补丁记忆化和表面修复问题，未来研究应致力于开发更可靠的漏洞修复方法。
...
### [20] [Formation Matrix and Energy-based Control of Multi-Agent Systems](http://arxiv.org/abs/2609.04158v1)
*Martín Crespo,Sergio Junco,Matías Nacusse*

Main category: eess.SY

TL;DR: 本文提出了一种基于能量的多智能体机器人编队控制器，通过模拟弹簧-阻尼网络实现编队控制与防碰撞。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体机器人在平面上移动时，如何实现并保持特定编队同时避免智能体间碰撞的问题。

Method: 提出一种模拟弹簧-阻尼模块网络的能量控制器，利用键合图模型描述系统动力学，并通过图论方法引入编队矩阵，将其与IDA-PBC理论中的互连控制（CbI）方案相对应，在端口哈密顿系统框架内求解，并进行了闭环稳定性分析。

Result: 通过多种场景下的数值仿真验证了理论发现。

Conclusion: 所提出的基于互连控制方案的能量控制器能有效解决编队控制与防碰撞问题，并保证闭环系统的稳定性。
...
### [21] [A Case Study on Emergent Cheating and Whistleblowing in Autonomous Research Swarms](http://arxiv.org/abs/2609.04170v1)
*Davide Paglieri,Logan Cross,Tim Genewein,Joel Z. Leibo,Nenad Tomasev,Alexander Sasha Vezhnevets*

Main category: cs.AI

TL;DR: 本文研究了多智能体LLM生态系统中作弊与举报行为自发涌现的现象，并提出采用知识公地治理机制来管理共享基础设施以防御漏洞利用。


<details>
  <summary>Details</summary>
Motivation: 多智能体AI生态系统的共享基础设施虽然促进了协作，但也引入了不良行为传染性传播的脆弱性。本文旨在解决如何管理这种共享基础设施以防止系统漏洞被利用的问题。

Method: 通过对100个自主LLM智能体集群证明数学猜想的案例研究，将共享基础设施管理构建为知识公地治理问题，提出采用分级制裁和集体选择规则等制度机制来支持去中心化的自治。

Result: 实验中，单个智能体发现的评估系统漏洞通过共享知识库和点对点消息在集群中传播，部分智能体因竞争压力采用作弊；同时另一组智能体自发产生反制，包括审计欺诈、发警报、抵制、投诉及提议修复补丁。

Conclusion: 透明通信渠道既传播了漏洞，也使非作弊智能体具备了检测欺诈和组织反制的能力；采用知识公地治理机制（如分级制裁和集体选择规则）是实现自主集群去中心化自治的关键未来方向。
...
### [22] [Terminal-Universe: Turning Agent Trajectories into Scalable Terminal Environments](http://arxiv.org/abs/2609.04148v1)
...
TL;DR: 提出Terminal-Universe框架，从终端代码智能体的历史轨迹中重建可执行环境，并合成新任务以支持模型训练。


<details>
  <summary>Details</summary>
Motivation: 解决终端代码智能体后训练中可执行环境稀缺的问题，因为环境能提供可验证任务和执行反馈，而单一轨迹仅是冻结的演示。

Method: 重放轨迹中的文件操作恢复初始部分工作区，由补全智能体补充缺失文件与依赖；在恢复的工作区上重建原任务并合成新任务，并通过跨工作区查询（广度）和用户代理的多轮交互（深度）扩展任务规模。

Result: 生成了3.73万个任务完备环境；使用该数据对Qwen3.5-27B进行监督微调，在Terminal-Bench 2.1上单轮性能提升11.9分，在EvoCode-Bench v2 MT@4上多轮性能提升13.8分。

Conclusion: 利用轨迹中的工具执行历史重建可执行环境是可行且高效的，能显著提升终端智能体在单轮和多轮场景下的性能。
...
### [23] [Efficient Test-Time Adaptation through Human-AI Interaction](http://arxiv.org/abs/2609.04141v1)
...
TL;DR: 本文提出TAHI方法，通过跨会话人机交互数据在测试时对AI代理进行个性化适应，有效缩小通用代理与个体专家的能力差距。


<details>
  <summary>Details</summary>
Motivation: AI代理生成的产物往往无法满足专业人员的个性化标准。由于开放式任务的成功标准具有异构性且记录不足，个体专业能力体现在对平均水平的超越，而用户无法在任务前期完全说明这些隐含标准。因此，本文旨在利用跨会话交互数据这一未充分利用的信号来弥合个体专家能力差距。

Method: 提出通过人机交互进行测试时适应的方法（TAHI）。该方法将跨会话的交互信号整合到代理的上下文和权重中，并引入一个不断演变的评估准则模块来具象化每个用户的训练和评估标准。

Result: 在写作和视觉创作两个领域对30名个体进行了600项任务的适应实验。代理在仅几十个任务内将独立任务成功率提升了4.5%至20.9%。此外，演化准则模块捕获的失败数量比单独使用语言模型或人类多16.0%至22.3%，且这些个性化代理在跨用户泛化时成功率最高提升了8.8%。

Conclusion: 跨会话人机交互数据是缩小AI代理与个体专家能力差距的丰富信号。TAHI不仅能有效实现个性化适应，其评估准则模块还可作为可扩展的标注工具，并且个性化适应带来的提升具有一定的跨用户泛化能力。
...
### [24] [The Natural Language Interaction Protocol and Standard for AI Agents](http://arxiv.org/abs/2609.04135v1)
...
TL;DR: 本文介绍了由Ecma International标准化的自然语言交互协议（NLIP），一种用于异构AI代理互操作的标准应用层协议。


<details>
  <summary>Details</summary>
Motivation: AI代理在组织中被开发和部署时使用异构的开发框架、AI模型、工具接口、协议和执行环境，为实现其社会和商业影响潜力，这些代理必须能够通过通用通信协议进行互操作。

Method: 提出NLIP协议，定义了一个轻量级语义消息信封，可承载于HTTP/HTTPS、WebSocket和AMQP等现有传输层之上，使NLIP感知的代理和网关能够在客户端、代理、本地上下文存储、本体、工具、企业服务和异构底层协议之间进行适配。

Result: 论文展示了NLIP的动机和设计原理、消息模型和传输绑定、安全设计考虑、参考实现、代表性应用、采用信号，以及与MCP和A2A等新兴代理协议的关系。

Conclusion: NLIP作为一种标准化的AI代理交互协议，通过提供通用的通信机制，解决了异构AI代理之间的互操作性问题，并与新兴的代理协议形成互补关系。
...
### [25] [Environment Evolution for Terminal Agents](http://arxiv.org/abs/2609.04128v1)
...
TL;DR: 本文提出“环境演化”方法，通过离策略方式逐步提升环境难度，为终端智能体训练提供持续的学习信号。


<details>
  <summary>Details</summary>
Motivation: 随着前沿模型能力增强，从头合成的环境挑战性下降，而现有协同演化方法依赖同策略rollout，限制了泛化能力且难以在模型变强时持续提供学习信号。

Method: 从多轮学习目标推导出三个影响环境难度的演化方向，利用循环工程化的多智能体框架离策略地逐步增加环境难度，并在训练过程中按代际调度演化环境。

Result: 使用Hy4 preview、Claude Opus 5和GPT-5.6 Sol进行的实验表明，该方法能持续生成更难的环境；在Qwen3.6-27B和Qwen3.6-35B-A3B上的长周期强化学习训练使其在Terminal-Bench 2.1上的性能分别提升了14.4和18.0个百分点。

Conclusion: 环境演化方法有效克服了现有协同演化方法的局限性，通过离策略难度递增持续提供学习信号，显著提升了终端智能体的性能，为未来智能体训练环境的动态构建提供了新方向。
...
### [26] [DRACO: Fine-Grained Credit Assignment with Dynamic Rubrics for Long-Horizon Agent Training](http://arxiv.org/abs/2609.04094v1)
*Shubham Gandhi,Saurabh Goyal,Kiran Kate,Yara Rizk*

Main category: cs.AI

TL;DR: 本文提出DRACO框架，通过在GRPO中闭式重分配基于规则的轨迹级奖励，解决无验证器长周期智能体任务中的信用分配问题。


<details>
  <summary>Details</summary>
Motivation: 长周期智能体任务缺乏程序化验证器，导致无法使用可验证奖励的强化学习；而现有的多准则规则仅提供稀疏的轨迹级标量奖励，难以有效指导多步决策。

Method: 提出DRACO方法，在训练中动态生成规则以追踪策略能力，对完整轨迹进行一次性评分后，将轨迹级判断以闭式解方式重新分配给产生该规则的各个步骤，从而在GRPO中产生差异化的每步优势，且无需额外的归因模块。

Result: 在AppWorld数据集上，DRACO比基础模型提升15.9分，比使用稀疏真实奖励的GRPO提升5.3分。在域外Tau-Bench数据集上，比基础模型提升5.3分，超越了真实奖励训练和其他基于规则的训练方法。

Conclusion: DRACO有效解决了无验证器环境下的长周期信用分配问题，在不引入额外训练模块的情况下，性能优于基于真实奖励的训练方法。
...
TL;DR: 本文首次系统性地测量了掩码扩散语言模型（dLLM）在真实并发服务负载下的行为特征。


<details>
  <summary>Details</summary>
Motivation: dLLM的服务基础设施正在快速构建，但缺乏在真实并发负载下的行为测量，可能导致错误地从自回归（AR）服务中沿用不适用于dLLM的假设。本文旨在填补这一空白。

Method: 使用LLaDA-8B-Instruct配合D2F LoRA适配器，在单张NVIDIA H200 GPU上，基于GSM8K和HumanEval数据集，对dLLM服务行为进行系统性测量与分析，并推导出固定填充同步批处理下的批处理超时规则。

Result: 发现请求难度（所需去噪步数）为离散而非连续，请求分布在11个固定步数级别，且生成前无法预测其级别（最佳R2=0.150）；生成预算低于320 tokens的基准测试会低估服务方差；单请求墙钟时间仅24%为GPU计算，其余为CPU侧调度开销；批处理通过摊销调度开销，在batch size 16时吞吐量提升16.0倍；GSM8K准确率为74%-76%。

Conclusion: 扩散语言模型的服务需要在每个去噪步骤层级进行并行化，这与AR服务在准入和驱逐机制与已共享前向传播交互方式上存在根本差异；未来需基于这些发现重新设计dLLM服务系统。
...
TL;DR: 提出一种无需训练的推理方法CAI-DLLM，利用首步置信度指导去噪，显著加速扩散语言模型的推理过程。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在推理时需要重复进行去噪步骤，特别是对已稳定的token进行重复计算，导致生成成本高昂。

Method: 提出CAI-DLLM，一种利用首步置信度指导去噪的免训练推理方法。该方法提前确定简单token，将更多去噪步骤分配给困难token，并调整不同输出块的解码计划，无需重新训练或额外预测器。

Result: 在LLaDA-8B和Dream-7B等模型上的评估表明，该方法在LLaDA GSM8K上实现最高18.2倍推理加速且准确率提升，在Dream HumanEval上实现13.1倍加速且pass@1提升；在更困难的推理任务中加速达44.8倍，最大准确率下降仅4.4个百分点，能耗最高降低95.3%。

Conclusion: CAI-DLLM在不需重训练的情况下，通过动态分配去噪步骤有效提升了扩散语言模型的推理速度并降低了能耗，同时保持了良好的准确率。
...
TL;DR: 本文提出了连续交互扩散（CID）架构，将工具交互集成到扩散语言模型的迭代去噪过程中，以克服传统自回归工具调用模式的限制。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（dLLMs）通过并行细化输出进行推理，而传统的“停止-恢复”式工具调用模式对dLLMs限制过多，会导致过早决策、延迟观察及冗余计算，从而影响任务准确性和推理效率。

Method: 提出Continuous Interaction Diffusion (CID)架构，将工具交互集成到迭代去噪中。CID分离出只读事实通道、由Typed Cognitive Tensor表示的思想通道和显示通道，允许在调用完全序列化前发起外部读取，并将返回结果投影到演变的思考状态中以修订认知。

Result: 本文形式化了该架构、运行时和训练目标，并定义了任务质量和端到端效率的评估协议，但专注于只读工具且未提出任何经验性能主张。

Conclusion: CID旨在更早暴露证据、将工具延迟与模型计算重叠、减少重复外部工作并保留新证据到达后的有用计算，未来方向可能包括进行实证性能验证。
...
TL;DR: 本文提出TAPS方法，通过选择紧凑的前缀闭包子树来优化基于扩散模型的投机解码中的验证过程。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的草稿树方法使用边际概率排序，忽略了验证的前缀条件性，导致验证不可达节点并增加延迟。

Method: 提出TAPS（目标感知前缀选择），将扩散边际概率转化为路径条件接受估计，并在固定验证预算下选择紧凑的前缀闭包子树。

Result: TAPS相比自回归解码实现了7.9倍的无损端到端加速，且比DFlash和DDree分别快1.36倍和1.74倍。

Conclusion: TAPS通过改善接受率与成本的权衡，有效解决了扩散模型投机解码的验证瓶颈问题。
...
### [31] [InSituMeasure: Probing Situated Measurement Grounding in Industrial Scenes with Multimodal Large Language Models](http://arxiv.org/abs/2609.04014v1)
*Chao Shen,Xinyuan Li,Yunfan Zhou,Jianguo Yao,Haibing Guan,Zhihai Wang,Xijun Li*

Main category: cs.AI

TL;DR: 本文提出了InSituMeasure基准，用于评估多模态大语言模型在真实工业场景中的仪表读数能力，揭示了现有模型在连续值测量上的显著不足。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在连续值测量方面仍不可靠，而现有基准缺乏真实场景上下文、专业仪器、现实噪声及配套诊断标注，导致真实感不足且无法进行根本原因分析。

Method: 提出InSituMeasure基准，包含2,922个真实工业监控场景和8类专业工程仪器，提供密集的仪表属性标注和噪声标签用于故障诊断；同时定义了数值精度、单位一致性、拒绝虚假或不可回答任务以及模型失败与标注误差因素对齐等评估指标。

Result: 对24个最先进的MLLMs评估发现，最佳模型的联合值-单位准确率仅为25.7%，置信度诊断F1分数仅为51.8%。失败原因主要源于文本诱导的捷径、过度自信的响应以及真实的工业噪声。

Conclusion: 研究揭示了多模态大模型在通用多模态能力与可靠的情境测量之间存在巨大差距，并指出了具体的失败原因，为未来提升模型在复杂工业环境中的测量可靠性提供了方向。
...
### [32] [NeoRed: A Knowledge-Logic-Alignment Multimodal Large Language Model for Neonatal Respiratory Disease Diagnosis](http://arxiv.org/abs/2609.03527v1)
*Yinan Liu,Hongtai Xia,Haoran Xu,Jiankang Hong,Jingkuan Song,Ye Luo*

Main category: cs.AI

TL;DR: 本文提出NeoRed，首个针对新生儿呼吸疾病的多模态大语言模型，通过知识-逻辑-对齐框架（KLA）整合临床上下文与胸片，实现精准的新生儿诊断报告生成。


<details>
  <summary>Details</summary>
Motivation: 新生儿呼吸疾病是新生儿发病和死亡的主要原因。现有MLLMs在新生儿诊断中存在两个关键限制：(1)训练数据主要为成人数据导致的领域差距；(2)对多维临床上下文整合不足，影响诊断准确性。

Method: 提出NeoRed模型，设计知识-逻辑-对齐(KLA)框架，包含三个组件：知识先验注入(KPI)将新生儿专家诊断先验融入多模态表示；诊断逻辑约束(DLC)将生成报告语义与多模态诊断逻辑对齐；视觉语义对齐(VSA)建立视觉特征与影像结论间的语义对应。同时构建了NeoCXR和NeoCXR-EV两个真实临床数据集。

Result: 在NeoCXR数据集上达到ROUGE-L 53.29%和临床效能F1分数65.19%，优于现有MLLMs。同时在成人基准（MIMIC-CXR和IU-Xray）上保持竞争力。

Conclusion: NeoRed填补了新生儿诊断报告生成领域的空白，证明了针对特定领域定制MLLMs的有效性，同时保持了跨年龄段的泛化能力。数据集将开放申请，为该领域后续研究提供基础。
...
### [33] [SWE-Gate: Passing Functional Tests Is Not Enough for Software Engineering Agents](http://arxiv.org/abs/2609.04167v1)
*Xin He,Yanlin Wang,Mingwei Liu,Jiachi Chen,Hongyu Zhang,Guanbin Li*

Main category: cs.SE

TL;DR: 本文提出SWE-Gate基准，评估代码代理在功能正确性和审查约束合规性两方面的表现，发现仅功能测试会高估代理能力。


<details>
  <summary>Details</summary>
Motivation: 现有仓库级软件工程基准仅测量生成的补丁是否通过功能测试，忽略了真实软件开发中影响补丁可接受性的审查约束，导致评估不全面。

Method: SWE-Gate从真实拉取请求审查评论中提取审查约束，合成仓库级修复实例。每个实例提供独立的功能测试和约束测试，以及不合规补丁和黄金补丁，实现问题解决能力与审查约束合规性的明确分离。

Result: SWE-Gate包含303个仓库级修复实例，覆盖75个开源Python仓库。使用4个LLM后端实验发现，在644个通过功能测试的修复中，有221个未能满足审查约束，表明仅功能评估高估了代理满足完整修复规范的能力。

Conclusion: 仅依赖功能测试的评估方法会显著高估代码代理在仓库级修复任务中的实际能力，未来评估应同时考虑审查约束合规性这一重要维度。
...
### [34] [StrixAE: An Intelligent Agent for Audio Enhancement under Complex Distortion Coupling in Real-World Scenarios](http://arxiv.org/abs/2609.03414v1)
*Chenglin Wu,Junjie Wu,Jinhang Chen,Mingyang Chen,Zixu Lin,Jiabian Chen,Xinghao Ding,Xiaotong Tu*

Main category: cs.SD

TL;DR: 本文提出了基于多模态大语言模型的智能体StrixAE，通过两阶段训练方法协同多个音频模型，以解决真实场景中复杂失真与个性化增强的双重挑战。


<details>
  <summary>Details</summary>
Motivation: 真实场景中的音频增强面临复杂失真耦合且需要个性化处理，而现有解决方案难以同时兼顾这两方面，亟需提升系统在真实复杂环境下的鲁棒性与自主运行能力。

Method: 提出StrixAE智能体，利用多模态大语言模型（MLLM）作为控制器来协调多种音频增强与个性化模型。该方法采用两阶段训练：首先在AcoustBench上进行思维链（CoT）监督微调以实现基础推理与工具调用；其次引入音频感知强化学习（APRL），通过定制化的结构化奖励机制联合优化格式有效性、结构连贯性和感知质量，确保生成无幻觉、可执行且可解释的增强管道。

Result: 在真实世界测试数据集上，该方法超越了大多数现有的开源和专有解决方案，在多个感知指标上取得了最先进（SOTA）的性能，并展示了强大的泛化能力和鲁棒性。

Conclusion: StrixAE有效解决了真实场景下复杂失真耦合与个性化增强的问题，验证了基于MLLM的智能体及专门定制的强化学习奖励机制在音频恢复管道中的有效性与可靠性。
...
TL;DR: 本文提出CForce方法，通过一致性约束蒸馏，改善扩散大语言模型在早期去噪阶段的预测可靠性，从而在高并行解码下获得更好的速度-质量权衡。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（dLLMs）通过单次前向传播预测多个掩码来加速语言生成，但在激进的并行策略下，早期去噪阶段的预测不可靠，错误会传播到后续阶段，导致生成质量下降。

Method: 提出Consistency Forcing（CForce）蒸馏方法，强制早期阶段的掩码预测与后期阶段对齐；在预收集的自展开轨迹上训练模型以改善训练-推理对齐；引入置信度自适应KL散度作为蒸馏目标，融合正向和反向KL的优势；该公式同时适用于掩码到token的解码和具备编辑能力的解码。

Result: 在非编辑和编辑能力的LLaDA模型上的实验表明，CForce改善了速度-质量权衡，尤其在高并行解码预算下表现突出；理论分析解释了CForce为何能近似最小化早期阶段的预测误差。

Conclusion: CForce为dLLMs提供了一种有效的训练-推理对齐方法，通过一致性蒸馏减少早期预测错误，在高速并行解码场景下具有显著优势；同一框架可统一处理掩码解码和编辑解码两种场景。
