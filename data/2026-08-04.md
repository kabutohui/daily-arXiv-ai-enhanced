...
TL;DR: 提出仅解码器的多模态嵌入模型UEmbed，在单次因果前向传播中统一生成稠密与稀疏词法表示。


<details>
  <summary>Details</summary>
Motivation: 现有的学习型稀疏检索（LSR）局限于编码器架构，且在多模态设置中严重依赖辅助跨模态模块，本文旨在解决这些局限性。

Method: UEmbed在输入后附加N个可学习的特殊标记，并将词汇表划分为N个不相交的子集；每个标记的因果隐藏状态预测其对应子集的稀疏权重，拼接后形成完整的稀疏向量，同时生成稠密表示。

Result: 在公开数据上训练并发布了2B、4B和9B规模的模型；UEmbed-9B在MMEB-v2上的稠密和稀疏得分分别达到71.8和71.0，优于RzenEmbed等基线，且在BEIR上表现具有竞争力，同时在有效性、效率和智能体应用上展现了实用价值。

Conclusion: UEmbed提供了一种新范式，不仅在单一模型中统一了稠密和稀疏嵌入，还将稀疏检索扩展至统一文本与多模态输入。
...
### [2] [MIEScore: Human-Aligned Evaluation for Multi-Source Image Editing](http://arxiv.org/abs/2608.02059v1)
*Zitong Xu,Huiyu Duan,Xinyun Zhang,Weifei Xiong,Tianyi Zheng,Xiongkuo Min,Qiang Hu,Zhengxue Cheng,Bo Li,Guangtao Zhai*

Main category: cs.CV

TL;DR: 论文提出了MIE-Bench，第一个大规模多源图像编辑基准，以及MIEScore评估模型，用于对齐人类偏好并提供反馈。


<details>
  <summary>Details</summary>
Motivation: 现有的图像编辑评估基准和方法主要关注单图像编辑任务，忽略了更具挑战性的多源图像编辑（MIE）设置，迫切需要一个全面且与人类偏好对齐的MIE基准。

Method: 提出MIE-Bench，包含3,000个编辑实例，覆盖16个任务，每个实例涉及两个以上源图像和编辑提示，以及12个最先进编辑模型生成的36K张编辑图像和超过108K个平均意见分数（MOSs），涵盖视觉质量、指令遵循和属性保持。基于此提出MIEScore，一个基于多模态大语言模型的评估模型，通过技能优化和多维度监督微调进行增强。

Result: MIEScore在与人偏好对齐方面达到了最先进的性能，并在其他IEQA数据集上具有良好泛化能力。

Conclusion: 提供了第一个多源图像编辑的全面基准和与人类对齐的评估模型，数据集和模型已开源。
...
### [3] [CultureVidBench: Benchmarking Cultural Understanding in Text-to-Video Generation](http://arxiv.org/abs/2608.01942v1)
*Xianjing Han,Yuhan Su,Yang Deng,Dong Ma,Wee Peng Tay,Bin Zhu*

Main category: cs.CV

TL;DR: 本文提出CultureVidBench基准以评估文本到视频生成模型的文化理解能力，发现现有模型在捕捉细粒度文化细节方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成基准主要关注感知质量、物理合理性和文本-视频对齐，缺乏对生成视频中特定文化对象、动作、仪式及多模态文化提示的直接评估。

Method: 提出CultureVidBench基准，包含1000个覆盖多国家、多区域和14个文化方面的精选提示，并通过人类用户研究和基于多模态大语言模型（MLLM）的自动评估对7个代表性模型进行多维评估。

Result: 结果表明，尽管当前模型在语义依从性和视觉质量上表现良好，但在忠实捕捉细粒度文化细节（特别是代表性不足的地区、仪式和多模态文化提示）方面常常失败。

Conclusion: 当前文本到视频生成模型在细粒度和多模态文化细节的捕捉与忠实度方面仍有待显著提升，特别是在处理代表性不足的文化内容时存在明显局限。
...
### [4] [Decoupling semantics from vision: A framework for faithful visual-text compression evaluation](http://arxiv.org/abs/2608.01848v1)
*Yonghan Gao,Zehong Chen,Lijian Xu,Jingzhi Chen,Jingwei Guan,Xingyu Zeng*

Main category: cs.CV

TL;DR: 本文提出一种解耦多模态大语言模型能力的评估框架及ZeroSense基准，以准确衡量视觉文本压缩的真实质量。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉文本压缩评估方法过度依赖下游任务表现，由于多模态大语言模型具有强大的语言先验，这种评估方式无法准确反映文本保留的真实质量。

Method: 提出一种将MLLMs能力解耦的新评估框架，并引入ZeroSense基准测试，通过确保测试样本间低语义相关性来消除文本依赖，使评估结果纯粹反映VTC质量。

Result: 在多个数据集上的广泛实验表明，VTC的真实质量与下游任务的准确率存在显著差异，证明了现有评估指标的不足以及解耦评估的必要性。

Conclusion: 下游任务准确率不能作为衡量VTC质量的可靠标准，所提出的解耦评估框架和ZeroSense基准能更忠实、准确地评估视觉文本压缩技术。
...
### [5] [DeepVoyager-VL: Incentivizing Vision-in-the-Loop Search for Long-Horizon Multimodal Agents](http://arxiv.org/abs/2608.01827v1)
...
TL;DR: 本文提出长视距多模态深度搜索框架DeepVoyager-VL，通过视觉在环搜索机制解决现有方法在中间推理中忽略视觉证据及缺乏长视距交互设计的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型受限于静态参数化知识，且现有多模态深度搜索方法通常将视觉限制在输入或输出阶段，忽略了其在中间推理中的作用，缺乏长视距交互设计，导致视觉证据无法驱动持续检索，限制了交互深度与推理跨度。

Method: 构建多模态事件图以驱动数据合成，生成具有中间视觉依赖和长推理链的问题；设计用于主动视觉获取和按需图像加载的代理框架；并在无需强化学习的情况下，利用合成数据对模型进行微调。

Result: 在十个多模态搜索基准测试上的广泛实验验证了该方法的有效性。

Conclusion: DeepVoyager-VL框架成功将视觉证据融入检索的中间推理过程，实现了视觉在环的长视距搜索，显著提升了多模态深度搜索的交互深度和推理能力。
...
### [6] [Illuminating Visual Identity in Universal Multimodal Embeddings](http://arxiv.org/abs/2608.01794v1)
*Jiawei Cao,Junyi Feng,Jiashen Hua,Ziheng Huang,Bing Deng,Kaijie Wu,Chaochen Gu,Jieping Ye*

Main category: cs.CV

TL;DR: 本文提出了一种联合优化框架和大规模基准MVEB，旨在解决通用多模态嵌入中视觉身份判别能力不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的通用多模态嵌入（UMEs）在视觉身份判别方面研究不足，而这种能力对实例检索、重识别和AI生成内容的身份保持等任务至关重要。

Method: 提出了视觉身份判别（VisID）的统一公式，构建了大规模基准MVEB，并设计了一个学习框架，通过身份感知采样机制联合优化通用多模态表示和视觉身份表示。

Result: 实验表明，该方法成功赋予了UMEs强大的身份判别能力，同时保持了具有竞争力的通用多模态性能。

Conclusion: 本研究揭示了UMEs中被忽视的关键能力，并朝着更全面的多模态嵌入技术迈出了重要一步。
...
### [7] [Learning Where to Look and How to Judge: Resolution-agnostic Image Quality Assessment with Quality-aware Saliency](http://arxiv.org/abs/2608.01730v1)
*Hakan Emre Gedik,Shashank Gupta,Alan Bovik*

Main category: cs.CV

TL;DR: 本文提出了ReLIQS，一种分辨率无关、基于CLIP和多尺度补丁的无参考图像质量评估模型，通过结合显著性学习在保持计算高效的同时实现了更优的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有SOTA无参考图像质量评估(NR IQA)系统存在因激进缩放丢失关键质量线索、难以跨分辨率泛化、无法在MOS尺度不匹配的异构数据集上联合训练或计算成本过高等问题。

Method: 提出基于CLIP的多尺度补丁驱动架构ReLIQS。模型从多分辨率（含原始分辨率）中采样固定大小补丁并用CLIP视觉骨干编码；利用轻量级感知重要性估计器预测IQA特定重要性图以筛选信息丰富的补丁；最后通过潜在质量轴模块将补丁嵌入聚合为图像级质量评分。

Result: 在涵盖真实、合成和AIGC（跨多种分辨率和失真类型）的基准测试中，ReLIQS比基于CNN、CLIP和MLLM的强基线具有更好的泛化能力，且计算成本持平或更低。

Conclusion: ReLIQS有效克服了现有NR IQA系统在缩放、分辨率泛化、异构数据集联合训练和计算成本上的局限，在保持计算高效和预算自适应的同时实现了优越的跨数据集泛化性能。
...
### [8] [Mitigating Visual Degradation in MLLMs via Spatial-Spectral Visual Anchor Learning](http://arxiv.org/abs/2608.01635v1)
*Qianlong Yang,Bowen Ye,Xianda Guo,Yanlun Peng,Wenke Huang,Hongyuan Zhang,Yulei Jia*

Main category: cs.CV

TL;DR: 提出空间频谱视觉锚点学习（SSVAL），通过注入视觉锚点提示和辅助对齐损失，解决多模态大语言模型推理过程中的表征偏移问题，提升视觉感知能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉感知上存在缺陷，其内部表征在推理过程中会迅速偏离原始语义状态导致信息退化，而现有利用外部视觉基础模型直接对齐的方法无法有效缓解该表征偏移问题。

Method: 提出空间频谱视觉锚点学习（SSVAL），其核心为视觉锚点提示注入（VAPI），在训练时吸收外部VFM知识以在推理时作为稳定的视觉锚点缓解表征偏移；并引入辅助的空间和频域表征对齐损失，在LLM中间层提供互补的视觉特定监督。

Result: 广泛的实验表明，SSVAL方法显著优于现有的其他方法。

Conclusion: 该方法成功缓解了多模态大语言模型的内部表征偏移与信息退化问题，有效提升了视觉感知性能，且代码已开源供后续研究。
...
### [9] [Slot2Text: Object-Centric Visual Tokenization for Efficient and Spatially Traceable Surgical MLLMs](http://arxiv.org/abs/2608.01473v1)
*Guiqiu Liao,Matjaz Jogan,Daniel A. Hashimoto*

Main category: cs.CV

TL;DR: 提出了一种名为Slot2Text的双模式手术多模态大语言模型，使用紧凑的槽隐空间替代密集视觉标记，在大幅降低推理成本的同时保持空间可追溯性。


<details>
  <summary>Details</summary>
Motivation: 现有的手术场景多模态大语言模型通常将数百个密集视觉标记注入语言模型，导致推理成本高昂且生成的答案缺乏空间可追溯性。

Method: 提出Slot2Text模型，将自监督视觉特征分组成少数区域（槽），并作为带区域标签的视觉标记供语言模型使用。包含Fast模式（用槽前缀快速回答）和Reason模式（定位相关区域并链接语言输出与槽标记、掩码以进行可追溯推理）。

Result: Slot2Text-Fast以更低成本与最先进基线竞争，平均总标记消耗减少91.8%，视觉前缀从1295个降至47个（减少96.4%）；Slot2Text-Reason以额外的标记和延迟换取了明确的区域身份、位置和可追溯的空间证据。

Conclusion: 紧凑的槽隐空间可作为手术多模态大语言模型高效的默认视觉接口，当需要更高的空间可追溯性时可调用有根据的推理。
...
### [10] [LiveMem: Maintaining Memory State Continuity in Long-Running LLM Inference](http://arxiv.org/abs/2608.02515v1)
*Zhichen Liu,Ruihan Sun,Hengjie Yang,Zipeng Wu,Zhaohan Chen,Xiaofan Zhang,Yang Xu*

Main category: cs.CL

TL;DR: 本文提出LiveMem方法，通过引入独立于活跃上下文的固定容量记忆状态，解决长期运行大语言模型中的上下文更替状态连续性问题。


<details>
  <summary>Details</summary>
Motivation: 长期运行的智能体交互流最终会超出上下文限制，而现有的上下文保留、摘要和检索方法在上下文更替时无法提供覆盖整个生命周期的持久计算状态。

Method: 提出LiveMem内在记忆方法，在预训练全注意力LLM上增加一个固定容量的记忆状态以保留全生命周期历史信息，同时主注意力路径保持有界KV窗口；结合上下文更替、记忆状态维护、记忆导向的后训练和状态感知服务来确保记忆状态的承载能力。

Result: 实验显示LiveMem在整体性能上领先于其他评估系统和内在记忆方法；在LongMemEval测试中，即使相关证据已移出当前上下文，模型仍能利用记忆状态回答问题，且证据距离分析证实有用信息可持久存在于活跃窗口之外。

Conclusion: LiveMem将状态连续性确立为持续LLM推理中一种独立且互补的新抽象概念，有效解决了长周期交互中的状态持久化问题。
...
### [11] [Faster but Different: Diagnosing and Controlling Content Drift in Accelerated Multimodal Diffusion Language Models](http://arxiv.org/abs/2607.29079v1)
*Yaoxuan Dou,Yang Shu*

Main category: cs.CL

TL;DR: 本文研究了免训练加速方法Fast-dLLM对基于扩散的多模态大语言模型生成内容一致性的影响，发现缩短KV缓存刷新间隔可恢复近乎完全的一致性。


<details>
  <summary>Details</summary>
Motivation: 免训练加速使基于扩散的多模态大语言模型更易部署，但可能悄然改变生成内容，因此需要诊断并控制这种服务时的一致性问题。

Method: 在300张真实图像上比较Fast-dLLM加速输出与未加速输出的差异，通过置信度阈值调优、状态刷新消融、图像交换干预、低一致性对内容审查及双标注者盲评等多种诊断手段定位漂移来源，并测试缩短KV缓存刷新间隔等一致性控制策略。

Result: 置信度阈值调优不改变基线一致性；陈旧的视觉和生成文本状态是漂移的主要原因；缩短KV缓存刷新间隔在1.3倍加速下实现近乎完全一致；dLLM-Cache需同时收紧两个缓存才能恢复一致性但失去加速优势；盲评未检测到事实错误差异（95% CI [-0.17,+0.17]）但无法确立事实等价性；固定间隔优于所有自适应或平滑刷新变体。

Conclusion: 贡献为配对诊断方法和实现层面的一致性控制策略，可恢复加速后的一致性，但不构成准确性或安全性保证。
...
TL;DR: 提出基于轨迹的在线策略蒸馏框架TOPD，通过教师监督将推理能力迁移至扩散大语言模型，无需奖励估计即可实现高效训练。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的推理后训练面临挑战：SFT需要密集但常为离策略的掩码状态，RL依赖稀疏奖励或价值建模，二者均存在效率和对齐问题。

Method: TOPD从目标dLLM采样在线策略扩散轨迹，获取教师模型在部分去噪状态上的token分布，采用token级反向KL散度目标更新目标模型，保留密集教师监督同时与模型自身去噪状态对齐。

Result: SDAR-4B-Chat在MATH500上达到与RL训练的TraDo-4B-Instruct相当的准确率，静态评估提升+5.7，动态评估提升+4.5；相比RL方法使用4倍更少的rollout轮次，计算效率提升约96倍。

Conclusion: TOPD无需奖励估计即可实现推理能力迁移，在保持密集监督的同时与模型自身去噪状态对齐，显著提升训练效率，为dLLMs的推理后训练提供了高效新范式。
...
TL;DR: 提出CoCommit方法，通过标记门控协调机制延迟扩散大语言模型的token提交，以减少多token独立解码产生的因式分解误差。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在每步去噪中独立解码多个token，当这些位置存在依赖关系时会产生因式分解误差（由条件总相关性捕获），而现有基于置信度的选择方法无法从边缘分布中观察到该误差。

Method: 提出CoCommit，一种标记门控协调过程。在常规束选择后，通过学习的标记确定提交集，并重新应用骨干网络的最后n层使标记位置相互协调（近似联合模式解码），然后再执行贪婪argmax。该方法复用现有权重，仅需一次额外的部分前向传播且无需辅助模型。

Result: 在LLaDA2.1-mini模型（使用LoRA适配器）及匹配的贪婪推理上，该方法在六个评估基准上均提升了准确率，在推理和精确答案任务上提升最大。

Conclusion: CoCommit通过轻量级的协调机制有效解决了扩散大语言模型多token提交时的依赖性问题，显著提升了模型在推理和精确答案任务上的性能。
...
TL;DR: 本文提出了DominoTree，一种利用路径依赖的非分解条件校正来构建最佳优先草稿树的免训练方法，显著加速了大型语言模型的推测解码。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散起草器和最佳优先树方法仅能建模位置边缘分布，无法表示具有路径依赖性的草稿token分布（如Domino起草器基于GRU的因果校正）。本文旨在解决如何有效利用这种非分解的条件校正结构来构建草稿树的问题。

Method: 提出DominoTree，一种免训练的最佳优先草稿树构建方法。该方法沿着每条从根到节点的路径使用Domino的条件、非分解校正进行评分，并通过将每个节点的校正限制在候选前M个集合中来降低计算成本。此外，还开发了一个GPU原生的CUDA图构建器以低成本构建草稿树。

Result: 在Qwen3-4B上实现了高达6.6倍于自回归解码的加速，并在所有测试温度下取得了最高的平均接受长度（高达10.7 tokens/轮）。相比Domino，吞吐量整体提升9%至10%（在Alpaca上最高提升22%），且在所有温度下均优于DDTree和CaDDTree。在Qwen3-8B上，各温度下接受长度保持最高，T=0时吞吐量比DDTree高24%。

Conclusion: DominoTree成功将路径依赖的草稿分布整合到树结构推测解码中，通过高效的GPU原生构建器在保持低成本的同时，实现了显著的吞吐量提升和接受长度增加，证明了非分解校正结构在加速LLM推理中的有效性。
...
TL;DR: 本文提出CaDDTree方法，通过显式建模延迟并联合优化树结构与节点预算，直接优化token吞吐量，无需离线预算搜索。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿器（如DDTree）在固定节点预算下最大化预期接受长度，但接受长度随预算非递减，导致无法提供有原则的预算选择依据，忽视验证成本。

Method: CaDDTree通过显式建模draft和验证延迟，将吞吐量目标分解为每轮对预算的一维搜索，并证明在凸验证成本下吞吐量函数是单峰的，从而实现高效的贪婪停止规则；每轮根据当前分布和验证成本自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B上跨八个基准（推理、编码、指令遵循任务）的实验表明，CaDDTree几乎在所有任务上匹配或超过具有oracle预算选择的DDTree。

Conclusion: CaDDTree解决了预算选择缺乏原则性依据的问题，无需离线搜索即可自适应优化吞吐量，性能优于现有方法。
...
TL;DR: 本文提出DDTree方法，通过从块扩散草稿模型构建草稿树来提升推测解码的接受长度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash方法每轮仅验证单条草拟轨迹，限制了其接受长度，从而制约了推测解码的加速潜力。

Method: DDTree利用块扩散草稿器的逐位置分布构建草稿树，在固定节点预算下采用最佳优先堆算法选择最可能匹配目标模型的延续路径，并通过仅祖先注意力掩码在单次目标模型前向传递中完成验证。

Result: 基于DFlash构建的DDTree取得了显著性能增益，使其跻身于推测解码领域的领先方法之列。

Conclusion: DDTree有效克服了单轨迹验证的局限性，代表了推测解码技术的前沿进展，有望进一步提升大语言模型的推理效率。
...
### [17] [ACEM: A Cost Estimation Model for Agentic Software Engineering](http://arxiv.org/abs/2608.02582v1)
*Mohammad El-Ramly*

Main category: cs.SE

TL;DR: 本文提出智能体成本估算模型（ACEM），通过引入LLM令牌、人类参与和基础设施三个维度，解决智能体软件工程中的成本估算问题。


<details>
  <summary>Details</summary>
Motivation: 传统成本估算模型假设人力是主要驱动因素，但智能体软件工程将工作转移给自主AI代理，产生了LLM令牌消耗、人类监督和基础设施等非确定性新成本，传统模型无法适用，需要新框架弥合传统规模指标与新成本结构的差距。

Method: 提出ACEM模型，将总成本分解为LLM、HITL和基础设施成本三项加和。引入修订因子(RF)建模重试令牌开销，上下文因子(CF)捕捉上下文积累导致的令牌消耗，HITL强度评分(HIS)进行四级监督分类。同时将用例点、故事点和功能点映射至估算的令牌消耗量。

Result: 提出了一个完整定义的模型结构和校准方法论，模型常数暂留为符号形式，尚处于早期阶段，无实证实验结果，待后续经验数据支撑。

Conclusion: ACEM为组织利用现有项目范围数据预测智能体开发成本提供了框架基础，并呼吁研究社区利用真实项目数据对该模型进行校准、测试和扩展。
...
### [18] [The Condition-Number Barrier in Sparse Least Squares](http://arxiv.org/abs/2608.02588v1)
*Honghao Lin,Vahab Mirrokni,David P. Woodruff*

Main category: cs.DS

TL;DR: 本文证明了在稀疏凸优化中，关于受限条件数线性依赖下界的猜想，并展示了AI在复杂数学证明中的应用。


<details>
  <summary>Details</summary>
Motivation: 旨在解决Axiotis和Sviridenko提出的猜想，即证明稀疏凸优化中对受限条件数的线性依赖无法被多项式时间算法改进。

Method: 在基于加权正则图的随机精确体积小集合扩展假设下进行理论证明，该证明最初由Google内部开发的基于Gemini的自动化智能体系统生成并经作者验证。

Result: 证明了对于最小二乘目标函数，不存在随机多项式时间算法能以高概率返回满足特定稀疏度和误差上界的向量，该结果在全列秩有理数实例上同样成立。

Conclusion: 成功确立了稀疏凸优化中受限条件数线性依赖的不可改进下界，同时表明大模型驱动的智能体系统具备辅助复杂数学证明的能力。
...
### [19] [Pairwise-Independent Dithering for Single-Stage Hadamard Quantization](http://arxiv.org/abs/2608.02564v1)
*Honghao Lin,Vahab Mirrokni,David P. Woodruff*

Main category: cs.DS

TL;DR: 本文证明了在基于Hadamard变换的无偏抖动量化器中，仅需两两独立的抖动即可省去残差量化阶段，从而实现更高效的单阶段内积估计。


<details>
  <summary>Details</summary>
Motivation: Feng等人提出的无偏抖动量化器在内积估计中使用了第二阶段随机变换和残差量化，增加了通信开销并使理论上界常数变大。本文旨在消除这一冗余阶段以降低通信成本并改善理论保证。

Method: 提出在Hadamard坐标间使用两两独立抖动（pairwise-independent dithers），构建单阶段无偏内积估计器，无需额外的随机变换和残差量化步骤。证明最初由Google内部的Gemini自动化系统获得，作者随后验证并整理了证明。

Result: 单阶段估计器每坐标使用b比特，其内积估计误差上界为(3π√3/2+o(1))·||y||²/(d·4^b)，其中o(1)项对单位输入和固定查询是维度无关的。相比Feng等人的两阶段方案，消除了O(d)比特的残差载荷，并将上界常数降低约5.93倍。

Conclusion: 额外的残差量化阶段是不必要的；两两独立抖动已足以保证理论保证，同时显著降低通信成本和上界常数。该工作由自动化AI系统辅助完成证明，表明AI辅助数学证明在理论计算机科学中的潜力。
...
### [20] [RoMeRL: Balancing Feedback Coverage and the Memory-Reward Trap in Self-Evolving Agent Memory via Reduced-Order Utility States](http://arxiv.org/abs/2608.02508v1)
*Yi Yang,Zhennan Chen,Yihong Zhuang,Tiehan Fan,Yinan Chen,Jian Li,Jian Yang,Ying Tai*

Main category: cs.LG

TL;DR: 该论文提出了RoMeRL（降阶记忆强化学习）方法，通过固定维度的任务记忆状态来解决自进化LLM智能体中效用空间增长和记忆-奖励陷阱的问题。


<details>
  <summary>Details</summary>
Motivation: 解决基于学习的自进化LLM智能体记忆系统面临的两个耦合挑战：一是随交互历史增长的轨迹索引效用导致有限反馈在扩大的状态空间中分散；二是轨迹级奖励共同分配给共同检索的记忆，导致无关经验获得误导性效用更新（即记忆-奖励陷阱）。

Method: 提出RoMeRL，利用由结果极性和记忆动态分解的固定维度任务记忆状态来表示不断增长的效用空间。通过一组固定的语义坐标更新或替换内容来纳入新经验，从而将反馈集中在有界的效用支撑上。

Result: 在ALFWorld和LifelongAgentBench上的实验表明，RoMeRL提升了任务性能，将Cold-Q比率降低80.0%，反馈密度提高约6.0倍，维持的记忆大小减少84.4%，并减少了21.1%的LLM调用。
...
### [21] [Beyond On-Policy Exploration: Integrating External Policy Rollouts for Reinforcement Learning in Diffusion Language Models](http://arxiv.org/abs/2608.01717v1)
*Wonseok Lee,Jimyeong Kim,Jungmin Ko,Wonjong Rhee*

Main category: cs.LG

TL;DR: 本文提出ERILS方法，通过结合更强的外部轨迹并控制其长度与分别处理奖励，提升了离散扩散大语言模型(dLLM)在成功轨迹稀缺时的强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有dLLM的强化学习依赖模型自身生成在线轨迹，当成功轨迹稀缺时，训练获得正向奖励少，进展有限。

Method: 提出外部轨迹集成方法ERILS，包含长度控制和特定来源的奖励处理机制，以解决直接引入外部轨迹带来的长度差异和奖励处理不稳定问题。

Result: 在Sudoku、Countdown和MATH500任务上均提升了多样本性能，Sudoku上best-of-4准确率达98.4%（基线40.3%），且在不同生成长度下保持约90%的单次准确率。组件分析证实了长度控制和分别处理奖励的有效性。

Conclusion: 在将外部轨迹整合进dLLM强化学习时，轨迹构建和奖励处理是关键的设计维度。
...
### [22] [A Continuous-Time Reinforcement Learning Framework for Fine-Tuning Discrete Diffusion Models](http://arxiv.org/abs/2607.14522v1)
*Zikun Zhang,Jiayuan Sheng,David D. Yao,Wenpin Tang*

Main category: cs.LG

TL;DR: 提出一种基于连续时间马尔可夫链的连续时间强化学习框架，并推导连续时间版PPO和GRPO，用于微调离散扩散模型。


<details>
  <summary>Details</summary>
Motivation: 解决离散扩散模型微调中奖励信号不可微的问题，并克服现有方法仅依赖终端奖励而无法利用去噪轨迹中间奖励的局限。

Method: 通过随机控制方法将状态动态建模为受控连续时间马尔可夫链，推导出连续时间PPO和GRPO；在掩码扩散模型中实现具有解析概率比的策略参数化；针对掩码扩散大语言模型提出轨迹子采样技术以降低概率比计算成本。

Result: 在低维熵正则化优化问题及大语言模型数学推理与编程任务的强化学习后训练中验证了方法的有效性。

Conclusion: 该框架为掩码扩散模型的探索与策略优化提供了统一视角，实现了无需可微奖励的高效强化学习微调。
...
### [23] [Accelerating Masked Diffusion Large Language Models: A Survey of Efficient Inference Techniques](http://arxiv.org/abs/2607.12829v1)
*Daehoon Gwak,Minhyung Lee,Junwoo Park,Jaegul Choo*

Main category: cs.LG

TL;DR: 本文提出一种用于扩散大语言模型的统一延迟分解框架，以解耦并分析算法、架构和系统层面的加速技术及基准测试指南。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散大语言模型在并行生成上具有理论优势，但实际加速仍面临挑战，且现有基准测试常将算法、架构和系统层面的复杂权衡混杂，导致难以进行严格比较。

Method: 引入统一的延迟分解框架以解耦影响推理速度的各类因素，并沿算法创新、架构与系统优化、推理时间缩放三个轴线对加速技术进行分类分析。

Result: 系统性地梳理了现有加速技术的影响，并提供了用于扩散大语言模型可复现基准测试的具体指南。

Conclusion: 指出了实现并行生成全部潜力的开放性挑战，并强调了可复现基准测试在实际部署中的关键作用。
...
### [24] [BlockServe: Block-Grained Continuous Batching for High-Throughput Diffusion LLM Serving](http://arxiv.org/abs/2607.08930v1)
*Yuanjie Zhu,Liangwei Yang,Ke Xu,Weizhi Zhang,Shanghao Li,Zihe Song,Philip S. Yu*

Main category: cs.LG

TL;DR: 本文提出了BlockServe连续批处理框架，通过块粒度调度、混合状态执行和计算感知准入控制有效解决了扩散大语言模型推理中的收敛异质性问题，显著提升了吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在批量推理时存在收敛异质性问题，不同序列收敛速度不同导致快请求被慢请求阻塞，从而产生计算气泡和尾部延迟，降低了服务效率。

Method: 提出BlockServe框架，集成了在块边界立即驱逐已完成请求的块粒度调度、通过gather-scatter索引将双缓存和并行解码扩展至异构批次的混合状态执行，以及基于令牌预算补充扩大有效批处理容量的计算感知准入控制器。

Result: 在Dream和LLaDA模型的五个基准测试中，BlockServe相比Fast-dLLM实现了1.9至10.6倍的吞吐量提升，同时保持了相当的生成质量。

Conclusion: 块粒度调度是高吞吐量离线扩散大语言模型推理的有效基础，BlockServe为解决其服务效率瓶颈提供了可行方案。
...
### [25] [AtumAI: A Principled Framework for Agentic Generation of Datacenter Control-Plane Policies](http://arxiv.org/abs/2608.02569v1)
*Qiushi Lin,Chaojie Zhang,Íñigo Goiri,Aditya Akella,Ricardo Bianchini,Jovan Stojkovic*

Main category: cs.AI

TL;DR: 本文提出了AtumAI框架，通过形式化、可迁移和系统化的代理式AI自动生成数据中心控制面策略，其表现优于专家设计的基线方案。


<details>
  <summary>Details</summary>
Motivation: 数据中心控制面策略的设计空间庞大且依赖复杂，传统原型设计耗时数月；而现有的代理式AI缺乏形式化约束保证、不可跨任务迁移，且仅依赖LLM导致搜索不系统并易陷入局部最优。

Method: 提出AtumAI框架，包含两个核心组件：1) 数据中心任务编译器，将自然语言需求转化为形式化、可机器检查与搜索的规范；2) 进化设计发现循环，结合扩散模型、进化算法和代理模型扩展搜索空间，自动提出、测试并优化候选策略。

Result: 在工作负载分配、资源扩展和电源管理三个任务上的评估表明，AtumAI生成的策略始终优于专家设计的基线，且将新任务的开发周期从数月缩短至仅需编写文本描述。

Conclusion: AtumAI成功实现了数据中心控制面策略设计的自动化，解决了现有AI方法的局限性，显著降低了工程时间并提升了策略性能，为复杂系统设计提供了新的范式。
...
### [26] [A Taxonomy of Cognitive Capability Gaps in Generative and Agentic AI](http://arxiv.org/abs/2608.02553v1)
*Taye Akinrele,Sindhuja Penchala,Noorbakhsh Amiri Golilarz,Sudip Mittal,Shahram Rahimi*

Main category: cs.AI

TL;DR: 本文通过分类学驱动的综述分析了认知AI在五个维度上的能力差距，并提出了自适应认知智能架构（ACIA）与评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式和智能体AI在长期推理、自适应行为、持久记忆和自我调节等基础认知功能上存在碎片化或发育不全的问题，限制了其在长时间跨度内的可靠运行。

Method: 采用分类学驱动的方法，围绕持久状态建模、目标导向自主性、自我监测与控制、环境交互及学习与适应五个维度综述相关文献，并提出了概念性架构ACIA及认知中心评估方向。

Result: 总结了五个维度上的最新进展、局限性及开放挑战，并提供了一个统一框架以组织现有研究并指导未来认知系统的设计。

Conclusion: 该分类法、架构视角和评估框架为推进具备长期推理、自适应决策和持续学习能力的AI系统提供了路线图，为实现认知AI及通用人工智能（AGI）奠定了基础。
...
### [27] [Magnet: Detecting Cross-Session AI Misuse Through Capability Accumulation](http://arxiv.org/abs/2608.02518v1)
*Natalie Isak,Matthew Dressman*

Main category: cs.AI

TL;DR: 本文提出了跨会话目标分解这一AI滥用规避技术，并设计了Magnet检测系统，通过跨会话和时间聚合证据来检测此类攻击。


<details>
  <summary>Details</summary>
Motivation: 现有AI滥用检测主要针对单次会话威胁模型，无法应对攻击者将有害目标分解为无害单元并在隔离的智能体会话中执行的跨会话攻击。

Method: 提出Magnet检测方法，在更高层级（如用户ID）上建模跨会话和时间积累的能力，将分散在无害会话中的危险产物提取并聚合成紧凑的证据包供检测器分析。

Result: 发现跨会话目标分解能比等效单会话或多轮攻击引发更多有害能力，并验证了Magnet方法能高效且稳健地从海量良性会话中提取跨会话的危险证据。

Conclusion: 跨会话目标分解是AI智能体系统的新型威胁，而Magnet填补了相关检测空白，为多智能体架构提供了有效的跨会话监控手段。
...
### [28] [Abduction Without a Body? Representational Grounding and the Abduction Loop for Scientific Hypothesis Generation](http://arxiv.org/abs/2608.02505v1)
*Michael Farmer*

Main category: cs.AI

TL;DR: 本文提出“溯因循环”架构和“约定空间”概念，论证科学溯因不必然需要在线物理具身性，并展示了跨领域科学图表同一性假设的生成与验证。


<details>
  <summary>Details</summary>
Motivation: 探讨科学溯因（特别是同一性溯因）是否必须依赖连续感觉运动具身性，并解决不同学科因缺乏共享词汇而难以发现数学相关研究的跨领域检索难题。

Method: 提出“约定空间”以规范科学图表中的对称性等结构，并构建“溯因循环”架构，通过表征生成、模体提取、规范空间规范化、跨域检索、假设生成和对抗性验证实现同一性溯因。

Result: 记录了多模态模型从引力记忆传输模型图表推导并验证其与弱透镜宇宙学球面Kaiser-Squires质量映射复形等价的案例，并提出了可证伪的评估程序DAB-30基准。

Conclusion: 在线具身性并非所有科学溯因行为的必要条件；本文核心贡献在于提供了机制性建议、“溯因循环”架构及测试程序。
...
TL;DR: 本文提出了无需训练的测试时扩展框架Evolutionary Decoding，通过演化解码过程解决扩散大语言模型在数学推理中的置信度陷阱问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在数学推理中存在局部token置信度与全局正确性不一致的“置信度陷阱”，导致采样不稳定或陷入重复的错误高置信度状态，降低了推理可靠性。

Method: 将扩散解码视为候选推理状态的演化过程，提出Evolutionary Decoding框架，结合逐步选择以保留有用符号信号并抑制重复，以及块级突变以引入结构化替代方案来逃离错误的高置信度区域。

Result: 在多个基准测试中，该方法使LLaDA 2.0超越了基于置信度的解码方法，显著提升了数学推理的可靠性。

Conclusion: Evolutionary Decoding有效克服了扩散模型的置信度陷阱，为提升扩散大语言模型的复杂数学推理能力提供了一种有效的测试时扩展策略。
...
TL;DR: 本文提出了FlowBlock，一种无需训练的并行解码框架，通过门控波前解码和异构波前打包机制，实现块间并行解码，显著提升扩散大语言模型的推理吞吐量并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 块间扩散大语言模型在块级别顺序解码，虽然能复用KV缓存，但块间解码严格串行成为性能瓶颈；现有基于后训练的并行方法加速有限且损害精度，因此需要一种无需训练且不损失精度的并行解码方案。

Method: 提出FlowBlock框架，包含两个核心机制：(1) 门控波前解码——当就绪门满足时将块纳入有界波前，通过T2T编辑联合精炼活跃块，并在窗口化块因果掩码下按序提交块，保持冻结前缀KV缓存的精确复用；(2) 异构波前打包——为每个请求分配独立波前，将异步窗口打包为密集且形状稳定的批量前向计算。

Result: 在多个基准测试中，相比LLaDA-2.1和LLaDA-2.0，TPS分别提升最高2.95倍和4.01倍，延迟分别降低最高53.6%和77.1%，平均精度提升1.3分；与基于训练的基线D2F相比，精度更高且批量服务吞吐量提升最高16倍。

Conclusion: FlowBlock证明了自校正dLLM的T2T编辑能力可将块间最终性从硬依赖转化为调度资源，无需额外训练即可实现高效并行解码，为扩散语言模型的推理加速提供了新范式。
...
TL;DR: 本文提出LaCache，一个针对扩散式大语言模型的无训练加速框架，通过无损缓存和混合精度消除算子级冗余计算，实现显著推理提速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式大语言模型在半自回归解码的去噪步骤中存在严重的算子级冗余，重复计算了块内保持不变的前缀和掩码后缀，造成计算资源浪费。

Method: 提出LaCache框架，利用无损状态记忆（LSM）缓存嵌入输出、预注意力状态和FlashAttention在线softmax统计以跳过不变token的冗余计算，并结合针对FFN层的逐组FP8量化策略缓解内存带宽瓶颈。

Result: 单独使用LaCache实现约1.3倍端到端加速；与现有加速方法结合时，在保持任务准确率相当的情况下，端到端加速最高可达40.2倍。

Conclusion: LaCache作为一种无需训练的加速方法，能有效消除冗余计算并缓解内存瓶颈，在不损失模型精度的前提下大幅提升扩散式大语言模型的推理效率。
...
TL;DR: 本文提出TAPS方法，通过将扩散模型的边际概率转化为路径条件接受估计，在固定验证预算下选择紧凑的前缀闭合子树，显著提升投机解码的加速效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的投机解码方法在验证阶段存在瓶颈：验证单个序列限制接受长度，验证大的草拟树产生过多目标模型延迟。更关键的是，现有方法按边际概率排序节点，忽略了验证是基于前缀条件的，可能验证被拒绝前缀的不可达后代，造成延迟增加而接受增益有限。

Method: 提出TAPS（target-aware prefix selection）方法，将扩散模型的边际概率转换为路径条件接受估计，并在固定验证预算下选择紧凑的前缀闭合子树，优化接受-成本权衡而非简单扩展草拟树。

Result: 在多个数据集和模型家族上的实验表明，TAPS相比原始自回归解码实现高达7.9倍无损端到端加速，分别比最先进的DFlash和DDTree方法快1.36倍和1.74倍。

Conclusion: TAPS通过解决扩散树方法中边际概率排序与前缀条件验证之间的不匹配问题，在固定验证预算下有效提升了投机解码的加速性能，为基于扩散模型的并行草拟提供了更优的验证策略。
...
### [33] [Exploring and Bridging Knowledge Holes in Unlearned Multimodal Large Language Models](http://arxiv.org/abs/2608.01849v1)
*Junxiang You,Junkai Chen,Yuhao He,Ruiqi Liu,Zhetao Guo,Shu Wu*

Main category: cs.AI

TL;DR: 本文提出SPAR方法，通过解决机器遗忘中的“知识空洞”问题，在保持模型效用的同时精准移除多模态大语言模型中的不安全内容。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型（MLLM）机器遗忘评估范式存在盲区，无法捕捉过度遗忘导致的“知识空洞”问题（即在与遗忘集共享通用模式的良性输入上发生严重性能退化），影响了遗忘的精确性。

Method: 提出选择性保护与锚定正则化（SPAR）方法，通过锚定激活过滤保护通用模式，并通过实体抽象增强来强化这些模式，从而弥合知识空洞。

Result: 在SafeEraser数据集上的实验表明，SPAR恢复了超过98%的原始响应质量（标准基线低于50%），实现了0.00%的攻击成功率，并保持了具有竞争力的模型效用。

Conclusion: 研究结果强调了在可信的MLLM机器遗忘中进行更细粒度评估的必要性，SPAR方法有效解决了良性相邻输入性能退化的问题。
...
### [34] [Douyin Multimodal Embedding Model Technical Report](http://arxiv.org/abs/2608.02148v1)
*Haonan Chen,Chu Li,Zhicheng Wang,Yuanwei Liu,Yuanjiang Wang,Shaohua Jiang,Zhicheng Dou*

Main category: cs.IR

TL;DR: 提出了DME模型，通过两阶段训练结合了对比学习的高效性和细粒度区分能力，实现了工业级多模态表征学习的突破。


<details>
  <summary>Details</summary>
Motivation: 现实中的大规模多模态平台需要兼顾十亿级索引的高效检索与困难匹配的细粒度区分，但现有MLLM嵌入模型难以同时满足：对比模型监督太粗，CoT模型在线推理成本太高。

Method: 提出两阶段训练的DME模型。第一阶段进行大规模对比预训练建立统一嵌入空间；第二阶段通过基于证据的类型化潜在推理和交叉条件重构两种机制补充语义充分性，两者仅在训练阶段生效，确保推理时具备与标准对比编码器相当的效率。

Result: 在MMEB-v2上，2B和9B版本达到同等规模的最优（分别为74.8和78.4分），视频和视觉文档任务表现突出；在抖音生产环境中，离线评估相对提升2.92%，在线A/B测试LT收益提升0.1%。

Conclusion: DME成功将对比模型的效率与CoT模型的细粒度区分相结合，并在抖音搜索等实际业务场景中部署落地，验证了其在工业级大规模多模态检索中的有效性。
