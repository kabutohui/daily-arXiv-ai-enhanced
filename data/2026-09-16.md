...
TL;DR: 本文提出了LLaDA-UI，一个基于块级扩散的167亿参数多模态GUI智能体，验证了扩散大语言模型在保持并行解码优势的同时能胜任GUI任务。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型具备高效的并行解码能力，适合实时感知屏幕状态并输出结构化操作的GUI智能体。本文旨在解决dLLMs能否在保留并行解码优势的前提下，扩展为具备多模态能力的GUI智能体这一开放性问题。

Method: 提出基于MoE的块级扩散视觉语言模型LLaDA-UI。采用两阶段训练：第一阶段进行通用多模态预训练，将原生分辨率视觉编码器与LLaDA2.0-mini-base扩散语言骨干对齐；第二阶段在多平台（移动、桌面、网页）的grounding和导航数据上进行有监督微调。

Result: 在多个主流的grounding和跨平台导航基准测试中，LLaDA-UI显著优于Qwen2.5-VL-7B，并在六项GUI基准测试中有四项超越了Qwen3-VL-8B。

Conclusion: 研究结果确立了块级扩散作为一种适用于多模态GUI智能体的实用生成范式，证明了其在延迟敏感应用中的潜力。
...
TL;DR: 本文提出基于离散扩散大语言模型的框架DRRG，通过迭代去噪生成放射学报告，克服了自回归模型无法修改且易错传播的缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归（AR）放射学报告生成方法逐个生成token且无法修改早期内容，容易导致错误传播，且与放射学报告的迭代修改过程不符。

Method: 提出DRRG框架将报告生成公式化为迭代掩码token去噪，引入临床实体感知互补掩码以提高token监督覆盖率，并设计概念条件模块将图像衍生的临床概念注入视觉表征。

Result: 在MIMIC-CXR数据集上，尽管使用更小的LLM解码器，DRRG在大多数指标上超越了对比方法（如BLEU-4达0.210）；在CheXpert Plus上取得了最高的BLEU-4（0.119）和CheXbert-F1（0.347）。

Conclusion: 离散扩散通过实现迭代和双向的报告修改，为自回归放射学报告生成提供了有效替代方案；结合临床聚焦掩码和图像衍生概念条件化可提升报告质量与临床一致性。
...
### [3] [Multi-modal Knowledge Preserving Adapter for Embedding Backward Compatibility](http://arxiv.org/abs/2609.16875v1)
*Jaeseok Byun,Gukyeong Kwon,Han-Kai Hsu,Meher Gitika Karumuri,Zhikang Zhang,Hao Yang,Davide Modolo*

Main category: cs.CV

TL;DR: 提出无需更新主干网络的仅适配器向后兼容训练方法（MKP-Adapter），以高效升级多模态嵌入模型并避免昂贵的数据库重索引。


<details>
  <summary>Details</summary>
Motivation: 升级嵌入模型常需昂贵的数据库重索引，而现有向后兼容训练（BCT）需更新主干网络，导致训练成本高、性能衰退风险及专有权重不可用等问题。

Method: 提出多模态知识保留适配器（MKP-Adapter），首个用于多模态大语言模型（MLLMs）的仅适配器BCT方法。引入多级保留损失以维持嵌入空间几何结构，结合焦点重加权策略优先学习困难样本，且仅在预提取的嵌入上训练。

Result: 在图像、文本、视觉文档及视频检索等多模态基准和多种模型类型上实现了强大的向后兼容性，且相比原始主干前向传播仅增加可忽略的延迟。

Conclusion: MKP-Adapter为多模态嵌入模型升级提供了一种高效、无需主干更新的向后兼容解决方案，规避了重索引和重训练主干的高昂成本。
...
### [4] [SAVOR: Self-Aware Visual Grounding via Confidence-Calibrated Reinforcement Learning for Multimodal Hallucination Mitigation](http://arxiv.org/abs/2609.16601v1)
*Zixiu Ding,Zilin Zhao,Yingjie He,Xinlang Kang,Guansu Wang,Wei Zhang*

Main category: cs.CV

TL;DR: 本文提出Savor训练框架，通过校准的自我评估信号，让多模态大语言模型学会表达置信度并在不确定时主动弃权或重新审视视觉证据，从而缓解幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs仍会产生与图像不符的流利声明。测试时修改解码会增加延迟；基于偏好的微调（如DPO变体）虽能教会模型选择更优答案，却无法告知模型自身答案何时不可靠。本文旨在弥补这一缺失的校准信号。

Method: Savor框架包含三个核心组件：(1) 在输出模式中增加token和答案的置信度；(2) 使用GRPO目标优化策略，惩罚校准误差和不良弃权决策；(3) 推理时利用学习到的置信度，仅在模型不确定时重新审视视觉证据。

Result: 在POPE、HallusionBench、AMBER和MMHal-Bench四个基准上，使用InternVL3-8B和Qwen3-VL-8B两个骨干模型进行实验，结果表明Savor减少了幻觉，同时在MME和MMBench上保持了通用能力，且相比DPO和基线方法获得了更低的期望校准误差。

Conclusion: 校准的自我评估是缓解MLLMs幻觉问题的关键信号。Savor通过显式建模置信度并基于GRPO优化，在不牺牲通用能力的前提下有效降低了幻觉和校准误差，为未来在多模态场景中实现可靠的不确定性量化提供了新方向。
...
### [5] [Circuit-MLLM: Topological Logic-Guided Latent-Space Visual Reasoning for Circuit Schematic Understanding](http://arxiv.org/abs/2609.15668v1)
*Jinyuan Deng,Yuqi Jiang,Wenjing Huang,Xin Li,Qi Sun,Cheng Zhuo*

Main category: cs.CV

TL;DR: 提出Circuit-MLLM框架，通过潜在空间中的器件定位、路径追踪和序列推理来解决多模态大模型解析电路原理图的难题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型难以处理具有密集布局和复杂拓扑逻辑的电路原理图，缺乏提取电气语义的细粒度结构解析能力。

Method: 提出Circuit-MLLM多模态推理框架，利用电路知识挖掘机制将视觉专家的结构特征与模型潜在表示对齐，并设计拓扑引导的序列化策略，沿电路拓扑逻辑在潜在空间中进行逐步推理。

Result: 在多种电路分析任务中，Circuit-MLLM表现优于强基线模型，平均得分较GPT-5.1高出25%。

Conclusion: 该框架在电路原理图拓扑分析中表现出极高的有效性，验证了拓扑逻辑引导潜在空间推理方法的可行性。
...
### [6] [VideoScout: Learning Agentic Active Exploration with Adaptive Reasoning Pacing for Long Video Understanding](http://arxiv.org/abs/2609.15606v1)
*Weixin Xu,Zhenyu Yang,Bing Wang,Shengsheng Qian,Changsheng Xu*

Main category: cs.CV

TL;DR: 本文提出VideoScout，一种基于序列证据获取（SEA）范式的多轮推理代理，通过动态控制观看速度在有限视觉上下文窗口内高效遍历长视频。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在长视频理解上受限于有限的视觉上下文窗口，现有均匀帧采样和粗到细代理缩放方法难以在长视频中定位稀疏的关键决策证据。

Method: 将长视频理解表述为序列证据获取（SEA）问题，提出VideoScout代理通过自适应推理节奏动态控制观看速度。构建VideoScout-66K数据集，采用两阶段训练：冷启动监督微调学习输出格式，以及解耦裁剪和动态采样策略优化（DAPO）算法进行轨迹级强化学习，使用包含答案准确性、格式合规性和时序对齐IoU的复合奖励函数。

Result: 在长视频理解和推理基准上的大量实验表明，7B VideoScout模型与现有训练的7B代理模型相比取得了强劲的性能表现。

Conclusion: VideoScout验证了SEA范式在长视频理解中的有效性，通过自适应观看速度控制和强化学习训练，能够在有限上下文窗口内高效处理长视频，为长视频理解提供了新的解决思路。
...
### [7] [MarKey: Marginal Utility Guided Greedy Keyframe Selection for Long Video Understanding](http://arxiv.org/abs/2609.15408v1)
*Hongchang Shi,Jinpeng Hu,Ao Wang,Wenzheng Zhou,Hui Ma,Feng Li,Zenglin Shi*

Main category: cs.CV

TL;DR: 提出了一种名为MarKey的无训练关键帧选择框架，通过子集感知的贪心优化策略有效提升多模态大语言模型在长视频理解任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 长视频理解中密集编码计算成本高且均匀采样易遗漏关键信息。现有无训练关键帧选择方法多独立评估帧，未考虑与已选子集的互补性，导致冗余选择和证据覆盖不全。

Method: 提出MarKey框架，将关键帧选择建模为子集感知贪心优化。通过综合查询相关性、边际覆盖增益和上下文冗余的替代指标对候选帧评分。利用代表性锚点近似全视频覆盖，并用有界窗口限制上下文比较以提升效率。

Result: 在六个涵盖整体、以人为中心及开放式视频理解的基准测试中，MarKey均优于现有方法。在不同MLLM骨干、模型规模和帧预算下均表现出稳健的性能提升。

Conclusion: MarKey有效解决了长视频理解中关键帧选择的冗余和覆盖不足问题，提供了一种高效且具有广泛适用性的无训练解决方案。
...
### [8] [Concept-Grounded Reasoning with Prompt-Driven Localization for Interpretable Structured Report Generation](http://arxiv.org/abs/2609.15334v1)
*Xinyue Xu,Hongbin Lin,Juangui Xu,Hualiang Wang,Lehan Wang,Lijie Hu,Weiyang Liu,Adrian Weller,Xiaomeng Li*

Main category: cs.CV

TL;DR: 本文提出CORAL框架，通过结合空间定位与概念级监督，提升多模态大语言模型在医学报告生成中的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在生成医学报告时多依赖端到端融合，缺乏对临床中间属性的建模，导致可解释性和定位能力不足。

Method: 提出CORAL框架，利用提示驱动的医学分割模型定位病灶，通过概念瓶颈模块预测临床属性，并将概念标记与掩码视觉特征结合输入MLLM以生成结构化报告。

Result: 在BUS-CoT和IU X-ray数据集上的实验表明，与通用及医学MLLMs相比，CORAL在诊断准确率、概念一致性和报告质量上均有持续提升。

Conclusion: 基于概念接地的推理能够更好地将医学报告生成与临床决策过程相统一。
...
### [9] [Enhancing Accessibility of Medical Texts through Large Language Model-Driven Plain Language Adaptation](http://arxiv.org/abs/2609.17398v1)
*Ting-Wei Chang,Hen-Hsen Huang,Hsin-Hsi Chen*

Main category: cs.CL

TL;DR: 本文探讨了利用大语言模型和混合代理技术将复杂医疗信息自动转化为通俗易懂语言的方法与有效性。


<details>
  <summary>Details</summary>
Motivation: 旨在弥合复杂医疗文本与患者阅读理解能力之间的鸿沟，通过自动化通俗语言改编（PLA）简化专业医学术语。

Method: 利用GPT-4o-mini、Gemini-1.5-pro和LLaMA等LLM进行零样本/少样本文本简化，整合混合代理（MoA）技术提升鲁棒性，对比不同提示策略，并使用QLoRA进行微调。

Result: 实验结果证明了基于LLM的PLA方法的有效性，该方法在保留关键医疗内容的同时显著提升了文本的可理解性。

Conclusion: LLM及其相关技术（如MoA和QLoRA微调）在自动化医疗信息简化方面具有巨大潜力，能更好地服务患者。
...
### [10] [Early-Bird Decoding: Accelerating Diffusion LLMs with Learnable Block Sizes and Parallel Sampling](http://arxiv.org/abs/2609.16450v1)
*Lixuan Wei,Wei Zhou,Jianwen Wu,Yipeng Shen,Meiling Wang,Haoran You*

Main category: cs.CL

TL;DR: 提出了一种早期解码框架(EB-Decode)，通过自适应分组和并行解码低熵token，显著加速扩散大语言模型的推理过程。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在并行解码时，token置信度达到解码阈值前需要许多迭代步骤，即使使用块级KV缓存，推理效率仍然低下。

Method: 提出EB-Decode框架，包含两个关键组件：1)可学习网络自适应地将具有相似不确定性的token分组为可变长度块，而非固定块大小；2)位置感知采样器学习在预测的可变长度块内使用更少解码步骤并行解码token。这两个组件无需修改预训练dLLM权重，可作为插件直接部署。

Result: 在三个模型和四个基准上的实验一致验证了观察结果和EB-Decode的有效性，相比原始解码方法实现了3.53-18.76倍的吞吐量提升，相比最强基线Fast-dLLM实现了高达1.58倍的吞吐量提升，且准确率相当。

Conclusion: EB-Decode框架能够在不修改预训练权重的情况下显著提高dLLM的推理效率，训练和推理开销可忽略不计，为dLLM的实际部署提供了有效解决方案。
...
### [11] [Register Tokens for Bounded-State Reasoning in Diffusion Language Models](http://arxiv.org/abs/2609.16372v1)
*Albert Ge,Chandan Singh,Yufan Zhuang,Xiaodong Liu,Jianfeng Gao,Frederic Sala*

Main category: cs.CL

TL;DR: 本文提出在掩码扩散语言模型中使用寄存器词元作为固定大小的携带状态，以在生成块之间清除文本后继续进行推理。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型通常需要保留先前生成的文本在上下文中以扩展推理。本文旨在解决dLLM在先前文本被清除后如何仅使用固定大小的携带状态来继续推理的问题。

Method: 提出一种使用“寄存器词元”的方法，这些是专门固定位置的词元，其连续隐藏状态被训练用于跨生成块携带推理进度。通过对dLLM进行后训练，使其解码文本块、清除文本但保留寄存器值，然后从提示和携带状态继续解码。

Result: 在LLaDA和Dream上的实验表明，寄存器在所有基准测试中均优于离散文本携带，数学任务提升了8.5分，代码任务提升了19.5分。该方法在有界代码生成中尤其有效，并可通过在长时序推理任务上进行强化学习来进一步优化。

Conclusion: 寄存器词元为dLLM跨生成块进行推理提供了一种有效且紧凑的连续状态机制，在数学和代码任务上取得了显著提升，并可通过强化学习进一步优化。
...
### [12] [In-Place Instruction Following in Diffusion Language Models](http://arxiv.org/abs/2609.07160v1)
*Zheng Nie,Zherui Li,Jiaming Zhang,Kun Wang,Zhenhong Zhou,Yufei Guo*

Main category: cs.CL

TL;DR: 本文提出了针对扩散大语言模型（dLLMs）的原位指令遵循（IIF）任务及基准IIF-Bench，并提出了后训练框架GRAFT以显著提升模型在任意输出位置满足约束的能力。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在去噪过程中往往对用户指定的约束片段重视不足，导致难以有效满足任意输出位置的约束要求。

Method: 提出了面向原位提示（IPP）的后训练框架GRAFT，结合约束感知监督微调（SFT）和偏好优化；同时构建了层次化基准IIF-Bench及基于评分标准的局部-全局评估协议。

Result: 在四个代表性dLLMs上，GRAFT将平均IIF得分从57.75提升至73.10（+15.35分），在字面和语篇功能约束上分别取得15.91和15.57分的绝对增益，同时保持了通用生成能力。

Conclusion: 通过约束感知的SFT和偏好优化，GRAFT有效增强了dLLMs在任意位置遵循约束的能力，且未损害其基础生成性能。
...
### [13] [A Ticket from Marginals to Joints: Coupled-Noise Distillation for One-Step Block Generation in Diffusion Language Models](http://arxiv.org/abs/2609.06324v2)
*Lin Yao*

Main category: cs.CL

TL;DR: 本文提出CONDOR方法，通过高斯噪声场扰动掩码嵌入，训练离散掩码模型在单次前向传播中生成连贯的文本块。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在单次前向传播中并行预测token时，各位置独立采样导致生成的块缺乏连贯性。本文旨在解决如何使掩码模型在一次传播中提交整个连贯文本块的问题。

Method: 提出CONDOR方法，无需目标侧编码器或自回归教师。训练结合两个信号：一是对真实文本在多个噪声样本下预测掩码token，仅用最拟合真实文本的样本监督；二是对其余样本在固定噪声下进行多步解码细化，再将细化结果蒸馏回单次传播。

Result: 在TinyStories受控环境中，该方法生成了随噪声变化且连贯的单次传播续写，不仅适用于单个块，也适用于块因果变体的连续生成。

Conclusion: CONDOR成功实现了无自回归教师的单次传播连贯文本块生成，验证了噪声耦合与蒸馏机制在扩散语言模型中的有效性。
...
### [14] [Beyond Token Positions: Safety Alignment Across Denoising Steps in Diffusion Language Models](http://arxiv.org/abs/2609.00495v1)
*Guoli Wang,Haonan Shi,Tu Ouyang,An Wang*

Main category: cs.CL

TL;DR: 本文提出了一种无需训练的解码方法RAEC，通过在早期去噪步骤中提交持久的拒绝信号，提升了扩散式大语言模型的安全性并基本保持了实用性。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型通过迭代去噪而非从左至右解码生成文本，此生成范式引入了影响安全对齐的两个维度（token生成时间和出现位置）。本文旨在探究dLLM在有害提示下的安全行为机制。

Method: 提出了Refusal-Aware Early Commitment (RAEC) 方法。这是一种无需训练的解码策略，通过追踪去噪过程中的token分布与提交决策，在早期去噪步骤强制提交持久的拒绝信号。

Result: 实验发现拒绝信号集中在早期去噪步骤和响应开头的位置，早期提交的token强烈影响最终的安全结果。在LLaDA和Dream模型上的实验表明，RAEC降低了攻击成功率，同时基本保持了模型实用性。

Conclusion: 去噪步骤和拒绝token提交的持久性对理解dLLM安全行为至关重要。RAEC方法提供了一种简单有效的途径来增强扩散式大语言模型的安全对齐。
...
### [15] [Trajectory-Level Speculative Decoding for Diffusion Language Models](http://arxiv.org/abs/2608.27514v1)
*Tianxiang Pan,Baitao Gong,Mo Guang,Hongwei Yong,Tianpeng Jiang,Yaqian Li,Zheng Cao,Kaiwen Long*

Main category: cs.CL

TL;DR: 本文提出了一种轨迹级推测解码框架，通过置信度分层树探索和块级并行评估，显著提升了扩散语言模型的生成速度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型在低置信度下会退化为单token生成，严重限制了吞吐量；且其多token更新机制使得传统自回归推测解码无法直接适用，因此需开发针对去噪轨迹的推测框架。

Method: 提出一种轨迹级推测框架，利用置信度分层树探索构建草稿去噪轨迹，并通过双向注意力掩码进行块级并行评估验证；同时引入块间推测实现跨块前瞻。该方法基于Fast-dLLM的双缓存基础设施构建。

Result: 将去噪迭代次数减少30-40%，每步生成token数从2.6增至4.3。在推理和代码基准测试中，相比原始dLLM实现7-14倍加速，相比Fast-dLLM实现1.3倍加速，且准确率变化不足1%。

Conclusion: 形式化刻画了该方法的精确条件，并指出轨迹漂移是增加并行度的根本代价；该框架在几乎不损失精度的前提下大幅提升了dLLMs的解码效率。
...
### [16] [Dependency-Aware Revocable Decoding for Efficient Diffusion Large Language Model Inference](http://arxiv.org/abs/2608.26574v1)
*Wooje Park,Insu Lee,Minyoung Noh,Jaeyun Jang,Sungmin Lee,Kyuhong Shim,Byonghyo Shim*

Main category: cs.CL

TL;DR: 本文提出了一种名为DARD的免训练框架，通过分离token状态并使用选择性上下文进行验证，解决了扩散大语言模型中可撤销解码的验证上下文损坏问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型通过并行解码提升速度，但早期错误会污染后续上下文从而降低质量。现有可撤销解码方法忽略了不可靠token会破坏验证上下文本身这一缺陷，本文旨在解决该问题。

Method: 提出依赖感知可撤销解码（DARD）免训练框架。将token分为掩码、候选和未掩码三种状态，利用排除不可靠token的选择性上下文验证候选token，并自适应调节其对后续解码的影响。

Result: 在3个开源dLLMs的12个文本和多模态基准测试中，DARD持续提升了相比最新可撤销解码方法的速度-质量帕累托前沿，在Flickr30K上较Saber实现了2.71倍加速和4.35分的CIDEr分数提升。

Conclusion: DARD有效缓解了不可靠token破坏验证上下文的问题，在多个基准测试上实现了更优的速度与质量平衡，为扩散大语言模型的并行解码提供了有效方案。
...
TL;DR: 本文提出了一种无需训练的树构建方法DominoTree，利用路径依赖的条件校正评分，显著提升了LLM推理的接受长度和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的树方法（如DDTree）的分解公式无法表示像Domino这样基于GRU的路径依赖草稿token分布。

Method: DominoTree是一种无需训练的最佳优先草稿树，它沿每条根到节点的路径使用Domino的条件（非分解）校正进行评分，并通过将每个节点的校正限制在候选top-M内来提高实用性。

Result: 在8个基准测试中，DominoTree在所有服务单元中达到最高平均接受长度；吞吐量相比自回归最高提升7.3倍，在SGLang中比Domino单请求吞吐量提升12%；在HELMET长上下文基准上，接受长度比Domino提升29-36%。

Conclusion: 利用路径依赖的条件校正可显著提升投机解码的接受长度和吞吐量；但在极端长上下文且预填充占主导时，其优势缩小至持平。
...
TL;DR: 提出GRAFT框架，通过目标蒸馏边缘评分和状态感知预算分配，优化基于扩散语言模型的树状推测解码，实现显著的端到端加速。


<details>
  <summary>Details</summary>
Motivation: 现有的树构建方法采用父节点条件扩展，与单次前向传递生成所有未来分布的扩散语言模型(DLM)草稿器不兼容；且现有DDTree方法仅依赖概率选择边、使用固定预算，导致父子节点兼容性差且无法适应解码状态变化。

Method: 提出GRAFT框架，包含目标蒸馏边缘评分(TDES)和状态感知预算分配(SABA)。TDES从目标模型轨迹中提取父子偏好以选择目标兼容边；SABA通过平衡预期草稿增益与验证成本来动态设置每轮树预算。

Result: 在多个模型和任务上，GRAFT相比自回归解码实现了2.13倍至6.36倍的端到端加速，每轮额外开销不到0.5毫秒，仅约占目标模型验证延迟的1.4%。

Conclusion: GRAFT有效弥合了树构建与DLM草稿器之间的差距，在极低计算开销下显著提升了大型语言模型的推理效率。
...
TL;DR: 本文提出CaDDTree，通过联合优化树结构和节点预算直接最大化令牌吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如DDTree）以固定预算最大化预期接受长度，但因接受长度随预算非递减，导致其总是倾向于构建更大的树而忽略验证成本，无法为预算选择提供原则性依据。

Method: CaDDTree显式建模草稿与验证延迟，将吞吐量目标转化为每轮针对预算的一维搜索；在凸验证成本假设下证明吞吐量函数为单峰，从而采用贪婪停止规则；无需离线搜索，每轮动态自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型及八个基准（涵盖推理、编程和指令遵循）上的实验显示，CaDDTree在几乎所有任务上均达到或超过了使用最优预算的DDTree。

Conclusion: CaDDTree有效解决了推测解码中节点预算选择缺乏原则性依据的问题，实现了无需离线调优的动态吞吐量优化。
...
TL;DR: 本文提出DDTree方法，通过块扩散模型构建草稿树并在单次前向传播中验证，提升了推测解码的性能与接受长度。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash方法在推测解码中每轮仅验证单一草稿轨迹，限制了接受长度和潜在的加速效果。

Method: 提出DDTree，直接利用块扩散模型的逐位置分布构建草稿树；在固定节点预算下，采用最佳优先堆算法选择最可能匹配目标模型的续写，并通过仅祖先注意力掩码在单次目标模型前向传播中高效验证。

Result: DDTree基于DFlash实现了性能提升，成为推测解码领域的领先方法之一。

Conclusion: DDTree突破了单轨迹验证的局限，有效提升了推测解码的效率和接受长度，确立了其在推测解码方法中的领先地位。
...
### [21] [ENCP: Episode-Normalized Conformal Prediction for Vision-and-Language Navigation](http://arxiv.org/abs/2609.17499v1)
*Vicky Feliren,A. Taufiq Asyhari,Muhamad Risqi U. Saputra*

Main category: cs.LG

TL;DR: 本文提出了Episode归一化共形预测（ENCP）框架，用于解决视觉-语言导航（VLN）中变长序列决策下标准共形预测无法保证覆盖率的问题。


<details>
  <summary>Details</summary>
Motivation: VLN模型的不确定性估计对于识别模糊和不可靠的预测、实现安全导航至关重要。然而，由于VLN智能体需要进行一系列步骤的决策，标准共形预测的校准方法无法在依赖且变长的VLN episode中提供其承诺的覆盖率保证。

Method: 提出Episode归一化共形预测（ENCP），通过策略的残差置信度对非一致性得分进行重缩放，并在每个episode中校准单个最大得分。该构造允许episode内各步骤间存在依赖性，并在每一步以至少1-α的概率覆盖真实值。

Result: 在R2R和REVERIE数据集上，针对四种VLN策略和三种非一致性得分，ENCP在已见至未见评估中满足了所有报告的经验步骤覆盖目标。

Conclusion: ENCP能够提供与模型无关的不确定性估计，可用于确定VLN智能体何时应推迟决策并寻求更强大的预测器（如人类协助）。
...
### [22] [Temporal Self-Distillation: Faster Inference in Discrete Diffusion Language Models](http://arxiv.org/abs/2609.15177v1)
*Shijian Xu,Andrea Miele,Metod Jazbec,Volker Roth,Eric Nalisnick,Ilija Bogunovic*

Main category: cs.LG

TL;DR: 本文提出了时间自蒸馏（TSD）方法，通过跨时间步蒸馏模型自身的去噪分布，有效加速了扩散语言模型的并行解码并提升了性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在过度激进的并行解码下会出现严重的性能下降，本文旨在解决这一问题以实现快速且高质量的推理。

Method: 提出时间自蒸馏（TSD）方法，将模型早期时间步的去噪分布蒸馏至最终时间步的分布，由于教师信号来自模型自身，该方法无需离线教师生成，且适用于基础和后训练策略。

Result: 在涵盖数学、规划和代码的七个基准测试中，TSD显著将速度-质量前沿推向低计算量区间，实现了与离线蒸馏相媲美的加速效果。

Conclusion: TSD提供了一种简单、单阶段的加速扩散语言模型的方法，无需复杂的两阶段流程，同时有效避免了传统离线蒸馏的局限性。
...
### [23] [ScienceBuddy: Recursive-in-Recursive Self-Improvement for Interactive Scientific Agents](http://arxiv.org/abs/2609.17523v1)
...
TL;DR: 本文发布了一个名为ScienceBuddy的交互式科研工作空间，它通过递归式自我改进范式将不断进化的科研智能体融入研究人员的日常工作中。


<details>
  <summary>Details</summary>
Motivation: 本文旨在支持研究人员开展科学任务，并将他们的请求、反馈和执行证据转化为持续学习的任务和评估准则，从而推动能够通过与研究人员持续协作而进化的科学人工智能（发现智能）的发展。

Method: 核心方法是一种“递归中递归”的自我改进范式：内层递归在固定模型的情况下改进执行框架，外层递归则在改进的框架下训练模型，将框架进化与模型强化学习相耦合。

Result: 主要结果展示了涵盖四个科学任务家族的研究人员交互、框架改进和模型学习的案例研究，并将ScienceBuddy作为研究产品公开发布。

Conclusion: 关键结论是ScienceBuddy向科学社区提供了一种持续自我改进的范式，使AI能够通过持续协作推进研究并与之共同进化，从而在发现智能方面迈出了一步。
...
### [24] [Verifiable Social Reasoning for LLM Assistants](http://arxiv.org/abs/2609.17496v1)
...
TL;DR: 本文提出了多智能体模拟框架Fuse，用于评估大语言模型在用户介导的社交咨询场景中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在社交咨询中的社交推理能力具有挑战性，因为它依赖于从主观用户叙述中获取社交情境，且社交属性（如他人意图）通常缺乏可验证的基准真值。

Method: 提出Fuse多智能体模拟框架，构造具有隐藏动机的目标智能体与其他智能体（包括用户）交互，随后用户向被评估的LLM咨询以推断目标动机，从而提供可验证的基准真值；并通过2.4万条人工标注验证了模拟的忠实度。

Result: 对12个LLM的评估表明：用户介导加剧了社交推理的固有难度；LLM对带有偏见的用户框架表现出系统性敏感；模型做出正确预测可能需要比人类更多的细节；更长的对话并不总能提升性能。

Conclusion: Fuse为大语言模型在主观用户介导社交场景中的推理提供了有效且可验证的评估工具，揭示了模型在处理偏见、信息需求及对话长度方面的局限性，并已开源框架及2.1万条示例数据集。
...
### [25] [FlashVector: Agent for Hierarchical Model Serving Stack Optimization](http://arxiv.org/abs/2609.17391v1)
...
TL;DR: 本文提出了FlashVector，一个跨越模型服务栈所有层级进行性能优化的智能体系统，在Unity广告平台部署后显著提升了吞吐量并降低了延迟。


<details>
  <summary>Details</summary>
Motivation: 模型服务是推荐系统中最大的成本驱动因素。最大化吞吐量需要跨越GPU内核、ML框架计算图、模型服务器和特征处理等多层级的领域专业知识，这难以获取且无法随工作负载的增长而扩展。同时，现有AI智能体仅能优化独立的GPU内核，服务栈其余部分的自动化调优仍待探索。

Method: 提出FlashVector智能体系统，其核心贡献是构建一个可扩展框架，将单内核优化智能体范式泛化至异构技术栈，从而在模型服务栈的所有层级（如GPU内核、计算图、C++模型服务器及Python特征转换服务）实现整体性能优化。

Result: 在Unity Vector广告平台部署后，模型服务器吞吐量提升最高达2倍，延迟加速最高达1.98倍；特征存储吞吐量提升最高达1.6倍。优化涵盖了GPU内核、计算图、NVIDIA Triton C++代码库及Python代码库，验证了框架的跨层级优化能力。

Conclusion: FlashVector证明了将AI智能体范式扩展至异构技术栈和复杂系统架构的可行性，实现了跨层级的整体性能优化，为自动化优化完整模型服务栈提供了有效范式与未来方向。
...
TL;DR: 本文提出了TAPS方法，通过目标感知的前缀选择策略优化扩散模型投机解码中的验证瓶颈，实现了高达7.9倍的无损端到端加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽能降低投机解码的起草延迟，但将瓶颈转移至验证阶段。现有方法忽略验证的前缀条件性，导致验证不可达节点并增加延迟。

Method: 提出TAPS（目标感知前缀选择方法），将扩散边际转化为路径条件的接受估计，在固定验证预算下选择紧凑的前缀闭子树以优化接受-成本权衡。

Result: 在多数据集和模型上的实验显示，TAPS实现了高达7.9倍的无损端到端加速，相比SOTA方法DFlash和DDree分别快1.36倍和1.74倍。

Conclusion: TAPS通过优化前缀选择有效解决了扩散模型投机解码的验证瓶颈，实现了显著推理加速，为该领域提供了更优的接受-成本权衡方案。
...
### [27] [Sparse MLLM Anchors, Dense Adaptation: Breaking the Self-Referential Loop in Wild Test-Time Adaptation](http://arxiv.org/abs/2609.17040v1)
*Zhenbin Wang,Lei Zhang,Lituan Wang,Yan Wang,Zhao Zhang,Wei Huang*

Main category: cs.AI

TL;DR: 本文提出MASA方法，通过引入冻结的多模态大语言模型（MLLM）提供的语义描述，解决狂野测试时自适应（WTTA）中模型因自我参照信号导致错误强化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的WTTA方法依赖正在被适应的模型本身获取自适应信号。当源模型在分布偏移下不可靠时，这些信号会强化模型自身的错误，形成自我参照的死循环。

Method: MASA利用冻结的MLLM为一小部分多样化且按可靠性排序的锚点样本生成结构化语义描述。该方法将描述编码并传播给邻近测试样本，存入在线原型记忆中；随后通过感知描述的记忆检索，为归一化仿射参数的轻量级自适应提供辅助目标。

Result: 在WTTA ImageNet-C基准上，针对有限批次、混合域和标签不平衡偏移等设置，使用ResNet和ViT骨干网络对MASA进行了评估。

Conclusion: 结合外部MLLM语义证据并采用轻量级在线记忆与参数调整，MASA有效打破了WTTA中的自我参照错误循环，提升了模型在复杂分布偏移下的鲁棒性。
...
### [28] [Layers, Sinks, and Scaling: Adaptive Evidence Selection for Multimodal Large Language Models](http://arxiv.org/abs/2609.16795v1)
*Zhenbin Wang,Lei Zhang,Lituan Wang,Wei Huang,Yan Wang,Zhenwei Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种无需训练的推理时方法AREA，通过自适应分配视觉和文本证据高亮，解决了多模态大语言模型在知识密集型视觉问答中容易忽略相关证据的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在结合图像和外部知识回答问题时，常忽略关键的视觉和文本证据。现有方法采用固定单次高亮策略，无法适应是否需要高亮、需要多少证据以及何时在生成中刷新文本这三个变化因素。

Method: 提出自适应相关性引导证据分配（AREA）方法，利用单个探测token从固定骨干层读取多模态相关性，并做出三项决策：是否干预（基于注意力覆盖率和visual sink污染）、暴露多少证据（基于相关性熵）以及何时刷新文本（基于因果上下文注意力峰值）。

Result: 在4个知识密集型视觉问答（KB-VQA）基准和7个标准多模态基准上，使用9个冻结的MLLM检查点进行测试，AREA在所有无需训练的高亮方法中取得了最佳性能。

Conclusion: 将证据高亮建模为自适应分配机制能有效弥补MLLMs的注意力缺陷，且无需任何模型训练，为高效提升多模态推理能力提供了新范式。
...
### [29] [VideoMM: Adaptive Macro-Micro Inference for Efficient Video MLLMs](http://arxiv.org/abs/2609.16722v1)
*Haoyu Guo,Yuan Feng,Junlin Lv,Mingjun Xiao,S Kevin Zhou,Xike Xie*

Main category: cs.AI

TL;DR: 提出VideoMM框架，通过自适应感知粒度将语义选择与推理解耦，实现高效且高精度的长视频理解。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在长视频理解中视觉token爆炸导致上下文饱和和高昂成本的问题，并克服现有轻量级模型丢失语义或重量级模型效率低下的两难困境。

Method: 将选择与推理过程解耦，在由降采样帧生成的低成本“宏观代理”上执行语义过滤，并在需要详细理解时将选定区域投影到高保真的“微观Token”上。

Result: 在LongVideoBench上，与全上下文基线相比实现6.13倍加速和7.4%的准确率提升，并比当前领先方法进一步加速2.73倍。

Conclusion: VideoMM成功将模型中心的缩减转变为自适应感知粒度，为长视频理解建立了一个高度可扩展的范式。
...
### [30] [HarnessVLN: Unifying Training-Free Embodied Navigation through an Agent Harness](http://arxiv.org/abs/2609.15195v1)
*Yang Chen,Lirong Che,Zhenyu Huang,Wenbo Fu,Chuang Wang,Xu Cao,Daqi Liu,Yuzhe Yang,Jian Su,Lan-Zhe Guo*

Main category: cs.RO

TL;DR: 本文提出了一种零样本、无需训练的具身导航框架HarnessVLN，通过Agent Harness协调各模块并验证动作提议，在多个基准上取得了超越以往无训练SOTA的性能。


<details>
  <summary>Details</summary>
Motivation: 基于训练的方法存在泛化困难，而基于多模态大语言模型的无训练方法缺乏将动作与空间证据、任务进度及执行失败进行有效协调的机制。

Method: 提出HarnessVLN框架，通过统一的工具接口协调感知、检索、接地等模块；利用分层事件记忆跟踪进度，持久化时空图维护空间证据和失败注释；可替换的导航执行器将验证后的目标转化为可执行动作。

Result: 在R2R、RxR、HM3D-v2和HM3D-OVON基准上的成功率分别达60.8%、53.9%、76.0%和59.3%，超越以往无训练SOTA，并在真实环境的人形机器人部署中得到验证。

Conclusion: HarnessVLN有效解决了无训练具身导航中动作与空间证据协调不足的问题，支持指令跟随和目标对象导航，并在仿真和真实世界中均展现出强大的应用潜力。
...
### [31] [Agentic Societies Need a Social Harness](http://arxiv.org/abs/2609.17527v1)
*Tapan Chugh,Vidushi Singh,Krish Jain,Arvind Krishnamurthy,Ratul Mahajan*

Main category: cs.MA

TL;DR: 本文提出一种用于AI代理社会的社会框架分层架构，以解决代理间通信漏洞导致的协作失败和恶意攻击问题。


<details>
  <summary>Details</summary>
Motivation: 在跨信任边界、目标仅部分对齐的AI代理社会中，现有通信机制和测试套件无法保证诚实代理达成满意结果，且故障或恶意代理会利用通信（“言论”）漏洞阻碍协作、影响结果并追求有害目标。

Method: 提出一种补充个人框架的分层社会框架架构，其核心机制包括：完全防止特定类别的故障、在运行时检测无效消息，以及支持事后调查与问责。

Result: 实验表明，在现有测试套件和消息传递原语下，即使诚实且胜任的代理也常无法达到满意结果，同时恶意代理能有效利用通信漏洞破坏协作并造成危害。

Conclusion: 结论是代理社会不仅需要管理私有上下文的个人框架，更需要规范代理间交互的社会框架；未来研究应致力于实现并完善这些社会框架的能力。
...
### [32] [Decomposition Buys Integrity, Not Yield](http://arxiv.org/abs/2609.17464v1)
*Rong He*

Main category: cs.MA

TL;DR: 本文通过概率树模型分析多智能体系统中任务分解对信息传递率（产出）的影响，发现扁平结构产出最优，但深度结构能降低成本和上下文暴露。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统常以缩小上下文等理由进行任务拆分，本文旨在探究这种拆分如何影响叶子节点的发现传递到根节点的比例。

Method: 将任务分解建模为树结构，其中处理b个项目的智能体保留一项的概率为r(b)，分析了r(b)=1/b和r(b)=Cb^{-δ}情况下的产出，并结合大规模生产环境数据进行验证。

Result: 理论上扁平结构对产出最优且无法逃脱指数δ的限制；实证测得δ=0.34，每层保留率C=0.571，对齐惩罚Cμ=0.536。深度结构将根上下文暴露降至N^{1/k}并降低成本。仅0.7%到11.3%的会话值得委派，且委派多为开场动作而非应对上下文填满。

Conclusion: 扁平架构在产出上最优，而深度结构在降低成本与上下文暴露上具有优势；生产环境中多数会话不值得委派，且现有的委派行为未根据上下文状态做出合理响应。
...
### [33] [A Time-to-Collision Barrier Function Approach to Collision Avoidance for Stochastic Systems](http://arxiv.org/abs/2609.17347v1)
*Benedikt Barthel Sorensen,Mitchell Black,Erfaun Noorani,Themistoklis P. Sapsis*

Main category: eess.SY

TL;DR: 本文提出了一种基于对抗性碰撞时间（aTTC）的控制屏障函数，使自主系统能够利用对手的动态限制实现预期性避障。


<details>
  <summary>Details</summary>
Motivation: 传统的避障约束通常在位置或速度空间中公式化，仅对几何邻近性做出隐式反应，缺乏对潜在碰撞的预见性，且无法利用追击者的动态限制作为规避策略。

Method: 提出一种基于对抗性碰撞时间（aTTC）的新范式，直接在时间域上定义控制屏障函数（CBF）。由于精确计算aTTC需积分完整系统动力学，研究采用轻量级神经网络代理模型，以支持基于实时二次规划的控制律。

Result: 在2D对比研究和3D多智能体追逃场景的验证中，基于aTTC的CBF方法优于高阶基于距离的基线方法，在对抗速度优势显著的追击者时更有效地争取了规避时间。

Conclusion: 基于aTTC的CBF方法通过在时间域内规划规避策略，使智能体能够调节速度并利用追击者的动态极限，从而有效对抗速度更快的追击者。
...
### [34] [Evaluating Verified Autonomy in Quantum Engineering](http://arxiv.org/abs/2609.17439v1)
*Naixu Guo,Changhao Li,Siyu Cheng,Qicheng Tang,Binzhao Luo,Bikun Li,Yuxuan Du,Shihao Ru,Jiaqi Cai*

Main category: quant-ph

TL;DR: 本文开发了Quantum-Harbor虚拟实验室及QIQCBench基准测试，用于系统评估AI智能体在量子工程任务中的可靠性表现。


<details>
  <summary>Details</summary>
Motivation: 随着量子平台规模与复杂度的增长，其表征和操作需要大量人力与协调；尽管科学AI智能体被视为实现自主量子工程的有前景途径，但当前智能体在此场景下的可靠性尚未得到系统性验证。

Method: 开发了Quantum-Harbor虚拟实验室，提供受控执行环境使智能体与量子系统交互，并支持对所采取行动及所得结论的直接验证；在此基础上构建了QIQCBench基准，包含49项专家设计的任务，涵盖校准与控制、纠错与编译、传感与网络等多个层面。

Result: 在17个前沿智能体系统上的测试表明，各系统的经验证性能存在巨大差异，暴露出能力展示与可靠操作之间的显著差距。

Conclusion: Quantum-Harbor为衡量量子工程领域自主性进展奠定了基础，结果指出了当前AI智能体在可靠操作方面的不足及未来改进方向。
...
### [35] [Coding Agents Have Converged: Why the SWE-bench Leaderboard Can No Longer Order Its Top Entries, and What to Measure Instead](http://arxiv.org/abs/2609.17394v1)
*Fengshuo Liu,Ying Liu,Ruize Sun,Lie Luo,Siyuan Guo*

Main category: cs.SE

TL;DR: 本文审计了SWE-bench排行榜上的微小分数差异是否足以支持系统排序，发现多数相邻排名在统计上无法区分。


<details>
  <summary>Details</summary>
Motivation: 现有编码智能体排行榜上的微小分差常被解读为系统优劣排序，但缺乏对这些差异是否具有统计显著性和实际意义的系统性检验。

Method: 对四个数据分割上的254个SWE-bench提交进行无模型运行的审计分析，包括：分析成功集共享程度（嵌套度）、配对McNemar检验评估相邻排名差异显著性、模型-支架配对效应的交互检验，并发布分区数据和五步审计协议。

Result: 顶尖系统成功集高度共享（嵌套度0.935）；模型内支架效应差异可达29.8个百分点；Verified分割上29对相邻排名在alpha=0.05下均无显著差异，而Test分割分离了23对中的14对；Holm校正后分为两个描述性层级。

Conclusion: 小幅度聚合分数差距不应被解释为确定的排名差异；建议报告特定比较集的分辨率和模型-支架来源信息，而非简单依赖总分排名。
