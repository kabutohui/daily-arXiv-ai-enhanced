...
TL;DR: 本文提出了智能体原生视频表示框架AVA-Encoder，通过将视频转换为知识图谱进行自动编码，使智能体能够学习并生成电影级视频。


<details>
  <summary>Details</summary>
Motivation: 创意智能体缺乏从高质量人类电影中学习的有效途径，限制了其制作电影级视频的能力，主要原因在于缺乏既忠实于电影内容又直接可用于智能体推理和操作的结构化视频表示。

Method: 提出AVA-Encoder框架，将视频转换为知识图谱（KG）并重构回视频。KG包含存储结构化文本的层次状态节点和保存生成资产的资产层，并利用类型化边保留关系。视频重构差异驱动了文本梯度优化框架，用于策略伪训练和知识图谱表示细化。

Result: 实验表明，AVA-Encoder比最强外部基线提高了20.7个百分点；在仅限策略的设置中，其伪训练策略优于人工微调策略，且系统提示词token减少了74.3%。

Conclusion: AVA-Encoder有效提升了智能体的视频创作能力，研究同时发布了完整的框架、可靠的智能体视频重构基准以及首个高质量电影知识图谱表示数据集。
...
TL;DR: 提出了一种基于扩散模型的航空视觉语言导航框架DreamFly，通过因果对齐记忆、滚动时域规划和显式终止机制有效提升了导航性能。


<details>
  <summary>Details</summary>
Motivation: 旨在解决视觉-语言-动作（VLA）模型应用于航空导航时面临的历史上下文有限、规划时域短以及隐式终止不可靠的挑战。

Method: DreamFly基于Dream-VLA构建，引入了因果对齐的历史记忆以防止未来信息泄漏，将导航表述为滚动时域扩散规划（预测K步动作仅执行第一步），并通过LiteStop直接从动作逻辑值估计停止概率以实现显式终止。

Result: 在OpenFly基准测试中，DreamFly在可见/不可见测试集上的成功率（SR）达到32.04%/29.46%，SPL达到28.22%/23.54%，优于所有对比方法且导航误差最低。

Conclusion: 联合建模历史上下文、未来动作结构和显式终止机制对于航空视觉语言导航任务是有效的。
...
TL;DR: 提出“智能体自我改进”框架，通过闭环优化解决黑盒图生视频模型缺乏精细控制的问题，显著提升了生成结果的可预测性与质量。


<details>
  <summary>Details</summary>
Motivation: 现代黑盒图生视频模型因内在随机性导致输出不可控，微小参数变化即产生巨大差异，迫使专业工作流依赖低效的暴力试错。本文旨在解决此缺乏可靠性和精细控制的问题。

Method: 提出一种两阶段闭环优化框架：第一阶段利用多模态大语言模型（mLLM）进行迭代提示词优化，结合Davidsonian场景图（DSG）与常见错误问题（CMQ）进行语义与伪影评估；第二阶段采用贝叶斯优化协同调整随机种子与CFG尺度，由包含新型视频-文本依从性（VTA）得分的质量指标引导搜索。

Result: 在人类偏好研究中，该方法显著优于无引导搜索，相比基线输出最高达到69%的胜率。

Conclusion: 该研究为增强视频生成模型的可预测性与控制力提供了实用且可扩展的方法，推动视频生成技术向可靠的生产级工具迈进。
...
### [4] [Diagram-MMU: A Multi-Modal Benchmark for Scientific Diagrams](http://arxiv.org/abs/2608.12262v1)
*Weihao Bo,Shan Zhang,Yanpeng Sun,Jie Liu,Yongke Yao,Jinhao Du,Wei He,Kai Zou,Zechao Li,Jingdong Wang*

Main category: cs.CV

TL;DR: 本文提出了一个名为Diagram-MMU的多模态基准，用于评估多模态大语言模型在科学图表解析与理解方面的能力。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型在科学写作中的应用增长，缺乏专门评估其科学图表解析与理解能力的基准。

Method: 构建了包含3.7k图表和18.3k验证问题的Diagram-MMU基准，覆盖六个领域，评估了图表转代码解析、编辑和图表问答三项任务及其代理设置。

Result: 对12个模型的评估表明，图表转代码任务比图表问答更具挑战性；在代理设置下，多数模型解析和编辑能力提升但问答下降，而Claude-4.6 Opus在所有任务上均有提升。

Conclusion: 当前模型在图表转代码生成方面存在困难，凸显了未来需要开发增强多模态大语言模型图表转代码能力的方法。
...
### [5] [Generation as Auxiliary Supervision: Enhancing Visual Understanding at Zero Inference Overhead via Decoupled Embedding Prediction](http://arxiv.org/abs/2608.12209v1)
*Zhongbin Guo,Jiahao Xie,Dongling Xiao,Qianle Wang,Ruiqi Lu,Xiaomin He,Wanxuan Sun,Cheng Yang*

Main category: cs.CV

TL;DR: 本文提出GAS框架，将视觉生成作为辅助监督来增强多模态大语言模型的理解能力，且不增加推理开销。


<details>
  <summary>Details</summary>
Motivation: 现有的统一多模态框架将视觉理解和生成视为分歧目标，生成目标与理解模型所需的连续表征不同，导致难以直接迁移以增强预训练的MLLMs。

Method: 提出GAS框架，在解耦的Mixture-of-Transformers架构中采用下一嵌入预测(NEP)范式，保持共享下部主干和并行上部层，使生成损失丰富共享视觉路径并保护理解层免受直接梯度影响，同时构建需要深度认知基础的相关生成任务。

Result: GAS在不同模型规模和训练阶段均提升了多模态理解能力，在感知和空间理解方面提升最显著，且训练后丢弃辅助生成分支实现了零推理开销。

Conclusion: 生成引导训练是增强多模态理解的实用可行途径，研究进一步阐明了生成引导训练在何时及为何能有益于理解能力。
...
### [6] [Context Blindness in DPO: Mitigating Object Hallucination in MLLMs via Context-Calibrated Preference Optimization](http://arxiv.org/abs/2608.12158v1)
*Byungoh Ko,Jinyoung Park,Jongha Kim,Jeehye Na,Jaewon Cho,Hyunwoo J. Kim*

Main category: cs.CV

TL;DR: 该论文提出上下文校准的偏好优化方法（C²-DPO），通过最大化上下文偏好增益（CPG）来促使多模态大模型有效利用上下文，从而缓解物体幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在物体幻觉问题。虽然直接偏好优化（DPO）利用包含相关上下文的偏好数据进行缓解，但DPO是否真正利用了这些上下文尚不明确，标准DPO对上下文的利用不足导致模型仍易产生幻觉。

Method: 首先提出上下文偏好增益（CPG）指标来衡量提供上下文时模型偏好的增强程度。基于发现标准DPO的CPG有限，提出上下文校准DPO（C²-DPO），在保持原始偏好排序的同时直接最大化CPG。

Result: 实验表明，更高的CPG对应更低的幻觉率。C²-DPO在多个基准测试中显著降低了幻觉且未损害通用推理能力，将Qwen2-VL-Instruct-2B在Object HalBench上的幻觉率相对降低了36%。

Conclusion: 标准DPO及其变体未能充分利用上下文信息，而提出的C²-DPO方法能有效改善上下文利用，从而减少多模态大语言模型的物体幻觉。
...
### [7] [GeoBridge: Decoupled Semantic Conditioning for Generative Image Geolocalization](http://arxiv.org/abs/2608.11838v1)
*Zhiyang Dou,Xumeng Han,Fengde Peng,Zipeng Wang,Moxuan Zhao,Zhipei Huang,Zhenjun Han*

Main category: cs.CV

TL;DR: GeoBridge提出了一种角色解耦条件机制，将冻结的多模态大语言模型与黎曼流匹配头连接，以生成连续且具有几何感知的坐标，从而解决图像地理定位中的解码瓶颈。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在图像地理定位中的瓶颈已从地理线索推理转移到如何将推理解码为坐标。现有方法通过预测地名调用地理编码API，这种离散且有损的方式忽略了图像证据，并将多粒度语义折叠为粗略查找。

Method: GeoBridge采用角色解耦条件机制，连接冻结的语义MLLM与在球面上生成坐标的冻结黎曼流匹配头。通过独立的投影层形成连续条件，将离散语义监督与条件接口分离，在不干扰球面解码器的情况下注入地理先验，解决了角色冲突问题。

Result: 在IM2GPS3K数据集上，GeoBridge在25/200/750公里阈值下的准确率分别达到38.67%/52.89%/70.37%，优于地名转API管道及推理增强的直接预测方法。

Conclusion: GeoBridge是一种解码端的算法贡献，与思维链推理正交互补，有效解决了坐标解码瓶颈。代码将公开。
...
### [8] [Advancing MLLM-based UAV Image Understanding and Reasoning: A Benchmark and a Training-Free Multi-Agent System](http://arxiv.org/abs/2608.11738v1)
*Haoyu Zhang,Shuoxun Zhang,Peng Ye,Lin Zhang,Jiakang Yuan,Shenghong Yi,Yuening Wang,Tao Chen*

Main category: cs.CV

TL;DR: 本文构建了UAVQA-Bench基准评估MLLM在无人机航拍图像理解中的能力，并提出了无需训练的多智能体系统UAV-MAS以解决现有模型的失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在无人机航拍图像理解中面临极端尺度变化、任意相机方向和高物体密度等挑战，且现有评估分散于单一数据集和狭窄任务，缺乏统一的能力评估框架。

Method: 提出UAV-MAS多智能体系统，包含三个核心模块：域特定感知引擎(DSPE)将查询路由至合适的视觉工具，上下文感知迭代精炼模块(CAIR)验证中间推理以抑制错误累积，难度感知自适应搜索机制(DAAS)根据问题难度调整搜索深度。

Result: UAV-MAS搭配32B开源MLLM在UAVQA-Bench上达到77.0%总体准确率，超越Gemini 3 Pro 4.0个百分点；8B变体较基座模型提升8.7%。

Conclusion: 通过统一基准系统评估揭示了MLLM在UAV场景的三大失败模式，所提出的训练-free多智能体框架有效提升了开源模型的航拍图像理解与推理能力，为UAV智能领域提供了新的评估和方法范式。
...
### [9] [Motion-as-Prompt: Enhancing Motion Reasoning in Multimodal Large Language Models via Motion-Guided Cross-Frame Visual Prompting](http://arxiv.org/abs/2608.11655v1)
*Xikai Sun,Kebin Liu,Haotian Wang,Li Liu,Xu Wang,Yunhao Liu*

Main category: cs.CV

TL;DR: 本文提出了Motion-as-Prompt (MaP)框架，通过在视觉输入上标记点轨迹，无需训练即可增强多模态大语言模型在视频中的运动中心推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型通常采用稀疏均匀采样来控制计算成本，这会丢弃帧间的关键转换信息，从而限制了模型对物体运动、碰撞和因果交互的推理能力。

Method: MaP框架恢复密集点轨迹，选择包含丰富运动信息的帧，并将连续采样帧之间累积的轨迹直接标记在视觉输入上，使冻结的多模态大语言模型能够观察到隐藏的位移、方向变化和交互。

Result: 在CLEVRER和Something-Something-v2数据集上，MaP使GPT-5.5的平均运动推理准确率分别提高了4.2%和8.9%，且未降低非运动理解能力。

Conclusion: MaP提供了一种简单有效且无需模型训练或架构修改的解决方案，显著增强了面向交互应用的运动中心视频推理能力。
...
TL;DR: 本文提出一种名为RPS的无训练解码方法，通过利用扩散大语言模型解码中的涟漪效应，显著加速并行推理且不损失生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型的并行解码调度器通常仅在位置满足标准后才提交，忽略了早期提交对减少剩余位置不确定性的潜在益处（即涟漪效应），从而限制了推理加速潜力。

Method: 提出Ripple-Pivot Search (RPS) 方法，主动寻找中熵位置作为枢纽，并通过前瞻评估确定能最大化下游收益的词元分配，从而触发涟漪效应，降低整体不确定性以实现更多并行解码。

Result: 在3个模型和4个基准测试中，RPS比标准解码器实现4-10倍实际运行时间加速且保持质量，较先前基线准确率提升最高5.49%，结合KV缓存时加速比可达18倍。

Conclusion: 在扩散大语言模型中利用涟漪效应进行早期位置提交可大幅提升解码吞吐量，证明了考虑下游收益对优化并行解码调度的重要性。
...
TL;DR: 本文提出PCD预训练目标，通过结合自回归前缀监督与去噪后缀，对齐扩散语言模型的训练与推理上下文分布，显著提升提示续接性能。


<details>
  <summary>Details</summary>
Motivation: 原生扩散语言模型（dLLM）在预训练时随机破坏提示和续接标记，导致训练上下文与推理时基于干净提示的条件生成不匹配，削弱了续接任务所需的干净前缀接口。本文旨在解决这一训练-推理不一致问题。

Method: 提出PCD（Prefix-Conditioned Diffusion）预训练目标：对干净前缀部分采用自回归监督，对未知后缀部分采用无偏移去噪；通过调整注意力掩码、破坏掩码和标签构造实现，无需自回归解码器、验证器或新推理模式。同时将样本内前缀条件化与样本间目标混合解耦。

Result: 在LLaDA2-Mini和Qwen-1.7B骨干网络上，PCD均稳定优于同族原生dLLM基线，LLaDA2-Mini六项基准平均相对提升4.2%（+2.56分），Qwen主要机制对比相对提升14.2%（+4.86分）。

Conclusion: 将预训练上下文分布与提示条件生成对齐，可以在不改变推理方式的前提下恢复扩散语言模型续接差距中可测量的部分，表明训练-推理对齐对dLLM性能至关重要。
...
TL;DR: 本文发现掩码扩散语言模型在推理任务中的任意顺序解码自由度会导致失败，并提出“前沿门控提交”方法来解决这一推理病理。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型的无约束解码会在推理区域仍被掩码时过早提交最终答案，导致输出退化为仅含答案，且随着序列变长该问题加剧。本文旨在解决这种无序提交导致的推理失败。

Method: 通过记录解码过程的token提交轨迹，分析提示与解码器的2x2交互设计，并提出了一种“前沿门控提交”的单一干预机制来限制提交位置。

Result: 思维链仅在有序提交下起作用；前沿门控提交弥补了性能差距（准确率从0.528提升至0.852），并保持最高4倍的并行解码效率；揭示了现有窗口式采样器实为该推理病理的最小修复方案。

Conclusion: dLLMs的任意顺序解码自由度在推理任务中是失败之源；限制提交顺序可因果性地修复该问题，这重新定义了现有窗口采样器的作用，并为未来并行解码与推理顺序的权衡提供了新方向。
...
TL;DR: 提出OPTD方法，通过在策略转移蒸馏和一致性引导的自适应压缩，解决扩散语言模型少步蒸馏中训练与推理状态分布不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有少步蒸馏方法在离策略轨迹上构建监督，但学生模型在推理时早期的并行预测会改变后续预测的上下文，导致实际访问的状态偏离监督状态，尤其在压缩最激进时最为严重；而在策略蒸馏虽可缓解此问题，但如何决定每次转移应推进多远仍不明确。

Method: OPTD从少步学生模型自身轨迹中采样部分状态，使用冻结的仅问题输入的教师模型识别与结果一致的未来候选，并按当前状态置信度排序；然后选择最长前缀使其联合承诺保持教师模型 rollout 的结果；采用集瓶颈目标促进已验证的未来候选达到解码器释放阈值，并用冻结教师KL锚正则化其他活跃位置。

Result: 在四个数学推理和代码生成基准上，OPTD持续改善质量-效率权衡，在所有评估的少步基线中取得了最强的质量受限AUP。

Conclusion: OPTD通过在策略蒸馏和自适应压缩有效解决了少步扩散语言模型蒸馏中的状态漂移问题，无需黄金响应即可实现目标构建和训练，在质量和效率之间达到了最优权衡。
...
TL;DR: 本文提出DominoTree，一种结合Domino条件非因子化校正与GPU原生CUDA图构建器的无训练最佳优先草稿树方法，显著提升了LLM推理速度与接受长度。


<details>
  <summary>Details</summary>
Motivation: 现有的最佳优先树方法（如DDTree）采用因子化公式，无法表示如Domino等具有GRU因果校正的草稿器所生成的路径相关草稿分布。

Method: 提出DominoTree，沿每条根到节点的路径应用Domino的条件非因子化校正进行评分，并将每节点校正限制在候选top-M集中；同时使用GPU原生的CUDA图构建器高效构建草稿树。

Result: 在Qwen3-4B上实现最高6.6倍加速及各方法中最高的平均接受长度（10.7 tokens/轮），在各温度下均优于DDTree和CaDDTree，且相比Domino吞吐量提升9%至22%；在Qwen3-8B上同样保持最高接受长度并在T=0时取得24%的吞吐增益。

Conclusion: DominoTree有效利用了路径相关的草稿分布，通过低成本的GPU原生树构建策略，在维持接受率不变的同时显著提升了大模型推理的吞吐量。
...
TL;DR: 提出CaDDTree方法，通过联合优化树结构和节点预算直接最大化令牌吞吐量，解决了块扩散草稿树中节点预算缺乏原则性选择依据的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的DDTree方法在固定节点预算下最大化预期接受长度，但由于接受长度随预算非递减，它总是偏向更大的树而忽略了验证成本，导致无法原则性地选择最优预算。

Method: 提出CaDDTree，显式建模草稿和验证延迟，将树结构和节点预算的联合选择转化为每轮对预算的一维搜索，并在凸验证成本下证明吞吐量函数为单峰函数，实现高效的贪心停止规则，无需离线预算搜索。

Result: 在Qwen3-4B和Qwen3-8B模型的8个基准测试上，CaDDTree在几乎所有任务上均匹配或超越了带有预言机预算选择的DDTree。

Conclusion: CaDDTree为块扩散草稿树的预算选择提供了原则性且成本感知的解决方案，能够根据实时分布和成本动态调整，有效提升推理吞吐量。
...
TL;DR: DDTree通过从块扩散模型的位置分布构建草稿树来改进DFlash投机解码，从而提升接受长度并保持领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash方法在每轮中仅验证单条草拟轨迹，限制了其接受长度，从而限制了投机解码的进一步加速。

Method: 提出DDTree，从块扩散草稿模型的逐位置分布直接构建草稿树；在固定节点预算下，利用最佳优先堆算法选择最可能匹配目标模型的续写，并使用仅祖先注意力掩码在目标模型单次前向传播中完成验证。

Result: DDTree在DFlash的基础上取得了性能提升，使其成为目前领先的投机解码方法之一。

Conclusion: 将块扩散草稿模型扩展为树结构验证有效提升了投机解码性能，确立了DDTree在该领域的领先地位。
...
### [17] [AWARe: Mitigating Catastrophic Forgetting via Activation-Weighted Adaptive REtention](http://arxiv.org/abs/2608.11758v1)
*Juncheng Liao,Jinfan Lv,Guoming Wang,Jupeng Zheng,Ling Xiao,Siliang Tang*

Main category: cs.CL

TL;DR: 本文提出AWARe微调方法，通过基于激活模式动态控制参数更新来缓解多模态大语言模型的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在下游任务微调时通常会发生灾难性遗忘，新任务知识会破坏已有能力，限制了模型的实际部署。

Method: 提出激活加权自适应保留（AWARe）方法，根据激活模式为参数分配重要性分数，动态控制参数更新，选择性地冻结对保留旧能力至关重要的参数，同时允许不重要的参数适应新任务。

Result: 实验表明，AWARe在有效保留上游能力的同时，相比现有方法取得了更优的下游性能。

Conclusion: AWARe无需修改模型架构即可缓解灾难性遗忘，与现有推理引擎完全兼容，有效提升了多模态大语言模型的微调效果。
...
### [18] [Convergent Detour Hijacking: Task-Preserving Resource Amplification in Skill-Based LLM Agents](http://arxiv.org/abs/2608.12273v1)
*Junliang Liu,Ruoyu Li,Wenxin Tang,Jingyu Xiao,Zhenyu Liu,Jingheng Xu,Laizhong Cui*

Main category: cs.CR

TL;DR: 本文提出了一种针对LLM代理第三方技能的端到端攻击方法CDH，通过操纵技能选择与规划阶段，在不影响任务完成率的前提下显著增加执行成本。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理依赖第三方技能的渐进式披露设计存在安全漏洞，技能选择和指令规划阶段易受攻击。以往研究孤立看待这些问题，本文旨在揭示两者端到端组合攻击的风险。

Method: 提出了一种纯文本、运行时无关的攻击方法CDH。该方法在技能选择阶段利用描述建立相关性，在规划阶段利用对齐的指令体伪造依赖，诱导代理招募不必要的良性技能进行绕道，最终重入原路线以完成任务。

Result: 在多个LLM和491个任务上的评估表明，在DeepSeek-V4-Pro上攻击器命中率达80.02%；在命中且完成的任务中，token消耗和端到端执行时间分别增加66.91%和92.45%，而总体任务完成率保持相当。

Conclusion: 正确的任务输出并不能保证执行轨迹的完整性或成本安全性，揭示了LLM代理在调用第三方技能时面临的新型隐蔽攻击风险。
...
### [19] [The SLT 2026 SmartGlasses Challenge: Benchmarking Egocentric Multi-Talker Speech Recognition and Understanding with Audio-Language Models](http://arxiv.org/abs/2608.12034v1)
...
TL;DR: 本文介绍了IEEE SLT 2026智能眼镜挑战赛，基于真实场景数据集评估以自我为中心的多说话人语音识别与理解，并总结了相关性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 由于动态声学条件、说话人重叠以及佩戴者中心记录几何带来的空间模糊性，智能眼镜场景下的语音识别与理解仍面临巨大挑战，亟需系统性的评估基准。

Method: 提出包含双向对话理解和多方会议理解两个赛道的挑战赛，基于106小时、714个真实场景的四通道以自我为中心语音数据集，联合评估带时间戳的说话人归属语音识别(TSA-ASR)和口语理解(SLU)。

Result: 评估结果显示，严重的说话人重叠是影响TSA-ASR性能的主要因素；同时，当前的音频语言模型在复杂的SLU场景下进行副语言声学理解依然困难。

Conclusion: 当前语音及音频语言模型在以自我为中心的复杂真实场景中，仍受限于说话人重叠和副语言理解的技术瓶颈，未来需针对这些难点提升模型的鲁棒性。
...
### [20] [SelectLight: Learning to Select Signal Plans Generated by Distributed Model Predictive Control for Urban Traffic Networks](http://arxiv.org/abs/2608.12256v1)
*Lyuzhou Luo,Chaopeng Tan,Zhengyong Gao,Hong Zhu,Andrea D'Ariano,Keshuang Tang*

Main category: eess.SY

TL;DR: 本文提出SelectLight方法，利用多智能体强化学习（MARL）在分布式模型预测控制（DMPC）生成的多目标信号计划中进行选优，以实现城市交通信号的自适应协调控制。


<details>
  <summary>Details</summary>
Motivation: 传统的多目标分布式模型预测控制（DMPC）在进行信号计划权衡选择时依赖预设规则，无法从闭环控制结果中学习以适应动态变化的交通需求。

Method: 提出SelectLight框架，首先通过状态剪枝多目标动态规划（SP-MODP）生成满足约束的非支配候选信号计划集，随后使用独立近端策略优化（IPPO）训练的拓扑感知注意力MARL策略从候选集中选择最终计划。该方法将学习限制在候选方案选择阶段，保留了预设的信号时序约束并支持计划审查。

Result: 在28交叉口SUMO网络上的实验表明，SelectLight的延迟相关性能最优且优势随需求增加而扩大；在两倍基准需求下，排队延迟和等待时间较最强基线分别降低5.57%和6.44%；在需求波动下迁移损失最低；单交叉口99百分位求解时间为5.408毫秒，满足实时控制要求。

Conclusion: 将MARL与DMPC相结合进行优化后选择，不仅能在满足交通运行约束的前提下实现实时高效的信号控制，还能通过学习机制显著提升系统对动态交通需求的适应性与整体控制性能。
...
### [21] [PACE-SIMS: Checkpoint-Gated Autonomous SIMS Characterization with AI-Agent Quality Control](http://arxiv.org/abs/2608.12277v1)
*Anton V Ievlev,Heather Hare,Yiyang Li,Sergei V Kalinin*

Main category: cond-mat.mtrl-sci

TL;DR: 本文提出了PACE-SIMS，一种将飞行时间二次离子质谱（ToF-SIMS）研究作为人机协作执行的智能体工作流程，显著减少了专家监督时间。


<details>
  <summary>Details</summary>
Motivation: ToF-SIMS广泛应用于材料化学分析，但运行过程中需要训练有素的研究人员全程监督并动态调整参数，多日实验耗费大量专家时间，成本高昂且效率低下。

Method: PACE-SIMS采用智能体工作流程：研究人员指定科学问题和质量要求后，AI智能体构建实验计划，经批准后自主执行，在检查点暂停以判断每次测量结果，并进行纠正、重试或上报。

Result: 在18O富集WOx薄膜的盲测随机双极性实验中（8.1小时，35次测量），智能体进行了3次固定脚本无法实现的非脚本纠正，4项预测全部与密封真值吻合，并获得了成分校准、两极性间5.3%同位素读数偏移及示踪剂输送机制，研究人员仅需不到2小时的关注时间。

Conclusion: 该智能体架构不限于SIMS，可推广至其他分析技术，尤其适用于难以应用优化方法的破坏性测量场景。
...
### [22] [VAKRA: Evaluating Multi-Hop Reasoning Across APIs and Retrieval Under Tool-Use Policies](http://arxiv.org/abs/2608.12282v1)
...
TL;DR: 本文提出了VAKRA基准，用于评估跨结构化API和文档集的API与知识检索智能体，发现当前前沿模型在多跳推理和策略约束方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 企业环境中的智能体需要跨结构化API和文档集进行推理，但现有的基准测试是孤立地评估这些能力的。

Method: 提出VAKRA基准，包含62个领域的8000多个可执行API，涵盖三种难度递增的任务设置；使用固定的ReAct框架隔离模型能力，并通过针对实时API重新执行预测的工具调用来验证正确性。

Result: 最佳模型在单跳端点任务上仅达70.4%准确率，在组合API上降至50-51%；推理深度增加导致性能下降超50%；策略约束问题失败率极高（低至2.4%）。失败主要集中于语言介导的推理而非工具调用机制。

Conclusion: 当前智能体的主要失败原因在于语言介导的推理能力不足（如实体消歧和跨源对齐），而非工具调用机制本身存在缺陷。
...
TL;DR: 本文提出了连续交互扩散（CID）架构，将工具交互直接集成到扩散语言模型的迭代去噪过程中，以克服传统停顿-恢复交互模式的限制。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行精炼输出进行推理，传统的自回归式“停顿-恢复”工具调用会迫使模型在推理稳定前做决定、延迟观察并引入冗余，从而损害任务准确率和推理效率。

Method: 提出CID架构，将只读事实通道、思考通道和显示通道分离；允许在调用完全序列化前启动外部读取，将返回结果投影到演化的思考状态中以修正早期认知，并利用持久绑定复用静态结果。

Result: 本文形式化了该架构、运行时和训练目标，并定义了任务质量与端到端效率的评估协议，但作为首篇论文，其专注于只读工具且未提出实证性能声明。

Conclusion: CID架构旨在更早暴露证据、实现工具延迟与模型计算的重叠、减少重复外部工作并保留新证据到达后的有用计算，为扩散语言模型的原生工具交互奠定了基础。
...
TL;DR: 本文系统研究了混合专家扩散语言模型的缩放规律，并基于发现训练了LLaDA MoE v2模型，仅用Qwen3约65%的预训练tokens即接近其性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归模型的替代方案，但MoE dLLMs的缩放行为尚不清楚，缺乏优化超参数、计算分配和架构规模方面的系统性理解和设计原则。

Method: 系统表征MoE dLLMs的优化超参数、计算分配和架构缩放规律，发现与AR模型的定量差异（最优批量大小增长更快、学习率衰减更快、数据侧倾斜、更大规模偏好更大专家池等），并基于这些发现从零训练30B-A3B的LLaDA MoE v2。

Result: LLaDA MoE v2在23.5T tokens上训练，仅用Qwen3约65%的预训练tokens即在知识、推理和编码基准上接近Qwen3；仅经监督微调即在8个推理和编码基准中的7个上超越SDAR Chat，并在多个任务上保持接近Qwen3的表现。

Conclusion: 为MoE扩散语言模型建立了实用的缩放规律和设计原则，证明dLLMs在合理计算预算下可达到与主流AR模型竞争力相当的性能。
...
TL;DR: 本文提出了一种无需训练的测试时扩展框架Evolutionary Decoding，通过将扩散解码视为演化过程来解决扩散大语言模型在数学推理中的置信度陷阱问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在数学推理任务上表现不可靠，原因是局部Token置信度与全局推理正确性之间存在错位（即扩散置信度陷阱），导致出现采样敏感或采样一致的错误模式。

Method: 提出Evolutionary Decoding，一种将扩散解码视为候选推理状态演化过程的免训练测试时扩展框架。该框架结合逐步选择（保留有用信号并抑制重复模式）和块级突变（引入结构化替代方案以逃离错误的高置信度区域）。

Result: 在多个基准测试上的实验表明，Evolutionary Decoding 优于基于置信度的解码方法，有效提升了 LLaDA 2.0 模型的数学推理可靠性。

Conclusion: Evolutionary Decoding 能够有效克服扩散模型解码过程中的置信度错位问题，无需额外训练即可显著增强模型的数学推理能力。
...
TL;DR: 本文提出TAPS方法，通过目标感知的路径条件接受估计选择紧凑的前缀闭包子树，解决了扩散模型在投机解码中的验证瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型用于并行起草投机解码虽降低了起草延迟，却将瓶颈转移至验证阶段。现有草稿树方法基于边缘概率排序，忽略了验证的前缀条件性，导致验证被拒前缀的不可达后代，徒增延迟且接受率提升有限。

Method: 提出TAPS（目标感知前缀选择方法），将扩散边缘概率转化为路径条件的接受估计，并在固定验证预算下选择紧凑的前缀闭包子树，以优化接受度与成本的权衡。

Result: 在多种数据集和模型上的实验表明，TAPS相比自回归解码实现了高达7.9倍的无损端到端加速，且分别比SOTA方法DFlash和DDTree快1.36倍和1.74倍。

Conclusion: TAPS通过优化验证策略有效解决了扩散模型投机解码的验证瓶颈，在提升推理速度方面显著优于现有方法。
...
### [27] [MBA: Multimodal Benchmark and Agents for Real-World Business Ideation](http://arxiv.org/abs/2608.11616v1)
*Hojun Choi,Jaeyo Shin,Suin Lee,Hyunjung Shim*

Main category: cs.AI

TL;DR: 本文提出了首个多模态商业创意基准MBA-Bench及配套训练方法，通过创新的奖励目标优化智能体表现，显著超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的商业创意智能体仅限于纯文本范式，未能利用现实世界中固有的多模态上下文（如视觉线索）。

Method: 构建包含3万样本的多模态基准MBA-Bench；利用GPT-4o生成参考创意；基于LoRA监督微调和组相对策略优化（GRPO）训练MBA-b（盲评）和MBA-k（已知标准）模型，并引入创造性与可行性等新颖奖励目标。

Result: 多模态输入基线在部分指标上接近闭源模型；MBA-b和MBA-k分别比文本描述基线提升63.9%和77.1%，比多模态基线提升25.6%和35.8%。

Conclusion: 所提多模态基准与特定奖励训练机制有效解决了纯文本局限性，大幅提升了商业创意智能体的性能。
...
TL;DR: 本文研究了扩散式大语言模型的安全机制漏洞，提出了一种利用安全神经元引导的离线黑盒越狱框架，以极低的成本实现了高成功率的攻击。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型（DLLMs）取代了自回归预测，但其内部的安全对齐机制仍不明确。本文旨在揭示并测试基于扩散的对齐机制中存在的机械性漏洞。

Method: 通过直接的安全神经元映射和剪枝发现DLLMs继承了自回归前代模型的安全足迹；据此提出了SN-Guided Diffusion框架，利用加权安全神经元损失引导扩散过程避开安全触发区域，实现离线黑盒越狱。

Result: 自剪枝和迁移剪枝大幅提升了攻击成功率（ASR）；SN-Guided Diffusion在区分良性提示与越狱提示时达到完美分离（AUROC=1.0），并在多个开源和闭源模型上实现了高达86.9%的迁移攻击成功率，而生成成本比现有框架低几个数量级。

Conclusion: DLLMs的安全对齐机制稀疏且可跨架构转移，暴露了显著的脆弱性，凸显了未来改进扩散式大语言模型安全对齐机制的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（dLLMs）虽通过并行解码加速推理，但块级半自回归解码中每个去噪步骤仍需对之前所有块进行前缀注意力计算，造成重复的计算开销，成为推理瓶颈。

Method: 提出block-hybrid attention：在活跃去噪块内保留精确softmax注意力，对先前块应用线性注意力；该方法基于LoLCAT方法以最小后训练成本迁移到预训练dLLM中，将LLaDA 2.1的20层注意力中的6层替换为线性注意力，转换仅需约60小时。

Result: LLaDA-Hybrid在基准性能上基本保持：HumanEval 72.0% vs 75.6%，MBPP+ 63.0% vs 57.7%，CMATH 86.7% vs 88.3%；Triton实现下解码吞吐量最高提升1.7倍，且在内存耗尽前支持更多并发请求。
...
TL;DR: 本文提出ERILS方法，通过引入外部策略的高奖励rollout并分别进行长度控制与奖励处理，解决了离散扩散大语言模型在线强化学习中正奖励稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的离散扩散大语言模型（dLLM）强化学习方法过度依赖模型自身生成的在线策略rollout。当成功的在线策略rollout稀缺时，模型难以获得正向奖励，导致训练进展有限。

Method: 提出了一种结合长度控制和特定来源处理的方法（ERILS）。该方法引入更强的外部策略生成高奖励rollout，同时控制外部rollout的长度以解决长度差异问题，并分别独立处理在线策略和外部rollout的奖励以避免联合处理的不稳定性。

Result: 在Sudoku、Countdown和MATH500的零样本评估中，ERILS全面提升了多样本性能，其中Sudoku提升最大（best-of-4准确率达98.4%，基线最高仅40.3%），并在不同生成长度下保持约90%的确定性单次准确率。组件分析表明长度控制比无控制更有效，且特定来源的奖励处理避免了联合处理导致的训练崩溃。

Conclusion: 在将外部rollout集成至dLLM强化学习时，rollout的构建和奖励处理是关键且有效的设计维度。
...
### [31] [Teaching a Large Language Model Tutor to Withhold the Answer: A Supervisor Architecture and an Evidence-Driven Method for Tuning Socratic Behavior](http://arxiv.org/abs/2608.12292v1)
*Yusuf Pisan*

Main category: cs.CY

TL;DR: 本文提出了一种部署型辅导系统，通过机器可查的合约、非LLM策略核心和自动化评估循环，强制大语言模型在辅导中拒绝直接给出答案。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在受压时无法可靠地拒绝直接给出答案，这会削弱辅导效果并降低学生后续的独立测试成绩，因此需要可靠的答案保留机制。

Method: 系统采用机器可查的逐轮合约强制保留答案：非LLM策略核心设定八级帮助阶梯上限，确定性检测器剥离代码，独立LLM裁判检查回复；并通过脚本化角色驱动的自动化评估流水线进行调整与诊断。

Result: 评估揭示了一个可解释的“过度帮助阶梯”（从泄露答案到指出错误再到过度引用事实），修复过程逐层暴露问题；最终该辅导系统在所有四项验收标准上达到完全合规。

Conclusion: 该“测量、诊断和修复”循环为任何需要限制自身能力输出的大语言模型智能体提供了可复用的解决方案。
...
### [32] [The Role Specialization Model (RSM): Coordinating LLM-Based Tools in Agentic Software Development - An Exploratory Case Study](http://arxiv.org/abs/2608.12311v1)
*Carlos Alberto Fernández-y-Fernández,Jorge R. Aguilar-Cisneros*

Main category: cs.SE

TL;DR: 本文探讨了通过角色专业化模型（RSM）协调多个基于LLM的工具进行智能体软件工程开发的探索性案例研究


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型融入软件开发流程，在SE 3.0范式下自主代理管理完整开发生命周期，需要解决具有不同能力的LLM工具如何协调配合的问题

Method: 提出角色专业化模型（RSM），协调三个基于LLM的工具（Antigravity/Gemini 2.5后端、Gemini CLI和Qwen Code/Ollama本地执行），以增量开发Python桌面气候数据可视化应用为案例进行验证

Result: 结果表明，明确的角色协调能够支持开发周期组织和架构质量，但需要刻意的协调策略、上下文管理和对代理生成输出的人工验证

Conclusion: 角色协调有助于开发流程组织和架构质量提升，但成功实施依赖于审慎的协调策略、上下文管理及对代理输出的人工验证
