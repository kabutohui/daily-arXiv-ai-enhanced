...
### [1] [Circuit-MLLM: Topological Logic-Guided Latent-Space Visual Reasoning for Circuit Schematic Understanding](http://arxiv.org/abs/2609.15668v1)
*Jinyuan Deng,Yuqi Jiang,Wenjing Huang,Xin Li,Qi Sun,Cheng Zhuo*

Main category: cs.CV

TL;DR: 提出Circuit-MLLM框架，通过器件定位、路径追踪和拓扑引导的序列推理机制，解决多模态大语言模型在电路原理图拓扑分析中的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在处理密集元件布局和独特拓扑逻辑的电路原理图时表现不佳，难以进行细粒度结构解析以提取电气语义。

Method: 提出Circuit-MLLM框架，将电路拓扑分析重构为潜在空间中的器件定位、路径追踪和序列推理过程；引入电路知识挖掘机制，将模型的潜在表示与多粒度电路视觉专家的特征对齐；设计拓扑引导排序策略，按电路拓扑逻辑进行逐步推理。

Result: 在多种电路分析任务中，Circuit-MLLM表现均优于强基线，其平均得分比GPT-5.1高出25%。

Conclusion: 该框架在电路原理图拓扑分析中非常有效，证明了内部化拓扑语义和拓扑引导推理策略的优越性。
...
### [2] [VideoScout: Learning Agentic Active Exploration with Adaptive Reasoning Pacing for Long Video Understanding](http://arxiv.org/abs/2609.15606v1)
*Weixin Xu,Zhenyu Yang,Bing Wang,Shengsheng Qian,Changsheng Xu*

Main category: cs.CV

TL;DR: 本文提出VideoScout，一种基于顺序证据获取（SEA）范式的多轮推理代理，通过动态控制观看节奏在有限视觉上下文窗口内高效理解长视频。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型（MLLMs）因视觉上下文窗口受限而难以理解长视频，且当前的均匀帧采样或从粗到细的代理缩放方法无法有效定位长视频中的稀疏决定性证据。

Method: 提出VideoScout代理，将长视频理解转化为顺序证据获取（SEA）问题，通过动态控制观看速度实现有界上下文窗口内的高效遍历。训练采用两阶段流水线：冷启动监督微调学习输出格式，以及使用DAPO算法进行轨迹级强化学习，综合奖励考虑答案准确率、格式合规性和时序对齐（IoU）。

Result: 在长视频理解和推理基准测试上的广泛实验表明，该7B模型与现有的7B代理模型相比取得了强劲的性能。

Conclusion: VideoScout成功验证了顺序证据获取范式和自适应推理步调在长视频理解中的有效性，通过平衡内容分析深度与阅读效率实现了性能提升。
...
### [3] [MarKey: Marginal Utility Guided Greedy Keyframe Selection for Long Video Understanding](http://arxiv.org/abs/2609.15408v1)
*Hongchang Shi,Jinpeng Hu,Ao Wang,Wenzheng Zhou,Hui Ma,Feng Li,Zenglin Shi*

Main category: cs.CV

TL;DR: 提出一种无训练框架MarKey，通过子集感知的贪心优化进行关键帧选择，有效解决了长视频理解中的帧选择冗余和证据覆盖不全问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在长视频理解中面临密集编码计算昂贵和均匀采样易遗漏关键证据的挑战；现有无训练关键帧选择方法孤立评分，导致冗余选择和证据覆盖不全。

Method: 提出MarKey框架，将关键帧选择表述为子集感知的贪心优化。通过可处理的代理函数综合评估查询相关性、边际覆盖率增益和上下文依赖性冗余；并利用代表性锚点集合和有界前序选择帧窗口提高评估效率。

Result: 在六项视频理解基准测试（涵盖整体、以人为中心和开放式视频理解）中，MarKey持续优于现有方法，且在不同MLLM骨干网络、模型规模和帧预算下均表现出稳健的提升。

Conclusion: MarKey是一种高效且无需训练的长视频关键帧选择框架，能够有效提升多模态大语言模型在有限视觉预算下的长视频理解能力，具有跨模型和规模的鲁棒性。
...
### [4] [Concept-Grounded Reasoning with Prompt-Driven Localization for Interpretable Structured Report Generation](http://arxiv.org/abs/2609.15334v1)
*Xinyue Xu,Hongbin Lin,Juangui Xu,Hualiang Wang,Lehan Wang,Lijie Hu,Weiyang Liu,Adrian Weller,Xiaomeng Li*

Main category: cs.CV

TL;DR: 本文提出了CORAL框架，通过将空间定位和概念级监督整合到统一推理过程中，提升了多模态大模型在医学报告生成中的准确性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有用于医学报告生成的多模态大语言模型依赖端到端融合，未建模临床定义的中间属性，导致其医学依据和可解释性受限。

Method: 提出CORAL框架，利用提示驱动的医学分割模型定位病灶，通过概念瓶颈模块预测多类临床属性，并将文本概念标记与掩码视觉特征结合输入多模态大语言模型进行结构化报告生成与诊断预测。

Result: 在BUS-CoT和IU X-ray数据集上的实验表明，CORAL在诊断准确率、概念一致性和报告质量上均显著优于现有的通用及医学大模型。

Conclusion: 基于概念接地的推理方法能更好地将生成过程与临床决策流程对齐，从而提升医学影像诊断的临床应用价值。
...
### [5] [AdaVSkip: Adaptive Visual Token Skipping Across Layers For Efficient MLLMs Inference](http://arxiv.org/abs/2609.15131v1)
*Yuyao Sun,Tao Deng,Shuang Li,Deqing Wang*

Main category: cs.CV

TL;DR: 本文提出了AdaVSkip，一种自适应跳过多模态大语言模型中自注意力和MLP模块的方法，能在保持性能的同时显著减少计算量。


<details>
  <summary>Details</summary>
Motivation: 现有高效的MLLM推理方法大多利用水平冗余（压缩视觉token）或固定层跳跃，但垂直冗余的程度和分布因输入而异，且在自注意力和MLP模块间存在差异。

Method: AdaVSkip为每层配备两个轻量级路由器，独立决定视觉token是否跳过自注意力和MLP模块。为解决离散决策的不可微问题，采用两阶段训练：第一阶段基于模块必要性评分进行监督训练；第二阶段利用强化学习结合答案正确性和跳跃一致性奖励优化路由，同时冻结主干网络。

Result: 在三个MLLM主干上，AdaVSkip在大幅减少计算量的同时保持了良好性能。在LLaVA-NeXT-7B上，FLOPs减少了53.2%且保持了原始平均性能；与token压缩结合后，FLOPs减少了91.2%，平均保留了97.2%的原始性能。

Conclusion: AdaVSkip通过动态决定模块跳跃有效利用了MLLM的垂直冗余，实现了巨大的计算节省且性能损失极小，并可与水平压缩技术结合进一步提升效率。
...
### [6] [From Density to Biopsy Decisions and Malignancy Prediction: A Benchmark Study of Multimodal Large Language Models Against Radiologists in Digital and Contrast-Enhanced Mammography](http://arxiv.org/abs/2609.14676v1)
...
TL;DR: 比较多模态大语言模型与放射科医生在乳腺X线摄影中的诊断表现，发现放射科医生在分类任务上更优，但结合病灶掩膜的MLLM在连续恶性概率估计上接近人类水平。


<details>
  <summary>Details</summary>
Motivation: 旨在评估多模态大语言模型在乳腺密度评估、BI-RADS分类、活检决策及恶性概率预测等临床任务中能否达到放射科医生的水平，以探索其作为辅助诊断工具的潜力。

Method: 研究纳入179例配对的数字化乳腺摄影（DM）和对比增强乳腺摄影（CEM）图像。测试四个MLLM模型在有/无病灶掩膜情况下的表现，并与三名放射科医生在无掩膜图像上的表现进行对比。

Result: 在二元密度分类、五分类BI-RADS和二元活检候选资格的准确率上，放射科医生普遍优于MLLM。CEM提升了所有评估者的表现。引入病灶掩膜显著提高了MLLM在连续恶性概率估计上的准确率（提升至72.56%-81.86%），其AUC与放射科医生重叠。总体上Muse Spark表现最佳。

Conclusion: 放射科医生在乳腺X线分类任务中总体优于MLLM；但在病灶掩膜辅助下，部分MLLM在连续恶性概率估计上接近人类表现，提示其有望作为临床辅助诊断工具。
...
### [7] [Func-R1: Incentivizing Mathematical Function Reasoning in Multimodal Large Language Models](http://arxiv.org/abs/2609.14779v1)
...
TL;DR: 本文提出了Func-R1框架，通过解耦架构与对齐优化策略，解决多模态大模型在视觉数学推理中的模态干扰问题。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在视觉语境下进行数学推理时，因忽视或误解视觉线索而导致的模态干扰问题。

Method: 提出基于显式解耦架构的Func-R1模型，采用分层后训练框架识别视觉证据并进行理论推理，并引入感知对齐理论优化（PATO）策略以动态纠正异构视觉信息。

Result: 在多个基准测试中，该模型在开源多模态大语言模型中达到最优性能，并在MathVerse函数任务上超越GPT-5达8.4%。

Conclusion: 通过协同视觉感知与逻辑推理并动态纠正视觉信息，可显著提升多模态模型在数学推理任务中的准确性与鲁棒性。
...
### [8] [HarnessVLN: Unifying Training-Free Embodied Navigation through an Agent Harness](http://arxiv.org/abs/2609.15195v1)
*Yang Chen,Lirong Che,Zhenyu Huang,Wenbo Fu,Chuang Wang,Xu Cao,Daqi Liu,Yuzhe Yang,Jian Su,Lan-Zhe Guo*

Main category: cs.RO

TL;DR: 本文提出零样本无训练具身导航框架HarnessVLN，通过Agent Harness协调多模块并验证动作与空间证据的一致性，在多个基准上刷新了无训练SOTA。


<details>
  <summary>Details</summary>
Motivation: 基于训练的导航方法面临泛化挑战，而现有无训练方法利用多模态大语言模型时缺乏将提议动作与空间证据、任务进度及执行失败相协调的机制。

Method: 提出HarnessVLN框架，其Agent Harness通过统一接口协调感知、导航和恢复；基于空间证据和几何可行性验证规划提案；采用分层事件记忆和持久时空图跟踪进度并记录失败以供恢复；可替换的执行器支持指令跟随和目标导航。

Result: 在R2R、RxR、HM3D-v2和HM3D-OVON上成功率分别达60.8%、53.9%、76.0%和59.3%，超越先前无训练SOTA，且成功在真实环境人形机器人上部署。

Conclusion: HarnessVLN通过有效的协调与验证机制解决了无训练导航的动作一致性难题，在仿真基准和真实世界人形机器人部署中均展现出卓越的适用性。
...
### [9] [MedVA: An End-to-End Neuro-Symbolic Agentic System for Medical Volume Visualization](http://arxiv.org/abs/2609.14874v1)
*Haill An,Suhyeon Kim,Minjun Kang,Eunwoo Lee,Bin Sheng,Lei Bi,Younhyun Jung*

Main category: cs.GR

TL;DR: 本文提出了MedVA，一个端到端的神经符号智能体系统，通过结合符号推理和预训练分割模型，解决医学体数据可视化中纯MLLM推理不理想的问题。


<details>
  <summary>Details</summary>
Motivation: 现有医学体数据可视化工作流需要大量专业知识和试错优化，而当前基于MLLM的智能体系统在推理时可能导致临床请求解释不完整以及ROI识别与可视化优化不可靠。

Method: 提出包含三个互补智能体的MedVA系统：1) 神经符号意图制定智能体，通过临床知识符号推理完善自然语言解释；2) 多模型ROI识别智能体，利用预训练医学分割模型识别ROI；3) 目标驱动可视化优化智能体，使用体素可见性目标评估ROI可见性和遮挡。

Result: 跨多种医学数据集和交互场景的智能体级与系统级评估证明了其有效性；形成性用户研究显示该系统在不同专业水平的用户中具有高可用性和实用价值。

Conclusion: MedVA通过神经符号智能体有效克服了纯MLLM在医学体数据可视化中的局限性，实现了高临床准确性与用户友好性。
...
### [10] [Not All Prompts Are Equal: Exploration-Guided Prompt Scaffolding for Multimodal Reinforcement Post-Training](http://arxiv.org/abs/2609.15051v1)
*Yuanhao Yue,Qianli Ma,Chengyu Wang,Haoting Wang,Lei Shen,Jun Huang*

Main category: cs.LG

TL;DR: 本文提出了一种探索引导的提示词脚手架框架，通过动态调整在线强化学习中的训练提示分布，显著提升了多模态大语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 在标准强化学习训练中，已饱和或过于困难的提示词均获得相同的rollout预算，导致学习信号效率低下。本文旨在解决在线RL中训练提示信息量不均的问题。

Method: 核心方法包括引入探索潜力分数（EPS）作为轻量级提示效用代理指标，并利用教师模型对低效用提示进行脚手架重写，将教师监督重构为训练数据优化而非简单的输出模仿。

Result: 在Geo3K和MMK12数据集上结合GRPO算法，该方法在域内基准上实现了高达9.7%的相对提升，并在MathVision和MMMU-Pro上分别取得了11.5%和11.1%的增益。

Conclusion: 该方法证明了在不增加额外开销的情况下动态优化训练提示分布的有效性，为多模态大语言模型的强化学习后训练提供了一种高效的教师数据精炼范式。
