...
### [1] [Deep Academic Survey: Stateful Agentic Closed-Loop Paradigm for Academic Survey Automation](http://arxiv.org/abs/2608.18034v1)
*Zhikai Xu,Zhucun Xue,Teng Hu,Yabiao Wang,Yong Liu,Jiangning Zhang*

Main category: cs.CV

TL;DR: 本文提出了DAS，一个有状态的智能体框架，用于生成面向出版的学术综述，通过分离可复用的论文分析与特定主题的稿件构建，实现高质量综述的自动生成。


<details>
  <summary>Details</summary>
Motivation: 学术综述的构建需要广泛的论文分析、连贯的知识组织、细粒度的引文支持和可靠的稿件组装，但现有系统通常未能通过共享可修订的状态来协调论文理解、文献组织、证据支撑的起草和稿件验证等流程。

Method: DAS框架的核心是将可复用的论文分析与特定主题的稿件构建分离，基于包含约200万篇论文的DAS-2M元数据湖，通过候选支撑的分类规划、论文到章节的反向路由、层次化声明与引文规划，以及语义审查触发作用域内闭环修复等机制来维护显式的文献、组织、写作和定稿状态。

Result: 在30个主题的DAS-Bench评估中，DAS在引用质量、分类综合、层次化论述和稿件组装可靠性四个维度上均获最高分，总分4.34（最强竞争者为4.03）；盲审专家评估在30个主题中有27个偏好DAS胜过Naive RAG，在21个共享计算机科学主题中有19个偏好DAS胜过AutoSurvey。

Conclusion: DAS通过有状态的智能体框架有效解决了学术综述自动生成中的多流程协调问题，在多个评估维度上均优于现有系统，表明将论文分析与稿件构建分离并通过共享可修订状态协调各流程是生成高质量学术综述的有效方法。
...
### [2] [Code as Representation: A Compilable Parsing Paradigm for Academic Documents](http://arxiv.org/abs/2608.17550v1)
...
TL;DR: 提出将学术文档解析为可编译LaTeX和可执行Python的新范式及基准，以保留结构化学术元素。


<details>
  <summary>Details</summary>
Motivation: 学术论文包含表格、公式等结构化学术元素，现有格式（如Markdown）难以有效保留其结构、数据和逻辑，阻碍了多模态大语言模型对PDF科学知识的机器利用。

Method: 提出可编译学术文档解析（CADP）范式，将页面重建为上下文LaTeX和可执行Python；构建专家验证的基准CADP-Bench，并使用多智能体基线和SOTA MLLMs进行评估。

Result: 实验结果表明，即使是前沿模型在生成高保真可执行重建方面仍存在困难。

Conclusion: 结构感知的科学文档解析仍有巨大改进空间，已发布CADP-Bench以促进未来研究。
...
### [3] [REChart: Reasoning-Efficient Chart Editing with Large Reasoning Models](http://arxiv.org/abs/2608.17414v1)
*Yuanbang Liu,Chenxi Ruan,Yihan Hou,Qiong Luo,Wei Zeng*

Main category: cs.CV

TL;DR: 本文提出了REChart两阶段训练框架，通过过程级监督和强化学习解决大型推理模型在图表编辑任务中的“过度思考”问题，实现高保真度与高效率的图表编辑。


<details>
  <summary>Details</summary>
Motivation: 图表编辑任务要求模型具备细粒度视觉推理和代码合成能力。大型推理模型的长思维链虽适合复杂任务，但存在“过度思考”现象，即过长的推理会导致视觉幻觉或冗余循环，从而降低性能。

Method: 提出REChart框架：第一阶段利用角色特化智能体工作流合成20万高质量推理轨迹进行监督微调；第二阶段通过强化学习优化，结合保真度奖励（评估代码与视觉一致性）和效率奖励（随机思考预算与截断机制）进行过程级监督。

Result: 在ChartEdit和ChartMIMIC基准上达到同等规模开源模型的最优性能，同时缓解了过度思考现象，在16384 token最大预算下平均推理token使用量减少了79.0%。

Conclusion: REChart有效解决了图表编辑中的过度思考问题，在提升编辑质量的同时大幅提高了推理效率，为多模态任务的推理长度优化提供了有效方案。
...
### [4] [MS-MFAD : Multimodal large language models for Face Anti-spoofing Detection](http://arxiv.org/abs/2608.17328v1)
*Xiaoyong Yu,Rongzhen Li,Shuming Shi,Xinge You*

Main category: cs.CV

TL;DR: 本文提出基于多模态大语言模型的MFAD系统，通过细粒度像素语义锚定与少样本高质量标注，实现了可解释、鲁棒且高性价比的统一人脸防伪检测。


<details>
  <summary>Details</summary>
Motivation: 现有人脸防伪系统面临生成式AI与物理欺骗的复合威胁，且存在泛化性差、推理不可审计及过度依赖大规模低质量数据集等系统性瓶颈。

Method: 提出MFAD系统，通过细粒度像素-语义锚定机制激活多模态大语言模型的内在推理能力以消除定位幻觉；采用跨攻击语义级统一标注范式（每类1000个掩码）生成推理证据链，并在Qwen-VL模型上进行监督微调。

Result: 在有限高质量样本下，域内ACER相对降低40%-50%，跨域性能退化控制在11.62%/5.23%内；白盒对抗攻击下精度仅降3.2%；专家对推理路径可靠性评分为4.57/5，且推理延迟满足实时部署要求。

Conclusion: 少样本、高质量语义标注范式能有效构建可信赖、可解释且高性价比的统一人脸防伪检测系统。
...
### [5] [Lymphocyte Mimicry Correction via Region-Level Tissue Reasoning and Unbalanced Optimal Transport](http://arxiv.org/abs/2608.17151v1)
*Xiang Li,Yuqi Wang,Casey C. Heirman,Jihye Heo,Kyle J. Lafata*

Main category: cs.CV

TL;DR: 本文提出Loki-OT方法，利用非平衡最优传输将区域级组织推理传播至细胞级预测，以解决病理图像中的细胞拟态问题。


<details>
  <summary>Details</summary>
Motivation: 在解决形态相似的细胞拟态问题时，现有视觉模型无法像人类病理学家那样有效利用组织上下文，尽管预训练的细胞基础模型特征中已包含此类上下文信息，但标准细胞级监督未能有效利用它们。

Method: 提出Loki-OT，通过非平衡最优传输将区域级MLLM推理传播至单细胞预测，利用MLLM衍生的密度先验作为软指导进行模糊细胞重分配，并将传输计划蒸馏为轻量级学生MLP分类器。

Result: 在TCGA-BRCA队列上，Loki-OT的患者级平均绝对误差（MAE）低于全监督域内分类器，并在富含上皮细胞的拟态组织中提升了F1分数，且仅使用了278个弱区域级MLLM估计。

Conclusion: 将区域级组织上下文与最优传输结合并蒸馏到细胞级分类器，能有效解决细胞拟态问题，提升预测准确率，并减少对大规模强监督数据的依赖。
...
### [6] [AnchorScore: A CLIP-Based Diagnostic of MLLM Annotation Difficulty](http://arxiv.org/abs/2608.16690v1)
*Yan Ma,Lizhuo Zhang*

Main category: cs.CV

TL;DR: 本文提出利用零样本CLIP准确率（AnchorScore）作为低成本先验诊断信号，预测多模态大语言模型在各类别上的标注难度。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLM）在自动标注中各类别准确率差异巨大（12%-98%），且评估成本高昂（27B模型评估5416张图片约需14小时，而CLIP仅需3分钟），缺乏低成本信号来预先排序类别标注难度。

Method: 基于AnchorProxy构造，系统评估其全帧形式AnchorScore（即各类别零样本CLIP准确率）作为先验诊断指标，并与DINOv2、ResNet-50、SigLIP及MLLM自述不确定性等替代预测因子进行对比验证。

Result: 在课堂行为数据上，AnchorScore与MLLM各类别准确率显著相关（Spearman ρ=0.769, p=0.002），替代预测因子均无显著相关；在Stanford40 Actions上复制得到近乎一致的效果（ρ=0.817, p<0.001）。实际应用中，混合CLIP/MLLM路由策略较纯CLIP提升23个百分点，同时节省约44%的MLLM成本。

Conclusion: AnchorScore不估计MLLM的精确准确率，而是提供低成本类别排序信号，可将昂贵的MLLM评估导向最具信息量的类别，适用于活动识别数据效果最佳，在医学和卫星图像上效果减弱。
...
### [7] [Multi-Agent AI System for Radiology Report Structuring and Quality Assurance with Independent Radiologist Evaluation](http://arxiv.org/abs/2608.18072v1)
*Iryna Hartsock,Cesar Lam,Christopher Otteni,Aliya Qayyum,Robert Gatenby,Cyrillo Araujo,Ghulam Rasool*

Main category: cs.CL

TL;DR: 本文开发并评估了一种用于放射学报告结构化和质量保证的本地部署多智能体AI系统，表现出良好的性能和临床应用潜力。


<details>
  <summary>Details</summary>
Motivation: 旨在解决放射学报告标准化和质量保证的需求，通过自动化工作流提高报告结构化程度并减少潜在的人为错误（如段落不一致、性别解剖冲突及关键发现未记录等）。

Method: 构建多智能体AI流水线，结合正则表达式规则和本地大语言模型，在句子级别将报告划分为标准化解剖结构，并自动检测发现与印象部分的不匹配、性别解剖冲突及未记录的关键发现沟通。

Result: 系统成功对全部638份报告（22,270个句子）进行结构化，标记了90份（14.1%）存在问题的报告。放射科医师评估45份报告子集显示，69%被正确重构，无重要信息遗漏或捏造，84%的总体质量保证表现被评为“优秀”或“良好”。

Conclusion: 本地部署的多智能体AI系统能在单一工作流中有效结合报告结构化与质量保证，表现优良，有望支持放射学实践中的报告标准化与质量控制。
...
### [8] [Language Has Two Parameters: Narrative-Induced Semantic Plasticity and Phase-Sensitive Interpretation](http://arxiv.org/abs/2608.18041v1)
*Hollis Robbins*

Main category: cs.CL

TL;DR: 本文提出语言存在第二个参数——相位（phase），它决定了共存意义如何组合，并论证当前语言模型和可解释性方法缺乏对该参数的表征。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型仅捕获词的关联强度（振幅），忽略了决定意义如何组合的第二参数（相位）；群体平均化的语料库删除了历史索引的相位信息，标准Transformer在推理时无显式相位表征，且以单义性为目标的可解释性研究正在优化掉相位所支撑的歧义共存现象。

Method: 提出相位参数的理论框架：相位是符号化的、持久的、索引到个体和二元关系的参数，存在于意义之间，决定共激活意义的组合方式，可逆转某意义的贡献而不消除该意义；以量子概率作为该参数的一种记法（不主张大脑中存在量子过程），并提出六项预测检验抑制意义是否保持活跃、相遇顺序是否改变短语功能、信号标记是否改变共享短语的解读、以及历史是否改变模型状态。

Result: 论证了弱版本主张：解释需要第二个关系参数（符号化、持久、索引到个体和二元关系）；强版本主张（量子微积分约束这些现象）依赖于尚未推导出的相遇顺序约束；指出现有可解释性程序以单义性衡量进度是在优化掉相位所支撑的互文、反讽和引用等现象。

Conclusion: 理论所需的架构是具有代理索引、承载相位的语义状态的语言模型；未来方向包括推导相遇顺序约束以验证强版本主张，以及在模型中显式引入相位表征以处理意义共存与组合。
...
TL;DR: 本文提出了一种无需训练的解码方法Ripple-Pivot Search (RPS)，通过利用扩散语言模型解码中的涟漪效应，在保持生成质量的同时实现显著的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有dLLM并行解码调度器仅在满足逐位置准则后才提交位置，忽略了早期提交对后续解码的潜在益处。本文发现了一种涟漪效应：主动提交中等熵的枢轴位置可以显著降低其余掩码位置的不确定性，从而加速整体解码过程。

Method: 提出Ripple-Pivot Search (RPS)，一种无需训练的解码方法：(1)寻找中等熵位置作为有前景的候选枢轴（决定在哪解码），(2)通过前瞻评估确定能产生最大下游收益的token分配（决定解码什么）。

Result: 在3个dLLM和4个推理与代码生成基准上，RPS在保持生成质量的同时实现4-10倍壁钟时间加速，较此前前瞻基线准确率提升最高5.49%，且在多数设置下吞吐量更高；与KV缓存结合后，最高实现18倍壁钟时间加速。

Conclusion: RPS通过有效利用dLLM解码中的涟漪效应，在不牺牲生成质量的前提下实现了显著的推理加速，表明战略性早期提交能够有效促进后续并行解码，为dLLM高效推理提供了新思路。
...
TL;DR: 本文提出了Prefix-Conditioned Diffusion (PCD)预训练目标，通过结合自回归前缀监督和后缀去噪，解决扩散语言模型预训练与提示条件生成之间的不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型原生预训练时会同时随机破坏提示词和后续词，削弱了提示条件生成所需的干净前缀接口，导致训练与生成环境不匹配。

Method: 提出PCD预训练目标，修改注意力掩码、破坏掩码和标签构建，对干净前缀侧进行自回归监督，仅对未知后缀应用扩散去噪，并分离样本内前缀条件与样本间目标混合。

Result: 在LLaDA2-Mini和Qwen-1.7B上，PCD相比原生dLLM基线持续提升，分别实现4.2%(+2.56分)和14.2%(+4.86分)的相对性能增益。

Conclusion: 将预训练上下文分布与提示条件生成相对齐，能够在不改变推理模式的情况下，有效缩小扩散语言模型在文本续写任务上的性能差距。
...
TL;DR: 本文揭示了掩码扩散语言模型在推理任务中因无序提交标记而导致的失败机制，并提出边界门控提交方法以恢复推理性能并保持并行解码。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型（dLLMs）被宣传为能以任意顺序提交标记，但在推理任务中，这种无序自由反而成为失败的根源，导致模型在推理区域仍被掩码时过早提交最终答案并退化为仅输出答案。

Method: 提出一种简单的单旋钮干预机制——边界门控提交（frontier-gated commitment），通过控制标记的提交可达性来恢复有序承诺，并沿测量的边界调整最优窗口大小。

Result: 实验表明无约束解码会导致模型在15-24%轨迹处提前提交答案，并在多达90%的问题上崩溃为仅输出答案；而边界门控提交将准确率从0.528因果性地恢复至0.852，同时保持高达4倍的并行解码速度；此外，思维链仅在有序提交条件下有效。

Conclusion: 现有的窗口式采样器虽然最初是为提升效率而设计的，但实际上无意中成为了解决这种dLLMs推理病态问题的最小修复方案。
...
TL;DR: 本文提出OPTD，一种基于一致性引导自适应压缩的在策略转换蒸馏方法，用于加速扩散语言模型的少步解码。


<details>
  <summary>Details</summary>
Motivation: 现有少步蒸馏方法在off-policy轨迹上构建监督，导致学生模型推理时实际访问的状态与监督状态发生偏离，尤其在步压缩最激进时。on-policy蒸馏虽能缓解此不匹配，但如何确定每个转换应推进多远仍存在挑战：仅匹配教师下一步动作限制压缩，盲目合并未来动作可能违反中间依赖。

Method: OPTD从少步学生自身轨迹采样部分状态，使用冻结的仅问题教师模型识别结果对齐的未来候选，并按当前状态置信度排序；选择保持教师rollout结果的最长前缀作为目标；采用set-bottleneck目标将验证的未来候选提升至解码器释放阈值，同时用冻结教师KL锚点正则化其他活跃位置。目标构建和训练均不使用黄金响应。

Result: 在四个数学推理和代码生成基准上，OPTD持续改善质量-效率权衡，在所有评估的少步基线中获得了最强的整体质量约束AUP。

Conclusion: OPTD通过在策略蒸馏与一致性引导的自适应压缩相结合，有效解决了off-policy蒸馏中的状态漂移问题，实现了更好的质量-效率平衡，无需黄金响应即可完成训练。
...
TL;DR: 本文提出DominoTree，一种无需训练的最佳优先草稿树方法，利用Domino草稿器的路径依赖条件校正来构建草稿树，显著提升了LLM推理中投机解码的接受长度和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有树方法（如DDTree）的因子化公式无法表示Domino草稿器中基于GRU的因果校正所带来的路径依赖（非因子化）分布，限制了投机解码的加速效果。

Method: 提出DominoTree，一种无需训练的最佳优先草稿树构建方法，沿每条根到节点路径使用Domino的条件（非因子化）校正进行评分，并通过将每节点校正限制为候选top-M集合来保证实用性。

Result: 在8个基准测试中，DominoTree在所有服务单元（两种模型规模、单请求与并发负载、32K上下文）均获得最高平均接受长度；在24个数据集单元中21个取得最优；吞吐量相比AR最高达7.3倍，在SGLang中单请求吞吐量比Domino高12%；在HELMET长上下文场景中接受长度提升29-36%、吞吐量提升10-34%。三分解实验表明应用校正贡献+10.1%接受长度，沿路径重计算校正额外贡献+4.7%。

Conclusion: DominoTree通过利用路径依赖条件校正构建草稿树，在不需额外训练的前提下显著提升了投机解码的接受长度和吞吐量；在最长上下文且预填充主导的场景中与EAGLE-3持平，在内存受限场景下链式方法在吞吐量上仍有优势。
...
TL;DR: 提出CaDDTree方法，通过联合优化树结构和节点预算直接最大化token吞吐量，无需离线预算搜索即可匹配或超越使用oracle预算的DDTree。


<details>
  <summary>Details</summary>
Motivation: 现有DDTree方法在固定节点预算下构建候选树以最大化期望接受长度，但接受长度随预算非递减，总是偏好更大的树而忽略验证成本，缺乏原则性的预算选择依据。

Method: CaDDTree显式建模草稿和验证延迟，将吞吐量目标分解为每轮对预算的一维搜索；在凸验证成本假设下证明吞吐量函数为单峰，从而采用高效贪心停止规则，每轮根据当前位置分布和验证成本自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B上跨八个基准（推理、编码、指令遵循）的实验表明，CaDDTree在几乎所有任务上匹配或超越使用oracle预算选择的DDTree。

Conclusion: CaDDTree通过成本感知的联合优化消除了预算选择的经验性调参需求，在保持或提升性能的同时实现自适应在线决策，为推测解码中的树构建提供了原则性框架。
...
TL;DR: 本文提出DDTree方法，通过从块扩散草稿模型的逐位置分布构建草稿树，提升推测解码的接受长度和整体性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash方法每轮仅验证单条草稿轨迹，限制了接受长度，从而制约了推测解码的加速效果。

Method: DDTree利用块扩散草稿模型的逐位置分布构建草稿树，在固定节点预算下使用最佳优先堆算法选择最可能匹配目标模型的续写，并通过仅祖先注意力掩码在单次前向传播中完成高效验证。

Result: 基于DFlash构建的DDTree成功提升了接受长度，使其成为当前领先的推测解码方法之一。

Conclusion: 通过将单轨迹验证扩展为树结构验证，DDTree有效克服了现有方法的局限性，显著提升了推测解码的性能上限。
...
### [16] [BEAR-Bench: A Bilingual Enterprise and Academic Reasoning Benchmark for Multimodal Models](http://arxiv.org/abs/2608.17895v1)
*Liubov Chubarova,Alexandra Kuleshova,Daniil Volkov,Kirill Sultanov,Alexey Zaytsev*

Main category: cs.CL

TL;DR: 本文提出了BEAR-Bench，一个基于文本密集型商业和科学文档的英俄双语基准，用于评估多模态大语言模型的推理能力及幻觉检测方法。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注信息提取、依赖外部知识或以英语/中文为中心，未能充分评估多模态大语言模型在文本密集型专业文档上的推理能力，且对俄语等其他语言的代表性严重不足。

Method: 构建了包含1000个人工标注问题的英俄双语基准BEAR-Bench，基于文本丰富的商业和科学文档；评估了包括Gemini 3.1 Pro和Qwen3.5-397B在内的16个模型，并比较了现有的幻觉检测方法。

Result: 评估结果显示，即使是最强的模型在BEAR-Bench上仍有明显的改进空间；同时，通过分析模型输出，对比了现有幻觉检测方法在识别这些失败时的可靠性。

Conclusion: BEAR-Bench填补了多模态模型在双语专业文档推理评估方面的空白，揭示了当前模型和幻觉检测方法的局限性，为未来的改进提供了重要参考。
...
### [17] [Auditing Exposure to Harmful Content on TikTok using Multimodal Language Models: A Cross-National, Age-Stratified Study](http://arxiv.org/abs/2608.17583v1)
*Hamidreza Saffari,Francesco Pierri*

Main category: cs.CL

TL;DR: 本文利用马甲账户和经验证的多模态大语言模型，对法、意、瑞三国的TikTok青少年有害内容暴露进行审计，发现搜索模式下有害内容激增且跨国差异显著。


<details>
  <summary>Details</summary>
Motivation: 在线视频平台会向青少年暴露有害内容，但由于视频标注成本高且跨语言的审核判断存在差异，难以进行独立审计。

Method: 创建四个年龄段的马甲账户，在法、意、瑞三国通过被动滑动和主动关键词搜索收集视频；通过多模态LLM与母语者标签对比，采用性能最优且成本低的Gemini 2.5 Flash（8帧+文本）进行大规模视频标注。

Result: 关键词搜索的有害内容占比为35-56%，比被动滑动高1.5-7.5倍；意大利被动滑动各年龄段有害率最高（19岁达48.6%）；而LLM提供商的安全过滤器（拒绝率1.1%）低估了最明显的有害内容。

Conclusion: 基于MLLM的审计为跨国青少年安全审计提供了可扩展的方案，但当前提供商的安全过滤器不足以准确统计最明显的有害内容。
...
### [18] [An Investigation of Translationese in the Generations of Multilingual Large Language Models](http://arxiv.org/abs/2608.17399v1)
*Maria Valentini,Téa Wright,Julisa Granados,Eliana Colunga,Katharina von der Wense*

Main category: cs.CL

TL;DR: 本文研究了多语言大语言模型生成的文本是否带有“翻译腔”特征，并分析了其与直接翻译产生的翻译腔的区别。


<details>
  <summary>Details</summary>
Motivation: 旨在探究MLLMs生成的多语言文本是否源于内部翻译从而产生“翻译腔”，以及这种翻译腔与人类直接翻译的差异，以填补该领域的研究空白。

Method: 利用既有的翻译文本指标评估五种语言的最先进MLLM生成文本，并与非翻译及人类翻译基线进行对比；综合使用高精度分类模型、语言学特征方差分析以及德语和西班牙语的人类标注进行评估。

Result: 评估了MLLM生成文本中的翻译腔含量，并提取出区分MLLM生成文本与典型翻译干扰的关键语言学特征。

Conclusion: 揭示了MLLM生成文本与人类直接翻译在翻译腔特征上的差异，为理解多语言大模型的语言生成机制及内部翻译过程提供了实证依据。
...
### [19] [Uncertainty-Aware Decision Making in Multimodal Large Language Models](http://arxiv.org/abs/2608.17084v1)
*Abderrahmene Boudiaf,Irfan Hussain,Sajid Javed*

Main category: cs.CL

TL;DR: 本文综述了不确定性感知的多模态大语言模型（MLLMs），提出了一个以决策为中心的框架，强调不确定性应通过其改善系统行为的能力来评估。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的错误不仅源于语言层面，还包括感知错误、接地薄弱、模态冲突、推理不稳定等。现有研究多将不确定性仅视为置信度数值，未能充分探讨其在证据不足或高风险情况下对系统行为的改善作用。

Method: 提出一个以决策为中心的框架：将不确定性来源映射为可观测信号，对信号进行校准或风险控制，并利用校准后的不确定性来决定系统动作（如选择性回答、弃答、澄清、检索、自检或升级）。

Result: 综述了在词元不确定性、语义分歧、一致性预测、弃答等方面的相关工作，论证了不确定性不应仅作为置信度数值被评估，而应根据其在多模态证据不足、冲突、分布偏移或高风险时是否改善系统行为来评估。

Conclusion: 指出了未来研究的开放性问题，包括源感知分解、动作感知基准、分布偏移下的校准、黑盒不确定性估计、更广泛的模态覆盖、可复现的报告以及以人为中心的不确定性交流。
...
### [20] [On the Fragility of Self-Improving Agents: Variance, Task Order, and Underspecification](http://arxiv.org/abs/2608.18066v1)
*Qinyuan Ye,Yu Li,Yada Pruksachatkun,Jiaxin Zhang,Chien-Sheng Wu*

Main category: cs.AI

TL;DR: 本文通过多次运行和打乱任务顺序重新评估了基于记忆的自改进代理，发现其性能极易受评估噪声和任务顺序影响，并指出任务与环境规范不足是导致该脆弱性的关键原因。


<details>
  <summary>Details</summary>
Motivation: 先前研究忽视了基于记忆的自改进代理的可靠性问题。本文旨在揭示这些方法在复杂环境中的脆弱性，包括评估噪声放大和对任务顺序的过度依赖，以推动更严谨的评估协议。

Method: 通过多次运行量化方差和随机打乱任务顺序来重新评估两种现有方法；手动检查代理记忆并提出任务与环境规范不足的假设；通过向记忆构建过程中引入详细评分标准和环境反馈等信息来验证该假设。

Result: 发现复杂环境中的评估存在固有噪声，自改进循环会放大该噪声；代理的改进高度依赖任务顺序，默认顺序构成了隐式课程；引入额外规范信息仅部分缓解了性能退化，仍存在显著差距。

Conclusion: 呼吁为自改进代理建立跨多次运行和压力测试的严谨评估协议，并建议开发支持人类有效监督的系统与接口，以应对规范不足问题并防止不可预见的失败。
...
### [21] [Delegation Asymmetry in Agentic Recommender Systems: Measuring Two-Sided Receptivity in Online Dating](http://arxiv.org/abs/2608.18058v1)
*Daria Leshchikova,Valentina V. Kuskova,Dmitry Zaytsev,Valerii Klimov*

Main category: cs.AI

TL;DR: 本文通过大规模调研研究了匹配平台用户对自主LLM代理通信的接受度，发现用户对发送代理通信和接收代理通信的意愿是可分离的构念，且存在显著的委派不对称性。


<details>
  <summary>Details</summary>
Motivation: 自主LLM代理在匹配平台上代替用户进行对话是一种新兴设计模式，但其可行性依赖于用户既愿意委派对话给代理，也愿意接收来自他人代理的通信。这一条件此前鲜有研究，本文旨在量化用户对代理通信的双向接受度，并评估其在实际匹配场景中的可行性。

Method: 对某主流约会平台的活跃用户进行两次大规模调研（生成式画像特征N=2,894；自主对话代理N=2,617），构建基于分级响应模型和潜在回归的潜变量测量模型，进行跨语言的部分测量不变性检验，并基于用户陈述的接受度推导随机配对反事实场景和设计反事实分析。

Result: 发送意愿与接收意愿高度相关（rho=0.92）但可分离（Delta BIC=52）；存在系统性委派不对称——部署自有代理的接受阈值（-0.38）远低于对接收方代理的参与阈值（+0.32），部署倾向约为参与倾向的3倍；在随机配对反事实下，仅4-13%的有向双人组同时满足代理部署与接收参与；基于接收接受度的路由策略使每次接触的参与率提升3倍（AUC 0.88，四分位提升3.1倍）。

Conclusion: 自主代理在匹配平台上的大规模应用受限于用户对接收代理通信的低接受度，且存在显著的性别方向性失衡。设计层面应引入互惠性要求、披露机制和基于接收接受度的感知路由匹配策略，以提升代理通信的有效性和用户接受度。
...
### [22] [StagedWorkspace: A Versioned Workspace for Knowledge-Work Agents](http://arxiv.org/abs/2608.18050v1)
*Yining Hua,Hongbin Na,Yifan Zhou,Akshay Kalose,Cyrus Ayubcha,Levi Lian*

Main category: cs.AI

TL;DR: 提出StagedWorkspace版本化工作空间，通过将解析视图和审查差异绑定到原生文件的内容哈希，解决知识工作Agent中视图与工作产品版本不一致的问题。


<details>
  <summary>Details</summary>
Motivation: AI Agent在进行知识工作时，其搜索的解析视图、编辑的原生文件、审查的变更和提交的产物可能指向同一工作产品的不同版本。编码Agent通过仓库合约部分解决了此问题，但PDF、电子表格、幻灯片、笔记本等格式缺乏类似的显式版本绑定合约，导致状态不一致。

Method: 提出StagedWorkspace——一种面向知识工作Agent的版本化工作空间。该工作空间将解析记录和审查差异（review diffs）绑定到原生文件随其变化的内容哈希上，从而建立工作空间状态的显式版本关联，确保每个视图都明确对应演化中的工作空间状态的某个版本。同时构建了SW-AGENT进行评估。

Result: 在OfficeQA Pro和APEX-Agents的消融实验中，双解析/原生访问模式在所有测试模型上均获得最高点估计；相较于单一视图，OfficeQA Pass@1提升8.3-12.1分，APEX平均rubric分数提升4.7-9.2分。SW-AGENT在OfficeQA上达63.9%（Gemini 3.1 Pro），在APEX上达42.1（GPT-5.4 Nano），远超已发表的同模型分数（29.3%和25.5）。57项文件编辑任务的配对审查消融显示，可见差异时得分更高。

Conclusion: 工作空间状态是知识工作Agent中的一个重要实验变量。结果促使未来基准测试应将证据、分阶段编辑和提交产物作为显式状态转换进行评分，激励对工作空间状态管理的进一步研究。
...
### [23] [Continuous Interaction Diffusion: A Diffusion-Native Runtime for Asynchronous Tool-Augmented Reasoning](http://arxiv.org/abs/2608.10438v1)
*Yuhang Cao*

Main category: cs.AI

TL;DR: 本文提出了连续交互扩散（CID）架构，将工具交互集成到扩散语言模型的迭代去噪过程中，以克服传统自回归式工具调用模式的局限性。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行迭代细化输出进行推理，而传统的“停止-恢复”工具调用模式过于受限，会迫使模型在推理稳定前做出决策、延迟观察并导致冗余细化与执行，从而影响任务准确性和推理效率。

Method: 提出连续交互扩散（CID）架构，将工具交互融入迭代去噪。CID分离了只读事实通道、由类型化认知张量表示的思考通道和显示通道；允许在调用完全序列化前启动外部读取，将结果投影到演变的思考状态中以修改早期认知，并利用持久绑定重用或刷新结果。

Result: 本文形式化了该架构、运行时和训练目标，并定义了任务质量和端到端效率的评估协议。作为系列首篇，本文仅关注只读工具，未提出任何经验性性能声明。

Conclusion: CID旨在更早暴露证据、实现工具延迟与模型计算的重叠、减少重复的外部工作，并在新证据到达后保留有用的计算，为提升扩散语言模型工具交互的效率和效果奠定了基础。
...
TL;DR: 本文系统研究MoE扩散语言模型的缩放规律，并基于发现训练了LLaDA MoE v2模型，在较少预训练token下接近Qwen3性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归模型的替代方案，但MoE dLLMs的缩放行为尚未被充分理解，缺乏实用的缩放规律和设计原则。

Method: 系统刻画MoE dLLMs的优化超参数、计算分配和架构规模的缩放行为，识别与AR模型的定量差异，并据此指导从零训练30B-A3B的LLaDA MoE v2模型。

Result: 发现最优批次大小增长更快、学习率衰减更快、最优token预算增长快于激活计算量；更大规模倾向更大专家池；LLaDA MoE v2在约65%的Qwen3预训练token量下，在多个知识、推理和编码基准上接近Qwen3，SFT后在8个基准中7个超越SDAR Chat。

Conclusion: 确立了MoE dLLMs的实用缩放规律和架构设计原则，为扩散语言模型的规模化发展提供了理论和实践指导。
...
TL;DR: 提出TAPS方法，通过目标感知的前缀选择优化基于扩散模型的推测解码中的草稿树验证，显著提升无损推理速度。


<details>
  <summary>Details</summary>
Motivation: 扩散模型用于推测解码时，现有草稿树方法基于边际概率排序节点而忽略了验证是基于前缀条件的，导致验证不可达节点并增加延迟。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边际概率转化为路径条件的接受估计，并在固定验证预算下选择紧凑的前缀封闭子树。

Result: 在多个数据集和模型上的实验表明，TAPS相比自回归解码实现高达7.9倍的无损端到端加速，分别比SOTA的DFlash和DDTree快1.36倍和1.74倍。

Conclusion: TAPS有效解决了草稿树验证中的前缀条件不匹配问题，改善了接受率与成本的权衡，实现了更优的端到端加速效果。
...
### [26] [COMIC: Reference-Aware Safety Gating for Multimodal Large Language Models](http://arxiv.org/abs/2608.17234v1)
*Md Abdullahil Oaphy,Anhao Xiang,Zongxing Xie,Huayue Gu,Chenyu Wang,Honghui Xu*

Main category: cs.CR

TL;DR: 本文揭示了多模态大语言模型中由视觉引用解析触发的安全失效模式，并提出COMIC这一引用感知的预生成安全门控机制加以缓解。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM安全防御将提示-图像对作为整体进行审核，但实际的安全风险单元是操作-目标对——当模型将摘要、翻译等看似良性的操作绑定到局部视觉目标时，有害行为才会涌现，现有防御在有害语义局部化且依赖引用解析时显著退化。

Method: COMIC首先推断请求的操作类型和引用类型，利用OCR和开放词汇提议构建候选目标，定位合理指代对象，然后对显式的操作-目标对进行安全评估；在处理歧义时，采用最大风险聚合与质量感知路由相结合的保守策略，决定是否放行或拦截请求。

Result: 在多个开源MLLM、局部化及更广泛的多模态越狱基准测试和良性引用敏感场景上的实验表明，COMIC持续提升了鲁棒性，同时保持了良性效用和实际效率。

Conclusion: 多模态安全无法在仅审核提示-图像对的前提下可靠实现，必须同时对请求的操作、操作所施加的视觉目标以及引用解析的置信度进行建模。
...
### [27] [Planning Against Learning in Rank-1 Games](http://arxiv.org/abs/2608.18067v1)
*William Overman*

Main category: cs.GT

TL;DR: 本文证明在秩为1的双矩阵博弈中，对抗学习对手的战略规划是NP-hard的，即使均衡计算可高效完成。


<details>
  <summary>Details</summary>
Motivation: 近期研究表明对抗复制器动态的优化在零和博弈中可处理，但在一般和博弈中困难。本文旨在研究超越零和博弈的第一个结构化类别——满足rank(A+B)=1的双矩阵博弈，以探索均衡可计算性是否能扩展到对抗学习动态的规划问题。

Method: 研究在rank(A+B)=1的双矩阵博弈中，优化者对抗遵循复制器动态的学习者时的规划问题。通过计算复杂性分析证明近似优化者最优连续时间奖励的NP-hard性，并对若干可处理特例进行结构刻画。

Result: 除非P=NP，即使rank(A+B)=1、学习者从均匀状态开始、优化者限于常数策略，近似优化者最优连续时间奖励在固定加性常数内仍为NP-hard。该困难性在有界收益矩阵和多项式有界视野下仍然存在。同时识别并刻画了若干可处理的特殊情形。

Conclusion: 秩为1博弈已经将高效的均衡计算与对抗学习对手的战略规划分离开来，表明均衡可计算性并不蕴含对抗学习动态规划的可处理性。
...
### [28] [Quo Vadis? Scientific Discovery in the Age of Artificial Intelligence](http://arxiv.org/abs/2608.17970v1)
*Petr O. Jedlicka*

Main category: cs.CY

TL;DR: 本文综述了人工智能在科学发现中的日益增长的作用，提出了研究型AI系统的类型学，并分析了其局限性、风险及人机认知劳动分工问题。


<details>
  <summary>Details</summary>
Motivation: 旨在探讨AI在科学发现中日益增长的作用，系统分析其能力扩散现状，并识别其在实际应用中面临的限制与潜在风险。

Method: 结合文献综述与科学计量学分析，提出涵盖专用科学AI、AI助手/代理及混合实验系统的类型学，并跨学科概述其最新成就。

Result: AI在推理、规划和长周期任务执行等能力上快速提升并在多学科取得进展，但当前系统仍受技术、认识论和制度层面的制约，且引入了近期与长期风险。

Conclusion: AI在科学领域的进步引发了人类研究人员与机器之间认知劳动分工的更广泛问题，未来需正视并解决相关局限性与风险。
...
### [29] [Policy-Invariant Reward Shaping from LLM Feedback: A Framework for Hybrid RL Agents](http://arxiv.org/abs/2608.18008v1)
*Christophe D. Hounwanou,John Emeka Eze,Yaé U. Gaba*

Main category: cs.LG

TL;DR: 本文将LLM与RL的混合架构形式化为目标增强马尔可夫决策过程，并证明了将LLM评分作为有界势函数进行奖励塑形可在评分不准确时依然保持最优策略集。


<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）与强化学习（RL）的结合日益普遍，但LLM生成的奖励信号的理论地位往往未被明确，缺乏理论保证。

Method: 将混合LLM规划器与RL控制器架构形式化为目标增强马尔可夫决策过程（Goal-Augmented MDP），并将LLM的每状态进度评分作为有界势函数用于奖励塑形。

Result: 在小型MDP的四种势能配置（包括奖励幅度放大20倍的对抗性配置）下进行数值验证，结果表明即使LLM评分不准确，该塑形项也能保持最优策略集，这比一般LLM作为奖励的方法提供了更强的理论保证。

Conclusion: 将LLM评分作为有界势函数可确保在不精确评分下仍能保持最优策略，为LLM与RL的混合架构提供了坚实的理论基础和更强的保证。
...
TL;DR: 本文提出一致性强制（CForce）方法，通过对齐早期与后期去噪阶段的掩码预测，提升扩散大语言模型在高并行度下的生成质量与速度。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在激进并行解码策略下，早期去噪阶段预测不可靠，导致错误向后期传播，从而影响整体生成效果。

Method: 提出CForce蒸馏方法，在预收集的自展开轨迹上训练模型以提升训练-推理一致性；引入置信度自适应KL散度结合正向与逆向KL的优势，强制早期阶段预测与后期阶段对齐。

Result: 在LLaDA模型上的实验表明，CForce在非编辑和可编辑解码场景下均改善了速度-质量权衡，尤其在高并行度解码预算下表现突出。

Conclusion: CForce理论上能近似最小化早期阶段预测误差，有效提升高并行度下的模型性能，且该方法具有广泛的适用性。
...
### [31] [Diffusion LLMs as Targets and Adversaries: Mechanistic Safety Exploits](http://arxiv.org/abs/2608.07430v1)
*Elena Dumitrescu,Gert Lek,Lydia Y. Chen,Jérémie Decouchant*

Main category: cs.LG

TL;DR: 本文揭示了扩散大语言模型(DLLMs)安全对齐机制的稀疏性与可迁移性漏洞，并提出了基于安全神经元引导的离线黑盒越狱框架SN-Guided Diffusion。


<details>
  <summary>Details</summary>
Motivation: DLLMs用迭代并行去噪替代自回归预测，但其内部安全机制尚未被充分理解。本文旨在揭示扩散式安全对齐的机制性脆弱性，探索DLLMs作为攻击目标和攻击者的双重角色。

Method: 发现DLLMs从自回归前身模型继承相同的安全神经元足迹，使安全神经元映射与剪枝迁移攻击成为可能。在此基础上提出SN-Guided Diffusion框架，利用加权安全神经元损失引导扩散过程远离安全触发区域，实现全离线黑盒越狱。

Result: 自剪枝将LLaDA攻击成功率从2.6%提升至73.8%，Dream从1.9%提升至86.6%；从Qwen2.5迁移剪枝使Dream ASR从1.9%升至73.2%，Fast-dLLM从7.0%升至86.3%。SN-Guided Diffusion实现AUROC=1.0的良性-越狱提示分离，在Llama-3-8B、Qwen2.5-7B和Gemini-2.5-Flash-Lite上分别达到77.1%、86.9%和74.3%的迁移ASR，且生成成本比现有框架低数个数量级。

Conclusion: DLLMs的安全对齐存在稀疏且可跨架构迁移的机制性漏洞，安全神经元足迹的继承性使迁移攻击成为现实威胁，亟需加强扩散模型的安全对齐机制设计。
...
### [32] [Retrofitting Linear Attention into Diffusion Language Models](http://arxiv.org/abs/2608.06628v1)
*Jinha Kim,Younghun Roh,Jaeyeon Kim*

Main category: cs.LG

TL;DR: 本文提出块混合注意力机制，将扩散语言模型中先前块的注意力线性化，在保持性能的同时实现最高1.7倍的解码吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（dLLMs）虽然通过并行解码加速推理，但每个去噪步骤仍需对所有先前块进行注意力计算，反复产生前缀注意力开销，成为推理瓶颈。

Method: 提出block-hybrid attention，在活跃去噪块内保留精确softmax注意力，对先前块应用线性注意力；基于LoLCAT方法，将LLaDA-2.1（16B参数dLLM）20层注意力中的6层替换为线性注意力，仅需约60小时后训练。

Result: LLaDA-Hybrid在基准测试上性能基本保持：HumanEval 72.0%对比75.6%、MBPP+ 63.0%对比57.7%、CMATH 86.7%对比88.3%；通过Triton实现，解码吞吐量提升最高1.7倍，且支持更多并发请求。

Conclusion: 预训练扩散语言模型可被高效线性化以实现更快推理，block-hybrid attention在性能与效率之间取得了良好平衡，为dLLM推理加速提供了可行路径。
...
### [33] [Beyond On-Policy Exploration: Integrating External Policy Rollouts for Reinforcement Learning in Diffusion Language Models](http://arxiv.org/abs/2608.01717v1)
*Wonseok Lee,Jimyeong Kim,Jungmin Ko,Wonjong Rhee*

Main category: cs.LG

TL;DR: 提出ERILS方法，通过长度控制与来源特定的奖励处理，将外部更强策略的高奖励轨迹整合至扩散大语言模型的强化学习中，解决了正样本稀缺导致的训练停滞问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的强化学习依赖目标模型自身生成的同策略轨迹，当成功轨迹稀缺时，模型难以获得正向奖励，导致训练进展受限。

Method: 提出ERILS框架，引入外部更强策略生成的高奖励轨迹，并通过控制外部轨迹长度以及分别独立处理同策略与外部轨迹的奖励，来解决长度差异和奖励处理不稳定的问题。

Result: 在Sudoku、Countdown和MATH500的零样本测试中均提升了多样本性能。在Sudoku上，best-of-4准确率达98.4%（最强基线为40.3%），且在不同生成长度下保持约90%的确定性准确率；消融实验验证了长度控制和独立奖励处理的必要性。

Conclusion: 在将外部轨迹整合到扩散大语言模型强化学习时，轨迹构建和奖励处理是关键设计维度；合理设计这些维度能显著提升模型性能并避免训练崩溃。
...
### [34] [aDSL: Agentic 3D Creation via Joint Agent-Program Design](http://arxiv.org/abs/2608.17975v1)
*Rui-Huan Wang,Si-Tong Wei,Jia-Qi He,Heng-Yi Wei,Baoquan Chen,Peng-Shuai Wang*

Main category: cs.GR

TL;DR: 本文联合设计了以智能体为中心的领域特定语言和免训练的多智能体系统，以解决大语言模型生成3D程序时在低层几何上的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 解决LLM智能体工作流在创作3D程序时过于脆弱、难以将高层意图转化为一致低层几何的问题。根本原因在于现有程序化接口与LLM擅长语义结构和空间关系而非精确数值的推理优势不匹配。

Method: 联合设计面向智能体的领域特定语言和角色专业化的免训练多智能体系统。aDSL通过强调可组合性和空间推理，允许使用关系算子而非绝对坐标操纵几何；多智能体系统遵循“计划-执行-批评”循环，利用执行反馈迭代修复错误与约束违例。

Result: 在文本到形状和图像到形状任务上优于现有基于LLM的基线方法，提升了鲁棒性、可控性和对用户意图的忠实度，并保留了显式结构、可编辑性与可解释性。

Conclusion: 该方法有效弥合了语义逻辑与几何约束的鸿沟，不仅提升了3D生成质量，还支持了铰接对象创建和结构化场景组合等下游应用。
