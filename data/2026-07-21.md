...
### [1] [O-VAD: Industrial Video Anomaly Detection through Object-Centric Tracking and Reasoning](http://arxiv.org/abs/2607.18142v1)
*Mei Yuan,Qi Long,Qifeng Wu,Zhenyang Li,Yizhou Zhao,Lei Wang,Yang Liu,Min Xu*

Main category: cs.CV

TL;DR: 提出一种免训练且无需领域知识的智能体框架，通过追踪物体状态演变来实现工业视频异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型（VLM）的方法在具有复杂物体变换和严格物理/程序约束的工业场景中性能下降，且依赖重训练或领域知识注入。

Method: 构建免训练的智能体框架，追踪检测对象的时空动态与底层变换以生成时间状态轨迹，并基于该轨迹推理识别异常。

Result: 在三个工业视频异常检测数据集上，该方法表现优于前沿VLM、智能体框架及微调过的传统VAD方法，并能提供可解释的异常报告。

Conclusion: 模拟人类检查员关注物体状态演变的机制，可在免训练和零领域知识下实现高效且可解释的工业视频异常检测。
...
### [2] [FlowMimic: Mask-free Visual Editing and Generation with Pixel-pair Warped Flow Field for Online Video Editing Data Generation and Modality Mimicry](http://arxiv.org/abs/2607.18227v1)
*Dingyun Zhang,Lixue Gong,Wei Liu*

Main category: cs.CV

TL;DR: 本文提出一种统一图像与视频生成及编辑能力的模型，利用像素对时间扭曲流场从图像编辑数据高效生成视频编辑样本，并通过模态模仿和区域感知损失使模型内化编辑能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑数据收集方法耗时、易引入误差且任务多样性受限，同时现有模型在理解指令和定位编辑区域时依赖外部辅助。本文旨在解决数据生成效率低和模型缺乏内部化编辑能力的问题。

Method: 开发像素对时间扭曲流场实时从图像编辑样本生成视频编辑样本；设计模态模仿生成损失和编辑损失以对齐图像与视频模态的能力与分布；引入感知相关任务及编辑区域感知的潜空间与注意力损失，使模型无需外部辅助即可理解指令并定位区域。

Result: 在多个级别的视频编辑任务中证明，模型仅使用由图像编辑数据生成的样本即可成功学习视频编辑。

Conclusion: 该方法有效统一了图像与视频的生成和编辑能力，实现了高效的数据生成，并使模型内部化解了指令理解和区域定位能力，摆脱了对外部辅助的依赖。
...
### [3] [HOMIE: Human-object Centric Video Personalization via Multimodal Intelligent Enchancement](http://arxiv.org/abs/2607.18217v1)
*Yiyang Cai,Nan Chen,Rongchang Xie,Junwen Pan,Chunyang Jiang,Cheng Chen,Wen Zhou,Zhenbang Sun,Wei Xue,Wenhan Luo,Yike Guo*

Main category: cs.CV

TL;DR: 本文提出HOMIE框架，通过改进的多模态大模型集成策略与模态参考嵌入，统一解决以人为中心的视频个性化任务中主体间与主体内的生成挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以平衡主体保真度与人物-物体交互模式的准确性（特别是针对抽象概念物体），且缺乏机制理解主体内参考输入（如OCR地图、多视角输入）的潜在对应关系。

Method: 提出HOMIE框架，采用改进的MLLM集成策略提取参考级关系知识；在自注意力中引入全局多模态引导对齐MLLM特征与VAE令牌；并提出模态参考嵌入以区分不同令牌并关联主体内参考图像令牌。

Result: 大量实验验证该方法在多种以人为中心的视频个性化任务上达到了当前最优（SOTA）性能。

Conclusion: HOMIE成功以统一方式解决了视频个性化任务中主体间与主体内的双重挑战，实现了高保真度与准确交互模式的兼顾。
...
### [4] [HAS: Highlight-guided Attention Steering for Multimodal LLM Video Summarization](http://arxiv.org/abs/2607.17994v1)
*Rui Chu,Yingjie Lao*

Main category: cs.CV

TL;DR: 本文提出了一种名为HAS的注意力引导方法，通过全局连续的帧级高光分布来指导多模态大语言模型进行视频摘要，以解决离散关键帧选择导致的信息丢失和连贯性降低问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频摘要方法主要依赖离散的关键帧选择，这会降低模型的理解连贯性、丢失重要的视频信息，并且未能充分利用多模态大语言模型（MLLM）的原始处理能力。

Method: 提出HAS（Highlight-guided Attention Steering）方法，包含两个步骤：首先全局寻找连续的帧级高光分布；其次将该高光分布作为MLLM的注意力引导向量，使模型在推理时更关注高光帧，同时通过减少注意力而非完全遗忘来保留非高光帧的信息。

Result: 在多个基准测试上的评估表明，HAS方法在视频摘要任务中表现出令人信服的性能。

Conclusion: 将连续的高光分布作为注意力引导向量可以有效提升MLLM的视频理解与摘要效果，避免了传统离散帧选择带来的信息遗忘与连贯性下降问题。
...
### [5] [ConsiSpace: Learning Geometric Consistency Matters for Video Spatial Reasoning](http://arxiv.org/abs/2607.17599v1)
*Ting Huang,Zhenyu Zhang,Wenyuan Huang,Jian Yang,Hao Tang*

Main category: cs.CV

TL;DR: 本文提出ConSiSpace框架，通过几何一致性感知的记忆机制和自监督强化学习，显著提升多模态大语言模型在视频空间推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型以语义为中心，难以从冗余视频观测中可靠地聚合一致的空间证据，导致长时序、多视角变化条件下的空间推理效率低下且不稳定。

Method: 提出ConSiSpace框架，核心包含两部分：一是构建几何一致性记忆（GCM），结合隐式证据token和显式几何线索以紧凑保存任务相关空间证据；二是在监督微调后采用统一一致性自监督强化学习（UC-SSRL），通过答案一致性、度量一致性和拓扑一致性奖励提升跨视角稳定性。

Result: 在VSI-Bench、OSI-Bench和MMSI-Video-Bench三个空间推理基准上取得一致提升，平均分比最强基线高12.6分。

Conclusion: 将空间一致性作为证据组织原则和显式学习信号，能够有效提升MLLM的视频空间推理能力，为导航感知和长视频问答等应用提供了新方向。
...
### [6] [TimeLens2: Generalist Video Temporal Grounding with Multimodal LLMs](http://arxiv.org/abs/2607.17423v1)
...
TL;DR: TimeLens2提出将时序证据视为区间集合的通用视频时序定位方法，通过构建多跨度监督数据集和新型的强化学习奖励机制，在多个基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视频多模态大语言模型难以准确定位视频事件发生的时间，且现有训练策略与集合值任务不匹配，存在长视频标注脆弱以及强化学习奖励无法有效区分不重叠预测或依赖脆弱片段匹配的问题。

Method: 提出TimeLens2模型，在监督和优化阶段将时序证据视为区间集；构建TimeLens2-93K数据集实现可靠多跨度监督；并设计无需匹配的密集反馈时间Wasserstein奖励（计算一维W1距离）与提供精确重叠反馈的时间IoU相结合的优化方法。

Result: 在7个基准测试中，2B版本超越所有同等规模基线，4B和8B版本达到SOTA，超越高达397B参数的开源模型；各版本相比Qwen3-VL基座分别提升了14.2、13.0和18.1个mIoU点。

Conclusion: 在监督和优化中一致地将时序证据作为区间集合处理处理，能有效解决视频时序定位中的多跨度监督和奖励失配问题，显著提升跨视频长度、领域和查询形式的定位性能。
...
### [7] [SkyVLaM: Multimodal Large Language Model for UAV Video Understanding in Remote Sensing](http://arxiv.org/abs/2607.17386v1)
*Kaiwen Jing,Ruixu Jia,Bingyao Li,Ruizhe Ou,Ming Wu,Chuang Zhang*

Main category: cs.CV

TL;DR: 本文提出SkyVLaM多模态大语言模型，通过稀疏和密集视觉令牌的联合处理提升无人机视频中的语言条件分割性能


<details>
  <summary>Details</summary>
Motivation: 无人机视频中语言条件分割面临小目标、视觉模糊以及动态航空视角等挑战，需要更精细的目标理解方法

Method: 通过时间基础感知器从块级视频表示构建稀疏令牌，正则化稀疏基以鼓励互补时间线索，自适应选择时间相干的密集分割段进行高分辨率检查，由大语言模型联合处理稀疏和密集令牌进行查询条件分割；同时构建了包含101个视频、33.6K帧和153万像素级实例的SkyVid数据集

Result: 实验表明SkyVLaM能够更有效地分配视觉令牌预算，并在无人机场景下的语言条件视频分割任务中取得改进
...
### [8] [Multi-Dimensional Quality Assessment for AI-Generated Human-Centric Videos: Dataset and Model](http://arxiv.org/abs/2607.16742v1)
*Sijing Wu,Yunhao Li,Huiyu Duan,Yucheng Zhu,Xiongkuo Min,Patrick Le Callet,Guangtao Zhai*

Main category: cs.CV

TL;DR: 该论文构建了目前最大的AI生成人类中心视频质量评估数据集HVEval+，并提出了一种基于混合专家机制的多模态大语言模型评估方法MoE-Rater。


<details>
  <summary>Details</summary>
Motivation: AI生成的人类中心视频常存在质量缺陷和语义不匹配问题，缺乏有效的质量评估手段，因此需要构建大规模的综合评估数据集和多功能评估方法。

Method: 构建了包含大量提示词、视频及人类标注的HVEval+数据集；提出MoE-Rater方法，引入混合投影专家和混合LoRA专家，结合三阶段训练策略，在单一模型内实现多维度质量评分、成对比较和类别特定问答。

Result: MoE-Rater在HVEval+和Human-AGVQA数据集上均取得了优异性能，实验分析证明了该数据集和方法的巨大潜力。

Conclusion: HVEval+数据集与MoE-Rater方法能够有效推动AI生成视频的质量评估，并进一步促进文本到视频模型的评估与优化。
...
### [9] [SWE-Pruner Pro: The Coder LLM Already Knows What to Prune](http://arxiv.org/abs/2607.18213v1)
*Yuhang Wang,Yuling Shi,Shaoqiu Zhang,Jialiang Liang,Shilin He,Siyu Ye,Yuting Chen,Kai Cai,Xiaodong Gu*

Main category: cs.CL

TL;DR: 本文提出SWE-Pruner Pro，利用编码智能体自身内部表征直接对工具输出进行逐行剪枝，在保持任务质量的同时节省最多39%的token。


<details>
  <summary>Details</summary>
Motivation: 现有上下文剪枝方法（如SWE-Pruner）依赖外部独立代码分类器，增加了系统复杂度。作者发现智能体在读取工具输出时，其内部表征已隐含代码上下文相关性信息，因此可利用智能体自身能力进行剪枝，避免引入额外分类器。

Method: SWE-Pruner Pro在智能体内部直接进行剪枝：通过一个小型预测头将智能体内部表征转换为每行的保留或剪枝标签，并结合基于工具输出行数的长度感知嵌入来辅助决策。

Result: 在两个开源骨干模型和四个多轮基准测试上，SWE-Pruner Pro节省最多39%的提示与补全token且任务质量不受影响，推理开销有界。在MiMo-V2-Flash上，SWE-Bench Verified解决率提升3.8%，长上下文Oolong准确率提升2.2个百分点。

Conclusion: 利用智能体自身内部表征进行上下文剪枝是一种高效且有效的方法，不仅显著降低token消耗，在部分场景下还能提升任务表现，为编码智能体的上下文管理提供了新方向。
...
TL;DR: 本文提出了一种基于轨迹的在线策略蒸馏（TOPD）框架，通过教师模型在模型自身去噪轨迹上的监督，高效地将推理能力转移到扩散大语言模型中。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的推理导向后训练面临挑战：有监督微调（SFT）需要密集但通常是偏离策略的掩码状态，而强化学习（RL）则依赖稀疏奖励或价值建模。

Method: 提出基于轨迹的在线策略蒸馏（TOPD）框架，从目标模型采样在线扩散轨迹，在部分去噪状态下获取教师模型的token分布，并使用token级别的逆向KL散度（Reverse-KL）目标更新目标模型，从而在保持密集教师监督的同时与模型自身状态对齐。

Result: 在数学推理基准上，TOPD使SDAR-4B-Chat在MATH500上的准确率与RL训练的对应模型相当，静态评估提升+5.7，动态评估提升+4.5；且所需rollout轮数减少4倍，计算速度估计提升96.0倍。

Conclusion: TOPD为扩散大语言模型的后训练提供了一种高效替代方案，无需奖励估计即可在计算成本显著降低的情况下实现与强化学习相当的推理性能。
...
TL;DR: 本文提出CoCommit方法，通过标记门控的协调传递减少扩散大语言模型中的因子分解误差，提升多token解码精度。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在每个去噪步骤中独立解码多个token，当这些位置存在依赖关系时会产生因子分解误差，而基于置信度的选择方法无法从边际分布中观察到该误差，因此需要一种能够协调多位置联合解码的机制。

Method: 提出CoCommit，一种标记门控的协调传递机制：在常规bundle选择后，由学习到的marker标记commit集合，重新应用骨干网络最后n层使标记位置协调（近似联合模式解码），然后再进行贪心argmax写入token。该方法复用已有权重，仅需一次额外部分前向传递，无需辅助模型。

Result: 在LLaDA2.1-mini上使用LoRA适配器并匹配贪心推理，联合提交在全部六个评测基准上均提升了准确率，其中推理类和精确答案类任务提升最大。

Conclusion: CoCommit通过短暂延迟提交并协调多位置解码，有效缓解了扩散语言模型中多token承诺的因子分解误差，在无需额外模型的情况下实现了显著的准确率提升，尤其在推理和精确答案任务上优势突出。
...
TL;DR: 本文提出dOPSD方法，通过从模型自身去噪轨迹中导出特权信息来实现扩散语言模型的高效自蒸馏后训练。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型(dLLM)在推理任务的后训练中面临困难：监督微调存在离策略和曝光偏差问题，强化学习奖励稀疏且缺乏可处理的序列似然，而现有自蒸馏方法依赖推理时不可用的外部特权信息，导致改进有限。

Method: dOPSD方法不依赖外部标签，而是直接从学生模型自身的去噪轨迹中提取教师特权信息，利用同一轨迹中较后、更充分解码的步骤来评估被掩码位置，使教师的优势信号源自模型自身的解码过程。

Result: 在Dream和LLaDA两个模型上，dOPSD在域内数学推理和域外代码生成任务上均取得改进，超越监督基线和在策略基线方法。

Conclusion: 从模型自身解码轨迹中派生特权信息的自蒸馏范式有效解决了dLLM推理后训练的瓶颈，为无需外部参考的密集token级在策略监督提供了新方向。
...
TL;DR: 本文提出了DominoTree，一种无需训练的草稿树构建方法，通过结合Domino的路径依赖条件修正和GPU原生CUDA图构建器，显著提升了LLM推测解码的吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的最优优先树方法（如DDTree）基于因子化公式，无法表示带有GRU因果修正的Domino草稿器所产生的路径依赖（非因子化）分布特征，限制了推测解码性能的进一步优化。

Method: 提出DominoTree，一种无需训练的最优优先草稿树，通过沿每条根到节点的路径利用Domino的条件非因子化修正进行评分（限定在候选top-M集合内），并引入GPU原生的CUDA-graph构建器以低成本且位级一致地构建树。

Result: 在Qwen3-4B上实现了最高6.6倍于自回归解码的加速，并在所有测试温度下取得了最高平均接受长度（高达10.7 tokens/轮）；相比基础Domino解码器吞吐量提升9%至10%（Alpaca上最高22%），且在所有温度下均优于DDTree和CaDDTree。

Conclusion: 利用非因子化的路径依赖修正评分结合高效GPU原生树构建器，能在不改变接受率的前提下有效提升推测解码的吞吐量，证明了该训练无关方法在多种温度和模型规模下的有效性。
...
TL;DR: 提出CaDDTree方法，通过联合选择树结构和节点预算来直接优化令牌吞吐量，解决了推测解码中节点预算选择缺乏原则性依据的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如DDTree）最大化期望接受长度，但这会导致其总是偏好更大的树而忽略验证成本，从而缺乏选择节点预算的原则性基础。

Method: 提出CaDDTree，通过显式建模草稿和验证延迟来直接优化令牌吞吐量。该方法将目标分解为每轮对预算的一维搜索，并在凸验证成本下证明吞吐量函数是单峰的，从而利用贪心停止规则实现无需离线搜索的动态预算自适应。

Result: 在Qwen3-4B和Qwen3-8B的八个基准测试（涵盖推理、编码和指令遵循任务）中，CaDDTree在几乎所有任务上均匹配或超越了使用Oracle预算选择的DDTree。

Conclusion: CaDDTree提供了一种无需离线调优的成本感知预算选择方案，能动态适应当前分布与验证成本，有效提升了目标语言模型的推理吞吐量。
...
TL;DR: DDTree通过从块扩散模型的分布中构建草稿树并进行高效验证，解决了单轨迹推测解码的局限性，提升了整体性能。


<details>
  <summary>Details</summary>
Motivation: 解决DFlash等现有方法在推测解码中每轮仅验证单一草拟轨迹，导致接受长度受限的问题。

Method: 提出DDTree方法，从块扩散起草器的逐位置分布构建草稿树；在固定节点预算下利用最佳优先堆算法选择最优续写，并通过祖先注意力掩码在单次前向传播中完成验证。

Result: DDTree基于DFlash实现了性能提升，跻身最先进的推测解码方法之列。

Conclusion: 将单轨迹验证扩展为草稿树验证能有效提升推测解码的接受长度与性能，DDTree为该领域提供了一个先进的解决方案。
...
### [16] [1-out-of-5 Maximin-Share Allocations Always Exist for Four Agents](http://arxiv.org/abs/2607.18139v1)
*Christoph Schwerdtfeger*

Main category: econ.TH

TL;DR: 本文证明了对于四个具有非负可加估价的智能体，完整的1-out-of-5最大最小份额（MMS）分配总是存在，从而完全刻画了四智能体情况。


<details>
  <summary>Details</summary>
Motivation: 旨在解决四个智能体在非负可加估价下的最大最小份额（MMS）分配存在性问题，改进先前的1-out-of-6保证，并对四智能体情况进行完全刻画。

Method: 提出“平衡剩余划分引理”（移除包含四个最高排名商品之一的被拒包后，剩余部分仍能容纳所需数量的单位价值平衡包），并在Lean 4中对定理进行了机器验证。

Result: 证明了1-out-of-5 MMS分配总是存在，改进了已有的1-out-of-6保证，结合已知反例表明该保证在d≥5时精确成立。

Conclusion: 四智能体的非负可加估价MMS分配问题已得到完全刻画，其精确保证界限为1-out-of-5，且该结论具有经过形式化验证的可靠性。

Abstract: For four agents with nonnegative additive valuations, a complete 1-out-of-5 maximin-share allocation always exists, improving the previous 1-out-of-6 guarantee. Together with known exact-MMS counterexamples, this completely characterizes the four-agent case: the guarantee holds exactly for $d\geq5$. The main technical contribution is a balanced-residual partition lemma: removing rejected bundles with one of the four highest-ranked goods apiece leaves a remainder that still admits the required number of unit-valued balanced bundles. In its central $2+2$ case, three unit bundles repair two pairs of colliding high-valued goods. The theorem is machine-checked in Lean 4.
...
### [17] [jina-reranker-v3.5: An Efficient Listwise Reranker with Hybrid Attention and Self-Distillation](http://arxiv.org/abs/2607.18152v1)
*Christina Nasika,Feng Wang,Antonis Krasakis,Han Xiao*

Main category: cs.IR

TL;DR: 本文提出了0.6B参数的列表式重排序器jina-reranker-v3.5，通过混合注意力调度和多领域自蒸馏训练，在保持跨文档比较能力的同时实现了高效且鲁棒的半结构化数据检索。


<details>
  <summary>Details</summary>
Motivation: 解决代理检索流水线中列表式重排序器在生产部署时难以同时兼顾效率、领域鲁棒性和半结构化数据处理能力的问题。

Method: 保留LBNL交互机制，将全局注意力替换为混合调度（3层滑动窗口层+2层全局层）；在多领域（法律、医疗、金融等）混合数据上训练；采用三阶段自蒸馏策略将全注意力教师模型的质量迁移至稀疏注意力学生模型。

Result: 在BEIR上达到63.20 nDCG@10，以少7倍的参数量媲美4B模型；在MIRACL和RTEB上优于前代模型；半结构化检索nDCG@10提升9.6个点；推理延迟最高降低1.56倍。

Conclusion: 该模型成功平衡了重排序任务的效率、多领域鲁棒性和半结构化数据处理性能，模型权重已在Hugging Face以非商业许可发布。
...
TL;DR: 提出了一种名为FlowBlock的无训练并行解码框架，通过门控波前解码和异构波前打包机制，解决了块状扩散大语言模型的块间串行解码问题，显著提升了推理速度和准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的块状扩散大语言模型在块间解码时存在严格的串行依赖，而以往的基于训练的并行方法加速有限且会损害模型准确率。

Method: 提出无训练并行解码框架FlowBlock，包含两个核心机制：一是门控波前解码，通过就绪门控控制块准入，利用Token到Token编辑联合优化活跃块，并在窗口化块因果掩码下顺序提交；二是异构波前打包，为每个请求分配独立波前，并将异步窗口打包为密集且形状稳定的批量前向计算。

Result: 与LLaDA-2.1和LLaDA-2.0等串行模型相比，每秒Token数（TPS）分别提升高达2.95倍和4.01倍，延迟分别降低53.6%和77.1%，平均准确率提升1.3个百分点；与基于训练的D2F基线相比，准确率更高且批量服务吞吐量提升达16倍。

Conclusion: FlowBlock成功将块最终性从硬依赖转化为调度资源，在不额外训练的前提下实现了高效的块间并行解码，兼顾了高吞吐量与准确率。
...
### [19] [LaCache: Exact Caching and Precision-Adaptive Inference for Diffusion Large Language Models](http://arxiv.org/abs/2607.16339v1)
*Xingru Chen,Zelang Liang,Yongjia Ma,Jiqing Zhan,Shuling Yang,Lian Wen,Kun Zhan*

Main category: cs.AI

TL;DR: 本文提出了一种名为LaCache的无训练加速框架，通过无损缓存和混合精度量化有效解决基于扩散的大型语言模型在去噪过程中的算子级冗余问题。


<details>
  <summary>Details</summary>
Motivation: 当前DLLM在半自回归解码的去噪步骤中会重新计算整个序列，忽略了不变的前缀和掩码后缀，导致严重的算子级冗余和内存带宽瓶颈。

Method: 提出无损状态记忆（LSM）缓存嵌入输出、预注意力状态及在线softmax统计信息三类中间结果以跳过冗余计算；同时针对FFN层引入逐组FP8量化策略以缓解内存带宽瓶颈。

Result: 单独使用LaCache可实现约1.3倍的端到端加速；与现有加速方法结合时，在保持相当任务准确率的前提下，端到端加速比最高可达40.2倍。

Conclusion: LaCache作为一种无损、无需训练的加速框架，能显著提升DLLM的推理速度，且与现有加速技术具有良好的兼容性和叠加效果。
...
TL;DR: 本文提出TAPS方法，通过将扩散边际概率转化为路径条件的接受估计并选择紧凑的前缀闭子树，优化了推测解码中的草稿树验证，实现了显著的无损加速。


<details>
  <summary>Details</summary>
Motivation: 使用扩散模型进行并行起草虽降低了起草延迟，但将瓶颈转移到了验证阶段。现有方法基于边际概率对节点排序，忽略了验证的前缀条件性，导致验证被拒绝前缀的不可达后代，增加了延迟且接受率提升有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边际概率转化为路径条件的接受估计，并在固定的验证预算下选择紧凑的前缀闭子树，以优化接受与成本之间的权衡。

Result: 在多个数据集和模型族上的实验表明，TAPS相比普通自回归解码实现了高达7.9倍的无损端到端加速，分别比最先进的DFlash和DDTree快1.36倍和1.74倍。

Conclusion: TAPS有效解决了基于扩散的推测解码中草稿树验证的不匹配问题，提供了更优的接受-成本权衡，并实现了最先进的加速效果。
...
### [21] [OrientSAM: Mitigating Camera-Centric Shortcut in Multimodal Spatial Reasoning via Orientation-Aware Spatial Alignment](http://arxiv.org/abs/2607.17657v1)
*Wenxiao Fan,Hang Yin,Kan Li*

Main category: cs.AI

TL;DR: 本文提出OrientSAM框架，通过引入显式方向信息来解决多模态大语言模型在空间推理中过度依赖相机视角的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在需要视角转换的空间推理中，常依赖以相机为中心的线索而非参考对象的视角，导致在非相机参考设置下出现系统性错误。

Method: 提出一种方向感知的空间对齐框架OrientSAM，通过方向感知标记和基于傅里叶的角度编码将显式方向信息注入多模态表示，并采用课程学习策略逐步提升视角感知推理能力；同时构建了空间数据生成流水线以提供方向感知的空间监督。

Result: 在Spatial-MM、ViewSpatial和3DSRBench数据集上的实验表明，OrientSAM在非相机视角、以人为中心及方向敏感的任务上持续优于强基线模型。

Conclusion: 显式的方向建模对于缓解以相机为中心的捷径行为、实现更稳健的非自我中心空间推理具有重要意义。
...
### [22] [TRIM: Reducing AI-Generated CodeSlop via Agent Trajectory Minimization](http://arxiv.org/abs/2607.18161v1)
*Alex Mathai,Shobini Iyer,Aleksandr Nogikh,Petros Maniatis,Franjo Ivancic,Junfeng Yang,Baishakhi Ray*

Main category: cs.SE

TL;DR: 本文指出AI编码智能体生成的代码存在冗余问题，并提出TRIM算法通过最小化智能体轨迹来有效减少该现象。


<details>
  <summary>Details</summary>
Motivation: 智能体在迭代过程中会残留无用的推测性编辑和临时更改，导致生成的代码冗长且难以维护，并随着代码库规模扩大而加剧。

Method: 正式定义了AI生成代码中的冗余现象“CodeSlop”，并提出TRIM算法，通过最小化智能体轨迹来间接减少代码冗余。

Result: TRIM将CodeSlop减少了17.9%至32.9%，性能几乎无损，且验证成本约为Delta Debugging等基线算法的一半。

Conclusion: 通过最小化智能体轨迹间接减少代码冗余是高效且有效的方法，在保持性能的同时显著提升了代码可维护性。
...
### [23] [DELTA-TTS: Adapting Autoregressive Model into Diffusion Language Model for Text-to-Speech](http://arxiv.org/abs/2607.04140v1)
*Junwon Moon,Seungbeom Kim,Yejin Lee,Hoseong Ahn,Sewoong Park,Heeseung Kim,Kyuhong Shim*

Main category: eess.AS

TL;DR: 本文提出DELTA-TTS框架，通过LoRA将预训练自回归TTS模型转换为离散扩散语言模型，实现置信度有序的语音token解码，提升推理速度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自回归TTS模型按从左到右顺序逐步生成离散语音token，导致推理速度慢且容易传播局部错误和幻觉；然而TTS任务中完整文本输入在合成前已知，并不存在固有的从左到右生成约束，因此本文旨在突破该约束以解决速度与鲁棒性问题。

Method: DELTA-TTS基于轻量LoRA适配，将预训练AR TTS模型转换为离散扩散语言模型（dLLM）进行置信度有序解码；引入卷积模块注入局部声学上下文，配合1/t加权训练目标和时间偏移推理调度，将低置信度位置推迟到后续步骤生成。

Result: 仅使用585小时LibriTTS数据训练，DELTA-TTS在Seed-TTS test-en上达到1.75%的WER，优于其AR骨干模型，且token生成速度提升3.3倍；进一步分析表明其产生更清晰的文本-语音对齐、更高的整体解码置信度，并缓解了AR生成中的幻觉问题。

Conclusion: 通过将AR TTS模型转换为离散扩散语言模型并采用置信度有序解码，DELTA-TTS在不牺牲质量的前提下同时提升了推理效率和鲁棒性，验证了非自回归生成范式在TTS任务中的可行性和优越性。
...
### [24] [Per Astronomix ad Astra: High-Order Differentiable (Magneto)hydrodynamics with Energy-Conserving Self-Gravity](http://arxiv.org/abs/2607.18176v1)
*Leonard Storcks,Nils Thuerey,Tobias Buck*

Main category: astro-ph.IM

TL;DR: 本文提出了一个用Python/JAX编写的高性能可微（磁）流体动力学模拟器astronomix，支持数百万参数的逆建模、机器学习训练，并具备高阶精度和多GPU扩展能力。


<details>
  <summary>Details</summary>
Motivation: 传统的天体物理流体模拟器缺乏可微性，难以进行高效的逆建模、敏感性分析以及与机器学习模型的集成，同时还需要兼顾高阶计算精度和GPU加速性能。

Method: 基于Python/JAX开发了astronomix模拟器，利用自动微分技术，实现了五阶有限差分约束传输磁流体动力学方案和新型四阶自重力方案；并利用代理技能生成和验证自定义Pallas GPU内核以优化性能。

Result: 模拟器的可微性通过了解析导数和有限差分的验证；在单GPU上运行时间与AthenaPK相当，但在平滑问题上误差更低；在8个GPU上实现约6.5倍强扩展加速，在4个节点的16个GPU上达到约76%的弱扩展效率；成功支持了数百万参数的逆建模和模拟器内的机器学习模型训练。

Conclusion: astronomix成功将可微性、高阶精度和多GPU扩展能力结合，为天体物理模拟中的逆建模、稳定性分析及机器学习应用提供了强大的开源工具。
...
### [25] [Nonexistence of Simultaneously EF1 and Pareto Optimal Allocations for Submodular Valuations](http://arxiv.org/abs/2607.18220v1)
*Harish Chandramouleeswaran,Prajakta Nimbhorkar*

Main category: cs.GT

TL;DR: 本文通过构造反例，证明了在单调次模估值下不存在同时满足EF1和PO的不可分割物品分配，并将非存在性边界推进至无权覆盖估值及杂务分配问题。


<details>
  <summary>Details</summary>
Motivation: 解决在单调次模估值下，不可分割物品分配是否同时满足公平性（EF1）和帕累托最优（PO）的长期开放问题。

Method: 构造了一个包含两名代理人的反例，使用属于单调次模估值严格子类的无权覆盖估值，并证明了判定该分配存在性的问题是NP-hard的。

Result: 证明了在单调次模估值下不存在同时满足EF1和PO的分配，且判定其存在性是NP-hard的；此外，将该估值视为负效用时，同样证明了杂务分配中EF1+PO的不存在性。

Conclusion: 将EF1+PO分配的非存在性边界从单调次可加估值推进到了无权覆盖估值（单调次模估值的严格子类），并扩展至杂务分配场景。
...
### [26] [Text2Villa: Hierarchical Generation of 3D Indoor Environments with Physics-Aware Analysis-by-Synthesis](http://arxiv.org/abs/2607.17145v1)
*Xiang Tang,Ruotong Li,Xiaopeng Fan*

Main category: cs.GR

TL;DR: 本文提出了一种分层生成框架Text2Villa，通过自回归布局生成器和基于可供性的物理-语义场景图，从文本生成物理合理且高保真的多层3D别墅环境。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到3D场景生成方法无法生成具有垂直连通性和任意多边形边界的复杂多层结构，且缺乏对连续3D物理定律的深刻理解，导致严重的几何穿透和悬浮伪影。

Method: 提出一种分层框架：宏观上微调自回归布局生成器以将文本直接解析为具有多边形边界和多层连通性的3D建筑基础；微观上引入可供性驱动的物理-语义场景图（A-PSSG），将场景实例化构建为闭环优化问题，结合几何碰撞检测引擎和多模态大语言模型（MLLM）的语义推理来动态解决网格碰撞等物理缺陷。

Result: 广泛的实验表明，Text2Villa在各项指标上均优于现有方法，能够稳健地从文本生成高保真且物理上合理的别墅级3D环境。

Conclusion: 该方法有效解决了从自然语言生成复杂且具有物理基础的3D室内场景的挑战，为下游应用提供了可靠且交互式的3D内容基础。
...
### [27] [FlashRT: Agent Harness for Guiding Agents to Deploy Real-Time Multimodal Applications](http://arxiv.org/abs/2607.18171v1)
*Krish Agarwal,Zhuoming Chen,Yanyuan Qin,Zhenyu Gu,Atri Rudra,Beidi Chen*

Main category: cs.LG

TL;DR: FlashRT是一个通过引导编程代理将简单参考实现转化为优化多GPU部署的智能体框架，旨在实现实时多模态应用的高效部署。


<details>
  <summary>Details</summary>
Motivation: 现有的服务系统和自动并行编译器转换能力有限且依赖固定假设，导致实时多模态应用在多GPU上的高效部署需要人工精心编写实现。

Method: 采用链式程序范式，指导通用编程代理通过多遍转换过程，将参考实现转化为中间表示以捕获数据依赖和状态范围，验证并执行静态分析，最后在测量门控优化循环中迭代实现、验证和基准测试候选转换。

Result: 在NVIDIA B200 GPU上实现高达约70倍的延迟降低和2.8倍的吞吐量提升；在AMD MI355X GPU上匹配该延迟降低并实现3.6倍的吞吐量提升，且在Qwen3-Omni上比专家优化的vLLM-Omni减少65%的响应延迟。

Conclusion: 智能体驱动的优化在优化生态较不成熟的平台上具有更好的可扩展性，能够有效生成跨越不同硬件预算的高效部署方案。
...
### [28] [A Continuous-Time Reinforcement Learning Framework for Fine-Tuning Discrete Diffusion Models](http://arxiv.org/abs/2607.14522v1)
*Zikun Zhang,Jiayuan Sheng,David D. Yao,Wenpin Tang*

Main category: cs.LG

TL;DR: 本文通过随机控制方法将强化学习推广至连续时间离散状态空间，提出了连续时间PPO和GRPO变体，并应用于离散扩散模型及扩散大语言模型的微调。


<details>
  <summary>Details</summary>
Motivation: 现有基于GRPO的方法仅依赖终端奖励，且在扩散大语言模型中轨迹似然计算成本过高。本文旨在构建一个连续时间RL框架，允许在去噪轨迹的中间阶段引入奖励信号，并高效估计概率比。

Method: 将状态动态建模为受控连续时间马尔可夫链（CTMC），推导出连续时间的策略梯度方法（连续时间PPO和GRPO）；针对掩码扩散模型（MDM）提出具有解析可处理概率比的多类策略参数化；针对扩散大语言模型提出轨迹子采样技术以降低逐位置概率比的计算成本。

Result: 方法在低维熵正则化优化问题上验证有效，并在扩散大语言模型的数学推理和编程任务的RL后训练中展示了有效性。

Conclusion: 本文提供了一个统一的连续时间RL框架，支持无需奖励可微性的奖励驱动优化，并将中间奖励信号融入去噪轨迹，为MDM中的探索与策略优化提供了统一视角。
...
### [29] [Accelerating Masked Diffusion Large Language Models: A Survey of Efficient Inference Techniques](http://arxiv.org/abs/2607.12829v1)
*Daehoon Gwak,Minhyung Lee,Junwoo Park,Jaegul Choo*

Main category: cs.LG

TL;DR: 本文提出了一个针对扩散大语言模型的统一延迟分解框架，用于剖析算法、架构和系统因素对推理速度的影响，并系统梳理了加速技术。


<details>
  <summary>Details</summary>
Motivation: dLLMs理论上具有并行生成优势，但实际加速需要专用推理机制；现有基准测试将算法、架构和系统层面的因素混为一谈，难以进行严格比较，因此需要一种框架来解耦这些因素。

Method: 提出统一延迟分解框架，将端到端延迟分解为算法、架构与系统、推理时扩展三个维度，并沿这三个轴对加速技术进行分类与分析。

Result: 通过该框架系统分析了各因素对真实部署中推理速度的影响，提供了可复现基准测试的指导原则，并总结了并行生成尚未解决的开放挑战。

Conclusion: dLLMs的实际加速需要在算法、架构和系统层面协同优化，未来研究应关注可复现的基准测试方法论以及实现并行生成全部潜力的开放问题。
...
### [30] [BlockServe: Block-Grained Continuous Batching for High-Throughput Diffusion LLM Serving](http://arxiv.org/abs/2607.08930v1)
*Yuanjie Zhu,Liangwei Yang,Ke Xu,Weizhi Zhang,Shanghao Li,Zihe Song,Philip S. Yu*

Main category: cs.LG

TL;DR: 提出BlockServe框架，通过块级调度和混合状态执行解决扩散大语言模型服务中的收敛异构性问题，显著提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的高效服务受限于收敛异构性：批量处理请求时不同序列收敛速度不同，导致较快请求被较慢请求阻塞，产生计算气泡和尾部延迟。

Method: 提出BlockServe连续批处理框架，整合块级调度（在块边界立即驱逐已完成请求）和混合状态执行（通过gather-scatter索引将双缓存和并行解码扩展至异构批次），并引入基于令牌预算补充的计算感知准入控制器以扩大有效批量容量。

Result: 在五个基准测试的Dream和LLaDA模型上，BlockServe的吞吐量比Fast-dLLM高出1.9至10.6倍，且生成质量相当。

Conclusion: 块级调度可作为高吞吐量离线扩散大语言模型推理的基础。
...
### [31] [LLMs and Agentic AI Systems for Smart Grids: A Tutorial on Architectures and Applications](http://arxiv.org/abs/2607.18147v1)
...
TL;DR: 本文提出了一种基于求解器接地（solver-grounded）的设计原则，用于在智能电网中构建和评估LLM智能体AI系统，确保数值结果仅来源于可信工具并通过显式验证。


<details>
  <summary>Details</summary>
Motivation: 当前文献缺乏统一的方法来设计和评估智能电网中的LLM智能体系统。LLM可能产生数值上看似合理但物理上不可行的输出，评估协议因任务而异，且模型应计算与不应计算的边界是隐含的。

Method: 提出求解器接地设计原则：数值结果仅在源自可信工具并通过显式验证时才报告。综述了电力系统LLM和智能体AI系统的基础构建模块（提示策略与智能体架构），并在四个案例研究（风电预测、EV充电调度、潮流分析、故障诊断）中实例化该原则，同时提出涵盖任务效用、求解器接地正确性、忠实性与安全失败、成本与延迟的四组评估框架。

Result: EVAgent复现了CVXPY最优解，将仅LLM基线的未满足能量降低7.5-9.5倍；GridDebugAgent修复了17/39个故障案例，总违规减少52.3%。实验显示一致的分工模式：智能体系统负责编排、检索和解释，可信工具负责计算，验证门控决定输出内容。

Conclusion: 基于求解器接地的设计原则为电力系统LLM智能体AI系统提供了统一的设计与评估方法，明确了LLM与可信工具之间的分工边界，为未来在该领域的系统化研究提供了框架指导。
...
### [32] [HyMCache: A KV Cache Framework for Multi-Turn LLM Serving with CXL-Hybrid Memory](http://arxiv.org/abs/2607.18141v1)
*Hakbeom Jang,Inho Song,Sam H. Noh,Jongryool Kim*

Main category: cs.DC

TL;DR: 本文提出HyMCache，一种基于CXL混合内存的KV缓存框架，通过结合少量DRAM与大容量SSD，以SSD级成本实现DRAM级KV缓存重用效率。


<details>
  <summary>Details</summary>
Motivation: 长上下文、多轮和代理式LLM工作负载日益依赖KV缓存重用，但GPU HBM和主机DRAM成本过高，无法扩展至TB级共享上下文容量，导致远程存储层成为瓶颈。

Method: HyMCache整合CXL混合内存(CXL-HM)，利用多轮KV缓存访问的读主导、可预测和仅追加特性，采用请求级前缀预取和机会性写入缓冲，将延迟敏感的读取暂存于设备DRAM中。

Result: 在相同DRAM预算下，单节点服务性能比本地LMCache高3.0倍，PD分离服务高1.45倍；与1TB分布式DRAM Mooncake相比性能仅低约30%，但DRAM使用量减少16倍。

Conclusion: HyMCache证明CXL-HM可有效支持TB级SSD后端KV缓存重用，为大规模LLM服务提供了成本效益更高的内存层解决方案。
...
TL;DR: 本文提出了Sangam，一个用于缓存扩散语言模型（dLLM）推理的服务系统，通过赤字令牌预算调度器和混合服务策略解决了近似缓存下的调度与资源分配问题。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型的双向注意力机制阻碍了精确的KV缓存，近似缓存技术引发的重复预填充/解码结构导致现有自回归服务机制无法直接适用，面临块级解码和重复预填充的调度挑战。

Method: Sangam引入赤字令牌预算调度器以实现无摊销停滞调度，优先处理解码并在预算允许时处理完整预填充；同时采用混合服务策略，将溢出的预填充转移到解码工作节点上并防止干扰。

Result: dLLM服务设计受预填充-解码干扰和资源分配影响；在同置服务下，解码密集型负载的平均延迟降低9-20%；在混合执行下，预填充密集型负载的平均延迟降低8-20%。

Conclusion: 扩散语言模型的服务设计同样取决于预填充与解码的干扰及资源分配，同置服务最适合解码密集型负载，而混合执行最适合预填充密集型负载。
...
### [34] [EdgeCoInfer: Hierarchical Collaborative Inference for On-Device Multimodal Large Models](http://arxiv.org/abs/2607.17143v1)
*Lin Tan,David K. Y. Yau,Songtao Guo*

Main category: cs.DC

TL;DR: 本文提出了EdgeCoInfer框架，通过模型间模块共享和模型内细粒度分区解决边缘环境中多模态大语言模型的并发部署问题。


<details>
  <summary>Details</summary>
Motivation: 在边缘环境中并发执行多模态大语言模型面临多任务并发和严格耦合的硬约束挑战，难以满足无处不在的智能应用需求。

Method: 提出EdgeCoInfer框架，通过混合演化分层强化学习（HE-HRL）范式解决混合整数非线性规划（MINLP）问题，结合遗传算法进行离散模型放置和Soft Actor-Critic进行连续资源分配，并引入可行性引导的构造性执行机制。

Result: 实验表明，该框架在高并发场景下实现100%任务完成率，系统成本降低76%，内存节省71.88%。

Conclusion: EdgeCoInfer有效克服了边缘设备上并发执行MLLMs的资源和约束限制，在保证任务完成率的同时大幅降低了系统成本和内存消耗。
