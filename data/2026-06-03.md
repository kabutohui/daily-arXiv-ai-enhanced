...
### [1] [Fast-dLLM++: Fréchet Profile Decoding for Faster Diffusion LLM Inference](http://arxiv.org/abs/2606.02955v1)
*Siva Rajesh Kasa,Yasong Dai,Sumit Negi,Hongdong Li*

Main category: cs.CL

TL;DR: 本文提出Fast-dLLM++，通过引入Fréchet概况解码利用异质置信度分布，在保持准确率的同时显著提升了扩散大语言模型的推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散大语言模型解码方法（如Fast-dLLM）基于同质置信度假设，将并行提交限制等同于最弱的选定token，忽略了真实解码步骤中异质置信度分布带来的并行潜力，限制了推理速度。

Method: 提出Fast-dLLM++，一种免训练的扩展方法，引入Fréchet概况解码。该方法根据完整的排序置信度概况选择并行提交集，是对原有规则在异质置信度下的推广，并在置信度不均时提供可证明的异质性加成，无需改变模型或缓存实现。

Result: 在LLaDA-8B模型上的实验表明，该方法有效提升了准确率与吞吐量的权衡边界，在准确率相当的情况下实现了高达37%的吞吐量提升。

Conclusion: Fast-dLLM++证明了利用异质置信度概况可以安全地挖掘被忽视的并行性，且作为现有方法的直接替代方案，能显著提升扩散大语言模型的推理效率。
...
TL;DR: 该论文提出了SimSD算法，通过即插即用的掩码策略为扩散大语言模型实现了投机解码，在保证生成质量的同时显著提升了推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（dLLMs）虽然支持并行解码，但其双向注意力机制导致上下文在去噪步骤中变化，无法像自回归模型那样利用因果掩码进行高效的标记级投机解码验证。

Method: 提出一种免训练的投机解码算法SimSD，引入草稿模型预测的参考标记，并设计特定的注意力掩码来调节其与当前步骤标记的交互，使dLLMs能在单次前向传递中计算草稿标记的有效logits以完成验证。

Result: 在四个基准测试上的实验表明，该方法在保持甚至提高生成质量的前提下，实现了最高7.46倍的解码吞吐量提升。

Conclusion: SimSD成功解决了dLLMs与投机解码不兼容的问题，恢复了类似AR模型的验证能力，且可灵活集成KV缓存等其他加速技术。
...
### [3] [Decoding in Order-Agnostic Language Models: Chain-Rule Deviation and Uniform Spreading](http://arxiv.org/abs/2606.00997v1)
*Lin Yao*

Main category: cs.CL

TL;DR: 本文揭示了序不可知语言模型（OALM）中似然度对揭示顺序的依赖性，并提出利用置信度方差作为评估解码路径质量的新诊断指标。


<details>
  <summary>Details</summary>
Motivation: 旨在解决OALM中学习的条件概率并非联合分布的精确分解、导致似然度指标混合了内容难度与路径依赖伪影的问题，填补评估解码路径有效性的空白。

Method: 提出基于置信度轨迹形状的诊断方法，利用均匀扩散定理证明均匀分布的置信度能最大化目标可恢复性，并引入对数置信度方差作为衡量解码路径优劣的指标。

Result: 改变揭示顺序会导致目标似然度发生显著变化（高达0.49 nats/token）；置信度优先解码在内容词上近似从左到右顺序；低置信度方差能有效区分结构化路径与随机排序，并与下游任务正确性高度相关。

Conclusion: 研究建议在比较OALM解码路径时，应联合报告平均置信度和置信度方差，以提供比单纯似然度更准确的评估。
...
### [4] [dMoE: dLLMs with Learnable Block Experts](http://arxiv.org/abs/2605.30876v1)
*Sicheng Feng,Zigeng Chen,Gongfan Fang,Xinyin Ma,Xinchao Wang*

Main category: cs.CL

TL;DR: 本文提出了dMoE框架，通过块级专家路由策略解决了扩散大语言模型与混合专家架构结合时的内存瓶颈问题，在保持性能的同时显著提升了推理效率。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的块级并行解码机制与传统混合专家架构的Token级独立路由存在根本性不匹配，导致唯一激活专家数量激增，造成推理过程中的内存受限瓶颈。

Method: 提出dMoE框架，核心方法是将每个块内的Token级专家分布聚合为统一的块级专家分布，以此指导专家路由，从而大幅减少唯一激活专家的数量并缓解内存瓶颈。

Result: 实验表明，dMoE将唯一激活专家数量从平均69.5降至14.6，保留了99.11%的原始性能，内存使用降低76.64%至79.84%，并实现了1.14倍至1.66倍的端到端延迟加速。

Conclusion: dMoE有效解决了dLLM与MoE结合时的内存瓶颈问题，证明了在不牺牲性能的情况下实现高效推理的可行性。
...
### [5] [Efficient Diffusion LLMs via Temporal-Spatial Parallel Decoding and Confidence Extrapolation](http://arxiv.org/abs/2605.30753v1)
*Zekai Li,Ji Liu,Yiqing Huang,Ziqiong Liu,Dong Li,Emad Barsoum*

Main category: cs.CL

TL;DR: 该论文提出了一种针对基于扩散的大型语言模型的轨迹感知解码框架，通过利用轨迹特征和预测未来趋势动态固定已收敛的Token，从而减少推理延迟。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的大型语言模型推理延迟较高，主要由于大量步骤被用于冗余细化和对已确定Token的重复重掩码；现有加速方法依赖局部置信度启发式或固定调度，对提示和任务变化敏感且忽略了位置效应。

Method: 提出一种轨迹感知解码框架，包含两个组件：一是时空并行解码（TSPD），利用轻量级控制器根据逐Token轨迹特征（置信度、熵、动量）和位置决定Token收敛；二是置信度外推（CE），一种免训练的状态空间模块，用于预测未来Logit趋势以支持主动决策。

Result: TSPD和CE减少了不必要的去噪迭代次数，同时保持了输出质量，且能与KV缓存等系统优化方法无缝结合。

Conclusion: 将扩散解码视为动态控制问题并利用Token级轨迹为可靠控制提供了关键信号，实现了在不牺牲输出质量前提下的高效推理加速。
...
### [6] [NaRA: Noise-Aware LoRA for Parameter-Efficient Fine-Tuning of Diffusion LLMs](http://arxiv.org/abs/2605.29716v1)
*Shuaidi Wang,Zhan Zhuang,Ruping Huang,Yu Zhang*

Main category: cs.AI

TL;DR: 该论文提出了NaRA，一种针对扩散大语言模型（dLLMs）的噪声感知参数高效微调方法，通过动态调整参数以适应去噪轨迹中的噪声水平变化，解决了现有静态PEFT方法的次优问题。


<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调（PEFT）方法（如LoRA）原本针对自回归模型设计，其静态参数忽略了扩散过程中噪声水平、输入分布和生成难度的动态变化，导致在dLLMs上性能欠佳。

Method: 提出噪声感知低秩自适应，利用一个以噪声水平为条件的轻量级全局共享超网络生成低秩核心矩阵，使更新矩阵能够沿扩散过程连续变化，且几乎不增加参数量和延迟。

Result: 论文提供了理论论证，并在常识推理、数学推理和代码生成基准测试中实证表明，NaRA相比噪声无关的基线方法取得了一致的性能提升。

Conclusion: NaRA成功将噪声感知机制引入PEFT，有效捕捉了扩散模型的内在动态特性，为扩散大语言模型提供了一种更优的参数高效微调范式。
...
### [7] [Diffusion Large Language Models for Visual Speech Recognition](http://arxiv.org/abs/2605.28456v1)
*Jeong Hun Yeo,Chae Won Kim,Hyeongseop Rha,Yong Man Ro*

Main category: cs.AI

TL;DR: 本文提出了首个基于扩散大语言模型的视觉语音识别框架DLLM-VSR，通过迭代掩码去噪和长度引导解码解决自回归解码中的视觉模糊性问题，并在LRS3数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语音识别系统通常采用从左到右的自回归解码，这迫使模型在获得足够上下文之前就对视觉上模糊的标记做出过早决策，从而影响识别效果。

Method: 提出DLLM-VSR框架，将转录建模为迭代掩码去噪过程；采用基于置信度的去掩码策略，优先确定高置信度标记作为双向上下文来优化模糊部分；设计了两阶段训练策略以分离视觉到文本的对齐与长度建模；开发了长度引导候选解码，利用视频时长减少目标长度的不确定性。

Result: 该方法在LRS3数据集上仅使用标记训练数据即达到了19.5%的词错误率（WER），创造了新的最先进记录。

Conclusion: DLLM-VSR成功将扩散模型应用于视觉语音识别，证明了利用双向上下文解决视觉模糊性以及减少目标长度不确定性的有效性，为VSR任务提供了一种新的高效范式。
...
### [8] [FLARE: Diffusion for Hybrid Language Model](http://arxiv.org/abs/2606.01774v1)
...
TL;DR: 本文提出FLARE框架，通过优化迁移数据质量将混合注意力大模型转换为扩散模型，实现了自回归验证与并行去噪的统一，显著提升了推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型的顺序解码存在延迟瓶颈，而现有的结合高效架构（混合注意力）与并行生成（扩散模型）的方法在转换过程中难以保留模型能力，且面临训练和服务方面的技术挑战。

Method: 提出FLARE系统化转换框架，确定迁移数据质量为能力保持的关键因素；结合令牌均衡的自回归与扩散目标、硬件感知内核及统一推理机制，使单一检查点支持双模式解码。

Result: 该方法在各模型规模上与领先的开源扩散大语言模型性能相当，并在单GPU并发服务中实现了持续的吞吐量提升。

Conclusion: 实用扩散模型的性能瓶颈不仅在于解码算法，更受限于迁移数据质量和当前块扩散目标的训练低效性，未来需对数据、目标、架构及推理系统进行联合设计。
...
### [9] [GDSD: Reinforcement Learning as Guided Denoiser Self-Distillation for Diffusion Language Models](http://arxiv.org/abs/2605.29398v1)
*Xiaohang Tang,Keyue Jiang,Che Liu,Qifang Zhao,Xiaoxiao Xu,Sangwoong Yoon,Ilija Bogunovic*

Main category: cs.LG

TL;DR: 本文提出了引导去噪器自蒸馏（GDSD）方法，通过无似然自蒸馏解决ELBO替代似然带来的偏差，显著提升了扩散大语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型的强化学习方法利用ELBO估计似然会导致训练-推理不匹配偏差，从而影响性能，本文旨在消除该偏差并提升训练稳定性。

Method: 提出GDSD方法，基于反向KL正则化RL的闭式最优解构建优势引导的自教师，通过无归一化目标直接将去噪器的对数几率与自教师匹配，将RL转化为无似然自蒸馏。

Result: 在规划、数学和编程基准测试中，GDSD训练动态更稳定，测试准确率提升高达19.6%，全面优于现有的基于ELBO的SOTA方法。

Conclusion: 不依赖ELBO似然替代的直接去噪器自蒸馏方法，为扩散大语言模型提供了一种更稳定、有效的强化学习范式。
...
### [10] [BlockBatch: Multi-Scale Consensus Decoding for Efficient Diffusion Language Model Inference](http://arxiv.org/abs/2605.29233v2)
*Xiaoyou Wu,Cheng-Jhih Shih,Binfei Ji,Yong Liu,Yingyan Celine Lin*

Main category: cs.LG

TL;DR: 本文提出了BlockBatch，一种通过并行执行多粒度分支并协调其状态来解决扩散语言模型推理中粒度权衡问题的免训练框架，实现了显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型推理面临粒度权衡难题（小块需大量去噪步，大块易致过早承诺和缓存误差），且现有加速方法单一固定块大小，未利用不同块大小间的互补性。

Method: 提出BlockBatch免训练在线推理框架，通过在批量前向传递中并行执行多个块大小分支，并利用置信度门控token合并、基于领导者的同步及周期性全序列刷新机制来协调分支状态。

Result: 在3个代表性dLLM和4个数据集上，BlockBatch平均减少26.6%的去噪NFE，相比Fast-dLLM实现1.33倍端到端加速，且保持了模型精度。

Conclusion: 块大小多样性是分支并行dLLM推理中一个实用且此前未被充分探索的优化维度，有效提升了推理效率。
