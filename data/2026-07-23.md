...
### [1] [Multimodal Large Language Models for Remote Sensing Image Understanding: Domain-Specific or General-Purpose?](http://arxiv.org/abs/2607.20284v1)
*Qiwei Ma,Chunping Qiu,Xinjun Cheng,Xiaoyu Zhang,Puhong Duan,Ke Yang,Xudong Kang,Shutao Li*

Main category: cs.CV

TL;DR: 本文对用于遥感图像场景理解的多模态大语言模型进行了系统性综述与诊断评估，发现通用视觉大模型具有强迁移性，在部分任务上可匹敌甚至超越专用遥感模型。


<details>
  <summary>Details</summary>
Motivation: 当前领域缺乏对现有遥感多模态大语言模型的能力边界、跨任务泛化能力及特定任务局限性的系统性理解。

Method: 系统性综述RS-MLLMs的技术演变（模型设计、多模态学习、训练数据等），并在多种任务和基准上将RS-MLLMs与通用计算机视觉MLLMs进行对比诊断评估。

Result: RS-MLLMs在特定任务（遥感视觉定位和高分辨率视觉问答）上具竞争力，但通用CV-MLLMs无需微调即可在部分任务上匹敌或超越它们；当前模型在空间与关系推理、细粒度视觉理解等方面仍存在局限。

Conclusion: 通用CV-MLLMs迁移能力强，现有RS-MLLMs未能在所有任务上保持绝对优势；未来研究应聚焦可靠评估、高分辨率推理、高效部署及工具增强的遥感智能体。
...
TL;DR: 本文提出PercepCap视频描述框架，通过先生成时空感知轨迹再生成描述的显式感知链来提升模型的细粒度时空理解与描述质量。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs直接生成视频描述而未暴露背后的感知证据，导致时空感知错误仅在最终描述中显现，难以直接定位和识别潜在的感知错误。

Method: 提出PercepCap框架，采用“感知-描述”生成链，先输出物体轨迹和时间事件的感知轨迹，再生成最终描述；配合两阶段训练策略（监督微调与强化学习）及描述锚定感知数据构建流程来生成对齐的训练数据。

Result: 在直接描述和描述转问答评估中，PercepCap持续优于Qwen3-VL基线，并展现出领先的描述质量。

Conclusion: 在生成最终描述前显式引入时空感知证据链，能够有效减少底层感知错误并显著提升视频描述质量。
...
### [3] [Look Less, Think Faster: Joint Token-Compute Adaptation for Multimodal LLMs](http://arxiv.org/abs/2607.20357v1)
*Pengcheng Wang,Zhiquan Wang,Jayoung Lee,Zhuoyan Xu,Ran Xu,Saurabh Bagchi,Yin Li,Somali Chaterji*

Main category: cs.CV

TL;DR: SmartVL是一个统一的自适应推理框架，通过联合控制视觉token数量和LLM计算能力来降低多模态大语言模型的推理成本。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型因大量输入视觉token和LLM的繁重计算导致推理成本高昂。现有方法通常独立优化这些维度，忽略了根据输入内容在所有维度上动态分配计算资源的根本耦合关系。

Method: 提出SmartVL框架，包含视觉侧token控制器和LLM侧计算控制器。通过共享预算编码和可微延迟估计器进行端到端训练，使两个控制器相互协调，实现跨阶段的联合调度，以满足目标计算预算。

Result: 在多个MLLM基准测试上的实验表明，SmartVL通过联合调度持续优于先前的自适应方法，并实现了更优的准确率与效率帕累托前沿。

Conclusion: 根据输入复杂度和运行时计算约束，联合调度视觉token和LLM计算资源，能够有效突破现有独立优化方法的局限，实现更优的准确率-效率权衡。
...
### [4] [Diverse-Intent Multi-Turn Fashion Image Retrieval](http://arxiv.org/abs/2607.20291v1)
*Mingqiang Tang,Haokun Wen,Meng Liu,Yupeng Hu,Weili Guan,Xuemeng Song*

Main category: cs.CV

TL;DR: 本文提出了包含多样化意图转换的多轮时尚检索基准DIM-Fashion，并开发了避免文本化处理的多模态对齐框架FashionAM以提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设多轮交互遵循相同的属性编辑范式，忽略了异构意图转换；且依赖文本化处理多模态查询会丢失细粒度的视觉线索。

Method: 构建了包含2.6万个多轮对话的DIM-Fashion基准；提出FashionAM（MLLM-VLP）框架，直接将多模态对话查询与面向时尚的图库嵌入空间对齐，绕过中间文本化过程。

Result: 大量实验证明了FashionAM框架相比现有方法在多轮时尚检索任务上的有效性。

Conclusion: 所提框架有效解决了视觉线索丢失和意图转换受限的问题，数据集和代码后续将公开以促进相关研究。
...
### [5] [RS-RIE-Bench: Benchmarking Reasoning-Guided Remote Sensing Image Editing](http://arxiv.org/abs/2607.20197v1)
*Zihan Qin,Boao Xu,Zhao Dong,Yingping Sun,Ziheng Jiao,Junying Wang,Hongwei Wang*

Main category: cs.CV

TL;DR: 本文提出了首个推理引导的遥感图像编辑基准RS-RIE-Bench，揭示了当前模型在地理推理、区域控制和传感器一致性生成方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的图像编辑基准主要针对自然图像或通用视觉场景，无法充分评估遥感图像编辑所需的推理、区域控制和传感器一致性能力。

Method: 提出RS-RIE-Bench基准，包含时间推理、因果推理和空间推理三类任务，并从目标区域合理性、非目标区域保留和图像质量一致性三个维度进行评估，同时利用多模态大语言模型结合交叉评判一致性和专家审查验证可行性。

Result: 对8个图像编辑模型的评估表明，当前模型存在明显局限；最强模型在严格联合满足标准下准确率仅为24.28%，所有模型在宽松联合成功率上平均值为32.23%，因果与空间推理尤为困难。

Conclusion: 该基准有效揭示了现有模型在地理推理、区域控制和传感器一致生成上的不足，为未来遥感智能编辑模型提供了标准化基准和明确的研究方向。
...
### [6] [MV-Bench: Benchmarking Multimodal Large Language Models for Coordinated Multi-View Interface Construction](http://arxiv.org/abs/2607.19910v1)
*Yue Zhao,Hongxu Liu,Feiyu Wang,Xiaoyu Yang,Tong Ge,Zhen Yang,Chao Wang,Qiong Zeng*

Main category: cs.CV

TL;DR: 提出了MV-Bench，一个用于评估多模态大语言模型在协调多视图界面构建任务上表现的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有的评估主要关注单图表生成，忽视了需要联合推理数据语义、视图协调和交互逻辑的协调多视图界面构建，导致该领域缺乏专门的系统性评估基准。

Method: 使用Tableau工作簿文件作为真实值，开发多阶段流水线将其转换为可执行Web界面，构建包含92个基础界面和1048个实例的基准，并从视觉保真度、数据绑定正确性和交互完整性三个维度评估五个最先进的MLLMs。

Result: 最强模型在视觉布局复现准确率达75.45%，但在数据绑定和交互完整性上仅分别为21.71%和11.68%；迭代优化虽提高了代码可执行性，但未显著弥合数据绑定和交互生成的差距。

Conclusion: 当前的多模态大语言模型能够复现视觉外观，但在生成协调多视图界面所需的数据语义和交互逻辑方面能力有限。
...
### [7] [Memory-Augmented Multimodal Large Language Models for Small Object Understanding in Streaming Aerial Videos](http://arxiv.org/abs/2607.19857v1)
...
TL;DR: 本文提出了首个微小航空目标分割数据集DroneEyes及多模态大模型SkyAnchor，以解决无人机在线流式感知中微小目标细节丢失与历史上下文记忆受限的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在无人机在线流式感知中存在两大问题：视觉压缩机制平等对待所有区域导致微小目标细节丢失；机载硬件资源有限，难以保留全部历史帧，而丢弃历史会导致目标漂移或消失。

Method: 构建了首个微小航空目标像素级开放词汇分割数据集DroneEyes；提出了MLLM模型SkyAnchor，设计了在有限预算下保留小目标的语义感知令牌路由器，以及维持流式视频目标理解一致性的分层记忆库。

Result: 构建了包含2,140个高清视频和176,623对数据的DroneEyes数据集，并设计了SkyAnchor模型架构以应对微小目标视觉压缩和流式上下文记忆挑战。

Conclusion: 通过数据（DroneEyes）与方法（SkyAnchor）的双重创新，有效解决了无人机在线流式场景下微小目标感知与历史上下文记忆的难题。
...
### [8] [D3VL: Understanding Driving Scenes from 3D Time Series Data and Video with Language Models](http://arxiv.org/abs/2607.19528v1)
*Heesang Han,A. Lynn Abbott,Abhijit Sarkar*

Main category: cs.CV

TL;DR: 本文提出了D3VL框架，将2D和3D时间序列数据集成到单一多模态大语言模型中，用于自动驾驶场景理解和安全问答。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型主要依赖2D图像，由于LiDAR数据稀疏且缺乏网格结构，3D传感器数据的融合较少见，但3D数据对自动驾驶系统的感知性能至关重要。

Method: 提出D3VL框架，在单一简单架构中整合2D和3D时间序列数据，以回答交通场景理解和安全相关问题。

Result: D3VL在KITTI问答数据集上比基线方法提升了11%，并引入了Waymo QA数据集扩展以评估模型在多样化驾驶条件下的3D和时间序列数据处理能力。

Conclusion: D3VL成功验证了在多模态大语言模型中集成3D时间序列数据的有效性，相关代码和WaymoQA数据集扩展已开源。
...
TL;DR: 本文提出了一种基于轨迹的同策略蒸馏（TOPD）框架，无需奖励估计即可高效地将推理能力迁移到扩散大语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有面向扩散大语言模型推理的后训练方法面临挑战：监督微调（SFT）需要密集但通常偏离策略的掩码状态，而强化学习（RL）则依赖稀疏奖励或价值建模。

Method: 提出基于轨迹的同策略蒸馏（TOPD）框架，从目标模型采样同策略扩散轨迹，在部分去噪状态下获取教师模型的token分布，并使用token级别的反向KL散度目标更新目标模型，在模型自身的去噪轨迹上进行监督且无需奖励估计。

Result: 在数学推理基准上，TOPD使SDAR-4B-Chat在MATH500上的准确率与RL训练的TraDo-4B-Instruct相当（静态评估提升5.7，动态评估提升4.5），且所需rollout轮次减少4倍，预计模型计算速度提升约96.0倍。

Conclusion: TOPD为扩散大语言模型的推理后训练提供了一种高效的教师监督方法，在保留密集监督的同时实现了与模型自身去噪状态的对齐，计算成本显著低于强化学习方法。
...
TL;DR: 本文提出了CoCommit方法，通过标记门控协调机制在扩散大语言模型中实现联合模式解码，以减少多词元独立解码产生的因子分解误差，从而提升模型准确率。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在每个去噪步骤中独立解码多个词元，当这些词元存在依赖关系时会产生因子分解误差，而现有的基于置信度的选择方法仅依赖边缘分布，无法观察到该误差。

Method: 提出CoCommit，一种标记门控协调过程。在常规束选择后，通过学习到的标记宣布提交集，并重新应用骨干网络的最后几层，使标记位置在贪婪argmax写入词元前进行协调，从而近似联合模式解码。该方法仅复用现有权重，需一次额外部分前向传播，无需辅助模型。

Result: 在使用LoRA适配器和匹配贪婪推理的LLaDA2.1-mini模型上，联合提交在所有六个评估基准上均提高了准确率，其中在推理和精确答案任务上的提升最为显著。

Conclusion: CoCommit通过近似联合模式解码有效缓解了扩散语言模型中的因子分解误差，在无需额外模型的情况下显著提升了多任务准确率，尤其是推理类任务。
...
<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型通过后训练激发推理能力十分困难：监督微调存在脱轨问题，强化学习奖励稀疏且难以应用；而传统的在策略自蒸馏依赖测试时不可用的外部特权信息，导致蒸馏效果有限。

Method: 提出dOPSD方法，直接从学生模型自身的去噪轨迹中提取教师模型的特权信息，利用同一轨迹中后续更充分的解码步骤来评估掩码位置，使优势从模型自身的解码过程中产生。

Result: 在Dream和LLaDA模型上的实验表明，dOPSD不仅提升了领域内的数学推理能力，还增强了领域外的代码生成能力，表现优于监督学习和在策略基线方法。

Conclusion: dOPSD无需外部特权信息即可提供密集的、token级别的在策略监督，成功突破了扩散大语言模型在推理任务上的后训练瓶颈。
...
TL;DR: 本文提出DominoTree，一种结合路径依赖条件修正与GPU原生构建器的免训练草案树方法，显著提升了LLM推理的接受长度和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草案器和优先最优树方法（如DDTree）仅建模边缘分布，无法表示Domino草案器中基于GRU的路径依赖条件修正，限制了推理加速的潜力。

Method: 提出DominoTree，利用Domino的条件、非分解修正沿根到节点路径进行评分，构建免训练的优先最优草案树；通过限制每节点修正的候选top-M集合并采用GPU原生CUDA图构建器，实现低成本且与参考实现位一致的树构建。

Result: 在Qwen3-4B上达到6.6倍自回归解码加速，各温度下平均接受长度最高（达10.7 token/轮）；吞吐量较Domino提升9-10%（Alpaca上达22%），全面超越DDTree和CaDDTree。在Qwen3-8B上T=0时吞吐量较DDTree提升24%，且在各温度下保持最高接受长度。

Conclusion: DominoTree通过将路径依赖的条件修正有效整合到草案树构建中，以较低的计算开销大幅提升了LLM的推理效率和接受长度，证明了其在不同模型规模和温度下的有效性。
...
TL;DR: 本文提出CaDDTree方法，通过显式建模延迟并联合优化树结构和节点预算，直接优化推测解码中的令牌吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的DDTree方法在固定节点预算下最大化期望接受长度，但该指标随预算非递减，导致其偏向选择更大的树而忽略验证成本，缺乏选择预算的原则性依据。

Method: 提出CaDDTree，通过显式建模草稿与验证延迟来直接优化令牌吞吐量。该方法将吞吐量目标分解为每轮的一维预算搜索，并证明在凸验证成本下吞吐量函数具有单峰性，从而利用高效的贪婪停止规则实现每轮自适应预算调整，无需离线搜索。

Result: 在Qwen3-4B和Qwen3-8B模型及涵盖推理、编码和指令遵循的八个基准测试中，CaDDTree在几乎所有任务上均达到或超过了具有先验最优预算的DDTree性能。

Conclusion: 显式建模验证成本并自适应调整节点预算能有效提升推测解码吞吐量，CaDDTree无需离线调优即可实现优异的推理加速效果。
...
TL;DR: DDTree通过从块扩散起草模型的分布中构建草稿树，在单次前向传递中验证多条轨迹，从而提升了推测解码的性能。


<details>
  <summary>Details</summary>
Motivation: 原始的DFlash模型每轮仅验证单条起草轨迹，这限制了其接受长度的进一步提升。

Method: 提出DDTree方法，直接从块扩散起草模型的位置分布构建草稿树；在固定节点预算下，使用最佳优先堆算法选择最可能匹配目标模型的延续，并利用仅祖先注意力掩码在单次目标模型前向传递中完成高效验证。

Result: 在DFlash的基础上取得了性能提升，使DDTree成为推测解码领域的领先方法之一。

Conclusion: 通过将块扩散起草模型与树状草稿验证机制相结合，DDTree有效突破了单轨迹验证的瓶颈，进一步推动了推测解码技术的发展。
...
### [15] [VizRAG: Enhancing Retrieval-Augmented Generation with Hypergraph Visualization](http://arxiv.org/abs/2607.19830v1)
*Yanbin Wei,Yang Chen,Renling Gan,Ziru Liu,Xinyu Fu,Chun Kang,Ning Lu,Rui Liu,Yu Zhang,James Kwok*

Main category: cs.CL

TL;DR: 提出VizRAG，首个结合超图视觉表示的检索增强生成系统，以发挥多模态大语言模型的视觉感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于超图的RAG系统受限于文本中心的单模态范式，未能充分利用现代多模态大语言模型强大的视觉感知能力。

Method: 引入VizRAG，将超图的视觉表示整合到RAG流程中，实现视觉超图结构感知。

Result: 实验结果表明，VizRAG显著优于强基线模型。

Conclusion: 超图可视化作为RAG系统的一种新方法具有广阔的潜力。
...
TL;DR: 本文提出了Sangam，一个用于扩散语言模型(dLLM)缓存推理的服务系统，通过赤字令牌预算调度器和混合服务策略优化性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型使用近似KV缓存技术导致重复的预填充/解码结构，但无法直接套用自回归(AR)模型的服务机制，且面临块级解码和双向注意力带来的预填充-解码干扰与资源划分挑战。

Method: 引入赤字令牌预算调度器优先处理在途解码，仅在预算允许时处理完整预填充并结转剩余预算，实现无停顿调度；采用混合服务策略将预填充溢出到解码工作节点并保护其解码过程。

Result: 在解码密集型负载中，同置服务比混合执行降低9-20%的平均延迟；在预填充密集型负载中，混合执行比同置服务降低8-20%的平均延迟。

Conclusion: dLLM服务设计空间同样受预填充-解码干扰和资源划分制约，需根据负载类型选择同置服务（适合解码密集型）或混合执行（适合预填充密集型）。
...
TL;DR: DELTA-TTS通过轻量级LoRA适配将预训练的AR TTS模型转换为离散扩散模型，实现了置信度排序的语音解码，提升了速度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决自回归（AR）TTS模型因从左到右顺序生成语音token导致的推理缓慢、局部错误传播和幻觉问题。

Method: 提出DELTA-TTS框架，利用LoRA将预训练AR TTS模型转化为离散扩散语言模型（dLLM）进行置信度排序解码；结合卷积模块注入局部声学上下文，采用1/t加权训练目标和时间偏移推理计划推迟低置信度位置的生成。

Result: 在仅使用585小时LibriTTS数据训练下，于Seed-TTS test-en测试集上达到1.75%的WER，优于其AR主干模型且生成速度提升3.3倍；同时产生更清晰的文本-语音对齐，提高了解码置信度并减轻了幻觉。

Conclusion: 将AR TTS模型转化为离散扩散模型不仅能加速生成并降低错误率，还能有效提升解码鲁棒性和缓解幻觉现象。
...
### [18] [The Ethics of Autonomous AI Agents for Offensive Security](http://arxiv.org/abs/2607.20255v1)
*Andreas Happe,Jürgen Cito,Jasmin Wachter*

Main category: cs.CR

TL;DR: 本文分析了LLM驱动的自主代理在进攻性安全中的不确定性，探讨了道德归属问题并提出了分层建议。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的进攻性安全工具在行动、影响范围和用户技能要求上表现出独立性不确定性，结合攻防成本不对称，使现有双用途框架失效。

Method: 分析在使用自主AI代理进行进攻性安全操作时，道德归属如何在用户、工具开发商和第三方之间分散，并评估利益相关者的影响。

Result: 不确定性与结构性成本不对称相结合，短期内有利于攻击者并实现了进攻性能力的工业化，尽管长期可能普及防御能力。

Conclusion: 现有的双用途网络安全和AI伦理框架不适用于此，本文针对利益相关者提供了分层建议。
...
TL;DR: 本文探讨了通过编排小语言模型（SLM）集成来分析恶意软件引爆报告，发现混合编排架构能显著提升模型性能并接近前沿大模型的水平。


<details>
  <summary>Details</summary>
Motivation: 恶意软件分析需要快速解读复杂的引爆报告。闭源大模型不透明且API成本高，而现有的开源模型体积大、计算资源需求高，不适合资源受限的部署环境。

Method: 设计并评估了四种小语言模型编排架构：多智能体流水线、对抗性辩论框架、层级咨询系统，以及结合证据溯源与对抗辩论的混合架构。

Result: 混合系统准确率达35.30%，超越最强的网络安全专用基线（22.54%）和无证据基础的前沿基线（34.77%）；在相同证据流水线下，基于证据的Gemini准确率最高为38.22%。

Conclusion: 基于证据溯源的模型编排能够显著提升协同小语言模型在解读恶意软件引爆报告方面的性能，为资源受限环境提供了可行的高效替代方案。
...
### [20] [IteraSim RAG: A Multi-Stage Retrieval-Augmented Agentic Back-End for OpenFOAM-Based Computational Fluid Dynamics](http://arxiv.org/abs/2607.20346v1)
*Pratyush Kumar*

Main category: cs.CE

TL;DR: 本文提出IteraSim RAG系统，通过多查询扩展、路由分流检索和多代理协作自动生成OpenFOAM CFD案例配置。


<details>
  <summary>Details</summary>
Motivation: OpenFOAM案例配置需要组装多目录、相互一致的求解器、离散化和边界条件字典，对非专业用户构成重大障碍；现有LLM-RAG系统存在单次平面查询、单一检索策略和单代理自审自校的局限。

Method: LLM将查询扩展为物理、求解器关键词和故障排除变体，使用Reciprocal Rank Fusion合并排名列表，Maximal Marginal Relevance对HNSW索引密集向量库重新排名；确定性关键词路由器将工具条件工作流查询和物理查询分流至不同检索路径；生成过程由Architect、InputWriter和Reviewer三个代理协作完成，并有静态规范知识层支持。

Result: 在28个案例基准上，平均检索覆盖率为77.9%（中位数79.1%），参数修改类别超过90%；六个参考配置在OpenFOAM v2506上全部运行完成；两个损坏案例在Reviewer循环内被诊断和修复。

Conclusion: IteraSim RAG有效降低了OpenFOAM配置门槛，通过多查询扩展、路由分流和多代理协作显著提升了检索覆盖率和案例生成可靠性；基准测试、评分标准和脚本已公开发布以支持可复现性。
...
### [21] [Learning to Decode Quantum LDPC Codes via Cluster-Based Sequential Belief Propagation](http://arxiv.org/abs/2607.20130v1)
*Mohsen Moradi,Taejoon Kim,Rémi A. Chou,David G. M. Mitchell*

Main category: cs.IT

TL;DR: 本文提出一种基于集群的强化学习调度方法用于量子低密度奇偶校验码的置信传播解码，在保留误码率优势的同时显著提升并行性并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 基于强化学习的顺序变量节点调度改善了量子低密度奇偶校验码的置信传播解码性能，但其逐节点更新的特性限制了迭代内并行度，导致延迟较高。

Method: 将变量节点划分为固定集群，强化学习代理每步选择一个集群，集群内所有节点利用预更新传入消息进行并行更新；同时提出基于局部不匹配权重归一化直方图和量化的置换不变状态表示，以控制马尔可夫决策过程的状态空间。

Result: 数值结果表明，该方法保留了节点级顺序调度的大部分误码率优势，并大幅减少了每次迭代的调度决策次数。

Conclusion: 基于集群的学习调度方法在解码延迟与并行性之间提供了有效的权衡，是改进量子LDPC码BP解码的实用方案。
...
### [22] [State-Dependent Observation Noise Reintroduces Epistemic Value in Linear-Gaussian Active Inference](http://arxiv.org/abs/2607.20306v1)
*Daniel Corva*

Main category: q-bio.NC

TL;DR: 本文证明状态依赖的观测噪声协方差R(x)能够在线性-高斯主动推理框架中恢复认知驱动，填补了双控制与主动推理之间观测侧的理论空白。


<details>
  <summary>Details</summary>
Motivation: 已有工作表明线性-高斯状态空间模型在主动推理下丧失认知驱动（信息获取激励），其Expected Free Energy的认知项变为常数，智能体退化为增益序列预先固定的卡尔曼滤波器。已知恢复该驱动的唯一途径是控制以乘法方式进入动力学，而观测侧的最小偏离条件尚属未知。

Method: 提出将观测噪声协方差设为状态依赖的R(x)，表示传感器精度随状态（如距离）变化。智能体采用该文献中的标准一阶高斯滤波器，在预测均值处评估R(x)。通过将R(x)与可控的潜在均值耦合，使后验协方差和有效卡尔曼增益依赖于动作，从而打破固定线性-高斯滤波器的等价性。配套库cpomdp可从模型规范中检测线性化不兼容性并抛出类型化异常。

Result: 在观测映射的秩条件与R(x)非退化条件下，认知价值不再为常数；对标量观测仅需可达非常数性即可。该构造是Bar-Shalom-Tse双重效应在智能体维护协方差中的最小构造实例：动作影响未来估计的质量而非仅状态。定理附带可执行见证程序，任何复现智能体信念的固定滤波器都将同时反驳定理与见证。

Conclusion: 本文提供了高斯智能体中好奇心的精确观测侧刻画，桥接了双控制理论与主动推理，表明状态依赖观测噪声是恢复认知驱动的最小充分条件，并为模型线性化兼容性提供了自动化检测工具。
...
### [23] [Improved Lower Bounds and Output Augmentation for Facility Location Mechanisms](http://arxiv.org/abs/2607.20196v1)
*Rafael Gomes,Sophie Klumper,Guido Schäfer,Jens Schlöter*

Main category: cs.GT

TL;DR: 本文研究了欧几里得空间中基于平均主义目标的战略设施选址问题，提出了防策略机制的近似比下界，并通过输出增强框架设计了多种高效机制。


<details>
  <summary>Details</summary>
Motivation: 旨在解决防策略机制在最小化最大距离时的性能极限问题，并探索在放宽设施放置位置限制时如何设计更优的防策略机制。

Method: 通过数学证明推导近似比下界，并引入输出增强框架（允许设施放置在智能体受限域之外）来设计确定性与随机性防策略机制。

Result: 证明了期望防策略机制近似比的渐近下界为 $1 + \sqrt{d/(2(d+1))}$；为双智能体提供了随机 $\sqrt{2}$ 近似机制；在直线到平面增强场景下给出匹配下界的确定性 $\sqrt{2}$ 近似机制；在单位圆到平面场景下给出期望群体防策略的随机 $3/2$ 近似机制。

Conclusion: 防策略机制的近似比下界主要由大规模群体驱动；输出增强框架能有效替代随机性以实现确定性防策略，并在特定受限域下取得更优性能。
...
### [24] [Distributed Motion Planning with Safety Guarantees for Self-Reconfiguring Robotic Boats](http://arxiv.org/abs/2607.20352v1)
*Alejandro Gonzalez-Garcia,Wei Wang,Wei Xiao,Wilm Decre,Jan Swevers,Carlo Ratti,Daniela Rus*

Main category: cs.RO

TL;DR: 提出一种结合分布式模型预测控制（MPC）与控制障碍函数（CBF）的混合框架，用于水下多智能体自重构机器人的安全形状形成与重构。


<details>
  <summary>Details</summary>
Motivation: 水下自重构机器人在组装成目标形状时，需要解决非凸优化和局部极小值问题，并必须确保多智能体交互的实时碰撞避免与安全性。

Method: 通过交替方向乘子法（ADMM）求解分布式MPC以计算协调轨迹，并利用基于分布式CBF的滤波器强制执行实时防碰撞；结合MPC的预测能力缓解局部极小值，CBF在非凸优化中提供形式化安全保证。

Result: 多达25个智能体的仿真和4个物理机器人的实验验证了该框架在多智能体形状形成与重构中的有效性及良好的可扩展性。

Conclusion: 该混合框架有效解决了水下自重构机器人在形状形成与重构过程中的轨迹规划与实时安全挑战，具有良好的可扩展性与实际应用潜力。
...
### [25] [Autonomous Collaborative Learning Among an Ensemble of Tsetlin Machines with Consensus-Based Inference](http://arxiv.org/abs/2607.20124v1)
*Yehuda Rudin,Osnat Keren,Michal Yemini,Alexander Fish*

Main category: cs.LG

TL;DR: 本文提出了一种基于共识推理的去中心化协作学习范式，用于垂直特征划分下的Tsetlin Machine集成，实现了与中心化模型相当的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 现有文献对分布式和去中心化Tsetlin Machine (TM) 学习的探索不足，亟需一种在不交换原始数据的前提下，整合异构数据源和多模态信息的去中心化学习方案。

Method: 提出一种在垂直特征划分下基于共识推理的去中心化TM学习范式。各智能体维护私有TM模型且互不交换原始数据，通过融合各智能体的模型预测形成全局共识，支持异构TM智能体协同。

Result: 在二维网格和连通图网络拓扑上的实验表明，该去中心化范式取得的分类准确率与中心化模型相当。

Conclusion: 该去中心化范式有效兼顾了数据隐私与异构资源融合，成功拓展了TM在多模态传感等分布式场景中的应用潜力。
...
### [26] [A Continuous-Time Reinforcement Learning Framework for Fine-Tuning Discrete Diffusion Models](http://arxiv.org/abs/2607.14522v1)
*Zikun Zhang,Jiayuan Sheng,David D. Yao,Wenpin Tang*

Main category: cs.LG

TL;DR: 提出基于连续时间马尔可夫链的连续时间强化学习框架，用于微调离散扩散模型，支持中间奖励并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于GRPO的方法仅依赖终端奖励、无法整合去噪轨迹中间奖励，以及计算大语言模型轨迹似然成本过高的问题。

Method: 采用随机控制方法建立连续时间马尔可夫链模型，推导连续时间的PPO和GRPO变体；针对掩码扩散模型提出具有解析概率比率的策略参数化；引入轨迹子采样技术降低概率比率计算成本。

Result: 在低维熵正则化优化问题以及离散扩散大语言模型的数学推理和编码任务的后训练中验证了方法的有效性。

Conclusion: 该框架为掩码扩散模型提供了统一的探索与策略优化视角，有效整合了中间奖励并提升了计算效率。
...
### [27] [Accelerating Masked Diffusion Large Language Models: A Survey of Efficient Inference Techniques](http://arxiv.org/abs/2607.12829v1)
*Daehoon Gwak,Minhyung Lee,Junwoo Park,Jaegul Choo*

Main category: cs.LG

TL;DR: 本综述引入统一的延迟分解框架以解耦扩散大语言模型的推理延迟因素，并从算法、架构/系统和推理时缩放三个维度系统分类了加速技术。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽具备理论上的并行生成优势，但难以直接转化为实际速度提升；现有基准测试混淆了算法、架构和系统层面的复杂权衡，阻碍了严格的比较与评估。

Method: 提出一个统一的延迟分解框架来剖析实际部署中影响推理速度的因素，并据此将加速技术按算法创新、架构与系统优化、推理时缩放三个轴线进行分类分析。

Result: 基于统一框架解构了端到端延迟的复杂权衡，系统性地梳理并归类了扩散大语言模型现有的跨维度加速技术。

Conclusion: 提出了可复现基准测试的指导原则，并指明了实现并行生成全部潜力的开放性挑战，强调了算法、架构与系统协同优化的必要性。
...
### [28] [BlockServe: Block-Grained Continuous Batching for High-Throughput Diffusion LLM Serving](http://arxiv.org/abs/2607.08930v1)
*Yuanjie Zhu,Liangwei Yang,Ke Xu,Weizhi Zhang,Shanghao Li,Zihe Song,Philip S. Yu*

Main category: cs.LG

TL;DR: 本文提出了BlockServe连续批处理框架，通过块粒度调度和混合状态执行解决扩散大语言模型服务中的收敛异构性问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在批处理多请求时面临收敛异构性问题，不同序列收敛速率不同导致快请求被慢请求阻塞，产生计算空泡和尾延迟，从而阻碍了高效服务。

Method: 提出BlockServe框架，结合块粒度调度（在块边界立即驱逐已完成请求）、混合状态执行（通过gather-scatter索引将双缓存和并行解码扩展至异构批次）以及计算感知准入控制器（通过令牌预算补充扩大有效批次容量）。

Result: 在Dream和LLaDA模型及五个基准测试上，BlockServe的吞吐量比Fast-dLLM高出1.9至10.6倍，且生成质量相当。

Conclusion: 确立了块粒度调度作为高吞吐量离线dLLM推理的基础。
...
### [29] [PoTRE: Test-Time Reasoning inspired by Cognitive Heterogeneity](http://arxiv.org/abs/2607.20268v1)
*Anmol Kankariya,Sercan Ö. Arık*

Main category: cs.AI

TL;DR: 本文提出了异构多智能体推理框架PoTRE，通过解耦推理过程并动态聚合多视角结果，显著提升了大型语言模型在复杂推理任务上的表现与token效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在需要长视界规划和迭代纠错的复杂推理任务中容易遇到困难，且标准单流提示在应对新抽象或严格领域约束时表现出脆弱性。

Method: 提出PoTRE框架，将推理解耦为四个异构智能体（对抗精炼、层次策略规划、频谱搜索、直接链），并通过任务自适应聚合层动态调和这些视角以产生稳健解。

Result: 在三个前沿基准测试中，PoTRE在HLE上取得了49.92%的最先进准确率，超越了之前的最佳官方分数；同时在相似或更少推理token的情况下，表现优于大规模同质基线。

Conclusion: 架构异构性能够有效提升大型语言模型的推理性能，同时保持或提升计算资源利用效率。
...
TL;DR: 本文提出了FlowBlock，一个无需训练的并行解码框架，通过门控波前解码和异构波前打包机制显著提升了块级扩散大语言模型的解码速度和准确性。


<details>
  <summary>Details</summary>
Motivation: 块级扩散大语言模型的块间解码严格串行，限制了速度；而现有的基于后训练的并行化方法加速有限且常导致精度下降。

Method: 提出无需训练的FlowBlock框架，包含两个机制：(1)门控波前解码，通过就绪门允许块进入波前，利用T2T编辑联合细化活动块，并在窗口化块因果掩码下按顺序提交；(2)异构波前打包，为每个请求分配独立波前，并将异步窗口打包成密集且形状稳定的批处理前向传播。

Result: FlowBlock相较于串行模型LLaDA-2.1和LLaDA-2.0，TPS分别提升了2.95倍和4.01倍，延迟分别降低了53.6%和77.1%，平均准确率提高了1.3分。与基于训练的基线D2F相比，准确率更高，且批处理服务吞吐量提升了高达16倍。

Conclusion: 利用自校正dLLM和token到token编辑，无需训练的并行解码能有效将块终结性转化为调度资源，在速度和精度上均显著优于基于训练的方法。
...
TL;DR: 提出了一种名为LaCache的训练免加速框架，通过无损缓存和混合精度量化技术，有效解决扩散式大语言模型（DLLM）在去噪过程中的算子级冗余问题。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散式大语言模型在去噪步骤中会重新计算整个序列，忽略了块内前缀和掩码后缀的不变性，导致严重的算子级冗余，本文旨在解决该冗余问题以提升推理效率。

Method: 提出LaCache框架，采用无损状态记忆（LSM）缓存嵌入输出、预注意力状态和FlashAttention在线softmax统计量三类中间结果以跳过冗余计算；同时引入针对FFN层的按组FP8量化策略以缓解内存带宽瓶颈。

Result: 单独使用LaCache可实现约1.3倍的端到端加速；与现有加速方法结合时，最高可达40.2倍的端到端加速，且保持相当的任务准确率。

Conclusion: LaCache作为一种无损且无需训练的加速方法，能显著提升DLLMs的推理速度并与其他加速技术良好兼容，为大语言模型的高效推理提供了有效方案。
...
TL;DR: 提出TAPS方法，通过目标感知的前缀选择优化扩散模型推测解码的验证过程，显著提升无损加速比。


<details>
  <summary>Details</summary>
Motivation: 扩散模型并行起草将推测解码的瓶颈转移至验证阶段。现有方法按边缘概率排序节点，忽略了验证的前缀条件性，导致验证不可达节点并增加目标模型延迟。

Method: 提出TAPS（目标感知前缀选择），将扩散边缘概率转化为路径条件的接受估计，并在固定验证预算下选择紧凑的前缀闭包子树，以优化接受与成本的权衡。

Result: 在多个数据集和模型上的实验表明，TAPS比原始自回归解码实现高达7.9倍的无损端到端加速，分别比SOTA方法DFlash和DDTree快1.36倍和1.74倍。

Conclusion: TAPS有效解决了扩散推测解码的验证瓶颈，证明了通过优化前缀选择而非盲目扩展草稿树能显著提升推理效率。
...
### [33] [Symbol and Footprint Database for Electronic Components by Agentic Recognition and Generation](http://arxiv.org/abs/2607.19767v1)
*Yichen Shi,Yuzhi Liu,Zhuofu Tao,Li Huang,Yuhao Gao,Ting-Jung Lin,Lei Hel*

Main category: cs.AI

TL;DR: 本文提出基于多模态大语言模型的智能体流程SFgen，用于自动生成电子元件符号与封装，并构建了元件数据库SFnet。


<details>
  <summary>Details</summary>
Motivation: 传统PCB设计中工程师手动创建符号和封装耗时且易错，缺乏高效的自动化元件库构建方法。

Method: 利用多模态大语言模型（MLLM）开发SFgen智能体识别与生成流程，实现电子元件符号和封装的自动生成，并以此构建SFnet数据库。

Result: SFgen的符号生成准确率达86%，封装生成准确率达80%；创建的SFnet数据库已包含1000个元件并持续扩展。

Conclusion: SFgen及SFnet数据库为PCB设计的自动化生成奠定了基础。
