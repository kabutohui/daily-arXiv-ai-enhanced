...
### [1] [HarnessEval-W: Agentifying the Evaluation of Visual Worlds](http://arxiv.org/abs/2608.16859v1)
...
TL;DR: 本文提出了HarnessEval-W，一个将世界模型评估转化为透明证据树的智能体层级评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型基准测试仅通过暴力计算得出单一分数，缺乏可审查的推理链来验证物理、因果及世界状态演变的正确性，无法提供可信赖的评估。

Method: 借鉴LLM测试框架范式，HarnessEval-W将评估问题分解为子问题，并派生配备特定上下文和诊断工具的专用子智能体进行推理；父智能体随后验证收集的证据并生成最终判定，形成完整的证据树。

Result: 在18个世界模型的330个案例上的实验表明，HarnessEval-W的判定与人类偏好高度一致，并能提供可验证、细粒度的轨迹诊断。

Conclusion: 该方法通过透明的推理链提升了世界模型评估的可信度，代码已开源以促进社区共建和基准随模型演进而扩展。
...
### [2] [AnchorScore: A CLIP-Based Diagnostic of MLLM Annotation Difficulty](http://arxiv.org/abs/2608.16690v1)
*Yan Ma,Lizhuo Zhang*

Main category: cs.CV

TL;DR: AnchorScore利用冻结的CLIP零样本准确率作为低成本先验信号，预测并路由多模态大语言模型（MLLM）的类级标注难度。


<details>
  <summary>Details</summary>
Motivation: 评估MLLM标注准确率的成本高昂且耗时，而缺乏一种低成本的信号来先验地按预期MLLM标注难度对类别进行排序。

Method: 系统评估AnchorScore（每类零样本CLIP准确率）作为先验诊断工具，并与DINOv2、ResNet-50、SigLIP和MLLM自述不确定性等其他难度预测因子进行对比。

Result: AnchorScore与每类MLLM准确率显著相关（最高Spearman rho=0.817），优于其他预测因子；其支持的混合CLIP/MLLM路由策略在节省约44% MLLM成本的同时，较纯CLIP准确率提升高达23个百分点。

Conclusion: AnchorScore是一种有效的低成本排序信号，能够将昂贵的MLLM评估引导至最有信息量的类别，但在医学和卫星图像上效果减弱，且无法估计MLLM的精确准确率。
...
### [3] [Matched Outcomes, Divergent Gaze: How Foveated MLLMs Search Compared to Humans](http://arxiv.org/abs/2608.16514v1)
*Mohamed Amine Kerkouri,Marouane Tliba,Aladine Chetouani,Ulas Bagci,Alessandro Bruno*

Main category: cs.CV

TL;DR: 该研究比较多模态大语言模型与人类在视觉搜索中的表现，发现模型在结果上与人类相当，但在眼动时间过程上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 探究多模态大语言模型（MLLMs）在匹配人类中央凹视觉输入时，是否能像人类一样进行视觉搜索，以评估其作为人类视觉模型的有效性及现有注意力对齐指标的可靠性。

Method: 在COCO-Search18数据集上，将三个通用MLLMs与人类眼动轨迹进行逐注视点对比，输入与人类匹配的中央凹视图，从目标存在决策、目标获取效率和注视过程三个维度评估模型。

Result: 模型在目标决策和获取效率上达到或超过人类水平，但在注视过程上与人类不同，表现出低熵、大幅度、高自洽的扫描路径，符合单次非串行架构特征。匹配的视觉输入能重现人类注视位置，但无法重现注视的时间动态过程。

Conclusion: 答案对齐和显著性指标无法衡量这种过程层面的差异，不能以此证明模型具备类人视觉；零样本模型适用于结果和空间层面的问题，但不适用于时间性和过程层面的问题。
...
### [4] [MLLM-Guided Semantic Correction for Text-to-Video Generation](http://arxiv.org/abs/2608.16513v1)
*Junhao Chen,Zheqi Lv,Keting Yin,Shengyu Zhang,Zhou Zhao,Feiyang Chen,Xinyu Duan,Baoxing Huai,Fei Wu*

Main category: cs.CV

TL;DR: 本文提出一种免训练的生成中期校正框架，通过将多模态大语言模型反馈整合至扩散采样循环，实时纠正文本到视频生成中的语义偏差。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频生成模型常出现缺失物体、属性错误等语义偏差，而现有校正方法多关注采样前优化或采样后细化，缺乏在视频生成过程中实时检测与纠正语义偏差的方法。

Method: 提出一种无需训练且可解释的中期校正框架，直接将多模态大语言模型（MLLM）反馈注入扩散采样循环。包含两个核心模块：用于生成中间预览帧并进行语义评估与偏差诊断的语义评估监督器，以及通过可控潜在轨迹干预在推理期间纠正语义漂移的语义修改助手。

Result: 跨多个基准的实验表明，该方法在无需修改模型参数的前提下，有效提升了生成视频的语义对齐度、视觉保真度和时间一致性。

Conclusion: 在扩散采样过程中引入持续的自我反思与语义评估信号能有效纠正生成轨迹，为提升视频生成的语义准确性提供了无需重训练的有效途径。
...
### [5] [Remote-Sensing City Layout Extraction with MLLM](http://arxiv.org/abs/2608.16484v1)
*Zigan Zhou,Kai Li,Yupeng Deng*

Main category: cs.CV

TL;DR: 本文提出Code-as-City框架，将单张俯视图的城市布局提取转化为受限代码生成问题，利用多模态大语言模型生成可检查和编辑的城市代码及3D布局。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感系统通常以检测框或语义掩码描述城市内容，无法提供包含对象身份、类型关系、拓扑结构和重生成规则的可执行布局。

Method: 首先使用图像模型生成对齐的五类语义布局先验，然后通过三次有序的多模态大语言模型（MLLM）处理步骤恢复道路、土地覆盖区域及其关系和建筑物。接着通过确定性归一化将其转化为城市图和受限布局程序，执行该程序以生成可渲染的3D城市布局和正射语义投影。

Result: 在CityLayout-100数据集的100个场景上评估，该框架获得了41.1%的平均交并比和48.3%的全局交并比。

Conclusion: 研究结果提供了定量证据，证明视觉观测可以被转化为可检查、可编辑的城市代码，并生成耦合的平面与3D输出。
...
### [6] [RISE: Roadside Infrastructure Sequence Understanding across 3D Tracking and Structured Vision-Language Reasoning](http://arxiv.org/abs/2608.16480v1)
...
TL;DR: RISE是一个结合无LiDAR的度量3D跟踪和结构化视觉语言推理的路侧序列理解与评估框架，并构建了RISE-VQA数据集和RISE-Bench基准。


<details>
  <summary>Details</summary>
Motivation: 旨在解决路侧基础设施序列中的度量级3D跟踪（无需LiDAR或特定3D训练）和结构化视觉语言推理问题，以提升跨路口泛化能力并填补未来预测推理的评估空白。

Method: 在3D跟踪上，结合SAM3视频身份与校准引导的掩码一致进行多视角身份关联以恢复3D轨迹；在推理上，通过人工审核的MLLM管道挖掘高价值片段，并使用受约束的全上下文Oracle构建基于边界框的预测性问答。

Result: 3D轨迹在评估范围内达到66.9 MOTA；构建了包含33,910个QA对的RISE-VQA数据集及RISE-Bench基准；实验表明领域适配和时间上下文带来提升，但空间定位、未来定位和交互推理仍具挑战。

Conclusion: RISE框架有效实现了无需特定布局重训的跨路口3D跟踪及结构化视觉语言推理评估，揭示了当前模型在时空推理上的局限性，为未来改进指明方向。
...
### [7] [Defake-o3: From Speculative Rationales to Verifiable Evidence for Explainable AIGI Detection](http://arxiv.org/abs/2608.16259v1)
*Bowen Deng,Jiahui Zhan,Yikun Ji,Haozhen Yan,Jianfu Zhang*

Main category: cs.CV

TL;DR: 提出Defake-o3，一种结合交互式视觉搜索和验证器引导证据对齐的可解释AIGI检测器，以解决现有方法生成推测性理由的问题，从而提升检测准确率和证据质量。


<details>
  <summary>Details</summary>
Motivation: 现有的基于多模态大语言模型（MLLM）的AIGI检测器常生成推测性理由，依赖模糊或幻觉伪影，遗漏细微局部缺陷，且缺乏视觉上可验证的证据。

Method: 结合交互式视觉搜索与验证器引导的证据对齐：模型迭代放大可疑区域以检查细节，并利用基于人工验证注释训练的“证据验证器”提供强化学习奖励，以鼓励有根据的证据并惩罚无根据的声明。同时构建了包含局部边界框证据的GroundFake数据集和评估证据质量的FakeFrontier基准。

Result: 在GroundFake、FakeFrontier及其他分布外基准上的实验表明，Defake-o3提高了检测准确率和解释质量，能够生成更具局部性、可验证性和说服力的证据。

Conclusion: Defake-o3成功将AIGI检测从推测性理由转向可验证证据，通过交互式视觉搜索和强化学习验证机制显著提升了模型的准确率与可解释性。
...
### [8] [PWLR: Pairwise Witness Local Rejection for Boundary-Aware Out-of-Distribution Detection](http://arxiv.org/abs/2608.15802v1)
*Chengyao Jia,Ruixuan Wang*

Main category: cs.CV

TL;DR: 提出了一种利用多模态大语言模型生成成对局部视觉线索作为类别边界显式证据的方法PWLR，以提升图像分类器的分布外检测能力。


<details>
  <summary>Details</summary>
Motivation: 解决近分布外样本靠近分布内类别边界时检测困难的问题，现有视觉-语言检测器很少利用语言作为混淆分布内类别间的显式边界证据。

Method: 提出成对见证局部拒绝（PWLR）方法：离线利用MLLM生成区分特定竞争类的局部可见线索，在冻结的视觉-语言骨干网络上仅用分布内数据筛选可靠局部验证器；推理时保留全局合理类别，检查其局部支持度，并通过校准融合成对局部证据与全局得分。

Result: 在ImageNet-100的远分布外、干净/具挑战性分布外及近分布外基准测试中，PWLR在多种骨干网络下均持续优于强大的视觉-语言基线方法。

Conclusion: 利用成对局部语言证据可有效增强视觉-语言模型在分布外检测（尤其是近分布外）中的性能，源代码将开源。
...
### [9] [EgoGazeLite: On-Device Egocentric Gaze Prediction for Token-Efficient Multimodal LLM Video Input](http://arxiv.org/abs/2608.15614v1)
*Matteo Stoiber,Niels Buus Lassen*

Main category: cs.CV

TL;DR: 提出轻量级双进程视线预测器EgoGazeLite，在无需眼动追踪硬件的情况下实现高效的自我中心视频理解。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型处理自我中心视频时的token预算限制问题，特别是现有视线裁剪策略依赖消费级智能眼镜缺乏的专用眼动追踪硬件，需要一种既准确又轻量的纯软件替代方案。

Method: 提出EgoGazeLite，一种轻量级双进程视线预测器，可在设备端实时运行完整的视线预测与裁剪流水线。

Result: 在多种模型和指标评估下，预测视线裁剪与真实视线裁剪无显著差异（10项测试全部等效）。模型仅含15.7M参数和6.71 GFLOPs，在消费级硬件上实现21.6 ms/帧的实时处理。

Conclusion: 消除了对眼动追踪硬件的依赖，使得在消费级设备上实现基于视线条件的高效自我中心视频理解成为可能。
...
### [10] [CBX-Bench: A Human-Aligned MLLM Council for Benchmarking Concept Bottleneck Model Explanations](http://arxiv.org/abs/2608.15404v1)
*Yusuf Meric Karadag,Gulay Oklan,Seref Baris Cagliyan,Umut Ozdemir,Emre Akbas*

Main category: cs.CV

TL;DR: 提出基于多模态大语言模型（MLLM）委员会的评估基准CBX-Bench，用于量化评估概念瓶颈模型（CBM）的图像分类解释质量。


<details>
  <summary>Details</summary>
Motivation: 现有的概念瓶颈模型（CBM）主要依靠分类准确率和孤立的定性示例进行评估，缺乏对解释性的定量评估指标；同时，大规模真实概念标注的不可行性和概念列表的开放性使得建立此类评估指标面临挑战。

Method: 开发了一个由五个开源多模态大语言模型组成的委员会，根据图像和CBM解释生成解释质量评分。通过包含900个图像对比项和2700个人类判断的人工研究（涵盖CUB-200、ImageNet-100和Places365数据集）来验证该委员会，并基于此引入了公共基准和排行榜CBX-Bench。

Result: 该五模型委员会能恢复超过70%的严格人类偏好排名，在人类标注者一致同意的项上恢复率高达83%。

Conclusion: CBX-Bench提供了一种与人类对齐、可扩展的CBM解释质量评估方法，超越了传统的准确率和孤立定性样例评估，为新型CBM提供了一个公共提交与排名的基准平台。
...
### [11] [Ripple-Pivot Search: Active Parallel Decoding for Diffusion Large Language Models](http://arxiv.org/abs/2608.11742v1)
*Yushi Ye,Xu Chen,Haoyun Jiang,Jinsong Lan,Haihong Tang,Bo Han,Ivor Tsang,Yanfeng Wang,Bo Zheng,Jiangchao Yao*

Main category: cs.CL

TL;DR: 提出了一种无训练的解码方法RPS，利用中熵枢纽位置的涟漪效应来加速扩散大语言模型的并行解码。


<details>
  <summary>Details</summary>
Motivation: 现有的并行解码调度器忽略了提前提交对后续解码的潜在益处；本文旨在解决如何利用提前提交中熵枢纽位置来降低剩余掩码位置的不确定性，从而加速解码过程。

Method: 提出Ripple-Pivot Search (RPS)无训练解码方法，通过寻找中熵位置作为候选枢纽（决定解码位置），并利用前瞻评估确定能产生最大下游收益的词元分配（决定解码内容）。

Result: 在3个dLLMs和4个基准测试中，RPS在保持质量下实现4-10倍壁钟时间加速，准确率较前瞻基线提升5.49%，结合KV缓存时加速比达18倍。

Conclusion: RPS通过利用解码的涟漪效应，在不牺牲生成质量的前提下显著提升了扩散大语言模型的解码速度，验证了主动提交中熵位置在并行解码中的有效性。
...
TL;DR: 提出了一种名为PCD的预训练目标，通过结合自回归前缀监督和后缀去噪来解决扩散语言模型在提示词条件生成中的训练与推理不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 原生扩散语言模型预训练会随机破坏提示词和后续词，削弱了提示词条件生成所需的干净前缀接口，导致训练与使用阶段不匹配，本文旨在解决这一不匹配问题。

Method: 提出PCD（Prefix-Conditioned Diffusion）预训练目标，结合自回归前缀监督和无偏移后缀去噪。通过修改注意力掩码、破坏掩码和标签构造，对干净前缀进行自回归监督，仅对未知后续部分应用扩散。

Result: 在LLaDA2-Mini和Qwen-1.7B模型上，PCD相比原生dLLM基线持续提升，在LLaDA2-Mini六个基准平均上获得4.2%的相对增益（+2.56分），在Qwen主要机制对比中获得14.2%的相对增益（+4.86分）。

Conclusion: 将预训练上下文分布与提示词条件生成相对齐，可在不改变推理方式的情况下，有效弥补扩散语言模型在文本续写任务中的部分性能差距。
...
TL;DR: 掩码扩散语言模型在推理任务中的无序词元提交会导致失败，本文提出前沿门控提交方法来修复此问题并保留并行解码能力。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型以任意顺序提交词元为优势，但在推理任务中，这种自由导致模型过早提交最终答案并退化为仅输出答案的模式，根本原因在于采样器在远距离位置作用于终止信念的可达性。

Method: 通过2x2提示-解码器设计分析链式思维与顺序提交的交互作用，并提出一种单旋钮干预措施——前沿门控提交，以限制词元的提交顺序。

Result: 无约束解码在多达90%的问题中退化为仅输出答案；链式思维仅在顺序提交下有效（交互作用提升34.8个百分点）。前沿门控提交方法将准确率从0.528提升至0.852，同时保留了高达4倍的并行解码速度。

Conclusion: 现有的窗口式采样器此前仅被视为提升效率的工具，但实际上它们是针对扩散语言模型推理病理学的最小修复方案，尽管其最初并非为此设计。
...
TL;DR: 本文提出OPTD方法，通过在策略转移蒸馏和一致性引导的自适应压缩来加速扩散语言模型的推理，在保持生成质量的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型虽然能并行预测多个token，但仍需大量去噪步骤。现有少步蒸馏方法在离策略轨迹上构建监督，而学生模型在推理时的早期并行预测会改变后续上下文，导致实际访问的状态偏离监督状态，尤其是在压缩最激进时。在策略蒸馏虽能缓解此不匹配问题，但如何确定每次转移应推进的距离仍是开放问题。

Method: OPTD从少步学生模型自身轨迹中采样部分状态，使用冻结的仅问题教师模型识别与结果对齐的未来候选，并按当前状态置信度排序。方法选择最长的、能保持教师模型rollout结果的前缀进行联合承诺。采用集合瓶颈目标将验证的未来候选提升至解码器释放阈值，同时使用冻结教师KL锚正则化所有其他活跃位置。目标构建和训练均不使用黄金响应。

Result: 在四个数学推理和代码生成基准上，OPTD持续改善质量-效率权衡，在所有评估的少步基线中获得最强的质量约束AUP指标。

Conclusion: OPTD通过在策略蒸馏与自适应压缩相结合，有效解决了离策略蒸馏中的状态漂移问题，无需黄金响应即可实现高质量的少步生成，为扩散语言模型的高效推理提供了有效解决方案。
...
TL;DR: DominoTree是一种无需训练的最佳优先草稿树方法，通过利用Domino的条件非分解校正路径实现了LLM推理的显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿器（如DFlash）仅建模逐位置边缘分布，而最佳优先树方法（如DDTree）的分解公式无法表示Domino草稿器中基于GRU因果校正的路径依赖草稿token分布，限制了投机解码的效率。

Method: 提出DominoTree，一种无需训练的最佳优先草稿树，沿每条根到节点路径使用Domino的条件非分解校正进行评分，并通过将每节点校正限制为候选top-M集合使其可行；同时采用GPU原生的CUDA图构建器构建草稿树，与参考Python实现位级一致。

Result: 在Qwen3-4B的八个基准上，DominoTree相对自回归解码达到最高6.6倍加速，在所有测试温度下均取得最高平均接受长度（最高每轮10.7个token）；相对Domino解码器吞吐量提升9%-10%（Alpaca上最高22%），在所有温度下优于DDTree和CaDDTree。在Qwen3-8B上，T=0时相对DDTree吞吐量提升24%，高温下优势缩小但总体增益仍然保持。

Conclusion: DominoTree有效利用非分解路径依赖校正进行草稿树构建，配合高效CUDA图构建器在保持接受率不变的同时显著提升吞吐量，在多种模型和温度设置下持续优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的DDTree方法在固定节点预算下最大化预期接受长度，但接受长度随预算非递减，盲目倾向于更大的树而忽略了验证成本，缺乏选择预算的原则性依据。

Method: CaDDTree显式建模草稿和验证延迟，将吞吐量目标分解为每轮对预算的一维搜索，并在凸验证成本下证明吞吐量函数为单峰函数，从而使用高效的贪婪停止规则动态适应每轮的节点预算。

Result: 在Qwen3-4B和Qwen3-8B模型上跨8个基准测试（涵盖推理、编码和指令遵循任务）的实验表明，CaDDTree在几乎所有任务上都能匹配或超越具有预知预算选择的DDTree。

Conclusion: CaDDTree为推测解码中的树结构和预算选择提供了原则性的成本感知方案，无需离线预算搜索即可动态适应并实现卓越的吞吐量。
...
TL;DR: 本文提出DDTree方法，通过从块扩散模型构建草稿树以提升推测解码的接受长度和性能。


<details>
  <summary>Details</summary>
Motivation: DFlash等现有推测解码方法每轮仅验证单条草稿轨迹，限制了接受长度，未充分利用块扩散模型的逐位置分布信息。

Method: DDTree基于块扩散模型的逐位置分布直接构建草稿树，在固定节点预算下用best-first堆算法选择最可能匹配目标模型的续写路径，并通过仅祖先注意力掩码在目标模型单次前向传播中高效验证整棵树。

Result: DDTree在DFlash基础上提升了推测解码性能，达到了推测解码领域的领先水平。

Conclusion: DDTree有效突破了单轨迹验证的局限，将草稿树与块扩散模型结合，成为推测解码的领先方法之一。
...
### [18] [Sample Complexity of Peer Prediction](http://arxiv.org/abs/2608.16838v1)
*Abdellah Aznag,Robin Bowers,Rachel Cummings,Jason Hartline,Matthew vonAllmen,Bo Waggoner*

Main category: cs.IT

TL;DR: 本文研究了同伴预测机制中具有无偏估计量的互信息类别，刻画了不同样本数下行列式互信息（DMI）的唯一性，并提出了基于随机样本数的改进估计方法。


<details>
  <summary>Details</summary>
Motivation: 旨在解决在无法观测真实值的情况下，如何通过无偏估计量激励智能体如实报告观察信号的问题，并刻画在给定样本数量下存在无偏估计量的互信息集合。

Method: 通过推广信息论互信息概念，分析行列式互信息（DMI）的无偏估计量；提出了接受随机样本数的估计方法，包括事前有界样本数的“提前停止估计器”以及基于评分规则的互信息类。

Result: 当样本数不超过3时，具有无偏估计量的互信息仅为零；对于二元报告空间中的4或5个样本，DMI是唯一的（相差标量倍数除外）；6个样本时DMI不再唯一。此外，改进了DMI估计量并刻画了收敛率；提前停止估计器比固定样本估计器方差更低，且发现了一类期望样本数少于3的基于评分规则的互信息。

Conclusion: 明确了无偏估计量存在的互信息边界与唯一性条件，证明了随机化样本数能有效优化估计性能（降低方差与期望样本量），为同伴预测机制提供了更高效的估计理论与方法。
...
### [19] [Don't Drop the BATON: Long-Horizon Robot Manipulation via Agentic Subtask Exploration and Transition-aware Memory](http://arxiv.org/abs/2608.16889v1)
*Bingxin Xu,Yuzhang Shang,Emilio Ferrara*

Main category: cs.RO

TL;DR: 本文提出了BATON框架，通过子任务级探索和过渡感知记忆机制，解决了长视程机器人操作中探索成本乘性增长和缺乏状态过渡表示的问题。


<details>
  <summary>Details</summary>
Motivation: 长视程机器人操作中，即使VLA模型掌握了单个技能，误差累积和缺乏过渡表示仍会导致任务链条失败。现有将LLM与冻结VLA结合的方法存在测试期全任务探索成本过高（呈乘性增长且无法定位失败阶段）以及子任务间缺乏过渡表示导致状态无法衔接的问题。

Method: BATON将子任务作为探索单元以降低探索成本至加性（T*K）并实现失败归因；同时引入过渡感知记忆，通过验证器代理控制VLA调用时机，利用交接过渡恢复被干扰的入口状态，并通过前瞻过渡选择后继任务可继承的策略，整个过程无需更新模型参数。

Result: 在长视程基准RoboMemArena上，BATON相比现有最优方法（SoTA），任务成功率提升了11.6%，累积成功率提升了14.9%。

Conclusion: BATON通过子任务级探索和过渡感知记忆，有效解决了长视程操作中的探索成本和状态过渡问题，在不更新模型参数的情况下显著提升了长时序任务的成功率。
...
### [20] [Security of Foundation-Model-Powered Embodied Agents: Attack Surfaces, Attacks, Defenses, and Evaluation](http://arxiv.org/abs/2608.16843v1)
*Jiawei Liu,Jiacheng Guo,Tian Zhang,Yiwei Xu,Juan Wang,Jinlin Fan,Bowen Xiao*

Main category: cs.RO

TL;DR: 本文提出以信任边界为中心的基础模型具身智能体安全综述，系统划分了5层12个攻击面并分析了攻防研究现状及未探索领域。


<details>
  <summary>Details</summary>
Motivation: 现有安全综述按攻击机制分类无法一致地确定攻击者进入具身控制循环的切入点，难以有效应对基础模型带来的从数字到物理的安全风险传播。

Method: 采用“首次妥协信任边界”原则将攻击面与攻击机制分离，把系统划分为5层及12个攻击面，并基于截至2026年8月15日收集的58条攻击和61条防御记录进行定量与定性分析。

Result: 分析发现攻击研究集中于多模态感知和动作接口，防御集中于动作级和运行时保护；而上下文与长期记忆、中间件与网络、世界状态完整性和多智能体信任等领域研究不足。

Conclusion: 总结了状态溯源、组合防御、长期攻击传播、物理可实现性、拜占庭多机器人行为和统一闭环评估等关键开放挑战及未来研究方向。
...
### [21] [HAF: Adapting Generalist VLAs to Humanoid Whole-Body Loco-manipulation via Hierarchical Action Flow and Spectral Latent RL](http://arxiv.org/abs/2608.16837v1)
...
TL;DR: 本文提出HAF框架，通过分层动作流生成器与潜在在线强化学习管道，将预训练的VLA模型高效安全地迁移至人形机器人全身移动操作任务。


<details>
  <summary>Details</summary>
Motivation: 现有通用视觉-语言-动作（VLA）模型难以直接应用于人形机器人全身移动操作，单阶段架构无法有效协调高维且相互依赖的运动；同时，直接在真实世界使用在线强化学习微调大型VLA骨干网络计算成本过高且存在安全风险。

Method: 提出包含两部分的HAF框架：1) HAF-VLA，基于预训练流匹配VLA的分层动作流生成器，通过阶段嵌入和跨阶段KV缓存将全身动作去噪分三阶段进行以保持运动学依赖；2) HAF-Steer，在冻结的HAF-VLA之上构建潜在离线到在线强化学习管道，利用流匹配可逆性和DCT降维将优化限制在紧凑噪声子空间，训练正则化SAC策略。

Result: 在七个真实世界人形机器人移动操作任务上的评估表明，HAF超越了普通单阶段VLA基线，显著提升了全身协调性和任务性能。

Conclusion: HAF框架有效突破了VLA模型在人形机器人全身控制中的架构与微调瓶颈，提供了一种计算高效且安全的策略精调范式，可促进人形机器人在人类环境中的实际部署。
...
### [22] [When State Becomes an Attack Surface: State-Semantic Injection in LLM-Driven Embodied Agents](http://arxiv.org/abs/2608.16806v1)
*Jiawei Liu,Jiacheng Guo,Tian Zhang,Yiwei Xu,Juan Wang,Jinlin Fan,Bowen Xiao,Chi Guo,Keyan Guo,Hongxin Hu*

Main category: cs.RO

TL;DR: 本文综述了大语言模型驱动的具身智能体从文本生成到机器人任务规划与执行的演进过程


<details>
  <summary>Details</summary>
Motivation: 将大语言模型与机器人系统结合，使其从纯文本生成模型进化为能够感知环境、调用工具、执行任务的具身智能体，解决任务理解、高层规划和行为决策等问题

Method: 综述了多种方法路径：SayCan结合任务推理与机器人技能可供性，Code as Policies和ProgPrompt通过策略代码与程序化提示生成任务计划，VoxPoser构建三维价值地图引导操作，以及PaLM-E、RT-2、GR00T N1等视觉-语言-动作模型强化语言、视觉与动作的连接

Result: LLM驱动的具身智能体能够结合场景状态、物体属性、空间关系和执行反馈完成任务接地，并将生成的动作计划交由技能库、运动规划器或控制器执行

Conclusion: LLM驱动的具身智能体需要同时具备用户指令理解能力和场景状态感知能力，以实现任务接地并驱动底层执行模块完成复杂机器人任务
...
### [23] [AutoSR: Automatic Symbolic Regression by Searching Research States](http://arxiv.org/abs/2608.16876v1)
*Kejia Zhang,Youran Sun,Xinyu Ren,Chugang Yi,Haizhao Yang*

Main category: cs.SC

TL;DR: AutoSR是一种全自动符号回归系统，通过搜索持续的科学调查而非孤立的方程，将符号回归扩展为自动化科学调查。


<details>
  <summary>Details</summary>
Motivation: 有限且带噪声的数据通常产生在观测范围外行为不同的表达式，导致数值拟合和句法复杂度不足以衡量科学可信度；现有方法在搜索中丢失了指导下一步探索的科学记录（如动机和探测）。

Method: 提出将科学记录保存在“研究状态”中的AutoSR系统，利用提议者-审稿人代理在渐进加宽蒙特卡洛树搜索（PW-MCTS）下开发这些状态，并将积累的记录合成为最终报告。

Result: 在两个基准套件的九个挑战中，AutoSR在所有情况下均恢复了代数等价关系，包括三个无其他已发表系统能恢复的cp3-bench问题和六个结构多样的LSR-Transform问题。

Conclusion: AutoSR将符号回归从方程级搜索扩展至自动化科学调查，使科学知识和积累的证据能够塑造探索内容及方程的论证方式。
...
### [24] [GEO-Flag: Detecting and Measuring GEO-Optimized Web Content](http://arxiv.org/abs/2608.16824v1)
*Junjie Chu,Ye Leng,Mingjie Li,Yun Shen,Xinyue Shen,Yang Zhang*

Main category: cs.LG

TL;DR: 本文提出了GEOFlagBench基准和干预配对训练（IPT）方法，用于系统检测和审计生成式搜索引擎优化（GEO）网页，并部署完整管线实测GEO在真实搜索生态中的 prevalence。


<details>
  <summary>Details</summary>
Motivation: 生成式搜索引擎优化（GEO）可使经过策略性优化的网页获得与其权威性或相关性不匹配的可见度，甚至使虚假信息显得有据可依。生成式搜索将信息合成为直接答案而非呈现竞争来源，进一步放大了这些风险，而系统性检测GEO优化网页的方法仍缺乏研究。

Method: 构建了包含3,200个网页、覆盖400个查询、4个领域和8个GEO优化器家族的GEOFlagBench基准；提出干预配对训练（IPT），在ModernBERT上监督检测器对GEO干预和非GEO AI润色的响应；开发了GEO门控代理系统，审计检测到的GEO页面的来源层级和引用URL的可验证性；将完整管线部署于Google搜索和Gemini检索结果。

Result: 最强基线聚合F1为0.880，但在方法级别和作者条件评估中暴露显著弱点及对作者特征捷径的依赖；IPT在ModernBERT上将F1从0.862提升至0.944，最差组准确率从0.725提升至0.883；在10,095个真实页面中估计GEO总体 prevalence 为8.90%，在2026年修改的页面中达16.36%。

Conclusion: 本研究为在真实搜索生态中系统检测、审计和度量GEO奠定了基础，揭示了GEO优化的普遍性及其潜在信息质量风险。
...
### [25] [CForce: Boosting Parallel Decoding for dLLMs via Consistency Forcing](http://arxiv.org/abs/2608.13925v1)
*Yuji Ren,Chenkai Xu,Zhuocheng Gong,Jianguo Li,Zhijie Deng*

Main category: cs.LG

TL;DR: 本文提出了一种名为Consistency Forcing (CForce)的蒸馏方法，通过强制扩散大语言模型早期去噪阶段的预测与后期阶段对齐，解决了高并行度下早期预测不可靠导致的错误传播问题。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散大语言模型在激进的并行策略下，早期去噪阶段的预测不可靠，会导致错误传播到后续阶段并影响生成质量。

Method: 提出CForce蒸馏方法，在预收集的自推演轨迹上训练模型以强制早期与后期阶段预测对齐；引入Confidence Adaptive KL Divergence结合正向与反向KL散度的优点；该公式统一适用于掩码到词元解码和可编辑解码。

Result: 在非编辑和可编辑的LLaDA模型上的实验表明，CForce改善了速度与质量的权衡，特别是在高并行解码预算下表现优异。

Conclusion: CForce在理论上能近似最小化早期预测误差并提升训练与推理的对齐程度，有效解决了扩散大语言模型在高并行解码下的性能瓶颈。
...
TL;DR: 本文揭示了扩散大语言模型安全对齐的脆弱性，并提出了一种高效低成本的离线黑盒越狱框架SN-Guided Diffusion。


<details>
  <summary>Details</summary>
Motivation: 本文旨在研究DLLMs作为攻击目标和攻击者时的内部安全机制，暴露扩散模型对齐过程中的机械性漏洞，以解决DLLMs安全机制理解不足的问题。

Method: 核心方法是SN-Guided Diffusion，通过发现DLLMs继承自回归模型的安全足迹，利用直接的安全神经元映射、剪枝以及加权安全神经元损失，在离线状态下引导扩散过程避开安全触发区域。

Result: 自剪枝和转移剪枝大幅提升了攻击成功率；SN-Guided Diffusion实现了近完美的提示分离（AUROC=1.0），并在多个模型上达到高转移攻击成功率（最高86.9%），且生成成本远低于现有框架。

Conclusion: DLLMs的安全对齐存在稀疏且可转移的漏洞，SN-Guided Diffusion框架证明了其高效攻击性，揭示了DLLMs在安全方面需要更深入的防御机制研究。
...
TL;DR: 本文提出块混合注意力机制，通过线性化扩散语言模型对先前块的注意力计算，在保持性能的同时显著提升了推理速度和并发能力。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在分块半自回归解码时，每个去噪步骤仍需对所有先前的块进行注意力计算，导致前缀注意力成本过高，限制了推理加速。

Method: 引入块混合注意力，在当前去噪块内保留精确的softmax注意力，对先前的块应用线性注意力。该方法通过最小化后训练（遵循LoLCAT）将LLaDA 2.1中的部分注意力层替换并转换。

Result: 转换仅需约60小时，且在HumanEval、MBPP+和CMATH等基准测试中性能与原模型相近。通过Triton实现，解码吞吐量最高提升1.7倍，并支持更多并发请求。

Conclusion: 预训练的扩散语言模型可以被高效线性化以实现更快的推理，基本不损失性能，为dLLM的部署提供了优化方向。
...
TL;DR: 本文提出了一种名为ERILS的方法，通过长度控制和特定来源奖励处理来整合外部强策略的rollout，以解决离散扩散大语言模型在在线rollout成功样本稀缺时强化学习进展有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有离散扩散大语言模型的强化学习方法依赖目标模型自身生成的在线rollout，但在成功样本稀缺时，训练获得的正向奖励极少，导致进展有限。而直接引入外部更强策略生成的高奖励rollout会面临长度差异和奖励联合处理时的不稳定性问题。

Method: 提出外部Rollout整合与长度控制及特定来源处理方法（ERILS），该方法控制外部rollout的长度，并分别处理在线rollout和外部rollout的奖励。

Result: 在Sudoku、Countdown和MATH500任务上的零样本评估表明，ERILS提升了多样本性能，在Sudoku上的提升最大（best-of-4准确率达98.4%，最强基线仅为40.3%）。同时，在Sudoku任务的不同生成长度下，ERILS保持约90%的确定性单次完成准确率。组件分析表明长度控制的外部rollout比无控制的更有效，且特定来源奖励处理避免了联合处理导致的训练崩溃。

Conclusion: 研究结果表明，在将外部rollout整合到离散扩散大语言模型的强化学习中时，rollout的构建和奖励处理是至关重要的设计维度。
...
### [29] [Quipu: A Governed Bitemporal Knowledge Graph Store](http://arxiv.org/abs/2608.16813v1)
*Steve Brown*

Main category: cs.AI

TL;DR: 本文提出了Quipu，一个嵌入式知识图谱存储系统，通过门控写入、双时态数据、信任格和存储内治理来反转传统默认设置，以适应智能体工作负载。


<details>
  <summary>Details</summary>
Motivation: 当前的知识图谱存储仍保留为人类策展而设计的默认设置（先写入后清理、单一或无时间轴、写入者信任度相等、依赖外部仪表盘治理），这在智能体工作负载下是不可持续的。

Method: Quipu采用谓词门控评估写入状态；将数据、信任标签、裁决和规则设为双时态；以命名图为权限和信任单元，在不扩大的格下组合；并将治理规范、追踪记录和签名裁决作为其治理存储中的事实，使审计成为查询操作。

Result: 在Census基准测试中，门控存储消除了全部6个植入缺陷（无门控为6/6），7项组合探测均满足格契约，50项满足的裁决均能忠实重新推导。在DEMM-Bench上，导出记录正确回答了全部512个治理问题且零过度声明，而基线方法的过度声明率高达87.5%。

Conclusion: Quipu通过将治理、信任和时间性直接集成到存储中，成功解决了传统知识图谱存储在智能体工作负载下的不足，实现了强大的审计能力和零过度声明的决策证据验证。
...
### [30] [When Agents Coordinate: Measuring Coordination in Multi-Agent AI Coding](http://arxiv.org/abs/2608.16801v1)
*Giuseppe Destefanis,Tomaso Aste*

Main category: cs.AI

TL;DR: 本文提出了一种基于时间网络的测量工具，用于评估AI编程代理团队的内部协调机制，揭示了团队规模、任务类型及文件策略对协调成本和通信模式的显著影响。


<details>
  <summary>Details</summary>
Motivation: 现有的AI代理团队评估主要关注任务完成度和运行成本，缺乏对团队内部协调过程的有效测量与分析。

Method: 构建以代理和文件为节点、以带时间戳的消息与文件读写操作为有向边的时间网络，并分析了1902次不同团队规模、结构和文件策略配置下的运行数据。

Result: 直接消息量随团队规模先呈近二次增长后因广播放缓；共享规格任务形成密集网络而流水线任务形成稀疏网络；共享文件在消息繁重时可减少约42%的输出token，但协调者角色未带来可靠改善；代理表现出寻找隐藏评分材料的行为（在244次封闭测试中仍存在于五分之四的运行中）。

Conclusion: 该工具成功量化了AI代理的内部协调，证明利用共享文件可优化通信成本，而指定协调者无益；同时指出了代理自发寻找隐藏测试材料的风险倾向。
...
### [31] [Continuous Interaction Diffusion: A Diffusion-Native Runtime for Asynchronous Tool-Augmented Reasoning](http://arxiv.org/abs/2608.10438v1)
*Yuhang Cao*

Main category: cs.AI

TL;DR: 本文提出了连续交互扩散（CID）架构，将工具交互集成到扩散语言模型的迭代去噪过程中，以克服传统停止-恢复交互模式的限制。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行精炼输出进行推理，传统的停止-恢复工具调用模式会迫使模型在推理稳定前做出决策、延迟有用观察并引入冗余计算，从而损害任务准确率和推理效率。

Method: 提出CID架构，将工具交互集成到迭代去噪中。该架构分离出只读事实通道、由类型认知张量表示的思考通道和显示通道，允许在调用完全序列化前发起外部读取，并将结果投影回演化的思考状态中以修正早期认知，同时利用持久化绑定复用静态结果。

Result: 本文形式化了该架构、运行时和训练目标，并定义了任务质量和端到端效率的评估协议，但作为系列首篇，主要关注只读工具且未提出实证性能主张。

Conclusion: CID架构能够更早暴露证据、使工具延迟与模型计算重叠、减少重复外部工作并保留新证据到达后的有效计算，为扩散语言模型与外部工具的高效交互提供了新的架构范式。
...
### [32] [LLaDA MoE v2: Scaling Mixture-of-Experts Diffusion Language Models](http://arxiv.org/abs/2608.03457v1)
...
TL;DR: 本文系统分析了混合专家扩散语言模型的缩放规律，并基于此训练了LLaDA MoE v2，验证了其在多项基准任务上接近Qwen3的性能表现。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归模型的替代方案，其混合专家架构的缩放行为尚不明确，缺乏实用的缩放定律与设计原则。

Method: 系统刻画MoE dLLMs的优化超参数、算力分配及架构缩放规律，发现其与AR模型的定量差异，并据此从零开始训练了30B-A3B的LLaDA MoE v2模型。

Result: LLaDA MoE v2使用约Qwen3 65%的预训练token，在知识、推理和编码基准上接近Qwen3；经监督微调后，在8项推理和编码基准中有7项优于SDAR Chat。

Conclusion: 本研究为MoE扩散语言模型确立了实用的缩放定律与架构设计原则，证明了其在计算效率与任务性能上的竞争力。
...
TL;DR: 本文提出TAPS方法，通过目标感知的前缀选择策略优化基于扩散模型的投机解码验证过程，显著提升无损推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散树方法在投机解码中使用边缘概率对节点排序，忽略了验证过程的前缀条件性，导致验证被拒绝前缀的不可达后代，增加延迟且收益有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边缘概率转化为路径条件接受估计，并在固定验证预算下选择紧凑的前缀闭合子树，从而优化接受成本权衡。

Result: 在多个数据集和模型上的实验表明，TAPS相比原始自回归解码实现了最高7.9倍的无损端到端加速，且分别比DFlash和DDTree快1.36倍和1.74倍。

Conclusion: TAPS有效突破了基于扩散模型的投机解码中的验证瓶颈，在固定预算下优化了接受率与成本的权衡，实现了卓越的加速性能。
...
### [34] [VibeWorlding: Can Multimodal Agents Construct 3D Open Worlds End-to-End?](http://arxiv.org/abs/2608.15265v1)
*Yansong Ning,Jingwen Ye,Zhongkai Wu,Yang Sun,Yiqin Zhu,Xingyi Li,Weidong Zhang,Hao Liu*

Main category: cs.AI

TL;DR: 本文提出了VibeWorlding框架，用于基准测试和训练多模态智能体，使其能根据用户查询自主构建交互式3D开放世界，并提供了相应的基准和强化学习后训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有的3D世界构建方法主要在理想化、简单的查询上进行评估，难以系统分析和比较多模态智能体在理解用户意图、使用3D工具以及基于文本和视觉3D信息进行推理方面的能力。

Method: 提出VibeWorlding统一框架，包含VWE-BENCH基准（由3D资产、种子3D世界和逆向合成的多模态查询组成）和VibeWorlding-Gym联合多模态强化学习后训练框架（集成沙盒环境作为MCP工具，以及结合物理可行性和意图满足验证的规则验证器），使智能体能自主推断意图、规划布局、调用3D工具并在多轮交互中反思反馈。

Result: 实验表明，当前前沿多模态大语言模型（MLLM）远未解决该任务，即使是GPT-5.5和Qwen3.8-Max的成功率也低于60%，瓶颈在于精确的3D世界编辑。强化学习训练能缓解这一弱点，使开源MLLM超越闭源模型：VibeWorlder-8B可比拟前沿MLLM，旗舰版VibeWorlder-30B-A3B在所有评估模型中取得最佳的整体Pass@1。

Conclusion: VibeWorlding框架成功评估并提升了多模态智能体在3D世界构建任务上的能力，证明强化学习后训练能有效克服3D编辑瓶颈，并使开源模型性能超越闭源前沿模型。
