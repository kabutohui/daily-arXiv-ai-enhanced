...
### [1] [HAT-4D: Lifting Monocular Video for 4D Multi-Object Interactions via Human-Agent Collaboration](http://arxiv.org/abs/2606.28215v1)
...
TL;DR: 提出HAT-4D框架，结合VLM与人在回路机制从单目视频重建多物体4D交互，并构建相关基准与评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有单目4D重建方法侧重孤立物体，在多物体交互中因严重遮挡和复杂动力学而失效，限制了具身AI的数据扩展。

Method: 提出HAT-4D智能体框架，整合视觉语言模型（VLM）与多层级人在回路反馈机制，解决3D生成与4D传播中的深度歧义与交互遮挡，实现单视频多物体3D几何、时序动态与物理交互的重建；并构建MVOIK-4D开放世界基准及关注物理合理性与时序一致性的多维评估协议。

Result: 模型在多数指标上达到SOTA且语义对齐表现良好；消融实验证实人工反馈可提升交互重建质量；生成的数据用于微调能有效提升基线模型性能。

Conclusion: HAT-4D作为一种可扩展数据引擎，无需多相机设备即可从单目视频生成物理合理的多物体4D交互资产，为具身AI的数据缩放提供了有效途径。
...
### [2] [Toward Robust In-Context Segmentation via Concept Guidance](http://arxiv.org/abs/2606.28149v1)
*Zhigang Chen,Xiawu Zheng,Rongrong Ji*

Main category: cs.CV

TL;DR: 本文提出概念引导的上下文分割方法（CG-ICS），通过提取高级语义概念替代低级视觉匹配，显著提升了模型的准确性与系统鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有上下文分割（ICS）研究忽略了系统鲁棒性问题，即面对同一查询图像、使用不同参考图像时，模型难以产生稳定的分割结果，且过度依赖低级视觉匹配。

Method: 提出CG-ICS范式：1. 概念推理模块：利用MLLM生成候选概念，结合SAM3评分函数与树搜索优化筛选可靠的文本概念；2. 并行视觉样例路径：提供查询端的空间定位；3. 融合文本概念与视觉样例共同激活冻结的SAM3主干进行分割。

Result: 在标准ICS基准测试中，CG-ICS不仅实现了最先进的准确率，还大幅提升了系统鲁棒性，显著降低了不同参考图像输入下的结果方差。

Conclusion: 通过从参考图像中提取高级语义概念进行引导，而非仅依赖低级视觉匹配，可有效解决ICS系统的不稳定性，构建更可靠的上下文分割系统。
...
### [3] [AirGroundBench: Probing Spatial Intelligence in Multimodal Large Models under Heterogeneous Multi-View Embodied Collaboration](http://arxiv.org/abs/2606.28049v1)
*Haotian Li,Yida Wang,Leyuan Wang,Jinshan Lai,Keyang Wang,Zonghao Guo,Qiang Ma,Liuyu Xiang,Jianwei Hu,Zhaofeng He*

Main category: cs.CV

TL;DR: 本文提出了AirGroundBench，一个用于评估异构无人机-无人车协作中多视角空间智能的诊断基准，揭示了当前多模态大语言模型在跨视角对齐和几何一致性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单智能体单视角感知，缺乏对异构空地协作场景下多视角空间理解的系统评估，而该场景中存在的尺度不匹配、非对称遮挡和参考系不一致等问题亟待解决。

Method: 构建AirGroundBench基准，包含11个高保真仿真环境中的1021对同步空地观测数据，生成约6.2万个双视角VQA实例和115个闭环视觉-语言导航片段；涵盖空间感知、跨视角对齐、空间变换与推理及具身决策四个维度，并提供结构化空间标注以支持几何基础的评估。

Result: 对13个代表性多模态大语言模型的评估表明，模型在空间感知上表现较好，但在跨视角对齐和空间变换推理上存在显著瓶颈，且缺陷会传导至具身决策中；尽管双视角输入优于单视角，但与人类表现仍存在持续差距。

Conclusion: 几何一致性是当前具身多模态大语言模型的关键局限，未来需重点攻克跨视角对齐与变换推理能力，以提升异构多智能体协作中的空间理解与决策表现。
...
### [4] [HumanMoveVQA: Can Video MLLMs reason about human movement in videos?](http://arxiv.org/abs/2606.27999v1)
*Pulkit Gera,Faegheh Sardari,Asmar Nadeem,Valentina Bono,Padraig Boulton,Adrian Hilton,Armin Mustafa*

Main category: cs.CV

TL;DR: 本文提出了HumanMoveVQA基准，用于评估多模态大语言模型在全局轨迹和朝向推理方面的能力，揭示了当前专有模型的缺陷并验证了通过微调改进的可行性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型将复杂人体运动简化为粗糙的语义标签，且现有基准主要关注场景中心事件或局部关节运动，无法评估随时间变化的全局人体运动（轨迹和朝向变化）。

Method: 提出HumanMoveVQA基准，采用基于首帧锚定的世界坐标系保留平移和旋转信息；设计可扩展的多阶段流水线，将2D视频观测提升为世界一致的3D运动轨迹，生成涵盖七个推理类别的10000+结构化问答对。

Result: 评估表明最先进的专有模型在深度人体运动理解上存在严重的能力缺陷；但通过使用该基准的世界一致性监督微调开源基线模型，可实现显著的性能提升。
...
### [5] [Understanding How MLLMs Describe Artworks Using Token Activation Maps](http://arxiv.org/abs/2606.27947v1)
*Nicola Fanelli,Pasquale De Marinis,Raffaele Scaringi,Eva Cetinic,Gennaro Vessio,Giovanna Castellano*

Main category: cs.CV

TL;DR: 本文利用词元激活图（TAM）探究多模态大语言模型描述艺术作品时的视觉定位机制，发现视觉定位因词元语义而异，且模型在标题预测上比艺术家归属更易产生幻觉。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型能流畅描述艺术作品，但其视觉推理机制不透明，尚不清楚其生成声明是基于画布特定区域、无差别视觉信号还是文本先验。

Method: 使用词元激活图（TAM）为每个生成词元生成热力图，以隔离特定词元的视觉证据与先验上下文干扰；将其应用于跨时期与流派的画作，分析五类语义词元（视觉对象、风格描述、元数据、图像学词元、情感表达）的定位模式。

Result: 视觉定位随词元语义发生显著变化；模型在艺术家归属上的准确率高于标题预测，且标题预测中幻觉更频繁；此外对TAM与SAM3开放词汇分割进行了对比。

Conclusion: MLLM在艺术作品分析中的视觉定位高度依赖语义类型，且在标题识别上易产生幻觉，这为理解与改进MLLM的视觉推理提供了依据；研究已开源以确保可复现性。
...
### [6] [Video-MME-Logical: A Controlled Diagnostic Benchmark for Video Temporal-Logical Reasoning](http://arxiv.org/abs/2606.27828v1)
*Hohin Kwan,Hongyu Li,Ray Zhang,Manyuan Zhang,Xianghao Kong,Anyi Rao,Jiahao Xie,Si Liu*

Main category: cs.CV

TL;DR: 本文提出Video-MME-Logical基准，以隔离和评估多模态大语言模型的视频时序逻辑推理能力，揭示了模型与人类之间显著的推理差距。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型多局限于单帧静态识别，且现有视频基准常将时序逻辑推理与场景复杂度或静态识别混淆，亟需一个受控基准来隔离并纯粹地评估模型在动态视觉证据上的时序逻辑推理能力。

Method: 提出Video-MME-Logical基准，围绕状态追踪、顺序计数、时间排序、动态空间性和结构组合五种时序逻辑操作构建25个细粒度任务；通过控制时间跨度和推理复杂度实现难度可控的最终答案评估，并支持验证中间逻辑推理轨迹的中间状态诊断。

Result: 最先进的MLLMs在时序逻辑推理上与人类存在显著差距，且差距随时序逻辑复杂度增加而扩大；即使使用50万样本进行监督微调能提升性能，但仍不足以弥合此推理差距。

Conclusion: 现有监督微调方法不足以解决MLLMs的时序逻辑推理缺陷，Video-MME-Logical可作为可扩展的测试平台，用于未来深入分析和提升模型的时序逻辑推理能力。
...
### [7] [Learning from the Self-future: On-policy Self-distillation for dLLMs](http://arxiv.org/abs/2606.18195v2)
*Yifu Luo,Zeyu Chen,Haoyu Wang,Xinhao Hu,Yuxuan Zhang,Zhizhou Sha,Shiwei Liu*

Main category: cs.CL

TL;DR: 本文提出了首个专为扩散语言模型设计的同策略自蒸馏框架d-OPSD，通过后缀条件和步级监督解决了现有自回归方法的不兼容问题，并以极高样本效率提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有的同策略自蒸馏（OPSD）方法针对自回归模型设计，依赖从左到右前缀条件和token级监督，这与扩散语言模型的任意顺序生成机制根本冲突，无法直接应用于dLLM的后训练。

Method: 提出d-OPSD框架，包含两项核心改进：1）使用自生成答案作为后缀条件重构自教师，使学生模型从“自我未来经验”学习；2）将监督粒度从token级转变为步级，以对齐dLLM的迭代去噪过程。

Result: 在四个推理基准测试中，d-OPSD始终优于RLVR和SFT基线，且仅需RLVR约10%的优化步数即可实现更优性能，展现出卓越的样本效率。

Conclusion: d-OPSD为扩散语言模型的后训练提供了一条高效且极具前景的新路径，验证了针对非自回归生成范式定制自蒸馏策略的有效性。
...
### [8] [LESS Is More: Mutual-Stability Sampling for Diffusion Language Models](http://arxiv.org/abs/2606.16908v1)
*Amr Mohamed,Guokan Shang,Michalis Vazirgiannis*

Main category: cs.CL

TL;DR: 本文提出LESS自适应采样器，将词元提交视为在线停止问题，显著减少了扩散大语言模型的反向去噪步骤与推理延迟，同时提升了准确率。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型通常采用预定义固定数量的反向去噪步骤，导致计算浪费在已稳定的词元位置，且可能过早提交不稳定的词元，限制了其实际推理效率。

Method: 提出一种免训练且模型无关的自适应采样器LESS，基于联合稳定性规则实现互稳定性采样：仅当掩码位置的top-1预测置信度高、top-1词元在近期步骤中持续存在，且预测分布在top-K步间Jensen-Shannon散度下保持稳定时，才对该位置进行解掩码。

Result: 在Dream-7B等模型及7个基准测试上，LESS相比强自适应采样器提升了平均准确率，且比固定预算解码减少了72.1%的反向步骤，有效降低了前向评估次数、挂钟延迟和推理计算量。

Conclusion: 将词元承诺建模为在线停止问题可高效解决dLLMs的采样效率瓶颈，LESS为免训练、模型无关的高效推理提供了有效范式。
...
### [9] [Follow the Latent Roadmap: Navigating Revocable Decoding for Diffusion LLMs with Anchor Tokens](http://arxiv.org/abs/2606.16847v2)
*Yizhen Yao,Qinglin Zhu,Runcong Zhao,Xiangxiang Dai,Yanzheng Xiang,Yulan He,Lin Gui*

Main category: cs.CL

TL;DR: 提出免训练的ASRD框架，通过解耦上下文并利用锚点令牌引导生成与扰动验证，解决扩散大语言模型中的错误传播和局部错误强化问题，显著提升准确率与推理速度。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的现有可撤销解码策略在混合质量上下文中运行，会导致错误传播（新令牌吸收错误上下文信息）和局部错误强化（错误相互增强以逃避检测），从而面临解码速度与质量的权衡问题。

Method: 提出免训练框架ASRD，在嵌入空间中将解码上下文解耦为可信锚点令牌（通过时间一致性识别）和不确定候选令牌；利用动态锚点缓存引入两种机制：1）锚点引导生成，将熵加权锚点信号注入掩码位置以纠正注意力至可靠全局骨架；2）锚点扰动验证，对不确定候选施加正交扰动以破坏并重新掩码由脆弱局部共识驱动的错误。

Result: 在数学和编程基准测试中，ASRD优于近期重掩码基线，准确率最高提升6.4%，推理吞吐量最高加速7.2倍。

Conclusion: ASRD有效缓解了dLLM可撤销解码中的错误传播与局部错误强化问题，在无需额外训练的情况下同时提升了生成质量和推理效率。
...
TL;DR: 本文提出AGDO框架，通过利用注意力衍生的标记依赖关系来指导扩散大语言模型的去噪与优化，显著提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型的训练后方法多采用随机掩码策略，忽略了标记间的内在依赖关系，且研究发现对未掩码上下文注意力更强的标记具有更高的生成稳定性并在推理中起关键作用。

Method: 提出注意力引导去噪与优化框架（AGDO），基于注意力结构决定去噪顺序，并在监督微调和强化学习阶段强调注意力关键的标记，使训练和优化与注意力衍生的依赖关系对齐。

Result: 在数学和编程基准测试中，AGDO持续提升了推理性能，超越了现有最先进的扩散大语言模型训练后方法。

Conclusion: 将去噪过程及优化策略与注意力衍生的依赖关系相对齐，能有效挖掘标记间的内在联系，从而显著增强扩散大语言模型的推理能力。
...
TL;DR: 本文提出ADAS，一种免训练的重排序规则，通过软注意力折扣惩罚缓解掩码扩散模型并行解码中的预测耦合问题，显著提升低NFE性能。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型并行解码时，现有免训练采样器仅依据单Token分数决定揭示数量，忽略了被选中Token间的预测耦合与交互，导致并行解码脆弱且易出错。

Method: 提出ADAS（免训练重排序规则），保留基础采样器的停止规则，仅修改子集构建过程：当候选Token强烈关注已选中且预测仍不确定的位置时，贪婪地对其进行折扣惩罚，将注意力作为软边际惩罚而非硬兼容性约束。

Result: 在LLaDA-8B和Dream-7B模型及四个基准测试中，将ADAS引入现有采样器，在匹配去噪评估次数下，低NFE性能分别平均提升9.11和10.46个百分点，每次前向传播仅增加3.1%的运行时开销。

Conclusion: 基于软注意力折扣的重排序是一种简单且模块化的方法，能有效提升掩码扩散语言模型在高度并行解码下的生成质量。
...
TL;DR: 本文提出CaDDTree，一种通过联合选择树结构与节点预算来直接优化token吞吐量的成本感知扩散草稿树方法，无需离线预算搜索即可自适应调整预算。


<details>
  <summary>Details</summary>
Motivation: 现有块扩散草稿树方法（如DDTree）在固定预算下最大化预期接受长度，但接受长度随预算非递减，总是偏向更大的树而无视验证成本，导致缺乏节点预算选择的原则性依据。

Method: 提出CaDDTree方法，显式建模草稿与验证延迟，通过联合选择树结构和节点预算直接优化token吞吐量；将吞吐量目标分解为每轮对预算的一维搜索，证明在凸验证成本下吞吐量函数为单峰，从而实现高效的贪心停止规则，无需离线搜索即可逐轮自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型及涵盖推理、编码和指令跟随的8个基准测试上，CaDDTree在几乎所有任务上均匹配或超越了具有先验最佳（oracle）预算选择的DDTree。

Conclusion: CaDDTree为推测解码提供了原则性的成本感知预算选择机制，通过自适应优化实现了高效的推理吞吐量，消除了对离线预算调优的依赖。
...
TL;DR: 本文提出DDTree方法，通过从块扩散起草模型的逐位置分布构建起草树，解决了DFlash单轨迹验证限制接受长度的问题，提升了推测解码性能。


<details>
  <summary>Details</summary>
Motivation: 先进的块扩散推测解码方法DFlash每次仅验证单一的起草轨迹，限制了推测解码的接受长度，未能充分发挥并行验证的潜力。

Method: 提出DDTree方法，利用块扩散起草模型的逐位置分布构建起草树；在固定节点预算下，采用最佳优先堆算法根据起草模型输出的代理指标选择最可能匹配目标模型的续写；并使用仅祖先注意力掩码在单次目标模型前向传播中高效验证该树。

Result: DDTree克服了DFlash的单轨迹验证限制，其性能提升使DDTree跻身推测解码领先方法之列。

Conclusion: 通过从块扩散模型构建起草树并结合高效验证机制，DDTree有效提升了推测解码的接受长度和整体性能，成为当前领先的推测解码方法之一。
...
### [14] [Empowering GUI Agents via Autonomous Experience Exploration and Hindsight Experience Utilization for Task Planning](http://arxiv.org/abs/2606.27330v1)
*Tianyi Men,Zhuoran Jin,Pengfei Cao,Yubo Chen,Kang Liu,Jun Zhao*

Main category: cs.CL

TL;DR: 本文提出PEEU方法，通过自主探索环境与利用事后经验合成高层训练数据，提升小型多模态大语言模型的规划与跨站泛化能力，其7B模型表现优于32B模型。


<details>
  <summary>Details</summary>
Motivation: 小型开源多模态大语言模型（MLLM）在成本与隐私上具优势，但存在任务规划能力弱和跨网站泛化受限的问题。

Method: 提出规划经验探索与利用（PEEU）方法，通过自主环境探索发现经验，并利用事后经验合成严格对齐的高层训练数据；同时提出任务分解层次分析框架（TDHAF）以系统性分析跨任务粒度（低、中、高层）的组合泛化行为。

Result: 分析表明掌握低层原子技能不保证高层规划能力，而高层任务训练能带来更强的分布外（OOD）泛化；实验中基于PEEU的7B模型在真实基准上达30.6%准确率，超越Qwen2.5-VL-32B模型。
...
### [15] [GBC: Gradient-Based Connections for Optimizing Multi-Agent Systems](http://arxiv.org/abs/2606.28187v1)
*Xiaocheng Yang,Abdulrahman Alrabah,Dilek Hakkani-Tür,Gokhan Tur*

Main category: cs.MA

TL;DR: 本文提出基于梯度的连接（GBC）方法，通过在词元级别对多智能体系统进行细粒度归因与提示优化，解决现有系统难以定位错误源的问题。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的多智能体系统常因协调不当和缺乏细粒度信用分配而受限，现有粗粒度反馈难以精确定位导致错误的具体智能体或交互步骤。

Method: 将多智能体系统建模为计算图，引入基于梯度的连接权重在词元级别量化智能体输出对下游的影响；通过构建归因图并反向传播任务特定损失信号来识别错误源并进行针对性提示优化；同时提出基于前缀梯度计算的高效实现AgentChord。

Result: 在MultiWOZ和τ-bench数据集上的实验表明，GBC提升了多智能体性能，优于强力的单智能体和多智能体基线，且归因质量越高，优化效果越显著。

Conclusion: 基于梯度的细粒度归因方法能够精准识别多智能体系统中的错误源并实现针对性提示优化，有效提升系统整体性能。
...
### [16] [Govern the Repository, Not the Agent: Measuring Ecosystem-Level Risk in AI-Native Software](http://arxiv.org/abs/2606.28235v1)
*Daniel Russo*

Main category: cs.SE

TL;DR: 本文提出“集成摩擦”概念，发现其是代码库生态系统的固有属性而非单个智能体缺陷，AI原生软件需在生态系统层面进行评估与治理。


<details>
  <summary>Details</summary>
Motivation: 现有自主编码智能体的评估局限于隔离的单智能体基准测试，忽略了并发贡献在共享代码库中累积的集成问题；本文旨在探究该累积问题是源于个体智能体还是代码库生态系统。

Method: 提出“集成摩擦”概念量化并发贡献的集成成本；基于超93万个智能体生成的拉取请求，在控制贡献属性及代码库特征等变量后，测量摩擦变异中归属于代码库的比例，并对比人机贡献差异。

Result: 约50%的摩擦变异归属于代码库且控制变量后依然稳健；智能体贡献引发的代码库级摩擦集中度约为人类的两倍（组内相关系数0.30 vs 0.16），该差距在控制代码库规模、年龄等变量后依然显著。

Conclusion: 集成摩擦风险是生态系统的固有属性而非智能体缺陷；AI原生软件应从传统的单智能体评估转向生态系统层面的评估与治理。
...
### [17] [Towards Automating Scientific Review with Google's Paper Assistant Tool](http://arxiv.org/abs/2606.28277v1)
*Rajesh Jayaram,Drew Tyler,David Woodruff,Corinna Cortes,Yossi Matias,Vahab Mirrokni,Vincent Cohen-Addad*

Main category: cs.LG

TL;DR: 本文提出了科学评估中AI与人类协作的四级分类法，并介绍了基于推理扩展技术的智能体AI框架PAT，用于自动化深度科学评审与验证。


<details>
  <summary>Details</summary>
Motivation: AI加速科学发现导致论文数量激增，传统人类同行评审无法规模化应对，因此亟需部署AI来加速科学验证与评审过程本身。

Method: 提出AI与人类协作评审的四级分类法，并构建PAT（Paper Assistant Tool）智能体框架，该框架利用推理扩展技术处理完整稿件，以检查理论结果、验证实验、识别缺陷并提出改进建议。

Result: PAT在SPOT基准的数学错误检测上较零样本召回率提升了34%；在STOC和ICML会议的试点部署中，成功识别了关键错误并提供了实质性改进建议。

Conclusion: PAT等AI工具能通过提前纠错减轻评审人员认知负担，同时保留人类对评审结果的控制权，推动了AI辅助科学评估的实践与发展。
...
### [18] [Towards Value-Constrained Credit Assignment in Fully Delegated AI Cooperatives](http://arxiv.org/abs/2606.28217v1)
*Young Yoon,Jimin Kim,Soyeon Park*

Main category: cs.LG

TL;DR: 提出一种基于遍历学习的全委托AI合作社奖励分配框架，以解决异构价值约束下的贡献归因问题。


<details>
  <summary>Details</summary>
Motivation: 解决全委托AI合作社中人类委托人存在异构价值约束时的奖励分配问题，克服现有聚合中心化分布式学习（如FedAvg）存在的质量损失和归因粒度不足的缺陷。

Method: 在遍历学习（TL）基底上，提出价值条件化梯度过滤（根据委托人价值特征筛选更新）、在线边际贡献信号和累积收益结算机制；TL通过保留显式遍历和梯度路径实现去中心化反向传播。

Result: 相比于FedAvg风格的联邦学习，该框架仅对满足价值约束的更新进行信用分配，提供了更精细的归因基底，且避免了聚合中心化分布式学习的质量损失。

Conclusion: 该框架为数据估值、联邦贡献估计、个性化联邦学习和多元对齐提供了新思路，通过去中心化反向传播和精细梯度归因提升了异构价值约束下奖励分配的合理性与模型质量。
...
TL;DR: 提出了HERALD，一种针对块扩散语言模型的KV缓存卸载系统，通过减少选择计算量及重叠选择与去噪过程，实现了长上下文下的高吞吐量推理。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型的KV缓存随上下文线性增长限制了长上下文吞吐量；将KV缓存卸载至主机DRAM虽可缓解内存压力，但受限于PCIe带宽需稀疏召回，而在卸载场景下对完整KV缓存进行精确注意力计算以选择相关条目的开销过大。

Method: 提出HERALD系统，利用块dLLMs中相关KV条目在块内去噪步间一致性的特点，通过两种机制解决精确选择开销大的问题：将所需选择计算量降低至原先的1/块大小，并实现选择过程与去噪过程的重叠。

Result: 在5-10%的KV预算下实现近乎无损的准确率；与纯GPU推理相比，单块延迟最高降低1.59倍，吞吐量最高提升2.47倍，且加速比随上下文长度增加而增长。

Conclusion: HERALD有效解决了块扩散语言模型在长上下文推理中的KV缓存卸载难题，在极低KV预算下保持了高准确率并显著提升了系统吞吐量。
...
TL;DR: 本文提出Diffusion-Proof框架，首次将扩散大语言模型应用于形式化定理证明，以克服自回归模型的长期一致性差和误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型在形式化定理证明中因下一标记预测机制面临长期一致性差和误差累积的局限，而具备多标记块迭代去噪能力的扩散大语言模型在该领域的应用尚属空白。

Method: 提出Diffusion-Proof框架，包含两个模型：1) dLLM-Prover-7B，用于生成具有长期一致性的完整证明；2) dLLM-Corrector-7B，利用大块扩散和双向信息填充能力进行局部证明纠错。

Result: 相较于同等数据集训练的自回归基线，Diffusion-Proof在ProofNet-Test和MiniF2F-Test上分别取得1.61%和6.14%的绝对提升，并成功解决了一个DeepSeek-Prover-V2-7B未能解决的IMO问题。

Conclusion: 扩散大语言模型在形式化定理证明中展现出独特优势，能有效提升证明连贯性与纠错能力，为形式化数学推理开辟了新路径。
...
TL;DR: 本文提出了首个面向智能手机NPU的扩散大语言模型推理框架llada.cpp，通过软硬件协同设计显著降低了生成延迟。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)在移动端重复去噪计算量大，而利用移动端NPU加速面临三大挑战：令牌提交导致块内有效负载缩减、令牌修正导致KV缓存复用困难、有限的NPU可见地址空间导致高昂的内存重映射与数据传输开销。

Method: 提出NPU感知推理框架llada.cpp，包含三项核心技术：(1)多块投机解码，利用未来块的投机令牌填补当前块解码后期的缩减负载；(2)双路径渐进修正，通过CPU侧路径刷新不稳定令牌，避免阻塞NPU的密集计算；(3)交换优化内存运行时，压缩NPU地址布局并重叠数据暂存与NPU计算以减少开销。

Result: 在多种硬件平台与dLLM负载上评估，llada.cpp在支持前缀KV缓存复用的条件下，将LLaDA-8B的生成延迟相比CPU基线降低了17倍至42倍，且未损失生成质量。

Conclusion: llada.cpp有效解决了dLLM推理特性与移动端NPU执行特征的不匹配问题，实现了智能手机上高效的dLLM推理，为端侧低延迟大模型部署提供了可行方案。
...
TL;DR: 本文重新评估了掩码扩散语言模型中基于置信度的后验重掩码方法，发现其优势高度依赖于解码设置，且在标准设置下并无显著收益。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型无法修正已解掩码的词元，易受早期采样错误影响。近期提出的基于置信度的后验重掩码方法声称能以无需训练的方式解决此问题，但需验证其实际有效性与适用条件。

Method: 对代表性后验重掩码方法WINO进行实证重评估，分别在标准解码设置（较短块长度）和非贪心解码设置下，对比其与仅使用基于置信度解掩码方法的表现。

Result: 在标准解码设置下，WINO相比仅使用基于置信度解掩码无明显收益；在非贪心解码下，重掩码虽能缓解随机性引入的错误，却加剧了多样性崩溃问题。

Conclusion: 基于置信度的后验重掩码方法的收益高度依赖于具体的解码设置，这凸显了建立更全面评估框架以衡量此类方法的必要性。
...
### [23] [NormGuard: Reward-Preserving Norm Constraints in Flow-Matching Reinforcement Learning](http://arxiv.org/abs/2606.27771v1)
*Tianlin Pan,Lianyu Pang,Cheng Da,Huan Yang,Changqian Yu,Kun Gai,Wenhan Luo*

Main category: cs.LG

TL;DR: 本文针对RL微调流式生成器引起的速度范数膨胀和质量退化问题，提出一种训练时的铰链惩罚方法，在保持奖励对齐的同时显著提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 强化学习（RL）后训练虽能提升流式生成器的奖励对齐，但会引发每步速度范数膨胀（5%-15%）从而导致感知质量退化。推理时的重缩放无法修复该问题，且伴随灵敏度分析表明范数抑制不损失奖励信号，因此亟需训练阶段的干预策略。

Method: 提出一种铰链惩罚机制，当模型的速度范数超过参考范数时激活该惩罚项，并将其与任意基于速度的局部基础损失相加，从而在训练时显式抑制速度范数膨胀。

Result: 在两种基础模型、三种后训练方法和两种奖励代理上的实验表明，该方法一致地提升了MLLM评估的图像质量和法医真实性，保留了奖励分数，且在少步推理下增益被放大，该提升无法由早停解释。

Conclusion: 训练时的范数膨胀干预是解决RL后训练质量退化的有效策略，且不会牺牲奖励信号，为流式生成器的RL对齐提供了关键的训练范式指导。
...
### [24] [Drift Behavior in a Bounded-Confidence Opinion Model with Media Influence](http://arxiv.org/abs/2606.28318v1)
*Oliver Zheng,Mason A. Porter*

Main category: physics.soc-ph

TL;DR: 本文通过引入两个极化媒体源扩展了Deffuant-Weisbuch有界信任模型，以研究竞争媒体对集体舆论形成及观点簇漂移行为的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的有界信任模型（BCMs）未能充分刻画极化媒体环境对舆论动态的影响，本文旨在探究个体互动与媒体互动共同作用下集体意见的形成与演变机制。

Method: 在原始Deffuant-Weisbuch（DW）有界信任模型中引入一正一负两个媒体代理以模拟极化媒体环境，并结合数值模拟与解析分析来研究模型动力学。

Result: 扩展的DW模型表现出漂移行为，即大型观点簇会向某一媒体代理偏移；研究明确了漂移轨迹和速度对模型参数的依赖关系，并确定了促进或抑制漂移的条件。

Conclusion: 竞争媒体源会显著影响社会系统中的集体舆论形成，该研究为理解极化媒体环境下群体意见的演化及漂移机制提供了理论洞见。
...
### [25] [Agentic Hardware Design as Repository-Level Code Evolution](http://arxiv.org/abs/2606.28279v1)
*Cunxi Yu,Chenhui Deng,Nathaniel Pinckney,Brucek Khailany*

Main category: cs.AR

TL;DR: HORIZON是一个将硬件设计视为仓库级代码演化的自演化智能体框架，在多个基准测试中实现了全自动100%完成率。


<details>
  <summary>Details</summary>
Motivation: 旨在解决硬件设计自动化问题，将仓库级自演化从EDA软件系统扩展至硬件设计产物本身，以实现无需人工干预的设计演化。

Method: 将Markdown线束编译为包含领域知识、可执行评估器、接受谓词及git/运行时策略的项目包；通过免手动智能体循环在隔离的git工作树中进行演化，并利用仓库操作进行状态管理、追踪与重放。

Result: 在ChipBench、RTLLM、Verilog-Eval及九个CVDP类别上，全自动化智能体循环实现了100%的基准测试完成率。

Conclusion: 尽管在受控基准测试中实现了100%完成率，但硬件设计智能体AI尚未被完全解决；当前基准测试仅为更广泛芯片设计工程问题的代理，研究存在局限性并指出了未来的开放研究挑战。
...
### [27] [Bifocal Diffusion Language Models: Asymmetric Bidirectional Context for Parallel Generation](http://arxiv.org/abs/2606.27732v1)
...
TL;DR: 本文提出Bifocal dLLMs及其实例R2LM，通过结合因果注意力与轻量级反向Mamba SSM，解决了离散扩散语言模型中生成质量与KV缓存推理吞吐量无法兼顾的困境。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型面临架构设计困境：双向注意力生成质量高但与KV缓存不兼容，限制了批处理推理吞吐量；因果注意力支持高效KV缓存但丢失右侧上下文，严重降低生成质量。

Method: 提出基于非对称双向上下文的Bifocal dLLMs范式，并实例化为R2LM（Right-to-Left Mamba）。该方法结合标准因果注意力（提供精确左侧上下文并兼容KV缓存）与轻量级反向Mamba SSM边车（提供压缩的右侧上下文且不破坏缓存兼容性）。

Result: 在Qwen3-1.7B上的持续预训练实验表明，R2LM吞吐量比双向dLLM高2.4至12.9倍，比自回归基线快1.9至2.9倍，同时在多数基准测试上超越因果基线，且平均表现优于双向dLLM。

Conclusion: R2LM成功解决了dLLMs的架构设计困境，在保持高效KV缓存和并行解码带来的高吞吐量的同时，通过引入右侧上下文保障了生成质量。
...
### [28] [Agent-Native Immune System: Architecture, Taxonomy, and Engineering](http://arxiv.org/abs/2606.28270v1)
*Bo Shen,Lifeng Chang,Tianyuan Wei,Yunpeng Li,Feng Shi,Yichen Han,Peijie Gao,Shiyi Kuang,Xin Chang,Dehui Li*

Main category: cs.AI

TL;DR: 本文提出首个内生于智能体认知循环的生物启发式防御架构——智能体原生免疫系统（ANIS），以解决AI智能体运行时动态防御缺失的问题。


<details>
  <summary>Details</summary>
Motivation: 当前防御机制（如外围安全、训练时对齐）独立于智能体的推理循环，导致已对齐的智能体仍极易受到运行时劫持（如记忆投毒、工具链操纵、多智能体协议攻击），亟需内源性的动态防御机制。

Method: 提出ANIS架构，包含四大核心贡献：1）设计含屏障免疫（L1）的六层免疫塔（L0-L5）；2）建立智能体病毒与疫苗统一分类法，区分非参数化防御与参数化疫苗；3）提出元/自/自动驾驭三重机制，驱动持续免疫学习（CIL）以动态适应新威胁；4）理论界定模型对齐（静态宪法基础）与智能体免疫（动态执法）的区别。

Result: 成功构建了内生于智能体认知循环的防御架构，在理论和架构层面填补了从静态训练对齐到动态运行时防御的空白，并形式化了疫苗对新威胁的动态自适应机制。

Conclusion: 指出了该领域的开放性挑战，包括免疫协议标准化、新型评估指标（如衡量误报干预的自身免疫率）以及集体智能生态中病原体与疫苗的协同进化动力学。
...
TL;DR: 提出TAPS方法，通过目标感知的前缀选择优化扩散推测解码中的草稿树验证，显著提升无损加速比。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的推测解码方法依据边际概率对草稿树节点排序，忽略了验证过程的前缀条件性，导致可能验证被拒绝前缀的不可达后代，增加延迟且接受率提升有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边际概率转化为路径条件的接受度估计，并在固定验证预算下选择紧凑的前缀闭合子树，优化接受率与计算成本的权衡。

Result: 实验表明，TAPS相比自回归解码实现最高7.9倍无损端到端加速，性能分别优于SOTA方法DFlash和DDTree 1.36倍和1.74倍。

Conclusion: TAPS有效解决了扩散推测解码的验证瓶颈，通过前缀条件化的子树选择策略显著提升了推理效率，为无损大模型推理加速提供了有效方案。
...
### [30] [Verifiable Geometry Problem Solving: Solver-Driven Autoformalization and Theorem Proposing](http://arxiv.org/abs/2606.27926v1)
*Can Li,Ting Zhang,Junbo Zhao,Hua Huang*

Main category: cs.AI

TL;DR: 本文提出SD-GPS框架，通过将符号求解器作为执行预言机贯穿形式化与推演阶段，解决几何问题求解中的自动形式化与定理预测瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前神经符号几何问题求解框架在自动形式化阶段将多模态转换与下游求解器解耦，且在定理预测阶段常因固定规则库导致推演僵局。

Method: 提出SD-GPS框架：1) 求解器驱动的自动形式化，基于QwenVL3-2B统一监督式形式语言适配与可解性引导的强化学习，以可执行性为核心训练信号；2) 验证定理提出，引入僵局感知智能体提出局部辅助引理，并通过符号验证过滤以确保合理性。

Result: 在Geometry3K和PGPS9K数据集上，SD-GPS在标准补全、多选和跨模态参考设定下均持续优于现有多模态大语言模型、纯神经及神经符号方法。

Conclusion: 闭合多模态感知与符号执行间的环路可显著提升几何推理能力，为利用形式系统为神经智能体提供基础以实现可验证的问题求解提供了重要见解。
...
### [31] [NormAct: A Benchmark for Hidden Social Norm Compliance in Embodied Planning](http://arxiv.org/abs/2606.27826v1)
*Shiyun Zhao,Xinwei Song,Tianyu Guo,Xiaomeng Gao,Mingyuan Liu,Xu Han,Yuanyuan Zhang,Zhenliang Zhang,Xue Feng,Bo Dai*

Main category: cs.AI

TL;DR: 本文提出NormAct基准评估具身规划中隐式社会规范的遵守情况，并引入NormPerceptor模块以提升模型结合隐式规范的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型评估通常关注显式目标达成或直接规范知识，极少评估具身规划器能否在未明确指示的情况下推断并应用隐式的社会规范约束。

Method: 构建NormAct基准（在普通任务中嵌入隐式规范进行评估），并提出NormPerceptor（一种基于上下文的提示生成器，在规划前推断场景相关规范）。

Result: 实验表明SOTA模型显式目标达成率为67.3%，但隐式规范遵守率仅26.4%；NormPerceptor将整体任务成功率从24.2%提升至46.7%。

Conclusion: 具身智能体亟需主动检测隐式规范、将其基于视觉证据落地并整合为动作规划约束；模型合规率低的原因是难以在上下文中激活和落地规范，而非缺乏通用社会知识。
...
### [32] [MER-R1: Multimodal Emotion Reasoning via Slow-Fast Thinking Synergy](http://arxiv.org/abs/2606.27652v1)
...
TL;DR: 本文提出MER-R1强化学习框架，通过结合快思考（高召回率）与慢思考（高精确率）的优势，提升了多模态情感识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 显式推理（慢思考）并不总能提升多模态情感识别（MER）的准确率，因为快思考倾向于高召回率而慢思考倾向于高精确率，需要解决二者互补性的联合优化问题。

Method: 提出MER-R1强化学习框架，采用双重目标解耦将召回率和精确率分离为独立优化信号进行联合优化，并通过慢快置信度校准将慢思考答案与快思考直觉对齐。

Result: 在MER-UniBench和MME-Emotion基准测试上达到最先进（SOTA）性能，使推理真正有效提升了情感识别效果。

Conclusion: 统一快思考的召回导向直觉与慢思考的精确导向选择性可缓解优化中的方差诱导干扰，是多模态情感识别的有效途径。
...
### [33] [Graphon Mean Field Game of mutual holding](http://arxiv.org/abs/2606.28254v1)
*Daorong Cui,Shuoqing Deng,Yang Xiang*

Main category: math.OC

TL;DR: 本文研究了基于图子交互的互持均值场博弈，给出了最优策略的显式表征并证明了纳什均衡的收敛性。


<details>
  <summary>Details</summary>
Motivation: 旨在解决Djete和Touzi提出的互持均值场博弈中，代理人间交互作用由图子描述的更一般化场景问题。

Method: 在扩展空间上采用价值过程与图子标签联合分布的建模方式，核心技巧包括对WOP_2度量下连续性的详细分析以及针对不同模型正则性的定制论证。

Result: 在图子函数适当条件下，提供了最优策略的显式表征，证明了相关Mckean-Vlasov随机微分方程的适定性，并建立了纳什均衡的收敛结果。

Conclusion: 成功将互持均值场博弈推广至图子交互情形，为不同正则性模型下的图子均衡提供了理论基础与收敛性保证。
