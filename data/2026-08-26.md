...
TL;DR: 本文提出了一种基于离散扩散大语言模型的放射学报告生成框架（DRRG），通过迭代去噪和掩码细化机制替代传统自回归方法，提升了报告质量。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归放射学报告生成方法按token生成且无法修改早期内容，易导致错误传播，且与放射学家迭代修改报告的实际过程不符。

Method: 提出DRRG框架，将报告生成公式化为迭代掩码token去噪过程，并引入临床实体感知互补掩码以提高监督覆盖率和强调重要实体，同时通过概念条件模块将图像衍生的临床概念注入视觉表示中。

Result: 在MIMIC-CXR和CheXpert Plus数据集上，即使使用更小的LLM解码器，DRRG在大多数指标（如BLEU-4、CheXpert-F1、RadGraph-F1等）上均优于对比方法。

Conclusion: 离散扩散模型通过支持迭代和双向报告细化，为自回归报告生成提供了有效替代方案；结合临床聚焦的掩码和图像概念条件化能进一步提升报告的临床一致性与质量。
...
### [2] [NeoWorld-Pro: Programming Interactive Scenes from Monocular Images for Embodied Simulation](http://arxiv.org/abs/2608.24212v1)
*Yumeng He,Yichen Song,Xiaotian Yang,Weijia Zhang,Zanwei Zhou,Junru Gong,Xiaokang Yang,Yunbo Wang*

Main category: cs.CV

TL;DR: 本文提出NeoWorld-Pro框架，利用多模态大语言模型将单张RGB图像转化为可执行程序来生成交互式3D仿真场景，并通过物理引擎闭环验证机制确保物理合理性。


<details>
  <summary>Details</summary>
Motivation: 具身AI需要高质量、忠实反映真实世界的仿真资产，但现有图像到URDF的方法缺乏物理基础和场景级交互能力，难以将原始视觉观测转化为仿真就绪的场景。

Method: 将单目场景重建重新定义为交互式3D环境的程序化编程问题，利用MLLM的零样本推理和代码合成能力，将RGB图像转换为指定物体几何、铰接结构和物理属性的可执行程序，并通过物理引擎中的循环验证机制迭代优化生成程序，确保铰接物理合理、物体组合有效及空间关系准确。

Result: 实验表明NeoWorld-Pro优于开环方法和现有单目重建方法，并能够支持稳定堆叠和细粒度操作等复杂下游任务。

Conclusion: NeoWorld-Pro成功弥合了视觉观测与仿真就绪场景之间的鸿沟，结合MLLM程序合成与物理闭环验证，为具身AI提供了高质量的交互式仿真资产生成能力。
...
### [3] [Boot-and-Feedback Framework for Generalist-Expert Model Collaboration in Breast Ultrasound Diagnosis](http://arxiv.org/abs/2608.23974v1)
*Ming Cheng,Hongyu Sun,Zhaolin Chen,Jun Liu,Hossein Rahmani,Qiuhong Ke*

Main category: cs.CV

TL;DR: 提出BooF模型协作框架，通过多模态大语言模型与专家模型的协同交互提升乳腺超声诊断的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 乳腺超声诊断依赖操作者且现有的多模态大语言模型因缺乏领域知识易产生虚假描述，导致误导下游专家模型并影响临床有效性。

Method: 提出Boot-and-Feedback (BooF)框架。在Boot阶段利用BI-RADS词汇和初步预测引导大模型避免幻觉并迁移推理能力；在Feedback阶段通过轻量级注意力门控跨模态融合模块结合文本与视觉特征，自适应过滤噪声。

Result: 在多个乳腺超声数据集上的实验表明，BooF在诊断准确性和可解释性方面显著优于现有最先进方法。

Conclusion: BooF框架有效解决了多模态大模型在医学图像分析中的幻觉问题，实现了性能提升与临床可解释性的双重目标。
...
### [4] [LG-GER: Language-Guided Group Emotion Recognition via Multimodal Evidence Distillation](http://arxiv.org/abs/2608.23880v1)
*Ahmed Shehab Khan,Zhiyuan Li,Yan Tong*

Main category: cs.CV

TL;DR: 提出语言引导蒸馏框架LG-GER，通过多模态大语言模型生成密集空间证据以训练单一视觉-语言模型，实现无需检测器的实时群体情感识别。


<details>
  <summary>Details</summary>
Motivation: 当前群体情感识别方法依赖检测器驱动的多流管道，且仅使用缺乏关键区域及其贡献强度指导的图像级监督进行训练。

Method: 利用多模态大语言模型（MLLM）生成包含边界框、情感信号和置信度分数的密集空间证据，并通过分类、区域文本定位、空间情感和空间置信度回归四种互补损失将其蒸馏到单一视觉-语言模型（VLM）主干中。

Result: 在GroupEmoW和GAF 3.0基准数据集上评估，与需要检测和多流处理的现有最先进方法相比，取得了具有竞争力或更优的结果。

Conclusion: LG-GER在推理时无需检测器、MLLM和多流融合，使群体情感识别在实时和资源受限环境中的部署成为可能。
...
### [5] [BrowserForge: Scaling Web Episode via Parallel Browser Sandboxes](http://arxiv.org/abs/2608.24848v1)
...
TL;DR: 本文提出BrowserForge框架，通过在开放网络上并行驱动浏览器沙盒来大规模生成多样化的网络交互数据，从而提升基于像素的网络代理的性能。


<details>
  <summary>Details</summary>
Motivation: 基于渲染像素训练网络代理需要大量高质量的交互轨迹，但现有公共数据集规模小且网站多样性不足，自动化合成管道也受限于预定义的网站列表，难以实现大规模扩展。

Method: 提出BrowserForge框架，包含三个核心组件：开放网络源获取（暴露于海量真实网站）、沙盒集群管理器（调度并发浏览器）以及Proposer-Solver双智能体循环（生成可执行任务并收集验证轨迹）。此外还采用规则与模型结合的清洗管道统一推理格式，训练和发布的代理纯粹基于截图进行操作。

Result: 构建了包含203,238条轨迹的大规模多样化数据集，每条轨迹均来自不同网站。基于该数据集微调紧凑型多模态模型，使其在Online-Mind2Web上的成功率从25.66%提升至33.33%，并在Multimodal-Mind2Web上持续提高步骤准确率，且性能随数据集规模扩大而提升。

Conclusion: 开放网络数据获取和广泛的网站覆盖是提升网络代理性能的关键因素，利用该方法生成的大规模多样化轨迹数据能有效增强纯像素驱动的网络代理能力。
...
### [6] [Structurally-bounded Agentic Graph Exploration for Evidence-Grounded Scholarly DeepSearch](http://arxiv.org/abs/2608.24809v1)
*Rima Hazra,Sayan Layek,Somnath Banerjee,Soumen Chakrabarti,Animesh Mukherjee*

Main category: cs.CL

TL;DR: 本文提出Crase，一种用于学术搜索的有界且可检查的深度研究智能体替代方案，通过单次查询和引文修剪提高召回率并降低成本。


<details>
  <summary>Details</summary>
Motivation: 旨在解决现有深度研究智能体采用开放式搜索循环导致的成本高昂、缺乏透明度且不可检查的问题。

Method: 单次查询获取种子论文，扩展至1.5跳引文邻域，修剪缺乏蕴含支持的引文边缘，并使用近期感知的随机游走进行排名，使候选集、保留原因和停止条件明确且固定。

Result: 在基于50万篇arXiv论文的LitSearch等基准测试中，Crase的recall@50性能比基于专有模型的深度研究智能体高出3倍，且成本仅约为三分之一。

Conclusion: Crase为学术搜索提供了一种比黑盒深度研究智能体更高效、透明且成本更低的替代方案。
...
### [7] [Length-Adaptive Decoding for Masked Diffusion Machine Translation](http://arxiv.org/abs/2608.22274v1)
*Yan Zhan,Mengkai Hou,Wanting Zhang,Zhijun Gao*

Main category: cs.CL

TL;DR: 本文提出一种无训练的长度选择器Entropy-Valley (EV)，通过预测熵优化掩码扩散语言模型在机器翻译中的目标长度决策。


<details>
  <summary>Details</summary>
Motivation: 现有的掩码扩散解码研究主要关注词元揭示顺序，而忽视了在去噪前必须确定目标长度这一直接影响翻译覆盖率和冗余度的关键问题。

Method: 提出Entropy-Valley (EV)，通过全掩码前向传递计算候选目标画布的平均预测熵进行评分，选择主干网络最准备填补的画布长度，该过程无需额外训练。

Result: 与基于语料长度统计的基线相比，EV在英译中、中译英和英译德上分别恢复了64.9%、65.3%和33.0%的COMET-22增益；专家评估证实了中英互译的充分性提升；在相同微调数据下，EV系统在英译中上与LLaMA-3-8B自回归模型持平，中译英上领先。

Conclusion: 适合去噪的目标长度无需与参考长度完全一致；在掩码扩散机器翻译设定中，如何提供目标长度比决定词元揭示顺序更为重要。
...
### [8] [Ripple-Pivot Search: Active Parallel Decoding for Diffusion Large Language Models](http://arxiv.org/abs/2608.11742v1)
*Yushi Ye,Xu Chen,Haoyun Jiang,Jinsong Lan,Haihong Tang,Bo Han,Ivor Tsang,Yanfeng Wang,Bo Zheng,Jiangchao Yao*

Main category: cs.CL

TL;DR: 本文提出一种无训练的解码方法RPS，通过利用扩散大语言模型中熵枢轴位置的连锁效应来加速并行解码。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散大语言模型并行解码调度器仅关注单个位置的解码标准，忽略了提前提交某些位置对减少后续解码不确定性的潜在连锁效应。

Method: 提出Ripple-Pivot Search (RPS)方法，寻找中熵位置作为候选枢轴，并通过前瞻评估确定能产生最大下游收益的token分配方案。

Result: 在多个模型和基准测试中，RPS在保持生成质量的同时实现了4-10倍的真实时间加速，准确率较前向基线提升最高5.49%；结合KV缓存时最高可达18倍加速。

Conclusion: 主动利用解码过程中的连锁效应不仅能显著加速扩散大语言模型的推理，还能提升生成质量，结合缓存技术可进一步大幅提高吞吐量。
...
### [9] [Reducing Pretraining-Generation Mismatch in Diffusion Language Models](http://arxiv.org/abs/2608.09424v1)
*Xiaocheng Lu,Huabin Liu,Song Guo,Jianguo Li*

Main category: cs.CL

TL;DR: 提出前缀条件扩散（PCD）预训练目标，结合自回归前缀监督和后缀去噪，解决扩散语言模型预训练与提示条件生成之间的不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型的原生预训练会同时破坏提示和续写词，削弱了提示条件生成所需的干净前缀接口，导致训练与使用场景不匹配。

Method: 提出PCD预训练目标，对干净前缀进行自回归监督，仅对未知续写部分应用扩散去噪，通过修改注意力掩码和破坏掩码等使局部训练接口与评估时的查询方式一致。

Result: 在LLaDA2-Mini和Qwen-1.7B上，PCD相比同族原生dLLM基线表现更优，分别在六个基准平均分上取得4.2%和14.2%的相对提升。

Conclusion: 将预训练上下文分布与提示条件生成相对齐，能在不改变推理模式的情况下有效缩小扩散语言模型的续写性能差距。
...
TL;DR: DominoTree利用路径相关的非分解校正构建最佳优先草拟树，显著提升大模型推理的接受长度和吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有的树结构推测解码方法（如DDTree）基于分解的边缘分布，无法表示像Domino这类具有路径相关（非分解）因果校正结构的草拟器。

Method: 提出DominoTree，一种无需训练的最佳优先草拟树。该方法沿每个根到节点的路径使用Domino的条件（非分解）校正进行评分，并通过将每个节点的校正限制在候选top-M范围内来实现实用性。

Result: 在所有服务单元和研究测试平台中均达到最高平均接受长度（24个数据集中有21个领先）。吞吐量最高达AR的7.3倍，在SGLang中单请求吞吐量比Domino高12%（Qwen3-8B）；在长上下文测试中，接受长度提升29-36%，吞吐量提升10-34%。

Conclusion: 通过沿候选路径重新计算非分解校正，DominoTree显著提升了推测解码的效率和接受长度，但在预填充占主导的超长上下文场景下，其相对于EAGLE-3的优势缩小至平局。
...
### [11] [GRAFT: Adaptive DLM-Based Draft Tree Construction with Target-Distilled Edge Scoring](http://arxiv.org/abs/2608.20375v1)
*Xuming Ye,Zeming Ma,Runjie Yu,Yuan Liu,Tianle Li,Shuhan Bai,Jian Zhou,Fei Wu*

Main category: cs.CL

TL;DR: 提出GRAFT框架，通过目标蒸馏边打分和状态感知预算分配，优化扩散语言模型推测解码中的草稿树构建。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于扩散语言模型的草稿树构建方法仅依赖概率选择边导致父子节点兼容性差，以及固定预算无法适应解码状态变化的问题。

Method: 提出GRAFT框架，包含目标蒸馏边打分（TDES）以选择目标兼容边，以及状态感知预算分配（SABA）通过平衡预期草稿增益与验证成本动态设定树预算。

Result: 在多个模型和任务上实现了2.13至6.36倍的端到端加速，每轮额外开销低于0.5毫秒（约占验证延迟的1.4%）。

Conclusion: GRAFT有效提升了基于DLM的推测解码效率，以极低开销实现了显著加速。
...
TL;DR: 提出CaDDTree方法，通过联合优化树结构和节点预算直接最大化令牌吞吐量，解决了现有方法忽略验证成本的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的DDTree方法最大化预期接受长度，但由于该指标随预算非递减，导致其忽略验证成本，缺乏选择节点预算的原则性依据。

Method: 提出CaDDTree，显式建模草拟和验证延迟，将吞吐量优化转化为每轮的一维预算搜索；并在凸验证成本下证明吞吐量函数为单峰，实现高效的贪心停止和无需离线搜索的自适应预算调整。

Result: 在Qwen3-4B和Qwen3-8B的8个基准测试中，CaDDTree在几乎所有任务上均达到或超越了使用预言机预算的DDTree性能。

Conclusion: CaDDTree提供了一种成本感知的预算自适应机制，有效提升了推测解码的令牌吞吐量，并证明了其在大语言模型推理加速任务上的优越性。
...
TL;DR: 提出DDTree方法，通过从块扩散起草模型的位置分布构建草稿树，改进DFlash的单轨迹验证限制，进一步提升推测解码性能。


<details>
  <summary>Details</summary>
Motivation: 尽管DFlash在推测解码中表现优于EAGLE-3等强基线，但其每轮仅验证单个草拟轨迹，限制了接受长度，未能充分挖掘并行验证的潜力。

Method: DDTree直接利用块扩散起草模型的逐位置分布构建草稿树；在固定节点预算下，采用最佳优先堆算法依据草稿模型输出的代理指标选择最匹配目标模型的延续路径，并使用仅祖先注意力掩码在目标模型单次前向传递中高效验证该树结构。

Result: DDTree克服了DFlash单轨迹验证的瓶颈，在固定节点预算下提升了接受长度，跻身于推测解码领域的领先方法之列。

Conclusion: 将块扩散模型与树状搜索相结合能显著提升自回归语言模型的加速效果，DDTree为推测解码提供了一个高效且领先的框架。
...
### [14] [CForce: Boosting Parallel Decoding for dLLMs via Consistency Forcing](http://arxiv.org/abs/2608.13925v1)
*Yuji Ren,Chenkai Xu,Zhuocheng Gong,Jianguo Li,Zhijie Deng*

Main category: cs.LG

TL;DR: 本文提出了Consistency Forcing (CForce)方法，通过蒸馏对齐扩散大语言模型早期和后期的掩码预测，以解决高并行度下早期预测不可靠导致错误传播的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散大语言模型在激进的并行策略下，早期去噪阶段的预测不可靠，导致错误传播到后期阶段，影响生成质量。

Method: 提出CForce蒸馏方法，在预收集的自展开轨迹上训练模型，强制早期阶段掩码预测与后期阶段对齐；引入置信度自适应KL散度作为蒸馏目标，并结合理论分析证明其有效性。

Result: 在非编辑和可编辑LLaDA模型上的实验表明，CForce改善了速度与质量的权衡，特别是在高并行解码预算下表现显著。

Conclusion: CForce能有效近似最小化早期阶段的预测误差，提升训练与推理的一致性，且该方法同时适用于掩码解码和可编辑解码场景。
...
### [15] [Diffusion LLMs as Targets and Adversaries: Mechanistic Safety Exploits](http://arxiv.org/abs/2608.07430v1)
*Elena Dumitrescu,Gert Lek,Lydia Y. Chen,Jérémie Decouchant*

Main category: cs.LG

TL;DR: 本文揭示了扩散大语言模型(DLLMs)安全对齐的稀疏性和可迁移性漏洞，并提出了SN-Guided Diffusion离线黑盒越狱框架，利用安全神经元引导实现高效跨模型攻击。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(DLLMs)用迭代并行去噪替代自回归预测，但其内部安全机制尚未被充分理解。本文旨在揭示基于扩散的对齐方法中存在的机制性漏洞，探究DLLMs作为攻击目标和攻击者的安全性问题。

Method: 首先通过安全神经元映射和剪枝分析DLLMs安全对齐的稀疏性和跨架构可迁移性；随后提出SN-Guided Diffusion框架，使用加权安全神经元损失引导扩散过程避开安全触发区域，实现完全离线的黑盒越狱攻击。

Result: 安全对齐在DLLMs中呈现稀疏性且可跨架构迁移。自剪枝使LLaDA的ASR从2.6%升至73.8%，Dream从1.9%升至86.6%；从Qwen2.5迁移剪枝使Dream的ASR从1.9%升至73.2%，Fast-dLLM从7.0%升至86.3%。SN-Guided Diffusion实现AUROC=1.0的完美提示分离，对Llama-3-8B、Qwen2.5-7B和Gemini-2.5-Flash-Lite的迁移ASR分别达77.1%、86.9%和74.3%，且生成成本比现有框架低数个数量级。

Conclusion: DLLMs的安全对齐存在严重的机制性漏洞，继承自自回归模型的安全特征可被直接利用进行迁移攻击。这凸显了扩散模型安全对齐方法需要重新设计，未来需开发更鲁棒的安全机制以应对此类神经元级攻击。
...
### [16] [Retrofitting Linear Attention into Diffusion Language Models](http://arxiv.org/abs/2608.06628v1)
*Jinha Kim,Younghun Roh,Jaeyeon Kim*

Main category: cs.LG

TL;DR: 本文引入块混合注意力机制，通过对前序块进行线性化处理来加速扩散语言模型（dLLM）的推理，同时保持基准性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在分块半自回归解码的去噪步骤中，仍需重复关注所有前序块，导致高昂的前缀注意力计算成本。本文旨在解决该瓶颈以进一步加速dLLM推理。

Method: 提出块混合注意力机制，在当前去噪块内保留精确的softmax注意力，而对前序块应用线性注意力。该方法基于LoLCAT，通过最少的后训练将预训练16B模型LLaDA 2.1中20层注意力的6层进行改造替换。

Result: 转换过程仅需约60小时且保持了基准性能（如HumanEval 72.0% vs 75.6%）。结合Triton实现，解码吞吐量最高提升1.7倍，并在内存耗尽前支持更多并发请求。

Conclusion: 预训练的扩散语言模型可以被高效地线性化，从而在不显著牺牲性能的前提下实现更快的推理速度和更高的内存效率。
...
### [17] [Learning Whom to Trust : Decision-Generated Credibility in Social Learning](http://arxiv.org/abs/2608.24851v1)
*Gabriel Bontemps,Abhishek Banerjee*

Main category: cs.NE

TL;DR: 本文研究了社会信息可信度由发送者自身决策过程动态生成时，社会互动在促进集体学习与放大早期错误之间的张力。


<details>
  <summary>Details</summary>
Motivation: 社会互动既能促进集体学习，也可能放大早期错误。当社会信息的可信度不是预先固定、而是由发送者自身的决策过程内生产生时，这一张力的机制尚不清楚，需要深入研究。

Method: 采用强化学习智能体通过漂移-扩散过程进行二元选择，联合决定选择、决策时间和置信度；决策置信度通过加权预期影响和回顾性社会学习转化为社会可信度。在平衡社区暴露条件下，对预期场进行解析推导，利用局部雅可比矩阵分析共模放大阈值和跨社区渗透率的作用。

Result: 蒙特卡洛实验显示非单调性能模式：中等强度传递加速纠错，强传递导致群体锁定错误共识，低渗透率维持群体分歧。消融实验揭示置信度的双重作用：可信度敏感的传递放大社会错误，而依赖置信度的私人学习起稳定作用。

Conclusion: 模型给出了可检验的预测：发送者置信度与接收者行为之间的关系以准确性为条件。中等社会传递强度和适当的跨社区渗透率对集体学习的正确性至关重要。
...
### [18] [A Co-Simulation Platform Coupling Land Use, Transportation, and Building Energy: Development and Case Study](http://arxiv.org/abs/2608.24817v1)
...
TL;DR: 该论文提出了首个同时整合土地利用、交通运输和建筑能耗的城市级联合仿真平台，用于评估政策的综合影响。


<details>
  <summary>Details</summary>
Motivation: 以往的城市规模研究通常将土地利用、交通和建筑能耗孤立建模，未能捕捉这三个领域之间的相互影响与反馈机制。

Method: 将UrbanSim（土地利用）、POLARIS（基于智能体的交通）和CityBES（城市建筑能耗）模型耦合为单一工作流，其中交通出行成本矩阵驱动土地利用，智能体活动驱动动态建筑占用率。

Result: 在芝加哥的预测中，远程办公使2045年车辆行驶里程降低12.5%并将活动分散至外围，里程费使其降低2.9%并将活动集中于核心区；土地利用反馈对县级交通影响贡献超1%，表明纯交通研究会误报区域影响；两项政策均使建筑能耗增加约1%。

Conclusion: 该平台是首个整合上述三领域的系统，用内生智能体占用率和预测驱动的建筑存量取代了静态预设，使规划者能评估政策的综合影响，且依赖全国可用数据，具备可移植性。
...
### [19] [Not All Tokens Are Equal: Region-Aware Consistency Repair of Backdoors in MLLMs](http://arxiv.org/abs/2608.24354v1)
*Jiali Wei,Ming Fan,Mingkun Zhang,Haoyu Wang,Jun Sun,Guoheng Sun,Xiaoning Ren,Haijun Wang,Ting Liu*

Main category: cs.CR

TL;DR: 本文提出RACER框架，通过模态感知的逐层不一致性归一化和对抗微调，从模型层面有效消除多模态大语言模型(MLLMs)的后门风险。


<details>
  <summary>Details</summary>
Motivation: MLLMs继承了后门风险，但现有模型级移除方法对MLLMs效果有限，而MLLM特定防御仅在推理时过滤输入，无法消除模型内部嵌入的后门。

Method: RACER将内部融合表示分解为视觉和文本词元区域，分别归一化其逐层不一致性并重组以构建区域感知目标，随后通过极小极大优化生成最坏情况扰动并进行对抗微调，以修复模型。该方法仅需100个干净样本且无需触发器先验知识。

Result: 在3个开源MLLM和36个后门设置的评估中，RACER将平均攻击成功率(ASR)降至1.1%（32个设置降至0%），并保持了干净任务的效用。

Conclusion: RACER能够在无需了解触发器或攻击细节的情况下，从源头上有效消除MLLMs中的潜在后门，同时不损害其正常性能。
...
### [20] [Recursive Experiential-Working Memory Evolution for Long-Horizon Agent Harnesses](http://arxiv.org/abs/2608.24876v1)
*Zhaochen Yu,Yingcheng Wu,Zhenfei Yin,Kaiyuan Chen,Zhe Zhao,Mengdi Wang,Shuicheng Yan,Ling Yang*

Main category: cs.AI

TL;DR: 本文提出了Recuris，一种递归经验-工作记忆架构，通过有界的递归记忆进化循环解决长时序任务中递归自我改进的难题。


<details>
  <summary>Details</summary>
Motivation: 递归自我改进在长时序任务中面临困难，因为不断增长的历史记录会掩盖任务状态并导致技能调用错位。

Method: 提出Recuris架构，利用工作记忆跟踪进度并指导从经验记忆中选择技能；将执行过程转化为结构化证据以定位故障；由固定的元智能体利用该证据对技能记忆进行局部且经验证的更新，从而形成有界的递归记忆进化循环。

Result: 在4个长时序基准和10个模型上测试，35/37个模型-基准对的任务成功率得到提升，推动前沿模型达到SOTA水平；随着交互时序变长优势更加明显（最长任务提升达32.2分），常见长时序失败率降低高达80%。

Conclusion: 递归演化记忆可作为递归自我改进的可扩展基础，使智能体能够持续将积累的经验转化为更有效的长时序行为。
...
### [21] [SPO++: Stream-Aligned Policy Optimization for Asynchronous Agentic RL](http://arxiv.org/abs/2608.24870v1)
*Kai Ruan,Jinghao Lin,Qianshan Wei,Ziqi Zhou,Zihe Huang*

Main category: cs.AI

TL;DR: 本文提出SPO++方法，通过在动作令牌度量下标准化优势并按策略事件组织证据，改进了单流策略优化在长时序工具使用任务中的在线学习效率。


<details>
  <summary>Details</summary>
Motivation: 群组相对强化学习在处理长且可变的工具使用轨迹时成本高昂，而单流策略优化（SPO）的轨迹中心化未能与动作消耗的令牌加权量对齐，导致优化受限。

Method: 提出SPO++方法，在动作令牌度量下标准化终端结果优势以解决中心化不匹配问题，并按生成提示证据的策略事件而非接收顺序组织证据。

Result: 在ALFWorld（两种模型规模）和Math-TIR上的实验表明，SPO++相比SPO提高了在线学习效率，消融实验显示动作令牌度量归一化是最有效的组件。

Conclusion: 通过修正优势标准化的度量并重新组织证据事件，能够显著提升强化学习在复杂工具使用轨迹中的学习效率。
...
### [22] [StarHarness: Evolving Harnesses with Stratified Search for Enterprise Environments](http://arxiv.org/abs/2608.24804v1)
*Esakkivel Esakkiraja,Denis Akhiyarov,Vikas Yadav,Sai Rajeswar,Patrice Bechard,Sridhar Nemala,Sagar Davasam*

Main category: cs.AI

TL;DR: StarHarness是一个在不改变模型权重的前提下演化环境特定智能体框架（harness）的系统，在企业工具密集任务上实现20-35个百分点的性能提升。


<details>
  <summary>Details</summary>
Motivation: 工具密集型企业任务中存在持续性的模型-环境不匹配问题，默认智能体框架无法有效适应特定环境，导致性能不佳。

Method: 固定模型权重，演化包含提示词、工具接口、技能、MCP提供者、子代理结构和代理循环配置的harness；通过按基线失败行为分层构建紧凑演化池，将提议者可见的搜索任务与隐藏的选择任务分离，并预留独立任务评估泛化能力。

Result: 在ITBench SRE、EnterpriseOps-Gym ITSM和AutomationBench Finance三个基准上，经4-12次 accepted changes 后性能提升20-35个百分点；提升在未参与演化的任务上持续存在，并可跨GPT和Qwen模型家族迁移而无需重新演化；轨迹分析显示改进源于接口修复、环境约定和压缩搜索的操作知识，减少了误诊并缩短了轨迹。

Conclusion: StarHarness提供了一种实用方法来减少工具密集型企业任务中持续存在的模型-环境不匹配问题，具有跨模型和跨任务的泛化能力。
...
### [23] [CAFE: Self-Improving Search Agents Need Co-Evolving Feedback](http://arxiv.org/abs/2608.24794v1)
...
TL;DR: 本文提出CAFE框架，通过共享参数模型交替扮演搜索代理和评论家角色，实现策略与反馈的协同进化，从而提升智能体搜索任务的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的结果监督搜索代理仅依赖终端奖励，无法定位中间错误或在错误累积前纠正轨迹；将纠正反馈作为轨迹内干预时，代理需同时决定何时请求反馈，而评论家需从结果混淆的rollout中推断有用的纠正，两者耦合带来优化困难。

Method: CAFE框架使用共享参数模型交替担任搜索代理和评论家。在线RL阶段，利用prompt级别的调用-跳过成功差距生成比较反馈估计来塑造请求回报，并通过反馈感知的优势塑造在反馈前后重新加权token优势；离线阶段，通过匹配成功与失败轨迹的偏好优化学习反馈。初始化时基于基础代理自身失败轨迹构建反馈条件恢复。

Result: 在七个智能体搜索基准上，CAFE平均优于所评估的基于RL的搜索代理，在六个域外基准上保持优势，并减少答案级别的幻觉。消融实验表明，仅改进代理或仅改进评论家最终都会停滞，而交替更新两者可持续提升性能。

Conclusion: 自改进搜索代理需要与其所引导的策略协同进化的反馈机制；代理策略与评论家反馈的交替优化是持续提升性能的关键。
...
### [24] [StepGuard: Learning Step-Level Guardrails with Scalable Supervision and Safety-Utility Balancing](http://arxiv.org/abs/2608.24777v1)
*Zhijie Zheng,Yu Li,Chen Qian,Yuqian Fu,Yanwei Fu,Lu Sheng,Jing Shao,Dongrui Liu*

Main category: cs.AI

TL;DR: 本文提出了StepGuard模型，通过在工具执行前审计代理的步骤级动作，有效防御大语言模型代理的安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有防御机制多在轨迹完成后进行评估，缺乏对执行前步骤级动作的监控，易导致文件篡改和信息泄露等安全风险。

Method: 提出StepGuard防御模型，结合StepGen数据引擎生成上下文相同但动作不同的安全与不安全轨迹，并采用Balance-GRPO方法动态平衡学习过程以减少过度和不足防御。

Result: StepGuard在开源防御模型中准确率最高且媲美GPT-5.4，在测试环境中将平均攻击成功率降低77.3%，同时任务效用仅微降2.8个百分点。

Conclusion: 执行前的步骤级监控能够高效提升大语言模型代理的安全性，且对任务效用的负面影响极小。
...
TL;DR: 本文分析了掩码扩散语言模型在并发服务负载下的行为特性，揭示了其在请求难度离散性、CPU调度开销及去噪步级并行化需求上与自回归模型的根本差异。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏对dLLM在真实并发负载下行为的实证测量，现有的服务系统存在盲目沿用自回归（AR）模型假设的风险，本文旨在填补这一空白以指导dLLM服务基础设施的构建。

Method: 使用配备D2F LoRA适配器的LLaDA-8B-Instruct模型，在单张NVIDIA H200 GPU上基于GSM8K和HumanEval数据集进行特性表征，并推导了泊松到达下固定填充同步批处理的超时规则。

Result: 请求难度呈离散分布（11个固定步数级别）且事前不可预测（最佳R2=0.150）；短生成预算（<320 tokens）会掩盖服务方差；单请求76%的挂钟时间为CPU调度开销；批处理通过分摊该开销在批次大小16时实现16.0倍吞吐量提升；且输出质量不随批次大小降低（GSM8K准确率74-76%）。

Conclusion: 服务扩散语言模型需要在每个去噪步骤级别实现并行，其准入和驱逐机制与共享前向传播的交互方式与自回归服务存在根本差异。
...
TL;DR: 提出无需训练的推理方法CAI-DLLM，利用首步置信度引导去噪，显著加速扩散语言模型推理并降低能耗。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在推理时需要重复去噪步骤，且会反复计算已经稳定的tokens，导致生成成本高昂。

Method: 提出CAI-DLLM，利用首步置信度信号引导去噪：尽早提交简单tokens，为困难tokens分配更多去噪步数，并动态调整跨输出块的解码计划，无需重新训练或额外参数。

Result: 在LLaDA GSM8K上实现18.2倍加速且准确率提升至77.41%；在Dream HumanEval上实现13.1倍加速且pass@1升至48.17%；困难推理任务加速达44.8倍（最大准确率降4.4个点），能耗最高降低95.3%。

Conclusion: 基于首步置信度的动态去噪调度能在不重训模型的前提下大幅减少冗余计算，实现推理加速与能耗降低，并在部分任务中维持或提升准确率。
...
TL;DR: 本文提出了连续交互扩散（CID）架构，将外部工具交互直接集成到扩散语言模型的迭代去噪过程中，以克服自回归模型中停顿-恢复交互模式的限制。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（dLLMs）通过并行地反复完善输出来进行推理，而传统的停止-恢复工具调用模式会迫使模型在推理稳定前做决定、延迟有用观察并引入冗余优化和工具执行，从而损害任务准确率和推理效率。

Method: CID架构将模型运行时分离为只读事实通道、由类型化认知张量表示的思维通道和显示通道。在文本或JSON调用完全序列化之前即可触发外部读取，返回的结果被投射到正在演变的思维状态中以修改早期认知。同时利用持久绑定复用静态结果并按需刷新动态源。

Result: 本文形式化了CID的架构、运行时和训练目标，并定义了任务质量和端到端效率的评估协议。作为系列第一篇论文，本文仅聚焦于只读工具，且未提出实证性能主张。

Conclusion: CID架构旨在更早暴露证据、使工具延迟与模型计算重叠、减少重复外部工作并保留新证据到达后的有用计算，为扩散语言模型的高效工具使用提供了新的范式和理论基础。
...
TL;DR: 本文提出了TAPS，一种用于扩散模型投机解码的目标感知前缀选择方法，通过路径条件化估计优化接受率与成本的权衡，实现了显著的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散模型的草稿树方法通过边缘概率对节点进行排序，忽略了验证过程是前缀条件化的。这导致模型会验证被拒绝前缀的不可达后代节点，在增加延迟的同时获取的接受收益有限。

Method: 提出TAPS方法，将扩散边缘概率转化为路径条件的接受估计，并在固定的验证预算下选择一个紧凑的前缀闭子树，以优化接受率与成本的权衡。

Result: 在多个数据集和模型族上的实验表明，TAPS相比原始自回归解码实现了高达7.9倍的无损端到端加速，且分别比最先进的DFlash和DDTree快1.36倍和1.74倍。

Conclusion: TAPS通过路径条件化估计有效解决了扩散模型投机解码中的验证瓶颈，取得了优于现有方法的加速效果和更优的接受-成本权衡。
...
### [29] [PhysMLLMs: Spatial Priors for Unified Referring Segmentation and Grounded Reasoning of Images and Videos](http://arxiv.org/abs/2608.24574v1)
*Siyao Yan,Bo Han,Jisheng Dang,Bimei Wang,Shude Wang,Hong Peng,Yulan Guo,Jianhuang Lai,Bin Hu,Tat-SengChua*

Main category: cs.AI

TL;DR: 提出PhysMLLMs架构，在训练阶段向视频多模态大语言模型注入物理启发的空间连续性先验，以解决语言引导视频分割中的时空不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频多模态大语言模型在语言引导视频分割中常出现抖动、漂移和身份切换等时空不一致问题，根本原因是训练阶段缺乏显式的空间先验，导致难以维持稳定的时空身份和形状。

Method: 提出训练阶段先验注入架构PhysMLLMs，通过全局表征先验对齐（REPA-Global）机制，利用离线嵌入缓存和计划蒸馏方案，从冻结的DINOv2教师模型向学生模型蒸馏全局视觉表征。

Result: 在多个视频基准上显著提升了分割掩码质量和跨帧一致性，特别是在遮挡、小目标等复杂场景中；且未增加推理成本，在单帧指代分割和通用多模态基准上保持了同等性能。

Conclusion: 物理启发的空间先验注入能够有效提升视频分割的时间稳定性，同时不损害图像级定位和通用多模态能力。
...
### [30] [HMGCLIP: Heterogeneous Multi-Granularity Contrastive Learning for E-commerce Representation Learning](http://arxiv.org/abs/2608.24467v1)
*Qiuyu Zhu,Yi Gao,Zhichao Wan,Mingyang Ma*

Main category: cs.AI

TL;DR: 本文提出HMGCLIP框架，利用异构超图对齐多粒度语义并挖掘结构感知硬负样本，以解决多模态大语言模型在细粒度产品属性区分上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型将产品信息隐式编码为全局嵌入，无法捕捉细粒度属性（如相似产品间的细微材质差异），这严重影响了需要精确属性区分任务的性能。

Method: 提出统一的多模态嵌入框架HMGCLIP，通过构建异构超图挖掘结构感知硬负样本，并在关系和超边层级对齐多粒度语义；同时设计双粒度推理机制动态融合属性证据。此外，发布了一个全面的细粒度电商数据集。

Result: 在新发布的数据集和公开的MAVE基准上的大量实验表明，HMGCLIP的性能优于强大的多模态编码器、多模态大语言模型（MLLMs）和电商基线模型。

Conclusion: HMGCLIP有效提升了多模态产品理解与细粒度属性判别能力，验证了其优越性，并为未来的细粒度电商基准测试提供了新数据集。
...
### [31] [SonarLLM: A Native Sonar--Optical Multimodal Large Language Model for Underwater Perception](http://arxiv.org/abs/2608.24325v1)
*Cong Su,longxuan ma,Ling Dong,Guofeng Tang,Weijie Yin,Haohui Chen,Zhengtao Yu*

Main category: cs.AI

TL;DR: 本文提出SonarLLM多模态大语言模型和SonarBench基准，通过融合声纳与光学感知实现复杂水下环境中的可靠感知。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型主要基于光学编码器，无法处理声纳数据或自适应利用声纳与光学的互补性，导致在浑浊水下环境中光学相机性能退化时感知不可靠。

Method: 提出SonarLLM模型，包含声纳专用编码器、特定模态的物理感知特征增强以及可靠性感知的分层融合机制；同时引入SonarBench基准，涵盖识别、计数、视觉问答和字幕生成四项任务以及声纳、光学和融合三种输入设置，用于测量跨模态互补性。

Result: 在仅声纳输入下，SonarLLM的宏观准确率达72.0%，超出最强基线34.4个百分点；在融合输入下达68.7%，超出最佳基线25.1个百分点。随着浑浊度增加，融合相比光学的增益从6.0提升至36.0个百分点。

Conclusion: 稳健的异构感知不仅依赖于引入声纳模态，更关键在于根据其感知特性进行合理的特征表示与动态权重分配。
...
### [32] [Giraffe: A Mapping Architecture from Hidden Text Representations to Visual Embeddings for Efficient Graphic Design](http://arxiv.org/abs/2608.23970v1)
*Nejla Ghaboosi*

Main category: cs.AI

TL;DR: 提出一种使用单个[IMG]标记将隐藏表示映射到视觉模型嵌入空间的新型架构，以解决多模态大模型在图形设计生成中的输入长度限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在媒体生成方面能力有限，且现有方法使用多个专用图像标记导致输入长度显著增加，无法满足图形设计等需要混合数千个标记的复杂任务需求。

Method: 提出一种新型架构，每张图像仅使用单个[IMG]标记将隐藏表示映射至CLIP ViT-L/14等视觉模型嵌入空间。该架构包含两个浅层MLP块（含独立压缩模块和共享扩展模块），采用六种损失函数训练；训练时一模块辅助另一模块，推理时省略以实现轻量化。

Result: 在图像到设计和文本到设计生成任务中均表现出优异的性能。

Conclusion: 该单标记映射方法有效克服了输入长度瓶颈，实现了轻量级且高性能的图形设计生成，拓展了多模态大语言模型在媒体生成领域的应用潜力。
...
### [33] [Learning to Prefer Reliably: Error-Augmented Emotion Preference Optimization with Calibrated Fusion](http://arxiv.org/abs/2608.24730v1)
*Zilong Huang,Junyi Peng,Junjie Li,Kai Li,Wenze Ren,Kong Aik Lee,Man-Wai Mak,Tatsuya Kawahara*

Main category: cs.MM

TL;DR: 本文提出EAPO框架，通过错误增强数据集和多模型评判融合，从数据和模型两个层面提升多模态大语言模型在情感偏好判断中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统成对监督数据稀疏，每个正样本仅对应一个负样本，无法覆盖情感描述出错的多种方式，尤其缺乏对语义流畅但情感不一致描述的暴露；同时依赖单一MLLM评判者会引入模型特定偏差，影响对细粒度或模糊多模态情感线索的判断。

Method: 在数据层面，从每个优选描述生成多个可控且情感感知的负样本描述构建错误增强数据集；在模型层面，使用多个独立MLLM评判者，并通过margin校准的软融合（margin-calibrated soft fusion）将异构margin映射到统一尺度后再聚合偏好间隔。

Result: 在MER2026-EmoPrefer挑战数据集及错误增强数据集上的实验表明，EAPO提升了情感偏好预测性能，并增强了MLLM评判者在评估与视频多模态情感证据冲突的流畅描述时的鲁棒性。

Conclusion: EAPO通过数据增强与多评判者融合有效解决了情感偏好学习中数据稀疏和单一模型偏差问题，为提升MLLM情感偏好判断的可靠性提供了可行方案，代码已开源。
...
### [34] [Test-Time Collaborative Classification over Multi-Agent Networks](http://arxiv.org/abs/2608.24787v1)
*Ping Hu,Mert Kayaalp,Ali H. Sayed*

Main category: cs.MA

TL;DR: 本文提出一种多智能体网络分布式二分类协作框架，允许异构的独立训练智能体在测试阶段通过交换局部决策统计量进行集体预测。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统的异构性（如架构、特征空间或模态差异）使得跨智能体联合训练全局模型面临巨大挑战，亟需一种无需联合训练即可实现分布式决策的机制。

Method: 采用独立训练与协作推理相结合的范式，智能体在测试阶段通过分布式学习协议交换局部决策统计量以协调行动并形成集体预测。

Result: 在有限轮次和有限精度通信下确立了分类误差保证和PAC式泛化界限，实验揭示了模型异构性、网络拓扑、组合策略及通信约束对精度的影响，量化了独立训练代价与集体预测收益。

Conclusion: 协作推理能在独立训练和通信约束下有效提升分布式决策性能，该框架为多智能体系统的预测精度提供了理论保证和实验依据。
...
### [35] [PonderPounce: A Pretrained MLLM as an Episode Context Engine for Robot Control](http://arxiv.org/abs/2608.24115v1)
*Suhwan Choi,Jaeyoon Jung,Sungkyung Kim,Yunsung Lee,Youngjae Yu*

Main category: cs.RO

TL;DR: PonderPounce通过重用多模态大语言模型的原生因果上下文作为机器人记忆，联合训练System2推理模型和System1动作模型，显著提升了机器人操作任务的性能与效率。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作（VLA）模型通常直接继承预训练表示，未能有效利用多模态大语言模型（MLLM）处理长视觉历史和在部分可观测性下推理的上下文记忆能力。

Method: 提出PonderPounce双系统架构：System2 MLLM（Ponder）在原生因果上下文中累积观测和认知并生成子目标推理，System1 VLA（Pounce）接收当前观测和指令，并通过接口异步接收Ponder的最新认知token。两者无需专用记忆模块即可端到端联合训练。

Result: 在RoboMME基准上，9B和0.8B模型分别达到60.83%和50.04%的准确率，优于现有基线；增加数据后达到75.54%。在RoboCasa-DC上达到12.5%，优于最强基线。系统延迟低至78ms（认知刷新）和25ms（动作调用），支持20Hz播放。

Conclusion: 重用MLLM的原生因果上下文作为机器人记忆是一种高效且有效的策略，无需构建专用记忆模块即可实现高性能的机器人控制，并具备良好的数据扩展性和任务泛化能力。
