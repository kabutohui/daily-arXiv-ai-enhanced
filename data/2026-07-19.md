...
TL;DR: 本文提出多智能体框架ARMOR++，利用视觉语言模型和大语言模型协调多种攻击原语，实现对深度伪造检测器的高迁移性黑盒规避。


<details>
  <summary>Details</summary>
Motivation: 现有的深度伪造检测器依赖脆弱的架构线索，在黑盒对抗迁移下易失效；且现有迁移攻击缺乏语义感知，在不可查询约束下（尤其是跨架构迁移）效果不佳。

Method: 提出ARMOR++多智能体框架，利用Qwen2.5-VL提供空间语义先验，Qwen3 LLM协调原语选择、自适应超参数重参数化和熵正则化扰动混合，并整合五种互补攻击原语以针对异构归纳偏置。

Result: 在AADD-2025基准测试中，ARMOR++在不同质量图像环境下均显著优于基线，盲目标攻击成功率(ASR)大幅提升，并在防御配置下表现优异。

Conclusion: 研究揭示了当前深度伪造检测器部署存在显著的残余可靠性差距，并证明了智能体协调在发现潜在漏洞方面的有效性。
...
TL;DR: 本文提出了MAGiSt3R，一个能以接近10 FPS速度对单目RGB视频进行3D重建和相机追踪的多智能体框架。


<details>
  <summary>Details</summary>
Motivation: 旨在解决单目RGB视频的实时3D重建和相机追踪问题，特别是多智能体场景下的局部地图融合以及前馈管线中累积的相机漂移问题。

Method: 采用3R系列前馈模型回归局部点图，利用MAGMA模型在智能体内外层面合并局部点图生成全局点图，并通过位姿图优化减轻累积的相机漂移。

Result: 在合成和真实数据集上的评估表明，该框架在重建和相机追踪精度上优于现有最先进方法。

Conclusion: MAGiSt3R能够高效、准确地进行多智能体3D重建和相机追踪，在精度和速度上均优于现有方法。
...
TL;DR: 本文提出了Symbal方法和SymbalBench基准，用于检测多模态大语言模型生成图像标题时的系统性错位问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在生成图像标题时常引入与特定视觉特征相关的系统性错位错误，本文旨在检测此类错误以辅助审计视觉-语言数据集，且无需访问底层模型。

Method: 提出Symbal，利用现成基础模型的双阶段结构化设置来识别系统性错位并以自然语言总结结果；同时引入包含420个数据集的SymbalBench基准进行评估。

Result: Symbal在SymbalBench上正确识别出63.8%数据集中的系统性错位，较最接近的基线提升近4倍；真实世界评估表明其能准确发现四种MLLM的标题错误并有效审计现有数据集。

Conclusion: 本文提出的任务、方法和基准可帮助用户在不访问底层MLLM的情况下审计其生成的标题并识别关键错误。
...
TL;DR: 本文提出ViPS框架，通过高效融合多个基础模型的互补视觉先验，显著提升了多模态大语言模型的空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有工作在将基础模型集成到多模态大语言模型时，未充分利用不同模型提供的互补空间先验来增强空间感知。

Method: 提出ViPS框架，包含高效先验代理（以极小推理开销生成基础先验）和动态先验融合机制（实现上下文感知的先验和谐融合与注入）。

Result: 在多个复杂的空间推理和3D空间理解基准测试中取得了新的最优性能。

Conclusion: 协调多种视觉先验能有效增强多模态大语言模型的空间理解能力，确立了新的技术基线。
...
TL;DR: VideoChat3是一个完全开源、高效的4B参数视频多模态大模型，通过3D视觉Transformer和可扩展数据管线，在通用、长视频和流媒体任务上实现了优异的泛化与效率平衡。


<details>
  <summary>Details</summary>
Motivation: 现有开源视频理解模型难以跨领域泛化、计算成本高，且通常不完全开源（缺乏训练代码或数据集），阻碍了可复现性和社区发展。

Method: 提出VideoChat3，采用膨胀3D视觉Transformer（I3D-ViT）和自适应帧分辨率流媒体感知技术提升效率；并开发可扩展的视频数据合成管线，构建三个高质量数据集以增强跨领域泛化能力。

Result: 在通用、长视频和流媒体基准测试中，VideoChat3以仅4B参数超越了同等或更大参数量的现有开源模型，且具有更高的计算效率。

Conclusion: VideoChat3成功解决了现有开源模型的局限性，实现了泛化能力与计算效率的平衡，为视频理解领域提供了完全开源的高效解决方案。
...
TL;DR: 本文提出ReBind框架，通过嵌入参考标记的语义指令实现多参考图像条件下的视频编辑。


<details>
  <summary>Details</summary>
Motivation: 现有多参考图像条件视频编辑方法难以准确协调多个视觉源的信息，编辑指令缺乏明确的参考关系，且大多多多模态大语言模型无法可靠地生成此类关系。

Method: 提出ReBind系统框架，包含两个核心组件：ReBind-Instruct通过两阶段渐进式方案训练专用MLLM生成带参考标记的语义指令；ReBind-Edit通过轻量级适配将文本到视频模型与多参考源绑定。

Result: 实验表明ReBind在指令质量上显著优于通用MLLM，在参考图像条件视频编辑任务上达到开源方法的最新性能水平。

Conclusion: 通过在语义位置嵌入参考标记消除歧义并建立视觉属性与源的精确绑定，可有效解决多参考视频编辑中的信息协调问题。
...
TL;DR: 本文提出了VIABench，一个专门用于评估多模态大语言模型在视障辅助场景下表现的视频基准，揭示了当前模型在主动提醒等任务中的局限性。


<details>
  <summary>Details</summary>
Motivation: 视障人群因缺乏视觉信息在日常生活中面临挑战，而现有的多模态大语言模型在真实世界视障辅助场景中的实际效用仍未被充分探索。

Method: 引入VIABench基准，使用第一人称视频定义了主动提醒、视觉问答和视觉引导交互三个核心任务，并提出了支持在线和离线设置的严格基准测试流程。

Result: 实验表明，当前的多模态大语言模型仍难以为视障人群提供全面支持，尤其在需要准确预期和实时响应的主动提醒任务上表现不佳。

Conclusion: VIABench旨在推动未来开发用于真实世界辅助的定制化多模态大语言模型，从而最终改善视障人群的导航和交互体验。
...
TL;DR: 引入Ego Scene Augmentation (ESA)框架，利用Ego-element Graph增强多模态大语言模型在自我中心视觉问答中的空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型由于空间感知能力有限，在复杂的自我中心场景中难以进行有效的空间推理。

Method: 提出ESA框架，通过视觉基础模型构建包含自我中心空间特征的Ego-element Graph作为中间表示，以此增强MLLMs的空间感知能力。

Result: 在EgoTextVQA基准测试上取得了显著提升，室内和室外设置分别提高了8.14%和8.72%，且在室内设置的购物子集中提升最为明显。

Conclusion: ESA框架有效解决了MLLMs在自我中心场景下的空间推理问题，显著提升了性能，且项目代码已开源。
...
TL;DR: 本文提出了SciDiagramEdit，一个通过自然语言指令编辑科研图表的基准测试和技能演化框架，证明了自然论文修订可作为指令驱动图表编辑的有效训练信号。


<details>
  <summary>Details</summary>
Motivation: 科研论文图表的编辑通常耗时且繁琐，而由于科学图表是由异构视觉元素在严格视觉语法下组成的密集信息图，使用自然语言指令自动执行此类编辑工作流面临巨大挑战。

Method: 提出SciDiagramEdit框架，从arXiv版本历史中挖掘带有作者修订意图的修改前/后图表对构建基准，并在图表可编辑矢量源上运行；采用基于技能演化的智能体学习方法，由智能体提议者根据多轮执行轨迹不断优化技能规范以适应多样化的编辑指令。

Result: 在留出验证集上，经演化的技能逐步提升了图表编辑的准确率。

Conclusion: 自然论文修订可作为指令驱动图表编辑的有效训练信号，该框架能在图表可编辑矢量源上运行并允许用户对单个基元进行检查与协同编辑。
...
TL;DR: 本文提出了CoCommit，一种用于扩散大语言模型的标记门控协调机制，通过近似联合模式解码来解决多令牌独立提交时的因子分解误差问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在单个去噪步骤中独立解码并提交多个令牌，当这些位置存在依赖关系时会产生因子分解误差，而现有的基于置信度的选择方法无法从边际分布中观察到这一误差。

Method: 提出CoCommit方法，在常规束选择后引入一个学习的标记宣布提交集，并重新应用骨干网络的最后n层使标记位置在贪婪argmax写入前进行协调，从而近似联合模式解码。该方法重用现有权重，仅需一次额外的部分前向传播，无需辅助模型。

Result: 在LLaDA2.1-mini模型（使用LoRA适配器和匹配的贪婪推理）上，联合提交在所有六个评估基准上均提高了准确性，其中在推理和精确答案任务上的提升最为显著。

Conclusion: 通过轻量级的标记门控协调机制解决多令牌提交中的依赖性问题，能够有效提升扩散大语言模型的解码准确性，且无需引入额外的模型参数。
...
TL;DR: 本文提出了dOPSD方法，通过从模型自身的去噪轨迹中推导特权信息，有效提升了扩散大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在通过后训练提升推理能力时面临挑战：监督微调存在暴露偏差且脱离策略，强化学习奖励稀疏且难以应用，而在策略自蒸馏（OPSD）又过度依赖推理时不可用的外部特权信息（如真实标签），导致蒸馏效果受限。

Method: 提出dOPSD方法，不依赖外部真实标签，而是直接从学生模型自身的去噪轨迹中获取特权信息。具体而言，利用同一轨迹中后续更充分的解码步骤来评估掩码位置，使教师模型的优势从模型自身的解码过程中产生。

Result: 在Dream和LLaDA模型上的实验表明，dOPSD不仅提升了领域内的数学推理能力，还改善了领域外的代码生成能力，且整体表现优于监督微调和在策略基线方法。

Conclusion: 通过内部去噪轨迹生成特权信息的自蒸馏策略，能够有效克服传统在策略自蒸馏在dLLM中的局限性，为提升扩散大语言模型的复杂推理能力提供了新方向。
...
TL;DR: 本文提出了一种无需训练的门控解码器TACG，利用轨迹感知信号来决定扩散语言模型的token提交时机，从而提高效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散语言模型解码器忽略了生成轨迹，仅基于当前快照提交token，混淆了瞬态置信度与实际提交就绪度，导致过早锁定token或延迟一致的候选词。

Method: TACG结合了时间隐式对数引导（TILG）和历史门（HG），前者通过对比当前对数与历史移动平均值来提供自参考，后者强制执行短期提议持久性，从而形成一种无需辅助网络的稳定性约束提交规则。

Result: 在LLaDA、Dream和LLaDA2-Mini模型上的代码和数学基准测试表明，TACG在减少去噪步骤和增加每次前向传播token数（TPF）的同时，通常能提高或保持准确性。

Conclusion: TACG通过轨迹感知信号成功将置信度与提交就绪度解耦，为扩散语言模型提供了一种高效且稳定的解码方案，且无需额外训练或前向传播开销。
...
TL;DR: 本文提出SLIM-RL方法，通过风险控制和方差缩减技术，无需重建轨迹即可解决扩散语言模型中随机掩码与推理轨迹不匹配的问题，性能超越现有最优方法TraceRL。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的TraceRL方法认为随机掩码与模型推理轨迹不匹配，需通过切片rollout重建轨迹，但这带来了随块大小K线性增长的训练成本。本文旨在消除这一高昂的轨迹重建成本。

Method: SLIM-RL使用τ-budget解码器限制每个rollout步骤的提交风险，减少训练数据中的总体风险；在优化阶段采用无轨迹的随机掩码目标函数，结合序列级重要性采样和新提出的均值保持、单调递减的每块掩码调度下的确定性求积来实现方差缩减。

Result: 在SDAR-4B上，SLIM-RL仅用0.46倍训练样本即匹配TraceRL在MATH500的最佳准确率；在匹配动态采样下，MATH500提升6.32%，GSM8K提升11.05%。4B模型在数学任务上超越LLaDA-8B和Dream-7B，代码任务MBPP和HumanEval分别提升4.20%和3.65%。τ-budget解码器可跨模型免训练迁移。

Conclusion: SLIM-RL证明了无需轨迹重建即可有效缓解掩码不匹配问题，通过风险控制与方差缩减实现了更高效的强化学习训练，在多个基准上取得显著提升，且所提解码器具备良好的跨模型泛化能力。
...
TL;DR: 本文研究了扩散大语言模型解码方法评估中的不一致性问题，发现评估结果对提示模板高度敏感，并提出了可靠的评估指南。


<details>
  <summary>Details</summary>
Motivation: 现有关于dLLMs高效解码策略的研究在相同评估设置下报告了不一致的评估结果，可能导致对解码方法产生偏见结论，因此需要理解并解决这一评估问题。

Method: 在多种评估设置下对当前dLLMs的解码方法进行严格评估，分析提示模板及被忽视的评估设置对并行解码方法表现的影响。

Result: 解码方法排名对提示模板高度敏感，单一模板评估会造成并行解码提升效率且无损性能的错觉；实际上当前并行解码始终逊于单token解码基线；有效的提示模板比增加去噪步骤能带来更显著的性能提升。

Conclusion: 当前并行解码方法未能克服速度-质量权衡，评估解码方法时必须谨慎处理提示模板等评估设置；作者基于实验结果提出了dLLMs解码方法可靠评估的实用指南。
...
TL;DR: 提出了一种无需训练的推测解码方法DominoTree，通过利用Domino的条件非分解校正构建最优草稿树，显著提高了LLM推理速度和接受长度。


<details>
  <summary>Details</summary>
Motivation: 现有的最优优先树方法（如DDTree）无法表示Domino草稿器中基于GRU的路径相关因果校正分布，本文旨在解决如何有效利用这种非分解的条件校正来构建草稿树的问题。

Method: 提出DominoTree，一种无需训练的最优优先草稿树，沿根到节点路径使用Domino的条件非分解校正进行评分，并将每个节点的校正限制在候选top-M集合中；同时使用GPU原生的CUDA图构建器低成本构建树。

Result: 在Qwen3-4B上达到最高6.6倍加速，在所有测试温度下获得最高平均接受长度（高达10.7个token/轮）；吞吐量比Domino提升9-10%（Alpaca上高达22%），在所有温度下均优于DDTree和CaDDTree。在Qwen3-8B上保持最高接受长度，T=0时比DDTree提升24%吞吐量。

Conclusion: DominoTree通过结合路径相关草稿器和高效的GPU原生树构建器，在推测解码中实现了显著的吞吐量提升和高接受率，尤其在低温下表现优异，并在高温下保持整体增益。
...
TL;DR: 提出CaDDTree方法，通过联合优化树结构和节点预算直接提升推测解码的Token吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如DDTree）以最大化接受长度为目标，导致其总是偏向更大的树而忽略验证成本，缺乏合理选择节点预算的依据。

Method: 引入成本感知扩散草稿树（CaDDTree），显式建模草稿与验证延迟，将吞吐量优化转化为逐轮一维预算搜索；在凸验证成本假设下证明吞吐量函数单峰性，从而实现无需离线搜索的高效贪婪停止规则并动态调整预算。

Result: 在Qwen3-4B和Qwen3-8B上的八个涵盖推理、编程和指令遵循的基准测试中，CaDDTree在几乎所有任务上均达到或超越了采用预言机预算选择的DDTree。

Conclusion: CaDDTree克服了现有方法忽略验证成本的缺陷，通过动态自适应预算选择实现了高效且原则性的吞吐量优化，有效加速了语言模型推理。
...
TL;DR: 本文提出DDTree方法，通过从块扩散草稿模型的逐位置分布构建草稿树，突破了单轨迹验证的限制，提升了推测解码的性能与接受长度。


<details>
  <summary>Details</summary>
Motivation: 原生DFlash推测解码方法在每轮验证中仅处理单条草稿轨迹，这限制了词元的接受长度和整体的加速潜力，本文旨在解决该瓶颈。

Method: DDTree直接利用块扩散草稿模型的逐位置分布构建草稿树，在固定节点预算下采用最佳优先堆算法选择最可能匹配目标模型的延续，并通过仅祖先注意掩码在单次前向传递中完成树验证。

Result: 在DFlash基础上构建的DDTree克服了单轨迹验证的限制，有效提升了接受长度，成为当前领先的推测解码方法之一。

Conclusion: 将块扩散草稿模型与草稿树验证机制相结合，能够有效突破单轨迹验证的瓶颈，进一步提升推测解码技术的性能表现。
...
### [18] [Beyond Success Rate: Cost-Aware Evaluation of Offensive and Defensive Security Agents](http://arxiv.org/abs/2607.15263v1)
*Paul Kassianik,Blaine Nelson,Yaron Singer*

Main category: cs.CR

TL;DR: 本文提出从成本-成功视角评估安全智能体，揭示红蓝两队任务在成本扩展性上的显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有安全智能体评估仅关注在充足推理预算下的最佳攻击能力，忽略了实际安全运营中每个推理步骤、工具调用和遥测查询均消耗预算的问题，因此评估不够完整。

Method: 在攻击性Cybench挑战和防御性Splunk BOTS v1调查挑战上评估语言模型安全智能体，在固定成本水平下比较模型，并将性能按推理支出和工具支出进行分解。

Result: 攻击性CTF性能随测试时计算量增加而提升，规模化开源模型可接近前沿专有系统且成本竞争力强；防御性SOC调查则不呈相同扩展规律，成功更依赖于规范的工具使用、遥测导航和选择性富集，而非纯推理预算。

Conclusion: 安全智能体基准测试应在任务成功之外同时衡量经济效率与运营适配性，成本感知的SOC原生评估能更清晰揭示模型的实际可用性及防御智能体仍需改进之处。
...
### [19] [Bridge Evidence: Static Retrieval Utility Does Not Predict Causal Utility in Multi-Step Agentic Search](http://arxiv.org/abs/2607.15253v1)
*Debayan Mukhopadhyay,Utshab Kumar Ghosh,Shubham Chatterjee*

Main category: cs.IR

TL;DR: 本文指出在智能体检索中静态相关性与因果效用存在差异，提出反事实轨迹效用（CTU）来评估文档对多轮搜索的实际贡献，发现大量静态评分低但起关键作用的“桥接文档”。


<details>
  <summary>Details</summary>
Motivation: 传统检索系统评估依赖静态效用，即文档能否直接回答当前问题。但在大语言模型作为搜索智能体进行多轮推理时，某些文档的价值在于引导后续搜索而非直接提供答案。本文旨在量化静态评估与智能体多轮检索实际效用之间的差距。

Method: 提出反事实轨迹效用（CTU），通过删除智能体读取的文档并重新运行后续轨迹，比较原始与反事实轨迹在最终答案质量、后续查询检索质量和轮次数上的差异来评估文档效用。同时使用可观察实体相关性（OER）分析其机制。

Result: 实验表明CTU与静态RAG效用（SRU）几乎统计独立（Spearman rho = -0.026）。约三分之一的文档在静态评估下看似无用，但实际具因果支撑作用（桥接文档），这类文档通过提供区分性实体使智能体在后续查询中重定向搜索的频率高出4.02倍。

Conclusion: 在智能体检索中，静态相关性与因果效用是截然不同的量，仅优化静态相关性无法保证多轮搜索的实际效用。检索系统的评估与优化需转向考虑文档在多轮交互中的动态因果价值。
...
### [20] [MM-IssueLoc: A Controlled Benchmark for Evaluating Visual Evidence in Multimodal Repository-Level Issue Localization](http://arxiv.org/abs/2607.15205v1)
*Shaoxiong Zhan,Shi Hu,Boyu Feng,Hai Lin,Andrew Gong,Zhengda Zhou,Jiaying Zhou,Yunyun Hou,Hao Su,Hai-Tao Zheng*

Main category: cs.SE

TL;DR: 本文提出了MM-IssueLoc，一个用于评估带有视觉证据的代码库级问题定位的受控基准，发现现有系统在多模态定位上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有的代码库级问题定位评估通常只将其视为纯文本任务，忽略了问题中常包含的视觉证据；而现有的多模态软件工程基准将定位与补丁生成纠缠在一起，掩盖了视觉输入的实际作用。本文旨在解决如何单独评估视觉证据在问题定位中的影响这一问题。

Method: 引入了MM-IssueLoc基准及评估协议，包含23种语言的652个issue-PR实例，标注了7种图像类别和4个相关度级别。该基准提供文件级和函数级金标签，支持纯文本与带图像的配对评估，并采用基于VCE的诊断将图像转化为结构化文本证据。同时评估了基于LLM和基于检索的系统（包括受控多模态检索器MM-IssueLoc-VL-Emb）。

Result: 实验结果表明，现有系统在多模态代码库定位方面仍不可靠：表现最好的Agent在文件级Acc@5上达到38.96%，函数级Acc@10上为22.45%；最强检索器在函数级Acc@10上为33.86%。跨基准比较显示，在纯文本主导的SWE基准上的高定位得分无法直接迁移到多模态问题定位任务上。

Conclusion: MM-IssueLoc将视觉证据转化为显式的评估变量，使得未来研究能够测试系统是否真正通过利用视觉证据来提升定位能力，而不是依赖纯文本线索或下游补丁生成效果。
...
### [21] [DELTA-TTS: Adapting Autoregressive Model into Diffusion Language Model for Text-to-Speech](http://arxiv.org/abs/2607.04140v1)
*Junwon Moon,Seungbeom Kim,Yejin Lee,Hoseong Ahn,Sewoong Park,Heeseung Kim,Kyuhong Shim*

Main category: eess.AS

TL;DR: 本文提出了DELTA-TTS框架，通过将自回归TTS模型转化为离散扩散语言模型来实现置信度排序解码，从而提升推理速度并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 自回归文本转语音模型由于从左到右的生成机制，导致推理速度慢且易传播局部错误和幻觉，降低了鲁棒性；由于TTS合成前已有完整文本输入，顺序生成并非内在要求，因此需要解决该局限性。

Method: 提出基于轻量级LoRA的适应框架DELTA-TTS，将预训练的AR TTS模型转换为离散扩散语言模型进行置信度排序解码。该方法结合了注入局部声学上下文的卷积模块、1/t加权训练目标以及推迟低置信度位置的时间偏移推理计划。

Result: 在LibriTTS数据集上训练后，DELTA-TTS在Seed-TTS test-en测试集上达到1.75%的WER，优于其AR骨干模型，且token生成速度快3.3倍；同时生成了更清晰的文本-语音对齐，提高了解码置信度并减轻了幻觉。

Conclusion: DELTA-TTS证明了将自回归TTS转化为离散扩散模型的有效性，不仅显著提升了生成速度和鲁棒性，还改善了文本与语音的对齐质量并减少了幻觉现象。
...
TL;DR: 提出基于随机控制的连续时间强化学习框架，用于微调离散扩散模型，支持中间奖励并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 旨在解决连续时间下离散状态空间的强化学习问题，并克服现有基于GRPO方法仅依赖终端奖励以及计算轨迹似然成本高昂的局限性，以更好地微调离散扩散模型。

Method: 将状态动态建模为受控连续时间马尔可夫链（CTMC），推导出连续时间的PPO和GRPO算法，在去噪轨迹中引入中间奖励；针对掩码扩散模型提供解析可处理概率比的策略参数化，并为dLLM提出轨迹子采样技术以高效估计似然。

Result: 在低维熵正则化优化问题以及离散扩散大语言模型（dLLMs）在数学推理和编程任务上的强化学习后训练中，均验证了所提方法的有效性。

Conclusion: 该框架为掩码扩散模型中的探索与策略优化提供了统一视角，实现了无需奖励信号可微的奖励驱动优化，并通过子采样技术有效降低了计算开销。
...
TL;DR: 本综述提出了一个统一的延迟分解框架来解耦影响扩散大语言模型推理速度的因素，并系统性地梳理了各类加速技术。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽在并行生成上具理论优势，但实际部署难获加速，且现有基准测试混淆了算法、架构和系统级因素对端到端延迟的影响，导致严格比较困难。

Method: 引入统一的延迟分解框架以理清复杂权衡，并沿算法创新、架构与系统优化、推理时扩展三个轴线对加速技术进行分类分析。

Result: 构建了分析推理速度影响的框架，系统性地归类了现有加速技术，为理清各因素对实际部署延迟的影响提供了清晰视角。

Conclusion: 提供了可复现基准测试的指南，并强调了实现并行生成全部潜力的开放性挑战与未来研究方向。
...
TL;DR: 本文提出BlockServe连续批处理框架，通过块级调度和混合状态执行解决扩散语言模型的收敛异构性问题，显著提升推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在批处理时存在收敛异构性问题，不同序列收敛速率不同导致快速请求受慢速请求拖累，产生计算气泡和尾部延迟，降低服务效率。

Method: 提出BlockServe框架，结合块粒度调度（在块边界立即驱逐已完成请求）、混合状态执行（通过gather-scatter索引扩展双缓存和并行解码至异构批次）以及计算感知准入控制器（利用令牌预算补充扩大有效批处理容量）。

Result: 在五个基准测试中，BlockServe在Dream和LLaDA模型上的吞吐量比Fast-dLLM高1.9至10.6倍，且生成质量相当。

Conclusion: 块粒度调度是实现高吞吐量离线dLLM推理的基础，BlockServe成功消除了计算瓶颈并大幅提升了服务效率。
...
### [25] [SearchOS-V1: Towards Robust Open-Domain Information-Seeking Agent Collaboration](http://arxiv.org/abs/2607.15257v1)
...
TL;DR: 本文提出了SearchOS，一个系统级多智能体框架，通过将隐式搜索进度转化为显式、持久和共享的状态，解决了信息检索智能体陷入重复循环的问题。


<details>
  <summary>Details</summary>
Motivation: 随着交互历史的增加，基于工具集成的大语言模型信息检索智能体难以跟踪任务进度，在搜索失败时易陷入重复循环，浪费搜索预算并降低输出质量与完整性。

Method: SearchOS将信息检索建模为带引用的关系模式补全，设计搜索导向上下文管理（SOCM）将状态外化为前沿任务、证据图、覆盖图和失败记忆，并结合流水线并行调度机制与包含分层技能系统的搜索工具中间件来优化执行与避免重复失败。

Result: 在WideSearch和GISA数据集上，SearchOS在所有单智能体和多智能体基线评估的各项指标中均取得领先。

Conclusion: SearchOS有效提升了多智能体信息检索的鲁棒性和协作效率，为实现稳健的信息检索协作系统奠定了基础。
...
### [26] [AutoSynthesis: An agentic system for automated meta-analysis](http://arxiv.org/abs/2607.15247v1)
*Moein Taherinezhad,Sebastian Maier,Gerardo Vitagliano,Francesco Pierri,Stefan Feuerriegel*

Main category: cs.AI

TL;DR: AutoSynthesis是一个端到端的多智能体系统，能够实现从自然语言研究问题到自动元分析的全流程自动化。


<details>
  <summary>Details</summary>
Motivation: 传统的定量证据综合主要依赖人工操作，难以规模化，该论文旨在解决这一瓶颈，以支持跨学科的循证决策。

Method: 提出AutoSynthesis多智能体系统，输入自然语言研究问题后，自动完成制定搜索策略、检索文献、筛选研究、评估全文合格性、提取定量数据、计算标准化效应量并执行随机效应元分析；支持异质性分析和偏倚风险评估，并生成符合PRISMA指南的报告。

Result: 在应用测试中，AutoSynthesis筛选了超过28项研究并提取了20多个定量声明，其生成的汇总效应估计值与专家手动元分析的结果高度一致。

Conclusion: AutoSynthesis能显著提升定量证据综合的可扩展性，从而支持科学、医学、教育和政策等领域的循证决策。
...
### [27] [When Words Are Safe But Actions Kill: Probing Physical Danger Beyond Text Safety in Hidden-State Risk Space](http://arxiv.org/abs/2607.15218v1)
*Weimeng Wang,Ziqiang Wang,Zihang Zhan,Chuanpu Fu,Qi Li,Ke Xu*

Main category: cs.AI

TL;DR: 本文提出PRISM探测方法，通过分析大语言模型的隐藏状态来区分并检测具身智能体中基于物理世界的安全风险。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为具身规划器时，文本无害的指令在物理世界可能引发危险。现有方法难以区分文本内容危险与物理危险，容易导致对安全任务的过度拦截。

Method: 通过隐藏状态方向分析证实内容危险和物理危险信号可分离，进而提出基于全隐藏状态的单层L2正则化逻辑探测方法PRISM，并引入无直接伤害关键词的物理风险对比基准PSB-1K。

Result: PRISM在SafeAgentBench上准确率达86.2-87.7%、误报率为11.7-13.7%，优于LLM审查器；在PSB-1K上准确率达99.6%、误报率仅0.7%，远超误报率极高的Qwen2.5-3B审查器。该方法在SafeText和EARBench上也得到验证。

Conclusion: 隐藏状态探测作为一种表示级方法，能有效识别物理安全风险，超越了传统文本审核的局限。
...
### [28] [Plover: Steering GUI Agents through Plan-Centric Interaction](http://arxiv.org/abs/2607.15193v1)
*Madhumitha Venkatesan,Shicheng Wen,Jiajing Guo,Jorge Piazentin Ono,Liu Ren,Dongyu Liu*

Main category: cs.AI

TL;DR: 本文提出了Plover，一个以计划为中心的视觉GUI自动化系统，通过将任务计划和重新规划外化为可检查、可修改的工件，提升了GUI自动化的透明度和可控性。


<details>
  <summary>Details</summary>
Motivation: 在动态布局、意外弹窗等真实环境挑战下，自主GUI代理容易偏离用户意图，而现有视觉多模态代理的内部规划限制了用户的监督和纠错能力，本文旨在解决这一缺陷。

Method: 采用规划器-执行器架构，将任务计划作为持久且可编辑的工件进行外部化，支持局部修正、自然语言引导、基于截图的干预，并在修复过程中保留已有进展。

Result: 基准失败案例修复和基于场景的工作流分析表明，当计划可见且干预局部化时，许多自主GUI代理失败在结构上是可修复的。

Conclusion: 显式重新规划与外部化规划使得GUI自动化过程更加透明、可控和适应性强，为未来提升人机协作自动化提供了有效路径。
...
TL;DR: 本文提出目标感知前缀选择方法（TAPS），通过在固定预算下选择紧凑子树来优化扩散模型推测解码的验证过程，显著提升解码速度。


<details>
  <summary>Details</summary>
Motivation: 在扩散模型推测解码中，验证过程成为新瓶颈。现有方法基于边缘概率排序节点，忽略了验证的前缀条件性，导致验证被拒绝前缀的不可达后代，增加延迟且接受率提升有限。

Method: 提出TAPS方法，将扩散边缘概率转化为路径条件下的接受估计，并在固定验证预算下选择紧凑的前缀闭包子树，从而优化接受率与成本的权衡。

Result: 在多种数据集和模型上的实验表明，TAPS相比自回归解码实现了高达7.9倍的无损端到端加速，且分别比SOTA方法DFlash和DDTree快1.36倍和1.74倍。

Conclusion: TAPS通过优化子树选择而非单纯扩展草稿树，有效解决了扩散模型推测解码中的验证瓶颈，实现了更优的接受率与成本权衡。
...
### [30] [Benchmarking Multimodal Large Language Models for Scientific Visualization Literacy](http://arxiv.org/abs/2607.15176v1)
*Patrick Phuoc Do,Chau M. Ta,Chaoli Wang*

Main category: cs.AI

TL;DR: 本文通过标准化的科学可视化素养测试对六个多模态大语言模型进行了基准评估，发现其表现不均衡且存在特定短板。


<details>
  <summary>Details</summary>
Motivation: 当前对多模态大语言模型的评估主要集中在图表方面，缺乏其对科学可视化理解能力的证据。

Method: 基于包含49个项目、18种科学可视化的标准化测试，在封闭世界协议下评估3个闭源和3个开源模型，并与485名人类参与者的数据进行对比。

Result: 模型素养表现不均；Gemini整体表现超越人类均值，开源模型低于人类基线；模型在插图、搜索和空间理解上表现较好，但在纹理、集成可视化和定量估计上表现较差，并存在精细定量估计和流向解释等重复性错误。
...
### [31] [TopoAgent: A Self-Evolving Topological Agent for Multimodal Scientific Reasoning](http://arxiv.org/abs/2607.14658v1)
*Mingze Xu,Yinghui Li,Jiayi Kuang,Zhanhui Kang,Di Yin,Ying Shen,Xing Sun,Yuxing Han*

Main category: cs.AI

TL;DR: 本文提出了TopoAgent，一个自演化的拓扑框架，通过动态的有向无环图和自适应原子裂变机制来解决多模态大语言模型在科学推理中的线性规划局限性。


<details>
  <summary>Details</summary>
Motivation: 旨在解决多模态大语言模型在严谨科学推理中因单体线性规划导致的视觉语义不对齐、长上下文幻觉以及在固定任务粒度下执行脆弱的问题。

Method: 提出TopoAgent框架，利用前端分解器将复杂查询分解为视觉锚定的原子，并根据依赖关系构建有向无环图（DAG）以实现上下文隔离；同时引入自适应原子裂变机制，在运行时动态拆分瓶颈节点为更细粒度的子原子。

Result: 在数学、物理和化学基准测试上的广泛实验表明，TopoAgent显著优于目前最先进的线性智能体框架。

Conclusion: TopoAgent为自主科学推理提供了一个鲁棒、抗噪和自我纠错的全新范式。
...
### [32] [Contextualized Evaluation of Vision Language Models through Dynamic, Multi-turn Interactions](http://arxiv.org/abs/2607.14499v1)
*Yijiang Li,Huiqi Zou,Bingyang Wang,Ziang Xiao*

Main category: cs.AI

TL;DR: 本文提出CEDI框架，通过动态多轮交互的三方评估范式，弥合多模态大语言模型静态基准测试与真实世界动态应用之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs基准测试在受控静态环境与动态、交互式、情境化的真实世界应用之间存在根本错位，导致模型在实际应用中的有效性不确定，亟需更真实的评估方法。

Method: 提出CEDI框架，将评估重构为被评估模型、自动考官和评分员的三方交互；考官基于任务图表示进行多轮半结构化对话，通过状态空间转换部署从澄清请求到对抗性探测的多种策略以收集性能证据。

Result: 在视觉幻觉任务上的多模型多领域实验表明，与传统静态评估相比，该交互式评估发现了显著更多且更贴近实际应用的幻觉；同时发现幻觉会在长上下文自我强化历史中累积，且模型在需要拒绝前提的问题上极为脆弱。

Conclusion: CEDI框架是迈向对MLLMs能力进行现实、系统和生态有效性评估的重要一步，为模型的真实世界性能评估提供了新的范式。
...
### [33] [Automatic Hard Example Synthesis with Multi-Level Agentic Data Curation](http://arxiv.org/abs/2607.14256v1)
...
TL;DR: 本文提出了一种自动化的多智能体红队测试框架，通过自主合成对抗样本并在测试时检索，无需人工标注即可显著提升多模态大模型的内容安全鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在内容安全任务中易受对抗性攻击和分布外边缘案例的影响，而传统的主动学习和人工标注方法无法有效应对复杂且海量新型多模态威胁的规模。

Method: 提出一种自动化的多智能体红队测试框架，采用提出新假设和基于过往尝试进行变异的迭代策略。系统包含高推理能力的架构师智能体、图像生成器和多级LLM验证委员会，并利用测试时检索将合成的对抗样本作为上下文示例。

Result: 在公共图像安全基准测试中，无需任何人工标注，该方法将目标模型的假阴性率（FNR）从41.2%显著降低至24.5%。

Conclusion: 该框架能够无需人工干预地自主发现政策边缘情况并合成对抗样本，从而有效提升模型的鲁棒性，为应对复杂多模态威胁提供了可扩展的解决方案。
...
TL;DR: 本文提出了Sangam，一个用于缓存扩散语言模型推理的服务系统，通过赤字令牌预算调度器和混合服务策略实现了无停顿调度并优化了不同负载下的延迟。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型的近似KV缓存技术会导致重复的预填充/解码结构，使得现有的自回归服务机制无法直接适用，因此需要针对dLLM块级解码和双向注意力特性设计高效的推理服务系统。

Method: 提出Sangam系统，引入赤字令牌预算调度器（优先处理进行中的解码，在预算允许时处理不可分割的预填充，并结转未用预算）以实现无停顿调度，同时采用混合服务策略，将溢出的预填充转移到解码工作节点并利用调度器保护解码过程。

Result: 在LLaDA-8B ShareGPT解码密集型负载上，协同部署较混合执行降低9-20%平均延迟；在Dream-7B arXiv预填充密集型负载上，混合执行较协同部署降低8-20%平均延迟。

Conclusion: dLLM服务设计空间受预填充/解码干扰和资源划分主导，协同部署最适合解码密集型负载，而混合执行最适合预填充密集型负载。
