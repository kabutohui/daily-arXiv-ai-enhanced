...
TL;DR: 提出UAV-DualCog基准，用于评估多模态大语言模型在无人机多视角时空场景中的双认知能力。


<details>
  <summary>Details</summary>
Motivation: 现有无人机基准未联合评估无人机代理在多视角时空背景下对自身状态和外部环境的推理能力，存在评估空白。

Method: 构建包含图像和视频任务的UAV-DualCog基准，要求空间或时间定位；开发基于场景级语义点云的自动化数据构建流程生成多样化问答样本。

Result: 当前MLLMs在无人机双认知方面不可靠，自身状态推理、视角转换、空间定位和时间定位是主要瓶颈；基准对人类可理解但对模型具有挑战性。

Conclusion: UAV-DualCog既可作为评估基准，也可作为训练资源推进基于MLLM的无人机代理发展。
...
### [2] [Searching Videos as Trees: Self-Correcting Agents for Grounded Long Video QA](http://arxiv.org/abs/2607.16189v1)
*Ce Zhang,Ziyang Wang,Yulu Pan,Oluwatumininu Oguntola,Pranav Wagh,Qiyu Wu,Hiromi Wakaki,Mohit Bansal,Gedas Bertasius*

Main category: cs.CV

TL;DR: 本文提出VideoTreeSearch (VTS)框架，将长视频问答建模为自适应时间树上的迭代自校正搜索，通过四个离散操作实现回溯和恢复，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有智能体方法在长视频问答中仅使用单一的crop_video动作进行粗到细的搜索，缺乏细到粗的回溯机制，导致过早收敛且无法从早期错误中恢复。

Method: VTS从视觉场景边界构建非均匀时间树，训练智能体通过zoom_in、zoom_out、shift和answer四个操作导航；引入轨迹合成管道生成包含错误分支和恢复的多步路径，采用监督微调和强化学习结合训练。

Result: 在CG-Bench上超越基线+12.5 mIoU，在Haystack-Ego4D上+7.4 T-F1；还能迁移到通用长视频问答，在Video-MME、MLVU和LVBench上提升高达+7.1准确率点；消融实验证明自适应下降和显式回溯均不可或缺。

Conclusion: 自校正层次搜索是性能提升的核心机制；移除自适应下降或显式回溯均会显著降低性能，验证了该设计的必要性。
...
TL;DR: 本文提出了ActiveVision基准以衡量多模态大语言模型的主动观察能力，发现当前前沿模型表现极差且远落后于人类。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言基准测试无法衡量多模态大语言模型（MLLMs）是否具备主动观察能力。本文旨在验证这些模型能否进行持续的、闭环的视觉感知，而非仅依赖单次静态快照。

Method: 引入了ActiveVision基准测试，包含3大类共17项任务，专门设计以迫使模型进行反复的视觉感知，而不是进行单一的静态描述。

Result: 前沿MLLMs在该基准上表现崩溃：得分最高的GPT-5.5仅解决10.6%的问题并在17个任务中的11个得分为零，Claude Fable 5仅解决3.5%，而人类参与者平均解决96.1%。即使模型编写并运行视觉代码也难以可靠地缩小差距。

Conclusion: 当前MLLMs缺乏稳健的主动视觉观察能力，这表明未来需要开发能够闭合感知-推理环路的新架构和训练目标。
...
### [4] [GeoChrono: Benchmarking and Rethinking Long-Term Temporal Understanding in Remote Sensing](http://arxiv.org/abs/2607.15768v1)
*Yujie Li,Jiancheng Pan,Zhiwei Wei,Jiuniu Wang,Mugen Peng,Wenjia Xu*

Main category: cs.CV

TL;DR: 本文提出了遥感地表演化多维认知基准ChronoBench，并基于此开发了GeoChrono模型，通过时间轨迹编码和粗到细Token压缩技术显著提升了模型在长期地理演化任务中的性能与效率。


<details>
  <summary>Details</summary>
Motivation: 遥感地表演化分析要求模型具备地表感知、变化追踪、历史记忆和时空推理等综合能力，但现有研究缺乏对这些能力的系统性评估，且主流多模态大语言模型在长期记忆方面存在严重瓶颈。

Method: 提出了包含4个认知级别和1.7万个QA对的ChronoBench基准；开发了GeoChrono模型，设计了基于地理位置固定先验的时间轨迹编码器以及自适应保留动态区域、压缩静态背景的粗到细Token压缩器；并构建了10.4万样本的指令微调数据集进行训练。

Result: 主流MLLM在评估中大幅落后于人类专家；GeoChrono达到SOTA，超越领先商业模型20%以上；粗到细Token压缩器减少了56%以上的视觉token，同时保留了94.6%的模型性能。

Conclusion: 结合地理物理先验与针对性架构设计能有效突破MLLM在遥感时空演化分析中的长期记忆瓶颈，在大幅提升性能的同时实现视觉token的高效压缩。
...
### [5] [Personalized Image Aesthetic Assessment via Preference-rich Sample Mining and Cohort Merging](http://arxiv.org/abs/2607.15752v1)
*Zhichao Yang,Tianjiao Gu,Zhixianhe Zhang,Xiangfei Sheng,Pengfei Chen,Leida Li*

Main category: cs.CV

TL;DR: 本文提出了一种基于多模态大语言模型的PRAC方法，通过挖掘偏好丰富样本和融合审美共振群体来有效建模个性化图像审美偏好。


<details>
  <summary>Details</summary>
Motivation: 个性化图像审美评估中个体偏好差异显著且呈现群体特定模式，论文旨在解决如何利用有限的用户数据有效捕捉和建模这些个性化偏好模式的问题。

Method: 提出PRAC方法：首先通过分析图像的集体争议性和个性化偏差挖掘偏好丰富样本以最大化有限数据效用；接着基于偏好嵌入测量跨用户相似度；最后采用基于群体的模型融合策略，聚合审美共振用户的偏好模式以增强目标个体的个性化评估。

Result: 在四个基准PIAA数据库上的广泛实验证明，所提出的PRAC模型性能优于现有最先进的方法。

Conclusion: 通过结合偏好丰富样本挖掘和审美共振群体模型融合，能有效提升个性化图像审美评估的准确性，且相关代码和模型将开源供未来研究使用。
...
### [6] [Debiasing Text-to-Image Evaluation via Implicit Cultural Alignment Reward Modeling](http://arxiv.org/abs/2607.15740v1)
*Bo-An Chang,Yu-Chih Chen*

Main category: cs.CV

TL;DR: 本文提出了一个基于轻量级多模态大语言模型的隐式文化对齐奖励模型，用于高效评估文生图系统中的文化真实性。


<details>
  <summary>Details</summary>
Motivation: 现有T2I评估指标和多模态评判器依赖的视觉语义表示未能充分捕捉隐式文化规范，导致偏好判断存在偏差且遗漏细粒度文化线索；同时基于VQA的评估器依赖自回归文本生成，限制了实时奖励建模的可扩展性。

Method: 构建了一个基于42亿参数MLLM的隐式文化对齐奖励模型，集成隐式文化探针与跳跃连接交叉注意力机制，使后期语义特征能直接关注早期视觉表示，从而更好地保留文化显著细节。

Result: 在CulturalFrames基准的3323对图像上，模型实现80.54%的成对准确率，Pearson和Kendall相关系数分别为0.546和0.377，优于代表性视觉语言指标和MLLM评估器；单次评估耗时0.21秒，比标准VQA评估器快10倍。

Conclusion: 该奖励模型可为RLHF和DPO等偏好优化流程提供高效且文化感知的标量信号，具有实际应用价值。
...
### [7] [Efficient Frame Selection for Long Videos at Test Time with Attention-Based MLLM Selectors](http://arxiv.org/abs/2607.15689v1)
*Yilin Wang,Xiangxi Zheng,Dongxing Mao,Linjie Li,Zhengyuan Yang,Ping Yu,Rui Yan,Yuan Yao,Alex Jinpeng Wang*

Main category: cs.CV

TL;DR: 本文提出DAFS，一种无需训练的帧选择方法，利用MLLM中跨模态注意力来动态选择与查询相关的视频帧，从而高效理解长视频。


<details>
  <summary>Details</summary>
Motivation: 长视频理解中存在循环依赖问题：选择关键帧需要先理解视频，而理解视频又需要先选择关键帧。本文旨在打破这一循环依赖，实现查询相关的帧选择。

Method: DAFS利用轻量级MLLM在验证选择的提取层上的跨模态注意力，通过查询条件聚合将注意力转换为帧相关性分数，无需自回归解码即可进行跨帧比较。同时将候选池大小和每帧token预算的联合分配建模为离散优化问题，用动态规划求解。

Result: 在32帧预算下，DAFS在Video-MME上比均匀采样最高提升6.4分，并在相同帧预算下优于先前的基于训练的帧选择方法，同时在不同选择器和回答器骨干网络及不同任务间具有良好的泛化能力。

Conclusion: DAFS证明了利用MLLM内部注意力进行帧选择的可行性，无需训练即可高效解决长视频理解的帧选择问题，为长视频多模态理解提供了实用且通用的解决方案。
...
### [8] [SLAPBench: Benchmarking Multimodal Large Language Models for Four-Finger SLAP Fingerprint Verification](http://arxiv.org/abs/2607.15517v1)
*Bibesh Pyakurel,M. G. Sarwar Murshed*

Main category: cs.CV

TL;DR: 本文提出SLAPBench，首个用于评估多模态大语言模型在四指SLAP指纹身份验证任务上的基准，揭示提示策略控制模型崩溃，而模型能力决定判别力。


<details>
  <summary>Details</summary>
Motivation: 四指SLAP指纹在边境管控和执法中用于身份验证，但此前缺少评估多模态大语言模型能否从SLAP图像进行身份验证的基准，填补该空白有助于推动该领域研究。

Method: 基于NIST SD302b构建SLAPBench，包含7,832对指纹（176对匹配，7,656对不匹配），在零样本、任务描述和相似度评分三种提示下评估InternVL3-8B、Qwen2.5-VL-7B、Qwen3-VL-8B、Gemma-3-12B及Claude Opus 4.8等模型，并设计匹配分辨率控制实验和公平性探针。

Result: 任务描述提示使所有开源模型崩溃至近100%误识率，仅Claude Opus 4.8抗崩溃（FAR=20.2%）；相似度评分消除崩溃，Claude AUC达0.953，Gemma为0.837，InternVL3-8B反转（0.590），Qwen2.5-VL-7B接近随机（0.567）；Qwen3-VL-8B达AUC=1.000，但归因于SD302b中匹配对为跨分辨率单次采集双重渲染，疑似近重复检测而非真实能力；公平性探针显示判别力越弱群体差异越大。

Conclusion: SLAPBench建立了首个SLAP专用的MLLM基线，表明提示策略决定模型是否崩溃，模型能力决定判别力，为后续指纹验证MLLM研究提供了基准与方法论启示。
...
TL;DR: 提出CoCommit方法，通过重用模型权重的部分前向传播来协调多令牌解码，解决扩散大语言模型中的分解误差问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在每个去噪步骤中独立解码多个令牌会导致分解误差，而基于置信度的选择方法无法仅从边缘分布中观察到此误差。

Method: 提出CoCommit标记门控协调机制，在常规束选择后短暂延迟提交，通过学习标记确定提交集并重新应用骨干网络最后n层使标记位置协调（近似联合模式解码），再进行贪心写入。该方法仅需一次额外部分前向传播且无需辅助模型。

Result: 在LLaDA2.1-mini及LoRA适配器上评估，联合提交在所有六个基准上均提升了准确率，在推理和精确答案任务上提升最大。

Conclusion: CoCommit有效缓解了扩散语言模型中的多令牌分解误差，在无需辅助模型的情况下显著提升了推理和精确匹配任务的性能。
...
TL;DR: 提出dOPSD方法，通过从学生模型自身的去噪轨迹中提取特权信息来进行在策略自蒸馏，从而有效提升扩散大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在后训练中难以激发强推理能力：监督微调存在曝光偏差，强化学习奖励稀疏且难以应用，而在策略自蒸馏依赖推理时不可用的外部特权信息，导致蒸馏效果受限。

Method: 提出dOPSD方法，直接从学生模型自身的去噪轨迹中推导教师模型的特权信息。该方法利用同一轨迹中更靠后、解码更充分的步骤来评估掩码位置，使教师模型的优势从模型自身的解码过程中涌现，无需外部标签。

Result: 在Dream和LLaDA模型上的实验表明，dOPSD不仅提升了域内数学推理能力，还增强了域外代码生成能力，整体表现优于监督学习和在策略基线方法。

Conclusion: dOPSD成功突破了在策略自蒸馏对外部特权信息的依赖，利用模型自身解码轨迹提供密集的token级监督，为扩散大语言模型的后训练和推理能力提升提供了新范式。
...
TL;DR: 本文提出了一种无需训练的轨迹感知门控解码器（TACG），通过利用历史预测分布轨迹来决定扩散语言模型中的词元提交时机，在保持准确率的同时提升了解码效率。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散语言模型解码器仅依赖当前快照提交词元，忽略了预测分布的轨迹变化。这导致模型容易混淆置信度与提交准备度，过早锁定不完整上下文下的瞬时高置信词元，而延迟具有跨步骤一致性的候选词元。

Method: 提出轨迹感知提交门控（TACG）解码器。该门控将词元身份锚定在基础后验分布上，并结合时间隐式对数几率引导（TILG，使用历史对数几率的指数移动平均作为自参考进行对比）与历史门控（HG，要求提交前具有短期提议持久性）。此外，引入了有上限的额外提升预算，构建了无需辅助网络或额外前向传播的稳定性约束提交规则。

Result: 在LLaDA、Dream和LLaDA2-Mini模型上的代码和数学基准测试表明，TACG通常能提高或保持准确率，同时显著减少去噪步骤并增加每次前向传播生成的词元数（TPF）。

Conclusion: TACG通过引入基于历史轨迹的稳定性约束机制，有效解决了扩散语言模型解码中过早提交的问题，在准确性与推理效率之间取得了更优的平衡。
...
TL;DR: SLIM-RL通过风险控制和无轨迹随机掩码目标，解决了扩散大语言模型强化学习中随机掩码与推理轨迹不匹配的问题，大幅提升了训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的轨迹感知强化学习方法（如TraceRL）在训练时需要重建推理轨迹，导致计算成本随块大小增加而上升。本文旨在解决随机掩码与推理轨迹不匹配的问题，同时避免昂贵的轨迹重建。

Method: 提出SLIM-RL，使用tau-budget解码器限制每个 rollout 步骤的提交风险；在优化阶段，采用无轨迹随机掩码目标函数，结合序列级重要性采样、确定性求积法以及一种新提出的均值保持且单调递减的块级掩码调度策略。

Result: 在SDAR-4B模型上，SLIM-RL仅用0.46倍的训练样本即匹配了TraceRL在MATH500上的最佳准确率，且在匹配动态采样下于MATH500和GSM8K上分别提升6.32%和11.05%。在块大小为4时，4B模型在数学和代码任务上超越了LLaDA-8B和Dream-7B等更大模型。tau-budget解码器还展现出良好的零样本跨模型迁移能力。

Conclusion: SLIM-RL无需重建轨迹即可有效缓解扩散语言模型强化学习中的轨迹不匹配问题，在提升训练效率的同时实现了更优的数学和代码生成性能。
...
TL;DR: 本文发现扩散大语言模型解码方法的评估结果对提示模板高度敏感，揭示了单模板评估的误导性，并提出了可靠的评估指南。


<details>
  <summary>Details</summary>
Motivation: 现有关于dLLM高效解码策略的研究在相似评估设置下报告了不一致的结果，存在得出偏见结论的风险，因此需要深入理解这一评估问题的根源。

Method: 在多种评估设置下对当前dLLM解码方法进行严格评估，系统分析解码方法排名对提示模板和被忽视的评估设置的敏感性。

Result: 解码方法排名对提示模板选择高度敏感；单模板评估会制造并行解码无损提升效率的错觉；当前并行解码方法始终劣于单令牌解码基线；有效提示模板用更少去噪步骤即可显著优于增加步骤的边际收益。

Conclusion: 现有并行解码方法未能突破速度-质量权衡，评估不一致源于对提示模板细微变化的高敏感性，研究提出了dLLM解码方法可靠评估的实用指南。
...
TL;DR: 本文提出DominoTree，一种无需训练的最佳优先草稿树推测解码方法，通过利用Domino的条件非因子化校正显著提升了LLM推理的吞吐量和接受长度。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散起草者和最佳优先树方法仅基于位置边缘分布，无法表示Domino草稿器中基于路径相关的因果校正结构。

Method: 提出DominoTree，利用Domino的条件非因子化校正沿根到节点路径对草稿树进行评分，并通过限制每节点校正的候选top-M集合来保持实用性；同时使用GPU原生的CUDA-graph构建器高效构建树。

Result: 在Qwen3-4B上相比自回归解码实现最高6.6倍加速，各温度下平均接受长度最高（每轮10.7个token），吞吐量比Domino提升9-10%（Alpaca上最高22%），并在各温度下优于DDTree和CaDDTree；在Qwen3-8B上接受长度最高，T=0时比DDTree吞吐量增益24%。

Conclusion: DominoTree有效利用了路径相关的草稿分布，在多个模型和温度下显著提升了推测解码性能，且其GPU原生构建器保证了树构建的高效性。
...
TL;DR: 本文提出了CaDDTree，一种成本感知的扩散草稿树方法，通过联合选择树结构和节点预算直接优化令牌吞吐量，从而加速大语言模型推理。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿树（DDTree）方法在固定节点预算下最大化期望接受长度，但接受长度随预算非递减，总是偏向更大的树而忽略验证成本，缺乏选择预算的原则性基础。

Method: 提出CaDDTree（成本感知扩散草稿树），显式建模草稿和验证延迟，将吞吐量目标分解为对预算的每轮一维搜索，并证明在凸验证成本下吞吐量函数是单峰的，从而实现高效的贪婪停止规则，无需离线预算搜索即可动态调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型上的八个基准测试（涵盖推理、编程和指令遵循任务）表明，CaDDTree在几乎所有任务上都能匹配甚至超越具有先知预算选择的DDTree。

Conclusion: CaDDTree为投机解码中的节点预算选择提供了一种原则性的成本感知方法，有效提升了推理吞吐量，并免除了繁琐的离线调优。
...
TL;DR: 本文提出DDTree方法，通过从块扩散草稿模型构建并验证草稿树，克服了现有方法仅验证单一轨迹的限制，进一步提升了推测解码的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash块扩散草稿模型在推测解码时每轮仅验证单个草稿轨迹，限制了其接受长度，本文旨在解决此瓶颈以提高解码效率。

Method: 提出DDTree，利用块扩散草稿模型的逐位置分布构建草稿树；在固定节点预算下，采用最佳优先堆算法基于代理似然度选择最优连续路径，并使用仅祖先注意力掩码在目标模型的单次前向传播中高效验证该树结构。

Result: DDTree突破了单一轨迹验证的局限，在DFlash基础上提升了接受长度和整体性能，确立了其在推测解码领域的领先地位。

Conclusion: 通过引入树状验证机制，DDTree有效提升了基于块扩散的推测解码效率，成为当前推测解码技术的领先方法之一。
...
### [17] [Sangam: Efficiently Serving Diffusion LLMs with the AR Stack](http://arxiv.org/abs/2607.04206v1)
*Nitin Kedia,Saurabh Agarwal,Myungjin Lee,Aditya Akella*

Main category: cs.DC

TL;DR: 本文提出了Sangam，一个针对缓存扩散语言模型（dLLM）推理的服务系统，通过赤字令牌预算调度器和混合服务策略优化了预填充与解码的调度。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归（AR）服务机制无法直接应用于使用近似KV缓存的扩散语言模型，因为dLLM的解码是块级的，预填充会重复出现，且双向注意力机制阻碍了分块预填充机制在无停顿协同部署中的应用。

Method: 提出了Sangam系统，其核心是赤字令牌预算调度器，优先执行解码，仅在预算允许时执行整体预填充并结转剩余预算，以实现摊余无停顿调度；同时采用混合服务策略，将预填充任务溢出到解码工作节点以缓解资源不足，并用该调度器保护解码免受干扰。

Result: 在解码密集型负载中，协同部署服务相比混合执行降低了9-20%的平均延迟；在预填充密集型负载中，混合执行相比协同部署服务降低了8-20%的平均延迟。

Conclusion: dLLM服务设计空间同样受预填充-解码干扰和资源划分支配，协同部署服务最适用于解码密集型工作负载，而混合执行最适用于预填充密集型工作负载。
...
### [18] [LLM-Powered Agentic AI for 5G/6G Networks: A Tutorial and Survey on Architectures, Protocols, and Standardization](http://arxiv.org/abs/2607.16066v1)
*Mazene Ameur,Abdelkader Mekrache,Bouziane Brik,Adlen Ksentini*

Main category: cs.NI

TL;DR: 本文提供了一个分为两部分的教程与综述，探讨基于大语言模型的智能体AI在5G/6G下一代网络中的协议集成、评估与标准化对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的综述将智能体AI和下一代网络孤立对待，导致协议集成、评估和标准化对齐方面的研究不足。

Method: 提出了一个两部分的综合框架：第一部分形式化5G/6G的控制、管理和AI原生平面并介绍智能体系统基础；第二部分将智能体能力映射到5G/6G控制面、标准化及6G倡议上。

Result: 系统性地梳理了智能体AI在下一代网络中的基础架构与能力映射，并明确指出了塑造自主电信网络的多个开放性挑战。

Conclusion: 智能体AI标志着下一代网络向自主、目标驱动控制的转变，未来研究需致力于解决自主电信网络面临的各项开放挑战。
...
### [19] [Physics-enhanced reinforcement learning for real-time optimal control of dynamical systems](http://arxiv.org/abs/2607.16177v1)
*Matteo Tomasetto,Nicolò Botteghi,Gabriele Bruni,Andrea Manzoni*

Main category: cs.LG

TL;DR: 本文提出物理增强强化学习范式（PEARL），利用动力系统的可微性解决高维参数化系统控制中的样本效率低与维度灾难问题。


<details>
  <summary>Details</summary>
Motivation: 旨在解决强化学习在控制高维复杂动力系统时存在的样本效率低、环境交互多，以及在高维空间中因探索-利用困境导致的维度灾难问题。

Method: 结合强化学习与传统最优控制，提出actor-adjoint算法，利用自动微分计算短时域策略梯度，并用神经网络逼近未来回报的伴随敏感度。

Result: 在非定常流导航实验中，PEARL性能优于现有RL算法，具备高样本效率、跨场景泛化能力，且无需降维即可扩展至高维状态与动作空间。

Conclusion: PEARL通过利用可微环境和物理引导策略学习，有效突破了强化学习在高维参数化动力系统控制中的样本效率与扩展性瓶颈。
...
### [20] [When Does Muon Help Agentic Reinforcement Learning?](http://arxiv.org/abs/2607.16169v1)
*Kai Ruan,Jinghao Lin,Zihe Huang,Ziqi Zhou,Qianshan Wei,Xuan Wang,Hao Sun*

Main category: cs.LG

TL;DR: 本文探讨了Muon优化器在稀疏奖励智能体强化学习（RL）后训练中的有效性，发现其能显著提升验证成功率。


<details>
  <summary>Details</summary>
Motivation: 尽管Muon在大规模预训练中与AdamW具有竞争力，但其在强化学习（RL）后训练阶段的价值尚不明确，特别是在稀疏奖励的智能体RL任务中。

Method: 在ALFWorld环境下使用Qwen2.5-0.5B-Instruct模型，通过Group-in-Group Policy Optimization (GiGPO) 框架，将原版Muon仅应用于隐藏权重矩阵，并与AdamW进行了匹配的单种子对比实验。

Result: Muon将最终窗口验证成功率从0.290提升至0.546（+88%），且效果受优势估计器和学习率影响。在1e-5学习率下，结合GraphGPO的Muon达到0.901的成功率，将归一化验证AUC从0.399提升至0.556，并提前30和60次更新达到0.5和0.75的成功率。

Conclusion: 探索性结果表明Muon能够使智能体RL受益，并强调了联合研究策略优化器、优势估计器和学习率的必要性。未来的研究方向包括多种子实验和跨任务验证。
...
### [21] [PRISA: Proactive Infrastructure LiDAR Framework for Intersection Safety Assessment](http://arxiv.org/abs/2607.16156v1)
...
TL;DR: PRISA是一个模块化的基础设施激光雷达框架，利用边缘计算和无需人工标注的自动轨迹预测，在城市交叉口进行实时、主动的安全监测。


<details>
  <summary>Details</summary>
Motivation: 城市交叉口是道路网络中最危险的区域，对弱势道路使用者构成重大风险。多智能体交互的复杂性要求建立连续的实时监控系统，以便在冲突升级为碰撞之前进行预测。

Method: 提出PRISA框架，包含感知层和即插即用风险评估模块。该模块自动从累积的感知输出中整理特定于站点的训练数据，无需人工标注即可训练轨迹预测模型，并采用时间碰撞（TTC）和预测后侵占时间（PPET）双重替代安全评估指标进行连续运动预测和冲突检测。

Result: 框架在公共R-LiViT数据集上进行了评估，并部署于NVIDIA Jetson AGX Thor设备上的实际信号交叉口。基于PPET的评估端到端延迟为194毫秒（预测范围2.4秒），基于TTC的检测和感知均满足实时约束。

Conclusion: 实验结果证明了该框架在主动式多智能体交叉口安全监测中的实际可行性。
...
### [22] [When Do Multi-Agent Systems Help? An Information Bottleneck Perspective](http://arxiv.org/abs/2607.16133v1)
...
TL;DR: 本文从信息瓶颈视角分析多智能体系统与单智能体系统的差异，揭示了多智能体设计在有界通信下的性能权衡机制。


<details>
  <summary>Details</summary>
Motivation: 旨在解决大语言模型驱动的多智能体系统相较于单智能体系统在复杂任务中优势不明确且表现不一致的问题。

Method: 提出信息瓶颈视角，将多智能体系统中由有限中继消息引起的上下文压缩与信息丢失的权衡形式化为受有效参数β控制的优化问题。

Result: 在5个基准和3种模型规模上的18项实验表明，当信息中继几乎充足时多智能体系统有优势（尤其是较弱模型）；而在信息丢失时优势缩小或逆转（尤其是较强模型）。
...
### [23] [The Honest Quorum Problem: Epistemic Byzantine Fault Tolerance for Agentic Infrastructure](http://arxiv.org/abs/2607.16109v1)
*Jun He,Deying Yu*

Main category: cs.LG

TL;DR: 本文提出了认知拜占庭容错（EBFT）模型，以解决代理验证器因推理错误导致的语义无效转换问题，即诚实仲裁问题。


<details>
  <summary>Details</summary>
Motivation: 传统SMR和BFT共识假设协议合规的参与者能正确执行转换语义，但代理验证器可能因推理错误而认可语义无效的转换（认知故障），且因共享模型权重、训练分布等而易受相关认知故障影响，导致仅达成共识不足以保证语义有效性或执行安全性。

Method: 定义EBFT模型，在传统拜占庭故障界限基础上引入两个置信度指标：e_δ限定拜占庭集合外的连贯无效背书，u_ε限定影响活性的不可用验证器支持；推导了语义有效性、共识一致性、活性和可行阈值选择的仲裁阈值条件，并提出了估算这些预算的校准方法。

Result: 证明了仅当名义上不同的代理可测量地降低无效背书或不可用支持的上尾集中度时，增加代理才能提升容错能力；推导并给出了各属性的仲裁阈值条件。

Conclusion: 单纯的共识一致不能保证语义有效性或执行安全性；EBFT为代理基础设施和后确定性分布式系统提供了独立表征语义安全风险和活性退化的容错框架。
...
### [24] [When Model Merging Rivals Joint Multi-Task Reinforcement Learning: A Task-Vector Geometry Analysis](http://arxiv.org/abs/2607.16062v1)
*S. Aaron McClendon*

Main category: cs.LG

TL;DR: 本文验证了强化学习中模型合并与联合多任务训练的效果对比，发现合并能匹敌联合训练，且因任务向量近乎正交导致不同合并方法均退化为均匀平均。


<details>
  <summary>Details</summary>
Motivation: 模型合并被提出作为联合多任务训练的替代方案，但在强化学习场景中通常缺乏联合训练基线进行直接对比验证。本文旨在填补这一空白，评估合并方法能否真正替代联合训练。

Method: 在AppWorld基准上使用LOOP训练不同难度的Qwen3-8B专家模型，采用TIES和RAM+方法合并，并与联合训练模型对比。通过测量和校准专家任务向量的几何特征（如正交性和支持重叠率）来分析合并机制。

Result: 实验表明，模型合并在任务完成度上与联合强化学习效果相当，各合并变体无统计差异。原因是专家任务向量近乎正交（余弦0.06-0.10），方向与支持解耦，导致基于支持或符号的合并方法退化为近乎均匀的权重平均。

Conclusion: 在特定强化学习设置下，模型合并能有效替代联合多任务训练。由于任务向量的几何特性，复杂的合并策略在此场景下会退化为简单的平均化，导致合并方法的选择对结果影响不大。
...
TL;DR: 本文提出了一种连续时间强化学习框架，用于微调基于分数的离散扩散模型，特别是掩码扩散语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法仅依赖终端奖励且计算轨迹似然代价高昂，需要一种支持中间奖励信号且计算高效的连续时间RL框架来优化离散扩散模型。

Method: 采用随机控制方法将状态动态建模为受控连续时间马尔可夫链(CTMC)，推导出连续时间版PPO和GRPO算法，允许在去噪轨迹中引入中间奖励信号，并提出轨迹子采样技术降低计算成本。

Result: 在低维熵正则化优化问题和dLLMs的数学推理、编码任务上验证了方法的有效性，框架支持不可微奖励信号的奖励驱动优化。

Conclusion: 该框架为掩码扩散模型提供了统一的探索与策略优化视角，具备解析可处理的概率比，在dLLMs后训练中展现出实用价值。
...
TL;DR: 本综述提出了一个统一的dLLMs延迟分解框架，用于分析和比较扩散大语言模型在算法、架构和系统层面的加速技术。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在并行生成上具有理论优势，但并行生成本身并不保证实际加速，且现有基准测试将算法、架构和系统因素混杂在一起，导致难以进行严格比较，因此需要系统性分析影响推理速度的因素。

Method: 引入统一的延迟分解框架，将端到端延迟分解为算法、架构和系统层面的因素，并沿三个轴线对加速技术进行分类：算法创新、架构与系统优化、以及推理时扩展。

Result: 该框架能够解耦各因素对推理速度的影响，为dLLMs的加速技术提供系统性分类和可复现的基准测试指南。

Conclusion: 提出了可复现基准测试的指导原则，并指出了实现并行生成全部潜力的开放性挑战，为未来dLLMs的高效部署提供了方向。
...
TL;DR: 本文提出BlockServe框架，通过块级调度和混合状态执行解决扩散大语言模型中的收敛异构性问题，显著提升了推理吞吐量。


<details>
  <summary>Details</summary>
Motivation: 解决扩散大语言模型在批处理时因不同序列收敛速率不同而产生的收敛异构性问题，该问题导致较快的请求被较慢的请求阻塞，产生计算气泡和尾延迟。

Method: 提出连续批处理框架BlockServe，包含块级调度（在块边界立即驱逐已完成请求）、混合状态执行（通过聚集-分散索引将双缓存和并行解码扩展至异构批次）以及计算感知的准入控制器（通过令牌预算补充扩大有效批次容量）。

Result: 在Dream和LLaDA模型上的五个基准测试中，BlockServe相比Fast-dLLM实现了1.9至10.6倍的吞吐量提升，且生成质量相当。

Conclusion: 确立了块级调度作为高吞吐量离线dLLM推理的基础。
...
TL;DR: 提出目标感知前缀选择方法（TAPS），通过优化扩散模型投机解码的验证过程显著提升无损加速效果。


<details>
  <summary>Details</summary>
Motivation: 在使用扩散模型进行投机解码时，现有草稿树方法通过边际概率排序节点，忽略了验证过程基于前缀条件的特性，导致验证被拒绝前缀的不可达子代，在有限接受度提升的情况下增加了延迟。

Method: 提出TAPS方法，将扩散边际概率转化为路径条件接受估计，并在固定验证预算下选择紧凑的前缀闭包子树，以改善接受度与成本之间的权衡。

Result: 在多种数据集和模型上的实验表明，TAPS相比原始自回归解码实现了高达7.9倍的无损端到端加速，且分别比最先进的DFlash和DDTree快1.36倍和1.74倍。

Conclusion: TAPS成功解决了扩散模型投机解码中的验证瓶颈，通过优化草稿树的前缀选择策略显著提升了无损推理速度。
...
### [29] [MGDT: MLLM-Guided Diffusion Transformer with Relation-Adaptive Mixture-of-Experts for Multimodal Knowledge Graph Completion](http://arxiv.org/abs/2607.15592v1)
*Xu Hou,Meiyu Liang,Wei Huang,Yawen Li,Zhe Xue,Wu Liu,Guanhua Ye,Lei Shi,Kangkang Lu*

Main category: cs.AI

TL;DR: 本文提出了一种基于“先对齐后扩散”范式的MGDT框架，利用关系自适应混合专家模块和冻结的多模态大语言模型引导扩散Transformer，有效提升了多模态知识图谱补全性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的多模态知识图谱补全方法直接对原始多模态特征去噪，迫使去噪器同时处理线索选择、跨模态对齐和实体生成，引入了噪声和语义不一致的条件，导致性能次优。

Method: 提出MGDT框架：首先利用关系自适应语义路由混合专家（RASR-MoE）模块选择关系相关的多模态路径并抑制干扰；然后使用冻结的多模态大语言模型（MLLM）作为语义锚点，将表示对齐到统一潜在空间；最后通过知识图谱扩散Transformer（KGDT）在该空间内进行图条件去噪生成缺失实体。

Result: 在三个基准数据集上的实验结果表明，MGDT方法始终优于现有强大的基线方法。

Conclusion: 将跨模态语义对齐与去噪生成解耦的“先对齐后扩散”范式能够有效减少语义异质性，是多模态知识图谱补全的有效解决方案。
...
### [30] [DrawingVQA: A Real-World Benchmark for Multi-Depth Visual-Textual Reasoning on Construction Drawings](http://arxiv.org/abs/2607.15418v1)
*Yoonhwa Jung,Junryu Fu,Mani Golparvar-Fard*

Main category: cs.AI

TL;DR: 本文提出了DrawingVQA，首个用于评估多模态大语言模型在真实建筑施工图上理解与推理能力的基准。


<details>
  <summary>Details</summary>
Motivation: 建筑施工图融合了几何图形、符号、表格和领域文本，是工程实践的核心媒介，但现有基准尚未覆盖此类复杂图文领域，模型在该领域的推理能力缺乏系统评估。

Method: 构建包含33张施工图和92组专家标注问答的基准，设计三层推理深度（感知理解、上下文解释、领域专家推理），并提出双分类框架将工程工作流与AI推理能力维度联合映射分析。

Result: 对最先进多模态大语言模型的评估显示，模型与专家表现存在显著差距，尤其在更高推理深度任务上差距尤为明显。

Conclusion: 该基准为领域专门化多模态推理研究奠定基础，推动AI理解能力与真实工程工作流的深度整合，为未来发展指明方向。
...
TL;DR: DELTA-TTS是一种将预训练自回归TTS模型转换为离散扩散语言模型的轻量级框架，通过置信度顺序解码提高了推理速度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自回归TTS模型由于从左到右的顺序生成，导致推理缓慢且容易传播局部错误和幻觉，尽管完整的输入文本在合成前已可用，这种顺序并非TTS的内在要求。

Method: 提出DELTA-TTS框架，基于LoRA将预训练AR TTS模型转换为离散扩散语言模型（dLLM）进行置信度顺序解码；结合卷积模块注入局部声学上下文，采用1/t加权训练目标和时间偏移推理调度，将低置信度位置推迟到后续步骤。

Result: 在LibriTTS上训练585小时后，在Seed-TTS test-en上达到1.75%的WER，优于其AR主干模型，且生成速度快3.3倍；分析表明其文本-语音对齐更清晰，提高了解码置信度并减轻了AR生成中的幻觉。

Conclusion: 通过采用扩散模型的置信度顺序解码取代严格的从左到右自回归生成，DELTA-TTS成功解决了AR TTS的推理速度慢和鲁棒性差的问题，实现了更优的语音合成性能。
...
### [32] [Evaluating Open-Weight LLMs for Generating Structured Threat Information for Autonomous Vehicle Vulnerabilities](http://arxiv.org/abs/2607.16175v1)
*Md Erfan,Ahmed Ryan,Md Kamal Hossain Chowdhury,Md Rayhanur Rahman*

Main category: cs.CR

TL;DR: 本文评估了开源大语言模型将自动驾驶车辆漏洞描述转换为STIX结构化威胁情报的能力，并构建了专用数据集。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆的漏洞通常以纯文本形式记录，安全人员难以直接获取结构化信息以有效缓解风险。

Method: 构建CAV-STIXGen数据集将漏洞映射至STIX对象、CWE和MITRE ATT&CK技术，并评估了11个开源大语言模型在多种策略下的转换能力及多智能体配置。

Result: 单模型在STIX域对象、关系对象和CWE映射上的F1分数分别达0.94、0.63和0.99；多智能体设置在域对象和关系对象上分别取得0.91和0.43的F1分数。

Conclusion: AI辅助的漏洞转换可实现威胁情报自动化，帮助识别CAV领域威胁模式并优先安排交通安全防御措施。
...
### [33] [Do Agents Dream of False Memories? Black-box Visual Attacks on Long-term Memory in Multimodal AI Agents](http://arxiv.org/abs/2607.15657v1)
*Halima Bouzidi,Mboutidem Ekemini Mkpong,Mohammad Abdullah Al Faruque*

Main category: cs.CR

TL;DR: 本文提出了Lucid，一种针对多模态记忆管道的黑盒对抗框架，通过不可见扰动图像成功实现记忆中毒和记忆注入攻击，揭示了视觉数据信任带来的结构性漏洞。


<details>
  <summary>Details</summary>
Motivation: 多模态AI智能体在长期记忆中无条件信任视觉数据，这带来了严重的安全漏洞，因此需要探索并暴露此类潜在威胁以评估系统安全性。

Method: 提出Lucid黑盒对抗框架，在严格受限的图像威胁模型下生成不可见扰动，根据历史上下文可用性实现两种攻击模式：上下文内的记忆中毒和上下文外的记忆注入。

Result: 在多种对话领域和五种黑盒记忆架构（包括图结构、LLM总结及商业部署系统）中，记忆中毒攻击成功率达61.6%，记忆注入攻击成功率达58.4%。

Conclusion: 多模态记忆管道中无条件信任视觉数据存在结构性漏洞，突显了在多模态AI智能体设计中加强视觉输入安全防御的必要性。
