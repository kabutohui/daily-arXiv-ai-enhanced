...
TL;DR: Reroute提出了一种免训练的可恢复路由插件，用于VLM视觉词元缩减，替代不可逆剪枝以在保持计算预算的同时提升定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM视觉词元缩减方法采用“排序并移除”范式，永久丢弃词元。这种不可逆操作存在缺陷，因为视觉词元的重要性随解码器深度变化，早期被判定为低重要性的词元在后续层可能变得关键，特别是对于定位敏感的查询。

Method: 提出Reroute，一种免训练插件，用可恢复路由替代永久移除。在路由阶段，被延迟的词元绕过当前解码器层，并在下一个路由决策时重新进入候选池。该插件复用现有的注意力分数排序规则和阶段调度，保持所增强剪枝方法的TFLOPs和KV缓存预算。

Result: 在LLaVA-1.5和Qwen主干网络上的FastV、PDrop和Nüwa变体实验表明，Reroute在激进的词元缩减下提升了视觉定位能力，同时维持了常规视觉问答（VQA）性能。

Conclusion: VLM词元缩减不应仅被视为不可逆剪枝，而应被视为可恢复路由，从而允许延迟词元在后续层中重新发挥作用。
...
### [2] [Bridging the Modality Gap in Forensic Image Retrieval](http://arxiv.org/abs/2606.12294v1)
*Ricardo González-Gazapo,Annette Morales-González,Yoanna Martínez-Díaz,Heydi Méndez-Vázquez,Milton García-Borroto*

Main category: cs.CV

TL;DR: 本文提出一种基于多模态大语言模型和特征融合的统一检索框架，以解决法医分析中视觉信息受限条件下的纹身和人脸检索任务。


<details>
  <summary>Details</summary>
Motivation: 现有研究对多模态检索系统在法医真实场景下的适用性评估不足，且传统法医检索在视觉信息有限或存在噪声（如素描、局部纹身、目击者口述）时面临挑战。

Method: 利用多模态大语言模型（MLLM）为查询和图库图像自动生成结构化文本描述，并通过句子转换器进行文本嵌入；结合前沿视觉特征提取器，评估视觉、文本及多模态融合（视觉与文本相似度结合）三种检索策略。

Result: 多模态融合策略一致提升了检索精度和鲁棒性，尤其在视觉信息受限或存在噪声的场景（如素描、局部纹身、碎片化证词）下表现尤为突出。

Conclusion: 统一的多模态检索流程具有重要的法医应用价值，现代MLLM能够将传统依赖人工的法医任务自动化，多模态检索是支持法医调查工作流程的极具前景的工具。
...
### [3] [Q-Fold: Query-Aware Focus-Context Spatio-Temporal Folding for Long Video Understanding](http://arxiv.org/abs/2606.12125v1)
*Biao Tang,Xu Chen,Shuxiang Gou,Jingyi Yuan,Yuhan Zhang,Chenqiang Gao*

Main category: cs.CV

TL;DR: 提出Q-Fold，一种无需训练的长视频理解输入构建框架，通过查询引导构建异构的聚焦-上下文表示，在有限视觉预算内同时保留高保真视觉证据与广泛时间覆盖。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解方法遵循以帧为中心的范式，对保留内容不加区分地分配相似表示，导致在有限视觉预算下难以同时保留高保真视觉证据和广泛的时间覆盖范围。

Method: 提出Q-Fold框架，以连续时间片段而非孤立帧为基础单位，在查询引导下构建异构的“聚焦-上下文”表示；将与查询相关的片段保留为高保真“聚焦帧”，将相关性低的片段折叠成保序的上下文布局。

Result: 在四个长视频基准测试和多个Video-MLLM上，Q-Fold在不增加输入预算的情况下持续提升性能，在超长视频基准上最高提升9.1个百分点。

Conclusion: Q-Fold通过异构表示成功平衡了长视频理解中关键视觉证据的保真度与时间覆盖的广度，且无需额外训练或增加输入预算，有效提升了长视频建模能力。
...
### [4] [Task-Aware Structured Memory for Dynamic Multi-modal In-Context Learning](http://arxiv.org/abs/2606.11853v1)
*Zhirui Chen,Ziwei Chen,Ling Shao*

Main category: cs.CV

TL;DR: 本文提出免训练框架TASM，通过任务引导压缩、语义感知令牌合并和分层动态检索，解决多模态大模型上下文学习中长序列KV缓存成本高及现有压缩破坏语义结构的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的上下文学习受限于有限上下文窗口和长多模态序列KV缓存的高昂成本；现有记忆压缩方法依赖僵化令牌移除或样本特定重要性估计，会引入偏差、破坏视觉语义结构且无法适应新查询。

Method: 提出TASM免训练框架：1) 基于任务向量引导压缩，以任务级方向替代样本特定信号；2) 利用二分图匹配进行语义感知的令牌合并以保留底层流形，避免破坏性剪枝；3) 构建包含紧凑核心记忆与潜在库的层次结构，实现查询自适应的动态检索。

Result: 评估表明，TASM在重度压缩下仍能维持高性能，有效平衡了推理效率与自适应性。

Conclusion: TASM通过任务感知与结构保留的记忆构建克服了传统压缩方法的局限，为多模态大模型的高效且自适应的上下文学习提供了有效方案。
...
### [5] [Ouroboros-Spatial: Closing the Data-Model Loop for Spatial Reasoning](http://arxiv.org/abs/2606.11719v1)
*Enhan Zhao,Wei Wu,Yuanrui Zhang,Xueliang Zhao,Di He*

Main category: cs.CV

TL;DR: 本文提出Ouroboros-Spatial自演化训练框架，通过提议器与求解器的闭环反馈动态生成匹配模型能力的数据，以极少训练样本大幅提升多模态大语言模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型的空间推理依赖静态数据集，未考虑模型能力的动态演进，导致训练容量浪费在当前过于简单或困难的样本上，数据效率低下。

Method: 提出Ouroboros-Spatial框架，模型分演提议器与求解器双角色。冻结的提议器生成空间QA对及推导真值的代码；可学习的求解器微调后，将其预测置信度作为难度信号反馈至下一轮提议器，指导其生成适配当前能力的样本，实现闭环自演化。

Result: 在六个空间推理基准上，该方法以比现有大规模数据集少一个数量级的样本量，显著提升Qwen3-VL-4B和8B性能；在VSI-Bench上分别实现9.9和6.8的绝对提升，超越众多开源与闭源基线。

Conclusion: 训练分布与模型能力的闭环协同演化可有效剔除冗余与低信息量样本，为多模态空间推理提供了一种高效的数据驱动范式。
...
### [6] [Reason, Then Re-reason: Cross-view Revisiting Improves Spatial Reasoning](http://arxiv.org/abs/2606.11683v1)
*Chaofan Ma,Zhenjie Mao,Yuhuan Yang,Fanqin Zeng,Yue Shi,Yingjie Zhou,Xiaofeng Cao,Jiangchao Yao*

Main category: cs.CV

TL;DR: 提出免训练的推理时框架ReRe，通过生成互补新视角视频验证和修正MLLM的空间假设，显著提升自我中心视频的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 自我中心视频的空间推理受限于相机轨迹导致观测证据有限；现有单次推理方法依赖语义先验解决几何歧义而非可验证证据，缺乏在获得互补视角时修正结论的能力。

Method: 提出ReRe框架，包含推理阶段（从原视频形成空间假设）和再推理阶段（通过合成新视角视频验证或修正假设）；设计Geometry-to-Video管线，基于预测的3D几何渲染具有俯视、倾斜及全局覆盖特征的互补新视角视频，且无需修改模型架构。

Result: 在VSI-Bench和STI-Bench上的评估表明，ReRe显著提升了开源多模态大语言模型的性能，使其达到媲美最先进专有模型的水平。

Conclusion: 空间推理应具备可修正性，结合3D几何引导的新视角合成补充视角信息，能有效消除几何歧义，大幅增强模型的空间推理表现。
...
### [7] [ARGUS: Stacked Multi-View Identity Mosaic Injection for Subject-Preserving Video Generation](http://arxiv.org/abs/2606.11670v1)
*Zijie Meng,Jiwen Liu,Yufei Liu,Chengzhuo Tong,Xiaoqiang Liu,Yuanxing Zhang,Yulong Xu,Pengfei Wan*

Main category: cs.CV

TL;DR: 本文提出Argus框架，通过引入动态身份记忆（SMII）和反事实自监督，有效解决了复杂条件下视频中主体身份保持的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的点参考范式将身份坍缩为与姿态、配饰、光照等纠缠的单一静态观察，导致在运动、大视角变化、遮挡及条件冲突下生成的角色难以保持身份一致性。

Method: 提出基于Wan的Argus框架，核心为堆叠多视图身份马赛克注入（SMII），将MLLM选取的身份证据转为3*3马赛克，与当前扩散时间同步并以负时间只读内存注入token空间，将身份转化为紧凑动态分布；辅以MLLM身份指导器解决条件冲突，结合无交叉配对反事实训练、时序身份退火和自适应自相似性指导提升鲁棒性。

Result: 在OpenS2V-Eval人类域上达到SOTA（总分64.38，FaceSim 71.86）；在发布的HardID-Celeb基准上，FaceSim达76.80，YawScore和OccScore较最强基线分别提升12.60和15.10分。

Conclusion: 将身份从单一静态参考转化为紧凑动态分布，并结合大规模反事实自监督，对主体保持视频生成极为有效。
...
### [8] [Context-Driven Incremental Compression for Multi-Turn Dialogue Generation](http://arxiv.org/abs/2606.12411v1)
*Yeongseo Jung,Jaehyeok Kim,Eunseo Jung,Jiachuan Wang,Yongqi Zhang,Ka Chun Cheung,Simon See,Lei Chen*

Main category: cs.CL

TL;DR: 本文提出上下文驱动增量压缩方法C-DIC，通过可修订的线程级压缩状态与跨轮次信息共享机制，解决长对话上下文建模成本高及信息易丢失的问题。


<details>
  <summary>Details</summary>
Motivation: 现有对话模型依赖不断增长的对话历史导致计算成本增加，而朴素的截断/摘要或现有压缩器因缺乏跨轮次记忆共享与修订机制，会导致保真度下降及误差累积。

Method: 提出C-DIC方法，将对话建模为交织的上下文线程，在紧凑记忆中存储可修订的线程压缩状态；采用轻量级检索-修订-写回循环实现跨轮次信息共享与记忆更新；并适配截断随时间反向传播（TBPTT）以学习跨轮次依赖。

Result: 在长对话基准测试中表现出卓越的性能与效率，且在数百轮对话中保持了稳定的推理延迟和困惑度。

Conclusion: C-DIC显著提升了长对话建模的效率与鲁棒性，为可扩展的高质量对话建模提供了有效路径。
...
### [9] [Which Models Are Our Models Built On? Auditing Invisible Dependencies in Modern LLMs](http://arxiv.org/abs/2606.12385v1)
*Sanjay Adhikesaven,Haoxiang Sun,Sewon Min*

Main category: cs.CL

TL;DR: 本文提出ModSleuth智能体系统，通过形式化方法从公开构件中递归重建大语言模型依赖图，以解决现代LLM开发中复杂且难以人工追踪的递归依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现代LLM训练流程严重依赖其他模型处理数据、过滤和评判，导致递归且碎片化的依赖结构；其复杂度远超人类手动追踪能力，且核心挑战已从信息提取转变为如何定义依赖关系及协调不一致文档中的构件引用。

Method: 提出ModSleuth智能体系统，通过形式化方法区分直接与间接依赖，以操作为中心的关系表示异构流水线角色，并跨名称、版本和仓库解析构件身份，从而基于溯源证据递归重建依赖图。

Result: 在四个LLM版本上恢复了1060个经源码验证的依赖并构建大规模依赖图，揭示了多跳许可义务、训练与评估耦合、发布与训练时构件差异及文档不一致等难以察觉的问题。

Conclusion: 公开发布ModSleuth及生成的依赖图，以支持对日益复杂的现代LLM生态系统进行透明分析，为揭示和治理深层依赖问题提供基础。
...
TL;DR: 本文提出注意力引导的去噪与优化框架AGDO，通过利用Token间的注意力依赖关系改进扩散大语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型的后训练方法多采用随机掩码策略，忽略了Token间的内在依赖关系，未能关注对推理至关重要的高注意力稳定Token。

Method: 提出AGDO框架，基于注意力结构决定去噪顺序，并在监督微调和强化学习阶段强调注意力关键Token，使训练与优化过程与注意力派生的依赖关系对齐。

Result: 在数学和编程基准测试中，AGDO持续提升了推理性能，优于现有的扩散大语言模型最先进后训练方法。

Conclusion: 对齐注意力依赖关系的去噪与优化策略能有效提升扩散大语言模型的推理能力，为后续后训练方法提供了新范式。
...
TL;DR: ADAS是一种免训练的重排序规则，通过软注意力折扣惩罚改进掩码扩散模型的并行解码质量。


<details>
  <summary>Details</summary>
Motivation: 现有免训练采样器在并行解码时依赖独立词元评分，忽略词元间的交互耦合关系，导致同时提交耦合但个体高置信度的词元时不安全。

Method: 提出ADAS，在构建子集时，对强烈关注已选但预测仍不确定位置的候选词元进行贪心折扣，将注意力作为软边际惩罚而非硬兼容性约束。

Result: 在多个基准和模型上，将ADAS嵌入现有采样器，在相同去噪评估次数下，低NFE性能平均分别提升9.11和10.46个百分点，仅增加3.1%的运行时开销。

Conclusion: 软注意力折扣重排序是一种简单、模块化的方法，能有效提升掩码扩散语言模型高并行解码的质量。
...
TL;DR: 提出Prefilling-dLLM免训练框架，通过分块缓存KV与稀疏选择机制降低扩散语言模型的二次计算复杂度，实现长上下文显著加速并消除中间丢失现象。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在每个去噪步骤需重新编码整个前缀，导致计算量随上下文长度呈二次方增长，在长上下文场景下计算代价极其高昂。

Method: 提出免训练的预填充-解码解耦框架：将前缀分为N块并一次性缓存KV表示；解码时通过块内token稀疏性选择最相关的前K块；设计并行注意力内核处理非连续缓存的块KV；并在每个分块前添加BOS token作为周期性注意力锚点。

Result: 稀疏预填充性能优于密集注意力；将每步复杂度从全序列长度二次方降至仅解码长度二次方；在LongBench和InfiniteBench上达到同领域SOTA质量；8K-32K上下文下实现9.1-28.0倍加速；BOS锚点消除了注意力“中间丢失”现象。

Conclusion: Prefilling-dLLM有效突破了dLLM的长上下文计算瓶颈，在大幅提升推理速度的同时超越了原有性能，且通过周期性注意力锚点显著改善了长文本的信息检索能力。
...
TL;DR: 本文提出统一能量（Uni-E）以解决扩散语言模型中由容量、依赖性和不变性导致的性能差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型无法充分捕获token间的关系，导致在高并行度下与自回归基线模型存在显著性能差距。

Method: 系统分析性能差距的三个关键因素（模型容量、依赖性、不变性），提出不变能量（Inv-E）与独立能量（Ind-E）相结合的统一能量（Uni-E）；Uni-E无需采样配分估计即可精确计算，且与模型无关。

Result: 在扩散语言模型（DLMs）和扩散大语言模型上的广泛实验证明了所提Uni-E的有效性。

Conclusion: Uni-E能纠正由依赖性和不变性引起的分布偏移，具有模型无关性，可扩展至任意规模的模型，有效弥合了DLMs与自回归模型间的性能差距。
...
<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)的强化学习存在真实生成轨迹与梯度更新过程的双重不对齐：1) 过程-奖励不对齐，稀疏的终端奖励被无差别分配给所有中间步骤，无法提供判别性的信用分配；2) 状态-轨迹不对齐，策略更新常偏向人工的轨迹外状态，导致梯度浪费。

Method: 提出过程对齐策略优化(PAPO)框架，包含：1) 步感知过程奖励(SPR)，将稀疏的终端奖励转化为密集的逐步信用分配；2) 熵引导历史重演(EHR)，在高不确定性步骤重放真实的生成轨迹。

Result: 在四个基准测试上显著优于基线，在GSM8K、MATH500、Countdown和Sudoku上分别取得高达4.5%、4.8%、42.2%和16.1%的性能提升。

Conclusion: PAPO有效对齐了强化学习更新与dLLMs的生成轨迹，大幅提升了模型在复杂推理任务上的性能，为dLLMs的强化学习优化提供了有效途径。
...
TL;DR: 提出SAID框架，通过重新分配词元间的去噪计算量，优先处理语义骨架词元，显著加速扩散大语言模型的推理。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(DLLMs)虽然支持并行非自回归生成，但由于需要大量去噪步数才能保证高质量的文本生成，导致推理成本依然高昂。

Method: 提出感知脚手架的迭代解码(SAID)框架，优先将去噪计算分配给脚手架词元以建立粗语义结构，再用较少步数补全可预测的细节词元；进一步适配块级扩散解码，并引入置信度分层生成(CHLG)机制，仅为低置信度词元分配额外步数。

Result: 在LLaDA-8B和LLaDA 1.5模型的数学、编程和知识基准测试中，SAID在保持竞争力的同时，最高实现了9.1倍的推理加速。

Conclusion: 基于语义骨架和置信度动态重新分配去噪计算的方法，能在不牺牲生成质量的前提下大幅提升DLLMs的推理效率。
...
TL;DR: 提出免训练的动态填充锚点（DIA）方法，解决扩散大语言模型中固定锚点导致的推理截断与内容冗余问题，提升格式合规性与生成准确性。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型使用固定锚点执行格式约束时会施加僵硬的跨度，导致推理截断或内容冗余，缺乏灵活性与语义连贯性。

Method: 提出动态填充锚点（DIA），一种免训练方法，通过在迭代填充前动态估计结束锚点位置来调整生成长度，确保结构正确与语义连贯。

Result: 在推理基准测试中，DIA显著提升了格式合规率和答案准确率，在GSM8K和MATH上取得了显著的零样本增益。

Conclusion: DIA为实现可靠且结构感知的生成提供了稳健路径，有效克服了固定跨度方法的低效性。
...
TL;DR: 本文提出CaDDTree，一种成本感知的扩散草稿树方法，通过联合优化树结构和节点预算来最大化词元吞吐量，解决了现有方法中预算选择缺乏原则性依据的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿树方法（如DDTree）在固定节点预算下最大化期望接受长度，但接受长度随预算非递减，导致总是倾向于更大的树而忽略验证成本，缺乏选择预算的原则性基础。

Method: 提出CaDDTree方法，显式建模草稿和验证延迟，将词元吞吐量（单位时间期望生成词元数）目标分解为对预算的每轮一维搜索；证明了在凸验证成本下吞吐量函数为单峰函数，从而实现高效的贪心停止规则，无需离线预算搜索即可动态自适应调整每轮预算和树结构。

Result: 在Qwen3-4B和Qwen3-8B模型及8个涵盖推理、编码和指令遵循的基准测试中，CaDDTree在几乎所有任务上的表现均匹配或超越了具有先知预算选择的DDTree。

Conclusion: CaDDTree为推测解码中的树结构和预算选择提供了原则性的成本感知优化方案，实现了自适应动态预算调整，并在实际应用中达到了与先知预算相媲美或更优的吞吐量，无需离线调参。
...
TL;DR: 本文提出DDTree方法，通过从块扩散草稿模型的位置分布构建草稿树，突破了单轨迹验证的局限，提升了投机解码的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的先进块扩散投机解码方法（如DFlash）每轮仅验证单条草稿轨迹，限制了接受长度的进一步提升。

Method: 提出DDTree，利用块扩散草稿模型的逐位置分布构建草稿树；在固定节点预算下，采用最佳优先堆算法选择最可能匹配目标模型的延续路径，并使用祖先注意力掩码在目标模型单次前向传播中高效验证该树。

Result: DDTree在DFlash基础上显著提升了投机解码性能，跻身当前最领先的投机解码方法之列。

Conclusion: 将块扩散模型与树状草稿结构相结合可有效增加投机解码的接受长度与整体效率，为自回归语言模型加速提供了更优方案。
...
### [19] [ATLAS: Active Theory Learning for Automated Science](http://arxiv.org/abs/2606.12386v1)
*Noémi Éltető,Nathaniel D. Daw,Kimberly L. Stachenfeld,Kevin J. Miller*

Main category: cs.LG

TL;DR: ATLAS是一个主动学习框架，通过迭代生成机制假设（解耦RNN）和最优区分性实验，自动发现可解释的认知行为模型。


<details>
  <summary>Details</summary>
Motivation: 机制建模需提出正确的实验问题以产生最大信息量数据，认知科学中该过程亟待自动化以提高发现效率。

Method: 交替进行机制假设生成（实例化为稀疏神经网络集成，即解耦RNN）与主动实验设计（旨在最优区分这些假设）。

Result: 在多臂老虎机任务恢复强化学习智能体中，相较随机实验，在行为、结构和计算相似性指标上样本效率提升5-10倍，且性能通过文献中的专家设计实验验证。

Conclusion: ATLAS有望在认知科学及依赖机制模型发现的其他领域中，加速产生人类可解释的科学洞见。
...
### [20] [APPO: Agentic Procedural Policy Optimization](http://arxiv.org/abs/2606.12384v1)
*Xucong Wang,Ziyu Ma,Yong Wang,Yuxiang Ji,Shidong Yang,Guanhua Chen,Pengkun Wang,Xiangxiang Chu*

Main category: cs.LG

TL;DR: 提出APPO算法，将智能体强化学习中的分支和信用分配从粗粒度交互单元转移到细粒度决策点，显著提升LLM智能体的多轮工具使用性能。


<details>
  <summary>Details</summary>
Motivation: 现有智能体强化学习方法在粗粒度的启发式单元（如工具调用边界）上进行信用分配，难以识别影响下游结果的中间决策；且仅靠词元熵无法可靠反映决策点对最终结果的影响。

Method: 提出智能体过程策略优化（APPO），利用结合词元不确定性与策略诱导似然增益的“分支分数”选择分支位置以实现定向探索，并引入过程级优势缩放来优化分支推演中的信用分配。

Result: 在13个基准测试中，APPO相比强大的智能体RL基线持续提升近4个百分点，同时保持了高效的工具调用和行为可解释性。

Conclusion: 将强化学习中的分支与信用分配细化至序列中的决策点而非粗粒度交互边界，能有效提升LLM智能体性能，为多轮智能体交互的信用分配提供了新范式。
...
### [21] [Breaking Entropy Bounds: Accelerating RL Training via MTP with Rejection Sampling](http://arxiv.org/abs/2606.12370v1)
...
TL;DR: 本文提出Bebop方法，通过揭示模型熵与多令牌预测(MTP)接受率的负线性关系，引入概率拒绝采样和端到端TV损失，有效解决了RL训练中MTP接受率下降问题，实现最高1.8倍的端到端训练加速。


<details>
  <summary>Details</summary>
Motivation: 在大语言模型强化学习(RL)训练中，推演阶段是速度瓶颈；而使用多令牌预测(MTP)进行推测解码加速时，其接受率在RL训练期间显著下降，导致加速效果受限。

Method: 1) 发现MTP接受率受模型熵波动限制且呈负线性关系；2) 采用概率拒绝采样替代贪婪草稿采样以缓解熵干扰；3) 提出端到端TV损失直接优化多步拒绝采样接受率，替代次优的传统交叉熵或KL损失；4) 采用RL前预训练MTP策略，免除高成本的在线MTP更新。

Result: MTP接受率提升约10%（最高达95%）；在数学推理、代码生成和智能体任务上带来高达25%的额外推理吞吐量提升；在Qwen3.5/3.6/3.7模型的异步RL训练中实现高达1.8倍的端到端加速。

Conclusion: 优化MTP训练目标与采样策略可有效克服RL训练中熵增导致的接受率下降问题，使MTP能稳定高效地集成至大规模RL管线中，显著提升整体训练效率，且无需进行在线更新。
...
### [22] [Claw-SWE-Bench: A Benchmark for Evaluating OpenClaw-style Agent Harnesses on Coding Tasks](http://arxiv.org/abs/2606.12344v1)
...
TL;DR: 本文提出多语言基准Claw-SWE-Bench及适配器协议，通过统一评估设置与成本核算，实现异构通用智能体编码能力的公平与可复现评估。


<details>
  <summary>Details</summary>
Motivation: 现有SWE-bench难以衡量通用智能体的编码能力，因其无法原生满足SWE-bench要求的Docker工作区、补丁及预测契约等严格评分标准。

Method: 构建包含350个多语言实例的Claw-SWE-Bench基准及适配器协议，通过固定提示、运行预算、工作区契约、补丁提取与评估器统一异构智能体线束的评估，并发布80实例的快速验证子集。

Result: 适配器设计显著影响性能（同骨干网下最小与完整适配器Pass@1分别为19.1%和73.4%）；模型与线束选择分别影响Pass@1达29.4和27.4个百分点；准确率相似的系统API成本差异显著。

Conclusion: 智能体线束设计与API成本核算应作为SWE编码智能体评估的首要维度，Claw-SWE-Bench为此提供了完整基准与低成本参考集以支持可复现比较。
...
TL;DR: 本文重新评估了掩码扩散语言模型中的后验置信度重掩码方法，发现其收益高度依赖设置且在标准解码下几无益处。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型在去掩码后无法修改令牌，易受早期采样错误影响；现有旨在解决此问题的无训练后验重掩码方法的实际有效性亟需严格验证。

Method: 在标准解码（较短块长度）和非贪婪解码设置下，对代表性的后验重掩码方法WINO进行实证重新评估，并与单纯的置信度去掩码方法进行对比。

Result: 在标准解码下，WINO相比单纯置信度去掩码几乎无收益；在非贪婪解码下，置信度重掩码虽能缓解随机性引入的误差，但加剧了置信度去掩码的多样性崩溃问题。

Conclusion: 后验置信度重掩码的收益高度依赖于具体解码设置，凸显了未来需要建立更全面的评估框架。
...
TL;DR: 提出FAIR-Calib两阶段训练后量化框架，通过感知边界和不稳定性重加权校准，解决扩散大语言模型中量化误差导致早期脆弱决策被永久翻转和放大的问题。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型迭代优化令牌时会进行不可逆提交，导致“稳定性滞后”，早期决策在写入边界处仍显脆弱。训练后量化（PTQ）误差极易翻转这些边界决策，且错误一旦提交便被永久锁定并放大。

Method: 提出FAIR-Calib两阶段PTQ框架：第一阶段探测全精度教师模型以估计结合边界命中与掩码阶段可靠性的位置先验；第二阶段通过最小化重加权的隐状态均方误差（MSE）进行离策略逐层校准，优先保护脆弱边界状态，免除了昂贵的端到端扩散rollout，并在理论上证明该加权目标是输出KL散度的替代。

Result: 在LLaDA和Dream（W4A4）模型上持续优于现有最先进基线，显著减少了边界决策翻转，并在多种基准测试中抑制了提交后的不匹配现象。

Conclusion: FAIR-Calib有效缓解了dLLMs量化过程中的不稳定性问题，为保护模型不可逆决策免受量化误差干扰提供了高效且具备理论支撑的解决方案。
...
TL;DR: 提出STaR-Quant框架，通过解决状态依赖的激活差异和时间误差积累问题，实现扩散大语言模型的低比特量化高效部署。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型因模型规模大和迭代去噪过程导致内存与计算开销大，需进行后训练量化；但低比特量化面临两大挑战：状态依赖的激活差异（掩码与未掩码词元分布不同）和时间误差积累（跨步骤解码误差累积）。

Method: 提出STaR-Quant框架：引入状态引导激活转换（SGAT），将掩码和未掩码词元分配至不同激活转换空间并保持统一静态权重转换；引入时间注意力补偿（TAC），通过轻量级块对角仿射映射校正量化注意力表示。

Result: 在代表性DLLMs上，低比特权重-激活量化性能持续优于强PTQ基线，相比FP16部署最高实现1.69倍加速和3.14倍内存节省。

Conclusion: STaR-Quant有效克服了DLLMs量化中的状态与时间一致性挑战，显著提升了低比特量化精度，为DLLMs的高效部署提供了有效方案。
...
TL;DR: 本文提出ART方法，通过优化冻结多模态大语言模型的原始像素输入来实现参数高效微调，无需修改计算图即可达到与LoRA相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调（PEFT）方法（如LoRA和软提示）需要修改大语言模型的计算图，导致其无法在vLLM等预编译的高吞吐量推理引擎中被完整支持。

Method: 提出ART（Art-based Reinforcement Training），利用梯度反向传播仅优化冻结多模态大语言模型（MLLM）的原始视觉输入（像素阵列），将微调信息注入模型以支持任意微调目标，且优化结果可风格化为任务相关的计算艺术图像。

Result: 在多种规模的Qwen架构和多个文本基准测试中验证了方法的有效性，ART在数学和结构化工具使用任务上的准确率与LoRA具有竞争力。

Conclusion: ART通过仅修改输入像素实现了预编译计算图上的软词元微调，为高吞吐量引擎下的大模型微调提供了可行方案，并拓展了微调信息注入的视觉表达形式。
...
### [27] [TAHOE: Text-to-SQL with Automated Hint Optimization from Experience](http://arxiv.org/abs/2606.12387v1)
*Zhiyi Chen,Jie Song,Peng Li*

Main category: cs.DB

TL;DR: Tahoe将提示优化视为动态数据管理问题，通过错误驱动的提示学习管线和策略层，在不微调模型参数的情况下显著提升Text-to-SQL的生产环境表现。


<details>
  <summary>Details</summary>
Motivation: Text-to-SQL从原型走向生产环境面临严格SQL方言、庞大模式及用户偏好演变的挑战，而监督微调成本高且僵化，代理式测试时计算扩展过于昂贵。

Method: 提出Tahoe系统，采用错误驱动的提示学习管线构建结构化提示库（包含源自编译器反馈的语法提示和源自执行/用户反馈的语义提示），并引入策略层利用新近度信号和归因统计来建模冲突的用户意图；推理时检索相关提示，引导LLM进行逻辑规划和SQL合成。

Result: 在Spider 2.0-Snow上（GPT-5.5），将通过率从61.95%提升至79.42%，pass-at-4从72.57%提升至87.61%，实现100%的Snowflake语法通过率，将平均编译器反馈轮次从2.79降至0.12；提示库可迁移至弱模型（如Doubao-2.0-lite提升19.7%）。

Conclusion: Tahoe无需更新模型参数即可显著提升Text-to-SQL性能，验证了将提示优化作为动态数据管理问题的有效性，未来工作将探索部署阶段的人类反馈更新。
...
TL;DR: 本文提出TAPS方法，通过目标感知的前缀选择优化基于扩散模型的推测解码中的草稿树验证，实现显著的无损加速。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的推测解码存在验证瓶颈：现有草稿树方法基于边缘概率对节点排序，忽略了验证的前缀条件性，导致可能验证被拒绝前缀的不可达后代，增加了延迟且接受率收益有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边缘概率转化为路径条件的接受度估计，并在固定的验证预算下选择紧凑的前缀闭合子树，从而优化接受度与成本的权衡。

Result: TAPS相比原始自回归解码实现了高达7.9倍的无损端到端加速，分别比最先进的DFlash和DDTree方法快1.36倍和1.74倍。

Conclusion: 通过考虑验证的前缀条件性，TAPS有效解决了基于扩散的推测解码中的验证瓶颈，提供了更优的接受度-成本权衡，显著提升了大语言模型的推理速度。
...
### [29] [MODF-SIR: A Multi-agent Omni-modal Distilled Framework for Social Intelligence Reasoning](http://arxiv.org/abs/2606.12018v1)
*Shang Ma,Jisheng Dang,Wencan Zhang,Yifan Zhang,Bimei Wang,Hong Peng,Bin Hu,Qi Tian,Tat-Seng Chua*

Main category: cs.AI

TL;DR: 本文提出了一种基于轻量级多模态大语言模型的多智能体协作框架，通过知识蒸馏与测试时适应技术增强社会智能推理，仅用30%训练数据即达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 解决社会智能推理中长尾事件关键信息在分词过程中易被头部事件和环境噪声掩盖的问题。

Method: 构建基于轻量级MLLM的多智能体协作框架；精确定位多模态数据并将长尾事件提取为格式化显式文本；在整个推理管线（含长尾事件提取、CoT提示和自反思）中集成利用LoRA微调的蒸馏增强测试时适应（TTA）机制以实现实例级推理。

Result: 在多个基准测试上对比各类开源和闭源模型，该框架仅使用IntentTrain约30%的训练数据便取得了最先进（SOTA）的性能。

Conclusion: 所提框架通过知识蒸馏、长尾事件格式化及蒸馏增强的TTA机制，显著且高效地提升了社会智能推理的性能，展现了极高的数据效率。
...
### [30] [SVoT: State-aware Visualization-of-Thought for Spatial Reasoning via Reinforcement Learning](http://arxiv.org/abs/2606.11770v1)
*Chao Lei,Yanbei Jiang,Markus Hiller,Zhijian Zhou,Xunye Tian,Krista A. Ehinger,Nir Lipovetzky*

Main category: cs.AI

TL;DR: 本文提出SVoT强化学习框架，通过生成可验证的交错中间状态与可视化来增强多模态大语言模型的多跳空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在多跳空间推理中缺乏对中间状态的验证，且将状态转换视为隐式过程，导致可靠性受限；同时现有基准过度简化了状态转换问题。

Method: 提出State-aware Visualization-of-Thought (SVoT)框架，将转换推理链融入生成过程以实现文本与视觉的交错推理和验证；采用Group Relative Policy Optimization (GRPO)训练并通过细粒度奖励设计实例化验证；构建包含Pacman和Gather等五个需要多对象交互与数值推理的新评估领域。

Result: 带有转换感知监督的SVoT在引入的领域上达到了最先进性能，在分布外测试集上实现了高达65%的绝对准确率提升。

Conclusion: 对中间状态和状态转换的显式验证能显著提升多模态大语言模型多跳空间推理的可靠性与泛化性，未来评估应更加关注复杂的多对象交互与数值推理。
...
### [31] [DIRECT: When and Where Should You Allocate Test-Time Compute in Embodied Planners?](http://arxiv.org/abs/2606.12402v1)
...
TL;DR: 本文提出DIRECT路由框架，通过多模态场景上下文动态分配测试时计算资源，避免盲目扩展计算量造成的浪费，在大幅降低延迟和成本的同时实现前沿级具身规划。


<details>
  <summary>Details</summary>
Motivation: 在具身智能体中扩展视觉语言模型的测试时计算量会增加延迟、token消耗和计算成本，且收益递减不均，限制了实际部署，因此需要一种智能选择何时何地分配计算资源的方法。

Method: 提出DIRECT路由框架，利用多模态场景上下文为每个提示动态分配计算量，在思维链深度、模型大小和记忆历史三个扩展轴上进行智能路由，以优化成功-成本帕累托前沿。

Result: 实验证明不同计算扩展轴带来截然不同的能力增益；在物理Franka机械臂上的验证表明，DIRECT路由器在平均延迟降低高达65%的情况下，成功率达到甚至超越更强的固定模型。

Conclusion: 盲目扩展测试时计算量是低效且浪费的，DIRECT路由框架能够以极低成本在机器人系统中实现前沿水平的具身规划能力。
...
### [32] [UniIntervene: Agentic Intervention for Efficient Real-World Reinforcement Learning](http://arxiv.org/abs/2606.12372v1)
*Haoyuan Deng,Yitong Gao,Yudong Lin,Haichao Liu,Zhenyu Wu,Ziwei Wang*

Main category: cs.RO

TL;DR: 本文提出UniIntervene模型，通过自主检测无效探索并恢复至高价值状态，将人机闭环强化学习中的被动人工纠正转变为价值感知的自主恢复过程，显著提升成功率并降低人工干预。


<details>
  <summary>Details</summary>
Motivation: 现有的人机闭环强化学习（HiL-RL）框架需要频繁的人工干预来纠正无效探索，导致人力成本高昂，限制了在真实世界场景中的可扩展性。

Method: 提出智能体干预模型UniIntervene：1) 利用未来条件化动作价值估计预测动作后果并评估价值以提供稳定进展信号；2) 通过时序价值-风险评论家聚合近期价值动态，在价值持续停滞或下降时触发干预；3) 从历史干预记忆中检索高价值恢复目标，并通过目标条件化恢复策略生成纠正动作。

Result: 在多种真实世界操作任务上的实验表明，相比最先进的HiL-RL基线，UniIntervene平均成功率提升了8.6%，人工干预减少了57%。

Conclusion: UniIntervene将干预机制从被动的人工纠正转变为价值感知的自主恢复过程，有效降低了人力成本并提升了真实世界强化学习的效率和可扩展性。
...
### [33] [Should LLM Agents Decide in Social Simulations? Comparing Finite-State and LLM-Based Decision Policies](http://arxiv.org/abs/2606.12369v1)
*Alejandro Buitrago López,Javier Pastor-Galindo,José A. Ruipérez-Valiente*

Main category: cs.CY

TL;DR: 本文评估了大语言模型（LLM）在社交网络仿真中作为动作选择器时对参考策略的保留能力，发现LLM无法可靠保留预期行为且计算成本显著增加。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为社交仿真中的决策组件存在方法论风险，可能导致仿真偏离研究人员定义的明确行为策略。

Method: 在包含1000个智能体和10000次动作决策的合成在线社交网络仿真中，以一阶马尔可夫模型为参考策略，测试三种开源LLM（LLaMA 3.1、GPT-OSS、Mistral 24B）在三种提示策略（基础、引导、概率）下的表现，并使用带拉普拉斯平滑的詹森-香农散度（JSD）评估策略对齐度及执行时间。

Result: LLM在某些配置下能近似参考策略但无法可靠保留；策略对齐度因模型和提示策略而异，额外引导甚至会引入系统性动作偏差；且即使是最优对齐的LLM配置，其执行速度也比直接马尔可夫链采样慢数百倍。

Conclusion: 基于LLM的动作选择不能直接替代显式决策策略，因为其不仅会改变预期行为，还会大幅增加计算成本。
