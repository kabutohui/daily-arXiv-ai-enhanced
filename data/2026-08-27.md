...
TL;DR: 本文提出了一种基于离散扩散大语言模型的放射学报告生成框架DRRG，通过迭代去噪实现报告的完善。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归框架逐词生成报告且无法修改前文，容易产生错误传播，且不符合放射学报告中迭代修改的实际过程。

Method: 提出DRRG框架，将报告生成表述为迭代掩码去噪过程；引入临床实体感知互补掩码提升监督覆盖率并强调关键实体；利用概念条件模块将图像衍生的临床概念注入视觉表示。

Result: 在MIMIC-CXR和CheXpert Plus数据集上，DRRG尽管使用了更小的LLM解码器，但在大多数指标（如BLEU-4、CheXbert-F1等）上均优于对比方法。

Conclusion: 离散扩散通过支持迭代、双向的报告完善，为自回归报告生成提供了有效替代方案；结合临床掩码和图像概念条件化可显著提升报告质量与临床一致性。
...
### [2] [VBVR-Pro: A Scalable and Verifiable Suite for Native Visual Reasoning](http://arxiv.org/abs/2608.26105v1)
...
TL;DR: 本文提出了VBVR-Pro，一个用于原生视觉推理的闭环测试平台，通过提供可扩展任务、可验证奖励和可控实验，解决了视觉推理训练的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 原生视觉推理将视觉生成作为推理媒介，但其进展受限于缺乏可扩展的训练任务、可靠的反馈机制以及跨生成介质的可控比较。

Method: 提出VBVR-Pro闭环测试平台，包含300个程序化生成的任务空间、基于确定性规则的可验证奖励评分器，并支持对30多种图像、视频及交错生成器进行可控机制研究。

Result: 训练后的模型在7个外部基准上表现出强泛化能力；所提评分器与人类判断高度一致，可作为可靠的强化学习奖励信号；机制研究表明视频生成在时空状态追踪上最强，交错生成更具计算效率，且视觉原生轨迹对视觉推理至关重要。

Conclusion: VBVR-Pro为原生视觉推理提供了可训练、可验证和可优化的基础设施，并开源全部数据、模型、评分器和代码，揭示了视觉生成介质在推理中的关键作用。
...
### [3] [A Visual Dependence-Aware Framework for Multimodal Unsupervised Continual Post-Training](http://arxiv.org/abs/2608.26095v1)
...
TL;DR: 本文提出视觉依赖感知（VDA）框架，通过利用视觉依赖的异质性和结构失真，解决多模态大语言模型在无监督持续后训练中的跨模态灾难性遗忘与可塑性问题。


<details>
  <summary>Details</summary>
Motivation: 现有无监督后训练方法均匀优化目标token，忽略了其异质的视觉依赖（VD）。本文旨在解决MLLMs在流式无标签数据上持续学习时，因忽略VD导致的跨模态灾难性遗忘及新任务学习低效问题。

Method: 提出视觉依赖感知（VDA）框架，包含：1) 视觉约束最优传输（VC-OT），将旧任务VD结构失真建模为最优传输问题以缓解跨模态遗忘，防止视觉依赖退化为语言偏见；2) 视觉调制适应（VMA），利用VD异质性强调基于视觉的新任务学习以促进可塑性。

Result: 大量实验验证了VDA在MU-CPT设定下的有效性，证明该方法能同时保持旧任务的稳定性和新任务的塑性。

Conclusion: 通过挖掘和利用token级视觉依赖的特性，VDA框架成功突破了多模态大语言模型在无监督持续后训练中面临的稳定性与可塑性困境。
...
### [4] [LongVU-TTT: Causal Test-Time Training for Visual Resampling in Long Video Understanding](http://arxiv.org/abs/2608.25729v1)
*Mahmoud Ahmed,Sameh Abdulah,Olatunji Ruwase,Sam Ade Jacobs,Mathis Bode,Mohamed Elhoseiny*

Main category: cs.CV

TL;DR: 本文提出LongVU-TTT框架，通过引入卷积测试时训练重采样器和帧选择器，解决长视频多模态大语言模型在有限视觉token预算下的时间变化建模与证据保留问题。


<details>
  <summary>Details</summary>
Motivation: 长视频多模态大语言模型在有限的视觉token预算下，通常会在保留大部分帧证据之前移除它们，导致难以有效建模时间变化。

Method: 在视觉编码器和大语言模型之间插入带有因果快速权重更新的卷积测试时训练（TTT）重采样器，利用分组2D快速权重在压缩前对帧特征进行上下文化处理，并结合混合均匀与变化感知选择器保留显式视觉证据。

Result: TTT-Conv在MLVU上比TTT-MLP提升高达2.12分，比双向Mamba2提升3.04分，在三个基准测试中优于注意力和固定状态循环重采样器；系统能处理512帧并压缩至128帧，在五个视频理解基准上表现具竞争力。

Conclusion: 快速权重更偏向于作为时间聚合状态而非可靠的长时情景记忆，其效益随证据距离增加而衰减，这证明了显式帧保留机制的必要性。
...
### [5] [MLLMCLIP: Feature-Level Distillation of MLLM for Robust Vision-Language Representations](http://arxiv.org/abs/2608.25575v1)
*Jongsuk Kim,Qiyu Wu,Zhuoyuan Mao,Hiromi Wakaki,Junmo Kim,Yuki Mitsufuji*

Main category: cs.CV

TL;DR: 本文提出MLLMCLIP异构蒸馏框架，通过从多模态大语言模型向CLIP学生模型进行特征级知识蒸馏，在不依赖合成数据的情况下同时提升了模型的组合性与通用表征能力。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉-语言模型（如CLIP）在组合性理解（如属性-对象和关系结构）上存在缺陷，而现有通过级联大模型生成合成困难负样本来缓解该问题的方法会带来巨大的流程开销。

Method: 提出一种异构蒸馏框架（MLLMCLIP），直接将生成式多模态大语言模型（MLLM）教师的知识转移给判别式CLIP学生模型。为弥合架构差异，引入了基于注意力的逐层token选择机制和基于CKA的蒸馏损失。

Result: 在组合性准确率上达到当前最优水平，同时在标准零样本分类和图文检索任务上也取得了持续的性能提升。

Conclusion: 特征级知识蒸馏能够同时增强视觉-语言模型的组合理解能力和通用视觉-语言表征能力，有效规避了传统合成数据方法的额外开销。
...
### [6] [Video-IFBench: Evaluating Instruction Following of Multimodal LLMs in Video Understanding Scenarios](http://arxiv.org/abs/2608.25529v1)
...
TL;DR: 本文提出了Video-IFBench，一个用于评估多模态大语言模型在视频理解任务中指令遵循能力的综合基准。


<details>
  <summary>Details</summary>
Motivation: 现有的视频理解基准主要关注任务准确性而忽略了指令遵循能力，导致模型在满足多样化用户约束（如视觉、音频内容及语义和格式要求）方面的能力未得到充分评估。

Method: 开发了一个包含四种指令模板（单任务、多任务、选择和嵌套指令）、32种任务类型和39种约束类别的分类法；并利用结合了MLLMs、程序化处理和人工验证的半自动数据构建管道生成了1.5K个评估样本。

Result: 对20多个最新MLLMs的评估显示，当前模型在视频指令遵循方面仍然面临挑战，特别是在处理包含多重约束、语义约束或需要基于视频内容选择正确分支的复杂条件结构时表现不佳。

Conclusion: 该基准的构建揭示了当前模型在视频指令遵循上的局限性，有望促进未来在视频理解场景下指令遵循能力的研究。
...
### [7] [SMART: MLLM-guided Temporal Alignment for Unifying Sign Language Recognition and Spotting](http://arxiv.org/abs/2608.25493v1)
*Eunjee Choi,JungHoon Sung,Seongwhan Cho,Chu Xin,Younggeun Choi*

Main category: cs.CV

TL;DR: 本文提出SMART框架，利用MLLM生成的运动描述作为辅助语义线索，结合多尺度时间适配器和CSFormer模块，实现联合连续手语识别与定位。


<details>
  <summary>Details</summary>
Motivation: 现有连续手语识别方法依赖句子级标注，缺乏细粒度的时序和语义指导；且传统视频-文本对齐需大批量训练，在内存密集的手语视频训练中效率低下。

Method: 提出SMART框架，利用MLLM生成的运动描述在小批量下进行稳定的视频-文本对齐；引入多尺度时间适配器（Multi-Scale Temporal Adapter）建模时序交互；设计CSFormer模块将识别证据注入边界感知定位网络，实现识别与定位监督的互补。

Result: 在PHOENIX14-T、CSL-Daily、大规模KSL及灾害与安全KSL四个手语基准数据集上的实验证明了SMART在识别和定位任务上的有效性。

Conclusion: SMART通过统一框架将弱监督CTC识别与定位监督相融合，利用多模态大模型辅助语义和多尺度时序建模，有效克服了现有方法的内存和细粒度指导局限。
...
### [8] [VISA: Agentic Self-Evolving Data Synthesis for Multimodal Instruction Following](http://arxiv.org/abs/2608.26013v1)
*Min Zeng,Guanxin Tan,Libin Cen,Yawei Wen,Rui Hu,Liuyang Bian,Xiaolong Chen,Xiaoxin Chen*

Main category: cs.CL

TL;DR: 本文提出VISA框架，通过将多模态指令合成重构为自演化循环，持续利用反馈改进数据质量与模型的多模态指令跟随能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态指令合成管道采用单次生成与过滤范式，丢弃了失败样本和目标模型错误的反馈，难以生成准确、多样、可验证且具挑战性的训练数据。

Method: VISA将指令合成建模为自演化循环：分析图像以提取可验证约束，从持久记忆中采样多样且难度感知的约束集生成指令，并利用工具和LLM裁判进行验证。失败样本触发诊断恢复，成功样本用于评估难度；验证信号与目标模型错误特征被写回记忆以指导后续迭代，并直接作为强化学习的奖励信号。

Result: 在MM-IFEval基准上的实验表明，VISA显著超越了强基线，提升了多模态指令跟随能力，且在七个公开基准上保持了通用的多模态性能。

Conclusion: VISA通过闭环反馈机制有效生成了高质量的多模态指令数据，免去了单独训练奖励模型的需求，为多模态指令跟随的提升提供了可扩展的解决方案。
...
TL;DR: 本文提出一种名为Entropy-Valley的免训练长度选择器，通过预测熵优化掩码扩散语言模型在机器翻译中的目标长度决策。


<details>
  <summary>Details</summary>
Motivation: 现有的掩码扩散解码研究主要关注词元揭示顺序，而在去噪前确定目标长度的问题未被充分探索，尽管该决策直接影响翻译的覆盖率和冗余度。

Method: 提出Entropy-Valley (EV)免训练长度选择器，通过全掩码前向传递计算候选目标画布的平均预测熵，并选择骨干网络最易填充的画布长度。

Result: 相比基线，EV在En-Zh、Zh-En和En-De上分别恢复了64.9%、65.3%和33.0%的COMET-22增益；专家评估证实了充分性提升；与LLaMA-3-8B自回归模型相比表现相当或更优；且诊断表明去噪友好长度无需匹配参考长度。

Conclusion: 在掩码扩散机器翻译设置中，如何提供目标长度比决定词元揭示顺序更重要，且最优去噪长度并不一定等同于参考长度。
...
TL;DR: 提出一种无需训练的解码方法RPS，利用中间熵枢纽位置的波纹效应来加速扩散大语言模型的并行解码。


<details>
  <summary>Details</summary>
Motivation: 现有dLLM并行解码调度器仅关注单位置标准，忽略了提前提交某些位置以降低其余掩码位置不确定性的潜在收益（即波纹效应）。

Method: 提出Ripple-Pivot Search (RPS)方法，寻找中间熵位置作为候选枢纽（决定在哪解码），并通过前瞻评估确定能带来最大下游收益的token分配（决定解码什么）。

Result: 在3个dLLM和4个基准测试中，RPS在保持生成质量的同时实现4-10倍真实时间加速，准确率较前瞻基线提升最高5.49%，结合KV缓存时加速比最高达18倍。

Conclusion: RPS有效利用波纹效应，在不损失质量的前提下显著加速dLLMs推理，突显了考虑下游收益的提前提交策略在并行解码中的重要性。
...
TL;DR: 提出了一种结合自回归前缀监督和后缀去噪的PCD预训练目标，以解决扩散语言模型中预训练与提示条件生成不匹配的问题，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型的原生预训练会随机损坏提示和续写标记，削弱了提示条件生成所需的干净前缀接口，导致训练和评估时存在不匹配问题。

Method: 提出Prefix-Conditioned Diffusion (PCD)预训练目标，通过修改注意力掩码、损坏掩码和标签构建，对干净前缀部分进行自回归监督，仅对未知续写部分应用扩散去噪，使局部训练接口与评估时的查询方式一致。

Result: 在LLaDA2-Mini和Qwen-1.7B骨干网络上，PCD相比原生dLLM基线持续提升，分别实现了4.2%（+2.56点）和14.2%（+4.86点）的相对增益。

Conclusion: 将预训练上下文分布与提示条件生成对齐，可以在不改变推理模式的情况下有效缩小扩散语言模型的续写性能差距。
...
TL;DR: 提出DominoTree，一种无需训练的最佳优先草拟树方法，利用Domino的条件路径相关修正评分提升推测解码效率。


<details>
  <summary>Details</summary>
Motivation: 现有树方法（如DDTree）基于因子化边缘分布展开候选树，无法表示Domino drafter中基于GRU的路径相关因果修正结构，导致无法充分利用路径条件信息。

Method: DominoTree使用Domino的非因子化条件修正沿每条根到节点路径进行评分，通过将每节点修正限制在候选top-M使计算可行，实现最佳优先树构建。

Result: 在8个基准测试中，于所有服务单元获得最高平均接受长度（24个数据单元中21个领先）；相比AR在Qwen3-8B上吞吐量提升最高7.3倍；在HELMET长上下文场景中接受长度提升29-36%、吞吐量提升10-34%；三臂分解显示路径修正贡献+4.7%接受长度增益。

Conclusion: DominoTree在verify-dominated场景中显著提升吞吐量和接受长度；在内存受限场景下链式方法在goodput方面占优；在prefill主导的超长上下文场景中与EAGLE-3持平。
...
TL;DR: GRAFT提出了一种用于基于扩散语言模型的推测解码的草稿树构建框架，通过改进边选择和预算分配来提升解码速度。


<details>
  <summary>Details</summary>
Motivation: 现有的树构建器采用基于父节点的条件扩展，不适用于单次前向传播生成所有未来位置分布的扩散语言模型草稿器；而现有的DLM树构建器仅基于令牌概率选择边且使用固定预算，忽略了父子兼容性和状态相关的最优树大小。

Method: GRAFT引入了目标蒸馏边缘评分（TDES），通过从目标模型轨迹中提取父子偏好来选择兼容的边；并采用状态感知预算分配（SABA），通过平衡预期草稿增益和验证成本来动态设置每轮树预算。

Result: 在多个模型和任务上，GRAFT实现了2.13倍至6.36倍的端到端加速，每轮增加的开销不到0.5毫秒，约占目标模型验证延迟的1.4%。

Conclusion: GRAFT通过优化边选择和预算分配，有效弥合了扩散语言模型草稿器与基于树的推测解码之间的差距，在极低开销下实现了显著的推理加速。
...
<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿树方法（如DDTree）在固定节点预算下最大化预期接受长度，但由于接受长度随预算非递减，忽略了验证成本，导致缺乏选择预算的原则性依据。

Method: CaDDTree显式建模草稿和验证延迟，将吞吐量目标分解为对预算的每轮一维搜索，并在凸验证成本下证明了吞吐量函数的单峰性，从而实现高效的贪婪停止规则，每轮动态调整预算且无需离线搜索。

Result: 在Qwen3-4B和Qwen3-8B模型及八个基准测试上的实验表明，CaDDTree在几乎所有任务上均匹配或超越了具有先知预算选择的DDTree。

Conclusion: CaDDTree为推测解码提供了一种原则性的成本感知方法，能够动态适应预算以最大化实际推理吞吐量，且无需依赖离线参数搜索。
...
TL;DR: 提出DDTree方法，通过块扩散草案模型的逐位置分布构建草案树并在单次前向传播中验证，有效提升了推测解码的接受长度和整体性能。


<details>
  <summary>Details</summary>
Motivation: 现有DFlash推测解码方法在每轮中仅验证单条草拟轨迹，限制了其接受长度，从而制约了加速潜力的充分发挥。

Method: 提出DDTree，直接从块扩散草案模型的逐位置分布构建草案树；在固定节点预算下，使用最佳优先堆算法依据草案模型输出定义的代理模型选择最优续写路径；随后利用仅祖先注意力掩码，在目标模型单次前向传播中高效验证该树。

Result: DDTree克服了DFlash单轨迹验证的限制，在固定计算预算下显著增加了接受的token长度，整体性能超越了EAGLE-3等强基线，跻身推测解码领域前列。

Conclusion: DDTree成功结合了块扩散草案模型与树结构验证机制，大幅提升了自回归语言模型的推理加速效果，是当前领先的推测解码方法之一。
...
### [16] [OmniPhys: A Unified Multimodal Benchmark for Physics Understanding and Generation from Chinese Educational Corpora](http://arxiv.org/abs/2608.25398v1)
*Hao Chen,Yumin Lin,Nadila Yushanjiang,Xin Lin,Min Zhang*

Main category: cs.CL

TL;DR: 本文引入了大规模多模态物理基准测试OmniPhys，以评估当前多模态大语言模型在物理推理和视觉生成方面的能力与不足。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在物理领域的发展因缺乏全面的基准测试而受到严重阻碍。

Method: 构建了OmniPhys基准，包含15,246个问题和19,850张从中学到大学水平的物理图像，提供详细标注，并系统性评估模型生成结构化物理图表等多模态输出能力。

Result: 广泛的评估揭示了当前MLLM在复杂推理和视觉生成方面存在关键缺陷。

Conclusion: 发布OmniPhys可作为推进物理和科学领域多模态智能的基础资源。
...
### [17] [Agentic Autoresearch for Cell-Edge Power Control: Radically Redefining the Researcher's Role](http://arxiv.org/abs/2608.26093v1)
*Ahmad Khan,Akram Bin Sediq,Sara Azadegi Naeini,Raviraj S. Adve*

Main category: cs.LG

TL;DR: 本文提出一种自主AI代理协议，完全接管无线资源管理中机器学习算法的设计，在大幅降低推理成本的同时达到接近最优的性能。


<details>
  <summary>Details</summary>
Motivation: 旨在解决无线资源管理的机器学习算法设计过程劳动密集且需人工指定架构、损失函数和训练配方的问题。

Method: 采用autoresearch协议，赋予AI编码代理对架构、输入表示、输出参数化、损失函数等的完全控制权，通过固定预算实验迭代优化非凸、强NP难的多小区网络功率控制问题，并使用哈希固定评估器等保障措施确保结果可信。

Result: 在81次无人值守实验中，代理达到收敛的次小化-最大化参考方法的99.5%性能，推理成本降低约600倍，缩小了94%的性能差距，且单一参数集适用于所有网络规模和百分位目标。

Conclusion: AI代理可完全接管算法设计层，不仅提升效率，还能恢复可证明的结构而非仅调优常数，其发现的输出参数化能在最小百分位下重现精确的最大最小最优分配。
...
### [18] [TraceML: An Empirical Analysis of Human-Agent Planning in Machine Learning Development](http://arxiv.org/abs/2608.26086v1)
*Jiarui Yan,Weiwei Sun,Sijie Li,Wenhan Li,Yiming Yang*

Main category: cs.LG

TL;DR: 本文提出TraceML数据集，通过版本级标注对比人类与AI代理在Kaggle竞赛中的开发轨迹，揭示代理行为模式窄化是性能差距的原因。


<details>
  <summary>Details</summary>
Motivation: 现有基于结果的基准只能记录大语言模型代理与人类在自主机器学习开发中的性能差距，却无法揭示差距成因，因为它们仅评分最终提交而忽略了开发过程。

Method: 构建TraceML数据集，采用统一版本级模式标注4,465条人类Kaggle轨迹和430条配对人类及207条代理轨迹，每个代码版本附带分数、时间戳、动作类型、意图、编辑规模及分数影响标签，从而对比人类专家与两种代理框架的行为模式。

Result: 专家交替进行数据处理、验证、模型修改和集成，并会重新尝试先前放弃的方法；而代理陷入狭窄循环——Codex反复重加权集成和调优提交，MLEvolve原地变异模型，二者均无法以人类频率切换策略或重开废弃工作。从人类实践中提炼的简短规划提示可改善部分行为并提升分数，但整体行为轮廓仍为代理型。

Conclusion: 指令式干预仅能弥合可还原为指令的那部分差距，代理与人类之间的深层行为模式差异难以通过提示完全消除；已公开数据集、标注模式、标注器和提取流程以供进一步研究。
...
TL;DR: 本文提出Consistency Forcing (CForce)方法，通过对齐扩散语言模型早期和后期去噪阶段的掩码预测，提升高并行解码下的速度与质量权衡。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在激进并行解码策略下，早期去噪阶段的预测不可靠，错误会传播到后续阶段，影响生成质量。本文旨在解决早期阶段预测误差累积的问题，以改善训练-推理对齐。

Method: 提出CForce蒸馏方法，在预收集的自rollout轨迹上训练模型，强制早期阶段掩码预测与后期阶段对齐；引入Confidence Adaptive KL Divergence作为蒸馏目标，融合正向和反向KL的优势；并提供理论分析说明CForce如何近似最小化早期阶段预测误差。该方法统一适用于mask-to-token解码和edit-capable解码。

Result: 在非编辑和edit-capable LLaDA模型上的实验表明，CForce改善了速度-质量权衡，尤其在高并行解码预算下效果显著。

Conclusion: CForce通过一致性蒸馏有效减少了扩散语言模型早期去噪阶段的预测误差，提升了高并行场景下的生成质量，且统一框架适用于多种解码模式，代码已开源。
...
### [20] [Diffusion LLMs as Targets and Adversaries: Mechanistic Safety Exploits](http://arxiv.org/abs/2608.07430v1)
*Elena Dumitrescu,Gert Lek,Lydia Y. Chen,Jérémie Decouchant*

Main category: cs.LG

TL;DR: 本文揭示了扩散大语言模型（DLLMs）安全对齐的机制性脆弱性，并提出了一种基于安全神经元引导的离线黑盒越狱框架SN-Guided Diffusion。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型以迭代并行去噪取代自回归预测，但其内部安全机制尚不明确，本文旨在揭示DLLMs安全对齐的机制性漏洞。

Method: 首先通过安全神经元映射与剪枝揭示DLLMs安全对齐的稀疏性和跨架构可迁移性；在此基础上提出SN-Guided Diffusion框架，利用加权安全神经元损失引导扩散过程远离安全触发区域，实现离线黑盒越狱。

Result: 自剪枝使LLaDA和Dream的攻击成功率（ASR）分别从2.6%升至73.8%、1.9%升至86.6%；跨模型迁移剪枝在Dream和Fast-dLLM上ASR分别达73.2%和86.3%；SN-Guided Diffusion在Llama-3-8B-Instruct、Qwen2.5-7B-Instruct和Gemini-2.5-Flash-Lite上分别实现77.1%、86.9%和74.3%的迁移ASR，且仅需20次生成episode，生成成本比现有方法低数个数量级。

Conclusion: DLLMs的安全对齐机制稀疏且可迁移，从自回归模型继承的安全足迹使其易受迁移攻击；SN-Guided Diffusion以极低计算成本实现了高迁移性的越狱攻击，凸显了扩散式对齐机制的改进需求。
...
### [21] [Retrofitting Linear Attention into Diffusion Language Models](http://arxiv.org/abs/2608.06628v1)
*Jinha Kim,Younghun Roh,Jaeyeon Kim*

Main category: cs.LG

TL;DR: 本文提出块混合注意力机制，通过线性化先前块的注意力来加速扩散语言模型的推理。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型虽然通过并行解码加速推理，但在块级半自回归解码中，每个去噪步骤仍需关注所有先前块，导致重复的前缀注意力开销，成为推理瓶颈。

Method: 提出块混合注意力，在活跃去噪块内保留精确的softmax注意力，对先前块应用线性注意力；通过少量后训练将其集成到预训练的16B模型LLaDA中。

Result: 模型转换仅需约60小时且基准性能基本保持（HumanEval 72.0% vs 75.6%，MBPP+ 63.0% vs 57.7%，CMATH 86.7% vs 88.3%）；Triton实现使解码吞吐量提升最高达1.7倍，并支持更多并发请求。

Conclusion: 预训练的扩散语言模型可以高效地通过线性化实现更快的推理，且不显著降低性能。
...
### [22] [Bayesian Optimization for Self-Driving Materials Laboratories: From Algorithms to Physics-Informed Workflows](http://arxiv.org/abs/2608.26016v1)
*Yuki K. Wakabayashi,Takuma Otsuka*

Main category: cond-mat.mtrl-sci

TL;DR: 本综述探讨了贝叶斯优化在材料研究自动驾驶实验室中的应用，重点解决了实际材料研发中的非标准黑盒挑战并展望了未来方向。


<details>
  <summary>Details</summary>
Motivation: 实际材料研发往往偏离标准贝叶斯优化黑盒设置，面临实验失败、噪声与漂移、混合变量、约束及多目标等复杂挑战，亟需解决这些实际问题以实现高效实验。

Method: 总结了基于高斯过程的贝叶斯优化与材料目标定量表述，讨论了代理建模和获取函数，并重点阐述了通过表示、先验、核函数和约束引入领域知识的物理信息贝叶斯优化（PIBO）。

Result: 调研了BO及相关主动学习在半导体、催化、电池和合金等领域的成果，展示了其在发现新材料、新合成路线、提升性能及生成可复用科学知识方面的进展。

Conclusion: 概述了非平稳性、多模态观测及基于人类与大语言模型的科学推理等开放性问题，指出解决这些问题将推动自动驾驶实验室向可解释和生成知识的实验方向发展。
...
### [23] [Epistemic Networks, Collective Misperception, and the Manipulation of Social Knowledge](http://arxiv.org/abs/2608.26075v1)
*Mihnea C. Moldoveanu,Joel A. C. Baum*

Main category: cs.SI

TL;DR: 本文提出了一个基于相互归因张量的网络交互信念框架，将集体认知共识、极化、根深蒂固的误知觉和不稳定性统一为单一算子的谱区间。


<details>
  <summary>Details</summary>
Motivation: 解决网络中交互信念的结构问题——即代理如何持有、修正并依据对他人认知状态的模型采取行动，以及集体误知觉、从众行为和极化等现象如何从高阶信念结构中涌现。

Method: 以相互归因张量（记录每个代理认为每个代理所持有的信念）为分析单元，区分私下持有、公开表达和被他人相信三个层次；社会信念通过张量对带符号的认知影响矩阵的收缩运算演化。

Result: 集体误知觉精确分解为观测可消除和观测不可触及两个分量；社会从众放大但不产生多元无知；共识、极化、顽固误知觉和不稳定性是单一算子的谱区间；高阶信念在认知一致性假设下归约为认知网络中的游走。

Conclusion: 多样化的集体认知现象可在单一谱框架下统一描述，高阶信念可归约为网络游走，但该归约依赖于认知一致性假设，其边界已被精确界定。
...
### [24] [PlanSightRAG: A Visual-First Multimodal RAG for Automating Question Answering and Compliance Checking for Civil Standard Plans](http://arxiv.org/abs/2608.26091v1)
*Nabaraj Subedi,Shuvo Dip Datta,Ahmed Abdelaty,Shivanand Venkanna Sheshappanavar*

Main category: cs.IR

TL;DR: 提出视觉优先的多模态RAG框架PlanSightRAG，通过直接索引和推理图像来解决土木基础设施合规检查中OCR丢失几何与布局信息的问题。


<details>
  <summary>Details</summary>
Motivation: 旨在解决土木基础设施合规性检查长期依赖人工阅读2D图纸，以及传统OCR自动化方法会剥离图纸中必不可少的几何和布局信息的问题。

Method: 提出PlanSightRAG框架，直接对图纸图像进行索引和推理，集成了ColNomic-3B多向量检索、智能体规划-检索-审计-合成器架构，并利用MaxSim热力图作为证据链，实现了从规范语料库中自主提取数值限制的视觉规则定位。

Result: 在4056对基准测试中零样本检索Recall@5达91.47%（密歇根州DOT语料库为91.40%）；在合成合规图纸测试中，Qwen2.5-VL-72B流水线在提供预解析规则阈值时达到100%准确率，优于非VLM OCR基线的76.4%。

Conclusion: 该研究证明了多模态RAG框架能够无需人工提供规则即可自主提取数值限制并完成视觉规则定位，为土木基础设施合规检查的自动化提供了有效的新方向。
...
### [25] [A Self-Evolving Multi-Agent Framework Defense against LLM Jailbreak Attacks](http://arxiv.org/abs/2608.26008v1)
*Tongyan Hu,Bryan Hooi*

Main category: cs.CR

TL;DR: 本文提出一种基于持久化跨交互规则记忆的自我进化测试时防御机制，通过提取攻击方法级规则来动态抵御大语言模型的越狱攻击。


<details>
  <summary>Details</summary>
Motivation: 现有的越狱防御手段多为静态，无法在部署后积累防御经验或适应未见过的攻击策略，难以应对不断演变的攻击手法。

Method: 提出一种基于持久化规则记忆的自我进化测试时防御机制。当攻击成功时，系统将失败抽象为捕获攻击结构而非特定主题的方法级规则并存入外部记忆，以便在后续交互中防御同类攻击。该机制完全通过外部记忆和提示词操作，无需参数更新，兼容开源与黑盒模型。

Result: 在四种黑盒越狱攻击家族和多个模型上的实验表明，该方法显著降低了攻击成功率，保持了良性实用性，在自适应复合包装攻击下表现鲁棒，且记忆增长不会导致过度拒绝。

Conclusion: 基于外部记忆的适应机制能够有效防御不断演变的越狱攻击，无需模型重训练即可实现跨攻击家族的泛化和动态防御。
...
### [26] [MMJailBench: A Factorized Benchmark for Disentangling Multimodal Jailbreak Vulnerabilities](http://arxiv.org/abs/2608.25490v1)
*Tianshi Wang,Jingsong Wang,Yafei Huang,Fengling Li,Xin Li,Lei Zhu*

Main category: cs.CR

TL;DR: 本文提出了MMJailBench，一个用于系统性评估和归因多模态大语言模型越狱漏洞的因子化基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态越狱基准通常将有害意图、提示框架、视觉语义和指令载体等因素耦合在一起，导致难以准确理解这些特定因素如何影响模型的越狱漏洞。

Method: 提出MMJailBench基准，在受控配置下系统地变化和组合不同越狱因素以实现细粒度的因子级归因；同时分析模型内部表示与跨模态交互的漏洞模式，并开发了一个模块化、可配置的多模态越狱评估套件。

Result: 对16个模型的评估表明漏洞特征高度异质且依赖模型；提示框架是漏洞变化的主导因素；任务相关的视觉语义会显著增加越狱易感性，特别是结合权威性线索时；视觉渲染指令相比直接文本指令并未持续增加易感性。

Conclusion: 当前多模态安全对齐在不同危害域上的覆盖不均匀；所开发的评估套件能够支持可复现、可扩展且成本高效的多模态越狱审计。
...
### [27] [SwarmWorld: Stigmergic technological evolution in societies of language-model agents](http://arxiv.org/abs/2608.26081v1)
*Subhadeep Pal,Fiona Y. Wang,Markus J. Buehler*

Main category: cs.AI

TL;DR: 本文提出SwarmWorld环境，证明去中心化、无预设角色的LLM智能体能通过共享物理环境自组织成进化的技术社会，并构建出比独立搜索更具韧性的技术组合。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统大多依赖直接对话、预设角色或集中式工作流，尚不清楚去中心化的智能体能否在没有这些设定的条件下构建功能性技术并超越独立的智能体搜索。

Method: 在SwarmWorld中，初始同质的LLM智能体在无预设角色或规则的情况下，探索环境、处理资源、构建人工制品并编写可执行控制器。系统将认知与后果分离，由确定性模拟器评估智能体提出的架构和控制器在未知干扰下的功能。

Result: 共享社会构建的技术组合比独立搜索基线更广泛且更具韧性（尽管独立搜索在单一最强制品上仍有竞争力）。智能体自发分化为探索、建造等行为；技术主要通过物理观察而非交流进行重用和积累。

Conclusion: 去中心化LLM智能体能够通过物理共识（stigmergy）自组织形成有能力的功能性社会，而交互过程主要驱动了持久的技术生态，而非单纯产生更优的个体发明。
...
### [28] [Trace Integrity for LLM Data Agents: A Vision for Auditable Structured Reasoning in Real-World Systems](http://arxiv.org/abs/2608.26036v1)
*Srimonti Dutta,Akshata Kishore Moharir*

Main category: cs.AI

TL;DR: 本文提出“追踪完整性”标准与“执行契约”，以解决大语言模型数据代理中仅凭答案准确率无法保证计算过程可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的答案准确率不足以作为大语言模型数据代理的可靠性信号，因为正确的答案可能由无效的计算追踪产生，这被称为结构鸿沟。

Method: 提出“追踪完整性”评估标准和“执行契约”来绑定用户意图与可执行查询等要素；同时引入CAIT率来衡量计算不支持但被算作成功的频率。

Result: 在BIRD Mini-Dev上的实验表明，不同方法答案准确率虽达20%-24%，但追踪完整性通过率仅为39%-43%，且CAIT率高达45.8%-59.1%，证明答案准确率、追踪有效性与静默失败风险是不同的评估信号。

Conclusion: 现实世界的LLM数据代理不应仅通过答案匹配来评估，还应根据其输出是否由可审计的计算过程支撑来进行评估。
...
### [29] [AsymSpec: Context-Asymmetric Speculative Decoding for Agentic LLMs](http://arxiv.org/abs/2608.26004v1)
*Sheng Liang,Yongyue Zhang,Nathanael Brian,Hang Lv,Hao Wang,Chen Zhang,Yong Liu*

Main category: cs.AI

TL;DR: AsymSpec提出一种非对称推测解码框架，由轻量级草稿模型读取完整上下文来引导处理压缩上下文的大模型验证器，以在不牺牲准确率的前提下降低LLM智能体的推理成本。


<details>
  <summary>Details</summary>
Motivation: 解决智能体LLM管道中上下文积累导致推理成本高昂的问题。常规输入压缩会降低任务准确率，而传统推测解码要求草稿模型与验证模型共享相同上下文，无法解决准确率与开销的权衡难题。

Method: 提出AsymSpec非对称推测解码框架：轻量级草稿模型读取完整输入，大模型验证器处理压缩视图；通过对比δ-融合logits引导验证器，并采用发散感知接受门进行调节，以保持验证稳定性和高接受率。

Result: 在四个智能体能力和两个端到端基准测试中，平均达到约90%的完整上下文准确率，并在独立文本能力上实现1.3至1.7倍的吞吐量加速，计算成本仅为0.2至0.3倍。

Conclusion: 非对称上下文访问机制在输入压缩导致关键推理信号丢失时能提供显著收益，有效平衡了LLM智能体的推理开销、延迟与任务准确率。
...
TL;DR: 本文分析了掩码扩散语言模型在并发服务下的行为特征，指出其服务机制不同于自回归模型，需要在去噪步骤级别进行并行化。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散语言模型服务系统缺乏在真实并发负载下的行为测量基础，直接套用自回归模型的服务假设可能存在风险，本文旨在填补这一空白。

Method: 使用LLaDA-8B-Instruct和D2F LoRA适配器在单张NVIDIA H200 GPU上运行GSM8K和HumanEval数据集，分析请求难度分布、延迟方差、计算开销及批处理性能，并推导泊松到达下的批处理超时规则。

Result: 请求难度呈离散分布且生成前不可预测；短生成预算会掩盖延迟方差；单请求中仅24%为GPU计算，批处理可通过摊销CPU调度开销使吞吐量提升16倍且不降低输出质量；推导出了批处理超时规则。

Conclusion: 扩散语言模型的服务需要在每个去噪步骤级别进行并行化，其准入和逐出机制与共享前向传递的交互方式与自回归模型存在本质差异。
...
TL;DR: 提出一种无需训练的推理方法CAI-DLLM，利用首步置信度指导扩散语言模型的去噪过程，在提升生成质量的同时显著加速推理并降低能耗。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在推理时需要重复去噪步骤，且会反复计算已经稳定的token，导致生成成本高昂。

Method: 提出CAI-DLLM，一种利用首步置信度信号指导去噪的无训练推理方法：提前提交简单token，将更多去噪步数分配给困难token，并跨输出块调整解码调度，无需重训练或额外预测器。

Result: 在LLaDA-8B和Dream-7B模型上测试，LLaDA GSM8K任务实现18.2倍加速且准确率升至77.41%；Dream HumanEval实现13.1倍加速且pass@1升至48.17%。在困难推理任务中加速达44.8倍，能耗降低高达95.3%。

Conclusion: 基于首步置信度的动态去噪调度能大幅提升扩散语言模型的推理效率与能耗表现，且在多数任务中保持或提升了准确率。
...
TL;DR: 本文提出了连续交互扩散（CID）架构，将工具交互集成到扩散语言模型的迭代去噪过程中，以解决传统自回归式工具调用的效率问题。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型通过并行精炼进行推理，传统的“停止-恢复”式工具调用模式会限制其能力，迫使模型在推理稳定前做出工具决策，延迟有用观察，并引入冗余精炼和工具执行，从而损害任务准确性和推理效率。

Method: 提出连续交互扩散（CID）架构，将工具交互集成到迭代去噪中。它分离了模型只读事实通道、由类型化认知张量表示的思维通道和显示通道，允许在调用完全序列化前启动外部读取，并将结果投影到演变的思维状态中，同时利用持久绑定重用静态结果。

Result: 本文形式化了该架构、运行时和训练目标，并定义了任务质量和端到端效率的评估协议，但未提出任何经验性能主张（本文仅关注只读工具）。

Conclusion: CID旨在更早暴露证据，将工具延迟与模型计算重叠，减少重复外部工作，并在新证据到达后保留有用计算；未来的方向将进行实证性能评估。
...
TL;DR: 本文提出TAPS方法，通过目标感知的前缀选择优化扩散模型推测解码中的验证过程，显著提升无损加速效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的推测解码将瓶颈转移至验证阶段，且现有草案树方法忽略验证的前缀条件性，使用边际概率排序，导致验证被拒绝前缀的不可达后代，增加不必要的延迟。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边际概率转化为路径条件的接受估计，并在固定验证预算下选择紧凑的前缀封闭子树。

Result: TAPS实现了高达7.9倍于原始自回归解码的无损端到端加速，分别比最先进的DFlash和DDTree方法快1.36倍和1.74倍。

Conclusion: 通过优化前缀选择而非简单扩展草案树，TAPS有效改善了接受率与成本的权衡，显著提升了推测解码的整体效率。
...
### [34] [Multi-Granularity Context-Enhanced RAG over Multimodal Knowledge Graphs](http://arxiv.org/abs/2608.25986v1)
*Zongyu Wu,Yilong Wang,Xiaochen Wang,Minhua Lin,Zhichao Xu,Fenglong Ma,Xiang Zhang,Suhang Wang*

Main category: cs.AI

TL;DR: 本文提出一种上下文增强的多模态知识图谱(CEMMKG)构建框架，通过引入局部和全局文本上下文弥合图文语义鸿沟，从而提升多模态GraphRAG的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于多模态知识图谱(MMKC)的RAG方法在融合前独立处理各模态，导致文本上下文利用不足，产生图文语义鸿沟，限制了多模态GraphRAG的性能。

Method: 提出构建上下文增强多模态知识图谱(CEMMKG)的框架，通过局部和全局视角利用互补文本上下文来丰富图像信息；局部上下文包含语义相关句子并采用多粒度设计，全局上下文提供整个段落的总结。

Result: 在视觉中心数据集上的实验验证了CEMMKG能有效利用上下文信息提升基于MMKG的RAG性能，且在不同RAG方法中均表现出广泛的适用性。

Conclusion: 引入多粒度的局部和全局文本上下文能够有效弥合图文语义鸿沟并增强多模态GraphRAG性能，该框架具有良好的通用性和应用价值。
...
### [35] [SciMIF: Understanding Multimodal Instruction Following in Scientific Domains](http://arxiv.org/abs/2608.25973v1)
*Ye Shen,Yuting Zheng,Dun Pei,Zijian Chen,Wenlong Zhang,Qi Jia,Guangtao Zhai*

Main category: cs.AI

TL;DR: 本文提出了SciMIF，一个用于评估多模态大语言模型在科学领域遵循复杂指令能力的新型基准测试。


<details>
  <summary>Details</summary>
Motivation: 科学领域中多模态大语言模型的指令遵循能力尚缺乏系统评估，而这一能力对于将MLLMs有效应用于推动科学发展至关重要，因此需要专门的基准测试来填补这一空白。

Method: 基于对5个代表性科学学科中22个不同任务的分析，构建了包含10个约束类别的综合分类体系，并据此开发了高保真指令注入流水线来系统增强现有科学数据集，最终对多个前沿闭源和开源MLLMs进行综合评估。

Result: 实验发现不同科学学科间存在显著性能差异，化学对当前MLLMs挑战最大；增加模型规模并不能相应提升约束遵循能力；当前模型在细粒度约束和需要深度应用学科知识的指令方面仍严重不足。

Conclusion: SciMIF填补了科学领域多模态指令遵循评估的空白，为未来在严谨科学应用中改进MLLMs奠定了重要基础，数据和代码已开源。
