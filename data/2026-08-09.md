...
### [1] [Evidence-Driven Dynamic Visual Selector for Efficient Long Video Understanding](http://arxiv.org/abs/2608.05780v1)
*Bo Zhang,Wenxin Wang,Feng Chen,Zhihao Zhang,Zixuan Wang,Changsheng Li,Yinjie Lei*

Main category: cs.CV

TL;DR: 提出基于目标MLLM内部注意力证据的动态视觉选择框架EviSelect，通过稀疏预填充和强化学习优化实现高效长视频理解。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解方法依赖外部代理评分器和死板的启发式规则，导致与目标MLLM的内在证据不对齐，且无法适应视频非均匀的时空信息密度。

Method: 提出EviSelect框架，利用高度压缩的视觉输入和稀疏注意力近似目标MLLM的注意力图作为结构化先验，设计轻量级选择器以定位相关时间戳并自适应调整局部采样率与空间分辨率；将选择器公式化为随机策略，并通过GRPO在联合准确率与效率的奖励下进行优化。

Result: 在三个长视频理解基准上取得优于现有方法的性能，同时将选择的视觉Token数量减少约50%，实现3.9倍的端到端加速。

Conclusion: 利用目标MLLM内部注意力证据动态分配计算资源的方法能够有效平衡长视频理解的准确性与效率，显著降低视觉成本并提升推理速度。
...
### [2] [Beyond Frame Selection: Rethinking Long-Video Understanding with MLLMs](http://arxiv.org/abs/2608.05592v1)
*Ziling Huang,Shin'ichi Satoh*

Main category: cs.CV

TL;DR: 本文提出VideoRouter(VR)，通过构建时序层级结构生成全局与局部互补视图，并利用验证引导的路由器协调推理，以解决长视频理解中全局上下文与细粒度证据难以兼顾的问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在长视频理解中受限于token数量，难以捕获时间稀疏证据。现有均匀采样或帧选择方法无法同时兼顾全局剧情上下文和细粒度局部证据。

Method: 将视频组织成与问题无关的时序层级结构以生成全局（覆盖性）和局部（细节性）两种互补视图；引入验证引导的路由器评估并选择证据支持度更高的视图来输出最终答案。

Result: 实验证实验证引导的路由器能有效协调全局与局部推理；在VideoMME数据集上，基于LLaVA-Video-7B主干网络，该方法比现有最先进的帧选择方法提升了2.9分。

Conclusion: 将长视频理解重新定义为协调互补证据视图的方法是有效的，显著提升了模型性能，作者计划开源代码以供后续研究。
...
### [3] [From Sports to Safety: Benchmarking Proactive Risk Inference in MLLMs](http://arxiv.org/abs/2608.05560v1)
*Jiawei Qiu,Yichen Xu,Jianzhe Ma,Mingyang Yu,Wenbin Zhu,Yang Han,Pinzheng Lv,Wenxuan Wang*

Main category: cs.CV

TL;DR: 本文提出了SPRINT基准，用于评估多模态大语言模型在体育视频中主动预测物理危险的能力，发现当前模型在危险归因和减少误报方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有的MLLM评估主要关注有害内容或一般风险，对动态物理环境中的主动物理危险预测探索不足；体育视频包含多样的受伤维度和事故前的时空线索，是研究该问题的理想测试床。

Method: 提出了SPRINT（Sports Proactive Risk INference Testbed）基准，包含2,888个真实世界体育视频及细粒度标注（早期危险线索、事故时机、分层原因），并在多种提示和时间窗口下对最先进的MLLM进行评估。

Result: 最佳模型在发出危险信号方面准确率超过95%，但在识别危险原因方面低于50%；此外，明确的危险查询会在无危险视频中引发严重的误报。

Conclusion: 当前MLLM仅表现出表面的主动安全性，缺乏稳定、基于因果关系的早期预警能力，凸显了在动态物理环境中开发可靠主动安全技术的必要性。
...
### [4] [The Bitter Lesson of Tool Calling](http://arxiv.org/abs/2608.06370v1)
*Ishan Patel,Sahil Sen,Elias Lumer,Vamse Kumar Subbiah*

Main category: cs.CL

TL;DR: 本文实证对比了程序化工具调用（PTC）与原生JSON工具调用，发现PTC在多数模型上表现相当或更优，且在并行和上下文退化条件下更具鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管程序化工具调用能替代僵化的JSON调用并支持自然的链式与并行操作，但目前缺乏在真实任务条件下跨不同模型代际的系统性基准评估。

Method: 在BFCL v4基准上对比14个语言模型的程序化工具调用与原生JSON调用。PTC范式将工具暴露为类型化的Python存根，由模型通过代码调用，并在单轮智能体交互中完成执行与结果处理。

Result: PTC在14个模型中有11个持平或优于JSON基线（GPT-5.6系列提升10.6%）；在并行扇出场景中14个里13个表现更优；在基线平均下降2.3%的上下文腐化条件下保持性能稳定。

Conclusion: 程序化工具调用是JSON工具调用的一种可行且稳健的替代方案，其性能表现随模型发布代际的能力提升而相应增强。
...
### [5] [Benchmarking the Benchmarks: Evaluating Benchmarks for Conversational Agents](http://arxiv.org/abs/2608.06329v1)
*Noam Koren,Roy Bar-Haim,Abigail Goldsteen*

Main category: cs.CL

TL;DR: 该论文提出了一个基于大语言模型（LLM）评判器的无参考框架，用于评估任务导向对话代理基准测试的质量。


<details>
  <summary>Details</summary>
Motivation: 现有任务导向对话代理的基准测试很少评估其自身质量，而质量低下的基准（包含不一致任务、过度简化的场景或有限的策略覆盖）会导致对代理的评估不可靠。

Method: 提出一种无参考框架，利用LLM评判器评估基准测试的一致性、复杂性和策略覆盖率，并提供可操作的弱点诊断。通过与人类标注对比、评估不同能力LLM生成的基准以及施加受控质量退化扰动来验证该框架。

Result: 在跨领域和不同评判模型下，所提指标能与人类标注达成一致，并能稳定区分不同基准测试的质量水平；同时验证了该框架对人工策划基准测试的适用性。

Conclusion: 该框架为评估合成和人工策划的对话代理基准测试提供了一种实用方法，有助于提升对话系统评估的可靠性。
...
### [6] [Benchmarking and Enhancing LLMs for Rule-Intensive Review of National Standard Documents](http://arxiv.org/abs/2608.06312v1)
*Tao Wang,Qihao Yang,Rongjiao Liang,Lianghong Lin,Haitao Wang,Xinyu Cao,Tianyong Hao*

Main category: cs.CL

TL;DR: 该论文引入了首个国家标准文件结构化审查基准GB/T-Bench，并提出多智能体框架GB/T-Reviewer以提升大语言模型在此类规则密集型任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在规则密集型文件审查中的能力尚未得到充分评估。现有基准侧重于领域知识和问答，忽略了专业文件的内在质量审查，而此类审查严重依赖昂贵且难以规模化的人类专家。

Method: 提出包含25种错误类型分类体系、可控反例生成机制和面向诊断的评估协议的GB/T-Bench基准；并开发多智能体框架GB/T-Reviewer，将审查知识转化为专业技能，协调全局检查、针对性诊断、规则扫描和结果验证。

Result: 14个主流大模型与人类专家表现差距显著（最强模型CMCS仅为0.3280，专家为0.6640），而GB/T-Reviewer框架将最佳CMCS提升至0.5094。

Conclusion: 结构化的技能协调对规则密集型文件审查具有重要价值，为标准化及其他高风险文件领域的可信AI奠定了基础。
...
### [7] [Answer First, Reason Later: Commitment Order in Diffusion LLMs](http://arxiv.org/abs/2608.05687v1)
*Jewon Yeom,Jaewon Sok,Seonghyeon Park,Jeongjae Park,Hwiyeong Lee,Taesup Kim*

Main category: cs.CL

TL;DR: 本文揭示掩码扩散语言模型在推理任务中无约束的任意顺序令牌提交导致推理失效，并提出前沿门控承诺机制恢复性能差距同时保留并行解码优势。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型被宣传的核心优势是可以任意顺序提交令牌，但在推理任务中这种自由反而成为失败根源。纯解码器在推理区域大部分仍被掩码时就过早提交最终答案，随着生成画布增长，高达90%的问题退化为仅输出答案而缺失推理过程，严重影响推理质量。

Method: 通过记录LLaDA-8B在GSM8K上的每次提交轨迹，采用2x2提示-解码器设计分离链式思维与提交顺序的交互效应，将交互分解为崩溃通道和顺序通道，并提出单旋钮干预——前沿门控承诺机制，根据测量的前沿曲线动态调整承诺窗口大小。

Result: 链式思维仅在有序提交下有效（交互效应+34.8个百分点，95% CI [26.8, 42.8]）；前沿门控承诺因果性地恢复了全部性能差距（准确率从0.528提升至0.852），同时保留高达4倍并行解码速度；最优窗口从完全精炼时的w=1翻转为8令牌/步时的无约束；结果在Dream-7B和MATH-500上得到复现。

Conclusion: 现有窗口式采样器此前仅从效率角度被理解，实际上它们是对推理病理学的最小修复方案——该病理学指无约束解码在远距离位置过早提交答案导致推理崩溃。研究重新框定了扩散语言模型解码策略的设计动机，为未来推理导向的采样器设计提供了理论基础。
...
### [8] [OPTD: On-Policy Transition Distillation with Consistency-Guided Adaptive Compression for Few-Step Diffusion Language Models](http://arxiv.org/abs/2608.02942v1)
*Xiaocheng Lu,Hualei Zhang,Shuhan Guo,Jie Zhang,Xiaoyi Pang,Jian Liu,Haoxi Li,Bohai Gu,Haoxuan Che,Jingcai Guo,Song Guo*

Main category: cs.CL

TL;DR: 本文提出OPTD方法，一种基于一致性引导自适应压缩的按策略过渡蒸馏技术，有效解决了扩散语言模型少步蒸馏中的状态偏移问题，提升了生成质量与效率的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散语言模型少步蒸馏方法在离策略轨迹上构建监督，导致推理时学生模型早期并行承诺改变后续预测上下文，引起状态偏移。而传统的按策略蒸馏难以平衡过渡推进程度，面临压缩受限或破坏中间依赖的问题，本文旨在解决这一矛盾。

Method: 提出OPTD（On-Policy Transition Distillation），从学生自身轨迹采样部分状态，利用冻结的仅问题教师模型识别结果对齐的未来候选并按置信度排序，选择保留教师rollout结果的最长前缀。通过集瓶颈目标提升验证候选至解码器释放阈值，并使用冻结教师KL散度正则化其他位置，全程无需标准答案。

Result: 在四个数学推理和代码生成基准测试中，OPTD持续改善了质量与效率的权衡，在评估的少步基线中实现了最强的大体质量受限AUP。

Conclusion: OPTD通过一致性引导的自适应压缩按策略蒸馏，成功解决了少步扩散语言模型中的状态漂移与过渡压缩限制，在不依赖黄金响应的情况下实现了最优的质量-效率折中。
...
### [9] [Faster but Different: Diagnosing and Controlling Content Drift in Accelerated Multimodal Diffusion Language Models](http://arxiv.org/abs/2607.29079v1)
*Yaoxuan Dou,Yang Shu*

Main category: cs.CL

TL;DR: 本文研究了无需训练的加速技术如何导致扩散多模态大语言模型的生成内容发生漂移，并提出通过缩短KV-cache刷新间隔来控制一致性。


<details>
  <summary>Details</summary>
Motivation: 无需训练的加速方法虽然提高了扩散多模态大语言模型的部署便利性，但会悄无声息地改变生成内容。本文旨在解决这种推理时的一致性漂移问题，以确保模型加速后的可靠性。

Method: 通过对比加速模型（Fast-dLLM等）与未加速模型的输出，利用置信度阈值调整、状态刷新消融和图像替换干预来诊断漂移原因，并提出了一种实现层面的控制策略：缩短KV-cache刷新间隔。

Result: 缩短KV-cache刷新间隔在1.3倍加速下实现了近乎完全的一致性；盲测评估未发现显著的事实错误差异，但自适应或平滑刷新变体的效果均不如固定间隔策略。

Conclusion: 本文的贡献在于提供了一种配对的诊断方法和实现范围内的一致性控制手段，而非对模型准确性或安全性的绝对保证。
...
TL;DR: 本文提出了一种基于轨迹的在线策略蒸馏框架（TOPD），通过教师监督将推理能力高效转移到扩散大语言模型（dLLM）中。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（dLLMs）的推理导向后训练面临挑战：监督微调（SFT）需要密集但通常是离策略的掩码状态，而强化学习（RL）则依赖稀疏奖励或价值建模。

Method: 提出基于轨迹的在线策略蒸馏（TOPD），从目标dLLM采样在线扩散轨迹，从教师模型获取部分去噪状态下的教师词元分布，并使用词元级别的反向KL散度（Reverse-KL）目标更新目标dLLM，从而在保留密集教师监督的同时与模型自身的去噪状态对齐。

Result: 在数学推理基准测试中，TOPD使SDAR-4B-Chat在MATH500上的准确率与RL训练的对应模型TraDo-4B-Instruct相当（静态评估提升5.7，动态评估提升4.5），且仅需1/4的展开轮次，实现了约96倍的模型计算加速。

Conclusion: TOPD高效地将推理能力转移到dLLMs，在避免RL和SFT局限性的同时，显著降低了计算成本并提升了训练效率。
...
TL;DR: 本文提出了无需训练的最佳优先草稿树构建方法DominoTree，通过利用Domino草稿器的路径依赖条件修正，显著提升了LLM推理速度和接受长度。


<details>
  <summary>Details</summary>
Motivation: 现有的最佳优先树方法（如DDTree）采用分解公式，无法表示Domino草稿器基于GRU引入的路径依赖且非分解的条件修正分布。

Method: 提出DominoTree，沿每个根到节点的路径使用Domino的条件和非分解修正进行评分，通过将节点修正限制在候选top-M集合中使其可行，并采用GPU原生CUDA图构建器低成本构建树。

Result: 在Qwen3-4B上实现高达6.6倍于自回归解码的加速，在各温度下均取得最高平均接受长度（最高10.7 tokens/轮）；吞吐量较Domino提升9%-10%（Alpaca上高达22%），并在各温度下优于DDTree和CaDDTree。在Qwen3-8B上保持最高接受长度，T=0时较DDTree提升24%吞吐量。

Conclusion: DominoTree成功利用了路径依赖的非分解草稿器结构，结合高效的GPU原生树构建器，在推测解码中实现了显著的推理加速和更高的接受率。
...
TL;DR: 本文提出CaDDTree方法，通过联合优化树结构和节点预算直接最大化令牌吞吐量，解决了推测解码中缺乏成本感知预算选择原则的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的块扩散草稿树方法（如DDTree）最大化期望接受长度，但该指标随节点预算非递减，总是偏向更大的树而忽略验证成本，缺乏选择预算的原则性依据。

Method: 提出CaDDTree，显式建模草稿和验证延迟以优化令牌吞吐量；将目标分解为每轮的一维预算搜索，并证明在凸验证成本下吞吐量函数是单峰的，从而采用高效的贪婪停止规则动态自适应调整预算，无需离线搜索。

Result: 在Qwen3-4B和Qwen3-8B模型及8个涵盖推理、编程和指令遵循的基准测试上，CaDDTree在几乎所有任务上均达到或超越了具备预言机预算的DDTree的性能。

Conclusion: CaDDTree为推测解码的树构建提供了一种原则性且自适应的成本感知方法，有效提升了推理吞吐量并消除了离线预算调优的需求。
...
TL;DR: 本文提出DDTree方法，通过从块扩散模型的位置分布构建草稿树，在单次前向传递中高效验证多个草稿路径，从而提升推测解码的接受长度和性能。


<details>
  <summary>Details</summary>
Motivation: 尽管DFlash模型能高效生成草稿块，但原版方法每轮仅验证单条轨迹，限制了推测解码的接受长度和整体加速效果。

Method: 提出DDTree方法，利用最佳优先堆算法从块扩散模型的逐位置分布中提取最可能匹配目标模型的延续，构建草稿树，并使用仅祖先注意力掩码在目标模型的单次前向传递中完成高效验证。

Result: DDTree在固定节点预算下验证多条轨迹，克服了单路径验证的局限，在DFlash基础上进一步提升了推测解码性能，跻身该领域领先方法之列。

Conclusion: 通过结合块扩散模型与树状草稿结构，DDTree显著优化了推测解码过程，确立了其在加速自回归语言模型方面的领先地位。
...
### [14] [M$^3$R-Bench: A Unified Benchmark for Evidence-Grounded Multimodal Metaphor Understanding](http://arxiv.org/abs/2608.05817v1)
...
TL;DR: 提出了一个统一且有证据支撑的多模态隐喻理解基准M3R-Bench，并设计了M3R-Reasoner模型，通过课程推理监督与任务感知强化学习对齐隐喻解释，以8B参数规模超越了更大的专有模型。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要通过孤立的子任务评估隐喻理解，缺乏基于证据的解释，难以判断模型是否真正基于视觉和文本线索建立跨域映射；而多模态隐喻同时需要概念理解与跨模态推理。

Method: 构建包含1000个图文实例及人工验证标注的M3R-Bench，提供隐喻出现、目标-源映射、情感及“证据识别—映射建立—情感推理”阶段式解释的联合标注；并提出M3R-Reasoner，结合基于课程的推理监督与任务感知的强化学习，使模型推理与隐喻解释过程对齐。

Result: 评估发现现有模型常忽略视觉证据、依赖表层文本线索并产生不准确的映射；M3R-Reasoner在四个统一任务指标上优于更大规模的专有多模态大模型，视觉证据与情感论证得分分别比GPT-5.5高28.45和30.11分，平均评分得分比Claude-Sonnet-4.6高8.00分。

Conclusion: 多模态隐喻理解中存在跨模态证据—映射不匹配问题，通过显式的阶段式推理监督与强化学习可有效缓解该问题，从而显著提升模型的隐喻理解与情感推理能力。
...
### [15] [OneEmo: A Unified Multimodal Reasoning Model for Emotion Perception, Understanding, and Interaction](http://arxiv.org/abs/2608.06013v1)
*Jiahao Huang,Zheng Lian,Jingyi Zhang,Zhide Chen,Xiaojiang Peng,Shaonan Wang*

Main category: cs.HC

TL;DR: 本文提出了统一的情感通用模型OneEmo，通过构建专用数据集与新型强化学习策略，在情感感知、理解和交互任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM的情感智能研究多集中于特定任务，忽略了任务间的协同作用，且未充分挖掘潜在的推理能力。

Method: 构建了通过人机协同工作流提取显式推理轨迹的EmoWorld-130K数据集进行监督微调，并提出通过统一多任务奖励分配稳定优化的Emo-Chord强化学习策略。

Result: 在大多数基准测试中取得了优于同等规模基线的SOTA性能，且在参数量远小于商业模型的情况下表现出高度竞争力。

Conclusion: 本研究为开发更可靠、更具可解释性的情感计算系统奠定了基础。
...
### [16] [AV-AIVAT: 74x Cheaper Agent Evaluation with Certified Anytime-Valid Stopping in Imperfect-Information Games](http://arxiv.org/abs/2608.06362v1)
*Boning Li,Yu Chen,Longbo Huang*

Main category: cs.GT

TL;DR: 该论文提出AV-AIVAT方法，将AIVAT方差缩减技术与置信序列相结合，在不完美信息博弈的智能体评估中实现了高效、可审计的随时有效早期停止。


<details>
  <summary>Details</summary>
Motivation: 评估智能体强弱需要对弈直到技术胜过运气，成本高昂且所需局数未知。固定预算评估会在结果已定时继续消耗资源或在能区分智能体前过早停止，而基于普通置信区间的朴素停时法则会破坏统计有效性。

Method: 提出Anytime-Valid AIVAT (AV-AIVAT)，将动作知情价值评估工具(AIVAT)与连续监测的置信序列(CSs)结合，其在线价值模型仅从历史对局学习。同时使用渐近CS进行渐近筛选，并利用经验伯恩斯坦CS进行精确有限样本认证。

Result: 在95%标称水平和±1大盲注目标精度下，使用AsympCS时原始结果所需对局数是AIVAT校正结果的74倍；在HUNL的EB-CS运行中显示出1.37倍的中位停止时间比率；此外为Leduc扑克结构性地建立了修正收益界限。

Conclusion: AV-AIVAT将方差缩减转化为高效且可审计的早期停止，同时分离了渐近筛选与精确认证，使评估能在证据充足时即刻停止，并向第三方提供在该停止时间重新核查判决所需的一切信息。
...
### [17] [Resourced Authority A Mechanism-Design Model for Participatory Governance of Deployed AI Agents](http://arxiv.org/abs/2608.06353v1)
*Praphul Chandra,Sujit Gujar,Ganesh Ghalme*

Main category: cs.GT

TL;DR: 本文提出了一个用于部署AI智能体连续参与式治理的形式化机制设计模型，利用计算预算使授权自我执行。


<details>
  <summary>Details</summary>
Motivation: 旨在解决如何有效治理已部署AI智能体的问题，通过确立计算资源作为有效治理杠杆的安全AI范式，确保控制与授权能够自我执行。

Method: 采用序贯博弈机制，利益相关者使用独立于计算资源的治理货币进行贡献；系统通过资金聚合器和双阈值滞后门将净支持转化为二元授权，并在外部认证的安全上限约束下释放计算预算（以硬件签名许可证实现）。

Result: 刻画了该机制可治理的智能体类别，并指出被治理的智能体操纵治理选民群体是当前面临的核心开放问题。

Conclusion: 基于计算预算的机制能有效实现AI治理，但未来需重点解决被治理AI操纵选民群体所带来的若干挑战。
...
### [18] [Fair and Efficient Balanced Allocations for Additive Valuations](http://arxiv.org/abs/2608.06325v1)
*Benjamin Cookson,Nisarg Shah,Paritosh Verma*

Main category: cs.GT

TL;DR: 论文证明了在平衡性约束下，针对任意加性估值必然存在同时满足EF1和分数帕累托最优（fPO）的不可分物品分配方案。


<details>
  <summary>Details</summary>
Motivation: 旨在解决在平衡性约束（任意两个代理的物品数量差不超过一）下，如何为任意加性估值实现公平且高效的不可分物品分配问题，突破以往仅在受限估值条件下证明存在性的局限。

Method: 将KKM引理应用于加权福利对偶框架，提出一种新颖的价格交错引理以克服先前的技术障碍，并将该技术扩展至类别约束（即划分拟阵约束）。

Result: 证明了对于任意加性估值，满足EF1和fPO的平衡分配必然存在；在类别约束下，证明了存在满足EF1的类别敏感松弛与fPO的分配。所有证明均在大语言模型GPT-5.6-Sol协助下完成并由作者验证。

Conclusion: 该研究推广了现有的存在性理论结果，为受限条件下的公平分配问题提供了新的证明技术，并成功将其扩展至更为复杂的类别约束场景。
...
### [19] [MMAligner: Safeguarding Multimodal Large Language Models through Representation Calibration](http://arxiv.org/abs/2608.05909v1)
*Shenyi Zhang,Keyan Guo,Zihao Wang,Xuebin Li,Lingchen Zhao,Hongxin Hu,Chao Shen,Qian Wang*

Main category: cs.CR

TL;DR: 本文提出MMAligner方法，通过将不安全的多模态表示校准到模型已有的拒绝区域内，有效解决了多模态大语言模型的安全差异问题且几乎不损失实用性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在面对语义等价的不安全多模态输入时会绕过文本安全机制产生有害响应，而现有的防御方法存在推理开销大、牺牲实用性或需要大量数据等问题。

Method: 通过几何分析发现安全退化源于多模态输入的表示错位，提出MMAligner方法，利用硬下界确保拒绝、软上界避免过度修改以及良性输入保留目标，将不安全的多模态表示校准至预存的拒绝区域。

Result: 在多个开源MLLM上的实验表明，MMAligner将不安全多模态输入的平均拒绝率提升至99%，实用性退化不到2%，且仅需极少训练数据。

Conclusion: 多模态安全退化源于表示错位而非缺乏安全能力；MMAligner在安全性与实用性权衡上显著优于现有基线。
...
### [20] [PromptShield Home: Ambient Multimodal Prompt Injection Defense for Smart-Home Agents](http://arxiv.org/abs/2608.05495v1)
*He Zhang,Feilong Li,Dingning Long,Yilin Cui,Peijun Zhang,Yuewen Zhang,Qianyao Xu,Xinyi Fu*

Main category: cs.CR

TL;DR: 本文提出PromptShield-Home基准，评估智能家居中MLLM代理区分真实用户指令与环境/外部内容（如电视语音、屏幕文字）的能力，发现传统检测器与MLLM代理在安全性上呈现互补但各自偏激的失败模式。


<details>
  <summary>Details</summary>
Motivation: 智能家居助手越来越多地使用多模态大语言模型（MLLM）直接感知视频和音频，但存在安全风险：代理无法区分真正的用户指令与环境中看似指令的内容（如电视语音、屏幕文字、旁听对话），可能导致误执行。

Method: 构建PromptShield-Home基准，涵盖收件人歧义、屏幕/音频注入、健康监测误触发、混合占用等场景，比较三个抽象层：传统检测器（L0）、单MLLM代理（L1，含视觉、视觉+ASR、视听三种配置）、多代理调解（L2，含投票、角色专家、跨模型仲裁）。

Result: 两类范式失败方向相反：检测器对一切内容都执行，而所有MLLM配置过度拒绝（几乎不完成真实指令且每次都漏检真实跌倒）。两者的正确集合不相交：理想情况下选择正确层的Oracle达到94.1%，而最佳单层仅76.5%。由于标签分布偏向不作为，恒定阻止预测器得分82%，因此分别报告不安全执行率和安全完成率。

Conclusion: 最佳单层无法兼顾安全性与可用性，但不同层的正确决策互补；家庭代理安全应通过学习型路由和传感器融合实现，而非用MLLM替代检测器。Oracle结果仅为上界，尚未实现实际路由系统。
...
### [21] [Tracing the Heart: An Evidence-Linked Pipeline for Heart-Failure Feature Engineering](http://arxiv.org/abs/2608.06366v1)
...
TL;DR: 本文开发了Nimblemind多智能体系统(nMAS)用于自动化的心力衰竭电子健康记录(EHR)特征工程，显著提升了表型分析的预测性能。


<details>
  <summary>Details</summary>
Motivation: EHR特征工程是临床AI研究的主要瓶颈（占用数据科学家39-45%的工作量），在心力衰竭领域尤为突出，现有方法在自动化程度、可维护性和证据可追溯性方面存在局限。

Method: 提出nMAS，一种基于证据链接和评分标准(rubric)的自动化特征工程流水线。在9个EHR源表的500份虚拟患者记录上进行评估，生成并验证结构化及聚合特征，结合受限大语言模型(LLM)进行审计。

Result: nMAS生成了132个结构化特征和70个聚合特征。加入聚合特征后，HFrEF表型分析的AUROC从0.895提升至0.963，HFpEF从0.870提升至0.910；独立LLM评估得分达到最高分的81.5%。

Conclusion: 结果证明对复杂心血管EHR数据进行自动化、可审计的特征工程是可行的，但受限于单中心虚拟数据，未来仍需外部验证。
...
### [22] [TRAJDEBUG: Tracing Error Lifecycle to Identify Critical Failures in Long-Horizon Agent Trajectories](http://arxiv.org/abs/2608.06346v1)
...
TL;DR: 本文提出TrajDebug框架，通过多粒度历史压缩和错误生命周期追踪来解决LLM智能体长轨迹中的关键错误检测与调试问题。


<details>
  <summary>Details</summary>
Motivation: 解决LLM智能体在长轨迹中因证据分散和存在多个局部错误，导致难以定位最终失败的关键错误步骤的问题。

Method: 提出TrajDebug错误生命周期追踪框架，利用多粒度历史压缩和基于证据的错误识别发现长轨迹错误，并通过追踪错误的解决状态和最终影响进行关键归因；此外还构建了TrajErrBench基准。

Result: 实验表明，TrajDebug在多项基准测试中超越了现有基线，取得了最佳整体性能，且其诊断结果能为提升下游智能体成功率提供可操作的反馈。

Conclusion: TrajDebug有效解决了LLM智能体的关键错误检测难题，提升了系统调试效率和成功率，相关代码和数据将开源。
...
### [23] [Beyond Top-K: Replacing Black-Box Retrieval with Interpretable Agentic Operations](http://arxiv.org/abs/2608.06305v1)
*Sagar Tamang,Ayush Vyas,Tabarakul Hazarika*

Main category: cs.AI

TL;DR: 论文提出了READ，一种无嵌入的智能体文档搜索方法，解决了传统分块嵌入RAG在处理长篇财务文档时破坏结构上下文并导致数值单位错误的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的分块嵌入检索方法在处理财务报表等结构化长文档时存在根本缺陷：分块边界常将数字与其单位或表头割裂，导致数量级误差，且表感知分块器仍无法解决大量数字块缺失表头的问题。

Method: 提出READ（Reliable Embedding-free Agentic Document-search），智能体通过模型上下文协议暴露的三种确定性操作（规范化词法搜索、结构导航和有界跨度读取）读取原始文档，将检索轨迹转化为可重放的审计追踪。

Result: 在51个验证问题中，READ准确率为58.8%，显著优于密集检索的15.7%和调优后的35.3%。接口设计（而非迭代）是性能提升的关键；此外，BM25在统计上与READ表现相当，无显著差异。

Conclusion: 在处理结构化财务文档时，基于词法和结构的无嵌入检索显著优于基于嵌入的密集检索，且该方法的性能提升源于检索接口设计而非智能体的迭代能力。
...
TL;DR: 本文系统研究了混合专家扩散语言模型的缩放规律，并据此训练出性能媲美自回归模型的LLaDA MoE v2。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型中的混合专家架构缩放行为尚不明确，本文旨在揭示MoE dLLMs在优化超参数、算力分配和架构方面的缩放规律及其与自回归模型的差异。

Method: 通过系统分析优化超参数、算力分配及架构缩放趋势，发现其最优批量大小增长更快、学习率衰减更迅速且算力分配偏向数据端。基于此规律，从零开始训练了总参数30B、激活参数3B的LLaDA MoE v2模型。

Result: LLaDA MoE v2仅使用Qwen3约65%的预训练tokens，便在知识、推理和编程基准上逼近Qwen3；经有监督微调后，在8项推理和编程基准中有7项优于SDAR Chat，并在多项任务上紧咬Qwen3。

Conclusion: 本研究确立了MoE dLLMs的实用缩放定律与架构设计原则，证明了其在预训练效率和下游任务性能上的巨大潜力。
...
TL;DR: 提出免训练的进化解码框架，通过逐步选择和逐块突变解决扩散大语言模型中的置信度陷阱，提升其数学推理可靠性。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在数学推理中存在局部token置信度与全局推理正确性错位的“扩散置信度陷阱”，导致采样敏感或采样一致的推理失败。

Method: 提出名为“进化解码”的测试时扩展框架，将扩散解码视为进化过程，结合逐步选择保留有用数值符号信号并抑制重复模式，以及逐块突变引入结构化替代方案以逃离错误高置信度区域。

Result: 在多个基准测试中，进化解码方法使 LLaDA 2.0 超越了传统的基于置信度的解码方法，显著提高了数学推理的可靠性。

Conclusion: 进化解码有效解决了扩散大语言模型在数学推理中的置信度对齐问题，为提升其推理可靠性提供了一种有效的免训练测试时方法。
...
TL;DR: FlowBlock是一个无需训练的并行解码框架，通过门控波前解码和异构波前打包机制，利用自纠正dLLM的token编辑能力实现块间并行解码，显著提升吞吐量并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 块级扩散大语言模型（dLLMs）在块间解码时严格串行，成为性能瓶颈；已有基于后训练的方法加速有限且损害精度。本文旨在无需训练地解锁块间并行性，同时保持KV缓存复用和模型精度。

Method: 提出FlowBlock框架，包含两个核心机制：（1）门控波前解码——通过就绪门控将块纳入有界波前，利用token到token编辑协同细化活跃块，并在窗口化块因果掩码下按序提交块以保持冻结前缀KV缓存的精确复用；（2）异构波前打包——为每个请求分配独立波前，同时将异步窗口打包为密集、形状稳定的批量前向计算。

Result: 在多个基准上，相比LLaDA-2.1和LLaDA-2.0两个串行块级dLLM，TPS分别提升最高2.95倍和4.01倍，延迟分别降低最高53.6%和77.1%，平均精度提升1.3分；相比基于训练的块间并行基线D2F，精度更高且批量服务吞吐量最高提升16倍。

Conclusion: FlowBlock证明利用自纠正dLLM的T2T编辑能力，可将块最终性从硬依赖转化为调度资源，实现无需训练的高效块间并行解码，在吞吐量、延迟和精度三方面均优于现有方法。
...
TL;DR: 本文提出了LaCache，一个无需训练的加速框架，通过无损缓存和混合精度量化消除扩散式大语言模型中的算子级冗余，实现显著推理加速。


<details>
  <summary>Details</summary>
Motivation: 当前的扩散式大语言模型在去噪步骤中会重新计算整个序列，忽略了块内前缀和被掩码后缀保持不变的特性，导致严重的算子级冗余和计算资源浪费。

Method: 提出LaCache框架，采用无损状态记忆化（LSM）缓存嵌入输出、预注意力状态和在线softmax统计等中间结果以跳过不变token的冗余计算；同时结合针对FFN层的分组FP8量化策略以缓解内存带宽瓶颈。

Result: 实验表明，LaCache单独使用可实现约1.3倍的端到端加速；与现有加速方法结合时，在保持任务准确率相当的同时，端到端加速比最高可达40.2倍。

Conclusion: LaCache能够有效且无损地消除扩散式大语言模型的算子级冗余计算，与其他加速方法结合具有极高的加速潜力，为大模型推理优化提供了新方向。
...
TL;DR: TAPS提出目标感知前缀选择方法，通过将扩散模型的边缘概率转化为路径条件接受估计，在固定验证预算下选择紧凑前缀闭合子树，显著提升推测解码效率。


<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的并行草稿生成虽降低了草稿延迟，但验证成为新瓶颈：验证单序列限制接受长度，而验证大草稿树导致目标模型延迟过高。现有方法按边缘概率排序节点，忽略了验证的前缀条件性，可能验证被拒绝前缀的不可达后代，增加延迟但接受收益有限。

Method: TAPS将扩散模型的边缘概率转化为路径条件接受估计，然后在固定验证预算下选择紧凑的前缀闭合子树，优化接受-成本权衡而非简单扩展草稿树。

Result: 在多种数据集和模型族上，TAPS相比自回归解码实现高达7.9倍无损端到端加速，分别优于最先进的DFlash和DDTree方法1.36倍和1.74倍。

Conclusion: TAPS通过目标感知的前缀选择策略有效解决了扩散模型推测解码中的验证瓶颈问题，在保证无损性的前提下显著提升解码速度，为推测解码优化提供了新方向。
...
### [29] [PaDoc: Layout-Grounded Parallel Decoding for Document Parsing](http://arxiv.org/abs/2608.06146v1)
*Hao Yu,Jiabo Zhan,Kang Liu,Linnan Zhao,Dongxu Yue,Rui Chen,Jinglin Wang,Chong Sun,Chen Li,Jing Lyu,Chun Yuan*

Main category: cs.AI

TL;DR: 本文提出基于布局分支结构的端到端文档解析器PaDoc，通过共享页面表示并行解码区域内容，在保留全页上下文的同时实现了高精度与高吞吐。


<details>
  <summary>Details</summary>
Motivation: 端到端文档解析器将页面布局和内容序列化为单一自回归序列导致解码路径过长，而裁剪式的两阶段解析器虽支持并行但存在视觉预填充重复和上下文碎片化问题，本文旨在保留全页上下文的同时消除区域解码依赖。

Method: 提出PaDoc，将预测布局视为共享页面表示上的分支结构，推导出前缀条件因子分解以实现布局和内容分支并发推进；在单一MLLM中利用打包变长祖先注意力维持可见性，并结合掩码并行解码与vLLM的共享前缀缓存重用进行加速。

Result: 在OmniDocBench Full上达到布局F1为91.1、总分94.24（端到端最优），并取得最佳文本编辑率(0.038)和公式CDM(95.59)；在单张A800 GPU上相比同骨干基线，有效页面吞吐量提升67.4-118%，P95延迟降低39.2-54.9%。

Conclusion: PaDoc成功结合了全页上下文感知与并行解码的优势，在端到端文档解析的准确性和推理效率上均取得显著突破。
...
### [30] [Unified Agent: Managing Interactions across Devices](http://arxiv.org/abs/2608.05729v1)
*Xinshuang Liu,Runfa Blark Li,Shaoxiu Wei,Xin Lin,Truong Nguyen*

Main category: cs.AI

TL;DR: 本文提出了统一智能体，通过维持一种跨设备、跨时间的紧凑且随时可用的状态设计，有效解决了多设备和多时间点下的智能体交互问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体系统在跨设备和跨时间运行时，由于观测数据散落且缺乏有效的状态管理，无法满足用户请求。单一智能体缺乏跨时间状态管理，而多智能体系统无法维持紧凑的携带状态，因此需要一种能有效组织交互证据、既定事实和常驻请求的状态设计。

Method: 提出了“统一智能体”，一种有状态智能体，能够跨设备和时间点携带交互证据，并结合当前观测决定行动。同时构建了一个跨设备和跨时间的用户-智能体交互基准测试来比较状态设计。

Result: 在默认设置下，该智能体显著优于四种已发表设计的适配版本。在改变多模态大语言模型（MLLM）系列、能力和推理努力程度的情况下，其性能优势依然保持稳定。

Conclusion: 优秀的紧凑状态设计对于跨设备、跨时间智能体至关重要，且该优势在不同MLLM设置下具有稳健性。相关代码和数据已开源。
...
### [31] [M$^3$Prune: Hierarchical Collaborative Pruning for Efficient Multi-Modal Multi-Agent Retrieval-Augmented Generation](http://arxiv.org/abs/2608.05967v1)
*Taolin Zhang,Weizi shao,Zijie Zhou,Chen Chen,Daiyang Yu,Tingyuan Hu,Chengyu Wang,Xiaofeng He*

Main category: cs.MM

TL;DR: 本文提出了一种名为M3Prune的层级通信图剪枝框架，通过消除多模态多智能体RAG系统中的冗余通信边，在保持任务性能的同时显著提高令牌效率。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体mRAG系统虽然性能强大，但存在大量的令牌开销和计算成本，对大规模部署构成了挑战。

Method: M3Prune首先在文本和视觉模态内进行图稀疏化以识别任务关键的通信链接；然后构建模态间通信图并利用模态对齐分数稀疏化跨模态连接；最后渐进式剪枝冗余边以获得高效的层级拓扑。

Result: 在通用和特定领域mRAG基准上的广泛实验表明，M3Prune在显著提高令牌效率的同时，持续优于单智能体和强大的多智能体mRAG系统。

Conclusion: 剪枝多模态多智能体系统中的冗余通信边可以有效平衡任务性能和令牌效率，有助于推动其大规模部署。
...
### [32] [CalibForge: Adversarial Solver Calibration for Scaling Learnable Terminal Tasks](http://arxiv.org/abs/2608.06352v1)
*Fanzhe Meng,Guoxin Chen,Jiale Zhao,Shuang Sun,Zhiyu Lin,Wayne Xin Zhao,Ruihua Song,Ji-Rong Wen,Kai Jia*

Main category: cs.LG

TL;DR: 本文提出了CalibForge系统，通过对抗性求解器校准来自主合成难度适中的终端任务，以提升终端智能体的训练效果。


<details>
  <summary>Details</summary>
Motivation: 训练终端智能体需要可执行且具有适当挑战性的任务，但现有的可执行验证只能证明任务可解，无法揭示任务相对于特定求解器的行为表现和学习难度。

Method: 提出CalibForge系统，利用已验证的求解器行为通过对抗性求解器校准来修改候选任务。包含多重求解器校准（针对异构求解器池中的分歧）和对比求解器校准（针对强通过/弱失败关系），两者均在已证明可解的基础上实现相对于求解器的可学习区域。

Result: 构建了5,431个校准后的终端任务。实验表明这两种策略比仅创作验证或单一求解器反馈提供更有效的监督。基于这些任务训练的模型在Terminal-Bench 2.0上达到32.58%和47.57%，在Terminal-Bench 2.0、SWE-bench Pro和Doc2Repo上分别比基础模型最高提升了24.71、27.68和30.04个百分点。

Conclusion: 相对于求解器的可学习性是构建有效且可迁移的智能体训练数据的实用目标。
...
TL;DR: 本文提出ERILS方法，通过整合外部更强策略生成的rollouts并分别处理不同来源的奖励，改进扩散大语言模型的强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型(dLLMs)的强化学习方法依赖目标模型自身生成的on-policy rollouts，当成功rollouts稀缺时，训练获得正面奖励有限，进展受阻。

Method: 提出ERILS（External Rollout Integration with Length Control and Source-Specific Processing），整合外部更强策略生成的高奖励rollouts，同时控制外部rollout长度，并对on-policy和外部rollouts的奖励分别处理，以解决长度差异和奖励联合处理的不稳定性问题。

Result: 在Sudoku、Countdown和MATH500三个任务的零样本评估中，ERILS均提升了多样本性能，其中Sudoku提升最大，best-of-4完成准确率达98.4%，而最强基线仅40.3%；在不同生成长度下保持约90%的确定性单次完成准确率。组件分析表明长度控制的外部rollouts优于无控制rollouts，且来源特定的奖励处理避免了联合处理导致的训练崩溃。

Conclusion: rollout构建和奖励处理是将外部rollouts整合到dLLM强化学习中的重要设计维度，ERILS验证了这些设计选择的有效性。
...
### [34] [A Continuous-Time Reinforcement Learning Framework for Fine-Tuning Discrete Diffusion Models](http://arxiv.org/abs/2607.14522v1)
*Zikun Zhang,Jiayuan Sheng,David D. Yao,Wenpin Tang*

Main category: cs.LG

TL;DR: 本文提出了一种连续时间强化学习框架，用于微调离散扩散模型（尤其是掩码扩散语言模型），支持沿去噪轨迹引入中间奖励进行策略优化。


<details>
  <summary>Details</summary>
Motivation: 旨在解决连续时间、离散状态空间下的强化学习问题，并克服现有基于GRPO方法仅依赖终端奖励且需要可微奖励信号的局限性，实现对离散扩散模型的奖励驱动优化。

Method: 基于随机控制将状态动态建模为受控连续时间马尔可夫链（CTMC），推导出连续时间的PPO和GRPO变体；将框架应用于离散扩散模型微调，允许在去噪过程中引入中间奖励；针对掩码扩散模型提供了具有解析概率比的策略参数化；并提出轨迹子采样技术以降低掩码扩散大语言模型中计算轨迹似然的成本。

Result: 在低维熵正则化优化问题及掩码扩散大语言模型的数学推理和编程任务后训练中均验证了方法的有效性，且轨迹子采样技术成功降低了计算成本。

Conclusion: 该框架为离散扩散模型提供了统一的连续时间强化学习视角，突破了以往限制，在推理和编程任务中展现出潜力，为掩码扩散模型中的探索与策略优化提供了新思路。
