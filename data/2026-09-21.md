...
### [1] [MintAct: A Unified Visual Agent for Digital Environments](http://arxiv.org/abs/2609.22083v1)
...
TL;DR: 本文提出了MintAct，一系列统一UI定位、跨域多步导航及视觉工具使用的视觉-语言模型，在多个基准测试中实现了同等规模下的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 旨在解决跨移动、桌面和网络平台统一UI交互能力的挑战，并试图在单一模型中达到与各领域专用模型相当的性能。

Method: 开发了可扩展的环境和强化学习（RL）基础设施，托管数百个跨域并发实例以支持数据收集和在线RL；同时引入异步RL训练框架来精确控制跨域训练分布，在存在噪声和离策略漂移时保持稳定性。

Result: 实验表明，MintAct模型在所有能力上均达到了各领域专用模型的水平，并在同等模型规模下于广泛的基准测试中取得了SOTA表现（如OSWorld-Verified上达48.9分）。

Conclusion: 通过精细的环境、数据和训练方案设计，MintAct成功证明了可以构建一个跨域统一的视觉-语言模型且不损失专用模型性能。
...
TL;DR: 本文提出了LLaDA-UI，一个16.7B参数的基于MoE的块级扩散视觉-语言GUI代理，验证了扩散语言模型在保持并行解码优势的同时能够成为高效的多模态GUI代理。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)因其块并行和任意顺序生成能力而具备高解码效率，在延迟敏感应用中具有吸引力。GUI代理需要实时感知屏幕状态并输出结构化的空间接地动作，是测试这一范式的理想场景。然而，dLLMs能否扩展为具备多模态能力的GUI代理同时保持并行解码优势仍是一个悬而未决的问题。

Method: LLaDA-UI采用两阶段训练管道：第一阶段进行通用多模态预训练，将原生分辨率视觉编码器与LLaDA2.0-mini-base扩散语言骨干对齐；第二阶段进行GUI代理监督微调，使用多样化的移动、桌面、网页和grounding数据。整体架构采用基于MoE的块级扩散视觉-语言模型，参数规模为16.7B。

Result: 在广泛采用的grounding基准测试和跨多个平台的导航基准测试中，LLaDA-UI显著优于Qwen2.5-VL-7B，并在六个报告的GUI基准测试中的四个上超越了Qwen3-VL-8B。

Conclusion: 实验结果确立了块级扩散作为多模态GUI代理的实用生成范式，证明了dLLMs可以有效扩展为具备多模态能力的GUI代理，同时保持其并行解码优势。
...
### [3] [DRT: Dense Reasoning Trace for Efficient and Grounded Multimodal Reasoning](http://arxiv.org/abs/2609.21675v1)
*Wan Xu,Yuanfan Guo,Kevin Han,LaLa Chen,Wangmeng Zuo*

Main category: cs.CV

TL;DR: 提出密集推理轨迹(DRT)范式，通过紧凑的结构化轨迹替代冗长的自然语言思维链，显著提升多模态大语言模型的推理效率与视觉定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型思维链范式受限于自然语言表达空间，导致语言冗余、信息稀释以及视觉定位能力较弱。

Method: 提出密集推理轨迹(DRT)范式，将推理表示为包含符号连接符的紧凑结构化轨迹，分离视觉观察与逻辑推演；通过密集轨迹初始化内化推理模式，并采用轨迹基础强化学习框架结合结构化奖励来优化模型生成。

Result: 在挑战性推理基准上，DRT相比Qwen3-VL基线准确率提升1.3个点，并实现了5.5倍的token效率提升。

Conclusion: 复杂的多模态推理无需冗长的自然语言轨迹，结构化轨迹为下一代多模态大语言模型开辟了更高效的路径。
...
### [4] [AgentVidBench: A Multi-Hop Video Question Answering Benchmark for Evaluating MLLM Agents](http://arxiv.org/abs/2609.21386v1)
*Seoyeon An,Hyeonseo Jang,Minsu Kim,Chanho Lee,Younghan Park,Kangwook Lee*

Main category: cs.CV

TL;DR: 本文提出了AgentVidBench，一个用于评估多模态大模型（MLLM）智能体空间、时间和因果推理能力的多跳视频问答基准。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准多局限于单步推理的场景级查询或全局摘要，缺乏评估现实世界中复杂多跳多模态推理等智能体能力的严格基准。

Method: 提出AgentVidBench基准，除标准问答对外，还提供逐步解决方案轨迹以支持轨迹评估；此外，提出了一种简单有效的智能体策略作为基线。

Result: 对12个开源和闭源MLLM的实验表明，在AgentVidBench上单轮性能有限，而将模型集成至最先进的智能体工作流可提升准确率和轨迹评分。

Conclusion: AgentVidBench可作为未来智能体视频理解研究的综合测试平台。
...
### [5] [AgriScope: Pixel-Grounded Multimodal Understanding for Agricultural Images](http://arxiv.org/abs/2609.20325v1)
*Abderrahmene Boudiaf,Mohamad Alanssari,Irfan Hussain,Sajid Javed*

Main category: cs.CV

TL;DR: 本文提出了一个用于农业图像理解的像素级定位多模态框架AgriScope，并构建了大规模指令微调数据集AgriGround。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型仅支持文本输出，缺乏像素级视觉定位能力，难以应对复杂真实农业环境下的细粒度识别任务。

Method: 提出统一框架AgriScope，结合生物语义编码、密集空间表征和像素解码，支持图像级、区域级和像素级理解；同时构建了AgriGround数据集，通过多阶段自动标注流水线生成密集定位监督。

Result: 在多个农业视觉-语言任务上的实验证明了AgriScope在像素级定位多模态理解方面的有效性，并建立了强基准。

Conclusion: 该研究为农业视觉-语言学习和视觉定位提供了坚实基础与基准，数据集和代码已开源。
...
### [6] [QCPruner: Query-Conditioned Population Coverage for Visual Token Pruning](http://arxiv.org/abs/2609.19990v1)
*Shengli He,Yongchao Liang,Roumeng He,Junjie Zeng,Jiyuan He,Can Wu,Li Zheng*

Main category: cs.CV

TL;DR: 本文提出了一种名为QCPruner的无训练视觉token剪枝方法，通过双边查询条件效用加权优化覆盖率，在多个多模态大语言模型上实现了最佳的剪枝性能保持。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中视觉token负载过高导致计算成本增加，现有剪枝方法在固定预算下未能有效利用共享的查询效用来平衡保留查询相关证据和避免冗余。

Method: 提出QCPruner，利用关键词匹配的查询锚点融合两种跨模态线索形成效用，并将其应用于视觉目标和候选代表中，基于视觉亲和力的覆盖率构建非负设施定位目标，该目标是单调且子模的，具有标准的(1-1/e)贪婪保证且无需模型训练或参数更新。

Result: 在LLaVA-1.5、LLaVA-NeXT、LLaVA-Video和Qwen2.5-VL上，QCPruner在所有报告的token预算下均实现了最高的平均相对性能。例如，在LLaVA-1.5-7B上保留32/576个token时保留了96.1%的性能（最强基线为93.9%），在Qwen2.5-VL-7B上保留256/1296个token时保留了96.7%的性能（最强基线为92.5%）。

Conclusion: QCPruner通过双边查询条件效用加权有效解决了视觉token剪枝中的冗余与相关性平衡问题，在大幅降低token数量的同时维持了模型的高性能，且无需额外训练，为MLLMs的高效推理提供了有效方案。
...
### [7] [Benchmarking MLLMs via Cognitive Expected Scene Graph for Safety-Critical Visual Negation Understanding](http://arxiv.org/abs/2609.19767v1)
*Zhiyun Jiang,Hanyong Wang,Binbin Liang,Yu Xie,Menglong Yang,Wei Li*

Main category: cs.CV

TL;DR: 本文提出安全认知下的场景否定理解任务（SNUS），构建了负向描述数据集并提出CESG评估指标，以解决现有多模态大语言模型在负向语义上的肯定偏差问题。


<details>
  <summary>Details</summary>
Motivation: 真正的机器智能需要超越被动像素匹配，掌握通过视觉否定理解对缺失信息进行自上而下的功能推理。然而，无约束的视觉否定范式过于开放，且普遍存在的肯定偏差导致现有MLLMs和评估指标在负向语义下失效。

Method: 将否定推理的边界锚定在特定认知目标内，聚焦安全性这一关键认知维度，定义SNUS任务；构建高保真负向描述数据集以映射局部危险密集断言；提出结构化、极性感知的CESG评估指标。

Result: 实验表明当前模型在该任务上表现挣扎，传统指标在语义反转下完全崩溃；相反，本文框架为SNUS提供了坚实的基准。

Conclusion: 该框架为推进风险感知情境理解和反事实认知提供了严谨的基础。
...
### [8] [Region-Level Policy Optimization for Fine-grained MLLM Perception](http://arxiv.org/abs/2609.19745v1)
*Yuheng Shi,Xiaohuan Pei,Minjing Dong,Chang Xu*

Main category: cs.CV

TL;DR: 本文提出Vision-RL2，通过区域级强化学习优化轻量级提议网络，在集中分辨率于关键区域的同时大幅减少视觉token，从而提升多模态大语言模型的细粒度感知能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLM）通常通过提高图像分辨率来改善细粒度视觉感知，但这会导致视觉编码和语言模型预填充的计算成本显著增加。由于定位感兴趣区域（RoI）和识别其内容对分辨率的要求不同，本文旨在解决如何在降低token消耗和计算成本的同时提升细粒度感知能力的问题。

Method: 提出了一种名为Vision-RL2的区域级强化学习方法。该方法使用一个轻量级提议网络，并将其优化视为强化学习问题：将连贯区域作为动作，冻结的MLLM读取器通过移除区域后答案似然的变化来评估奖励。通过互补的减法（抑制干扰）和加法（恢复缺失证据）目标，无需区域标注、响应采样或推理轨迹即可仅更新预测器。最终利用优化后的提议网络进行稀疏编码，放大证据并排除背景token。

Result: 在六个细粒度基准测试和四个MLLM骨干网络上，Vision-RL2在所有token预算下均提高了基础模型的准确率，并且仅使用约四分之一的视觉token就超越了基础模型在最大预算下的准确率。

Conclusion: Vision-RL2通过区分定位与识别的分辨率需求，利用区域级强化学习优化区域提议网络，实现了更高效、更准确的细粒度视觉感知，在显著降低计算开销的同时超越了基线模型的性能上限。
...
### [9] [An Interpretable Memory Decision Controller for LLM Agents Based on Three-Signal Complementarity: Decoupling Confidence and Consistency](http://arxiv.org/abs/2609.22043v1)
*Yiming Zhang,Jinghong Zhang,Haoran Zhao,Yiren Ma,Chunlei Zhao*

Main category: cs.CL

TL;DR: 本文提出了一种零参数的记忆决策层（MDL），通过评估检索记忆的可信度，显著降低了冲突记忆下检索增强生成（RAG）的幻觉率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型记忆系统缺乏对检索记忆可信度的决策机制，当记忆库存在冲突观点时，标准RAG盲目注入记忆会显著加剧幻觉现象。

Method: 提出位于检索与生成阶段之间的零参数记忆决策层（MDL）。其核心是三信号互补编码器，利用基于QR的正交子空间投影与元工作记忆信号融合相关性、可靠性与任务风险，显式解耦信心与一致性，并引入风险倒置与显式弃权机制。

Result: 在主流模型与开源数据集上，MDL将冲突记忆下的幻觉率降低约56.04%，高风险场景下接近零幻觉。该白盒方法无需训练参数，每次决策仅增加约0.14毫秒，比前置的嵌入检索步骤快约50倍。

Conclusion: MDL为检索增强生成提供了一种高效、可解释且零参数的白盒记忆信任决策方案，有效解决了冲突记忆引发的幻觉问题。
...
### [10] [RecreationWorld: Scalable and Verifiable Environments for Hybrid Computer-Use Agents](http://arxiv.org/abs/2609.22000v1)
...
TL;DR: 本文提出了RecreationWorld和RecreationBench框架，用于训练和评估能够交替进行图形交互与软件开发的混合型计算机使用代理。


<details>
  <summary>Details</summary>
Motivation: 现有计算机使用代理（CUA）在图形交互和软件开发方面发展割裂，而真实的数字工作需要两者交替进行。本文旨在解决混合型CUA无法自主决定何时探索界面、实现软件并视觉验证产物的问题。

Method: 提出基于“复现”的五平台框架RecreationWorld，要求代理发现运行参考程序的行为并构建忠实实现。该框架提供统一工具和以参考程序为预言机的执行奖励，并利用生成的轨迹训练模型。

Result: 训练后的模型在五个分布外编码和混合计算机使用基准上均获提升，且更频繁地验证渲染输出。在RecreationBench评估中，GPT-6 Astra总体领先（58.1%），但仅2.8%任务通过所有程序化测试；代理重现静态界面结构比交互和计算输出更可靠，生成应用也更小、更单一。

Conclusion: 尽管模型在混合任务上展现出一定的技能迁移能力，但在完全通过程序化测试方面仍面临巨大挑战。本文发布了相关基准、环境和测试套件以促进未来研究。
...
TL;DR: 本文提出了一种名为EB-Decode的“早鸟”解码框架，通过自适应分块和位置感知采样提前联合解码低熵token，显著加速了扩散大型语言模型（dLLM）的推理过程。


<details>
  <summary>Details</summary>
Motivation: 扩散大型语言模型（dLLM）在推理时通常需要许多步骤才能使token置信度达到解码阈值，导致即使使用块状KV缓存，推理效率依然低下。本文旨在解决dLLM推理过程中的效率低下问题。

Method: 提出EB-Decode框架，包含两个核心组件：1) 一个可学习网络，自适应地将具有相似不确定性的token分组为可变长度块，而非依赖固定块大小；2) 一个位置感知采样器，在预测的可变长度块内使用更少步骤并行解除token掩码。该框架无需修改预训练dLLM权重，可作为插件直接部署。

Result: 在三个模型和四个基准上的广泛实验表明，EB-Decode在保持相当准确率的情况下，吞吐量比原始解码方法高3.53-18.76倍，比最强基线Fast-dLLM高1.58倍。

Conclusion: EB-Decode利用低熵token聚类特性实现早期解码，作为一种即插即用模块且开销可忽略，有效提升了dLLM的推理吞吐量，为高效dLLM部署提供了新方案。
...
TL;DR: 本文提出在掩码扩散语言模型中使用寄存器标记跨生成块携带推理状态，以替代保留先前生成的文本。


<details>
  <summary>Details</summary>
Motivation: 解决掩码扩散语言模型在跨生成块扩展推理时需要将先前生成的文本保留在上下文中导致的问题，探索仅使用固定大小状态来继续推理的可行性。

Method: 将固定大小的携带状态实现为寄存器标记（专用固定位置标记），通过训练使其连续隐藏状态跨块携带推理进度。模型被后训练以解码文本块、清除文本但保留寄存器值并继续解码，且可通过强化学习进一步优化。

Result: 在LLaDA和Dream模型上的主要比较表明，寄存器方法在所有基准测试中均优于离散文本携带方法，数学和代码任务分别提升高达8.5分和19.5分，在有限代码生成任务中表现尤为出色。

Conclusion: 利用寄存器标记携带状态可有效提升dLLM的跨块推理能力，并且这些寄存器可以通过强化学习在长视野推理任务中得到进一步优化。
...
TL;DR: 本文提出了In-place Instruction Following (IIF)任务及评估基准IIF-Bench，并开发了针对扩散大语言模型的后训练框架GRAFT以提升其遵循位置约束的能力。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽支持在任意位置锚定用户约束，但现有模型在去噪过程中对这些约束段关注度不足，需评估并提升其在此类任务上的表现。

Method: 构建了分层评估基准IIF-Bench及配套评估协议，并提出GRAFT后训练框架，结合约束感知的监督微调与偏好优化来改进模型。

Result: 在四个代表性扩散大语言模型上，GRAFT将平均IIF得分从57.75提升至73.10（+15.35），在字面和语篇功能约束上分别提升15.91和15.57分，且保持了通用生成能力。

Conclusion: 利用约束感知的后训练方法（如GRAFT）能有效增强扩散语言模型在任意位置遵循指令约束的能力，同时不损害其整体生成性能。
...
TL;DR: 本文提出CONDOR模型，利用高斯噪声扰动和自蒸馏机制，使离散掩码语言模型能在单次前向传播中生成连贯且受噪声控制的文本块。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型并行预测Token时，因各位置从边缘分布独立采样，单次生成的文本块往往缺乏连贯性。本文旨在解决如何让离散掩码模型在单次前向传递中生成完整且连贯的文本块。

Method: 提出CONDOR（单步读取的耦合噪声蒸馏）方法，无需自回归教师模型或目标侧编码器，从零开始训练。训练结合两种信号：一是对多个噪声样本预测掩码Token，仅利用最匹配真实文本的样本进行监督；二是在固定噪声下对单步预测进行多步解码细化，并将其蒸馏回单步操作。

Result: 在TinyStories数据集的控制实验中，该方法在单块及连续块生成任务上，均能生成随噪声变化且高度连贯的单次续写文本。

Conclusion: 研究表明，通过引入高斯噪声场和耦合噪声蒸馏，离散掩码语言模型能够在无自回归教师的情况下，于单次读取中生成连贯的文本续写。
...
TL;DR: 本文提出了RAEC，一种无需训练的解码方法，通过在扩散大语言模型早期去噪阶段提交持续的拒绝信号来提升安全性。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型通过迭代去噪生成文本，其去噪步骤和token位置会影响到安全对齐。本文旨在理解并解决dLLM在有害提示下的安全行为问题。

Method: 提出Refusal-Aware Early Commitment (RAEC)，一种无需训练的解码方法，通过追踪中间token分布并在早期去噪步骤提交持续的拒绝信号来增强安全性。

Result: 在LLaDA和Dream模型上的实验表明，RAEC降低了攻击成功率，同时基本保持了模型的实用性。

Conclusion: 去噪步骤和拒绝token提交的持续性对理解dLLM安全性至关重要；RAEC作为一种简单有效的训练无关解码方法，成功提升了dLLM的安全对齐。
...
TL;DR: 本文提出了一种面向扩散语言模型的轨迹级推测解码框架，通过置信度分层树探索和分块并行验证显著提升了生成吞吐量。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型虽然支持并行token生成，但在低置信度下现有解码策略退化为单token生成，严重限制了吞吐量；与自回归模型不同，dLLM需要在去噪轨迹上进行推测，这带来了独特挑战。

Method: 提出轨迹级推测框架，通过置信度分层树探索构建草稿去噪轨迹，利用双向注意力掩码进行分块并行验证，并引入跨块推测利用扩散模型的双向结构进行前瞻；该框架基于Fast-dLLM的双缓存基础设施构建。

Result: 去噪迭代次数减少30-40%，每步token数从2.6提升至4.3，相比原始dLLM实现7-14倍加速，相比Fast-dLLM实现1.3倍加速，在推理和代码基准测试中准确率变化低于1%。

Conclusion: 该框架在保持精度的同时显著加速了扩散语言模型的生成过程，形式化刻画了方法的精确性条件，并指出轨迹漂移是增加并行度的根本代价，为dLLM的高效解码提供了新方向。
...
TL;DR: 提出了DominoTree，一种无需训练的best-first草稿树方法，通过利用Domino路径相关的非分解因果修正来加速LLM推理。


<details>
  <summary>Details</summary>
Motivation: 现有的基于分解公式的树方法（如DDTree）无法表示Domino drafter中基于GRU的路径相关因果修正结构，限制了推测解码的性能。

Method: 引入DominoTree，利用Domino的条件（非分解）修正沿着每条根到节点的路径对草稿树进行评分，并通过将每个节点的修正限制在top-M候选集来保证实用性。

Result: 在多种模型大小、负载和上下文长度设置下，DominoTree均取得了最高的平均接受长度和最高的吞吐量（如相比AR最高达7.3x，在HELMET上接受长度比Domino高+29-36%）。

Conclusion: DominoTree通过有效利用非分解的路径相关修正，在单流和SGLang服务框架中均实现了最先进的LLM推理加速，特别是在长上下文场景中表现优异。
...
TL;DR: 本文提出了GRAFT框架，通过改进边缘评分和动态预算分配，解决了扩散语言模型在树状推测解码中的树构建问题。


<details>
  <summary>Details</summary>
Motivation: 现有树构建器与扩散语言模型（DLM）草稿器不兼容，且已有的DDTree方法仅依赖概率进行边缘选择导致父子兼容性差，同时固定预算无法适应解码状态的动态变化。

Method: GRAFT提出两项核心技术：一是目标蒸馏边缘评分（TDES），从目标模型轨迹中提取父子偏好以选择目标兼容的边缘；二是状态感知预算分配（SABA），通过平衡预期草稿增益与验证成本来动态设定每轮的树预算。

Result: 在多个模型和任务上，GRAFT相比自回归解码实现了2.13倍至6.36倍的端到端加速，且每轮增加的额外开销不到0.5毫秒，仅占目标模型验证延迟的约1.4%。

Conclusion: GRAFT以极低的开销显著提升了DLM推测解码的效率，验证了动态预算和目标兼容边缘选择在树构建中的有效性。
...
TL;DR: 本文提出了CaDDTree，一种通过联合选择树结构和节点预算来直接优化令牌吞吐量的成本感知扩散草稿树方法。


<details>
  <summary>Details</summary>
Motivation: 现有的DDTree方法在固定节点预算下最大化预期接受长度，但该指标随预算单调递增，忽略了验证成本，导致缺乏选择预算的原则性基础。

Method: 提出CaDDTree，显式建模草稿和验证延迟，将吞吐量目标分解为每轮对预算的一维搜索，并在凸验证成本下证明吞吐量函数是单峰的，从而实现高效的贪婪停止规则，无需离线预算搜索即可逐轮自适应调整预算。

Result: 在Qwen3-4B和Qwen3-8B模型上跨8个基准测试（涵盖推理、编码和指令遵循任务）的实验表明，CaDDTree在几乎所有任务上均匹配或超越了具有预言机预算选择的DDTree。

Conclusion: CaDDTree为块扩散草稿提供了一种原则性的成本感知自适应预算选择方法，有效优化了推理吞吐量，消除了离线调优的需要。
...
TL;DR: 本文提出DDTree方法，通过从块扩散起草器的位置分布构建草稿树，克服了DFlash单轨迹验证的局限，实现了领先的推测解码性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DFlash推测解码方法每轮仅验证单个草拟轨迹，可能限制其接受长度。

Method: DDTree直接从块扩散起草器的逐位置分布构建草稿树，在固定节点预算下使用最佳优先堆算法选择最可能匹配目标模型的延续，并在单次目标模型前向传播中利用仅祖先注意力掩码进行高效验证。

Result: DDTree在DFlash基础上取得了显著增益，跻身推测解码领域的领先方法之列。

Conclusion: 通过构建并高效验证草稿树，DDTree有效提升了推测解码的接受长度和整体性能。
...
### [21] [Omni Demand Understanding: A Benchmark for Contextual User-Intent Inference in Multimodal Interaction](http://arxiv.org/abs/2609.21392v1)
...
TL;DR: 本文提出了全需求理解（ODU）基准，评估多模态大语言模型从复杂音视频交互中推断用户潜在需求的能力，发现当前最强模型仅能恢复44.7%的关键上下文信息。


<details>
  <summary>Details</summary>
Motivation: 现有交互能力基准主要关注响应质量，而忽视了更根本的问题：模型能否从复杂的多模态交互中正确推断用户的潜在需求。现实中的用户需求往往在语音中未明确指定，需要从多模态线索和对话历史中推断，同时受到模糊表达、不流畅表达和嘈杂声学环境的复杂化影响，此外类似请求的语音可能并不构成对助手的需求而导致误触发。

Method: 提出Omni Demand Understanding（ODU）作为独立的多模态上下文推断问题，要求模型检测是否存在用户需求并从多模态和对话上下文中推断意图，从五个维度评估涵盖单轮和多轮交互的能力。构建ODU-Bench基准，采用挑战驱动的分类法、分类法引导的智能体视频生成和人工记录的交互，并进行媒体锚定的注释和人工验证。

Result: 评估14个原生多模态大语言模型后发现，即使是最强的Gemini 3.1 Pro，也只能恢复44.7%必须从视觉、声学或对话上下文中推断的关键信息。此外，14个模型中有11个在非需求场景上的误触发率超过50%。

Conclusion: 结果揭示了当前多模态大语言模型在推断上下文用户需求方面存在系统性能力差距。希望ODU能够建立对多模态交互中一个先前未被充分探索但至关重要的能力的评估，即在生成适当响应之前正确理解用户需求。
...
### [22] [Benchmarking World Models for Continual Learning on Compositional Tasks](http://arxiv.org/abs/2609.22055v1)
*Haoyu Zhou,Joe Watson,Anson Lei,Ingmar Posner*

Main category: cs.LG

TL;DR: 本文提出了一个用于机器人操作世界模型的可组合持续学习基准，旨在隔离并评估模型的知识复用能力。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在评估适应性时，将学习新任务的能力与复用已有知识的能力混淆在一起，难以有效隔离和衡量知识复用。

Method: 提出一个针对机器人操作世界模型的可组合持续学习基准，设计结合已见任务特征的可组合任务课程，并将组合沿动作和感知轴进行分解；同时评估了现有最先进世界模型及一个具有显式可复用组件的模块化世界模型。

Result: 实验结果表明，与传统方法相比，模块化设计能更好地平衡知识复用与遗忘问题，但目前没有任何方法能完全解决该问题。

Conclusion: 模块化机制在持续学习中具有平衡复用与遗忘的优势，但仍需开发能够“无遗忘复用”的持续世界模型，为未来研究指明了方向。
...
TL;DR: 该论文对扩散大语言模型的三种主要范式（掩码、均匀、高斯扩散）在并行生成文本能力方面进行了细粒度比较，并证明了它们在并行性上的首个可证明分离。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型以并行文本生成能力为卖点，但目前对掩码、均匀和高斯扩散这三种主要范式在并行性方面的能力对比缺乏原理性的理解。

Method: 从理论上对比分析这三种范式的并行性，证明均匀和高斯扩散在采样前向传播次数上与底层分布的对偶全相关性相关，并针对特定随机经验测度家族分析所需的前向传播次数。

Result: 证明了均匀和高斯扩散可以使用与对偶全相关性成比例的前向传播次数进行采样；对于特定的随机经验测度，均匀和高斯扩散所需的前向传播次数为O~(sqrt(d))，而掩码扩散在特定近似分数预言机下需要O~(d)次，从而确立了并行性上的首个可证明分离。

Conclusion: 掩码扩散在并行性上的劣势并非因为需要提前确定词元值，而是因为其采样过程中的临界窗口在渐近意义上比均匀和高斯扩散更窄。
...
TL;DR: 本文提出了上下文分片块并行（CSBP）技术，通过将损坏块计算分配到不同秩并分片共享干净序列，有效解决了块扩散语言模型长上下文训练中的通信和内存瓶颈。


<details>
  <summary>Details</summary>
Motivation: 块扩散语言模型在长上下文训练时受限于分布式注意力通信和激活内存。传统的上下文并行方法在分片组合序列时，会产生通信开销和内存重复占用的问题。

Method: 本文引入块并行（BP），将每个损坏块计算分配至单一秩；并进一步提出上下文分片块并行（CSBP），将共享的干净序列在秩间分片。该方法使损坏的K/V和梯度保持在本地，避免了重复的干净前缀，并保留了模型原有的训练语义。

Result: 在16张H200 GPU上，256K上下文时吞吐量较最佳基线提升1.18-1.45倍，且峰值显存持平或降低；512K时全模型加速达1.61倍。在8张H100 GPU上，512K和1M上下文下加速比分别为2.48倍和7.59倍。在12小时的SFT训练中，CSBP在各检查点上均于SWE-bench Verified和Terminal-Bench Lite取得更高通过率。

Conclusion: CSBP成功克服了块扩散语言模型长上下文训练的扩展性瓶颈，在大幅提升训练吞吐量与降低内存开销的同时，进一步改善了下游任务的模型性能。
...
TL;DR: 本文提出时间自蒸馏（TSD）方法，通过跨时间步蒸馏模型自身的去噪分布，加速扩散语言模型的并行解码。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在过度激进的并行解码下会出现严重的性能退化，本文旨在解决该问题以实现更快且高质量的推理。

Method: 提出一种简单的在线策略方法——时间自蒸馏（TSD）。该方法将模型在早期时间步的去噪分布向最终确定令牌的时间步分布进行蒸馏，利用模型自身作为教师信号，无需离线教师生成，且适用于基础和后训练策略。

Result: 在涵盖数学、规划和代码的七个基准测试中，TSD显著将速度-质量权衡边界推向低计算区域，实现了与离线蒸馏相当的加速效果。

Conclusion: TSD作为一种简单、单阶段的方法，有效加速了扩散语言模型，避免了传统离线蒸馏复杂的两阶段流程。
...
### [26] [Value-Sensitive Delegation in Everyday AI Agent Use: Evidence from OpenClaw](http://arxiv.org/abs/2609.22067v1)
*Renkai Ma,Ruyuan Wan,Xuan Lu,Fan Yang,Chen Chen,Lingyao Li*

Main category: cs.HC

TL;DR: 本文分析了自主AI代理在任务委托中对用户价值观的满足情况，发现价值实现更多取决于运行条件而非输出结果。


<details>
  <summary>Details</summary>
Motivation: 现有评估通常仅衡量任务完成度而忽视用户重视的价值观，本文旨在探究人机委托中的价值实现问题。

Method: 采用价值敏感设计方法，利用大语言模型辅助分析了73,093篇关于OpenClaw的Reddit第一人称帖子，评估了人类价值、代理方面、价值实现和用户结果。

Result: 识别出6个价值组的21种价值。价值主要集中在用户设定的运行条件而非代理输出上；描述交付结果时价值多被满足，描述监督过程时价值多未被满足。

Conclusion: 提出“价值敏感委托”概念，指出支持人类价值不仅需关注代理任务完成度，更需关注委托时的运行条件如成本、访问和监督。
...
### [27] [Designer-RSI: Evolving Procedural Memory from User Traffic for Agentic Graphic Design](http://arxiv.org/abs/2609.22086v1)
*Hongyang Du,Lan Yan,Christian Flores,Asim Kadav*

Main category: cs.AI

TL;DR: 本文提出一种基于程序化记忆的持续适应框架，使冻结的前沿模型无需权重更新或人工标注即可通过经验积累不断改进专业图形设计能力。


<details>
  <summary>Details</summary>
Motivation: 专业图形设计是一个长周期的智能体任务，由大量相互依赖的动作产生结构化可编辑产物，但缺乏可靠的程序化验证机制（oracle），使得在噪声化、不可验证反馈下的持续适应成为核心难题。

Method: 冻结的前沿模型通过230+工具操作专业设计软件，外部程序化记忆以自然语言技能形式积累可复用设计流程。记忆通过两种机制增长：（1）拓宽——为反复出现的未覆盖子任务获取新技能；（2）深化——根据成功和失败执行修订已有技能。匹配重放门控确保仅接受修复失败且不回退已有成功的变更。

Result: 在5轮迭代中（1,406条真实用户需求，1,869条自动评分轨迹，无权重更新、无人工标注），技能库从76增长至139，Claude-Sonnet-4上GenEval2执行成功率从72.7%提升至99.3%（生成质量+11.99分），在四个专业设计基准上对无技能智能体的胜率分别达61.8%（Sonnet-4）和67.6%（Opus-4.6）。单独拓宽或深化的胜率为49.4%/48.6%，二者组合达58.5%（p=0.025）。

Conclusion: 程序化记忆为在噪声化、不可验证反馈条件下持续适应智能体提供了一条可行路径，拓宽与深化机制的组合优于任一单独使用。
...
### [28] [CodeMidas: Scaling Agentic Coding RL Environments from Code Itself](http://arxiv.org/abs/2609.22068v1)
...
TL;DR: 本文提出了CodeMidas，一个将现有代码库中的已实现功能转化为可执行强化学习环境的智能体流水线，仅以源代码作为任务特定输入。


<details>
  <summary>Details</summary>
Motivation: 通过强化学习训练编程智能体需要具有可靠验证器的多样化任务。现有方法通常依赖于问题和提交等开发工件，限制了可提取任务的范围。本文旨在更好地扩展强化学习环境。

Method: CodeMidas在环境构建的每个阶段分配智能体计算：智能体探索已实现功能以制定行为规范，构建基于原始代码执行的测试，并通过执行检查和重复解决方案 rollout 来验证和过滤候选任务。

Result: 生成的数据集包含来自3,185个开源代码库的5,545个训练任务，涵盖23种编程语言和15个技术领域。在这些任务上用GRPO训练MiMo-V2.5在所有五个多样化基准测试中均提升了性能，覆盖问题修复（DeepSWE +11.7%）、整程序构造（ProgramBench +17%）和终端工作（Terminal-Bench v2.1 +8.5%）。消融实验表明增加高质量训练任务数量能提升性能。轨迹分析显示经强化学习训练的智能体展现出更好的行为，如增加代码库探索和更多样化的自我验证。

Conclusion: 这些结果确立了源代码作为构建强化学习环境的可扩展基础，能够跨多样化软件任务提升编程智能体性能。
...
TL;DR: 本文提出TAPS方法，通过将扩散模型的边缘概率转化为路径条件的接受估计，在固定验证预算下选择紧凑的前缀闭子树，解决了推测解码中草稿树验证的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 在推测解码中使用扩散模型进行并行草拟虽能降低延迟，但将瓶颈转移到了验证阶段。现有方法基于边缘概率排序节点，忽略了验证的前缀条件性，导致验证被拒绝前缀的不可达后代，增加了延迟且接受度提升有限。

Method: 提出TAPS（目标感知前缀选择）方法，将扩散边缘概率转化为路径条件接受估计，并在固定验证预算下选择紧凑的前缀闭子树，以改善接受与成本的权衡。

Result: 在多种数据集和模型上的实验表明，TAPS比原始自回归解码实现了高达7.9倍的无损端到端加速，分别比最先进的DFlash和DDTree高出1.36倍和1.74倍。

Conclusion: TAPS有效解决了基于扩散的推测解码中草稿树方法的失配问题，提供了显著的加速和更好的接受成本权衡。
...
### [30] [PolyBridgeBench: Benchmarking Multimodal LLMs for Physics-Grounded Bridge Design](http://arxiv.org/abs/2609.21493v1)
*Zicheng Zhao,Dongyin Chen,Rui Xu,Yinghui Xu*

Main category: cs.AI

TL;DR: 本文提出了PolyBridgeBench，一个用于评估多模态大语言模型生成、执行和修复桥梁设计能力的可执行基准。


<details>
  <summary>Details</summary>
Motivation: 现有的基准无法评估MLLMs是否能合成完整的承重结构并在模拟器执行失败后进行修复，因为它们仅评估空间推理、结构有效性或基于物理的构建。

Method: 提出PolyBridgeBench基准，模型接收视觉场景和结构工程约束以生成节点-构件-材料拓扑结构；通过确定性合法性检查在原生动态物理模拟中执行；若失败，返回失败过程的时序视觉证据，并在固定交互预算下评估修复能力；分别测量确定性有效性、动态功能成功率和故障后恢复率。

Result: 对6个代表性MLLMs在189个关卡上的实验表明，确定性有效性和动态成功之间存在巨大差距，模型对材料预算高度敏感，且在严格预算设定下故障后恢复能力有限。

Conclusion: 该基准能够准确定位设计失败的具体阶段（合法性、动态功能或修复），揭示了当前MLLMs在动态物理执行和错误恢复方面的严重局限性。
...
### [31] [APort Vault: Benchmarking AI Agent Payment Authorization with the Open Agent Passport](http://arxiv.org/abs/2609.22076v1)
*Uchi Uchibeke*

Main category: cs.CR

TL;DR: APort Vault基准测试通过重放人类攻击，评估了AI代理支付授权安全性及Open Agent Passport预操作检查的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决使用工具的AI代理在处理支付授权时面临的安全风险，验证其在对抗人类编写攻击时的防御能力。

Method: 提出APort Vault基准，重放4,371次人类攻击，对14个模型在不同配置下进行225,964次评估，对比有无Open Agent Passport (OAP)确定性预操作检查的效果。

Result: 无预操作检查时存在140次违规转账，有检查时降为0（零结果涵盖790个会话）。该层并未拒绝正常支付，25,370次支付成功执行，仅187次被策略拒绝。

Conclusion: 实施OAP的确定性预操作检查能有效且无副作用地阻止违规支付。研究已开源评估数据和代码。
...
### [32] [Fingerprinting Multimodal Large Language Models](http://arxiv.org/abs/2609.20457v1)
*Chao Huang,Meng Tong,Kejiang Chen*

Main category: cs.CR

TL;DR: 本文提出了首个多模态模型指纹识别方法AttnPrint和DistillTrace，以保护模型所有权并检测未经授权的蒸馏行为。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）易受非法部署和未授权蒸馏的威胁，而现有模型溯源方案因共享语言主干而失效，且难以检测蒸馏违规。

Method: 提出白盒溯源方法AttnPrint，提取跨模态注意力分布的低频分量作为指纹；并提出黑盒审计方法DistillTrace，利用对MLLM输出的假设检验来识别潜在侵权。

Result: 在19种多模态架构的154个模型实例上的实验表明，AttnPrint在派生模型检测上表现优异且对五种下游修改技术具有鲁棒性；DistillTrace在三种参数无关技术下成功提供了蒸馏关系证据。

Conclusion: 提出的AttnPrint和DistillTrace方法有效填补了多模态模型指纹识别的空白，能够有效保障模型所有权并追溯蒸馏关系。
...
### [33] [AVT-Fabric: Active Visuo-Tactile Perception via Adaptive Evidence Selection for Efficient Robotic Fabric Comparison](http://arxiv.org/abs/2609.21377v1)
*Chang Gao,Zhuo Chen,Suhang Xia,Jihong Zhu,Jiankang Deng,Shan Luo*

Main category: cs.RO

TL;DR: 本文提出AVT-Fabric框架，通过自适应选择触觉证据，利用紧凑的7B模型在机器人织物比较中实现了高精度与高效率。


<details>
  <summary>Details</summary>
Motivation: 解决机器人织物比较中视觉和触觉线索难以高效结合，以及现有大模型推理延迟高、计算开销大的问题。

Method: 提出以RGB为先的AVT-Fabric框架，利用双尺度门评估置信度以决定是否引入额外的力标记GelSight触觉观测，并结合紧凑文本记忆和多数投票机制整合预测结果。

Result: 使用7B多模态大模型在400次对比中达到98.0%准确率，超越90B基线4.0个百分点，平均仅处理1.60个阶段，模型侧延迟降低61.8%；在真实机器人系统中达到78.1%的成对排序准确率，8个场景中7个正确选择织物。

Conclusion: 自适应证据选择能够同时显著提升机器人视觉-触觉推理的准确率和效率，且该框架在多种大模型骨干上具备良好的泛化能力。
...
### [34] [Bayesian Belief Layer for Controllable Opinion Dynamics in LLM Agents](http://arxiv.org/abs/2609.21997v1)
*Hafsa Akbar,Daniel Platnick,Marjan Alirezaie,Hossein Rahnama*

Main category: cs.MA

TL;DR: 提出了一种贝叶斯编年史代理（BCA）信念层，将LLM代理的信念与表达分离，实现可控且可审计的社会意见动态模拟。


<details>
  <summary>Details</summary>
Motivation: 当前社会模拟中的LLM代理隐式地在上下文中修改意见，导致其对说服的开放程度无法指定或验证，且集体结果会继承模型的训练先验。

Method: 引入贝叶斯编年史代理（BCA），构建一个极简信念层将代理信念与言语表达分离；每次接收的话语通过单步贝叶斯更新概率化立场；采用单一先验强度参数κ模拟固执程度，基于Friedkin-Johnsen意见动力学模型。

Result: 调整参数κ可按需产生共识、持续分歧和坚定少数派影响三种经典意见动态模式；持续分歧与FJ闭式不动点的匹配度达R²=0.93至0.99；在语言往返后能完美恢复所有四个模型中κ的秩阶；显式信念层暴露了端到端模拟会吸收的系统性模型立场偏差。

Conclusion: 显式信念层（BCA）使LLM社会模拟变得可审计且可控，成功克服了隐式上下文意见修改和模型训练先验带来的局限。
...
### [35] [Auditing bipartite motif interpretations: a worked example with conservation checks and open-path decomposition](http://arxiv.org/abs/2609.22014v1)
*Tengfei Shao*

Main category: cs.SI

TL;DR: 本文指出二分网络中的fan类模体计数完全由度序列决定，并提出四步预解释检查法以避免对网络结构的误读。


<details>
  <summary>Details</summary>
Motivation: 现有研究在解释二分网络模体轮廓时，常忽略度序列已固定的信息，可能导致对结构角色和网络差异的错误推断。

Method: 基于二分图中k-fan计数在度序列保留的零假设下方差为零的数学原理，对重构的旅游网络和已发布的奢侈品网络数据进行分析，并提出了四步预解释检查法与参考实现。

Result: 发现四种fan类计数是度序列的精确函数；在旅游网络中开放路径模体显著缺失，但该缺失的类别级解释不稳定且无法归因；此外，奢侈品网络的发布数据存在边数不一致的问题。

Conclusion: 强调在网络模体轮廓推断前需进行严格的度序列依赖性检查，提供的检查步骤和实现代码为未来相关研究提供了规范。
